...
uses
  ...
  A000UInterfaccia, A000USessione, A000UCostanti, C004UParamForm,
  C180FunzioniGenerali, C190FunzioniGeneraliWeb, R010UPaginaWeb,
  USelT004, W022USchedaValutazioniDtM,
...
type
  TW022FDettaglioValutazioni = class(TR012FWebAnagrafico)
    ...
    dmemStoriaCommentiValutato: TmeIWMemo;
    lblDataAccettazioneValutato: TmeIWLabel;
    lblTestata: TmeIWLabel;
    ...
    procedure chkLegendaPunteggiAsyncClick(Sender: TObject; EventParams: TStringList);
  ...
  private
    RigaInserita: String;
    ModificaDettaglio,InserisciDettaglio,ModificaProtocollo: Boolean;
    ...
    procedure AggiornaDescrizionePunteggio(Sender: TObject);
    ...
    function RecuperaScaglioniIncentivi: String;
    procedure EsecuzioneStampa;
    procedure VisualizzaStampa;
  ...
  public
    ...
    ModificaTestata,NoteValidaRichiesto:Boolean;
...
begin
  ...
  if not WR000DM.SoloStampa then
  begin
    C004DM:=CreaC004(SessioneOracle,'W022',Parametri.Operatore,False);
    chkLegendaPunteggi.Checked:=C004DM.GetParametro('chkLegendaPunteggi','N') = 'S';
  end;
end;
...
begin
  ...
  dgrdDettaglio.medpAggiungiColonna('CF_DESC_PUNTEGGIO','Desc. punt. elem.','',nil);
  ...
  (dgrdDettaglio.Columns.Items[11] as TIWDBGridColumn).Visible:=R180InConcat('7',W022DtM2.selSG741.FieldByName('CAMPI_OPZIONALI_COMPILAZIONE').AsString);
  (dgrdDettaglio.Columns.Items[11] as TIWDBGridColumn).Width:='300';
  (dgrdDettaglio.Columns.Items[11] as TIWDBGridColumn).Wrap:=True;
  (dgrdDettaglio.Columns.Items[12] as TIWDBGridColumn).Visible:=W022DtM2.VisColNotePunteggio;
  (dgrdDettaglio.Columns.Items[13] as TIWDBGridColumn).Visible:=R180InConcat('4',W022DtM2.selSG741.FieldByName('CAMPI_OPZIONALI_COMPILAZIONE').AsString);
  ...
  dgrdDettaglio.medpPreparaComponenteGenerico('R',dgrdDettaglio.medpIndexColonna('CF_DESC_PUNTEGGIO'),0,DBG_LBL,'','','','','');
...
begin
  ...
  begin
    ...
    begin
      ...
      insSG710.SetVariable('COMMENTI_VALUTATO_DIP',selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString);
      insSG710.Execute;
...
begin
  ...
  begin
    ...
    else if dgrdDettaglio.medpNumColonna(AColumn) = dgrdDettaglio.medpIndexColonna('CF_DESC_PUNTEGGIO') then
      ACell.Text:=W022DtM2.RecuperaEtichetta('DESCRIZIONE_PUNTEGGIO_ITEM_C')
  ...
  if R180In(dgrdDettaglio.medpNumColonna(AColumn),[dgrdDettaglio.medpIndexColonna('CF_PUNTEGGIO_AREA'),dgrdDettaglio.medpIndexColonna('CF_PUNTEGGIO_PESATO')]) then
    ACell.Css:=IfThen(W022DtM2.selSG730.FieldByName('CALCOLO_PFP').AsString = 'S',ACell.Css,'invisibile');

  if (ARow <> 0)
  and R180In(dgrdDettaglio.medpNumColonna(AColumn),[dgrdDettaglio.medpIndexColonna('COD_VALUTAZIONE'),dgrdDettaglio.medpIndexColonna('D_VALUTAZIONE')])
  and (dgrdDettaglio.medpValoreColonna(ARow - 1,'VALUTAZIONE_ORIGINALE') = 'N') then
  ...
  if (ARow > 0) and not R180In(dgrdDettaglio.medpNumColonna(AColumn),[dgrdDettaglio.medpIndexColonna('D_AREA'),dgrdDettaglio.medpIndexColonna('D_VALUTAZIONE'),dgrdDettaglio.medpIndexColonna('CF_DESC_PUNTEGGIO')]) then
...
begin
  ...
  begin
    ...
    c:=dgrdDettaglio.medpIndexColonna('CF_DESC_PUNTEGGIO');
    (dgrdDettaglio.medpCompCella(i,c,0) as TmeIWLabel).Text:=dgrdDettaglio.medpValoreColonna(i,'CF_DESC_PUNTEGGIO');
    // note punteggio
    c:=dgrdDettaglio.medpIndexColonna('NOTE_PUNTEGGIO');
    if StileNote = '' then
          ...
          begin
            ...
            begin
              if W022DtM2.selSG730.FieldByName('ITEM_GIUDICABILE').AsString = 'S' then
                AddRow(Format('%s;%s',[W022DtM2.selSG730.FieldByName('CODICE').AsString,W022DtM2.selSG730.FieldByName('DESCRIZIONE').AsString]));//se mai sarà necessario, usare RecuperaDescrizionePunteggio
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              if W022DtM2.selSG730.FieldByName('ITEM_GIUDICABILE').AsString = 'S' then
                AddRow(Format('%s;%s',[W022DtM2.selSG730.FieldByName('CODICE').AsString,
                                       W022DtM2.RecuperaDescrizionePunteggio(dgrdDettaglio.medpValoreColonna(i,'COD_AREA'),
                                                                            dgrdDettaglio.medpValoreColonna(i,'COD_VALUTAZIONE'),
                                                                            W022DtM2.selSG730.FieldByName('CODICE').AsString)]));
              W022DtM2.selSG730.Next;
            end;
...
begin
  if W022DtM2.selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString = 'N' then
  begin
    chkAccettazioneValutatoSi.Checked:=False;
    chkAccettazioneValutatoNo.Checked:=True;
  end
  else if chkAccettazioneValutatoSi.Checked and chkAccettazioneValutatoNo.Checked then
      chkAccettazioneValutatoSi.Checked:=False;
  W022DtM2.Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString:=IfThen(chkAccettazioneValutatoSi.Checked,'S',IfThen(chkAccettazioneValutatoNo.Checked,'N',''));
  W022DtM2.ImpostaDataAccettazioneValutato;
  AbilitazioneControlli;
end;
...
begin
  ...
  begin
    ...
    FreeAndNil(dgrdDettaglio.medpCompGriglia[i].CompColonne[12]);
  end;
end;
...
begin
  AggiornaDescrizionePunteggio(Sender);
  VisualizzaNote(Sender);
end;

procedure TW022FDettaglioValutazioni.AggiornaDescrizionePunteggio(Sender: TObject);
var
  i,c: Integer;
  CodPunteggio:String;
begin
  ...
  for i:=0 to High(dgrdDettaglio.medpCompGriglia) do
    if dgrdDettaglio.medpCompGriglia[i].CompColonne[c] <> nil then
      with (dgrdDettaglio.medpCompCella(i,c,0) as TmeIWLabel) do
        if FriendlyName = (Sender as TmedpIWMultiColumnComboBox).FriendlyName then
        begin
          CodPunteggio:=IfThen((Sender as TmedpIWMultiColumnComboBox).Css = 'invisibile',
                                 IfThen((dgrdDettaglio.medpCompCella(i,dgrdDettaglio.medpIndexColonna('GIUDICABILE'),0) <> nil)
                                         and not (dgrdDettaglio.medpCompCella(i,dgrdDettaglio.medpIndexColonna('GIUDICABILE'),0) as TmeIWCheckBox).Checked,
                                        VarToStr(W022DtM2.selSG730.Lookup('ITEM_GIUDICABILE','N','CODICE')),''),
                               (Sender as TmedpIWMultiColumnComboBox).Text);
          Text:=W022DtM2.RecuperaDescrizionePunteggio(dgrdDettaglio.medpValoreColonna(i,'COD_AREA'),
                                                      dgrdDettaglio.medpValoreColonna(i,'COD_VALUTAZIONE'),
                                                      CodPunteggio);
  ...
  for i:=0 to High(dgrdDettaglio.medpCompGriglia) do
    if dgrdDettaglio.medpCompGriglia[i].CompColonne[c] <> nil then
      with (dgrdDettaglio.medpCompCella(i,c,0) as TmeIWImageFile) do
        if FriendlyName = (Sender as TmedpIWMultiColumnComboBox).FriendlyName then
        begin
          Css:=IfThen(VarToStr(W022DtM2.selSG730.Lookup('CODICE',(Sender as TmedpIWMultiColumnComboBox).Text,'GIUSTIFICA')) = 'S',StileNote,'invisibile');
          DoRefreshControl:=True;
          Break;
        end;
end;
      ...
      begin
        with (dgrdDettaglio.medpCompCella(i,c,0) as TmedpIWMultiColumnComboBox) do
          ...
          begin
            ...
            DoRefreshControl:=True;
            AggiornaDescrizionePunteggio(dgrdDettaglio.medpCompCella(i,c,0) as TmedpIWMultiColumnComboBox);
...
begin
  ...
  begin
    ...
    cdsNoteItem.FieldByName('D_PUNTEGGIO').AsString:=RecuperaDescrizionePunteggio(Q711.FieldByName('COD_AREA').AsString,Q711.FieldByName('COD_VALUTAZIONE').AsString,CodPunteggio);
...
begin
  ...
  lblAccettazioneValutato.Visible:=(W022DtM2.selSG741.FieldByName('ABILITA_ACCETTAZIONE_VALUTATO').AsString = 'S') and (W022DtM2.ModificaDelValutato or (W022DtM2.selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString <> 'S') or (Parametri.S710_SupervisoreValut = 'S'));
  ...
  lblDataAccettazioneValutato.Text:=IfThen(W022DtM2.Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime > 0,'(' + FormatDateTime('dd/mm/yyyy hh:nn',W022DtM2.Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime) + ')');
  lblDataAccettazioneValutato.Visible:=lblDataAccettazioneValutato.Text <> '';
  ...
  dmemCommentiValutato.Visible:=((W022DtM2.selSG741.FieldByName('ABILITA_COMMENTI_VALUTATO').AsString = '1') or ((W022DtM2.selSG741.FieldByName('ABILITA_COMMENTI_VALUTATO').AsString = '3') and (W022DtM2.Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'S')) or ((W022DtM2.selSG741.FieldByName('ABILITA_COMMENTI_VALUTATO').AsString = '4') and (W022DtM2.Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'N')))
                                and (W022DtM2.ModificaDelValutato or (W022DtM2.selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString <> 'S') or (Parametri.S710_SupervisoreValut = 'S'));
  dmemCommentiValutato.Editable:=dmemObiettiviPianificati.Editable;
  dmemCommentiValutato.Css:=dmemObiettiviPianificati.Css;
  dmemStoriaCommentiValutato.Visible:=(W022DtM2.selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString = 'S') and not W022DtM2.Q710.FieldByName('STORIA_COMMENTI_VALUTATO').IsNull;
  lblCommentiValutato.Visible:=dmemCommentiValutato.Visible or dmemStoriaCommentiValutato.Visible;
  // Pagina Note
...
begin
  ...
  dmemStoriaCommentiValutato.Text:=W022DtM2.Q710.FieldByName('STORIA_COMMENTI_VALUTATO').AsString;
  dmemNote.Text:=W022DtM2.Q710.FieldByName('NOTE').AsString;
end;

procedure TW022FDettaglioValutazioni.chkLegendaPunteggiAsyncClick(Sender: TObject; EventParams: TStringList);
begin
  inherited;
  if not WR000DM.SoloStampa then
  begin
...
begin
  ...
  begin
    ...
    FieldDefs.Add('Scaglioni_Incentivi',ftString,500,False);
    FieldDefs.Add('Presa_Visione',ftString,500,False);
    FieldDefs.Add('Accettazione_Valutato',ftString,2,False);
    FieldDefs.Add('Data_Accettazione_Valutato',ftString,20,False);
...
var
  ...
  SchedaProtocollata,StampaPresaVisioneValutato: Boolean;
  Nominativo: String;
...
begin
  ...
  begin
    ...
    // Scaglioni per incentivi
    cdsRiepilogo.FieldByName('SCAGLIONI_INCENTIVI').AsString:=RecuperaScaglioniIncentivi;
    // Presa visione da parte del valutato
    StampaPresaVisioneValutato:=R180InConcat('6',selSG741.FieldByName('CAMPI_OPZIONALI_STAMPA').AsString);
    DataConsegna:=0;
    if StampaPresaVisioneValutato then
      with selSG745 do
      begin
        Close;
        SetVariable('DATA',Q710.FieldByName('DATA').AsDateTime);
        ...
        SetVariable('STATO_AVANZAMENTO',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger);
        Open;
        Filter:='(TIPO_CONSEGNA = ''PV'') AND (ESITO = ''S'') AND (PROG_UTENTE = PROGRESSIVO)';
        Filtered:=True;
        First;
        ...
        begin
          ...
          cdsRiepilogo.FieldByName('PRESA_VISIONE').AsString:=Format('%s %s',[FormatDateTime('dd/mm/yyyy hh:nn',FieldByName('DATA_CONSEGNA').AsDateTime),Nominativo]);
        end
        else if (Q710.FieldByName('PROGRESSIVO').AsInteger = Parametri.ProgressivoOper) and PresaVisioneRegistrabile then
        begin
          //Se ci sono le condizioni per salvare la presa visione del valutato, prelevare l'orario attuale da visualizzare nella stampa e da registrare mantenendo l'attuale flusso.
          DataConsegna:=R180Sysdate(SessioneOracle);
          R180SetVariable(selT030,'PROGRESSIVO',Parametri.ProgressivoOper);
          selT030.Open;
          Nominativo:=selT030.FieldByName('NOMINATIVO').AsString;
          cdsRiepilogo.FieldByName('PRESA_VISIONE').AsString:=Format('%s %s',[FormatDateTime('dd/mm/yyyy hh:nn',DataConsegna),Nominativo]);
        end;
        Filtered:=False;
      end;
    cdsRiepilogo.FieldByName('ACCETTAZIONE_VALUTATO').AsString:=IfThen(Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'N','NO',
                                                                IfThen(Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'S','SI',''));
    cdsRiepilogo.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsString:=IfThen(Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime > 0,'(' + FormatDateTime('dd/mm/yyyy hh:nn',Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime) + ')');
    if Fase1StampaDirigenzaConObiettivi or Fase2StampaDirigenzaConObiettivi then
...
var lstLegendaPunteggi: TStringList;
begin
  Result:='';
...
function TW022FDettaglioValutazioni.RecuperaScaglioniIncentivi: String;
var lstScaglioniIncentivi: TStringList;
    StampaScaglioniIncentivi:Boolean;
    LungDescPFP:Integer;
begin
  Result:='';
  with W022DtM2 do
  begin
    StampaScaglioniIncentivi:=not selSG741.FieldByName('CODTIPOQUOTA_STAMPA').IsNull
                              and (Q710.FieldByName('CHIUSO').AsString = 'S')
                              and (Q710.FieldByName('VALUTABILE').AsString = 'S')
                              and (cdsRiepilogo.FieldByName('CALCOLO_PFP').AsString = 'S');
    if not StampaScaglioniIncentivi then Exit;

    lstScaglioniIncentivi:=TStringList.Create;
    LungDescPFP:=Max(18,Length(RecuperaEtichetta('PUNTEGGIO_FINALE_SCHEDA_S')));//18 sono i caratteri prodotti da: 'da %6.6s a %6.6s'
    //Elenco scaglioni incentivi
    R180SetVariable(selSG735,'QUOTA',selSG741.FieldByName('CODTIPOQUOTA_STAMPA').AsString);
    R180SetVariable(selSG735,'DATA_VAL',Q710.FieldByName('DATA').AsDateTime);
    with selSG735 do
    begin
      Open;
      First;
      if RecordCount > 0 then
        lstScaglioniIncentivi.Add(Format('%-' + IntToStr(LungDescPFP) + 's   %s',[RecuperaEtichetta('PUNTEGGIO_FINALE_SCHEDA_S'),'Premialità']));
      while not Eof do
      begin
        if (Q710.FieldByName('PUNTEGGIO_FINALE_PESATO').AsFloat >= selSG735.FieldByName('PUNTEGGIO_DA').AsFloat)
        and (Q710.FieldByName('PUNTEGGIO_FINALE_PESATO').AsFloat <= selSG735.FieldByName('PUNTEGGIO_A').AsFloat) then
          lstScaglioniIncentivi.Add(Format('da %6.6s a %6.6s%' + IntToStr(Max(1,LungDescPFP - 18 + 1)) + 's= %6.6s %% %s',[FormatFloat('##0.00',selSG735.FieldByName('PUNTEGGIO_DA').AsFloat),FormatFloat('##0.00',selSG735.FieldByName('PUNTEGGIO_A').AsFloat),' ',FormatFloat('##0.00',selSG735.FieldByName('PERC').AsFloat),' <-- Livello raggiunto']))
        else
          lstScaglioniIncentivi.Add(Format('da %6.6s a %6.6s%' + IntToStr(Max(1,LungDescPFP - 18 + 1)) + 's= %6.6s %% %s',[FormatFloat('##0.00',selSG735.FieldByName('PUNTEGGIO_DA').AsFloat),FormatFloat('##0.00',selSG735.FieldByName('PUNTEGGIO_A').AsFloat),' ',FormatFloat('##0.00',selSG735.FieldByName('PERC').AsFloat),'']));
        Next;
      end;
    end;
  end;
  Result:=lstScaglioniIncentivi.Text;
  FreeAndNil(lstScaglioniIncentivi);
end;
...
begin
  try
    ...
    try
      ...
      // Impostazione degli scaglioni finalizzati al calcolo degli incentivi
      rvComp :=rvProject.ProjMan.FindRaveComponent('bndScaglioniIncentivi',rvPage);
      (rvComp as TRaveContainerControl).Visible:=Trim(cdsRiepilogo.FieldByName('SCAGLIONI_INCENTIVI').AsString) <> '';
      rvComp:=rvProject.ProjMan.FindRaveComponent('qlblTitoloScaglioniIncentivi',rvPage);
      (rvComp as TRaveText).Text:=W022DtM2.RecuperaEtichetta('SCAGLIONI_INCENTIVI_S','U');
      // Impostazione della presa visione del valutato
      rvComp :=rvProject.ProjMan.FindRaveComponent('bndPresaVisione',rvPage);
      (rvComp as TRaveContainerControl).Visible:=Trim(cdsRiepilogo.FieldByName('PRESA_VISIONE').AsString) <> '';
...
procedure TW022FDettaglioValutazioni.VisualizzaStampa;
begin
