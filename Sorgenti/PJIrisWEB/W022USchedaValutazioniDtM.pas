...
uses
  ...
  A000USessione, A000UInterfaccia, C017UEMailDtM, QueryStorico, USelI010, Math;
...
type
  ...
  TW022FSchedaValutazioniDtM = class(TDataModule)
    ...
    selStoriaCommVal: TOracleQuery;
    Q710STORIA_COMMENTI_VALUTATO: TStringField;
    updSG710b: TOracleQuery;
    Q711CF_DESC_PUNTEGGIO: TStringField;
    Q710DATA_ACCETTAZIONE_VALUTATO: TDateTimeField;
  ...
  public
    ...
    SoloStampa2,ModificaDelValutato: Boolean;
    Azione,Accesso: String;
    TotPerc: Real;
    DataRif,DataConsegna: TDateTime;
    ScalaPnt: String;
    selI010:TselI010;
    ...
    procedure ImpostaDataAccettazioneValutato;
    ...
    function RecuperaDescrizionePunteggio(CodArea,CodItem,CodPunteggio:String):String;
    ...
    function PresaVisioneRegistrabile:Boolean;
    procedure InviaEMail(Operazione:String;StatoPartenza:Integer);
    procedure InviaEMailErrProt;
  end;
...
begin
  ...
  //Stato della scheda
  Q710.FieldByName('STATO_SCHEDA').AsString:=IfThen(selSG741.FieldByName('CODICE').IsNull or selSG746.FieldByName('CODICE').IsNull,'Non valutabile: Regole non trovate',
  ...
  selStoriaCommVal.SetVariable('DATA',Q710.FieldByName('DATA').AsDateTime);
  selStoriaCommVal.SetVariable('PROGRESSIVO',Q710.FieldByName('PROGRESSIVO').AsInteger);
  selStoriaCommVal.SetVariable('TIPO',Q710.FieldByName('TIPO_VALUTAZIONE').AsString);
  selStoriaCommVal.SetVariable('STATO',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger + IfThen((not ModificaDelValutato) and (Parametri.S710_SupervisoreValut = 'N'),1));
  selStoriaCommVal.Execute;
  Q710.FieldByName('STORIA_COMMENTI_VALUTATO').AsString:=VarToStr(selStoriaCommVal.Field(0));
  MessNoValida:='';
...
begin
  ...
  begin
    ...
    Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsString:='';
  ...
  PeriodoValutazioneCambiato:=
    (Q710.State = dsEdit) and
    ((Q710.FieldByName('DAL').AsDateTime <> Q710.FieldByName('DAL').OldValue) or
    (Q710.FieldByName('AL').AsDateTime <> Q710.FieldByName('AL').OldValue));
end;

procedure TW022FSchedaValutazioniDtM.ImpostaDataAccettazioneValutato;
begin
  //Registro la data di accettazione della scheda se previsto dalle regole
  if (Q710.State = dsEdit) and (selSG741.FieldByName('REGISTRA_DATA_ACCETTAZIONE_VAL').AsString = 'S') then
  begin
    if Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = '' then
      Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsString:=''
    else if (Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = VarToStr(Q710.FieldByName('ACCETTAZIONE_VALUTATO').OldValue))
         and (VarToStr(Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').OldValue) <> '') then
      Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime:=Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').OldValue
    else if Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString <> VarToStr(Q710.FieldByName('ACCETTAZIONE_VALUTATO').OldValue) then
      Q710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime:=R180SysDate(SessioneOracle);
  end;
end;
...
begin
  ...
  and (ModificaDelValutato or (selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString <> 'S') or (Parametri.S710_SupervisoreValut = 'S'))
...
begin
  ...
  if Azione = 'I' then
  begin
    if selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString = 'S' then //selSG741 impostato nel BeforePostNoStorico
    begin
      //Svuoto il campo ACCETTAZIONE_VALUTATO per lasciarlo gestire al dipendente
      updSG710b.SetVariable('PROGRESSIVO',Q710.FieldByName('PROGRESSIVO').AsInteger);
      updSG710b.SetVariable('DATA',Q710.FieldByName('DATA').AsDateTime);
      updSG710b.SetVariable('TIPO_VAL',Q710.FieldByName('TIPO_VALUTAZIONE').AsString);
      updSG710b.SetVariable('STATO_AVANZAMENTO',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger);
      updSG710b.Execute;
      SessioneOracle.Commit;
      Q710.RefreshRecord;
    end;
...
begin
  ...
  Q711.FieldByName('CF_DESC_PUNTEGGIO').AsString:=IfThen((VarToStr(selSG730.Lookup('CODICE',Q711.FieldByName('COD_PUNTEGGIO').AsString,'ITEM_GIUDICABILE')) = 'N') or (ProprietaArea(Q711.FieldByName('COD_AREA').AsString,DataRif,'PUNTEGGI_CON_PERCENTUALE') = 'N'),RecuperaDescrizionePunteggio(Q711.FieldByName('COD_AREA').AsString,Q711.FieldByName('COD_VALUTAZIONE').AsString,Q711.FieldByName('D_PUNTEGGIO').AsString));
...
begin
  ...
  //Regole non trovate
  if selSG741.FieldByName('CODICE').IsNull or selSG746.FieldByName('CODICE').IsNull then
  begin
...
function TW022FSchedaValutazioniDtM.RecuperaDescrizionePunteggio(CodArea,CodItem,CodPunteggio:String):String;
var DescrizioniPunteggi:String;
    PosIni,PosFin:Integer;
begin
  Result:=VarToStr(selSG730.Lookup('CODICE',CodPunteggio,'DESCRIZIONE'));
  R180SetVariable(selSG700,'COD_AREA',CodArea);
  R180SetVariable(selSG700,'DATA',DataRif);
  selSG700.Open;
  DescrizioniPunteggi:=VarToStr(selSG700.Lookup('COD_VALUTAZIONE',CodItem,'DESCRIZIONI_PUNTEGGI'));
  PosIni:=Pos('<' + CodPunteggio + '>',DescrizioniPunteggi);
  PosFin:=Pos('</' + CodPunteggio + '>',DescrizioniPunteggi);
  if (PosIni > 0) and (PosFin > 0) and (PosIni < PosFin) then
    Result:=Copy(DescrizioniPunteggi,PosIni + Length('<' + CodPunteggio + '>'),PosFin - (PosIni + Length('<' + CodPunteggio + '>')));
end;

function TW022FSchedaValutazioniDtM.SchedeCollegateAperte(var Msg:String): Boolean;
var svMatricola,svData:String;
begin
  Result:=False;
  Msg:='';
  svMatricola:='';
  ...
  begin
    ...
    while not Eof do
    begin
      if (FieldByName('MATRICOLA').AsString <> svMatricola) or (FieldByName('DATA').AsString <> svData) then
        Msg:=Msg + CRLF + Format('%s %s (%s) - %s',[FieldByName('COGNOME').AsString,FieldByName('NOME').AsString,FieldByName('MATRICOLA').AsString,
                                                    IfThen(FieldByName('DATA').AsDateTime <> EncodeDate(R180Anno(FieldByName('DATA').AsDateTime),12,31),FormatDateTime('dd/mm/yyyy',FieldByName('DATA').AsDateTime),IntToStr(R180Anno(FieldByName('DATA').AsDateTime)))]);
      ...
      svData:=FieldByName('DATA').AsString;
      Next;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    BM:=GetBookMark;
      ...
      TotPerc:=R180Arrotonda(TotPerc,0.0001,'P');
      TotVal:=R180Arrotonda(TotVal,0.01,'P');
      //Applico l'eventuale arrotondamento specificato nelle regole
      if selSG741.FieldByName('TIPO_ARROTONDAMENTO').AsString <> 'N' then
        TotVal:=R180Arrotonda(TotVal,1,selSG741.FieldByName('TIPO_ARROTONDAMENTO').AsString);
      GotoBookMark(BM);
	  finally
      FreeBookMark(BM);
	  end;
...
var
  OldProg:Integer;
  OldArea,MotivoNV,StatoScheda,DescDip:String;
  function GetStatoScheda:String;
  begin
    Result:='';
    RecuperaRegole(DataLavoro2,SelAnagrafeW022.FieldByName('PROGRESSIVO').AsInteger,Copy(selRiepilogoSchede.FieldByName('D').AsString,1,Pos('.',selRiepilogoSchede.FieldByName('D').AsString) - 1));
    Result:=IfThen(selRiepilogoSchede.FieldByName('G').AsString = 'N','Non valutabile: Impostato dall''ufficio',
            IfThen(selRiepilogoSchede.FieldByName('B').AsString = 'S','Scheda definitiva',
            VarToStr(selSG746.Lookup('CODICE',StrToIntDef(Copy(selRiepilogoSchede.FieldByName('D').AsString,Pos('.',selRiepilogoSchede.FieldByName('D').AsString) + 1),0),'DESCRIZIONE'))));
    Result:=Result + IfThen(not selRiepilogoSchede.FieldByName('E').IsNull,' (P.V.: ' + selRiepilogoSchede.FieldByName('E').AsString + ')');//Presa visione valutato
  end;
begin
  ...
  begin
    if Active then
    begin
      Filtered:=False;//Altrimenti crea problemi in fase di successivo caricamento
      EmptyDataSet;
    end
    else
      CreateDataSet;
    ...
    begin
      ...
      DescDip:=SelAnagrafeW022.FieldByName('COGNOME').AsString + ' ' + SelAnagrafeW022.FieldByName('NOME').AsString + ' (' + SelAnagrafeW022.FieldByName('MATRICOLA').AsString + ')';
      FieldByName('B').AsString:=DescDip;//Nominativo e matricola
      Post;
      OldArea:='';
      ...
      Edit;
      FieldByName('E').AsString:='N';
      FieldByName('F').AsString:='N';
      Post;
      ...
      begin
        ...
        FieldByName('C').AsString:=MotivoNV;//Stato scheda
        FieldByName('E').AsString:=IfThen(MotivoNV = 'Senza scheda', 'S', FieldByName('E').AsString); // Senza scheda
        FieldByName('F').AsString:=IfThen((MotivoNV <> '') and (MotivoNV <> 'Senza scheda'), 'S', FieldByName('F').AsString); // Non valutabile: XXX
        Post;
      end
      else
      begin
        StatoScheda:=GetStatoScheda;
        if selSG741.FieldByName('CODICE').IsNull then
        begin
          Edit;
          FieldByName('C').AsString:=StatoScheda;//Stato scheda
          FieldByName('D').AsString:='Punt: ' + 'N.V.';//Punteggio scheda
          Post;
        end
        else
        begin
          ...
          begin
            if selRiepilogoSchede.FieldByName('A').AsString = '1' then //Testata massimo stato raggiunto
            begin
              Edit;
              FieldByName('C').AsString:=StatoScheda;//Stato scheda
              FieldByName('D').AsString:='Punt: ' + selRiepilogoSchede.FieldByName('F').AsString;//Punteggio scheda
              Post;
            end
            else if selRiepilogoSchede.FieldByName('A').AsString = '2' then //Elenco di tutti gli stati
            begin
              StatoScheda:=GetStatoScheda;
              Append;
              FieldByName('A').AsString:='4';
              FieldByName('B').AsString:=DescDip;//Nominativo e matricola
              FieldByName('C').AsString:=StatoScheda;//Stato scheda
              FieldByName('D').AsString:='Punt: ' + selRiepilogoSchede.FieldByName('F').AsString;//Punteggio scheda
              Post;
            end
            else if selRiepilogoSchede.FieldByName('A').AsString = '3' then //Dettaglio massimo stato raggiunto
            begin
...
function TW022FSchedaValutazioniDtM.PresaVisioneRegistrabile:Boolean;
begin
  //Posso registrare la presa visione quando:
  Result:=
    //1) sono nella funzione Stampa scheda di valutazione
    SoloStampa2
    //2) il mio utente è configurato per la presa visione
    and R180In(Parametri.WEBRegistraConsegnaVal,['R','S'])
    //3) mi trovo nel periodo di registrazione della presa visione
    and (Trunc(R180SysDate(SessioneOracle)) >= selSG746.Lookup('CODICE',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger,'DATA_DA_RICHIESTA_VISIONE'))
    ...
    //4) non ho già registrato positivamente (esito = S) la presa visione
    and not selSG745.SearchRecord('TIPO_CONSEGNA;UTENTE;PROG_UTENTE;ESITO',
                                          VarArrayOf(['PV',
                                                      Parametri.Operatore,
                                                      Parametri.ProgressivoOper,
                                                      'S']),[srFromBeginning])
    and (   (Parametri.ProgressivoOper = -1)
         or not selSG745.SearchRecord('TIPO_CONSEGNA;PROG_UTENTE;ESITO',
                                              VarArrayOf(['PV',
                                                          Parametri.ProgressivoOper,
                                                          'S']),[srFromBeginning]));
end;
...
procedure TW022FSchedaValutazioniDtM.InviaEMailErrProt;
var sOggetto,sTesto: String;
    nTag: Integer;
    C017DtM:TC017FEMailDtM;
begin
  if Trim(Parametri.CampiRiferimento.C21_EMailDestIndirizzo) = '' then
    exit;
  try
    sOggetto:='Scheda di valutazione: errore protocollazione';
    sTesto:='La scheda di valutazione di ' + SelAnagrafeW022.FieldByName('COGNOME').AsString + ' ' + SelAnagrafeW022.FieldByName('NOME').AsString + ' (' + SelAnagrafeW022.FieldByName('MATRICOLA').AsString + ')' +
             IfThen(Q710.FieldByName('DATA').AsDateTime = EncodeDate(R180Anno(Q710.FieldByName('DATA').AsDateTime),12,31),
                    ' per l''anno ' + FormatDateTime('yyyy',Q710.FieldByName('DATA').AsDateTime),
                    ' al ' + FormatDateTime('dd/mm/yyyy',Q710.FieldByName('DATA').AsDateTime)) +
            ' non è stata automaticamente protocollata.' + CRLF +
            'Consultare le anomalie tramite la funzione "Messaggi delle elaborazioni", correggere gli eventuali errori e ritentare la protocollazione tramite la funzione "Elaborazione schede di valutazione".';
    nTag:=443; //"Stampa scheda di valutazione" per il valutato
    C017DtM:=TC017FEMailDtM.Create(GGetWebApplicationThreadVar);
    try
      C017DtM.SollevaEccezioni:=True;
      C017DtM.Sessione:=SessioneOracle;
      C017DtM.Destinatari:=Parametri.CampiRiferimento.C21_EMailDestIndirizzo;
