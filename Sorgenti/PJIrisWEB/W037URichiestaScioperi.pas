...
type
  ...
  TW037FRichiestaScioperi = class(TR013FIterBase)
    ...
    mnuEsportaExcelDipendenti: TMenuItem;
    mnuEsportaCSVDipendenti: TMenuItem;
    ...
    procedure mnuEsportaExcelDipendentiClick(Sender: TObject);
    procedure mnuEsportaCSVDipendentiClick(Sender: TObject);
  private
    Al: TDateTime;
...
begin
  ...
  if WR000DM.Responsabile then
  begin
    W037DM.selT251.Filter:=Format('TIPO_RICHIESTA <> ''%s''',[T251TR_P]);
  end;
...
procedure TW037FRichiestaScioperi.mnuEsportaCSVDipendentiClick(Sender: TObject);
begin
  if not Assigned(Sender) then
    csvDownload:=grdDipendenti.ToCsv
  else
    InviaFile('ElencoDipendenti.xls',csvDownload);
end;

procedure TW037FRichiestaScioperi.mnuEsportaExcelDipendentiClick(Sender: TObject);
begin
  if not Assigned(Sender) then
    streamDownload:=grdDipendenti.ToXlsx
  else
...
begin
  ...
  begin
    ...
    WR000DM.selAnagEventoSciopero.Close;
    WR000DM.selAnagEventoSciopero.SetVariable('DATALAVORO',PEvento.Data);
    WR000DM.selAnagEventoSciopero.SetVariable('FILTRO',FiltroAnag);
    WR000DM.selAnagEventoSciopero.SetVariable('FILTRO_IN_SERVIZIO',FILTRO_IN_SERVIZIO);
    WR000DM.selAnagEventoSciopero.Open;
...
begin
  ...
  begin
    ...
    begin
      ...
      VisCanc:=(TR = T251TR_P) and (Revocabile = 'CANC') and (not BloccoRiep);
      // modifica consentita se tipo richiesta = 'P'  e stato non negato
      VisMod:=(TR = T251TR_P) and (not StatoNegato) and (not BloccoRiep);
      // chiusura consentita se tipo richiesta = P
      VisChiudi:=(TR = T251TR_P) and (not BloccoRiep);
      if grdRichieste.medpCompGriglia[i].CompColonne[0] <> nil then
      begin
        // cancella
        if not VisCanc then
...
begin
  ...
  if R180In(W037DM.selT251.FieldByName('TIPO_RICHIESTA').AsString,[T251TR_P,T251TR_D]) then
  begin
