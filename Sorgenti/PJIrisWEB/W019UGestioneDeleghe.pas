...
type
  ...
  private
    ...
    Operatore: TUtente;
    Operazione,UtenteDelegato,ProfiloAssegnato,Autoesclusione: String;
    ...
    function  ControlliInserimentoOK(cmbUtente:TmeIWComboBox; edtProfilo,edtDal,edtAl:TmeIWEdit; cmbAutoesclusione:TmeIWComboBox):Boolean;
    function  ControlliModificaOK(edtDal,edtAl:TmeIWEdit; cmbAutoesclusione:TmeIWComboBox):Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      Operatore.ProfiloDelegato:=Operatore.NomeProfilo;
      if (FindField('PROFILO_DELEGATO') <> nil) and (FieldByName('PROFILO_DELEGATO').AsString.Trim <> '') then
        Operatore.ProfiloDelegato:=FieldByName('PROFILO_DELEGATO').AsString;
      Operatore.DelegatoDa:=Parametri.Operatore;
      if FieldByName('DELEGATO_DA').AsString.Trim <> '' then
        Operatore.DelegatoDa:=FieldByName('DELEGATO_DA').AsString;
  ...
  lstVarPers.Add(TVarPers.Create('NOME_UTENTE',otString,Parametri.Operatore));
  lstVarPers.Add(TVarPers.Create('NOME_PROFILO',otString,Parametri.ProfiloWEB));
  lstVarPers.Add(TVarPers.Create('DELEGATO',otString,Parametri.ProfiloWEBDelegatoDa));
...
function TW019FGestioneDeleghe.ControlliInserimentoOK(cmbUtente:TmeIWComboBox; edtProfilo,edtDal,edtAl:TmeIWEdit; cmbAutoesclusione:TmeIWComboBox):Boolean;
...
begin
  ...
  begin
    if FiltroDaEseguire then
      Msg:=Format(A000TraduzioneStringhe(A000MSG_W019_FMT_RICHIESTA_FILTRO),[btnFiltra.Caption])
    else
      Msg:=Format(A000TraduzioneStringhe(A000MSG_W019_FMT_LISTA_USR_VUOTA),[btnFiltra.Caption]);
  ...
  begin
    ...
    begin
      ...
      GGetWebApplicationThreadVar.ShowMessage(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_PROFILO_ASSEGNATO),[UtenteDelegato]));
      ActiveControl:=edtProfilo;
      Exit;
    end;
    Close;
  ...
  Autoesclusione:=cmbAutoEsclusione.Text;
  // inizio periodo
  ...
  begin
    GGetWebApplicationThreadVar.ShowMessage(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_VALIDITA_PROFILO),[DateToStr(Operatore.InizioValidita),DateToStr(Operatore.FineValidita)]));
    ActiveControl:=edtProfilo;
    Exit;
  end;
  ...
  begin
    ...
    begin
      ...
      Msg:=Format(A000TraduzioneStringhe(A000MSG_W019_FMT_PROFILO_GIA_DELEGATO),[UtenteDelegato,DateToStr(OldDataDal),DateToStr(OldDataAl)]);
      ...
      begin
        ...
        Msg:=Msg + Format(A000TraduzioneStringhe(A000MSG_W019_FMT_DELEGA_SOVRASCITTA),
      ...
      begin
        ...
        begin
          Msg:=Msg + Format(A000TraduzioneStringhe(A000MSG_W019_FMT_NO_POSTICIPARE),[DateToStr(DataDal)]);
          GGetWebApplicationThreadVar.ShowMessage(Msg);
        end
        else
        begin
          Msg:=Msg + Format(A000TraduzioneStringhe(A000MSG_W019_FMT_SOVRASCRIVI_DELEGA),
    ...
    begin
      ...
      Msg:=Format(A000TraduzioneStringhe(A000MSG_W019_FMT_DELEGATO_N_VOLTE),[UtenteDelegato,IntToStr(RecordCount),DateToStr(DataDal),DateToStr(DataAl)]);
      MsgBox.MessageBox(Msg,INFORMA);
      ActiveControl:=edtDal;
      Exit;
    end;
...
function TW019FGestioneDeleghe.ControlliModificaOK(edtDal,edtAl:TmeIWEdit; cmbAutoesclusione:TmeIWComboBox): Boolean;
var
  OldDataDal, OldDataAl: TDateTime;
  DataAttuale: TDateTime;
  Msg: String;
begin
  ...
  if (DataDal < Operatore.InizioValidita) or (DataAl > Operatore.FineValidita) then
  begin
    MsgBox.MessageBox(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_VALIDITA_PROFILO),[DateToStr(Operatore.InizioValidita),DateToStr(Operatore.FineValidita)]),INFORMA);
    Exit;
  end;
  ...
  begin
    ...
    begin
      // il periodo interseca più deleghe dello stesso profilo: nessuna azione possibile
      Msg:=Format(A000TraduzioneStringhe(A000MSG_W019_FMT_DELEGATO_N_VOLTE),
  ...
  begin
    ...
    begin
      MsgBox.MessageBox(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_ACCESSO_EFFETTUATO),[WR000DM.selI061DelegheUtente.FieldByName('ULTIMO_ACCESSO').AsString]),INFORMA);
    ...
    begin
      MsgBox.MessageBox(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_ACCESSO_EFFETTUATO2),[WR000DM.selI061DelegheUtente.FieldByName('ULTIMO_ACCESSO').AsString]),INFORMA);
      Exit;
    end;
  end;
...
begin
  ...
  begin
    ...
    FieldByName('AUTO_ESCLUSIONE').AsString:=AutoEsclusione;
    FieldByName('DELEGATO_DA').AsString:=Operatore.DelegatoDa;
    if FindField('DELEGA_INSERITA_DA') <> nil then
      FieldByName('DELEGA_INSERITA_DA').AsString:=Parametri.Operatore;
    if FindField('PROFILO_DELEGATO') <> nil then
      FieldByName('PROFILO_DELEGATO').AsString:=Operatore.ProfiloDelegato;
    try
      Post;
      ...
      begin
        SessioneOracle.Commit;
        GGetWebApplicationThreadVar.ShowMessage(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_INS_FALLITO),[E.Message]));
        Cancel;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    FieldByName('AUTO_ESCLUSIONE').AsString:=(grdDeleghe.medpCompCella(i,'AUTO_ESCLUSIONE',0) as TmeIWComboBox).Text;
    ...
    try
      ...
      begin
        ...
        SessioneOracle.Commit;
        GGetWebApplicationThreadVar.ShowMessage(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_VARIAZIONE_FALLITA),[e.Message]));
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    Messaggio(A000TraduzioneStringhe(A000MSG_MSG_CONFERMA),
              A000TraduzioneStringhe(A000MSG_W019_MSG_FMT_RICERCA),FiltraComboUtenti,nil);
    Exit;
  end;
  FiltraComboUtenti;
end;
...
begin
  ...
  begin
    ...
    Filtro:=Format('(upper(T030.COGNOME) %s upper(''%s''))',[Operatore,edtCognomeCerca.Text]);
  end;
  ...
  begin
    ...
    Filtro:=IfThen(Filtro <> '',Filtro + ' OR ') +
            ...
            Format('(upper(I060.MATRICOLA) %s upper(''%s''))',[Operatore,edtMatricolaCerca.Text]);
  end;
  ...
  begin
    ...
    Filtro:=IfThen(Filtro <> '',Filtro + ' OR ') +
            ...
            Format('(upper(I060.NOME_UTENTE) %s upper(''%s''))',[Operatore,edtUserCerca.Text]);
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        GGetWebApplicationThreadVar.ShowMessage(Format(A000TraduzioneStringhe(A000MSG_W019_FMT_CANCELLARE),[FieldByName('ULTIMO_ACCESSO').AsString]));
    end;
  end;
end;
...
begin
  ...
  begin
    if not ControlliModificaOK((grdDeleghe.medpCompCella(i,'INIZIO_VALIDITA',0) as TmeIWEdit),
                               (grdDeleghe.medpCompCella(i,'FINE_VALIDITA',0) as TmeIWEdit),
                               (grdDeleghe.medpCompCella(i,'AUTO_ESCLUSIONE',0) as TmeIWCombobox)
                               ) then
    Exit;
...
begin
  ...
  if Parametri.CampiRiferimento.C90_NomeProfiloDelega = '' then
    Result:=Operatore.ProfiloDelegato
  else
  begin
    ...
    try
      try
        ...
        begin
          ...
          ODS.SetVariable('DELEGANTE',Operatore.DelegatoDa);
        end;
        // delegato
        ...
        begin
          ...
          ODS.SetVariable('PROFILO',Operatore.ProfiloDelegato);
        end;
        // dal
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        // aggiunge item alla combo di scelta utente
        //Item:=StringReplace(R180DimLung(ElencoComboUtenti[i].NomeUtente, 30),' ',SPAZIO, [rfReplaceAll]);
        //Item:=Item + ' - ' + ElencoComboUtenti[i].Cognome + ' ' + ElencoComboUtenti[i].Nome;
        Item:=Format('%s %s (%s)',[ElencoComboUtenti[i].Cognome,ElencoComboUtenti[i].Nome,ElencoComboUtenti[i].NomeUtente]);
...
begin
  ...
  begin
    ...
    begin
      ...
      grdDeleghe.medpPreparaComponenteGenerico('C',0,0,DBG_CMB_COUR,'50','','','','S');
      ...
      grdDeleghe.medpPreparaComponenteGenerico('C',0,0,DBG_CMB,'2','','','','S');
      c:=grdDeleghe.medpIndexColonna('AUTO_ESCLUSIONE');
      grdDeleghe.medpCreaComponenteGenerico(0,c,grdDeleghe.Componenti);
      with (grdDeleghe.medpCompCella(0,c,0) as TmeIWComboBox) do
      begin
        Items.Add('N');
        Items.Add('S');
        DefColonna:=WR000DM.GetColumnDefault('I061_PROFILI_DIPENDENTE','AUTO_ESCLUSIONE');
        DefColonna:=DefColonna.Substring(1,1);
        if Items.IndexOf(DefColonna) >= 0 then
        begin
          ItemIndex:=Items.IndexOf(DefColonna);
        end
        else
        begin
          ItemIndex:=0;
    ...
    begin
      ...
      c:=grdDeleghe.medpIndexColonna('AUTO_ESCLUSIONE');
      grdDeleghe.medpCreaComponenteGenerico(i,c,grdDeleghe.Componenti);
      with (grdDeleghe.medpCompCella(i,c,0) as TmeIWComboBox) do
      begin
        Items.Add('N');
        Items.Add('S');
        ItemIndex:=Items.IndexOf(grdDeleghe.medpValoreColonna(i,'AUTO_ESCLUSIONE'));
      end;
    end;
  end
  else
...
begin
  ...
  grdDeleghe.medpAggiungiColonna('AUTO_ESCLUSIONE','Escludi il delegato','',nil);
  ...
  begin
    ...
    grdDeleghe.medpPreparaComponenteGenerico('R',0,0,DBG_IMG,'','CANCELLA',A000TraduzioneStringhe('Elimina delega'),A000TraduzioneStringhe('Eliminare la delega selezionata?'),'S');
