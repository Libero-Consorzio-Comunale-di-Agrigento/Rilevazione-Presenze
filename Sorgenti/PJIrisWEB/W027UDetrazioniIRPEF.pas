...
uses
  ...
  IWCompButton, OracleData, A000USessione, A000UInterfaccia, QueryStorico,
...
type
  ...
  TW027FDetrazioniIRPEF = class(TR012FWebAnagrafico)
    ...
    dsrcSG101: TDataSource;
    cdscSG101: TClientDataSet;
    grdANF: TmedpIWDBGrid;
    cdsdSG101: TClientDataSet;
    dsrdSG101: TDataSource;
    lblANF: TmeIWLabel;
    lblANFTotale: TmeIWLabel;
    lblNoFamiliari: TmeIWLabel;
  ...
  private
    ...
    DipInSer:TDipendenteInServizio;//DA 03/01/2017
...
begin
  ...
  //DA 03/01/2017 cmbDipendentiDisponibili.Enabled:=False;
...
begin
  ...
  //DA 30/11/2015 Se si dovesse mai ripristinare la sezione relativa al selP500 tenere conto che i dati andrebbero presi con COD_AZIENDA_BASE = T440AZIENDA_BASE
  ...
  //DA 03/01/2017 inizio
  DipInSer:=TDipendenteInServizio.Create(nil);
  DipInSer.Session:=SessioneOracle;
  CampiV430:='V430.P430SCADENZA_ANF,V430.P430REDDITO_ANF,V430.P430REDDITO_ALTRO_ANF,V430.P430INABILE_ANF';
  //DA 03/01/2017 fine
  vData:=Parametri.DataLavoro;
  GetDipendentiDisponibili(vData);
  grdDetrazioni.medpPaginazione:=False;//Non gestire la paginazione!
  //DA 03/01/2017 grdDetrazioni.medpDataSet:=WR000DM.selSG122;

  //DA 03/01/2017 inizio
  grdDetrazioni.medpDataSet:=WR000DM.seldSG101;
  grdDetrazioni.medpBrowse:=True;
  grdANF.medpPaginazione:=False;//Non gestire la paginazione!
  grdANF.medpDataSet:=WR000DM.selcSG101;
  grdANF.medpBrowse:=True;
  lblDetrazioniDipendente.Visible:=False;
  lblDetrazioniDipendenteSiNo.Visible:=False;
  //DA 03/01/2017 fine

  CreazioneForm:=True;
end;

procedure TW027FDetrazioniIRPEF.IWAppFormDestroy(Sender: TObject);
begin
  inherited;
  FreeAndNil(DipInSer);
end;
...
begin
  ...
  *)
  ...
  //Si attiva il calcolo dell’assegno se dipendente in servizio nel mese, P430.SCADENZA_ANF è vuoto oppure >= mese di calcolo, esiste un reddito totale sul richiedente
  TotRedditoANF:=selAnagrafeW.FieldByName('P430REDDITO_ANF').AsFloat + selAnagrafeW.FieldByName('P430REDDITO_ALTRO_ANF').AsFloat;
  CercaANF:=
    DipInSer.DipendenteInServizio(selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger,R180InizioMese(DataAgg),R180FineMese(DataAgg)) and
    (selAnagrafeW.FieldByName('P430SCADENZA_ANF').IsNull or (R180FineMese(selAnagrafeW.FieldByName('P430SCADENZA_ANF').AsDateTime) >= DataAgg)) and
    (TotRedditoANF > 0);

  //apertura dataset del dettaglio delle dichiarazioni di assegno nucleo familiare (ANF)
  with WR000DM.selcSG101 do
  begin
    Close;
    SetVariable('PROGRESSIVO',selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger);
    SetVariable('DATA_AGG',DataAgg);
    ...
    while not Eof do
    begin
      TotRedditoANF:=TotRedditoANF + FieldByName('REDDITO_TOT_ANF').AsFloat;
      Next;
    end;
  end;
  grdANF.medpCreaCDS;
  grdANF.medpEliminaColonne;
  grdANF.medpAggiungiColonna('DESC_GRADO','Parentela','',nil);
  grdANF.medpAggiungiColonna('COGNOME','Cognome','',nil);
  grdANF.medpAggiungiColonna('NOME','Nome','',nil);
  grdANF.medpAggiungiColonna('DATANAS','Data nascita','',nil);
  grdANF.medpAggiungiColonna('DESC_SPECIALE','Studente / apprendista','',nil);
  grdANF.medpAggiungiColonna('DESC_INABILE','Inabile','',nil);
  grdANF.medpAggiungiColonna('DESC_REDDITO','Reddito totale','',nil);
  grdANF.medpInizializzaCompGriglia;
  grdANF.medpCaricaCDS;
  grdANF.Visible:=(WR000DM.selcSG101.RecordCount > 0) and CercaANF;

  //apertura dataset del dettaglio delle dichiarazioni di detrazioni per familiari a carico
  with WR000DM.seldSG101 do
  begin
    Close;
    SetVariable('PROGRESSIVO',selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger);
    SetVariable('DATA_AGG',DataAgg);
  ...
  grdDetrazioni.medpCreaCDS;
  grdDetrazioni.medpEliminaColonne;
  grdDetrazioni.medpAggiungiColonna('DESC_GRADO','Parentela','',nil);
  grdDetrazioni.medpAggiungiColonna('COGNOME','Cognome','',nil);
  grdDetrazioni.medpAggiungiColonna('NOME','Nome','',nil);
  grdDetrazioni.medpAggiungiColonna('DATANAS','Data nascita','',nil);
  grdDetrazioni.medpAggiungiColonna('DESC_DETRAZIONE','Tipo detrazione','',nil);
  grdDetrazioni.medpAggiungiColonna('DESC_PERC_CARICO','% a carico','',nil);
  grdDetrazioni.medpAggiungiColonna('DESC_HANDICAP','Portatore di handicap','',nil);
  grdDetrazioni.medpInizializzaCompGriglia;
  grdDetrazioni.medpCaricaCDS;
  grdDetrazioni.Visible:=WR000DM.seldSG101.RecordCount > 0;

  //messaggi
  lblANFTotale.Caption:=R180Formatta(TotRedditoANF,10,2) + IfThen(selAnagrafeW.FieldByName('P430INABILE_ANF').AsString = 'S',' (richiedente inabile)');
  lblANF.Visible:=CercaANF;
  lblANFTotale.Visible:=CercaANF;
  lblNoFamiliari.Visible:=(WR000DM.selcSG101.RecordCount = 0) and (WR000DM.seldSG101.RecordCount = 0);
...
procedure TW027FDetrazioniIRPEF.grdDetrazioniRenderCell(ACell: TIWGridCell; const ARow, AColumn: Integer);
...
begin
  ...
  begin
    ...
    if (ACell.Grid as TmedpIWDBGrid).medpNumColonna(AColumn) = (ACell.Grid as TmedpIWDBGrid).medpIndexColonna('DATANAS') then
