...
uses W001UIrisWebDtM, W002UAnagrafeElenco, W001ULogin;
...
var
  LFiltroAnagrafe,LFiltroUtente,LOrdinamUtente,LFiltroInServizio,LErrMsg: String;
  LPosOrderBy: Integer;
...
begin
  ...
  LFiltroAnagrafe:=Trim(Parametri.Inibizioni.Text);

  // filtro dipendenti in servizio / cessati
  LFiltroInServizio:='';
  if not VisualizzaCessati then
  ...
  WR000DM.selAnagrafe.SetVariable('FILTRO_IN_SERVIZIO',LFiltroInServizio);
  ...
  begin
    ...
    LFiltroAnagrafe:=R180InserisciAliasT030(LFiltroAnagrafe);
    WR000DM.selAnagrafe.SetVariable('FILTRO',' AND ' + LFiltroAnagrafe);
    try
      //Open;
      WR000DM.selAnagrafe.FieldDefs.Update;
    except
      on E:Exception do
      begin
        LErrMsg:=Format(A000TraduzioneStringhe(A000MSG_W002_ERR_FMT_FILTRO_NON_ASSOCIATO),[Parametri.Operatore,E.Message]);
      end;
    end;
  end;
  ...
  if LErrMsg = '' then
  begin
    LOrdinamUtente:='ORDER BY COGNOME,NOME,MATRICOLA';
    if WR000DM.lstSQL.Count > 0 then
    begin
      LFiltroUtente:=Trim(EliminaRitornoACapo(WR000DM.lstSQL.Text));
      LPosOrderBy:=Pos('ORDER BY',UpperCase(LFiltroUtente));
      if LPosOrderBy > 0 then
      begin
        LOrdinamUtente:=Trim(Copy(LFiltroUtente,LPosOrderBy,Length(LFiltroUtente) - LPosOrderBy + 1));
        LFiltroUtente:=Trim(Copy(LFiltroUtente,1,LPosOrderBy - 1));
      end;
      LFiltroAnagrafe:=CRLF + TAG_FILTRO_ANAG_INIZIO + CRLF + LFiltroAnagrafe + IfThen(LFiltroAnagrafe <> '',CRLF) + TAG_FILTRO_ANAG_FINE + CRLF +
                      IfThen((LFiltroAnagrafe <> '') and (LFiltroUtente <> ''),' AND ') +
                      LFiltroUtente;
    end
    else
      LFiltroAnagrafe:=CRLF + TAG_FILTRO_ANAG_INIZIO + CRLF + LFiltroAnagrafe + IfThen(LFiltroAnagrafe <> '',CRLF) + TAG_FILTRO_ANAG_FINE + CRLF;
    ...
    LOrdinamUtente:=R180InserisciAliasT030(LOrdinamUtente);

    // apertura dataset
    WR000DM.selAnagrafe.Close;
    WR000DM.selAnagrafe.ReadBuffer:=IfThen(Parametri.InibizioneIndividuale,2,100);
    WR000DM.selAnagrafe.SetVariable('FILTRO',Trim(LFiltroAnagrafe + ' ' + LOrdinamUtente));
    try
      WR000DM.selAnagrafe.Open;
      //AOSTA_ASL: se nessuna anagrafica selezionata provo a includere i cessati
      if (WR000DM.selAnagrafe.RecordCount = 0) and (not VisualizzaCessati) and (not Parametri.InibizioneIndividuale) and (WR000DM.TipoUtente <> 'Supervisore') then
      begin
        VisualizzaCessati:=True;
        LFiltroInServizio:='';
        WR000DM.selAnagrafe.Close;
        WR000DM.selAnagrafe.SetVariable('FILTRO_IN_SERVIZIO','');
        WR000DM.selAnagrafe.Open;
      end;
      //AOSTA_ASL.fine
      WR000DM.selAnagrafe.ReadBuffer:=WR000DM.selAnagrafe.RecordCount + 1; // aggiorna readbuffer per migliorare il recupero dati nei successivi refresh del dataset
    except
      on E:Exception do
      begin
        if Sender = lnkRicercaSalvata then
          LErrMsg:=Format(A000TraduzioneStringhe(A000MSG_W002_ERR_FMT_SELEZIONE_SBAGLIATA),[cmbSelezioni.Items[cmbSelezioni.ItemIndex],e.Message])
        else
          LErrMsg:=A000TraduzioneStringhe(A000MSG_W002_ERR_FMT_SELEZIONE_SBAGLIATA2);
        LErrMsg:=Format(LErrMsg,[E.Message]);
      end;
    end;
  end;
  ...
  begin
    WR000DM.selAnagrafe.Close;
    WR000DM.selAnagrafe.SetVariable('FILTRO','AND T030.PROGRESSIVO < 0');
    WR000DM.selAnagrafe.Open;
  end;

  // salva filtro selezione e ordinamento
  WR000DM.FiltroRicerca:=LFiltroInServizio + LFiltroAnagrafe;
  WR000DM.OrdinamentoRicerca:=LOrdinamUtente;
  Log('Traccia','RicercaAnagrafe: ' + WR000DM.TipoUtente + ' ' + Parametri.Operatore +
                ';Filtro complessivo: ' + WR000DM.FiltroRicerca +
                ';Filtro in servizio: ' + LFiltroInServizio +
                ';Filtro anagrafe ' + IfThen(LErrMsg <> '','(ERRATO)') + ': ' + LFiltroAnagrafe +
                ';OrdinamentoUtente: ' + LOrdinamUtente);
  ...
  begin
    ...
    if LErrMsg <> '' then
    begin
      MessaggioStatus(ERRORE,LErrMsg);
