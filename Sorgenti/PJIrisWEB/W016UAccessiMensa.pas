...
uses
  ...
  IWCompListbox, OracleData, A000USessione, A000UInterfaccia, A000UCostanti,
...
begin
  ...
  if (D < StrToDate(edtPeriodoDal.Text)) or (D > StrToDate(edtPeriodoAl.Text)) then
  begin
    GGetWebApplicationThreadVar.ShowMessage('La data dell''accesso deve essere compreso fra ' + Format('%s e %s',[edtPeriodoDal.Text,edtPeriodoAl.Text]));
    ActiveControl:=edtData;
    exit;
  end;
...
begin
  try
    ...
    if R180Anno(Dal) <> R180Anno(Al) then
      raise ExceptionNoLog.Create('Le date devono essere riferite allo stesso anno!');
  except
    on E:Exception do
    begin
