...
uses
  SysUtils, Classes, Controls, Forms, OracleData, DB, StrUtils, Math, Variants,
  ...
  IWCompJQueryWidget, IWCompGrids, medpIWMessageDlg, C180FunzioniGenerali, A000UCostanti;
...
procedure TW004FModificaTabelloneFM.CaricaGriglia(pRecapitoDefault: string);
var Dipendente:String;
begin
  RecapitoDefault:=pRecapitoDefault;
  ...
  begin
    ...
    if RecordCount = 0 then
    begin
      //Pianificazione del dd/mm/yyyy non abilitata al proprio profilo
      MsgBox.MessageBox(Format('Pianificazione del %s non abilitata al proprio profilo', [DateToStr(Data)]),ESCLAMA);
      btnAnnullaClick(btnAnnulla);
      exit;
    end;
  ...
  begin
    ...
    medpAggiungiColonna('RECAPITO1','Recapito 1','',nil);
    medpColonna('RECAPITO1').Visible:=Tipo = 'R';
    ...
    medpAggiungiColonna('RECAPITO2','Recapito 2','',nil);
    medpColonna('RECAPITO2').Visible:=Tipo = 'R';
    ...
    medpAggiungiColonna('RECAPITO3','Recapito 3','',nil);
    medpColonna('RECAPITO3').Visible:=Tipo = 'R';
...
begin
  W004DM_Mod.delT380.SetVariable('RID',RI);
  try
    W004DM_Mod.delT380.Execute;
  except
  end;
...
procedure TW004FModificaTabelloneFM.cmbChange(Sender: TObject; EventParams: TStringList; Index: Integer; Value: string);
var lc: string;
begin
  with (Sender as TMedpIWMultiColumnComboBox) do
  begin
    lc:=AnsiRightStr(Name, 1);
    if Text <> '' then
      if (grdPianificazione.medpCompCella(0,grdPianificazione.medpIndexColonna('RECAPITO'+ lc),0) as TmeIWEdit).Text = '' then
        (grdPianificazione.medpCompCella(0,grdPianificazione.medpIndexColonna('RECAPITO'+ lc),0) as TmeIWEdit).Text:=RecapitoDefault;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      // recapito turno 1
      c:=medpIndexColonna('RECAPITO1');
      medpPreparaComponenteGenerico('C',0,0,DBG_EDT,'15','','','','');
      medpCreaComponenteGenerico(i,c,Componenti);
      with (medpCompCella(i,c,0) as TmeIWEdit) do
      begin
        Text:=medpValoreColonna(i,'RECAPITO1');
        MaxLength:=50;
        NonEditableAsLabel:=True;
        Editable:=False;
      end;
      ...
      // recapito turno 2
      c:=medpIndexColonna('RECAPITO2');
      medpPreparaComponenteGenerico('C',0,0,DBG_EDT,'15','','','','');
      medpCreaComponenteGenerico(i,c,Componenti);
      with (medpCompCella(i,c,0) as TmeIWEdit) do
      begin
        Text:=medpValoreColonna(i,'RECAPITO2');
        MaxLength:=50;
        NonEditableAsLabel:=True;
        Editable:=False;
      end;
      ...
      // recapito turno 3
      c:=medpIndexColonna('RECAPITO3');
      medpPreparaComponenteGenerico('C',0,0,DBG_EDT,'15','','','','');
      medpCreaComponenteGenerico(i,c,Componenti);
      with (medpCompCella(i,c,0) as TmeIWEdit) do
      begin
        Text:=medpValoreColonna(i,'RECAPITO3');
        MaxLength:=50;
        NonEditableAsLabel:=True;
        Editable:=False;
      end;
...
begin
  ...
  begin
    ...
    (medpCompCella(0,medpIndexColonna('RECAPITO1'),0) as TmeIWEdit).Text:='';
    ...
    (medpCompCella(0,medpIndexColonna('RECAPITO2'),0) as TmeIWEdit).Text:='';
    ...
    (medpCompCella(0,medpIndexColonna('RECAPITO3'),0) as TmeIWEdit).Text:='';
  end;
end;
  ...
  with grdPianificazione do
    ...
    begin
      if not RecordFittizio then
        ...
        raise ExceptionNoLog.Create('Selezionare almeno un turno!');
    end
    else
    begin
