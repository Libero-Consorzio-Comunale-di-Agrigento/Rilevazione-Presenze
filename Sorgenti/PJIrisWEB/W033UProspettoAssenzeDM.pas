...
var
  DescCaus,TipoGius,Orario,Aut: String;
begin
  with selaT050 do
  begin
    ...
    if TipoGius = 'I' then
      FieldByName('D_TIPOGIUST').AsString:=A000TraduzioneStringhe('Giornate')
    else if TipoGius = 'M' then
      FieldByName('D_TIPOGIUST').AsString:=A000TraduzioneStringhe('Mezze giorn.')
    else if TipoGius = 'N' then
      FieldByName('D_TIPOGIUST').AsString:=A000TraduzioneStringhe('Numero Ore')
    else if TipoGius = 'D' then
      FieldByName('D_TIPOGIUST').AsString:=A000TraduzioneStringhe('Da ore/A ore');
    ...
    if FieldByName('AUTORIZZ_UTILE').AsString = '' then
      ...
      Aut:=A000TraduzioneStringhe('No')
    else
      Aut:=A000TraduzioneStringhe('Si');
