...
uses
  ...
  C004UParamForm, C018UIterAutDM, C180FunzioniGenerali, C190FunzioniGeneraliWeb,
...
type
  ...
  TW025FCambioOrario = class(TR013FIterBase)
    ...
    lblModalitaRichiesta: TmeIWLabel;
    rgpModalitaRichiesta: TmeIWRadioGroup;
    ...
    procedure rgpModalitaRichiestaClick(Sender: TObject);
    procedure grdRichiesteAfterCaricaCDS(Sender: TObject; DBG_ROWID: string);
    procedure rgpModalitaRichiestaAsyncChange(Sender: TObject; EventParams: TStringList);
  private
    Operazione: String;
    ...
    procedure RicavaOrario(Prog:Integer; Data: TDateTime; TipoElencoOrari:String; OrarioO:String; var Orari: TStringList);
...
begin
  ...
  C004FParamFormPrv:=CreaC004(SessioneOracle,'W025',Parametri.Operatore,False);
...
begin
  ...
  C190VisualizzaElemento(JQuery,'w025filtroModalitaRichiesta',TCO = 'E');//visibilita di lblModalitaRichiesta e rgpModalitaRichiesta
end;
...
procedure TW025FCambioOrario.rgpModalitaRichiestaAsyncChange(Sender: TObject; EventParams: TStringList);
begin
  inherited;
  PutParametriFunzione;
end;

procedure TW025FCambioOrario.rgpModalitaRichiestaClick(Sender: TObject);
begin
  inherited;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        RicavaOrario(cdsT085.FieldByName('PROGRESSIVO').AsInteger,cdsT085.FieldByName('DATA').AsDateTime,'1','',ListaOrari);
      ...
      begin
        ListaOrari:=TStringList.Create;
        RicavaOrario(cdsT085.FieldByName('PROGRESSIVO').AsInteger,cdsT085.FieldByName('DATA_INVER').AsDateTime,'1','',ListaOrari);
        for j:=0 to ListaOrari.Count - 1 do
          Orario:=Copy(ListaOrari[j],1,Pos(' ',ListaOrari[j]) - 1);
        ListaOrari.Free;
...
begin
  ...
  begin
    ...
    begin
      ListaOrari:=TStringList.Create;
      RicavaOrario(selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger,Parametri.DataLavoro,'1','',ListaOrari);
      for j:=0 to ListaOrari.Count - 1 do
    ...
    if TCO <> 'I' then
    begin
      grdRichieste.medpPreparaComponenteGenerico('C',0,0,DBG_CMB_COUR,'40','','','','S',True);
      ...
      begin
        ...
        ListaOrari:=TStringList.Create;
        RicavaOrario(selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger,Data,IfThen(Data = GiornoOriginale,'2','3'),(grdRichieste.medpCompCella(i,ColCFOrario,0) as TmeIWLabel).Caption,ListaOrari);
        for j:=0 to ListaOrari.Count - 1 do
          Items.Add(ListaOrari[j]);
        ListaOrari.Free;
        ListaOrari:=TStringList.Create;
        RicavaOrario(selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger,Data,'1','',ListaOrari);
        for j:=0 to ListaOrari.Count - 1 do
          S:=ListaOrari[j];
        ListaOrari.Free;
        Items.EndUpdate;
        ItemIndex:=max(0,Items.IndexOf(S));
        Editable:=Data = GiornoOriginale;
      end;
    end
    else
    begin
      ...
      begin
        ...
        try
          ...
          if not ((DayOfWeek(Data - 1) = 7) and (Sett = 1)) then
            RicavaOrario(selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger,Data,'1','',ListaOrari);
        except
        end;
        for j:=0 to ListaOrari.Count - 1 do
...
begin
  ...
  begin
    ...
    begin
      ...
      ItemIndex:=max(0,Items.IndexOf(S));
      Editable:=Items.Count > 1;
      cmbGiornoChange((grdRichieste.medpCompCella(i,ColCFDataInver,0) as TmeIWComboBox));//si occupa di aggiornare l'orario richiesto
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      Items.Clear;
      ListaOrari:=TStringList.Create;
      RicavaOrario(selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger,Data,'2',(grdRichieste.medpCompCella(i,ColCFOrario,0) as TmeIWLabel).Caption,ListaOrari);
      for j:=0 to ListaOrari.Count - 1 do
        Items.Add(ListaOrari[j]);
      ListaOrari.Free;
      ListaOrari:=TStringList.Create;
...
begin
  ...
  try
    ...
    //abort;
    (Sender as TmeIWComboBox).Css:=CssCombo;
    if TCO <> 'I' then
    begin
      with (grdRichieste.medpCompCella(i,ColCFOrarioInver,0) as TmeIWComboBox) do
      begin
    ...
    exit; //Esco perché non c'è un giorno col quale effettuare lo scambio di orario
  end;
...
procedure TW025FCambioOrario.RicavaOrario(Prog:Integer; Data: TDateTime; TipoElencoOrari:String; OrarioO:String; var Orari: TStringList);
var
  TEO,Orario,OrarioProfilo,OrariScambio: String;
begin
  TEO:=TipoElencoOrari;
  if TEO = '1' then
  begin
    ...
    begin
      ...
      Orari.Add(Format('%-5s %s',[Orario,W025DM.RicavaDescrizioneOrario(Orario,Data)]));
    end;
  end
  else
  begin
    if TEO = '2' then
    begin
      //cerco elenco orari per lo scambio
      OrariScambio:=Trim(W025DM.RicavaOrariScambioOrario(Copy(OrarioO,1,Pos(' ',OrarioO) - 1),Data));
      if OrariScambio <> '' then
      begin
        OrariScambio:=OrariScambio + ',';
        //Aggiungo tutti gli orari elencati (filtro dizionario MODELLI ORARIO)
        WR000DM.selT020.Close;
        WR000DM.selT020.Open;
        while Pos(',',OrariScambio) > 0 do
        begin
          Orario:=Copy(OrariScambio,1,Pos(',',OrariScambio) - 1);
          if WR000DM.selT020.Locate('CODICE',Orario,[]) then
          begin
            Orario:=Format('%-5s %s',[Orario,W025DM.RicavaDescrizioneOrario(Orario,Data)]);
            if Orari.IndexOf(Orario) < 0 then
              Orari.Add(Orario);
          end;
          OrariScambio:=Copy(OrariScambio,Pos(',',OrariScambio) + 1);
        end;
        WR000DM.selT020.Close;
      end
      else
        TEO:='3';//se l'orario originale non ha impostato un elenco di orari per lo scambio, allora seguo le regole standard
    end;
    if TEO = '3' then
    begin
      ...
      begin
        ...
        while not Eof do
        begin
          Orario:=Format('%-5s %s',[FieldByName('CODICE').AsString,W025DM.RicavaDescrizioneOrario(FieldByName('CODICE').AsString,Data)]);
          if Orari.IndexOf(Orario) < 0 then
            Orari.Add(Orario);
          Next;
        end;
      ...
      begin
        ...
        while not Eof do
        begin
          Orario:=Format('%-5s %s',[FieldByName('ORARIO').AsString,W025DM.RicavaDescrizioneOrario(FieldByName('ORARIO').AsString,Data)]);
          if Orari.IndexOf(Orario) < 0 then
            Orari.Add(Orario);
          Next;
        end;
...
var
  ...
  i,Sett: Integer;
begin
  if (TCO = 'E') and (rgpModalitaRichiesta.ItemIndex = 0) then
    Giorni.Add(Copy(R180NomeGiorno(Data),1,3) + ' ' + FormatDateTime('dd/mm/yyyy',Data))
  else //(TCO = 'I') or (rgpModalitaRichiesta.ItemIndex = 1)
  begin
    Sett:=Max(StrToIntDef(Parametri.CampiRiferimento.C90_WebSettCambioOrario,1),1);
    if (DayOfWeek(Data - 1) = 7) and (Sett = 1) then
      Exit;
    d:=Data + 1;
...
procedure TW025FCambioOrario.GetParametriFunzione;
...
begin
  if not WR000DM.Responsabile then
  begin
    rgpModalitaRichiesta.ItemIndex:=StrToIntDef(C004FParamFormPrv.GetParametro('rgpModalitaRichiesta','0'),0);
  end;
end;

procedure TW025FCambioOrario.PutParametriFunzione;
{Scrivo i parametri della form}
begin
  if not WR000DM.Responsabile then
  begin
    C004FParamFormPrv.Cancella001;
    C004FParamFormPrv.PutParametro('rgpModalitaRichiesta',rgpModalitaRichiesta.ItemIndex.ToString);
    SessioneOracle.Commit;
  end;
end;
