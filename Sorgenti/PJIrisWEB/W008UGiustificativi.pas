...
uses
  A000USessione, A000UInterfaccia, A000UCostanti, B021UUtils, B021UWebSvcClientDtM,
  C180FunzioniGenerali, C190FunzioniGeneraliWeb,
  A004UGiustifAssPresMW, R012UWebAnagrafico, RegistrazioneLog,
  A000UMessaggi, W000UMessaggi, R600, USelT004, W010UCalcoloCompetenzeFM,
...
type
  ...
  private
    ...
    procedure SetRgpTipoGiustItemIndex(TipoGiust:String);
...
begin
  ...
  if ParametriForm.Causale <> '' then
  begin
    SetRgpTipoGiustItemIndex(GetParam('TIPOGIUST'));
    edtDaOre.Text:=GetParam('DAORE');
    edtAOre.Text:=GetParam('AORE');
  end;
...
begin
  ...
  AssegnaGestPeriodo(edtDaData,edtAData);
...
begin
  ...
  if (not MsgBox.IsActive) then
    (*if (Caus <> '') then*) // BugFix: TORINO_ITC - 169399: eseguo sempre "CausaleCambiata" in modo che venga nascosto il btnRecapitoAlternativo se non è selezionata nessuna causale
    AddToInitProc('CausaleCambiata("' + Trim(Copy(cmbCausale.Text,1,5)) + '");');
end;

procedure TW008FGiustificativi.SetRgpTipoGiustItemIndex(TipoGiust:String);
begin
  if rgpCausali.ItemIndex = 0 then
  begin
    if TipoGiust = 'I' then
      rgpTipoGiust.ItemIndex:=0
    else if TipoGiust = 'M' then
      rgpTipoGiust.ItemIndex:=1
    else if TipoGiust = 'N' then
...
procedure TW008FGiustificativi.GetCausaliDisponibili(pMantieniValore: Boolean = False);
...
var
  Q: TOracleDataSet;
  Codice,JsCodeFruiz,FruizG,FruizMG,FruizN,FruizD,JsCodeFam,Fam,JsCodeVisFisc,VisFisc, CampoCausale: String;
  CausAb, ValoreSelezionato: String;
  LstCausaliAbilitate, LstRaggrCausAb: TStringList;
  DataDa, DataA: TDateTime;
  EscludiCausale: Boolean;
begin
  Log('Traccia','GetCausaliDisponibili - inizio');
  ValoreSelezionato:=cmbCausale.SelectedValue;
  ...
  LstCausaliAbilitate:=TStringList.Create;
  LstCausaliAbilitate.Sorted:=True;
  LstCausaliAbilitate.Duplicates:=dupIgnore;
  LstCausaliAbilitate.CaseSensitive:=True;
  ...
  CampoCausale:=IfThen(rgpCausali.ItemIndex = 0, 'ABCAUSALE1', 'ABPRESENZA1');

  if (not WR000DM.Responsabile) and
     WR000DM.selT430CausaliAbilitate.Active and
     WR000DM.selT430RaggrAssenzeAbilitati.Active then
  begin
    if TryStrToDate(edtDaData.Text, DataDa) and not TryStrToDate(edtAData.Text, DataA) then
      edtAData.Text:=edtDaData.Text;
    if TryStrToDate(edtAData.Text, DataA) and not TryStrToDate(edtDaData.Text, DataDa) then
      edtDaData.Text:=edtAData.Text;
    if not TryStrToDate(edtDaData.Text, DataDa) then
      DataDa:=IncMonth(Parametri.DataLavoro, -12);
    if not TryStrToDate(edtAData.Text, DataA) then
    ...
    begin
      ...
      begin
        if Max(DataDa, FieldByName('DECORRENZA').AsDateTime) <= Min(DataA, FieldByName('SCADENZA').AsDateTime) then
          for CausAb in FieldByName(CampoCausale).AsString.Split([',']) do
            LstCausaliAbilitate.Add(CausAb);  //se Causali di Assenza (rgpCausali.ItemIndex = 0) CausAb è un codice causale, altrimenti è un codice raggruppamento!
        Next;
      end;
    end;
  ...
  begin
    ...
    //Valuta se la causale deve essere visibile nella combo
    if rgpCausali.ItemIndex = 0 then
      EscludiCausale:=(not WR000DM.Responsabile) and
                      (((Q.FieldByName('FRUIBILE').AsString = 'N') and (LstCausaliAbilitate.IndexOf(Codice) < 0)) or
                       ((Q.FieldByName('CONTASOLARE').AsString = 'S') and (LstRaggrCausAb.IndexOf(Q.FieldByName('CODRAGGR').AsString) < 0)))
    else
      EscludiCausale:=(not WR000DM.Responsabile) and (LstCausaliAbilitate.IndexOf(Q.FieldByName('CODRAGGR').AsString) < 0);
    ...
    if (not EscludiCausale) and
       ((not Parametri.ModuloInstallato['TORINO_CSI_PRV']) or
  ...
  begin
    ...
    Add('  try { ');
    //Add('    // 0. gestione visite fiscali: nascondo sempre il btnRecapitoAlternativo; sarà reso visibile a 3. se ci sono le condizioni');
...
//    Add('        alert("Visite fiscali = " + GestVisFisc); ');
  ...
  if pMantieniValore and (cmbCausale.Items.Count > 0) then
    cmbCausale.ItemIndex:=Max(0, cmbCausale.Items.IndexOf(ValoreSelezionato));
...
var
  Prog, IndCaus: Integer;
  Caus: String;
begin
  inherited;
  Log('Traccia','VisualizzaDipendenteCorrente - inizio');
  // salva parametri
  if (ParametriForm.Progressivo <> selAnagrafeW.FieldByName('PROGRESSIVO').AsInteger) or
     (not WR000DM.selT430CausaliAbilitate.Active) or
     (not WR000DM.selT430RaggrAssenzeAbilitati.Active)
  then
  begin //Evita che sia eseguito quando non necessario
    ...
    Caus:=Trim(Copy(cmbCausale.Text,1,5)); //CAUSALE CORRENTE
    rgpCausaliClick(nil);
    IndCaus:=R180IndexOf(cmbCausale.Items,Caus,5); //RIPOSIZIONA SULLA CAUSALE CORRENTE
    if IndCaus >= 0 then
      cmbCausale.ItemIndex:=IndCaus;
  end;
...
procedure TW008FGiustificativi.btnNascondiRiepilogoAsyncClick(Sender: TObject; EventParams: TStringList);
begin
...
begin
  ...
  begin
    A004MW.R600DtM1.RiepilogaAssenze(Prog,dDataRiep,G,False,0); (*Passo 0 perchè qui non serve questo parametro*)
  end;
...
begin
  ...
  // oggetto per chiamate webservice firlab
...
procedure TW008FGiustificativi.edtDataAsyncChange(Sender: TObject; EventParams: TStringList);
var D: TDateTime;
begin
  inherited;
