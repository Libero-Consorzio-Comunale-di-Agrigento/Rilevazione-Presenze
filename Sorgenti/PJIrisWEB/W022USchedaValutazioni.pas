...
uses
  ...
  W022USchedaValutazioniDtM, W022URiepilogoSchedeFM, W022UValutatoFM,
  IWVCLBaseContainer, IWContainer, medpIWMessageDlg,
  ...
  Data.DBXJSON,{$IF CompilerVersion >= 31}System.JSON,{$ENDIF} ActiveX;
...
type
  TW022FSchedaValutazioni = class(TR012FWebAnagrafico)
    ...
    procedure imgCommentiClick(Sender: TObject);
  ...
  private
    ...
    function RecuperaHintCommVal: String;
    procedure VisualizzaStampa;
    procedure VerificaRicezionePdf;
    procedure MostraAvvisoPresaVisione(Sender: TObject; PRes: TmeIWModalResult; PKeyID: String);
    procedure PresaVisioneEVisualizza;
    procedure ImpostaPresaVisioneSi;
    procedure ImpostaPresaVisioneNo;
    procedure ImpostaDataConsegna(Esito: String);
    procedure ProtocollazioneImmediata;
    procedure CallDCOMPrinter(Tipo:String);
    function CreateJSonStringPDFA: String;
    function CreateJSonStringProt: String;
  protected
...
begin
  ...
  begin
    selAnagrafeW.SQL.Text:=StringReplace(selAnagrafeW.SQL.Text,':DATALAVORO','greatest(least(nvl(V430.T430FINE,:DataLavoro),:DataLavoro),trunc(:DataLavoro,''yyyy''))',[rfReplaceAll,rfIgnoreCase]);
...
begin
  ...
  dgrdSchede.medpAggiungiColonna('PUNTEGGIO_FINALE_PESATO','Punteggio scheda','',nil);
  ...
  dgrdSchede.medpAggiungiColonna('PERIODO_REGISTRA_VISIONE','Periodo registrazione presa visione','',nil);
  ...
  dgrdSchede.medpColonna('PERIODO_REGISTRA_VISIONE').Visible:=VisColPRPV;

  dgrdSchede.medpAggiungiRowClick('DATA',DBGridColumnClick);
  dgrdSchede.medpInizializzaCompGriglia;
  if not SolaLettura then
  begin
    dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_COMANDI'),0,DBG_IMG,'','INSERISCI','null','','S');
    dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_COMANDI'),1,DBG_IMG,'','CANCELLA','null','null','S');
    dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_COMANDI'),2,DBG_IMG,'','ACCEDI','Accedi','','D');
  end;
  dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_COMANDI2'),0,DBG_IMG,'','STAMPA','Stampa','','');
  dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_COMANDI2'),1,DBG_IMG,'','ELENCO','Commenti','','D');
  dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_COMANDI2'),2,DBG_IMG,'','DUPLICA','Crea autovalutazione','','D');
  dgrdSchede.medpPreparaComponenteGenerico('R',dgrdSchede.medpIndexColonna('DBG_ALLEGATI'),0,DBG_IMG,'','ALLEGATI','Informazioni','','');
  dgrdSchede.medpCaricaCDS;
end;

procedure TW022FSchedaValutazioni.PopolaGriglia;
var AnnoMin,AnnoMax,AnnoRich,Anno:Integer;
  DataDaComp,DataAComp,DataScheda,DataDaRegistraVisione,DataARegistraVisione:TDateTime;
  //Interrompi:Boolean;
begin
  with W022DtM do
  begin
    ...
    begin
      ...
      begin
        ...
        if SoloStampa
        and (R180In(Parametri.WEBRegistraConsegnaVal,['R','S']) or (Parametri.S710_SupervisoreValut = 'S')) then
        begin
          DataDaRegistraVisione:=0;
          DataARegistraVisione:=0;
          if VarToStr(selSG746.Lookup('CODICE',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger,'DATA_DA_RICHIESTA_VISIONE')) <> '' then
          begin
            DataDaRegistraVisione:=selSG746.Lookup('CODICE',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger,'DATA_DA_RICHIESTA_VISIONE');
            selQ710.FieldByName('PERIODO_REGISTRA_VISIONE').AsString:=FormatDateTime('dd/mm/yyyy',DataDaRegistraVisione);
            if DataDaRegistraVisione > EncodeDate(1900,1,1) then
              VisColPRPV:=True;
          end;
          if VarToStr(selSG746.Lookup('CODICE',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger,'DATA_A_RICHIESTA_VISIONE')) <> '' then
          begin
            DataARegistraVisione:=selSG746.Lookup('CODICE',Q710.FieldByName('STATO_AVANZAMENTO').AsInteger,'DATA_A_RICHIESTA_VISIONE');
            selQ710.FieldByName('PERIODO_REGISTRA_VISIONE').AsString:=selQ710.FieldByName('PERIODO_REGISTRA_VISIONE').AsString + ' - ' + FormatDateTime('dd/mm/yyyy',DataARegistraVisione);
            if DataARegistraVisione < EncodeDate(3999,12,31) then
              VisColPRPV:=True;
          end;
          if (DataDaRegistraVisione = EncodeDate(1900,1,1)) and (DataARegistraVisione = EncodeDate(3999,12,31)) then
            selQ710.FieldByName('PERIODO_REGISTRA_VISIONE').AsString:='sempre';
        end;
        selQ710.Post;
        selSchedeAnno.Next;
      end;
...
begin
  ...
  W022DtM.CaricaCDS;
  W022RiepilogoSchedeFM.Visualizza(DataLavoro, chkVisualizzaDipNonValutabili.Checked);
end;
...
begin
  ...
  NomeFile:=W022.NomeFile;
  VisualizzaStampa;
  W022.ClosePage;
end;

procedure TW022FSchedaValutazioni.imgCommentiClick(Sender: TObject);
begin
  inherited;
  if not (Sender is TmeIWImageFile) then
    Exit;
  ...
  W022ValFM:=TW022FValutatoFM.Create(Self);
  W022ValFM.W022DtM4:=W022DtM;
  W022ValFM.Visualizza;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        (dgrdSchede.medpCompCella(i,1,1) as TmeIWImageFile).OnClick:=imgCommentiClick;
        (dgrdSchede.medpCompCella(i,1,1) as TmeIWImageFile).ImageFile.FileName:=IfThen(Q710.FieldByName('COMMENTI_VALUTATO').IsNull and Q710.FieldByName('ACCETTAZIONE_VALUTATO').IsNull,fileImgElenco,fileImgElencoHighlight);
        (dgrdSchede.medpCompCella(i,1,1) as TmeIWImageFile).Hint:=RecuperaHintCommVal;
        (dgrdSchede.medpCompCella(i,1,2) as TmeIWImageFile).OnClick:=imgCreaAutovalutazioneClick;
        ...
        //Condizioni che non permettono i commenti del valutato
        ...
        or (Q710.FieldByName('TIPO_VALUTAZIONE').AsString = 'A') //autovalutazione
        or (Pos('P4',selSG741.FieldByName('PAGINE_ABILITATE').AsString) <= 0) //pagina commenti valutato non abilitata
        or (selSG741.FieldByName('COMMENTI_VALUTATO_DIP').AsString = 'N') //commenti del valutato gestiti dal valutatore
        or SolaLettura
        then
        ...
        then
          (dgrdSchede.medpCompGriglia[i].CompColonne[1] as TmeIWGrid).Cell[0,2].Css:='invisibile'
        else
        begin
          selSG710.Close;
          selSG710.SetVariable('PROGRESSIVO',Q710.FieldByName('PROGRESSIVO').AsInteger);
...
begin
  ...
  if (ARow > 0) and R180In(dgrdSchede.medpNumColonna(AColumn),[dgrdSchede.medpIndexColonna('D_DATA'),
                                                               ...
                                                               dgrdSchede.medpIndexColonna('PERIODO_REGISTRA_VISIONE')])
  then
  ...
  begin
    ...
    //Aggiorno l'icona in caso di variazione dei commenti del valutato quando torno dalla W022ValFM
    if (NumColonna = dgrdSchede.medpIndexColonna('DBG_COMANDI2'))
    and (dgrdSchede.medpValoreColonna(ARow - 1,'DATA') = W022DtM.Q710.FieldByName('DATA').AsString)
    and ((ACell.Control as TmeIWGrid).Cell[0,1].Css <> 'invisibile')
    and (dgrdSchede.medpCompCella(ARow - 1,NumColonna,1) is TmeIWImageFile)
    then
    begin
      (dgrdSchede.medpCompCella(ARow - 1,NumColonna,1) as TmeIWImageFile).ImageFile.FileName:=IfThen(W022DtM.Q710.FieldByName('COMMENTI_VALUTATO').IsNull and W022DtM.Q710.FieldByName('ACCETTAZIONE_VALUTATO').IsNull,fileImgElenco,fileImgElencoHighlight);
      (dgrdSchede.medpCompCella(ARow - 1,NumColonna,1) as TmeIWImageFile).Hint:=RecuperaHintCommVal;
    end;
  end;
end;

function TW022FSchedaValutazioni.RecuperaHintCommVal: String;
var CommVal:String;
begin
  Result:=W022DtM.RecuperaEtichetta('COMMENTI_VALUTATO_C');
  CommVal:=Trim(Trim(IfThen(W022DtM.Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'S','Accettata.',IfThen(W022DtM.Q710.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'N','Non accettata.',''))) + ' ' + Trim(Copy(W022DtM.Q710.FieldByName('COMMENTI_VALUTATO').AsString,1,30) + IfThen(Copy(W022DtM.Q710.FieldByName('COMMENTI_VALUTATO').AsString,31) <> '','...')));
  if CommVal <> '' then
    Result:=Result + ': ' + CommVal;
end;
...
procedure TW022FSchedaValutazioni.VisualizzaStampa;
var AvvisoPV:String;
begin
  ...
  begin
    ...
    if W022DtM.PresaVisioneRegistrabile then
    begin
      AvvisoPV:=Trim(W022DtM.selSG741.FieldByName('TESTO_AVVISO_PRESA_VISIONE').AsString);
      if Parametri.WEBRegistraConsegnaVal = 'R' then //Registro la presa visione dopo conferma di visione dell'utente
        VisualizzaFile(NomeFile,'Anteprima stampa valutazione',nil,VerificaRicezionePdf)
      else if Pos('?',AvvisoPV) > 0 then //Registro la presa visione se l'utente conferma la volontà di stampare
        Messaggio('Conferma',AvvisoPV,PresaVisioneEVisualizza,nil)
      else if AvvisoPV <> '' then //Registro la presa visione dopo lettura avviso
        MsgBox.WebMessageDlg(AvvisoPV,mtInformation,[mbOK],MostraAvvisoPresaVisione,'','Avviso')
      else //Registro la presa visione senza chiedere
        PresaVisioneEVisualizza;
    end
    else //Non registro la presa visione
...
procedure TW022FSchedaValutazioni.MostraAvvisoPresaVisione(Sender: TObject; PRes: TmeIWModalResult; PKeyID: String);
begin
  if PRes = mrOK then
    PresaVisioneEVisualizza;
end;

procedure TW022FSchedaValutazioni.PresaVisioneEVisualizza;
begin
  VisualizzaFile(NomeFile,'Anteprima stampa valutazione',ImpostaPresaVisioneSi,nil);
end;

procedure TW022FSchedaValutazioni.ImpostaPresaVisioneSi;
begin
  ImpostaDataConsegna('S');
  ProtocollazioneImmediata;
end;
...
begin
  ...
  begin
    ...
    if DataConsegna > 0 then
      FieldByName('DATA_CONSEGNA').AsDateTime:=DataConsegna //Uso la data di presa visione recuperata prima di creare il documento
    else
...
procedure TW022FSchedaValutazioni.ProtocollazioneImmediata;
begin
  with W022DtM do
  begin
    selSG750.Open;
    //Condizioni per protocollazione immediata:
    //dipendente valutabile
    if (Q710.FieldByName('VALUTABILE').AsString = 'S')
    //scheda definitiva
    and (Q710.FieldByName('CHIUSO').AsString = 'S')
    //presa visione del valutato (attualmente questa procedure viene richiamata soltanto al momento della presa visione positiva)
    and (Q710.FieldByName('PROGRESSIVO').AsInteger = Parametri.ProgressivoOper)
    //scheda non protocollata
    and Q710.FieldByName('NUMERO_PROTOCOLLO').IsNull
    and Q710.FieldByName('ANNO_PROTOCOLLO').IsNull
    and Q710.FieldByName('DATA_PROTOCOLLO').IsNull
    //flag per invio all'atto della presa visione attivato
    and (VarToStr(selSG750.Lookup('CODICE',selSG741.FieldByName('COD_PARPROTOCOLLO').AsString,'INVIO_CONSEGNA')) = 'S') then
    begin
      IdAnomalia:='';
      try
        try
          CallDCOMPrinter('PDFA'); //Creazione file PDF/A
          if IdAnomalia = '' then
            CallDCOMPrinter('Prot'); //Protocollazione
        except
          IdAnomalia:='ERROREGENERICO'; //forzo invio email
        end;
      finally
        if IdAnomalia <> '' then
          //invio email errore protocollazione
          InviaEMailErrProt
        else
          //refresh per vedere dati protocollazione
          Q710.RefreshRecord;
      end;
    end;
    selSG750.Close;
...
procedure TW022FSchedaValutazioni.CallDCOMPrinter(Tipo:String);
var DettaglioLog, MessaggioAggiuntivo:OleVariant;
    DatiUtente: String;
begin
  DCOMNomeFile:='';
  if Tipo = 'PDFA' then
    DatiUtente:=CreateJSonStringPDFA
  else //Tipo = 'Prot'
    DatiUtente:=CreateJSonStringProt;
  ...
  try
    ...
    DCOMConnection1.AppServer.PrintS715(selAnagrafeW.SubstitutedSQL,
                                       DCOMNomeFile,
                                       Parametri.Operatore,
                                       Parametri.Azienda,
...
function TW022FSchedaValutazioni.CreateJSonStringPDFA: String;
...
begin
  ...
  with W022DtM do
    try
      ...
      json.AddPair('edtDataDa',Q710.FieldByName('DATA').AsString);
      json.AddPair('edtDataA',Q710.FieldByName('DATA').AsString);
      json.AddPair('rgpTipoValutazione',IfThen(Q710.FieldByName('TIPO_VALUTAZIONE').AsString = 'V','0','1')); //Valutazione/Autovalutazione
      json.AddPair('rgpTipoChiusura','2'); //Scheda definitiva
      json.AddPair('rgpStatoAvanzamento','0'); //Attuale
      json.AddPair('edtStatoAvanzamento','');
      json.AddPair('rgpDipValutabile','0'); //Sì
      json.AddPair('rgpPresaVisione','0'); //Sì
      json.AddPair('rgpSchedeProtocollo','1'); //No
      json.AddPair('chkControllaRegola','N');
      json.AddPair('chkAggiornaPunteggi','N');
      json.AddPair('chkAvanzaStato','N');
      json.AddPair('chkChiudiScheda','N');
      json.AddPair('chkRiapriScheda','N');
      json.AddPair('chkRetrocediStato','N');
      json.AddPair('chkAggiornaIncentivi','N');
      json.AddPair('chkStampa','N');
      json.AddPair('chkFilePDF','S'); //Sì
      json.AddPair('chkProtocolla','N'); //No
      json.AddPair('chkSostituisciRegola','N');
      json.AddPair('chkAssegnaValutatore','N');
      json.AddPair('chkLegendaPunteggi','N');
      json.AddPair('chkPresaVisione','S'); //Sì
      json.AddPair('ListaTipologieQuote','');
...
function TW022FSchedaValutazioni.CreateJSonStringProt: String;
var json: TJSONObject;
begin
  json:=TJSONObject.Create;
  with W022DtM do
    try
      ...
      json.AddPair('rgpSchedeProtocollo','1'); //No
      json.AddPair('chkControllaRegola','N');
      //TEMP json.AddPair('chkControllaRegola','S'); //TEMP per DEBUG: per non protocollare davvero, commentare la riga precedente e scommentare questa!!!!!
      json.AddPair('chkAggiornaPunteggi','N');
      json.AddPair('chkAvanzaStato','N');
      json.AddPair('chkChiudiScheda','N');
      json.AddPair('chkRiapriScheda','N');
      ...
      json.AddPair('chkAggiornaIncentivi','N');
      json.AddPair('chkStampa','N');
      json.AddPair('chkFilePDF','N'); //No
      json.AddPair('chkProtocolla','S'); //Sì
      //TEMP json.AddPair('chkProtocolla','N'); //TEMP per DEBUG: per non protocollare davvero, commentare la riga precedente e scommentare questa!!!!!
      json.AddPair('chkSostituisciRegola','N');
      json.AddPair('chkAssegnaValutatore','N');
      json.AddPair('chkLegendaPunteggi','N');
      json.AddPair('chkPresaVisione','N');
      json.AddPair('ListaTipologieQuote','');
