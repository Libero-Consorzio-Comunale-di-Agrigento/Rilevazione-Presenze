...
uses
  ...
  IWTypes, IWCompExtCtrls, Generics.Collections,
  IWBaseInterfaces, IWHTMLTag, IWRenderContext,Menus,
  meIWEdit, meIWComboBox, medpIWMultiColumnComboBox, meIWMemo, A000UMessaggi;
...
type
  ...
  public
    ...
    procedure medpPreparaComponenteGenerico(TipoRiga:String; i,j:Integer; Tipo:String; Formato:String; Elementi:String; Hint:String; Confirmation:String; Allineamento:String = ''; Grid: Boolean = False); overload;
    procedure medpPreparaComponenteGenerico(TipoRiga:String; NomeCampo:String; j:Integer; Tipo:String; Formato:String; Elementi:String; Hint:String; Confirmation:String; Allineamento:String = ''; Grid: Boolean = False); overload;
    function  GetStile(PComponente: TComponenti): String;
    procedure medpCreaComponenteGenerico(nr,nc:Integer; Componenti: array of TComponenti); overload;
    procedure medpCreaComponenteGenerico(nr:Integer; NomeCampo:String; Componenti: array of TComponenti); overload;
    ...
    function  SetCurrRecord:Boolean;
    function  GetCurrRecord:Boolean;
    procedure medpDisabilitaNavBar;
    procedure medpAbilitaNavBar;
    ...
    procedure medpInserisciClick(Sender: TObject);
...
begin
  ...
  AfterScroll:=medpDataset.AfterScroll; //[IW15.2.48][22]
  try
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //BugFix CHIOGGIA_COMUNE Chiamata: 157103
          Valore:=StringReplace(Valore,CRLF,' ', [rfReplaceAll, rfIgnoreCase]);
          Valore:=StringReplace(Valore,BB,'', [rfReplaceAll, rfIgnoreCase]);
          //BugFix fine
    ...
    medpDataset.AfterScroll:=AfterScroll;
  end;
end;

function TmedpIWDBGrid.ToXlsx: TStream;
var
  Campo: String;
  i: Integer;
  Col: TIWDBGridColumn;
...
begin
  ...
  Result:=R180DatasetToXlsx(SessioneOracle,True,medpDataset,ListaColonne);
end;
...
procedure TmedpIWDBGrid.medpAbilitaNavBar;
begin
  grdNavBar.Browse:=True;
end;

procedure TmedpIWDBGrid.medpDisabilitaNavBar;
begin
  grdNavBar.Browse:=False;
end;
...
function TmedpIWDBGrid.medpReadOnly:Boolean;
begin
  Result:=False;
  if (medpDataSet is TOracleDataSet) and TOracleDataSet(medpDataSet).ReadOnly then
    Result:=True;
end;
...
begin
  ...
  try
    if medpDataset is TClientDataSet then
      medpClientDataset.DisableStringTrim:=TClientDataset(medpDataset).DisableStringTrim;
  except
  end;
...
begin
  if InsertOnDataset and (medpDataSet.State <> dsInsert) then
    medpDataSet.Insert;
...
procedure TmedpIWDBGrid.medpInserisciClick(Sender: TObject);
begin
  // creato per richiamare la procedura "Protected" imgInserisciClick
  imgInserisciClick(Sender);
end;
...
var
  ...
  FieldEsistenti:array of TFieldEsistenti;
begin
  medpBrowse:=True;

  SetLength(FieldEsistenti,0);
  if medpDataSet.Active then
  begin
    SetLength(FieldEsistenti,medpDataSet.FieldCount);
    for i:=0 to medpDataSet.FieldCount - 1 do
    begin
      FieldEsistenti[i].FieldName:=medpDataSet.Fields[i].FieldName;
      FieldEsistenti[i].DisplayLabel:=medpDataSet.Fields[i].DisplayLabel;
      FieldEsistenti[i].Visible:=medpDataSet.Fields[i].Visible;
    end;
  end;
  ...
  if medpDataSet.FieldCount > 0 then
    SetLength(FieldEsistenti,0);
  //A questo punto il dataset deve essere per forza aperto
  medpDataSet.Open;
  for i:=0 to High(FieldEsistenti) do
  begin
    if medpDataSet.Fields[i].FieldName = FieldEsistenti[i].FieldName then
    begin
      medpDataSet.Fields[i].DisplayLabel:=FieldEsistenti[i].DisplayLabel;
      medpDataSet.Fields[i].Visible:=FieldEsistenti[i].Visible;
    end;
  end;
  ...
  begin
    ...
    if Pos('medpDettaglioFMfixed',css) > 0 then
    begin
      if medpColonna(medpDataSet.Fields[i].FieldName).Visible then
      begin
        //imposto la larghezze
        if medpDataSet.Fields[i].DisplayWidth > 0 then
          medpColonna(medpDataSet.Fields[i].FieldName).Width:=IntToStr(medpDataSet.Fields[i].DisplayWidth * 4) (*fattore di proporzione carattere-->pixel*);
      end;
    end;
  end;
  ...
  try
    ...
    try
      ...
      begin
        ...
        else if medpClientDataset.Fields[i] is TFloatField then
          TFloatField(medpClientDataset.Fields[i]).DisplayFormat:=TFloatField(medpDataset.Fields[i]).DisplayFormat;
      end;
...
const
  NOME_FUNZIONE = 'TmedpIWDBGrid.medpPreparaComponenteGenerico';
begin
  // aiuti per debug
  if DebugHook <> 0 then
  begin
    if i < 0 then
      raise Exception.Create(Format('%s: riga %d non valida!',[NOME_FUNZIONE,i]));
    if j < 0 then
      raise Exception.Create(Format('%s: colonna %d non valida!',[NOME_FUNZIONE,j]));
  end;
  ...
  begin
    ...
    medpDescCompGriglia.Riga[i,j].MaxLength:=-1;
  end
  ...
  begin
    ...
    medpDescCompGriglia.RigaIns[i,j].MaxLength:=-1;
  end
  ...
  begin
    ...
    medpDescCompGriglia.RigaWR102[i,j].MaxLength:=-1;
  end
  ...
  begin
    ...
    medpDescCompGriglia.RigaWR102R[i,j].MaxLength:=-1;
  end
  ...
  begin
    ...
    medpDescCompGriglia.RigaWR102I[i,j].MaxLength:=-1;
  end;
end;

procedure TmedpIWDBGrid.medpPreparaComponenteGenerico(TipoRiga:String; NomeCampo:String; j:Integer; Tipo:String; Formato:String; Elementi:String; Hint:String; Confirmation:String; Allineamento:String = ''; Grid: Boolean = False);
var
  IndCol:Integer;
begin
  IndCol:=Self.medpIndexColonna(NomeCampo);
  if IndCol < 0 then
  begin
    raise Exception.Create(Format('Colonna "%s" non trovata all''interno della grid "%s".',[NomeCampo,Name]));
  end;
  Self.medpPreparaComponenteGenerico(TipoRiga,IndCol,j,Tipo,Formato,Elementi,Hint,Confirmation,Allineamento,Grid);
end;
...
begin
  ...
  begin
    ...
    begin
      // memo
      IWC:=C190DBGridCreaMemo(Possessore,Contenitore,medpCompGriglia[nr].RowID,Stile,Componenti[i].MaxLength)
    end
...
procedure TmedpIWDBGrid.medpCreaComponenteGenerico(nr:Integer; NomeCampo:String; Componenti: array of TComponenti);
var
  IndCol:Integer;
begin
  IndCol:=Self.medpIndexColonna(NomeCampo);
  ...
  medpCreaComponenteGenerico(nr, IndCol, Componenti);
end;
...
begin
  ...
  if (DebugHook <> 0) and (ARow = 0) and (Intestazione) then
  begin
    ACell.Hint:=medpColonna(NumColonna).DataField;
    ACell.ShowHint:=True;
  end;
...
function TmedpIWDBGrid.SetCurrRecord:Boolean;
var i,j,d:Integer;
begin
  Result:=False;
  FCurrRecord.RI:='';
  if medpDataSet.RecordCount = 0 then
    exit;
  if (medpDataSet is TOracleDataSet) and ((medpDataSet as TOracleDataSet).RowID <> '') and (medpClientDataSet <> nil) then
    FCurrRecord.RI:=medpClientDataSet.FieldByName('DBG_ROWID').AsString
  else
  begin
    d:=-1;
    for i:=0 to medpDataSet.FieldCount - 1 do
    ...
    if d < 0 then
      exit;
    FCurrRecord.VarValori:=VarArrayCreate([0,d],VarVariant);
    FCurrRecord.ElencoCampi:='';
    j:=-1;
    for i:=0 to medpDataSet.FieldCount - 1 do
      if medpDataSet.Fields[i].FieldKind in [fkData,fkInternalCalc] then
      begin
        inc(j);
        FCurrRecord.VarValori[j]:=medpDataSet.Fields[i].Value;
        FCurrRecord.ElencoCampi:=FCurrRecord.ElencoCampi + medpDataSet.Fields[i].FieldName + ';';
      end;
  end;
  Result:=True;
end;

function TmedpIWDBGrid.GetCurrRecord:Boolean;
begin
  Result:=False;
  if (medpDataSet is TOracleDataSet) and (TOracleDataSet(medpDataSet).RowID <> '') and (FCurrRecord.RI <> '') then
  begin
    if medpDataSet.Locate('ROWID',FCurrRecord.RI,[]) then
      Result:=True;
  end
  else
  begin
    ...
    if (medpDataSet.RecordCount > 0) and (FCurrRecord.ElencoCampi <> '') then
    begin
      if medpDataSet.Locate(FCurrRecord.ElencoCampi,FCurrRecord.VarValori,[]) then
        Result:=True;
    end;
    VarClear(FCurrRecord.VarValori);
  end;
end;

procedure TmedpIWDBGrid.medpColumnTitleClick(Sender: TObject);
var Idx_medpSortField:String;
    Old_medpSortField:String;
    Old_SortAsc:Boolean;
begin
  ...
  //Memorizzo dati per gestione eventuale di colonna ambigua
  Old_medpSortField:=medpSortField;
  Old_SortAsc:=FSortAsc;
  Idx_medpSortField:='';
  medpSortField:=(Sender as TIWDBGridColumn).DataField;
  if (medpDataSet.FieldDefList.Find(medpSortField) <> nil) then
    Idx_medpSortField:=(medpDataSet.FieldDefList.Find(medpSortField).Index + 1).ToString;
  ...
  try
    ...
    on E:EOracleError do
      //Colonna ambigua nell'ordinamento: servirebbe il suffisso ma non è possibile ricavarlo in generale
      if Pos('ORA-00918',E.Message) > 0 then
      begin
        //Provo ad ordinarlo col numero d'ordine della colonna nella query
        if Idx_medpSortField <> '' then
        begin
          if Old_medpSortField = Idx_medpSortField then
          begin
            FSortField:=Old_medpSortField;
            FSortAsc:=Old_SortAsc;
          end;
          medpSortField:=Idx_medpSortField;
        end
        else
          //Forzo l'ordinamento per la prima colonna
          medpSortField:='1';
        //medpDataSet.Refresh;
        //medpDataSet.Close;
        medpDataSet.Open;
...
var i:Integer;
  ...
  begin
    ...
    else if medpCompCella(Row,i,0) is TMeIWMemo then
    begin
      (medpCompCella(Row,i,0) as TMeIWMemo).Text:=IfThen(medpStato = msInsert,medpDataSet.FieldByName(Campo).AsString,medpClientDataSet.FieldByName(Campo).AsString);
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          //Impostazione MaxLength per componenti Memo
          if (medpDescCompGriglia.RigaWR102[i][0].Tipo = DBG_MEMO) and
             (medpDataSet.FindField(Campo) <> nil) and
             (medpDataSet.FieldByName(Campo) is TStringField)
          then
            medpDescCompGriglia.RigaWR102[i][0].MaxLength:=medpDataSet.FieldByName(Campo).Size;
...
//  medpColumnClick(Sender,TmeIWImageFile(Sender).FriendlyName);
  ...
  begin
    ...
    // (migliorata gestione eccezioni con un messagebox più significativo)
    try
      OnConferma(Sender);
    except
      on E: EAbort do
      ...
      begin
        {$IFNDEF BPL}
        MsgBox.MessageBox(E2.Message,ERRORE,'Errore conferma');
        {$ENDIF}
        Exit;
      end;
    end;
...
// Riporta i valori dai componenti al dataset effettuando le operazioni di Append o Edit
  procedure SetValore(Campo,Valore:String);
  var keyF:String;
  begin
    if (medpDataset.FieldByName(Campo).FieldKind = fkLookup) and
       (medpDataset.FindField(medpDataset.FieldByName(Campo).KeyFields) <> nil) then
    begin
      keyF:=medpDataset.FieldByName(Campo).KeyFields;
      if (not medpDataset.FieldByName(keyF).Visible) or
         (medpDataset.FieldByName(keyF).ReadOnly) or
         (medpCompCella(Row, keyF, 0) = nil) or
         (medpCompCella(Row, keyF, 0) is TmeIWLabel) then
        medpDataset.FieldByName(keyF).AsString:=VarToStr(medpDataset.FieldByName(Campo).LookupDataSet.Lookup(medpDataset.FieldByName(Campo).LookupResultField, Valore, medpDataset.FieldByName(Campo).LookupKeyFields));
    end
    else if not Assigned(medpDataSet.FindField(Campo).OnSetText) then
      medpDataSet.FindField(Campo).AsString:=Valore
    else
      medpDataSet.FindField(Campo).Text:=Valore;
  end;
var
  i: Integer;
begin
  if medpCompGriglia <> nil then
  begin
    for i:=0 to High(medpCompGriglia[Row].CompColonne) do
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            //Usato Text invece che AsString per sfruttare l'evento OnSetText del dataset//medpDataSet.FindField(medpColonna(i).DataField).AsString:=(medpCompCella(Row,i,0) as TmeIWEdit).Text
            SetValore(medpColonna(i).DataField,(medpCompCella(Row,i,0) as TmeIWEdit).Text);
        end
        else if medpCompCella(Row,i,0) is TmeIWComboBox then
          //Usato Text invece che AsString per sfruttare l'evento OnSetText del dataset//medpDataSet.FindField(medpColonna(i).DataField).AsString:=(medpCompCella(Row,i,0) as TmeIWComboBox).Text
          SetValore(medpColonna(i).DataField,(medpCompCella(Row,i,0) as TmeIWComboBox).Text)
  ...
  //        medpDataSet.FindField(medpColonna(i).DataField).AsString:=(medpCompCella(Row,i,0) as TmeTIWMultiColumnComboBox).Text
        //CARATTO 16/05/2013 gestione medpMulticolumnCombobox
          ...
          //Usato Text invece che AsString per sfruttare l'evento OnSetText del dataset//medpDataSet.FindField(medpColonna(i).DataField).AsString:=(medpCompCella(Row,i,0) as TmedpIWMultiColumnComboBox).Text;
          SetValore(medpColonna(i).DataField,(medpCompCella(Row,i,0) as TmedpIWMultiColumnComboBox).Text)
        else if medpCompCella(Row,i,0) is TmeIWMemo then
          SetValore(medpColonna(i).DataField,(medpCompCella(Row,i,0) as TmeIWMemo).Text);
      end;
    end;
  end;
  if Length(medpDescCompGriglia.RigaWR102) > 0 then
