...
begin
  ...
  SQL.Add('  FROM');
  SQL.Add('  (SELECT COLUMN_NAME,DATA_TYPE,DATA_LENGTH,TABLE_NAME,COLUMN_ID,DATA_DEFAULT FROM COLS WHERE TABLE_NAME IN (''T030_ANAGRAFICO'',''V430_STORICO'') AND NOT (TABLE_NAME = ''T030_ANAGRAFICO'' AND COLUMN_NAME = ''RAPPORTI_UNITI'')');
  SQL.Add('   UNION ALL');
  ...
  if UsaCampiI060 then
  begin
    SQL.Add('   UNION ALL SELECT ''I060EMAIL'',''VARCHAR2'',100,''I060_LOGIN_DIPENDENTI'',9999,NULL FROM DUAL');
    SQL.Add('   UNION ALL SELECT ''I060EMAILPEC'',''VARCHAR2'',100,''I060_LOGIN_DIPENDENTI'',9999,NULL FROM DUAL');
    SQL.Add('   UNION ALL SELECT ''I060EMAILPERSONALE'',''VARCHAR2'',100,''I060_LOGIN_DIPENDENTI'',9999,NULL FROM DUAL');
    SQL.Add('   UNION ALL SELECT ''I060CELLULARE'',''VARCHAR2'',100,''I060_LOGIN_DIPENDENTI'',9999,NULL FROM DUAL');
    SQL.Add('   UNION ALL SELECT ''I060CELLULAREPERSONALE'',''VARCHAR2'',100,''I060_LOGIN_DIPENDENTI'',9999,NULL FROM DUAL');
  end;
  SQL.Add('   )');
  //SQL.Add('  ,I010_CAMPIANAGRAFICI I010');
  SQL.Add(',(SELECT APPLICAZIONE,NOME_CAMPO,NOME_LOGICO,RICERCA,POSIZIONE,VAL_DEFAULT,PROVVEDIMENTO FROM I010_CAMPIANAGRAFICI');
  SQL.Add(') I010');
  if Layout <> '' then
  ...
  Session:=Sessione;
  //ReadBuffer:=CountQueryHits + 1;
  R180SetReadBuffer(Self);
  Open;
end;
