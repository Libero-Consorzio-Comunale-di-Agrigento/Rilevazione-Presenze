...
begin
  try
    Result:=0;
    case FormatoData of
      fdGiorno: TryStrToDate(edtFine.Text,Result);
      fdMese:   if TryStrToDate('01/' + edtFine.Text,Result) then Result:=R180FineMese(Result);//Result:=R180FineMese(TryStrToDate('01/' + edtFine.Text));
      fdAnno:   TryStrToDate('31/12/' + edtFine.Text,Result);//Result:=StrToDate('31/12/' + edtFine.Text);
    end;
  except
    Result:=0;
  end;
