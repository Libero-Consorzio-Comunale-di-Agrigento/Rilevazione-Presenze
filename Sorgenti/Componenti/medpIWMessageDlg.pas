...
type
  ...
  private
    ...
    procedure Execute;
    function GetLarghezzaFinestra(Msg: String): Integer;
  protected
    FActive: Boolean;
...
begin
  ...
  if Assigned(AOwner) and (not (AOwner is TWR010FBase)) then
    raise Exception.Create(Format('%s: questo componente può essere utilizzato solo in un form di classe %s!',
                                  [Self.ClassName,TWR010FBase.ClassName]));
...
begin
  try FreeAndNil(FElementText); except end;
  try FreeAndNil(lstControlli); except end;
  try FreeAndNil(jQMsg); except end;
  SetLength(MultipleMessages,0);
  inherited Destroy;
end;
...
begin
  ...
  grd.Name:=GRD_MSGBOX_NAME;
  grd.DlgComponent:=Self;

  // testo messaggio
  grd.Cell[0,0].RawText:=TextIsHTML;
  grd.Cell[0,0].Text:=FText;

  // icona tipo messaggio
  case FDlgType of
    mtWarning:       grd.Cell[0,0].Css:='medpMsg warning';
    mtError:         grd.Cell[0,0].Css:='medpMsg error';
    mtInformation:   grd.Cell[0,0].Css:='medpMsg information';
    mtConfirmation:  grd.Cell[0,0].Css:='medpMsg confirmation';
  ...
  if mbIgnore in FButtons then
    grd.AddButton(FElementText.FBtnTxtIgnore,mbIgnore = FDefButton);
  if mbAbort in FButtons then
    grd.AddButton(FElementText.FBtnTxtAbort,mbAbort = FDefButton);
  if mbRetry in FButtons then
    grd.AddButton(FElementText.FBtnTxtRetry,mbRetry = FDefButton);
  if mbCancel in FButtons then
    grd.AddButton(FElementText.FBtnTxtCancel,mbCancel = FDefButton);
  if mbNo in FButtons then
    grd.AddButton(FElementText.FBtnTxtNo,mbNo = FDefButton);
  if mbNoToAll in FButtons then
    grd.AddButton(FElementText.FBtnTxtNoToAll,mbNoToAll = FDefButton);
  if mbAll in FButtons then
    grd.AddButton(FElementText.FBtnTxtAll,mbAll = FDefButton);
  if mbYesToAll in FButtons then
    grd.AddButton(FElementText.FBtnTxtYesToAll,mbYesToAll = FDefButton);
  if mbYes in FButtons then
    grd.AddButton(FElementText.FBtnTxtYes,mbYes = FDefButton);
  if mbOK in FButtons then
    grd.AddButton(FElementText.FBtnTxtOk,mbOK = FDefButton);

  // imposta la visibilità della grid
  grd.Visible:=True;
  ...
  if grd.GetDefaultButton = nil then
    CodOpen:=''
  else
  begin
    // metodo html5 con autofocus
    CodOpen:=Format('$("#%s").attr("autofocus","autofocus"); ',[grd.GetDefaultButton.HTMLName]);
    // metodo focus su componente (non funziona!)
    //CodOpen:=Format('$("#%s").focus(); ',[grd.GetDefaultButton.HTMLName]);
  end;
  ...
  *)
  (Self.Owner as TWR010FBase).VisualizzaJQMessaggio(jQMsg,GetLarghezzaFinestra(FText),-1,EM2PIXEL * 30,FTitle,'#msgDialog',False,True,-1,'',CodOpen);
  FActive:=True;
end;

function TmedpIWMessageDlg.GetLarghezzaFinestra(Msg: String):Integer;
...
procedure TmedpIWMessageDlg.CancelMessageBox;
// elimina il messagebox eventualmente impostato
var
  IWC: TComponent;
begin
  // esce subito se il messagebox non è attivo
  if not IsActive then
    Exit;

  // non è prevista per ora la gestione con il thread di elaborazione separato
  if ThreadElaborazione <> nil then
  begin
    raise Exception.Create('CancelMessageBox: non implementato per gestione con thread di elaborazione separato');
    //GestioneThreadElaborazione; // ???
  end;

  // estrae il componente grid principale del messagebox e lo distrugge
  try
    IWC:=(Owner as TIWAppForm).FindComponent(GRD_MSGBOX_NAME);
    if (Assigned(IWC)) and
       (IWC is TmeIWGridMessaggio) then
    begin
      // distrugge grid
      FreeAndNil(IWC);
    end;
...
// ### GESTIONE MESSAGGI MULTIPLI.INIZIO ### //
...
begin
  ...
  if (not IsActive) and (Length(MultipleMessages) > 0) then
    ShowMessages;
end;

// ### GESTIONE MESSAGGI MULTIPLI.FINE ### //
...
begin
  ...
  // 1. testo messaggio
  Cell[0,0].Css:='msgDialogTesto';
  Cell[0,0].Wrap:=True;
  Cell[0,0].RawText:=True;

  // 2: pulsanti di interazione
  FBtnIndex:=0;
  grdPulsanti:=TmeIWGrid.Create(Self);
  grdPulsanti.Name:='grdPulsanti';
  grdPulsanti.Parent:=Self.Parent;
  grdPulsanti.FriendlyName:='grdPulsanti';

  if Parametri.AccessibilitaNonVedenti = 'S' then //se è per nonvedenti mettiamo il testo una riga sopra la riga dei bottoni
  begin
    grdPulsanti.RowCount:=2;
  end
  else
    grdPulsanti.RowCount:=1;
  grdPulsanti.ColumnCount:=4;
  grdPulsanti.Css:='msgDialogComandiTab';
  grdPulsanti.Visible:=True;
  ...
  // riga per i pulsanti
  Cell[1,0].Control:=grdPulsanti;
  Cell[1,0].Css:='msgDialogComandi';
  Cell[1,0].RawText:=True;
  Cell[1,0].Text:='<hr class="gradiente">';
end;
...
begin
  ...
  indice:=0;   //inzio modifiche per non vedenti
  if Parametri.AccessibilitaNonVedenti = 'S' then
  begin
    for j:=0 to grdPulsanti.ColumnCount - 1 do
    begin
      grdPulsanti.Cell[indice,j].Css:='';
      grdPulsanti.Cell[indice,j].Width:='0';
    end;
    grdPulsanti.Cell[indice, grdPulsanti.ColumnCount-1].Css:='100%';
    grdPulsanti.Cell[indice,grdPulsanti.ColumnCount-1].RawText:=DlgComponent.TextIsHTML;
    grdPulsanti.Cell[indice,grdPulsanti.ColumnCount-1].Text:=DlgComponent.FText;
    indice:=indice + 1;
  end;


  grdPulsanti.Cell[indice,grdPulsanti.ColumnCount - FBtnIndex].Control:=btn;
  ...
  case FBtnIndex of
    1: begin
         grdPulsanti.Cell[indice,3].Css:='cellaCentrale';
       end;
    2: begin
         grdPulsanti.Cell[indice,2].Css:='cellaSx';
         grdPulsanti.Cell[indice,3].Css:='cellaDx';
       end;
    3: begin
         grdPulsanti.Cell[indice,1].Width:='45%';
         grdPulsanti.Cell[indice,1].Css:='align_right';
         grdPulsanti.Cell[indice,2].Width:='19%';
         grdPulsanti.Cell[indice,2].Css:='align_center';
         grdPulsanti.Cell[indice,3].Width:='35%';
         grdPulsanti.Cell[indice,3].Css:='align_left';
       end;
    4: begin
         grdPulsanti.Cell[indice,0].Width:='25%';
         grdPulsanti.Cell[indice,0].Css:='align_right';
         grdPulsanti.Cell[indice,1].Width:='25%';
         grdPulsanti.Cell[indice,1].Css:='align_center';
         grdPulsanti.Cell[indice,2].Width:='25%';
         grdPulsanti.Cell[indice,2].Css:='align_center';
         grdPulsanti.Cell[indice,3].Width:='25%';
         grdPulsanti.Cell[indice,3].Css:='align_left';
       end;
  else
  end;
...
begin
  ...
  Free;// distrugge qui??? Non è più corretto alla fine? No! Se si ditrugge alla fine si innesca il problema di sovrapposizione delle MessageBox!
