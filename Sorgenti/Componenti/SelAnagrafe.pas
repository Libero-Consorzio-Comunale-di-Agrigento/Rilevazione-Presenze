...
uses
  ...
  R005UDataModuleMW, System.UITypes;
...
type
  ...
  public
    ...
    OldC700SelAnagrafeBridge:TSelAnagrafeBridge;
...
begin
  ...
  FSoloPersonaleInterno:=Parametri.DefTipoPersonale <> 'E';
...
begin
  ...
  C700FSelezioneAnagrafe._frmSelAnagrafe:=Self;
...
var procCP:TCambiaProgressivo;
begin
  procCP:=OnCambiaProgressivo;
  OnCambiaProgressivo:=nil;
  CreaSelAnagrafe(Sessione,sb,Panel,AbilitaBrowse);
  OnCambiaProgressivo:=procCP;
...
begin
  ...
  try
    ...
    if Pos(S,C700Selanagrafe.SQL.Text) > 0 then
    begin
      C700Selanagrafe.SQL.Text:=C700Selanagrafe.SQL.Text.Replace(S,S1);
      C700FSelezioneAnagrafe.CorpoSQL.Text:=C700FSelezioneAnagrafe.CorpoSQL.Text.Replace(S,S1);
    end;
    if (Pos(':C700DATADAL',C700Selanagrafe.SQL.Text.ToUpper) > 0) and (C700Selanagrafe.VariableIndex('C700DATADAL') < 0) then
    begin
      C700Selanagrafe.DeclareVariable('C700DATADAL',otDate);
      C700Selanagrafe.SetVariable('C700DATADAL',Parametri.DataLavoro);
    end;
    if (Pos(':C700DATADAL',C700Selanagrafe.SQL.Text.ToUpper) <= 0) and (C700Selanagrafe.VariableIndex('C700DATADAL') >= 0) then
    begin
      C700Selanagrafe.DeleteVariable('C700DATADAL');
    end;
  except
  end;
end;
