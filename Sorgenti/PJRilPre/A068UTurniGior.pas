...
begin
  ...
  A068FStampa.QRep.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
end;
...
begin
  ...
  A068FTurniGiorDtM1.Q081.SetVariable('DATA',Data);
  A068FTurniGiorDtM1.selT080.SetVariable('DATA',Data);
  A068FTurniGiorDtM1.Q040.SetVariable('DATA',Data);
  C700SelAnagrafe.First;

  A068FStampa.QRep.DataSet:=C700SelAnagrafe;
  A068FStampa.QRDbText1.DataSet:=C700SelAnagrafe;
  A068FStampa.QRDbText2.DataSet:=C700SelAnagrafe;
  A068FStampa.LEnte.Caption:=Parametri.DAzienda;
  A068FStampa.LTitolo.Caption:='Turni previsti per il ' + Label1.Caption;
  A068FStampa.LTitolo1.Caption:=edtIntestazione.Text;

  if (Trim(DocumentoPDF) <> '') and (Trim(DocumentoPDF) <> '<VUOTO>') and (Trim(TipoModulo) = 'COM')then
  begin
    A068FStampa.QRep.ShowProgress:=False;
    A068FStampa.QRep.ExportToFilter(TQRPDFDocumentFilter.Create(DocumentoPDF));
  end
  else
    A068FStampa.QRep.Preview;
end;
