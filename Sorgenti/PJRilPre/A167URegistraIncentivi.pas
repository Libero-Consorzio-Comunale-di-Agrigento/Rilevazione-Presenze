...
uses
  ...
  Generics.Collections, A167URegistraIncentiviMW, QRPDFFilt, A000UMessaggi, InputPeriodo;
...
type
  ...
  TA167FRegistraIncentivi = class(TForm)
    ...
    lblMeseCompMax: TLabel;
    edtMeseCompMax: TMaskEdit;
    sbtMeseCompMax: TSpeedButton;
    chkCalcolaImportiSaldoAnnoCorr: TCheckBox;
    ...
    procedure sbtMeseCompMaxClick(Sender: TObject);
    procedure edtMeseCompMaxDblClick(Sender: TObject);
    procedure edtQuoteExit(Sender: TObject);
  ...
  private
    ...
    QuotaIntera,QuotaProporzionata,QuotaNetta,   //Incentivi
      ...
      PercGGEff,
      PesoIndivid,PesoStrutt,ValutIndivid,ValutStrutt,
      TotAssenzeGGPrec,TotAssenzeGGSucc,
      TotQuotaIntera,TotQuotaNetta,TotGGIntera:Real;
    ...
    RegoleC,Decurta,MaturaIncentivi,InterrompiElaborazione,Anteprima,Cancella,AccontoValutativo,AccontoValutativoOrig:Boolean;
    MeseCorr,InizioProva,FineProva,InizioOrig,InizioSospensione,FineSospensione:TDateTime;
    DurataProva,DurataSospensione,TotMesiSaldoI:Integer;
    R450DtM1:TR450DtM1;
    R600DtM1:TR600DtM1;
    ...
    CodForm: String;
    //Giorni di servizio del mese
    GGServDaCalMese: Integer;
    ...
    procedure CancellaAnno(Data:TDateTime);
    procedure RegistraMese(Data:TDateTime; Giorni,Quota:Real; TipoImporto:String);
    procedure RegistraAbbattimenti(Data:TDateTime; Codice:String; Quota:Real; TipoAbb:String);
    procedure LimitaRisparmio(Data:TDateTime);
    procedure LimitaRisparmioStampa(Data:TDateTime);
    ...
    procedure AbbattimentoSaldoOre(Data:TDateTime);
...
var Cessaz,MeseApp,DApp:TDateTime;
  ...
  TabellaTest:String;
  DataF_Originale: TDateTime;
begin
  Anteprima:=(Sender = btnAnteprima) or (Sender = btnStampa);
  DataF_Originale:=DataF;
  Controlli;
  //Se si tratta di una cancellazione - chiedo ulteriore conferma
  if TipoModulo = 'CS' then
    if chkAnnulla.Checked then
      if R180MessageBox(Format(A000MSG_A167_MSG_FMT_CONFERMA_CANC,[edtQuote.Text, frmInputPeriodo.edtInizio.Text,frmInputPeriodo.edtFine.Text]),'DOMANDA') <> mrYes then
        Exit;
  ...
  if frmSelAnagrafe.SettaPeriodoSelAnagrafe(StrToDate('01/' + frmInputPeriodo.edtInizio.Text), DataF) then
    C700SelAnagrafe.Close;
  ...
  if chkAggiorna.Checked or chkAnnulla.Checked then //Registro i parametri di esecuzione da consultare se ho disattivato i log delle operazioni
    RegistraMsg.InserisciMessaggio('I',Copy(Format('Oper:%s. Mesi:%s-%s. Tipo:%s. Quote:%s. Dip:%s',[IfThen(chkAggiorna.Checked,'Aggiorna','Annulla'), Copy(frmInputPeriodo.edtInizio.Text,1,2), frmInputPeriodo.edtFine.Text, Copy(cmbTipoCalcolo.Text,1,1),edtQuote.Text,C700SelAnagrafe.SubstitutedSQL]),1,2000));
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        TotAssenzeGGSucc:=0;
        TotGGIntera:=0;
        if (TipoQuota = 'I') and (VarToStr(A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'RIF_SALDO_ANNO_CORR')) = 'S') then
        begin
          selT134.Close;
          selT134.SetVariable('PROG',C700Progressivo);
          selT134.SetVariable('ANNO',R180Anno(DataF));
          selT134.Open;
        end;
        ...
        begin
          ...
          DatiStorici:='T430INIZIO,T430FINE,T430PARTTIME,T430TIPORAPPORTO';
          ...
          if Parametri.CampiRiferimento.C7_InizioSospensione <> '' then
            DatiStorici:=DatiStorici + ',T430' + Parametri.CampiRiferimento.C7_InizioSospensione;
          if Parametri.CampiRiferimento.C7_EscludeIncentivo <> '' then
            DatiStorici:=DatiStorici + ',T430' + Parametri.CampiRiferimento.C7_EscludeIncentivo;
          QSIncentivi.GetDatiStorici(DatiStorici,C700Progressivo,R180InizioMese(DataI),DataF);
          //Ciclo su ogni mese del periodo
          if R180In(TipoQuota,['D','N','Q']) then
            MeseCorr:=DataF
          else
            MeseCorr:=DataI;
          while MeseCorr <= DataF do
          begin
            ...
            if (not R180In(TipoQuota,['D','N','Q'])) and (not DSIncentivi.DipendenteInServizio(C700Progressivo,R180InizioMese(MeseCorr),MeseCorr)) then
              Continua:=False;
            if Continua and (not R180In(TipoQuota,['D','N','Q'])) and (not QSIncentivi.FieldByName('T430FINE').IsNull) and
...
(*            if (TipoQuota = 'Q') and (not DSIncentivi.DipendenteInServizio(C700Progressivo,R180InizioMese(MeseCorr),MeseCorr)) then  //Lorena 08/10/2012
            ...
            begin
              AccontoValutativo:=False;
              AccontoValutativoOrig:=not AccontoValutativo;
            ...
            if R180In(TipoQuota,['D','N']) then  //SALDO COLLETTIVO VALUTATIVO E SALDO COLLETTIVO CON VALUTAZIONE NON OBBLIGATORIA
            begin
          ...
          begin
            if chkAggiorna.Checked then
            begin
              RegistraMese(DataF,DaysBetween(DataF,EncodeDate(R180Anno(DataF),1,1)) + 1,TotaleIntera,'1');
              RegistraMese(DataF,TotaleGG,TotaleQuota,'2');
              RegistraMese(DataF,TotaleGG,TotaleQuota,'3');
              LimitaRisparmio(DataF);
            end;
            if Anteprima then
            begin
              CaricaTabellaStampa(DataF,DaysBetween(DataF,EncodeDate(R180Anno(DataF),1,1)) + 1,TotaleIntera,'1');
              CaricaTabellaStampa(DataF,TotaleGG,TotaleQuota,'2');
              CaricaTabellaStampa(DataF,TotaleGG,TotaleQuota,'3');
              LimitaRisparmioStampa(DataF);
            end;
          end;
    ...
    if R180In(TipoQuota,['D','N']) and (TabellaTipoD.RecordCount > 0) and chkAggiorna.Checked then
    begin
...
begin
  ...
  if R180InizioMese(DataI) > R180InizioMese(DataF) then
  ...
  if R180In(TipoQuota,['D','N'])
  and ((R180Mese(DataI) <> 1) or
       (R180Mese(DataF) <> 12)) then
  ...
  if (TipoQuota = 'R') and (frmInputPeriodo.edtInizio.Text <> frmInputPeriodo.edtFine.Text) then
    raise Exception.Create('Il calcolo delle Rateizzazioni va eseguito un mese alla volta!');
  if (TipoQuota = 'Q') and (frmInputPeriodo.edtInizio.Text <> frmInputPeriodo.edtFine.Text) then
  ...
  if ((TipoQuota = 'T') or (TipoQuota = 'I') or (TipoQuota = 'V') or (TipoQuota = 'C')) and
     ((StrToIntDef(Copy(frmInputPeriodo.edtInizio.Text,1,2),0) <> 1) or (StrToIntDef(Copy(frmInputPeriodo.edtFine.Text,1,2),0) <> 12)) then
    raise Exception.Create('Il calcolo del saldo va eseguito per tutto l''anno!');
  if edtMeseCompMax.Visible then
  begin
    if (StrToDate('01/' + frmInputPeriodo.edtFine.Text) < StrToDate('01/' + edtMeseCompMax.Text))
    or (StrToDate('01/' + frmInputPeriodo.edtInizio.Text) > StrToDate('01/' + edtMeseCompMax.Text)) then
      raise Exception.Create('Il "Mese comp. max" non può essere esterno al periodo di elaborazione!');
  end;
...
//Determina la QuotaIntera e imposta GiorniMese.QuotaGG e GiorniMese.GG basandosi sulle regole generali e individuali delle quote
var s:String;
  i,j,nQuoteInd:Integer;
  GG,App,Acconto,GGRifMeseQuotaInd,GGQuotaInd,GGRifMeseAbbIntera:Real;
  lstAcconti:TStringList;
  MeseCoperto,ConsideraProva,EscludeIncentivo:Boolean;
begin   //Operazioni fatte prima del ciclo su tutti i giorni del mese
  ...
  begin
    ...
    begin
      ...
      ConsideraProva:=VarToStr(selT770.Lookup('CODTIPOQUOTA',CodQuota,'CONSIDERA_PROVA')) = 'S';
      if Parametri.CampiRiferimento.C7_EscludeIncentivo <> '' then
        if QSIncentivi.FieldByName('T430' + Parametri.CampiRiferimento.C7_EscludeIncentivo).AsString <> '' then
          EscludeIncentivo:=R180InConcat(QSIncentivi.FieldByName('T430' + Parametri.CampiRiferimento.C7_EscludeIncentivo).AsString,VarToStr(selT770.Lookup('CODTIPOQUOTA',CodQuota,'ESCLUSIONE_VALORI')));
//      if (TipoQuota <> 'V') or ((TipoQuota = 'V') and AccontoValutativo) then  //Lorena 01/06/2011
      ...
      begin
        ...
        begin
          ...
          for i:=1 to 31 do
            GiorniMese[i].QuotaGG:=QuotaIntera / GiorniRifMese;
      ...
      if (not selT775.Active) or
        ...
        ((selT775.GetVariable('PROGRESSIVO') <> C700Progressivo) or
         ...
         (selT775.GetVariable('DFIN') <> MeseCorr) or
         (selT775.GetVariable('CODQUOTA') <> ''))) then
      begin
        selT775.Close;
        ...
        selT775.SetVariable('CODQUOTA','');//non deve creare quote fittizie
        selT775.Open;
      end;
//      if ((TipoQuota <> 'C') and (TipoQuota <> 'V')) or ((TipoQuota = 'V') and AccontoValutativo) then  //Lorena 01/06/2011
      ...
      begin
        //Se c'è una sola quota individuale che copre l'intero mese oppure una o più quote che non lo coprono interamente, allora si divide per i giorni del coefficiente.
        //Invece, se ci sono più quote individuali e la somma dei giorni copre l'intero mese, allora si divide per i giorni di calendario.
        //Quindi bisogna effettuare un primo scorrimento per recuperare queste due informazioni: numero quote individuali e somma dei giorni
        GG:=0;
        nQuoteInd:=0;
        MeseCoperto:=False;
        if selT775.SearchRecord('CODTIPOQUOTA',CodQuota,[srFromBeginning]) then
        begin
          while (not selT775.Eof)
          and (selT775.FieldByName('CODTIPOQUOTA').AsString = CodQuota) do
          begin
            GG:=GG + selT775.FieldByName('GIORNI').AsInteger;
            inc(nQuoteInd);
            selT775.Next;
          end;
          MeseCoperto:=GG = R180GiorniMese(MeseCorr);
          if MeseCoperto and (nQuoteInd > 1) then
            GGRifMeseQuotaInd:=R180GiorniMese(MeseCorr)
          else
            GGRifMeseQuotaInd:=GiorniRifMese;
        end;
        //Secondo scorrimento quote individuali per effettuare i calcoli
        if selT775.SearchRecord('CODTIPOQUOTA',CodQuota,[srFromBeginning]) then
        begin
          ...
          begin
            GG:=GG + selT775.FieldByName('GIORNI').AsInteger;
            if MeseCoperto and (nQuoteInd = 1) then
              GGQuotaInd:=GGRifMeseQuotaInd //se il mese è interamente coperto da una sola quota individuale, allora i giorni della quota sono ininfluenti perché l'importo non va proporzionato
            else
              GGQuotaInd:=selT775.FieldByName('GIORNI').AsInteger;
            if TipoQuota = 'Q' then
            begin
              App:=App + (R180OreMinutiExt(selT775.FieldByName('NUM_ORE').AsString) *
                          GGQuotaInd / GGRifMeseQuotaInd);
              ...
              begin
                ...
                begin
                  if (GiorniMese[i].Data <= selT775.FieldByName('SCADENZA').AsDateTime) and
                    ...
                    GiorniMese[i].QuotaGG:=selT775.FieldByName('IMPORTO').AsFloat / GGRifMeseQuotaInd;
                end;
              end
              else
              begin
                ...
                begin
                  if (GiorniMese[i].Data <= selT775.FieldByName('SCADENZA').AsDateTime) and
                     (GiorniMese[i].Data >= selT775.FieldByName('DECORRENZA').AsDateTime) then
                    GiorniMese[i].QuotaGG:=(QuotaIntera * selT775.FieldByName('PERCENTUALE').AsFloat / 100) / GGRifMeseQuotaInd;
                end;
              end;
          ...
          if TipoQuota = 'Q' then
          begin
            if not MeseCoperto then
              App:=App + (GGIntera * (GiorniRifMese - Min(GG,GiorniRifMese)) / GiorniRifMese);
            GGIntera:=App;
          end
          else
          begin
            if not MeseCoperto then
            begin
              App:=App + (QuotaIntera * (GiorniRifMese - Min(GG,GiorniRifMese)) / GiorniRifMese);
              if (TipoQuota = 'V') or (TipoQuota = 'I') then
              begin
                ValutIndivid:=ValutIndivid + 100 *
                           ((GiorniRifMese - Min(GG,GiorniRifMese)) / GiorniRifMese);
      ...
      begin
        ...
        begin
          ...
          begin
            QuotaIntera:=selT770.FieldByName('IMPORTO').AsFloat;
            for j:=1 to 31 do
              GiorniMese[j].QuotaGG:=QuotaIntera / GiorniRifMese;
          end;
          //Considerazione quote individuali che variano la Quota intera letta
          //Se c'è una sola quota individuale che copre l'intero mese oppure una o più quote che non lo coprono interamente, allora si divide per i giorni del coefficiente.
          //Invece, se ci sono più quote individuali e la somma dei giorni copre l'intero mese, allora si divide per i giorni di calendario.
          ...
          begin
            while (not selT775.Eof)
            and (selT775.FieldByName('CODTIPOQUOTA').AsString = lstAcconti.Strings[i]) do
            begin
              GG:=GG + selT775.FieldByName('GIORNI').AsInteger;
              inc(nQuoteInd);
              selT775.Next;
              ...
              begin
                ...
                begin
                  if (GiorniMese[j].Data <= selT775.FieldByName('SCADENZA').AsDateTime) and
                    ...
                    GiorniMese[j].QuotaGG:=selT775.FieldByName('IMPORTO').AsFloat / GGRifMeseQuotaInd;
                end;
              end
              else
              begin
                ...
                begin
                  if (GiorniMese[j].Data <= selT775.FieldByName('SCADENZA').AsDateTime) and
                     (GiorniMese[j].Data >= selT775.FieldByName('DECORRENZA').AsDateTime) then
                    GiorniMese[j].QuotaGG:=(QuotaIntera * selT775.FieldByName('PERCENTUALE').AsFloat / 100) / GGRifMeseQuotaInd;
                end;
              end;
              selT775.Next;
            end;
      ...
      begin
        ...
        //Se se la somma dei giorni non copre il mese interamente, allora si divide per i giorni del coefficiente.
        //Invece, se la somma dei giorni copre l'intero mese, allora si divide per i giorni di calendario.
        //Quindi bisogna effettuare un primo scorrimento per recuperare questa informazione: somma dei giorni
        GG:=0;
        MeseCoperto:=False;
        selT040B.First;
        ...
        //Calcolo giorni di servizio del mese da calendario, segnalando anomalia bloccante se vengono individuati periodi intersecanti
        GGServDaCalMese:=TDipendenteInServizio.GiorniServizio(C700Progressivo,R180InizioMese(MeseCorr),R180FineMese(MeseCorr),'S');
        if GGServDaCalMese = -1 then
        begin
          //Anomalia bloccante
          s:='Periodi di assunzione/cessazione incongruenti tra il ' + DateToStr(R180InizioMese(MeseCorr)) + ' e il ' + DateToStr(MeseCorr);
          RegistraMsg.InserisciMessaggio('A',s,'',C700SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger);
          Exit;
        end;
        //Nando 15/05/2018 MeseCoperto:=GG = R180GiorniMese(MeseCorr);
        MeseCoperto:=GG >= GGServDaCalMese;
        if MeseCoperto then
          //Nando 15/05/2018 GGRifMeseAbbIntera:=R180GiorniMese(MeseCorr)
          GGRifMeseAbbIntera:=GGServDaCalMese
        else
          GGRifMeseAbbIntera:=GiorniRifMese;
        //Secondo scorrimento assenze per effettuare i calcoli
        App:=0;
        GG:=0;
        selT040B.First;
        while not selT040B.Eof do
        begin
          GG:=GG + 1;//Questa variazione fissa da per assunto che nella stessa giornata non vengano fruite causali diverse che abbiano impostazioni eterogenee sulla T770, altrimenti il rischio è che vengano abbattuti più giorni di quelli in cui vengono fruite tali assenze
          if (TipoQuota = 'A') or
          ...
          begin
            ...
            begin
              App:=App + (selT040B.FieldByName('IMPORTO').AsFloat / GGRifMeseAbbIntera);
              GiorniMese[R180Giorno(selT040B.FieldByName('DATA').AsDateTime)].QuotaGG:=selT040B.FieldByName('IMPORTO').AsFloat / GGRifMeseAbbIntera;
            end
            else
            begin
              App:=App + ((QuotaIntera * selT040B.FieldByName('PERCENTUALE').AsFloat / 100) / GGRifMeseAbbIntera);
              GiorniMese[R180Giorno(selT040B.FieldByName('DATA').AsDateTime)].QuotaGG:=(QuotaIntera * selT040B.FieldByName('PERCENTUALE').AsFloat / 100) / GGRifMeseAbbIntera;
            end;
          end
    ...
    //Se “Inizio sospensione” valorizzato su A008, e valore significativo, e Nvl("Durata sospensione",0) > 0,
    //e T430_STORICO.TIPORAPPORTO in "Tipi rapporto sospensione", salto valutazione del periodo di prova
    //e replico ragionamenti sul periodo di prova usando i parametri prioritari.
    if Parametri.CampiRiferimento.C7_InizioSospensione <> '' then
      if QSIncentivi.FieldByName('T430' + Parametri.CampiRiferimento.C7_InizioSospensione).AsDateTime <> 0 then
      begin
        InizioSospensione:=QSIncentivi.FieldByName('T430' + Parametri.CampiRiferimento.C7_InizioSospensione).AsDateTime;
        FineSospensione:=InizioSospensione;
        DurataSospensione:=0;
        if (TipoCalcolo = 'C') and (TipoQuota <> 'Q') then
          DurataSospensione:=Round(StrToFloatDef(VarToStr(selT770.Lookup('CODTIPOQUOTA',CodQuota,'DURATA_SOSPENSIONE')),0));
        if (DurataSospensione > 0)
        and (QSIncentivi.FieldByName('T430TIPORAPPORTO').AsString <> '')
        and R180InConcat(QSIncentivi.FieldByName('T430TIPORAPPORTO').AsString,VarToStr(selT770.Lookup('CODTIPOQUOTA',CodQuota,'TIPIRAPPORTO_SOSPENSIONE'))) then
        begin
          ConsideraProva:=False;

          FineSospensione:=InizioSospensione + DurataSospensione - 1;
          //Se il dip. a fine mese è in sospensione non matura incentivi
          if MeseCorr - InizioSospensione + 1 <= DurataSospensione then
            QuotaIntera:=0;
        end;
      end;

    if ConsideraProva then
    begin
    ...
    //Se il dipendente ha un dato anagrafico di quelli indicati sulla quota, non matura incentivi
    if EscludeIncentivo then
      QuotaIntera:=0;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      LimitaRisparmio(AData);
    end;
    if Anteprima then
    begin
      CaricaTabellaStampa(AData,0,TotaleQuota,'2');
      LimitaRisparmioStampa(AData);
    end;
  end;
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          QuotaProg:=QuotaIntera - (QuotaIntera * Min(GGSenzaRegole,GiorniRifMese) / GiorniRifMese);
        if GGProg <> 0 then
        begin
        ...
        begin
          ...
          begin
            ...
            LimitaRisparmioStampa(MeseCorr);
          end;
        end
...
procedure TA167FRegistraIncentivi.ElaboraQuotaQuantitativa(AData:TDateTime);//TipoQuota = 'Q'
...
begin
  ...
  begin
    ...
    if selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(AData),'T070') then
      Exit;
    if not DSIncentivi.DipendenteInServizio(C700Progressivo, R180InizioMese(DataF), DataF) then
    begin
...
//      selSQL.SQL.Add('  AND MESE < ' + IntToStr(R180Mese(AData)));  //Lorena 08/10/2012
      A167FRegistraIncentiviMW.selSQL.SQL.Add('  AND MESE < ' + IntToStr(R180Mese(DataF)));
      ...
      MaxOre:='999.59';
      PercOre:=100;
      MeseRif:=IntToStr(IfThen(DataF > 0, R180Mese(DataF), 0));  //IntToStr(StrToIntDef(Copy(edtAData.Text,1,2),0));
...
//Determina GGSenzaRegole e GGPT e proporziona GiorniMese.QuotaGG e GiorniMese.GG basandosi sui giorni di servizio, sui GGSenzaRegole, sul periodo di prova e sul parttime (GGPT)
var InizioPeriodo,FinePeriodo:TDateTime;
  s,Flex:String;
  GGOK,SospendiPT,ConsideraProva,EscludeIncentivo:Boolean;
  PT:Real;
begin
        ...
        begin
          ...
          begin
            ...
            begin
              FineSospensione:=InizioSospensione + DurataSospensione - 1;
              //Se il dip. a fine mese è in sospensione non matura incentivi
              if InizioPeriodo - InizioSospensione + 1 <= DurataSospensione then
              begin
                GGSenzaRegole:=GGSenzaRegole + 1;
                GGOK:=False;
                GiorniMese[R180Giorno(InizioPeriodo)].GG:=0;
...
procedure TA167FRegistraIncentivi.ElaboraAcconto(DataFine:TDateTime;Dal:TDateTime = 0;Al:TDateTime = 0);
  ...
  begin
    ...
    if (not selT040.Active) or
      ...
      ((selT040.GetVariable('PROG') <> C700Progressivo) or
       ...
       (selT040.GetVariable('DATO3') <> Dato3) or
       (selT040.GetVariable('CODTIPOQUOTA') <> CodQuota))) then
    begin
      selT040.Close;
      ...
      selT040.SetVariable('CODTIPOQUOTA',CodQuota);
      selT040.Open;
    end;
    if (not selT769.Active) or
      ((selT769.Active) and
      ((selT769.GetVariable('DATA') <> FinePeriodo) or
       (selT769.GetVariable('DATO1') <> Dato1) or
       (selT769.GetVariable('DATO2') <> Dato2) or
       (selT769.GetVariable('DATO3') <> Dato3) or
       (selT769.GetVariable('CODTIPOQUOTA') <> CodQuota))) then
    begin
      selT769.Close;
      ...
      selT769.SetVariable('CODTIPOQUOTA',CodQuota);
      selT769.Open;
    end;
    selT040.First;
    OldCausale:='';
    ...
    begin
      ...
      begin
        AbbattimentoGS:=QuotaIntera * Min(GGSenzaRegole,GiorniRifMese) / GiorniRifMese;
        QuotaProporzionata:=QuotaIntera - AbbattimentoGS;
      end;
    end
    else
    begin
    ...
    begin
      ...
      begin
        ...
        QuotaProporzionata:=QuotaProporzionata - AbbattimentoPT;
      ...
      if (GGProporzionata >= GGAssenza) or Decurta then
        ...
        GGNetta:=0;
      if (not R180In(TipoQuota,['D','N'])) and (QuotaProporzionata < 0) then
      begin
        s:='Attenzione: quota negativa (' + FloatToStr(QuotaProporzionata) + ') al ' + DateToStr(MeseCorr);
        RegistraMsg.InserisciMessaggio('A',s,'',C700SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger);
      end;
      if ((abs(QuotaProporzionata) >= abs(QuotaAssenza)) or Decurta)
      and not ((StrToFloatDef(VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'GIORNI_MESE')),0) > 0) and (GGAssenza >= GGProporzionata)) then
      begin
    ...
    if R180In(TipoQuota,['A','I','D','N']) then
    ...
    begin
      ...
      begin
        ...
        if AbbattimentoGS <> 0 then
          RegistraAbbattimenti(MeseCorr,CodQuota,AbbattimentoGS,'GS');
        AbbattimentoPT:=R180Arrotonda(AbbattimentoPT,Decimali,'P');
        if AbbattimentoPT <> 0 then
          RegistraAbbattimenti(MeseCorr,CodQuota,AbbattimentoPT,'PT');
        AbbattimentoAS:=R180Arrotonda(AbbattimentoAS,Decimali,'P');
        if AbbattimentoAS <> 0 then
          RegistraAbbattimenti(MeseCorr,CodQuota,AbbattimentoAS,'AS');
      end
      ...
      begin
        ...
        begin  //Proporziono su % di valutazione in base alle % di incidenza
          ...
          begin
            ...
            begin
              TotQuotaIntera:=AppInd;
              TotGGIntera:=selT762.FieldByName('Giorni_Ore').AsFloat;
            end
            else if selT762.FieldByName('TIPOIMPORTO').AsString = '3' then
              TotQuotaNetta:=AppInd;
            selT762.Post;
            selT762.Next;
          end;
          SessioneOracle.Commit;
    ...
    begin
      ...
      begin
        ...
        LimitaRisparmioStampa(DataFine);
        if not chkAggiorna.Checked then
          TotGGIntera:=TotGGIntera + GGIntera;
        if (TipoQuota = 'I') and (MeseCorr = DataFine) then
        begin  //Proporziono su % di valutazione in base alle % di incidenza
    ...
    //Abbattimento saldo incentivi in base al saldo ore anno corrente
    if (TipoQuota = 'I') and (MeseCorr = DataFine) then
      AbbattimentoSaldoOre(DataFine); //RIFERIMENTO A SALDO ORE ANNO CORRENTE
  end;
end;

procedure TA167FRegistraIncentivi.ElaboraAssenzeGG(DataCorr,OldData,DataFine:TDateTime;OldCausale:String);
var QuotaAss,GGAss,Assenza,Q,HRese,TotAssenze,TotAssenzeGGCorr,TotAssenzeGGSucc,PercAbbattimento,PercAbbFranchigia,n,GGFranchigia:Real;
    G:TGiustificativo;
    UM,ContaFruitoOre,ForzaAbbGGInt,SoloGGInt,s,AssenzeAgg:String;
    Inizio,Fine:TDateTime;
    CambioRicercaAssenze:Boolean;
  function RecuperaTotAssenze: Real;
  begin
    Result:=0;
    with A167FRegistraIncentiviDtM do
      //ciclo su tutte le assenze trovate
      ...
      begin
        ...
        begin
          ...
          A167FRegistraIncentiviMW.selSQL.SQL.Add('   AND DATA BETWEEN TO_DATE(''' + VarToStr(selT040A.GetVariable('DADATA')) + ''',''DD/MM/YYYY'')');
          A167FRegistraIncentiviMW.selSQL.SQL.Add('                AND TO_DATE(''' + VarToStr(selT040A.GetVariable('ADATA')) + ''',''DD/MM/YYYY'')');
          if ContaFruitoOre = 'N' then
            A167FRegistraIncentiviMW.selSQL.SQL.Add('   AND TIPOGIUST = ''I''');
          A167FRegistraIncentiviMW.selSQL.Execute;
          Result:=Result + (StrToFloatDef(VarToStr(A167FRegistraIncentiviMW.selSQL.Field(0)),0) * PercAbbFranchigia / 100);
        end
        else
        begin
          ...
          begin
            if (SoloGGInt = 'N') or
              ...
              Result:=Result + ((Q / R600DtM1.ValenzaGiornaliera) * PercAbbFranchigia / 100)
          end
          else
          begin
            if (SoloGGInt = 'N') or
              ((SoloGGInt = 'S') and (Q >= 1)) then
              Result:=Result + (Q * PercAbbFranchigia / 100);
          end;
        end;
        selT040A.Next;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if Assenza <> 0 then
      begin
        if GGFranchigia = 0 then   //ass non toll
        begin
          GGAss:=Assenza * PercAbbattimento / 100;
          QuotaAss:=Assenza * PercAbbattimento / 100;
          if not R180In(TipoQuota,['D','N']) then
          begin
            GGAss:=GGAss * GiorniMese[R180Giorno(DataCorr)].GG;
            QuotaAss:=QuotaAss * GiorniMese[R180Giorno(DataCorr)].QuotaGG;
          end;
        ...
        begin
          //Data Inizio per ricerca sul passato e per decurtazioni
          if (selT040.FieldByName('GESTIONE_FRANCHIGIA').AsString = 'D') or
             (selT040.FieldByName('GESTIONE_FRANCHIGIA').AsString = 'R') then
          ...
          AssenzeAgg:='''' + StringReplace(selT040.FieldByName('ASSENZE_AGGIUNTIVE').AsString,',',''',''',[rfReplaceAll]) + '''';
          CambioRicercaAssenze:=(Inizio <> InizioOrig) or
                                (AccontoValutativo <> AccontoValutativoOrig) or //per ogni mese ricalcolo TotAssenzeGGPrec quando passo da acconto a saldo e viceversa
                                (selT040.FieldByName('COD_TIPOACCORPCAUSALI').AsString <> VarToStr(selT040A.GetVariable('TIPOACCORP'))) or
                                (selT040.FieldByName('COD_CODICIACCORPCAUSALI').AsString <> VarToStr(selT040A.GetVariable('CODACCORP'))) or
                                (AssenzeAgg <> VarToStr(selT040A.GetVariable('ASSENZEAGG')));
          InizioOrig:=Inizio;
          AccontoValutativoOrig:=AccontoValutativo;
          TotAssenzeGGCorr:=0;
          if (DataCorr <> OldData) or CambioRicercaAssenze then //Se non ho già calcolato il giorno oppure sono cambiati i parametri per la ricerca delle causali
          begin
            //1) Se sono cambiati i parametri per la ricerca delle causali, prelevo il totale da inizio anno fino al giorno precedente (TotAssenzeGGPrec)
            if CambioRicercaAssenze then
            begin
              selT040A.Close;
              selT040A.SetVariable('PROG',selT040.FieldByName('PROGRESSIVO').AsInteger);
              selT040A.SetVariable('DADATA',InizioOrig);
              selT040A.SetVariable('ADATA',DataCorr - 1);
              ...
              selT040A.SetVariable('ASSENZEAGG',AssenzeAgg);
              selT040A.Open;
              TotAssenzeGGPrec:=RecuperaTotAssenze;
            end;
            //Data Inizio per ricerca sul giorno corrente e fino a fine mese
            if (selT040.FieldByName('GESTIONE_FRANCHIGIA').AsString = 'D') or
               (selT040.FieldByName('GESTIONE_FRANCHIGIA').AsString = 'R') then
              Inizio:=Max(QSIncentivi.FieldByName('T430INIZIO').AsDateTime,Max(selT040.FieldByName('DECORRENZA').AsDateTime,DataCorr))
            else
              Inizio:=Max(QSIncentivi.FieldByName('T430INIZIO').AsDateTime,DataCorr);
            //2) Prelevo il totale del giorno di assenza di T040 (TotAssenzeGGCorr)
            selT040A.Close;//Cambio solo le variabili diverse
            selT040A.SetVariable('DADATA',Inizio);
            selT040A.SetVariable('ADATA',DataCorr);
            selT040A.Open;
            TotAssenzeGGCorr:=RecuperaTotAssenze;
            //3) Se GESTIONE_FRANCHIGIA = 'R' prelevo il totale dal giorno successivo al giorno di assenza di T040 fino a fine mese (TotAssenzeGGSucc)
            TotAssenzeGGSucc:=0;
            if selT040.FieldByName('GESTIONE_FRANCHIGIA').AsString = 'R' then
            begin
              selT040A.Close;//Cambio solo le variabili diverse
              selT040A.SetVariable('DADATA',Inizio + 1);
              selT040A.SetVariable('ADATA',R180FineMese(DataCorr));
              selT040A.Open;
              TotAssenzeGGSucc:=RecuperaTotAssenze;
            end;
          end;
          //4) Calcolo il TotAssenze al giorno del giustificativo
          TotAssenze:=R180Arrotonda(TotAssenzeGGPrec + TotAssenzeGGCorr + TotAssenzeGGSucc,0.001,'P');
          //5) Reimposto la data Inizio come in origine (ma con QSIncentivi posizionato all'ultimo giustificativo)
          Inizio:=InizioOrig;
          //6) Controllo la franchigia
          ...
          //7) Predispongo il TotAssenzeGGPrec per il giustificativo successivo
          //(se quest'ultimo sarà nello stesso giorno e avrà gli stessi parametri di ricerca delle causali, salterà la ricerca di TotAssenzeGGCorr perché già contenuto in TotAssenzeGGPrec)
          TotAssenzeGGPrec:=TotAssenzeGGPrec + TotAssenzeGGCorr;
        end;
      end;
    end;
  end;
...
procedure TA167FRegistraIncentivi.CancellaAnno(Data:TDateTime);
var lstAccontiTipoI:TStringList;
    i:Integer;
    AppQuotaMax,AppAbbattimento:Real;
begin
  with A167FRegistraIncentiviDtM do
  begin
    if (TipoQuota = 'I') and (VarToStr(A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'RIF_SALDO_ANNO_CORR')) = 'S') then
    begin
      //Cancello su tutto l'anno gli abbattimenti degli acconti collegati al saldo
      try
        lstAccontiTipoI:=TStringList.Create;
        lstAccontiTipoI.Clear;
        lstAccontiTipoI.CommaText:=VarToStr(A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'ACCONTI'));
        for i:=0 to lstAccontiTipoI.Count - 1 do
        begin
          selT763.First;
          while not selT763.Eof do
          begin
            if (selT763.FieldByName('TIPOQUOTA').AsString = lstAccontiTipoI.Strings[i])
            and (selT763.FieldByName('TIPOABBATTIMENTO').AsString = 'SO')
            and not selDatiBloccati.DatoBloccato(C700Progressivo,EncodeDate(selT763.FieldByName('ANNO').AsInteger,selT763.FieldByName('MESE').AsInteger,1),'T762') then
            begin
              AppQuotaMax:=-1;
              AppAbbattimento:=selT763.FieldByName('quotaabbattimento').AsFloat;
              if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([selT763.FieldByName('MESE').AsInteger,selT763.FieldByName('TIPOQUOTA').AsString,'2']),[srFromBeginning]) then //Quota proporzionata
                AppQuotaMax:=selT762.FieldByName('Importo').AsFloat;
              if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([selT763.FieldByName('MESE').AsInteger,selT763.FieldByName('TIPOQUOTA').AsString,'3']),[srFromBeginning]) then //Quota netta
              begin
                if AppQuotaMax <> - 1 then
                  AppAbbattimento:=Min(selT762.FieldByName('Importo').AsFloat + selT763.FieldByName('quotaabbattimento').AsFloat,AppQuotaMax) - selT762.FieldByName('Importo').AsFloat;
                selT762.Edit;
                selT762.FieldByName('Importo').AsFloat:=selT762.FieldByName('Importo').AsFloat + AppAbbattimento;
                selT762.Post;
              end;
              if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([selT763.FieldByName('MESE').AsInteger,selT763.FieldByName('TIPOQUOTA').AsString,'4']),[srFromBeginning]) then //Quota netta + risparmio
              begin
                selT762.Edit;
                selT762.FieldByName('Importo').AsFloat:=selT762.FieldByName('Importo').AsFloat + AppAbbattimento;
                selT762.Post;
        ...
        FreeAndNil(lstAccontiTipoI);
      end;
      //Cancello gli assestamenti del saldo anno corrente
      if selDatiBloccati.DatoBloccato(C700Progressivo,EncodeDate(R180Anno(Data),12,31),'T134') then
        exit;
      while selT134.SearchRecord('NOTE','Record automatico: Ore abbattute da saldo incentivi ' + CodQuota,[srFromBeginning]) do
        selT134.Delete;
    end;
    SessioneOracle.Commit;
  end;
...
procedure TA167FRegistraIncentivi.LimitaRisparmio(Data:TDateTime);
//Il risparmio non deve superare la quota proporzionata registrata
var nQuotaProporzionata,nQuotaDiff,nQuotaNettaRisp:Real;
begin
  with A167FRegistraIncentiviDtM do
  begin
    if not chkAggiorna.Checked then
      exit;
    if StrToFloatDef(VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'GIORNI_MESE')),0) <= 0 then
      exit;
    if selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T762') then
      exit;
    selT762.Refresh;
    ...
    begin
      nQuotaProporzionata:=selT762.FieldByName('Importo').AsFloat;
      nQuotaDiff:=0;
      nQuotaNettaRisp:=0;
      if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([R180Mese(Data),CodQuota,'4']),[srFromBeginning]) then
      begin
        if (selT762.FieldByName('Importo').AsFloat > nQuotaProporzionata) then
        begin
          nQuotaDiff:=selT762.FieldByName('Importo').AsFloat - nQuotaProporzionata;
          selT762.Edit;
          selT762.FieldByName('Importo').AsFloat:=nQuotaProporzionata;
          selT762.Post;
        end;
        nQuotaNettaRisp:=selT762.FieldByName('Importo').AsFloat;
      end;
      selT766.First;
      while not selT766.Eof do
      begin
        if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([R180Mese(Data),CodQuota,selT766.FieldByName('Codice').AsString]),[srFromBeginning]) then
        begin
          if (selT766.FieldByName('Risparmio_Bilancio').AsString = 'S')
          and (nQuotaDiff > 0) then
          begin
            selT762.Edit;
            selT762.FieldByName('Importo').AsFloat:=selT762.FieldByName('Importo').AsFloat - nQuotaDiff;
            selT762.Post;
          end;
          if (selT762.FieldByName('Importo').AsFloat > nQuotaProporzionata) then
          begin
          ...
          if (nQuotaProporzionata > 0)
          and (selT766.FieldByName('Risparmio_Bilancio').AsString <> 'S')
          and (selT762.FieldByName('Importo').AsFloat > 0) then
          begin
            selT762.Edit;
            selT762.FieldByName('Importo').AsFloat:=nQuotaProporzionata - nQuotaNettaRisp;
            selT762.Post;
          end;
        end;
        selT766.Next;
...
procedure TA167FRegistraIncentivi.LimitaRisparmioStampa(Data:TDateTime);
//Il risparmio non deve superare la quota proporzionata calcolata
var nQuotaProporzionata,nQuotaDiff:Real;
begin
  with A167FRegistraIncentiviDtM do
  begin
    if not Anteprima then
    ...
    begin
      nQuotaDiff:=0;
      if TabellaStampa.FieldByName('QuotaNettaRisp').AsFloat > TabellaStampa.FieldByName('QuotaProporzionata').AsFloat then
      begin
        nQuotaDiff:=TabellaStampa.FieldByName('QuotaNettaRisp').AsFloat - TabellaStampa.FieldByName('QuotaProporzionata').AsFloat;
        TabellaStampa.Edit;
        TabellaStampa.FieldByName('QuotaNettaRisp').AsFloat:=TabellaStampa.FieldByName('QuotaProporzionata').AsFloat;
        TabellaStampa.Post;
      end;
      if nQuotaDiff > 0 then
      begin
        TabellaStampa.Edit;
        TabellaStampa.FieldByName('AbbRispIncentivi').AsFloat:=TabellaStampa.FieldByName('AbbRispIncentivi').AsFloat - nQuotaDiff;
        TabellaStampa.Post;
      end;
      if TabellaStampa.FieldByName('AbbRispIncentivi').AsFloat > TabellaStampa.FieldByName('QuotaProporzionata').AsFloat then
      begin
        TabellaStampa.Edit;
        TabellaStampa.FieldByName('AbbRispIncentivi').AsFloat:=TabellaStampa.FieldByName('QuotaProporzionata').AsFloat;
        TabellaStampa.Post;
      end;
      if TabellaStampa.FieldByName('AbbNoRispIncentivi').AsFloat > TabellaStampa.FieldByName('QuotaProporzionata').AsFloat then
      begin
        TabellaStampa.Edit;
        TabellaStampa.FieldByName('AbbNoRispIncentivi').AsFloat:=TabellaStampa.FieldByName('QuotaProporzionata').AsFloat;
        TabellaStampa.Post;
      end;
      if (TabellaStampa.FieldByName('QuotaProporzionata').AsFloat > 0)
      and (TabellaStampa.FieldByName('AbbNoRispIncentivi').AsFloat > 0) then
      begin
        TabellaStampa.Edit;
        TabellaStampa.FieldByName('AbbNoRispIncentivi').AsFloat:=TabellaStampa.FieldByName('QuotaProporzionata').AsFloat - TabellaStampa.FieldByName('QuotaNettaRisp').AsFloat;
        TabellaStampa.Post;
      end;
    end;
  end;
...
procedure TA167FRegistraIncentivi.AbbattimentoSaldoOre(Data:TDateTime);
var GiorniAnno,MinTot,MinRif,MinSOAC,MinAbb:Integer;
    PercRagg,ImpAbbTot,ImpAbb:Real;
begin
  //Abbattimento saldo incentivi in base al saldo ore anno corrente
  with A167FRegistraIncentiviDtM do
  begin
    //Condizioni necessarie
    MinTot:=R180OreMinutiExt(selT770.FieldByName('NUM_ORE').AsString);
    if MinTot <= 0 then exit;
    if VarToStr(A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'RIF_SALDO_ANNO_CORR')) <> 'S' then exit;
    if TotQuotaIntera <= 0 then exit;

    //ABBATTIMENTO SALDO ORE ANNO CORRENTE
    //Proporziono le ore da abbattere
    GiorniAnno:=DaysBetween(DataF,R180InizioMese(DataI)) + 1;                    //Giorni dell'anno
    MinTot:=Trunc(R180Arrotonda(MinTot / GiorniAnno * TotGGIntera,1,'P'));       //Ore annuali da abbattere proporzionate al periodo del saldo
    MinRif:=Trunc(R180Arrotonda(MinTot / TotQuotaIntera * TotQuotaNetta,1,'P')); //Ore da abbattere del periodo proporzionate al rapporto tra QuotaNetta e QuotaIntera
    TotGGIntera:=0;
    //Recupero il saldo ore anno corrente (in minuti)
    MinSOAC:=0;
    if not chkAggiorna.Checked then
    begin
      selT134.First;
      while selT134.SearchRecord('NOTE','Record automatico: Ore abbattute da saldo incentivi ' + CodQuota,[]) do
        MinSOAC:=MinSOAC + R180OreMinutiExt(selT134.FieldByName('VARIAZIONE_ORE').AsString) * -1;
    end;
    if R450DtM1 = nil then
      R450DtM1:=TR450DtM1.Create(Self);
    R450DtM1.ConteggiMese('Generico',R180Anno(Data),R180Mese(Data),C700Progressivo);
    if R450DtM1.ttrovscheda[R180Mese(Data)] = 1 then
      MinSOAC:=Max(MinSOAC + R450DtM1.salliqannoatt + R450DtM1.salcompannoatt,0) //Campo "Saldo ore anno corrente" della scheda riepilogativa
    else
      exit;
    //Abbatto il saldo ore anno corrente
    MinAbb:=IfThen(MinSOAC >= MinRif,MinRif,MinSOAC);
    if chkAggiorna.Checked then
    begin
      if selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T134') then
        exit;
      if MinAbb > 0 then
      begin
        selT134.Insert;
        selT134.FieldByName('PROGRESSIVO').AsInteger:=C700Progressivo;
        selT134.FieldByName('ANNO').AsInteger:=R180Anno(Data);
        selT134.FieldByName('DATA').AsDateTime:=R180InizioMese(Data);
        selT134.FieldByName('ORE_LIQUIDATE').AsString:=' 000.00';
        selT134.FieldByName('VARIAZIONE_ORE').AsString:=R180MinutiOre(MinAbb * -1);
        selT134.FieldByName('NOTE').AsString:='Record automatico: Ore abbattute da saldo incentivi ' + CodQuota;
        selT134.Post;
      end;
    end;
    //Cancello il saldo se non si vuole il calcolo degli importi
    if not chkCalcolaImportiSaldoAnnoCorr.Checked then
    begin
      if chkAggiorna.Checked then
        CancellaMese(Data);
      if Anteprima then
        if TabellaStampa.Locate('ANNO;MESE;CODTIPOQUOTA',VarArrayOf([R180Anno(Data),R180Mese(Data),CodQuota]),[]) then
          TabellaStampa.Delete;
      exit;
    end;

    //Determinare percentuale di raggiungimento delle ore da abbattere
    if MinRif = 0 then
      PercRagg:=0
    else
      PercRagg:=MinAbb * 100 / MinRif;
    //Intervengo sull'incentivo solo se non ho raggiunto le ore da abbattere
    if PercRagg >= 100 then exit;
    //ABBATTIMENTO INCENTIVI
    //Aggiungo al saldo tutti gli acconti già pagati
    selT762.Last;
    while not selT762.Bof do
    begin
      //Totalizzo solo fino a quando trovo lo stesso saldo su un precedente mese dello stesso anno
      if (VarToStr(selT762.Lookup('MESE;CODTIPOQUOTA',VarArrayOf([selT762.FieldByName('MESE').AsInteger,CodQuota]),'CODTIPOQUOTA')) = CodQuota)
      and (selT762.FieldByName('MESE').AsInteger <> R180Mese(Data)) then
        Break;
      //Aggiungo solo gli acconti
      if Pos(',' + selT762.FieldByName('CODTIPOQUOTA').AsString + ',',',' + VarToStr(A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'ACCONTI')) + ',') > 0 then
        if selT762.FieldByName('TIPOIMPORTO').AsString = '3' then
          TotQuotaNetta:=TotQuotaNetta + selT762.FieldByName('IMPORTO').AsFloat;
      selT762.Prior;
    end;
    //Abbatto partendo dal saldo e poi sugli acconti a ritroso
    if chkAggiorna.Checked then
    begin
      if selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T762') then
        exit;
      //Determinare importo da abbattere (totale pagato - (totale pagato (acconti+saldo) * % di raggiungimento ore))
      ImpAbbTot:=R180Arrotonda(TotQuotaNetta - (TotQuotaNetta * PercRagg / 100),Decimali,'P');
      //ABBATTIMENTO SALDO
      selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([R180Mese(Data),CodQuota,'3']),[srFromBeginning]); //Quota netta
      ImpAbb:=IfThen(ImpAbbTot >= selT762.FieldByName('Importo').AsFloat,selT762.FieldByName('Importo').AsFloat,ImpAbbTot);
      selT762.Edit;
      selT762.FieldByName('Importo').AsFloat:=selT762.FieldByName('Importo').AsFloat - ImpAbb;
      selT762.Post;
      if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([R180Mese(Data),CodQuota,'4']),[srFromBeginning]) then //Quota netta + risparmio
      begin
        selT762.Edit;
        selT762.FieldByName('Importo').AsFloat:=selT762.FieldByName('Importo').AsFloat - ImpAbb;
        selT762.Post;
      ...
      if ImpAbbTot > 0 then
        RegistraAbbattimenti(Data,CodQuota,R180Arrotonda(ImpAbb,Decimali,'P'),'SO');
      ImpAbbTot:=ImpAbbTot - ImpAbb;
      //ABBATTIMENTO ACCONTI
      selT762.Last;
      while not selT762.Bof and (ImpAbbTot > 0) do
      begin
        //Abbatto solo fino a quando trovo lo stesso saldo su un precedente mese dello stesso anno
        if (VarToStr(selT762.Lookup('MESE;CODTIPOQUOTA',VarArrayOf([selT762.FieldByName('MESE').AsInteger,CodQuota]),'CODTIPOQUOTA')) = CodQuota)
        and (selT762.FieldByName('MESE').AsInteger <> R180Mese(Data)) then
          Break;
        //Abbatto solo gli acconti
        if Pos(',' + selT762.FieldByName('CODTIPOQUOTA').AsString + ',',',' + VarToStr(A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'ACCONTI')) + ',') > 0 then
          if selT762.FieldByName('TIPOIMPORTO').AsString = '3' then //Quota netta
          begin
            ImpAbb:=IfThen(ImpAbbTot >= selT762.FieldByName('Importo').AsFloat,selT762.FieldByName('Importo').AsFloat,ImpAbbTot);
            selT762.Edit;
            selT762.FieldByName('Importo').AsFloat:=R180Arrotonda(selT762.FieldByName('Importo').AsFloat - ImpAbb,Decimali,'P');
            selT762.Post;
            if selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([selT762.FieldByName('MESE').AsInteger,selT762.FieldByName('CODTIPOQUOTA').AsString,'4']),[srFromBeginning]) then //Quota netta + risparmio
            begin
              selT762.Edit;
              selT762.FieldByName('Importo').AsFloat:=R180Arrotonda(selT762.FieldByName('Importo').AsFloat - ImpAbb,Decimali,'P');
              selT762.Post;
              selT762.SearchRecord('MESE;CODTIPOQUOTA;TIPOIMPORTO',VarArrayOf([selT762.FieldByName('MESE').AsInteger,selT762.FieldByName('CODTIPOQUOTA').AsString,'3']),[srFromBeginning]); //Ritorno sulla Quota netta
            end;
            RegistraAbbattimenti(EncodeDate(selT762.FieldByName('ANNO').AsInteger,selT762.FieldByName('MESE').AsInteger,1),selT762.FieldByName('CODTIPOQUOTA').AsString,R180Arrotonda(ImpAbb,Decimali,'P'),'SO');
            ImpAbbTot:=ImpAbbTot - ImpAbb;
          end;
        selT762.Prior;
      end;
    ...
    begin
      ...
      if TabellaStampa.Locate('ANNO;MESE;CODTIPOQUOTA',VarArrayOf([R180Anno(Data),R180Mese(Data),CodQuota]),[]) then
      begin
        ImpAbb:=IfThen(ImpAbbTot >= TabellaStampa.FieldByName('QuotaNetta').AsFloat,TabellaStampa.FieldByName('QuotaNetta').AsFloat,ImpAbbTot);
        TabellaStampa.Edit;
        TabellaStampa.FieldByName('QuotaNetta').AsFloat:=R180Arrotonda(TabellaStampa.FieldByName('QuotaNetta').AsFloat - ImpAbb,Decimali,'P');
        TabellaStampa.FieldByName('QuotaNettaRisp').AsFloat:=R180Arrotonda(TabellaStampa.FieldByName('QuotaNettaRisp').AsFloat - ImpAbb,Decimali,'P');
        TabellaStampa.Post;
      end;
    end;
  end;
...
procedure TA167FRegistraIncentivi.RegistraAbbattimenti(Data:TDateTime; Codice:String; Quota:Real; TipoAbb:String);
begin
  with A167FRegistraIncentiviDtM do
  begin
    if selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T762') then
    ...
    selT763.FieldByName('tipoquota').AsString:=Codice;
    selT763.FieldByName('tipoabbattimento').AsString:=TipoAbb;
    selT763.FieldByName('meseapplicazioneabbattimento').AsDateTime:=R180InizioMese(DataF);
...
begin
  ...
  begin
    ...
    R180SetVariable(selT775,'CODQUOTA',IfThen(TipoQuota = 'N',CodQuota,''));//deve creare la quota fittizia solo se "Saldo Coll. val. non obbl." e se non esistono quote reali
    selT775.Open;
    selT775.First;
    if selT775.SearchRecord('CODTIPOQUOTA',CodQuota,[srFromBeginning]) then
      ...
      begin
        ...
        ElaboraAcconto(DataF,Dal_Scheda,Al_Scheda);//Serve solo a recuperare GGAssenza e PercGGEff. I calcoli riferiti alle Quote non sono significativi
        selT430Lav.First;
        while not selT430Lav.Eof do
        begin
          ...
          begin
            ...
            //Solo per "Saldo Coll. val. non obbl.", se richiesto e se necessario, riduco il mese di competenza
            if (TipoQuota = 'N')
            and (VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'IMPOSTA_MESE_COMP_MAX')) = 'S')
            and (DataReg > R180FineMese(StrToDate('01/' + edtMeseCompMax.Text))) then
              TabellaTipoD.FieldByName('Data_Registrazione').AsDateTime:=R180FineMese(StrToDate('01/' + edtMeseCompMax.Text))
            else
...
var i:Integer;
    Esiste,SommaSaldiDettModificato:Boolean;
    Variazione:Real;
    s:String;
begin
  with A167FRegistraIncentiviDtM.TabellaTipoD do
    ...
    begin
      ...
      begin
        ...
        begin
          s:='Saldo negativo per la quota ' + CodQuota + ' nel periodo di dettaglio ' + FieldByName('Dal_Dettaglio').AsString + '-' + FieldByName('Al_Dettaglio').AsString + '.';
          if FieldByName('Giorni_Presenza').AsFloat < 0 then
            s:=s + #$D#$A + 'Le assenze (' + FieldByName('Giorni_Assenza').AsString + ') superano i giorni di servizio (' + FieldByName('Giorni_Scheda_Effettivi').AsString + '): controllare le causali elencate negli accorpamenti indicati nella funzione "Abbattimento incentivi per assenze".';
          RegistraMsg.InserisciMessaggio('A',s,'',C700SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger);
        end;
        with A167FRegistraIncentiviDtM do
        begin
      ...
      for i:=0 to High(Fondi) do
      begin
        SommaSaldiDettModificato:=True;
        while (Fondi[i].SommaSaldiDett <> Fondi[i].FondoQuotaSaldo) and SommaSaldiDettModificato do //Se non trovo saldi positivi su cui intervenire, esco anche se non ho distribuito tutto lo scostamento
        begin
          Variazione:=Decimali * IfThen(Fondi[i].SommaSaldiDett > Fondi[i].FondoQuotaSaldo,-1,1);
          SommaSaldiDettModificato:=False;//Serve a non andare in loop quando finiscono i saldi positivi su cui intervenire
          First;
          ...
          begin
            ...
            and (Fondi[i].Dato2 = FieldByName('DATO2_QUOTASALDO').AsString)
            and (Fondi[i].Dato3 = FieldByName('DATO3_QUOTASALDO').AsString)
            and (FieldByName('Saldo').AsFloat > 0) //aumento o riduco solo se ho maturato dell'incentivo
            then
            begin
              SommaSaldiDettModificato:=True;//Ho trovato un saldo positivo su cui intervenire
              CodQuota:=FieldByName('CodTipoQuota').AsString;
              Edit;
...
procedure TA167FRegistraIncentivi.AbilitaComponenti;
...
begin
  ...
  if edtQuote.Visible then
    ...
    lstQuote.Add('A#D');
  if A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Active then
    for i:=0 to lstQuote.Count - 1 do
    begin
      if VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',lstQuote.Strings[i],'IMPOSTA_MESE_COMP_MAX')) = 'S' then
        VisMeseCompMax:=True;
      if VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',lstQuote.Strings[i],'RIF_SALDO_ANNO_CORR')) = 'S' then
  ...
  sbtMeseCompMax.Visible:=VisMeseCompMax;
  chkCalcolaImportiSaldoAnnoCorr.Visible:=VisCalcolaImporti;
  if not chkCalcolaImportiSaldoAnnoCorr.Visible then
...
begin
  ...
  begin
    try
      elencoTipoCalcolo:=getElencoTipoCalcolo(R180InizioMese(DataF));
...
begin
  ...
  frmInputPeriodo.CaptionDataOutI:='Mese inizio elaborazione';
  frmInputPeriodo.CaptionDataOutF:='Mese fine elaborazione';
end;
...
begin
  C700DataDal:=StrToDate('01/' + frmInputPeriodo.edtInizio.Text); //????? StrToDate('01/' + edtDaData.Text); diverso dalla gestione DataI delle altre parti?
  C700DataLavoro:=DataF; //R180FineMese(StrToDate('01/' + edtAData.Text));
...
var s:String;
  ...
  tmp : TDateTime;
begin
  if TryStrToDateTime('01/' + C004FParamForm.GetParametro('DATADA',Copy(DateToStr(Parametri.DataLavoro),4,7)), tmp) then
    DataI:=tmp
  else
    DataI:=0;

  if TryStrToDateTime('01/' + C004FParamForm.GetParametro('DATAA',Copy(DateToStr(Parametri.DataLavoro),4,7)), tmp) then
    DataF:=R180FineMese(tmp)
  else
    DataF:=0;

  edtMeseCompMax.Text:=C004FParamForm.GetParametro('MESECOMPMAX','12/' + IntToStr(R180Anno(DataF)));
  frmInputPeriodoedtFineExit(nil);
  try
...
begin
  A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.SettaDataQuote(R180InizioMese(DataF));
end;

procedure TA167FRegistraIncentivi.PutParametriFunzione;
begin
  C004FParamForm.Cancella001;
  C004FParamForm.PutParametro('DATADA', frmInputPeriodo.edtInizio.Text);
  C004FParamForm.PutParametro('DATAA', frmInputPeriodo.edtFine.Text);
  C004FParamForm.PutParametro('MESECOMPMAX',edtMeseCompMax.Text);
...
procedure TA167FRegistraIncentivi.edtMeseCompMaxDblClick(Sender: TObject);
begin
  edtMeseCompMax.Text:=frmInputPeriodo.edtFine.Text;
end;

procedure TA167FRegistraIncentivi.edtQuoteExit(Sender: TObject);
begin
  AbilitaComponenti;
end;

procedure TA167FRegistraIncentivi.sbtMeseCompMaxClick(Sender: TObject);
begin
  edtMeseCompMax.Text:=Copy(DateToStr(DataOut(StrToDate('01/' + edtMeseCompMax.Text),'Mese competenza massimo','M')),4,7);
end;

function TA167FRegistraIncentivi.GiorniRifMese:Real;
begin
  if R180In(TipoQuota,['A','S','T','I','V','C','Q']) //escluse: D, N, P e R perché non usano le variabili calcolate coi giorni del mese
  and (StrToFloatDef(VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'GIORNI_MESE')),0) > 0) then
    Result:=StrToFloat(VarToStr(A167FRegistraIncentiviDtM.A167FRegistraIncentiviMW.selT765.Lookup('CODICE',CodQuota,'GIORNI_MESE')))
  else
    Result:=R180GiorniMese(MeseCorr);
end;
...
procedure TA167FRegistraIncentivi.frmInputPeriodoedtFineExit(Sender: TObject);
begin
  ImpostaDataQuote;
  CaricaComboTipoCalcolo;
  edtMeseCompMax.Text:=Copy(edtMeseCompMax.Text,1,2) + '/' +  IntToStr(R180Anno(DataF));
end;

{ DataF }
function TA167FRegistraIncentivi._GetDataF: TDateTime;
begin
  Result:=R180FineMese(frmInputPeriodo.DataFine);
end;

procedure TA167FRegistraIncentivi._PutDataF(const Value: TDateTime);
begin
...
{ DataI }
function TA167FRegistraIncentivi._GetDataI: TDateTime;
begin
  if TipoQuota = 'Q' then
    //Result:=StrToDate('31/01/' + IntToStr(R180Anno(frmInputPeriodo.DataInizio)));
    Result:=EncodeDate(R180Anno(frmInputPeriodo.DataInizio),1,31)
  else
    Result:=R180FineMese(frmInputPeriodo.DataInizio);
end;

procedure TA167FRegistraIncentivi._PutDataI(const Value: TDateTime);
begin
  frmInputPeriodo.DataInizio:=R180InizioMese(Value);
end;
{ ----DataI }
