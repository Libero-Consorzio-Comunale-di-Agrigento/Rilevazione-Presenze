...
uses
  ...
  Spin,Grids,Variants,C180FunzioniGenerali,
  A000UCostanti, A000UMessaggi, A000USessione,A000UInterfaccia,
  A050UOrologi, A020UCausPresenze, A021UCausGiustif,
...
var I : Integer;
    CausOld,CausNew,TipoCaus:String;
begin
  BitBtn1.SetFocus;
  if A023FTimbratureDtM1.A023FTimbratureMW.Q100.FieldByName('ORA').IsNull then
    exit;
  CausOld:=VarToStr(A023FTimbratureDtM1.A023FTimbratureMW.Q100.FieldByName('CAUSALE').OldValue);
  CausNew:=VarToStr(A023FTimbratureDtM1.A023FTimbratureMW.Q100.FieldByName('CAUSALE').Value);
  A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.EU:=#0;
  TipoCaus:=IfThen(A023FTimbratureDtM1.A023FTimbratureMW.Q275.Locate('CODICE',CausNew,[]),'P','G');
  //Filtro dzionario su causale
  if A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.StatoTimb = stInserimento then
  begin
    if CausNew <> '' then
    begin
      if TipoCaus = 'P' then
      begin
        if not A000FiltroDizionario('CAUSALI PRESENZA',CausNew) then
          raise Exception.Create(A000MSG_ERR_CAUSALE_DIZIONARIO);
        // PRE: Q275 locate su causale corretta
        if A023FTimbratureDtM1.A023FTimbratureMW.Q275.FieldByName('INSERIMENTO_TIMB').AsString = 'N' then
          raise Exception.Create(A000MSG_ERR_CAUSALE_NO_INSERIMENTO_TIMB);
      end;
      if (TipoCaus = 'G') and (not A000FiltroDizionario('CAUSALI GIUSTIFICAZIONE',CausNew)) then
        raise Exception.Create(A000MSG_ERR_CAUSALE_DIZIONARIO);
    end
  end
  else
  begin
    if (CausNew <> '') and (CausOld <> CausNew) then
    begin
      if TipoCaus = 'P' then
      begin
        if not A000FiltroDizionario('CAUSALI PRESENZA',CausNew) then
