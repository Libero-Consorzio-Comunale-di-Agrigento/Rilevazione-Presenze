...
uses
  ...
  R001UGESTTAB, StdCtrls, DBCtrls, ExtCtrls, Mask, DB, Menus, Buttons, ComCtrls,
  ...
  C005UDatiAnagrafici, C180FunzioniGenerali, C700USelezioneAnagrafe, SelAnagrafe, A017URaggrAsse,
  ActnList, ImgList, ToolWin, Variants,
...
type
  TA010FProfAsseInd = class(TR001FGestTab)
    ...
    lblDataRes: TLabel;
    dedtDataRes: TDBEdit;
    lblVariazManuale: TLabel;
    dedtVariazManuale: TDBEdit;
    ...
    lblVariazFerieSol: TLabel;
    dedtVariazFerieSol: TDBEdit;
    ...
    procedure dcmbFamiliariKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
  private
...
begin
  with A010FProfAsseIndDtM1 do
  begin
    selSG101.SetVariable('Progressivo',C700Progressivo);
    selSG101.Close;
    selSG101.Open;
...
begin
  Querystampa.Clear;
  QueryStampa.Add('SELECT T1.PROGRESSIVO,T1.DAL,T1.AL,T1.CODRAGGR,T2.DESCRIZIONE,T1.UMISURA,T1.AGGIORNABILE,T1.DATARES,T1.DECURTAZIONE,T1.FAMILIARE_DATANAS,T1.VARIAZ_FERIESOL');
  ...
  NomiCampiR001.Add('T1.VARIAZ_FERIESOL');
...
procedure TA010FProfAsseInd.dcmbFamiliariKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  if Key = vk_Delete then
    if DButton.State in [dsEdit,dsInsert] then
    begin
      A010FProfAsseIndDtM1.T263.FieldByName('FAMILIARE_DATANAS').AsDateTime:=DATE_NULL;
...
begin
  ...
  begin
    ...
    begin
      ...
      FieldByName('Variaz_FerieSol').EditMask:='!#990.00;1;_';
    end
    else
    begin
      FieldByName('Decurtazione').EditMask:='!#990,9;1;_';
      FieldByName('Variaz_FerieSol').EditMask:='!#990,9;1;_';
    end;
  end;
end;
