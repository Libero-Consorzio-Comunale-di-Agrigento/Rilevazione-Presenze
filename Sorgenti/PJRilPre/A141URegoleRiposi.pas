...
uses
  ...
  C180FunzioniGenerali, A016UCausAssenze, System.Actions, System.ImageList;
...
type
  TA141FRegoleRiposi = class(TR001FGestTab)
    ...
    Panel7: TPanel;
    Label2: TLabel;
    dChkPrioritaFestivi: TDBCheckBox;
    lblRipCompSaldoNeg: TLabel;
    dcmbCausaleNeg: TDBLookupComboBox;
    lblSelNeg: TLabel;
    lblRiposoLavorato: TLabel;
    dcmbRiposoLavorato: TDBLookupComboBox;
    lblSelRiposoLavorato: TLabel;
    dChkRichNonElab: TDBCheckBox;
    ...
    procedure dcmbCausaleNegCloseUp(Sender: TObject);
    procedure dcmbCausaleNegKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
  private
...
begin
  inherited;
  dCmbCausale3CloseUp(Sender);
end;

procedure TA141FRegoleRiposi.dcmbCausaleNegCloseUp(Sender: TObject);
begin
  inherited;
  if dCmbCausaleNeg.KeyValue <> Null then
    lblSelNeg.Caption:=A141FRegoleRiposiDtM.QCausaleNegT265DESCRIZIONE.AsString
  else
    lblSelNeg.Caption:='';
end;

procedure TA141FRegoleRiposi.dcmbCausaleNegKeyUp(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
  dcmbCausaleNegCloseUp(Sender);
end;
...
begin
  ...
  dCmbCausale2CloseUp(Nil);
  dCmbCausale3CloseUp(Nil);
  dcmbCausaleNegCloseUp(Nil);
  dcmbRiposoLavoratoCloseUp(Nil);

  lblRipCompSaldoNeg.Enabled:=drdgTipo.ItemIndex = 0;
  dCmbCausaleNeg.Enabled:=drdgTipo.ItemIndex = 0;
  lblSelNeg.Enabled:=drdgTipo.ItemIndex = 0;
end;
