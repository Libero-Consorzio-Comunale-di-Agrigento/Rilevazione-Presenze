...
uses
  ...
  System.ImageList, C013UCheckList;
...
type
  TA164FQuoteIncentivi = class(TR004FGestStorico)
    ...
    dchkConsideraProva: TDBCheckBox;
    lblDurataSospensione: TLabel;
    dedtDurataSospensione: TDBEdit;
    lblTipiRapportoSospensione: TLabel;
    dedtTipiRapportoSospensione: TDBEdit;
    btnTipiRapportoSospensione: TBitBtn;
    lblEsclusioneValori: TLabel;
    dedtEsclusioneValori: TDBEdit;
    btnEsclusioneValori: TBitBtn;
...
begin
  ...
  begin
    ...
    begin
      ...
      dedtNumOre.Enabled:=(TipoQuota = 'Q') or A164FQuoteIncentiviMW.getRifSaldoAnnoCorrByCod(txtTipoQuota);
...
begin
  ...
  QueryStampa.Add('  T1.considera_prova, T1.tipirapporto_sospensione, T1.durata_sospensione, T1.esclusione_valori,');
  ...
  NomiCampiR001.Add('T1.CONSIDERA_PROVA');
  NomiCampiR001.Add('T1.TIPIRAPPORTO_SOSPENSIONE');
  NomiCampiR001.Add('T1.DURATA_SOSPENSIONE');
  NomiCampiR001.Add('T1.ESCLUSIONE_VALORI');
...
procedure TA164FQuoteIncentivi.btnTipiRapportoSospensioneClick(Sender: TObject);
var ElencoValoriChecklist: TElencoValoriChecklist;
begin
  inherited;

  ElencoValoriChecklist:=A164FQuoteIncentiviDtM.A164FQuoteIncentiviMW.ListTipiRapporto;
  ...
  try
    ...
    R180PutCheckList(dedtTipiRapportoSospensione.Text,5,C013FCheckList.clbListaDati);
    if ShowModal = mrOK then
      dedtTipiRapportoSospensione.Text:=R180GetCheckList(5,C013FCheckList.clbListaDati);
  finally
...
procedure TA164FQuoteIncentivi.btnEsclusioneValoriClick(Sender: TObject);
var ElencoValoriChecklist: TElencoValoriChecklist;
begin
  inherited;

  ElencoValoriChecklist:=A164FQuoteIncentiviDtM.A164FQuoteIncentiviMW.ListEsclusioneValori;
  ...
  try
    ...
    R180PutCheckList(dedtEsclusioneValori.Text,A164FQuoteIncentiviDtM.A164FQuoteIncentiviMW.LungCodEscl,C013FCheckList.clbListaDati);
    if ShowModal = mrOK then
      dedtEsclusioneValori.Text:=R180GetCheckList(A164FQuoteIncentiviDtM.A164FQuoteIncentiviMW.LungCodEscl,C013FCheckList.clbListaDati);
  finally
    FreeAndNil(ElencoValoriChecklist);
    Release;
  end;
