...
type
  TA010FProfAsseIndDtM1 = class(TDataModule)
    ...
    T263FAMILIARE_DATANAS: TDateTimeField;
    T263VARIAZ_FERIESOL: TStringField;
...
begin
  if T263.FieldByName('FAMILIARE_DATANAS').IsNull then
    T263.FieldByName('FAMILIARE_DATANAS').AsDateTime:=DATE_NULL;

  //Specificare il familiare di riferimento è possibile solo se si tratta di una variazione
  if (T263.FieldByName('FAMILIARE_DATANAS').AsDateTime <> DATE_NULL) and
     (T263.FieldByName('DECURTAZIONE').IsNull) then
    T263.FieldByName('FAMILIARE_DATANAS').AsDateTime:=DATE_NULL;

  //Ferie solidali
  if not T263.FieldByName('VARIAZ_FERIESOL').IsNull then
    if R180MessageBox('Attenzione: considerare anche la variazione dovuta alle ferie solidali. Continuare?','DOMANDA') <> mrYes then
      Abort;
  ...
  begin
    ...
    SetVariable('FAMILIARE_DATANAS',T263.FieldByName('FAMILIARE_DATANAS').AsString);
  ...
  if QueryPK1.EsisteChiave('T263_PROFASSIND',T263.RowId,T263.State,['PROGRESSIVO','DAL','CODRAGGR','FAMILIARE_DATANAS'],[T263Progressivo.AsString,T263Dal.AsString,T263CodRaggr.AsString,T263Familiare_Datanas.AsString]) then
  ...
  T263Variaz_FerieSol.AsString:=StringReplace(T263Variaz_FerieSol.AsString,' ','',[rfReplaceAll]);
