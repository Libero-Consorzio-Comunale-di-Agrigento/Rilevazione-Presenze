...
type
  TA077FGeneratoreStampe = class(TR003FGeneratoreStampe)
    ...
    pnlFiltroConteggi: TPanel;
    Label7: TLabel;
    Label10: TLabel;
    edtDalleOre: TMaskEdit;
    edtAlleOre: TMaskEdit;
    chkConteggiGGIter: TCheckBox;
...
begin
  ...
  A077FGeneratoreStampeDtM.R003FGeneratoreStampeMW.CodForm:='A077';//Uguale per Win e Cloud
  inherited;
end;
...
begin
  inherited;
  chklstAssenze.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstPresenza.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstIndPresenza.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstRimborsi.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstVociPaghe.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstIscrizioniSindacali.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstPartecipazioniSindacali.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstPermessiSindacali.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
  chklstCorsiFormazione.Enabled:=(DButton.State in [dsEdit,dsInsert]) or (not GeneratoreDiStampe);
end;
  ...
  begin
    ...
    begin
      ...
      if Molteplice(26) >= 0 then
        CreateT920_xx(A077FGeneratoreStampeMW.Ins920_26,'VSG101_FAMILIARI');
    end;
    CreateT920;
  end;
end;
...
begin
  ...
  pnlFiltroConteggi.Visible:=cmbSerbatoi.ItemIndex in [6];
end;
