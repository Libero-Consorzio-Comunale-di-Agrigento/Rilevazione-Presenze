...
uses
  ...
  C180FunzioniGenerali, Variants, Math, A000UMessaggi, System.UITypes;
...
type
  TA080FModConteDtM1 = class(TDataModule)
    ...
    Q025LIMITE_MM_ECCRES_CAUSALI: TStringField;
    selT275LimiteCaus: TOracleDataSet;
    Q025CAUSALI_COMP_ANN_ADDEB_PAGHE: TStringField;
    Q025CAUSALI_COMP_MENS_SALDO: TStringField;
    Q025RIPCOM_INCLUSI_SALDO: TStringField;
    Q025CAUSALI_COMP_ANN_SALDO: TStringField;
    Q025ITER_AUTSTR_ARROT_LIQ: TStringField;
    Q025ITER_AUTSTR_MINIMO_LIQ: TStringField;
    Q025ITER_AUTSTR_CAUSALE: TStringField;
    Q025ITER_AUTSTR_ARROT_ECC: TStringField;
    selT275: TOracleDataSet;
    dsrT275: TDataSource;
    selT275CODICE: TStringField;
    selT275DESCRIZIONE: TStringField;
    Q025D_ITER_AUTSTR_CAUSALE: TStringField;
    Q025XPARAM: TStringField;
    Q025ITER_AUTSTR_ARROT_LIQ_FASCE: TStringField;
    Q025RIEPASS_COMPENSABILI_ANNO: TStringField;
    Q025RIEPASS_COMPENSABILI_MESE: TStringField;
    dsrT265RNF: TDataSource;
    Q025RECUPERODEBITO_PERIODICITA: TStringField;
    Q025GGVUOTO_TURNISTA: TStringField;
    Q025CAUSALI_COMP_MESE_RECNEG: TStringField;
    Q026TIPO_ABBATTIMENTO: TStringField;
    Q025ITER_AUTSTR_DISPONIBILITA: TStringField;
    procedure A080FModConteDtM1Create(Sender: TObject);
    ...
    procedure Q025ITER_AUTSTR_MINIMO_LIQValidate(Sender: TField);
    procedure Q025ITER_AUTSTR_ARROT_LIQValidate(Sender: TField);
    procedure Q025ITER_AUTSTR_ARROT_ECCValidate(Sender: TField);
    procedure Q026BeforePost(DataSet: TDataSet);
  private
...
procedure TA080FModConteDtM1.A080FModConteDtM1Create(Sender: TObject);
var i:Integer;
begin
...
begin
  ...
  (*Alberto 21/07/2017 - MESSINA_UNIVERSITA: consentito usare stessa causale per compensazione mensile e annuale
  ...
  *)
  ...
  if Q025.FieldByName('LIMITE_MM_ECCRES_TIPO').AsString <> 'EG' then
  begin
    Q025.FieldByName('LIMITE_MM_ECCRES_CAUSALI').Clear;
  end;

  // verifica dati tipo iter straordinari
  if Q025.FieldByName('ITER_AUTORIZZATIVO_STR').AsString = TTipoRichStrRec.StraordAnnuoCausPagComp then
  begin
    // arrotondamento liquidabile specifico
    LArrotLiq:=Q025.FieldByName('ITER_AUTSTR_ARROT_LIQ').AsString;
    if LArrotLiq = '' then
      LMinutiArrotLiq:=1
    else
    ...
    begin
      A080FModConte.PageControl1.ActivePage:=A080FModConte.TabSheet4;
      A080FModConte.dedtIterAutStrArrotLiq.SetFocus;
      raise Exception.Create(A000MSG_A080_ERR_ARRLIQ_POSITIVO);
    end;

    // minimo liquidabile
    LMinimoLiq:=Q025.FieldByName('ITER_AUTSTR_MINIMO_LIQ').AsString;
    ...
    begin
      A080FModConte.PageControl1.ActivePage:=A080FModConte.TabSheet4;
      A080FModConte.dedtIterAutStrMinimoLiq.SetFocus;
      raise Exception.CreateFmt(A000MSG_A080_ERR_FMT_MINLIQ_MOD_ARRLIQ,[LMinimoLiq,LArrotLiq]);
    end;
  end;
end;
...
begin
  ...
  Q025.FieldByName('D_CAUSRIPCOM_FASCE').AsString:=VarToStr(selT275EsclNorm.Lookup('CODICE',Q025.FieldByName('CAUSRIPCOM_FASCE').AsString,'DESCRIZIONE'));
end;

procedure TA080FModConteDtM1.Q025ITER_AUTSTR_ARROT_ECCValidate(Sender: TField);
begin
  if not Sender.IsNull then
    OreMinutiValidate(Sender.AsString);
end;

procedure TA080FModConteDtM1.Q025ITER_AUTSTR_ARROT_LIQValidate(Sender: TField);
begin
  if not Sender.IsNull then
    OreMinutiValidate(Sender.AsString);
end;

procedure TA080FModConteDtM1.Q025ITER_AUTSTR_MINIMO_LIQValidate(Sender: TField);
begin
  if not Sender.IsNull then
    OreMinutiValidate(Sender.AsString);
end;
...
procedure TA080FModConteDtM1.Q026BeforePost(DataSet: TDataSet);
begin
  Q026.FieldByName('TIPO_ABBATTIMENTO').AsString:=Q026.FieldByName('TIPO_ABBATTIMENTO').AsString.ToUpper;
  if not R180In(Q026.FieldByName('TIPO_ABBATTIMENTO').AsString,['N','P']) then
    raise Exception.Create(A000MSG_A080_ERR_TIPOABBATTIMENTO);
end;
