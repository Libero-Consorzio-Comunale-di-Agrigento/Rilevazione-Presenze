...
uses
  ...
  A003UDataLavoroBis, QRExport, Variants, CheckLst, Mask, StrUtils, Math,
  R500Lin, InputPeriodo;
...
type
  TA042FDialogStampa = class(TForm)
    ...
    btnTabella: TBitBtn;
    Panel6: TPanel;
    lblSeparatoreIntestazione: TLabel;
    edtSeparatoreIntestazione: TEdit;
    frmInputPeriodo: TfrmInputPeriodo;
    edtGGContinuativi: TEdit;
    lblGGContinuativi: TLabel;
...
begin
  ...
  A042FStampa.RepR.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
  A042FStampaTab.RepR.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
  A042FStampaGrafico.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
  A042FStampaProspetto.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
  A042FStampaEUCausalizzate.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;

  frmInputPeriodo.CaptionDataOutI:= 'Dalla data:';
  frmInputPeriodo.CaptionDataOutF:= 'Alla data:';
end;
...
begin
  ...
  A042FStampaPreAssDtM1.A042MW.SelAnagrafe:=C700SelAnagrafe;
  while not A042FStampaPreAssDtM1.A042MW.selI010.Eof do
  begin
    A042FDialogStampa.chkLIntestazione.Items.Add(A042FStampaPreAssDtM1.A042MW.selI010.FieldByName('NOME_LOGICO').AsString);
    A042FDialogStampa.chkLDettaglio.Items.Add(A042FStampaPreAssDtM1.A042MW.selI010.FieldByName('NOME_LOGICO').AsString);
    A042FStampaPreAssDtM1.A042MW.selI010.Next;
  end;
...
begin
  ...
  begin
    ...
    begin
      if Trim(S) <> '' then
        S:=S + A042MW.SeparatoreIntestazione;
...
var DataCorr:TDateTime;
    ...
    GGContinuativiMin, GGContinuativiContati: integer;
    DataPrec: TDateTime;
    Inserimento: Boolean;
    LstDate:TStringList; //Lista temporana di date da inserire in stampa
    i: integer;
begin
  frmSelAnagrafe.ElaborazioneInterrompibile:=True;
  Self.Enabled:=False;
  TryStrToInt(edtGGContinuativi.Text, GGContinuativiMin);
  ...
  begin
    ...
    begin
      Inserimento:=not ((rgpTipoStampa.ItemIndex = 2) and (GGContinuativiMin > 0));
      LstDate.Clear;
      GGContinuativiContati:=1;
      DataPrec:=DaData-1;
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    begin
                      LstDate.Add(DateTimeToStr(DataCorr));
                      GGContinuativiContati:=IfThen(DataPrec = DataCorr-1,GGContinuativiContati+1,1);
                      DataPrec:=DataCorr;
                      if GGContinuativiContati >= GGContinuativiMin then
                      begin
                        for i:=0 to LstDate.Count-1 do
                          A042MW.InserisciDipendente(C700Progressivo, StrToDateTime(LstDate[i]));
                        Inserimento:=True;
                      end;
                    end
                    else
...
begin
  ...
  if rgpTipoStampa.ItemIndex = 2 then //Assenti senza giustif.
  begin
    if TryStrToInt(edtGGContinuativi.Text, GG) and (GG > AData-DaData+1) then
      raise exception.Create('Il numero di giorni minimi continuativi non deve superare la durata del periodo.');
  end;
end;
...
begin
  ...
  edtGGContinuativi.Text:=C004FParamForm.GetParametro('GGCONTINUATIVI','0');
  rgpTipoStampa.ItemIndex:=StrToInt(C004FParamForm.GetParametro('TIPOSTAMPA','0'));
  edtSeparatoreIntestazione.Text:=C004FParamForm.GetParametro('SEPARATORE_INTESTAZIONE',' - ');
...
begin
  ...
  C004FParamForm.PutParametro('GGCONTINUATIVI', edtGGContinuativi.Text);
  C004FParamForm.PutParametro('TIPOSTAMPA',IntToStr(rgpTipoStampa.ItemIndex));
  C004FParamForm.PutParametro('SEPARATORE_INTESTAZIONE',edtSeparatoreIntestazione.Text);
...
begin
  ...
  R180AbilitaOggetti(frmInputPeriodo, not chkGiornoCorrente.Checked);
end;
...
begin
  ...
  frmInputPeriodo.btnDataFine.Enabled:= (rgpTipoStampa.ItemIndex in [0,1,2,5]) and (not chkGiornoCorrente.Checked);
  frmInputPeriodo.edtFine.Enabled:= (rgpTipoStampa.ItemIndex in [0,1,2,5]) and (not chkGiornoCorrente.Checked);
  //Disabilita l'autocompletamento se i controlli sono disabilitati
  if frmInputPeriodo.edtFine.Enabled then frmInputPeriodo.AutoCompletamento:= acMese
    else frmInputPeriodo.AutoCompletamento:= AcNessuno;
  ...
  lblSeparatoreIntestazione.Enabled:=chkLIntestazione.Enabled;
  edtSeparatoreIntestazione.Enabled:=chkLIntestazione.Enabled;
...
{ DataF }
function TA042FDialogStampa._GetDataF: TDateTime;
begin
  Result := frmInputPeriodo.DataFine;
end;

procedure TA042FDialogStampa._PutDataF(const Value: TDateTime);
begin
  frmInputPeriodo.DataFine := Value;
end;
{ ----DataF }
{ DataI }
function TA042FDialogStampa._GetDataI: TDateTime;
begin
  Result := frmInputPeriodo.DataInizio;
end;

procedure TA042FDialogStampa._PutDataI(const Value: TDateTime);
begin
