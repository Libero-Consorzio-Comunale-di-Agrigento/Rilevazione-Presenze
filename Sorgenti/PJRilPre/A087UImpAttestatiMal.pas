...
type
  ...
  private
    ...
    ChiediFiltroDipendenti: Boolean;
...
begin
  ...
  try
    ...
    if Parametri.ModuloInstallato['TORINO_CSI_PRV'] and (DebugHook = 0) then
    begin
    ...
    (*
    //Apertura automatica della C700 su tutte le anagrafiche disponibili
    C700SelAnagrafe.Close;
    C700FSelezioneAnagrafe.CorpoSQL.Text:='T030.PROGRESSIVO >= 0';
    ...
    *)
    ...
    begin
      ...
      ChiediFiltroDipendenti:=False;
      try
        FiltroDipendenti;
      finally
        ChiediFiltroDipendenti:=True;
      end;
    end;
...
begin
  ...
  begin
    ...
    if btnDipEsclusi.Enabled and Self.ChiediFiltroDipendenti then
    begin
...
begin
  ...
  try
    dbgrdPreview.DataSource.DataSet.DisableControls;
    FiltroDipendenti;
  finally
    dbgrdPreview.DataSource.DataSet.EnableControls;
  end;
  if MyTab = TabCancManuale then
  try
    dgridCanc.DataSource.DataSet.DisableControls;
    selT048CancManC700;
  finally
    dgridCanc.DataSource.DataSet.EnableControls;
  end;
end;
