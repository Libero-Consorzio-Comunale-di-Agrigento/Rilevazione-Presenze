...
uses
  ...
  Oracle, OracleData, ActnList, ImgList, ToolWin, SelAnagrafe, Variants, System.Actions;
...
type
  ...
  TA094FSkLimitiStr = class(TR001FGestTab)
    ...
    dgrdT830: TDBGrid;
    Panel5: TPanel;
    SplitterT825_T830: TSplitter;
    pnlT830: TPanel;
    dgrdT825: TDBGrid;
    procedure DButtonDataChange(Sender: TObject; Field: TField);
    procedure dgrdT820DrawColumnCell(Sender: TObject; const Rect: TRect;
...
var
  i,DiffWidth,DiffHeight:Integer;
const
  NEW_HEIGHT = 600;
  NEW_WIDTH = 1100;
begin
  ...
  dgrdT820.ReadOnly:=Parametri.A094_Mese = 'N';
  TOracleDataSet(dgrdT820.DataSource.DataSet).ReadOnly:=Parametri.A094_Mese = 'N';
  dgrdT825.ReadOnly:=Parametri.A094_Anno = 'N';
  dgrdT830.ReadOnly:=Parametri.A094_Anno = 'N';
  TOracleDataSet(dgrdT825.DataSource.DataSet).ReadOnly:=Parametri.A094_Anno = 'N';
  TOracleDataSet(dgrdT830.DataSource.DataSet).ReadOnly:=Parametri.A094_Anno = 'N';
  TabSheet3.Enabled:=Parametri.A094_Raggr = 'S';
  TabSheet4.Enabled:=Parametri.A094_Raggr = 'S';
  TabSheet5.Enabled:=Parametri.A094_Raggr = 'S';
  for i:=dgrdT820.Columns.Count - 1 downto 0 do
    if (UpperCase(dgrdT820.Columns[i].FieldName) = 'CAUSALE') or
       (UpperCase(dgrdT820.Columns[i].FieldName) = 'DAL') or
       (UpperCase(dgrdT820.Columns[i].FieldName) = 'AL') then
      dgrdT820.Columns[i].Visible:=Parametri.CampiRiferimento.C15_LimitiMensCaus = 'S';
  if not A094FSkLimitiStrDtM1.A094FSkLimitiStraordMW.EsisteT203 then
  begin
    pnlT830.Visible:=False;
    dgrdT830.Visible:=False;
    SplitterT825_T830.Visible:=False;
    dgrdT825.Align:=alClient;
  end
  else
  begin
    DiffWidth:=NEW_WIDTH - Width;
    DiffHeight:=NEW_HEIGHT - Height;
    Width:=NEW_WIDTH;
    Height:=NEW_HEIGHT;
    Left:=Left - (DiffWidth div 2);
    Top:=Top - (DiffHeight div 2);
  end;
end;
...
procedure TA094FSkLimitiStr.FormCreate(Sender: TObject);
const
  INI_HEIGHT = 500;
  INI_WIDTH = 700;
begin
...
procedure TA094FSkLimitiStr.dgrdT820DrawColumnCell(Sender: TObject;
    ...
    for i:=0 to High(ElencoMesi) do
      try
        ...
        begin
          ...
          begin
            dgrdT820.Canvas.Brush.Color:=clHighLight;
            dgrdT820.Canvas.Font.Color:=clWhite;
          end
          else
          begin
            dgrdT820.Canvas.Brush.Color:=$00FFFF80;
            dgrdT820.Canvas.Font.Color:=clWindowText;
          end;
          dgrdT820.DefaultDrawColumnCell(Rect,DataCol,Column,State);
          Break;
        end;
      except
      end;
