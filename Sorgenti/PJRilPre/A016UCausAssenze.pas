...
uses
  ...
  Oracle, System.Generics.Collections, System.ImageList;
...
type
  TA016FCausAssenze = class(TR001FGestTab)
    ...
    gbxUMInserimento: TGroupBox;
    dchkUMInserimento: TDBCheckBox;
    dchkUMInserimentoMg: TDBCheckBox;
    dchkUMInserimentoH: TDBCheckBox;
    ...
    dchkUMInserimentoD: TDBCheckBox;
    lblFruizMax: TLabel;
    ...
    dchkPeriodoCumuloPersonalizzato: TDBCheckBox;
    lblPropPtVgg: TLabel;
    dcmbPropPtVgg: TDBComboBox;
    dchkCTMantieniResidNeg: TDBCheckBox;
    dchkIterEccGG: TDBCheckBox;
    dchkFruizGGNeutra: TDBCheckBox;
    dchkIterIgnoraFineRapporto: TDBCheckBox;
    TabSheet7: TTabSheet;
    ...
    Label26: TLabel;
    dedtFruizMinComp: TDBEdit;
    dedtCausaliCumuloL133: TDBEdit;
    btnCausaliCumuloL133: TButton;
    lblCausaliCumuloL133: TLabel;
    lblVocePaghe2: TLabel;
    dedtVocePaghe2: TDBEdit;
    ...
    procedure dchkUMInserimentoHClick(Sender: TObject);
    ...
    procedure dchkUMInserimentoClick(Sender: TObject);
...
begin
  ...
  begin
    ...
    begin
      ...
      dchkIterEccGG.Enabled:=DBComboBox1.Text = 'A';
      dchkTimbPMClick(nil);
    end;
  end;
end;
  ...
  begin
    ...
    begin
      if RadioValue = 0 then
        FieldByName('COMPETENZA' + i.ToString).EditMask:='!9990:00;1;_'  {hhhh.mm}
      else
        FieldByName('COMPETENZA' + i.ToString).EditMask:='!990,9;1;_';   {ggg,[5]}
    end;
  end;
...
begin
  ...
  with A016FCausAssenzeDtM1.A016MW do
  begin
    if (ppmnuProcPers.PopupComponent = ETipoCumulo) or (ppmnuProcPers.PopupComponent = dchkPeriodoCumuloPersonalizzato) then
      GetSourceSQL('T265P_GETPERIODOCUMULO')
...
begin
  ...
  (* Massimo: Riportato su MW
  with A016FCausAssenzeDtM1 do
  begin
    if T265.State in [dsInsert,dsEdit] then
    begin
      if Cod in ['D'] then
    ...
    *)
    ...
    dchkTempoDeterminato.Enabled:=not(Cod in ['H']);//(Cod in ['C','I','O']);
    ...
    dchkCTMantieniResidNeg.Left:=dchkCumuloFamGGDopo.Left;
...
procedure TA016FCausAssenze.AbilitaTipoPropPtVGG;
begin
  dcmbPropPtVgg.Visible:=chkPartTimeV.Checked;
 // and (EUMisura.ItemIndex = 1)
 //   and (A016FCausAssenzeDtM1.A016MW.Q260.FieldByName('CONTASOLARE').AsString = 'N')
 //   and (A016FCausAssenzeDtM1.A016MW.Q260.FieldByName('RESIDUABILE').AsString = 'N');
  if not dcmbPropPtVgg.Visible then
  begin
    if (DButton.State in [dsEdit,dsInsert]) then
      A016FCausAssenzeDtM1.T265.FieldByName('PropPtVGG').AsString:='A';
  end;
  lblPropPtVgg.Visible:=dcmbPropPtVgg.Visible;
end;
...
begin
  ...
  else if Control = dcmbPropPtVgg then
    (Control as TDBComboBox).Canvas.TextRect(Rect,Rect.Left + 2,Rect.Top,A016FCausAssenzeDtM1.A016MW.D_PropPtVGG[Index].Item);
end;
...
procedure TA016FCausAssenze.DBLookupComboBox1Click(Sender: TObject);
begin
  inherited;
  A016FCausAssenzeDtM1.A016MW.Q260.SearchRecord('CODICE',DbLookupComboBox1.Field.AsString,[srFromBeginning]);
  AbilitaTipoPropPtVGG;
...
begin
  ...
  with C013FCheckList do
  try
    Caption:='<C013> Causali per il cumulo';
...
procedure TA016FCausAssenze.btnCausaliCumuloL133Click(Sender: TObject);
var
  LElenco: string;
begin
  if not (DButton.State in [dsEdit,dsInsert]) then
  ...
  C013FCheckList:=TC013FCheckList.Create(nil);
  try
    C013FCheckList.Caption:='<C013> Causali primi 10 giorni';
    LElenco:=dedtCausaliCumuloL133.Field.AsString;
    CaricaLista;
    R180PutCheckList(LElenco,5,C013FCheckList.clbListaDati);
    if C013FCheckList.ShowModal = mrOK then
      dedtCausaliCumuloL133.Field.AsString:=R180GetCheckList(5,C013FCheckList.clbListaDati);
  finally
...
procedure TA016FCausAssenze.dchkUMInserimentoClick(Sender: TObject);
begin
  inherited;
  dchkFruizGGNeutra.Enabled:=dchkUMInserimento.Checked;
end;

procedure TA016FCausAssenze.dchkUMInserimentoHClick(Sender: TObject);
begin
  R180AbilitaOggetti(grpVincoliFruizione,dchkUMInserimentoH.Checked or dchkUMInserimentoD.Checked);
end;
...
begin
  ...
  with C013FCheckList do
  try
    Caption:='<C013> Causali inizio periodo';
    ...
    if DButton.State = dsEdit then
      ...
      begin
        ...
        begin
          ...
          DBCheckBox15.OnClick:=nil;
          DBCheckBox15.Checked:=True;
          DBCheckBox15.OnClick:=DBCheckBox15Click;
        end;
      end;
  end;
end;
