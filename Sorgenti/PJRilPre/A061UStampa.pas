...
begin
  ...
  dlblCognome.Enabled:=A061FDettAssenze.A061MW.StampaNominativo;
  dlblBadge.Enabled:=A061FDettAssenze.A061MW.StampaBadge;
  lblBadge.Enabled:=A061FDettAssenze.A061MW.StampaBadge;
  dlblMatricola.Enabled:=A061FDettAssenze.A061MW.StampaMatricola;
  lblMatricola.Enabled:=A061FDettAssenze.A061MW.StampaMatricola;
  dlblDatiAnagrafici.Enabled:=A061FDettAssenze.A061MW.StampaDatiAnagrafici;
  if not dlblCognome.Enabled then
  begin
    lblMatricola.Left:=dlblCognome.Left;
    dlblMatricola.Left:=lblMatricola.Left + 44;
  end
  else
  begin
    lblMatricola.Left:=268;
    dlblMatricola.Left:=312;
  end;
  ...
  dlblDatiAnagrafici.Top:=dlblCognome.Top;
  QRGroup2.Height:=ALTEZZA_BANDA;
...
var
  ...
  Trovato,FruitoConiuge,TotRaggrOTotGen:Boolean;
  S:String;
begin
  //DETTAGLIO
  with A061FDettAssenze.A061MW do
  begin
    ...
    if i = -1 then exit;
    FruitoConiuge:=Pos('^',Copy(RepR.DataSet.FieldByName('Causale').AsString,6)) > 0;
    TotRaggrOTotGen:=A061FDettAssenze.chkTotRaggrup.Checked or A061FDettAssenze.chkTotGenerali.Checked;
    if not FruitoConiuge or not TotRaggrOTotGen then
      TotInd[i].MinutiCont:=TotInd[i].MinutiCont + RepR.DataSet.FieldByName('MinutiCont').AsInteger;
    if not FruitoConiuge then
    begin
    ...
    Posiz:=Pos('^',Copy(RepR.DataSet.FieldByName('Causale').AsString,6));
    //Totali NON divisi per familiare ma divisi per coniuge
    if (not A061FDettAssenze.chkTotFam.Checked) and (A061FDettAssenze.chkConiuge.Checked)
    and (not FruitoConiuge or not TotRaggrOTotGen) then
    begin
      S:=Copy(RepR.DataSet.FieldByName('Causale').AsString,1,5);
      if Posiz > 0 then
      begin
        Posiz:=Posiz + 5;
        S:=S + Copy(RepR.DataSet.FieldByName('Causale').AsString,Posiz,Length(RepR.DataSet.FieldByName('Causale').AsString) - Posiz + 1);
      end;
      SalvaJ:=0;
      for j:=1 to 50 do
      begin
    ...
    Trovato:=False; //Lorena 26/07/2005
    Posiz:=Pos('#',Copy(RepR.DataSet.FieldByName('Causale').AsString,6));
    if (A061FDettAssenze.chkTotFam.Checked) and (Posiz > 0)
    and (not FruitoConiuge or not TotRaggrOTotGen) then
    begin
      Posiz:=Posiz + 5;
...
begin
  ...
  with A061FDettAssenze.A061MW do
    for i:=1 to NumCausali do
      if Trim(Copy(S,1,5)) = Trim(TotInd[i].Causale) then
      begin
        Result:=i;
        Break;
      end;
...
begin
  ...
  dlblDatiAnagrafici.DataSet:=A061FDettAssenze.A061MW.TabellaStampa;
end;
...
begin
  ...
  //            S:=Format('%-10s%-39s  GG:%5s  HH:%7s',                        //Lorena 31/08/2011
  ...
  //          S:=Format('%-10s%-39s  GG:%5s  HH:%7s',[Trim(TotInd[i].Causale),TotInd[i].Descrizione,S1,S2]);
            if A061FDettAssenze.chkConteggiGG.Checked then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if TotInd[i].Fam[j].TotMM = 0 then S2:='' else S2:=R180MinutiOre(TotInd[i].Fam[j].TotMM);
            if TotInd[i].Fam[j].TotMMCont = 0 then S3:='' else S3:=R180MinutiOre(TotInd[i].Fam[j].TotMMCont);
  //          S:=Format('%-10s%-39s  GG:%5s  HH:%7s',
  //            [Trim(TotInd[i].Causale) + TotInd[i].Fam[j].Codice,Copy(TotInd[i].Descrizione,1,21) + ' (' + IfThen(TotInd[i].Fam[j].GradoPar <> '',TotInd[i].Fam[j].GradoPar + ':','') + DateToStr(TotInd[i].Fam[j].DataNas) + ')',S1,S2]);
            if A061FDettAssenze.chkConteggiGG.Checked then
              S:=Format('%-10s%-33.33s  GG:%5s  HH:%7s  Cont:%7s',
...
begin
  Value:=Trim(Copy(Value,1,5)) + StringReplace(Copy(Value,6),'#','*',[]);
end;
