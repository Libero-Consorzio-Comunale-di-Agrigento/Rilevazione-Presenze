...
//  QRDetail1.Enabled:=A093FOperazioni.chkRicercaDipendenti.Checked and A093FOperazioni.chkDettaglio.Checked;
  QRDetail1.Enabled:=A093FOperazioni.chkRicercaDipendenti.Checked;
...
//  QRAnagrafico.Enabled:=A093FOperazioni.chkDipendenti.Checked and A093FOperazioni.chkDettaglio.Checked;
  QRAnagrafico.Enabled:=A093FOperazioni.chkDipendentiSelezionati.Checked;
...
begin
  with A093FOperazioni do
    ...
    QRLTitolo.Caption := 'Stampa log delle operazioni dal ' +
                         FormatDateTime('dd/mm/yyyy',DataI)+ ' al '+
                         FormatDateTime('dd/mm/yyyy',DataF);
    end;
end;
...
procedure TA093FStampa.QRDetail1BeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
...
procedure TA093FStampa.qrAnagraficoBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
var
  LProg: Integer;
begin
  {
  ...
  LProg:=A093FOperazioniDtM1.Q000.FieldByName('PROGRESSIVO').AsInteger;
  if LProg > 0 then
  begin
    C700SelAnagrafe.SearchRecord('PROGRESSIVO',LProg,[srFromBeginning]);
    qlblAnagrafico.Caption:=Format('MATRICOLA: %-8s  %s %s',[C700SelAnagrafe.FieldByName('MATRICOLA').AsString,C700SelAnagrafe.FieldByName('COGNOME').AsString,C700SelAnagrafe.FieldByName('NOME').AsString]);
  end
  else
  begin
