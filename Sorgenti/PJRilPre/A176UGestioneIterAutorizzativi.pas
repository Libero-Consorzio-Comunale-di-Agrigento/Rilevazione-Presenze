...
uses
  ...
  C180FunzioniGenerali, A000UCostanti, A000USessione, A083UMsgElaborazioni, A000UInterfaccia;
...
begin
  ...
  A176FRiepilogoIterAutorizzativiDM.A176MW.AnomaliePresenti:=False;
  ...
  begin
    EsitoElaboraborazione:=A176FRiepilogoIterAutorizzativiDM.A176MW.ConfermaAutorizzazioni;
  end;
  ...
  if not A176FRiepilogoIterAutorizzativiDM.A176MW.AnomaliePresenti then
  begin
  ...
  begin
    if R180MessageBox(Format('ERRORE:' + #13#10 + '"%s"!' + CRLF + 'Visualizzare le anomalie?',[EsitoElaboraborazione]),DOMANDA) = mrYes then
      OpenA083MsgElaborazioni(Parametri.Azienda, Parametri.Operatore,'A176','');
  end;
...
begin
  ...
  //Condizioni di abilitazione cancellazione dati su DataBase
  //Missioni
  ...
  //Richiesta str giorn
  AbilitaCancellazione:=AbilitaCancellazione or (A176FRiepilogoIterAutorizzativiDM.A176MW.C018.Iter = ITER_STRGIORNO);
...
begin
  ...
  begin
    btnEsegui.Enabled:=(cmbLivello.Items.Count = 0) and
                       (((A176FRiepilogoIterAutorizzativiDM.A176MW.C018.Iter = ITER_GIUSTIF) and (A176FRiepilogoIterAutorizzativiDM.A176MW.DataSetAttivo.FieldByName('T050ELABORATO').AsString.ToUpper <> 'S')) or
                        ((A176FRiepilogoIterAutorizzativiDM.A176MW.C018.Iter = ITER_TIMBR) and (A176FRiepilogoIterAutorizzativiDM.A176MW.DataSetAttivo.FieldByName('T105ELABORATO').AsString.ToUpper <> 'S')))
  end;
  AbilitazioneComponenti(rgpAzioni.Items[rgpAzioni.ItemIndex]);
end;
