...
begin
  ...
  CheckListBox1.Items.Add('DESCRIZIONE VOCE');
...
//  S:=UpperCase(EliminaRitornoACapo(C700SelAnagrafe.SQL.Text));
  ...
  begin
    ...
    begin
      ...
      else if CheckListBox1.Items[i] = 'DESCRIZIONE VOCE' then
      begin
        C:='DESCRIZIONE_VOCE';
        O2:='T195.' + C;
      end
  ...
  C700SelAnagrafe.SetVariable('DATALAVORO',A038FVociVariabili.DataF);
  C700SelAnagrafe.SetVariable('DATA1',A038FVociVariabili.DataI);
  C700SelAnagrafe.SetVariable('DATA2',A038FVociVariabili.DataF);
  ...
  with A038FStampaVoci do
    ...
    LTitolo.Caption:=Format('Voci variabili scaricate da %s a %s',[FormatDateTime('mmmm yyyy',A038FVociVariabili.DataI),FormatDateTime('mmmm yyyy',A038FVociVariabili.DataF)]);
    QRGroup1.Enabled:=R <> '';
