...
implementation
uses A092UStampaStorico, A092UStampaStoricoDtM1, A092UStampaStoricoMW;
{$R *.DFM}
...
procedure TA092FStampa.ChildBand2AfterPrint(Sender: TQRCustomBand;
  BandPrinted: Boolean);
begin
  LastMatricola:=A092FStampaStoricoDtM1.A092MW.TabellaStampa.FieldByName('Matricola').AsString;
end;
...
begin
  ...
  QRLTitolo.Caption := 'Stampa storicizzazioni dal ' + DateToStr(A092FStampaStorico.DataI) +
                       ' al '+DateToStr(A092FStampaStorico.DataF);
...
begin
  ChildBand2.Enabled:=LastMatricola = A092FStampaStoricoDtM1.A092MW.TabellaStampa.FieldByName('Matricola').AsString;
  ChildBand3.Enabled:=LastMatricola = A092FStampaStoricoDtM1.A092MW.TabellaStampa.FieldByName('Matricola').AsString;
end;

procedure TA092FStampa.QRBand1BeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  if CampoPrec <> A092FStampaStoricoDtm1.A092MW.TabellaStampa.FieldByName('Campo').AsString then
    begin
    CampoPrec:=A092FStampaStoricoDtm1.A092MW.TabellaStampa.FieldByName('Campo').AsString;
