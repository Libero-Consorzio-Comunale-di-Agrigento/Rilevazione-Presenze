...
type
  TA028FSCVisual = class(TForm)
    ...
    Label7: TLabel;
    grdStruttureConvenzionate: TStringGrid;
...
begin
  ...
  begin
    ...
    //
    grdStruttureConvenzionate.RowCount:=2;
    grdStruttureConvenzionate.ColCount:=3;
    grdStruttureConvenzionate.ColWidths[0]:=80;
    grdStruttureConvenzionate.ColWidths[1]:=70;
    grdStruttureConvenzionate.ColWidths[2]:=50;
    grdStruttureConvenzionate.Cells[0,0]:='Struttura';
    grdStruttureConvenzionate.Cells[1,0]:='Ore previste';
    grdStruttureConvenzionate.Cells[2,0]:='Ore rese';
    if Length(RiepStruttureConvenzionate) > 0 then
      grdStruttureConvenzionate.RowCount:=Length(RiepStruttureConvenzionate) + 1;
    for i:=0 to High(RiepStruttureConvenzionate) do
    begin
      grdStruttureConvenzionate.Cells[0,i + 1]:=RiepStruttureConvenzionate[i].Causale;
      grdStruttureConvenzionate.Cells[1,i + 1]:=R180MinutiOre(RiepStruttureConvenzionate[i].OrePianificate);
      grdStruttureConvenzionate.Cells[2,i + 1]:=R180MinutiOre(RiepStruttureConvenzionate[i].OreRese);
    end;
  end;
end;
//_________
...
begin
  ...
  begin
    ...
    GAnomalie2.Cells[1,i]:=tanom2riscontrate[i].ta2testo;//tdescanom2[tanom2riscontrate[i].ta2puntdesc].D;
  end;
  //_________
  ...
  begin
    GAnomalie3.Cells[0,i]:=IfThen(tanom3riscontrate[i].ta3timb >= 0,R180MinutiOre(tanom3riscontrate[i].ta3timb));
    GAnomalie3.Cells[1,i]:=tanom3riscontrate[i].ta3testo;//tdescanom3[tanom3riscontrate[i].ta3puntdesc].D;
  end;
  end;
end;
//_________
