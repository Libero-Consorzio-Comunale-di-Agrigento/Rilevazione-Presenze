...
uses
  ...
  DBCtrls, Buttons, DB, Variants, OracleData, Math, StrUtils,
  R500Lin, Rp502Pro, R600, RegistrazioneLog, SelAnagrafe, QueryStorico, DatiBloccati,
  C005UDatiAnagrafici, C180FunzioniGenerali, (*FunzioniGenerali,*) C700USelezioneAnagrafe,
  A000UMessaggi, A000UCostanti, A000USessione, A000UInterfaccia, A000UGestioneTimbraGiustMW,
  A004UGiustifAssPres, A006UModelliOrario, A023UValidaAssenze, A025UPianif, A027UCarMen,
  A028USc, A097UPianifLibProf, B021UUtils,A159UArchiviazioneCartellini;

type
  TNomiGiorni = array[1..7] of String;
  ...
  TA023FTimbrature = class(TFrmTabelle99)
    ...
    spArchiviazioneCartellini: TSpeedButton;
    Inforichiesta1: TMenuItem;
    Inforichiesta2: TMenuItem;
    Label1: TLabel;
    pnlRichiesteInCorso: TPanel;
    btnInfoRichieste1: TSpeedButton;
    btnInfoRichieste2: TSpeedButton;
    btnInfoRichieste3: TSpeedButton;
    btnInfoRichieste4: TSpeedButton;
    btnInfoRichieste5: TSpeedButton;
    btnInfoRichieste6: TSpeedButton;
    Accediamodificacausale1: TMenuItem;
    Accediamodificacausale2: TMenuItem;
    spLiberaProfessione: TSpeedButton;
    procedure FormCreate(Sender: TObject);
    procedure EMeseChange(Sender: TObject);
    procedure GTimbrat1DrawCell(Sender: TObject; Col, Row: Integer;
    ...
    procedure GTimbrat1MouseMove(Sender: TObject; Shift: TShiftState; X, Y: Integer);
    ...
    procedure Splitter1CanResize(Sender: TObject; var NewSize: Integer; var Accept: Boolean); {$IFNDEF DEBUG} inline; {$ENDIF}
    procedure GTimbrat2SelectCell(Sender: TObject; ACol, ARow: Integer; var CanSelect: Boolean);
    procedure spArchiviazioneCartelliniClick(Sender: TObject);
    procedure PopupMenu1Popup(Sender: TObject);
    procedure RichiesteInCorsoClick(Sender: TObject);
  private
    Mese,Anno:Word;
    ...
    procedure PulisciGriglia; {$IFNDEF DEBUG} inline; {$ENDIF}
    ...
    procedure GetRichiesteInCorso(PData: TDateTime; PControl: TControl);
...
uses A016UCausAssenze, A020UCausPresenze, A021UCausGiustif,
     ...
     C023UInfoDati, A023URichiesteInCorso;
...
begin
  ...
  FGriglie[1].RichiesteInCorso:=btnInfoRichieste1;
  ...
  FGriglie[2].RichiesteInCorso:=btnInfoRichieste2;
  ...
  FGriglie[3].RichiesteInCorso:=btnInfoRichieste3;
  ...
  FGriglie[4].RichiesteInCorso:=btnInfoRichieste4;
  ...
  FGriglie[5].RichiesteInCorso:=btnInfoRichieste5;
  ...
  FGriglie[6].RichiesteInCorso:=btnInfoRichieste6;
  ...
  {$ENDIF}
  spArchiviazioneCartellini.Visible:=A000GetInibizioni('Funzione','OpenA159ArchiviazioneCartellini') <> 'N';
  //Pianificazioneliberaprofessione1.Visible:=False;
  Pianificazioneliberaprofessione1.Visible:=(Parametri.ModuloInstallato['LIBERA_PROFESSIONE']) and (A000GetInibizioni('Funzione','OpenA097PianifLibProf') <> 'N');
  spLiberaProfessione.Visible:=Pianificazioneliberaprofessione1.Visible;
...
begin
  ...
  begin
    ...
    //***Inforichiesta2.Visible:=A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FGiustificativi[Day,Col].IDRichiesta > 0;
  end;
...
var
  ...
  LDataCorr:TDateTime;
  LIndex: Integer;
procedure EvidenziaNoteGiorno(Data: TDateTime; LGiorno: TLabel);
begin
  LGiorno.Hint:='';
  LGiorno.ShowHint:=False;
  if A023FTimbratureDtM1.A023FTimbratureMW.Q080.Locate('Data',Data,[]) then
  begin
    if A023FTimbratureDtM1.A023FTimbratureMW.Q080.FieldByName('Note').AsString <> '' then
    begin
      LGiorno.Caption:=LGiorno.Caption + ' *';
      LGiorno.Hint:=A023FTimbratureDtM1.A023FTimbratureMW.Q080.FieldByName('Note').AsString;
...
begin
  ...
  begin
    ...
    begin
      FGriglie[LIndex].Giorno.ParentColor:=True;
      FGriglie[LIndex].Giorno.Caption:='';
      FGriglie[LIndex].Giorno.Hint:='';
      FGriglie[LIndex].Giorno.ShowHint:=False;
      FGriglie[LIndex].Pianif.Visible:=False;
      FGriglie[LIndex].Orario.ParentFont:=True;
      FGriglie[LIndex].Orario.Text:='';
      FGriglie[LIndex].Timbrat.ColCount:=1;
      FGriglie[LIndex].Giustif.ColCount:=1;
      FGriglie[LIndex].OreLavorate.Caption:='';
      FGriglie[LIndex].Scostamento.Caption:='';
      FGriglie[LIndex].Debito.Caption:='';
      FGriglie[LIndex].EscluseDaNorm.Caption:='';
      FGriglie[LIndex].RichiesteInCorso.Visible:=False;
    end
    else
    begin
      LDataCorr:=EncodeDate(EAnno.Value,EMese.Value,G);
      Calendario(LDataCorr,FGriglie[LIndex].Giorno);
      Orario(LDataCorr,FGriglie[LIndex].Orario,FGriglie[LIndex].Turni);
      FGriglie[LIndex].Giorno.Caption:=NomiGiorni[DayOfWeek(LDataCorr)]+' '+IntToStr(G);
      EvidenziaNoteGiorno(LDataCorr,FGriglie[LIndex].Giorno);
      FGriglie[LIndex].Pianif.Visible:=A023FTimbratureDtM1.A023FTimbratureMW.FPianif[G];
      FGriglie[LIndex].Timbrat.ColCount:=A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FNumTimbrature[G]+1;
      FGriglie[LIndex].Giustif.ColCount:=A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FNumGiustif[G]+1;
      A023FTimbratureDtM1.A023FTimbratureMW.R502ProDtM1.PrimaVolta:='si';
      // effettua conteggi per determinare ore lavorate, scostamento, debito, ore escluse dalle normali
      if SBConteggi.Down then
        Conteggi(LIndex,EncodeDate(EAnno.Value,EMese.Value,G));
      GetRichiesteInCorso(LDataCorr,FGriglie[LIndex].RichiesteInCorso);
    end;
    FGriglie[LIndex].Timbrat.Hint:='';
    FGriglie[LIndex].Giustif.Hint:='';
    FGriglie[LIndex].Timbrat.Repaint;
    FGriglie[LIndex].Giustif.Repaint;
  end;
end;
...
begin
  ...
  begin
    ...
    EOrario.Text:=A023FTimbratureDtM1.A023FTimbratureMW.Q080.FieldByName('Orario').AsString;
...
procedure TA023FTimbrature.GetRichiesteInCorso(PData: TDateTime; PControl: TControl);
// verifica se per la data indicata esistono richieste di timbrature oppure di giustificativi
begin
  PControl.Tag:=Trunc(PData);
  Assert(not A023FTimbratureDtM1.A023FTimbratureMW.selVT105.Filtered, 'selVT105 è filtrato!!!');
  Assert(not A023FTimbratureDtM1.A023FTimbratureMW.selVT050.Filtered, 'selVT050 è filtrato!!!');
  Assert(not A023FTimbratureDtM1.A023FTimbratureMW.selVT085.Filtered, 'selVT085 è filtrato!!!');
  PControl.Visible:=A023FTimbratureDtM1.A023FTimbratureMW.selVT105.Locate('Data',PData,[]) or
                    A023FTimbratureDtM1.A023FTimbratureMW.selVT050.Locate('Data',PData,[]) or
                    A023FTimbratureDtM1.A023FTimbratureMW.selVT085.Locate('Data',PData,[]);
end;
...
var Timbrature:Boolean;
  ...
  begin
    ...
    if Verso = 'E' then
      ColoraCella(Griglia,Retng,clLime)
    else
      ColoraCella(Griglia,Retng,clAqua);
  end;
  ...
  begin
    RetGius:=Rect;
    if Tipo = 'I' then
      ColoraCella(Griglia,Rect,clRed);
    if Tipo = 'M' then
    begin
...
begin
  ...
  if (C700SelAnagrafe.RecordCount = 0) or
     (SolaLettura and ((Sender as TMenuItem).Tag in [1,2,3,7])) or
     ((AccessoA004 = 'N') and ((Sender as TMenuItem).Tag in [4,5,8])) then
    begin
      ShowOK:=False;
      Exit;
    end
  ...
  case (Sender as TMenuItem).Tag of
    1:
      if A023FTimbratureDtM1.A023FTimbratureMW.selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T100') then
    ...
    2,3:
      begin
        if A023FTimbratureDtM1.A023FTimbratureMW.selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T100') then
          raise Exception.Create(A023FTimbratureDtM1.A023FTimbratureMW.selDatiBloccati.MessaggioLog);
        //Chiamata: 83085
        if not A000FiltroDizionario('OROLOGI DI TIMBRATURA',A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FTimbrature[Day,Col].Rilevatore) then
    ...
    5:// modifica giustificativo
      begin
        ...
        if VarToStr(A023FTimbratureDtM1.A023FTimbratureMW.Q265.Lookup('CODICE',A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FGiustificativi[Day,Col].Causale,'VISITA_FISCALE')) = 'S' then
        ...
        if A023FTimbratureDtM1.A023FTimbratureMW.selDatiBloccati.DatoBloccato(C700Progressivo,R180InizioMese(Data),'T040') then
          raise Exception.Create(A023FTimbratureDtM1.A023FTimbratureMW.selDatiBloccati.MessaggioLog);
      end;
  end;
  with A023FTimbratureDtM1.A023FTimbratureMW do
    case (Sender as TMenuItem).Tag of
      ...
      9,10:
        begin
          ...
          try
            if (Sender as TMenuItem).Tag = 9 then
            begin
              C023.MostraInfoTimb(A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FTimbrature[Day,Col]);
            end
            else
            begin
              C023.MostraInfoGiust(A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FGiustificativi[Day,Col]);
            end;
          finally
            FreeAndNil(C023);
          end;
...
begin
  if Parametri.InserisciTimbrature <> 'S' then
    Exit;

  if A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FNumTimbrature[Day] = MaxTimbrature then
    Exit;

  A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.StatoTimb:=stInserimento;
  Col:=A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FNumTimbrature[Day] + 1;
  ...
  try
    ...
    A023FTimbratureDtM1.A023FTimbratureMW.Q100.Append;
    A023FGestTimbra.ShowModal;
  finally
    if A023FTimbratureDtM1.A023FTimbratureMW.Q100.State in [dsEdit,dsInsert] then
...
procedure TA023FTimbrature.RichiesteInCorsoClick(Sender: TObject);
var
  LDate: TDateTime;
  LA023Rich: TA023FRichiesteInCorso;
begin
  LDate:=(Sender as TControl).Tag;

  LA023Rich:=TA023FRichiesteInCorso.Create(nil);
  try
    LA023Rich.Data:=LDate;
    LA023Rich.ShowModal;
  finally
    FreeAndNil(LA023Rich);
  end;
end;
...
var PData,PTipo,POre1,POre2,PCausale:String;
begin
  Result:=True;
  ...
  frmSelAnagrafe.SalvaC00SelAnagrafe;
  C700Distruzione;
  OpenA004GiustifAssPres(C700Progressivo,PData,PTipo,POre1,POre2,PCausale,A023FTimbratureDtM1.A023FTimbratureMW.A000FGestioneTimbraGiustMW.FGiustificativi[Day,Col].DataNas,True);
...
begin
  ...
  if Col > MaxGiustif then
    Exit;
//commentato with A023FTimbratureDtM1.A023FTimbratureMW.Q040
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                begin
                  if Ng1 > 0 then
                    A023FTimbratureMW.R600DtM1.GestioneGiustifAssFuturi(C700Progressivo,Data - 1,OldGiustif,StrOldDataNas);
                  if Ng2 > 0 then
                    A023FTimbratureMW.R600DtM1.GestioneGiustifAssFuturi(C700Progressivo,Data - 1,NewGiustif,StrNewDataNas);
                  AssFutureElab:=True;
                  R180MessageBox('Elaborazione terminata',INFORMA);
                end;
              end;
            end;
            A023FTimbratureMW.B021FWebSvcClientDtM.ResetRecordT040;
            A023FTimbratureMW.B021FWebSvcClientDtM.RegistraRecordT040('I',A023FTimbratureMW.Q040,RGCausali.ItemIndex,'');//Per successiva chiamata al web sevice (Firlab)
            if A023FTimbratureMW.B021FWebSvcClientDtM.EsisteRecordT040 then
            begin
              A023FTimbratureMW.B021FWebSvcClientDtM.WebSvcRecordT040;
            end;
          end;
        end;
      end;
...
procedure TA023FTimbrature.spArchiviazioneCartelliniClick(Sender: TObject);
var
  Data: TDateTime;
begin
  inherited;
  ...
  OpenA159ArchiviazioneCartellini(Data, R180FineMese(Data));
  frmSelAnagrafe.RipristinaC00SelAnagrafe(A023FTimbratureDtM1.A023FTimbratureMW);
end;
