...
type
  ...
  private
    ...
    lstRilevatoriUsati: TStringList;
...
begin
  ...
  lstRilevatoriUsati:=TStringList.Create;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not QTimbrature.FieldByName('Rilevatore').IsNull then
        begin
          if lstRilevatoriUsati.IndexOf(QTimbrature.FieldByName('Rilevatore').AsString) < 0 then
            lstRilevatoriUsati.Add(QTimbrature.FieldByName('Rilevatore').AsString);
        end;
        if rgpTimbrature.ItemIndex in [0,3] then
...
var
  S,App,LTestoLegenda:String;
const
  RIGA_LEGENDA_FMT = '%-5s %-50s %-40s'#13#10;
begin
  CalcolaDate;
  if DataI > DataF then
  ...
  //Svuotamento utile quando il richiamo avviene da B028 (TipoModulo = 'COM') visto che non ripassa dal GetParametriFunzione
  ...
  A051FStampa.RepR.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
  ...
  // prepara il testo della legenda dei rilevatori opportunamente formattato
  //LTestoLegenda:=Format('%-*s%s%-*s'#13#10,[40,' ','Legenda rilevatori',40,' ']) +
  //               Format(RIGA_LEGENDA_FMT,['Cod.','Descrizione','Indirizzo']);
  LTestoLegenda:='';
  A051FTimbOrigDtM1.selT361.Close;
  A051FTimbOrigDtM1.selT361.Open;
  while not A051FTimbOrigDtM1.selT361.Eof do
  begin
    if lstRilevatoriUsati.IndexOf(A051FTimbOrigDtM1.selT361.FieldByName('CODICE').AsString) >= 0 then
    begin
      LTestoLegenda:=LTestoLegenda +
                    Format(RIGA_LEGENDA_FMT,
                           [A051FTimbOrigDtM1.selT361.FieldByName('CODICE').AsString,
                            Copy(A051FTimbOrigDtM1.selT361.FieldByName('DESCRIZIONE').AsString,1,50),
                            Copy(A051FTimbOrigDtM1.selT361.FieldByName('INDIRIZZO').AsString,1,40)]);
    end;
    A051FTimbOrigDtM1.selT361.Next;
  end;
  if LTestoLegenda <> '' then
    LTestoLegenda:=Format('%-*s%s%-*s'#13#10,[40,' ','Legenda rilevatori',40,' ']) +
                   Format(RIGA_LEGENDA_FMT,['Cod.','Descrizione','Indirizzo']) +
                   LTestoLegenda;
  A051FStampa.TestoLegenda:=LTestoLegenda;
