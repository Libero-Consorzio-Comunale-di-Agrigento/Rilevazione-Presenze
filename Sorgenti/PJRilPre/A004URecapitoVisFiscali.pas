...
uses
  ...
  C015UElencoValori, C180FunzioniGenerali, DBCtrls, DB, Vcl.Grids, Vcl.DBGrids,
  System.Character;
...
type
  TA004FRecapitoVisFiscali = class(TForm)
    ...
    edtNominativo: TEdit;
    grpRecapitiCertINPS: TGroupBox;
    dgrdRecapitiCertINPS: TDBGrid;
    edtDettagliIndirizzo: TEdit;
...
begin
  ...
  begin
    ...
    //Leggo eventuali dati su certificati INPS acquisiti
    selT048.Close;
    selT048.SetVariable('PROGRESSIVO',T047Progressivo);
    selT048.SetVariable('DATA',T047DataInizio);
    selT048.Open;
    grpRecapitiCertINPS.Visible:=selT048.RecordCount > 0;
    dgrdRecapitiCertINPS.DataSource:=dsrT048;
  end;
...
var
  ...
  function StripNonNumeric(const PStr: String): String;
  var
    I: Integer;
    c: char;
  begin
...
begin
  ...
  FTelefono:=StripNonNumeric(Trim(edtTelefono.Text));
  ...
  if (FNote <> '') and (FDettagliIndirizzo = '') then
    if R180MessageBox(A000TraduzioneStringhe(A000MSG_A004_DLG_NOTE_NO_DETT_INDIRIZZO),DOMANDA) = mrNo then
      Exit;
