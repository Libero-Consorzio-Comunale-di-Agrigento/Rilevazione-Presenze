...
uses
  ...
  A000UMessaggi, System.Actions, System.ImageList, Oracle;
...
begin
  ...
  try
    ...
    A000GetChiavePrimaria(SessioneOracle, TabelleDatiLiberi[TabControl1.TabIndex].NomeTabelle, lstPK);
    if (TabelleDatiLiberi[TabControl1.TabIndex].Accesso = 'R') or A005SolaLettura or (lstPK.Count = 0) then
    begin
      SolaLettura:=True;
    ...
    begin
      Tabella.Fields[i].Visible:=R180In(Tabella.Fields[i].FieldName,['CODICE','DESCRIZIONE']);
      if TabelleDatiLiberi[TabControl1.TabIndex].NomeTabelle.ToUpper = 'I501' + Parametri.CampiRiferimento.C29_ChiamateRepFiltro1.ToUpper then
      begin
        Tabella.Fields[i].Visible:=Tabella.Fields[i].Visible or R180In(Tabella.Fields[i].FieldName,['TELEFONO','EMAIL']);
      end;
    end;
  finally
    FreeAndNil(lstPK);
