...
uses
  ...
  ToolWin,A000UMessaggi, System.Actions, System.ImageList, InputPeriodo;
...
type
  ...
  public
    { Public declarations }
    CassaEffettiva,CassaUtilizzata,DataDa,DataA:TDateTime;
...
begin
  ...
  DataI:=R180InizioMese(Date);
  DataF:=R180InizioMese(Date);
end;
...
begin
  if DataDa <> 0 then //richiamo esterno
    ...
    DataI:=StrToDate(C004FParamForm.GetParametro('DADATA',DateToStr(DataI)));
  if DataA <> 0 then //richiamo esterno
    DataF:=DataA
  else
    DataF:=StrToDate(C004FParamForm.GetParametro('ADATA',DateToStr(DataF)));
  S:='''' + StringReplace(C004FParamForm.GetParametro('RIEPILOGHI',''),',',''',''',[rfReplaceAll]) + '''';
  for i:=0 to chklstRiepiloghi.Items.Count - 1 do
    chklstRiepiloghi.Checked[i]:=Pos('''' + Trim(Copy(chklstRiepiloghi.Items[i],1,5)) + '''',S) > 0;
end;
...
begin
  ...
  C004FParamForm.PutParametro('DADATA',DateToStr(DataI));
  C004FParamForm.PutParametro('ADATA',DateToStr(DataF));
...
begin
  ...
  C700DataDal:=DataI;
  C700DataLavoro:=R180FineMese(DataF);
...
begin
  ...
  for i:=0 to chklstRiepiloghi.Count - 1 do
    if chklstRiepiloghi.Checked[i] then
      Num:=Num + 1;
  if C700SelAnagrafe.GetVariable('DATALAVORO') <> R180FineMese(DataF) then
  begin
    C700SelAnagrafe.SetVariable('DATALAVORO',R180FineMese(DataF));
    C700SelAnagrafe.Close;
  end;
  if C700SelAnagrafe.VariableIndex('C700DATADAL') >= 0 then
    if C700SelAnagrafe.GetVariable('C700DATADAL') <> R180InizioMese(DataI) then
    begin
      C700SelAnagrafe.SetVariable('C700DATADAL',R180InizioMese(DataI));
      C700SelAnagrafe.Close;
    end;
  C700SelAnagrafe.Open;

  A126FBloccoRiepiloghiDtM1.A126FBloccoRiepiloghiMW.DaData:=DataI;
  A126FBloccoRiepiloghiDtM1.A126FBloccoRiepiloghiMW.AData:=DataF;
...
begin
  ...
  for i:=0 to chklstRiepiloghi.Items.Count - 1 do
    ...
    begin
      if Result <> '' then
        Result:=Result + ',';
      Result:=Result + '''' + Trim(Copy(chklstRiepiloghi.Items[i],1,5)) + '''';
    end;
  if Result = '' then
    Result:='''*''';
end;
  ...
  if PageControl1.ActivePage = tabBrowse then
    ...
    begin
      ...
      if VarToStr(GetVariable('RIEPILOGO')) <> S then
        Close;
      if GetVariable('DATA1') <> DataI then
        Close;
      if GetVariable('DATA2') <> DataF then
        Close;
...
{ DataF }
function TA126FBloccoRiepiloghi._GetDataF: TDateTime;
begin
  Result:=frmInputPeriodo.DataFine;
end;
procedure TA126FBloccoRiepiloghi._PutDataF(const Value: TDateTime);
begin
  frmInputPeriodo.DataFine:=Value;
end;
{ ----DataF }
{ DataI }
function TA126FBloccoRiepiloghi._GetDataI: TDateTime;
begin
  Result:=frmInputPeriodo.DataInizio;
end;
procedure TA126FBloccoRiepiloghi._PutDataI(const Value: TDateTime);
begin
