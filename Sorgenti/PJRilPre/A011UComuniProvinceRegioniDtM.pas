...
type
  TA011FComuniProvinceRegioniDtM = class(TR004FGestStoricoDtM)
    ...
    procedure T480AfterScroll(DataSet: TDataSet);
  private
...
begin
  ...
  T480.FieldByName('D_COD_IRPEF').LookupDataSet:=A011FEntiLocaliMW.selT480;
...
procedure TA011FComuniProvinceRegioniDtM.T480AfterScroll(DataSet: TDataSet);
begin
  inherited;
  A011FComuniProvinceRegioni.StatusBar.Panels[1].Text:=Format('Record %d/%d',[Dataset.RecNo,Dataset.RecordCount]);
end;
