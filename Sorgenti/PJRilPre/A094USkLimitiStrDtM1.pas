...
begin
  ...
  begin
    ...
    dgrdT825.DataSource:=A094FSkLimitiStraordMW.dsrT825;
    dgrdT830.DataSource:=A094FSkLimitiStraordMW.dsrT830;
  ...
  begin
    A094FSKLimitiStr.dgrdT820.Columns[5].PickList.Add(A000LimiteMensileLiquidabile);
    A094FSKLimitiStr.dgrdT820.Columns[5].PickList.Add(A000LimiteMensileResiduabile);
    Open;
    while not Eof do
    begin
      A094FSKLimitiStr.dgrdT820.Columns[5].PickList.Add(FieldByName('CODICE').AsString);
      Next;
    end;
    Close;
  end;
...
begin
  ...
  begin
    ...
    A094FSkLimitiStraordMW.selT830.SetVariable('Progressivo',C700Progressivo);
...
begin
  ...
  begin
    k:=-1;
    for i:=0 to dgrdT820.Columns.Count - 1 do
      if dgrdT820.Columns[i].FieldName = Sender.FieldName then
      begin
        k:=i;
        Break;
      end;
    if k = -1 then exit;
    for i:=0 to dgrdT820.Columns[k].PickList.Count - 1 do
      if Sender.AsString = dgrdT820.Columns[k].PickList[i] then
      begin
        Trov:=True;
        Break;
      end;
