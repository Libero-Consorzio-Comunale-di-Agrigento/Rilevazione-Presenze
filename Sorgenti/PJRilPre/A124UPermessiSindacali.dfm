inherited A124FPermessiSindacali: TA124FPermessiSindacali
  Left = 209
  Top = 229
  HelpContext = 124000
  BorderIcons = [biSystemMenu, biMinimize, biMaximize]
  Caption = '<A124> Permessi sindacali'
  ClientHeight = 542
  ClientWidth = 639
  ExplicitWidth = 655
  ExplicitHeight = 601
  PixelsPerInch = 96
  TextHeight = 13
  inherited StatusBar: TStatusBar
    Top = 524
    Width = 639
    ExplicitTop = 524
    ExplicitWidth = 639
  end
  inherited Panel1: TToolBar
    Top = 24
    Width = 639
    ExplicitTop = 24
    ExplicitWidth = 639
    object ToolButton2: TToolButton
      Left = 371
      Top = 0
      Width = 23
      Caption = 'ToolButton2'
      ImageIndex = 10
      Style = tbsSeparator
    end
    object btnVisualizzaTimbGius: TToolButton
      Left = 394
      Top = 0
      Action = actVisualizzaTimbGius
    end
    object ToolButton5: TToolButton
      Left = 417
      Top = 0
      Width = 10
      Caption = 'ToolButton5'
      ImageIndex = 11
      Style = tbsSeparator
    end
    object btnAnomalie: TBitBtn
      Left = 427
      Top = 0
      Width = 88
      Height = 22
      Anchors = [akTop, akRight]
      Caption = '&Anomalie'
      Glyph.Data = {
        
        0000}
      NumGlyphs = 2
      TabOrder = 0
      OnClick = btnAnomalieClick
    end
  end
  inline frmSelAnagrafe: TfrmSelAnagrafe [2]
    Left = 0
    Top = 0
    Width = 639
    Height = 24
    Align = alTop
    TabOrder = 2
    TabStop = True
    ExplicitWidth = 639
    ExplicitHeight = 24
    inherited pnlSelAnagrafe: TPanel
      Width = 639
      Height = 24
      ExplicitWidth = 639
      ExplicitHeight = 24
    end
    inherited pmnuDatiAnagrafici: TPopupMenu
      inherited R003Datianagrafici: TMenuItem
        OnClick = frmSelAnagrafeR003DatianagraficiClick
      end
    end
  end
  object Panel2: TPanel [3]
    Left = 0
    Top = 48
    Width = 639
    Height = 230
    Align = alTop
    TabOrder = 3
    object lblData: TLabel
      Left = 298
      Top = 6
      Width = 32
      Height = 13
      Caption = 'a Data'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object lblDalle: TLabel
      Left = 388
      Top = 6
      Width = 32
      Height = 13
      Caption = 'da Ore'
      FocusControl = dEdtDalle
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object lblAlle: TLabel
      Left = 430
      Top = 6
      Width = 26
      Height = 13
      Caption = 'a Ore'
      FocusControl = dEdtAlle
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object lblOre: TLabel
      Left = 474
      Top = 6
      Width = 35
      Height = 13
      Caption = 'Ore tot.'
      FocusControl = dEdtOre
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object lblSindacato: TLabel
      Left = 210
      Top = 84
      Width = 48
      Height = 13
      Caption = 'Sindacato'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object lblOrganismo: TLabel
      Left = 210
      Top = 45
      Width = 50
      Height = 13
      Caption = 'Organismo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object lblDataDa: TLabel
      Left = 210
      Top = 6
      Width = 38
      Height = 13
      Caption = 'da Data'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
    end
    object dlblDSindacato: TDBText
      Left = 304
      Top = 102
      Width = 208
      Height = 17
      DataField = 'D_SINDACATO'
      DataSource = DButton
    end
    object dlblDOrganismo: TDBText
      Left = 304
      Top = 63
      Width = 208
      Height = 17
      DataField = 'D_ORGANISMO'
      DataSource = DButton
    end
    object dEdtDalle: TDBEdit
      Left = 388
      Top = 20
      Width = 38
      Height = 21
      DataField = 'DALLE'
      DataSource = DButton
      TabOrder = 3
      OnChange = dEdtDalleChange
      OnExit = dEdtAlleExit
    end
    object dEdtAlle: TDBEdit
      Left = 430
      Top = 20
      Width = 38
      Height = 21
      DataField = 'ALLE'
      DataSource = DButton
      TabOrder = 4
      OnChange = dEdtDalleChange
      OnExit = dEdtAlleExit
    end
    object dEdtOre: TDBEdit
      Left = 474
      Top = 20
      Width = 38
      Height = 21
      Color = cl3DLight
      DataField = 'ORE'
      DataSource = DButton
      ReadOnly = True
      TabOrder = 5
      OnChange = dEdtDalleChange
      OnDblClick = dEdtOreDblClick
    end
    object grbProtocollo: TGroupBox
      Left = 5
      Top = 68
      Width = 197
      Height = 59
      Caption = 'Protocollo'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 8
      object lblNumProt: TLabel
        Left = 6
        Top = 16
        Width = 37
        Height = 13
        Caption = 'Numero'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object lblDataProt: TLabel
        Left = 100
        Top = 16
        Width = 23
        Height = 13
        Caption = 'Data'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object dEdtNumProt: TDBEdit
        Left = 6
        Top = 30
        Width = 90
        Height = 21
        DataField = 'NUMERO_PROT'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object dtpDataProt: TDateTimePicker
        Left = 100
        Top = 30
        Width = 90
        Height = 21
        Date = 37901.000000000000000000
        Time = 37901.000000000000000000
        TabOrder = 1
        OnChange = dtpDataProtChange
      end
    end
    object grbModifica: TGroupBox
      Left = 5
      Top = 127
      Width = 197
      Height = 98
      Caption = 'Protocollo di Modifica'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 9
      object lblNumProtMod: TLabel
        Left = 6
        Top = 18
        Width = 37
        Height = 13
        Caption = 'Numero'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object lblDataProtMod: TLabel
        Left = 100
        Top = 18
        Width = 23
        Height = 13
        Caption = 'Data'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object Label10: TLabel
        Left = 8
        Top = 67
        Width = 28
        Height = 13
        Caption = 'Stato:'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlue
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        PopupMenu = PopupMenu2
      end
      object lblStato: TLabel
        Left = 40
        Top = 67
        Width = 9
        Height = 13
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clNavy
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsItalic]
        ParentFont = False
        PopupMenu = PopupMenu2
      end
      object dtpDataProtMod: TDateTimePicker
        Left = 100
        Top = 32
        Width = 90
        Height = 21
        Date = 37901.000000000000000000
        Time = 37901.000000000000000000
        TabOrder = 1
        OnChange = dtpDataProtModChange
      end
      object dEdtNumProtMod: TDBEdit
        Left = 6
        Top = 32
        Width = 90
        Height = 21
        DataField = 'PROT_MODIFICA'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clBlack
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
      end
      object btnElimina: TBitBtn
        Left = 121
        Top = 64
        Width = 67
        Height = 20
        Hint = 'Cancellazione definitiva del permesso'
        Caption = 'ELIMINA'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        OnClick = btnEliminaClick
      end
    end
    object dtpData: TDateTimePicker
      Left = 298
      Top = 20
      Width = 84
      Height = 21
      Date = 37901.000000000000000000
      Time = 37901.000000000000000000
      TabOrder = 2
      OnChange = dtpDataChange
    end
    object dCmbSindacato: TDBLookupComboBox
      Left = 210
      Top = 98
      Width = 90
      Height = 21
      DataField = 'COD_SINDACATO'
      DataSource = DButton
      DropDownWidth = 300
      KeyField = 'CODICE'
      ListField = 'CODICE;DESCRIZIONE'
      PopupMenu = PopupMenu1
      TabOrder = 7
      OnKeyDown = dCmbSindacatoKeyDown
    end
    object dCmbOrganismo: TDBLookupComboBox
      Left = 210
      Top = 59
      Width = 90
      Height = 21
      DataField = 'COD_ORGANISMO'
      DataSource = DButton
      DropDownWidth = 300
      KeyField = 'CODICE'
      ListField = 'CODICE;DESCRIZIONE'
      PopupMenu = PopupMenu1
      TabOrder = 6
      OnCloseUp = dCmbOrganismoCloseUp
      OnKeyDown = dCmbSindacatoKeyDown
      OnKeyUp = dCmbOrganismoKeyUp
    end
    object GroupBox1: TGroupBox
      Left = 204
      Top = 127
      Width = 423
      Height = 98
      TabOrder = 10
      object btnCompetenze: TSpeedButton
        Left = 314
        Top = 9
        Width = 97
        Height = 21
        AllowAllUp = True
        GroupIndex = 1
        Caption = 'Competenze'
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = [fsBold]
        ParentFont = False
        OnClick = btnCompetenzeClick
      end
      object lblCompetenze: TLabel
        Left = 16
        Top = 13
        Width = 3
        Height = 13
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clMaroon
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
      end
      object dGrdCompetenze: TDBGrid
        Left = 2
        Top = 33
        Width = 419
        Height = 63
        Align = alBottom
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentFont = False
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clMaroon
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'AREA_SINDACALE'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'TIPO'
            Width = 26
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DECORRENZA'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'SCADENZA'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'COMPETENZA'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'FRUITO'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'RESIDUO'
            Visible = True
          end>
      end
    end
    object drdgTipoPermesso: TDBRadioGroup
      Left = 5
      Top = 2
      Width = 197
      Height = 60
      Caption = 'Tipo'
      Columns = 2
      DataField = 'TIPO_PERMESSO'
      DataSource = DButton
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      Items.Strings = (
        'Giornata'
        'Mezza giornata'
        'Numero ore'
        'Da ore a ore')
      ParentFont = False
      TabOrder = 0
      Values.Strings = (
        'I'
        'M'
        'N'
        'D')
      OnClick = drdgTipoPermessoClick
    end
    object dtpDataDa: TDateTimePicker
      Left = 210
      Top = 20
      Width = 84
      Height = 21
      Date = 37901.000000000000000000
      Time = 37901.000000000000000000
      TabOrder = 1
      OnChange = dtpDataDaChange
    end
    object gpbGEDAP: TGroupBox
      Left = 518
      Top = 2
      Width = 109
      Height = 125
      Caption = 'GEDAP'
      Font.Charset = DEFAULT_CHARSET
      Font.Color = clBlue
      Font.Height = -11
      Font.Name = 'MS Sans Serif'
      Font.Style = []
      ParentFont = False
      TabOrder = 11
      object dlblSiglaGedap: TDBText
        Left = 11
        Top = 17
        Width = 86
        Height = 17
        Alignment = taCenter
        Color = clBtnFace
        DataField = 'SIGLA_GEDAP'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentColor = False
        ParentFont = False
      end
      object dchkRetribuito: TDBCheckBox
        Left = 11
        Top = 33
        Width = 86
        Height = 17
        Alignment = taLeftJustify
        BiDiMode = bdLeftToRight
        Caption = 'Retribuito'
        Color = clBtnFace
        DataField = 'RETRIBUITO'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clHighlight
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBiDiMode = False
        ParentColor = False
        ParentFont = False
        ReadOnly = True
        TabOrder = 0
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
      object dchkStatutario: TDBCheckBox
        Left = 11
        Top = 50
        Width = 86
        Height = 17
        Alignment = taLeftJustify
        BiDiMode = bdLeftToRight
        Caption = 'Statutario'
        Color = clBtnFace
        DataField = 'STATUTARIO'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clHighlight
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBiDiMode = False
        ParentColor = False
        ParentFont = False
        ReadOnly = True
        TabOrder = 1
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
      object dchkAbbatteCompetenzeOrg: TDBCheckBox
        Left = 11
        Top = 67
        Width = 86
        Height = 17
        Alignment = taLeftJustify
        BiDiMode = bdLeftToRight
        Caption = 'Abbatte comp.'
        Color = clBtnFace
        DataField = 'ABBATTE_COMPETENZE_ORG'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clHighlight
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBiDiMode = False
        ParentColor = False
        ParentFont = False
        ReadOnly = True
        TabOrder = 2
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
      object dchkRsu: TDBCheckBox
        Left = 11
        Top = 84
        Width = 86
        Height = 17
        Alignment = taLeftJustify
        BiDiMode = bdLeftToRight
        Caption = 'RSU'
        Color = clBtnFace
        DataField = 'RSU'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clHighlight
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBiDiMode = False
        ParentColor = False
        ParentFont = False
        ReadOnly = True
        TabOrder = 3
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
      object dchkRaggruppamento: TDBCheckBox
        Left = 11
        Top = 101
        Width = 86
        Height = 17
        Alignment = taLeftJustify
        BiDiMode = bdLeftToRight
        Caption = 'Raggrupp.'
        Color = clBtnFace
        DataField = 'RAGGRUPPAMENTO'
        DataSource = DButton
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clHighlight
        Font.Height = -11
        Font.Name = 'MS Sans Serif'
        Font.Style = []
        ParentBiDiMode = False
        ParentColor = False
        ParentFont = False
        ReadOnly = True
        TabOrder = 4
        ValueChecked = 'S'
        ValueUnchecked = 'N'
      end
    end
  end
  object grdPermessi: TDBGrid [4]
    Left = 0
    Top = 278
    Width = 639
    Height = 232
    Align = alClient
    DataSource = DButton
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgConfirmDelete]
    ParentFont = False
    PopupMenu = PopupMenu3
    ReadOnly = True
    TabOrder = 4
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clBlue
    TitleFont.Height = -11
    TitleFont.Name = 'MS Sans Serif'
    TitleFont.Style = []
  end
  object rgpModalita: TRadioGroup [5]
    Left = 522
    Top = -5
    Width = 116
    Height = 55
    Anchors = [akTop, akRight]
    Color = clBtnFace
    Font.Charset = DEFAULT_CHARSET
    Font.Color = clBlue
    Font.Height = -11
    Font.Name = 'MS Sans Serif'
    Font.Style = []
    ItemIndex = 0
    Items.Strings = (
      'Modalit'#224' singola'
      'Modalit'#224' collettiva')
    ParentColor = False
    ParentFont = False
    TabOrder = 5
    OnClick = rgpModalitaClick
  end
  object ProgressBar1: TProgressBar [6]
    Left = 0
    Top = 510
    Width = 639
    Height = 14
    Align = alBottom
    TabOrder = 6
  end
  inherited MainMenu1: TMainMenu [7]
    Left = 376
    Top = 65523
    object Azioni1: TMenuItem
      Caption = 'Azioni'
      object InviopermessiGEDAP1: TMenuItem
        Action = actInvioPermessiGEDAP
      end
      object Inviopermessiaspettativefunzionipubbliche1: TMenuItem
        Action = actInvioPermFP
      end
    end
  end
  inherited DButton: TDataSource [8]
    Left = 432
    Top = 65523
  end
  inherited PrinterSetupDialog1: TPrinterSetupDialog [9]
    Left = 404
    Top = 65523
  end
  inherited ImageList1: TImageList [10]
    Left = 320
    Top = 65523
    Bitmap = {
      
      000000000000}
  end
  inherited ActionList1: TActionList
    Left = 348
    Top = 65523
    object actVisualizzaTimbGius: TAction
      Caption = 'Visualizza timbrature/giustificativi'
      Hint = 'Visualizza timbrature/giustificativi'
      ImageIndex = 24
      OnExecute = actVisualizzaTimbGiusExecute
    end
    object actInvioPermessiGEDAP: TAction
      Caption = 'Invio GEDAP permessi sindacali'
      OnExecute = actInvioPermessiGEDAPExecute
    end
    object actInvioPermFP: TAction
      Caption = 'Invio GEDAP permessi funzioni pubbliche'
      OnExecute = actInvioPermFPExecute
    end
    object actInvioAspFP: TAction
      Caption = 'Invio GEDAP aspettative funzioni pubbliche'
      Enabled = False
    end
  end
  object PopupMenu1: TPopupMenu
    OnPopup = PopupMenu1Popup
    Left = 460
    Top = 65523
    object Nuovoelemento1: TMenuItem
      Caption = 'Accedi'
      OnClick = Nuovoelemento1Click
    end
  end
  object PopupMenu2: TPopupMenu
    OnPopup = PopupMenu2Popup
    Left = 84
    Top = 234
    object RitornaallostatoMODIFICATO1: TMenuItem
      Caption = 'Ritorna allo stato '#39'MODIFICATO'#39
      OnClick = RitornaallostatoMODIFICATO1Click
    end
  end
  object PopupMenu3: TPopupMenu
    OnPopup = PopupMenu3Popup
    Left = 265
    Top = 295
    object Copiapermesso1: TMenuItem
      Caption = 'Copia permesso'
      OnClick = Copiapermesso1Click
    end
    object Permessi1: TMenuItem
      Caption = 'Permessi non '#39'CANCELLATI'#39
      OnClick = Permessi1Click
    end
    object Permessi2: TMenuItem
      Caption = 'Permessi anno corrente'
      OnClick = Permessi2Click
    end
  end
end
