...
uses
  ...
  A000UInterfaccia, A000UCostanti, A000USessione, A027UCostanti, A000UMessaggi,
  C180FunzioniGenerali, C700USelezioneAnagrafe, R400UCartellinoDtM, R600,Printers;
...
type
  ...
  public
    ...
    BandLogoHeightIni,QRLMeseHeightIni:Integer;
        ...
        for NL:=0 to ControlCount - 1 do
          if Controls[NL].Tag = StrToInt(Posiz) then
            with TQRMemo(Controls[NL]) do
              ...
              if R180In(Tag,[903,904,905,906,907,909,910,911,912,913,914,915,916,917,918,919]) then
...
procedure TA027StampaTimb.QRepBeforePrint(Sender: TCustomQuickRep; var PrintReport: Boolean);
var ParCartellino:String;
...
begin
  ...
  (*if R400FCartellinoDtM.PrimoPassaggioStampa then
    R400FCartellinoDtM.PrimoPassaggioStampa:=False
  else*)
  if not R400FCartellinoDtM.PrimoPassaggioStampa then //Aggiornamento di PrimoPassaggioStampa dopo ImpostaStampa
  begin
  ...
  if ParametrizzazioneCambiata or R400FCartellinoDtM.PrimoPassaggioStampa then
  begin
    ImpostaStampa;
    (*EndPage per resettare le bande*)
    (*
...
begin
  ...
  begin
    ...
    QRSDetBeforePrint(qrbIntestazionePagina,bDummy);
  end;
end;
...
//        if R400FCartellinoDtM.DipInSer1 = 'no' then  LORENA 28/07/2004
        ...
        begin
          ...
          if R180In(QrLbl.Tag,[C_CAUF1,C_CAUF2,C_CAUF3,C_CAUF4,C_CAUF5,C_CAUF6]) then
            QRLbl.Caption:=QRLbl.Caption + '      ';
...
procedure TA027StampaTimb.ImpostaLogo;
var LogoLarghezza:Integer;
begin
  try
    ...
    begin
      ...
      with R400FCartellinoDtM.selT004Img do
      begin
        Azienda:=Self.DataSet.FieldByName('T430AZIENDA_BASE').AsString;
        Data:=EncodeDate(AStampa, MStampa, 1);
        if EsisteImmagine and (LarghezzaCartellino <> 0) then
        begin
          qimgLogo.Picture.BitMap.Assign(Immagine);
          if LarghezzaCartellino = -1 then
            LogoLarghezza:=R400FCartellinoDtM.Q950Int.FieldByName('LOGO_LARGHEZZA').AsInteger
          else
            LogoLarghezza:=LarghezzaCedolino;
        end
        else
        begin
          qimgLogo.Picture.BitMap.Assign(nil);
          qimgLogo.Enabled:=False;
        end;
      end;
      qimgLogo.AutoSize:=False;
      qimgLogo.Stretch:=True;
      qimgLogo.Height:=Trunc(qimgLogo.Height * (LogoLarghezza / qimgLogo.Width));
      qimgLogo.Width:=LogoLarghezza;
    end;
  except
    qimgLogo.Enabled:=False;
  end;
...
begin
  ...
  //Al primo passaggio imposta l'orientamento pagina; ai successivi controlla che l'orientamento non cambi (non fattibile)
  if R400FCartellinoDtM.PrimoPassaggioStampa then
  begin
    if R400FCartellinoDtM.Q950Int.FieldByName('ORIENTAMENTO').AsString = 'V' then
      Self.Page.Orientation:=poPortrait
    else if R400FCartellinoDtM.Q950Int.FieldByName('ORIENTAMENTO').AsString = 'O' then
      Self.Page.Orientation:=poLandScape;
  end
  else
  begin
    if ((R400FCartellinoDtM.Q950Int.FieldByName('ORIENTAMENTO').AsString = 'V') and (Self.Page.Orientation <> poPortrait))
    or ((R400FCartellinoDtM.Q950Int.FieldByName('ORIENTAMENTO').AsString = 'O') and (Self.Page.Orientation <> poLandScape)) then
      RegistraMsg.InserisciMessaggio('A','Non è stato possibile applicare l''orientamento della pagina (' + R400FCartellinoDtM.Q950Int.FieldByName('ORIENTAMENTO').AsString + ') previsto dalla parametrizzazione','',Self.DataSet.FieldByName('PROGRESSIVO').AsInteger);
  end;
end;

procedure TA027StampaTimb.QRSDetBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
var S:String;
    LarghezzaIntestazione:Integer;
begin
  QRLNow.Caption:=FormatDateTime(R400FCartellinoDtM.Q950Int.FieldByName('DATA_STAMPA').AsString,Now);
  QRLNow.Caption:=StringReplace(QRLNow.Caption,':OPERATORE',Parametri.Operatore,[rfIgnoreCase]);
  (*
  ...
  *)
  ...
  LAzienda.Caption:=C700RagioneSociale(Self.DataSet.FieldByName('PROGRESSIVO').AsInteger);//Parametri.RagioneSociale;
  QRLMese.Top:=LAzienda.Top + LAzienda.Height + 1;
  LarghezzaIntestazione:=Sender.Width;
  ImpostaLogo;

  S:=UpperCase(R180NomeMese(MStampa)) + ' ' + IntToStr(AStampa);
  S:=S + IfThen(qimgLogo.Enabled,IfThen(AggSchedaRiep <> '',CRLF),' ') + AggSchedaRiep;
  QRLMese.Caption:=A000MSG_R400_MSG_RILEVAZIONE_MESE + ' ' + Trim(S);
  QRLMese.Height:=QRLMeseHeightIni * IfThen(Pos(CRLF,S) > 0,2,1);
  //Impostazioni dell'altezza della banda QRSDet
  //Valore maggiore tra:
  //1) altezza originale
  //2) spazio del logo se previsto
  //3) spazio dell'etichetta del titolo, eventualmente spezzato su due righe a causa della presenza del logo in caso di aggiornamento della scheda riepilogativa
  TQRBand(qimgLogo.Parent).Height:=max(max(BandLogoHeightIni,IfThen(qimgLogo.Enabled,qimgLogo.Top + qimgLogo.Height,0)),QRLMese.Top + QRLMese.Height);

  if qimgLogo.Enabled then
    dec(LarghezzaIntestazione,qimgLogo.Width * 2);
...
begin
  ...
  begin
    ...
    begin
      ...
      if T in [C_GI1..C_GI2,C_TM1,C_NOTE,C_RICHIESTE] then
      begin
...
begin
  ...
  begin
    ...
    begin
      Da:=z + (x-1) * R400FCartellinoDtM.LungTimb;
      RETimb.SelStart:=y + (x-1) * R400FCartellinoDtM.LungTimb;//Da;
    ...
    for i:=0 to Riepilogo2.ControlCount - 1 do
      if Riepilogo2.Controls[i].Enabled then
        case Riepilogo2.Controls[i].Tag of
          ...
          //Fruito richiesto
          918:with TQRMemo(Riepilogo2.Controls[i]) do
                if not R400FCartellinoDtM.trpassemes[NA].tCompetenze then Lines.Add('')
                else Lines.Add(R400FCartellinoDtM.R600DtM1.GetIterRichiesto);
          //Fruito autorizzato
          919:with TQRMemo(Riepilogo2.Controls[i]) do
                if not R400FCartellinoDtM.trpassemes[NA].tCompetenze then Lines.Add('')
                else Lines.Add(R400FCartellinoDtM.R600DtM1.GetIterAutorizzato);
          //Descrizione assenza
  ...
  begin
    ...
    if R180CarattereDef(VarToStr(R400FCartellinoDtM.Q265Riep.Lookup('Codice',G.Causale,'Cumulo_Familiari')),1,'N') in ['S','D'] then
      ...
      begin
        ...
        SetVariable('Data1',R400FCartellinoDtM.DataDa(*EncodeDate(1900,1,1)*));
...
begin
  ...
  for i:=qrbIntestazionePaginaChild.ControlCount - 1 downto 0 do
  begin
    Temp:=qrbIntestazionePaginaChild.Controls[i];
    qrbIntestazionePaginaChild.RemoveControl(Temp);
    Temp.Free;
  end;
  ...
  for i:=0 to TotaliDett.ControlCount - 1 do
    with TQRLabel(TotaliDett.Controls[i]) do
      case Tag of
        ...
        //Debito da orario
        C_DGO2:Caption:=IfThen(R400FCartellinoDtM.RiepDebitoGO <> 0,R180MinutiOre(R400FCartellinoDtM.RiepDebitoGO));
        //Debito medio
        C_DGM2:Caption:=IfThen(R400FCartellinoDtM.RiepDebitoM <> 0,R180MinutiOre(R400FCartellinoDtM.RiepDebitoM));
        //Scostamento debito
        C_SD2:Caption:=IfThen(R400FCartellinoDtM.RiepScostDebito <> 0,R180MinutiOre(R400FCartellinoDtM.RiepScostDebito));
        //Eccedenze
        C_EC2:begin
              Caption:='';
              for j:=1 to R400FCartellinoDtM.R450DtM1.NFasceMese do
        ...
        C_PRNE2:if R400FCartellinoDtM.RiepProlENonCaus = 0 then
                 Caption:=''
               else
                 Caption:=R180MinutiOre(R400FCartellinoDtM.RiepProlENonCaus);
        //Prolungamento U non causalizzato
        C_PRNU2:if R400FCartellinoDtM.RiepProlUNonCaus = 0 then
                 Caption:=''
               else
                 Caption:=R180MinutiOre(R400FCartellinoDtM.RiepProlUNonCaus);
        ...
        C_BP2:Caption:=IfThen(R400FCartellinoDtM.RiepBuoniPasto <> 0,R400FCartellinoDtM.RiepBuoniPasto.ToString);
        //Ore causalizzate
        C_CAU2:Caption:=IfThen(R400FCartellinoDtM.RiepMinLavCau <> 0,R180MinutiOre(R400FCartellinoDtM.RiepMinLavCau));
        //Ore causalizzate 2
        C_CAU4:Caption:=IfThen(R400FCartellinoDtM.RiepMinLavCau2 <> 0,R180MinutiOre(R400FCartellinoDtM.RiepMinLavCau2));
        //Ore causalizzate 3
        C_CAU6:Caption:=IfThen(R400FCartellinoDtM.RiepMinLavCau3 <> 0,R180MinutiOre(R400FCartellinoDtM.RiepMinLavCau3));
        //Ecc.gg disponibile per iter web
        C_ECCRICH2:Caption:=IfThen(R400FCartellinoDtM.RiepEccedIterDisp <> 0,R180MinutiOre(R400FCartellinoDtM.RiepEccedIterDisp));
        //Ind.presenza
        ...
        //Ind.festiva
        C_CC2:if R400FCartellinoDtM.RiepCoperturaCarenza = 0 then
                Caption:=''
              else
                Caption:=R180MinutiOre(R400FCartellinoDtM.RiepCoperturaCarenza);
      end;
end;
...
begin
  ...
  if not R400FCartellinoDtM.ExistsSchedeAggiornate(Self.DataSet.FieldByName('PROGRESSIVO').AsInteger,EncodeDate(AStampa,MStampa,1)) then
