...
uses
  ...
  DateUtils, ToolbarFiglio, C600USelAnagrafe, System.Actions, StrUtils;
...
type
  TA121FOrganizzSindacali = class(TR004FGestStorico)
    ...
    DBEdit1: TDBEdit;
    Panel2: TPanel;
    lblDescrizioneOrg: TLabel;
    lblCodiceOrg: TLabel;
    dedtDescrizioneOrg: TDBEdit;
    dedtCodiceOrg: TDBEdit;
    GroupBox1: TGroupBox;
    ...
    dchkFruizionePermessi: TDBCheckBox;
    dchkAbbatteCompetenzeOrg: TDBCheckBox;
    dlblSiglaGedap: TDBText;
    lblSiglaGedap: TLabel;
    dchkPartecipazioneRichiesta: TDBCheckBox;
    ...
    procedure dchkFruizionePermessiClick(Sender: TObject);
  private
    { Private declarations }
  public
    { Public declarations }
    procedure AbilitaComponentiOrg;
  end;

var
  A121FOrganizzSindacali: TA121FOrganizzSindacali;

procedure OpenA121OrganizzSindacali(Tipo,ValoreChiave:String);
...
begin
  ...
  begin
    ...
    A121FOrganizzSindacali.PageControl2.ActivePage:=A121FOrganizzSindacali.TabOrganizzazioni;
  end
  else if Tipo = 'O' then
  begin
    A121FOrganizzSindacaliDtM.A121MW.selT245.SearchRecord('CODICE',ValoreChiave,[srFromBeginning]);
    A121FOrganizzSindacali.PageControl2.ActivePage:=A121FOrganizzSindacali.TabOrganismi;
  end;
  try
...
begin
  inherited;
  //frmToolbarFiglio1
  dedtCodiceOrg.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dedtDescrizioneOrg.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dchkFruizionePermessi.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dchkPartecipazioneRichiesta.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dchkRetribuito.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dchkStatutario.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dchkAbbatteCompetenzeOrg.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dlblSiglaGedap.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dcmbCausale.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dlblDCausale.DataSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT245;
  dcmbCausale.ListSource:=A121FOrganizzSindacaliDtM.A121MW.dsrT265;
...
procedure TA121FOrganizzSindacali.dchkFruizionePermessiClick(Sender: TObject);
begin
  with A121FOrganizzSindacaliDtM.A121MW.selT245 do
    if State in [dsEdit,dsInsert] then
    begin
      FieldByName('FRUIZIONE_PERMESSI').AsString:=IfThen(dchkFruizionePermessi.Checked,'S','N');
      FieldByName('PARTECIPAZIONE_RICHIESTA').AsString:=IfThen(dchkPartecipazioneRichiesta.Checked,'S','N');
      FieldByName('RETRIBUITO').AsString:=IfThen(dchkRetribuito.Checked,'S','N');
      FieldByName('STATUTARIO').AsString:=IfThen(dchkStatutario.Checked,'S','N');
    end;
  AbilitaComponentiOrg;
end;

procedure TA121FOrganizzSindacali.AbilitaComponentiOrg;
begin
  lblCausale.Enabled:=dchkFruizionePermessi.Checked;
  dcmbCausale.Enabled:=dchkFruizionePermessi.Checked;
  with A121FOrganizzSindacaliDtM.A121MW do
    if selT245.State in [dsEdit,dsInsert] then
      if not dchkFruizionePermessi.Checked then
        AssegnaValoriDefaultOrg;
  dchkPartecipazioneRichiesta.Enabled:=dchkFruizionePermessi.Checked;
  dchkRetribuito.Enabled:=dchkFruizionePermessi.Checked and not dchkStatutario.Checked;
  dchkStatutario.Enabled:=dchkFruizionePermessi.Checked and dchkRetribuito.Checked;
  dchkAbbatteCompetenzeOrg.Enabled:=dchkFruizionePermessi.Checked;
  lblSiglaGedap.Enabled:=dchkFruizionePermessi.Checked;
  dlblSiglaGedap.Enabled:=dchkFruizionePermessi.Checked;
end;

procedure TA121FOrganizzSindacali.dcmbCausaleKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
begin
  inherited;
