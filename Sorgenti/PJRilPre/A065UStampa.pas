...
var DataMin,DataMax,DataCorr:TDateTime;
  S:String;
begin
  with A065FStampaBudget.A065MW do
...
begin
  Mese:=A065FStampaBudget.A065MW.cdsStampa.FieldByName('Mese').AsInteger;
  lblMese.Caption:=R180NomeMese(A065FStampaBudget.A065MW.cdsStampa.FieldByName('MESE').AsInteger);
  if A065FStampaBudget.dcmbTipo.KeyValue = '#ECC#' then
  begin
    // recupero i totali di gruppo
    if R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE').AsString) <> 0 then
      TotOreGruppo:=R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE').AsString) + TotOreFruitoGruppo;
    if R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString) <> 0 then
      TotOreFruitoGruppo:=R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString);
  end
  else
  begin
    // incremento i totali generali
    inc(TotOreGenerale,R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE').AsString));
    inc(TotOreFruitoGenerale,R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString));
    inc(TotOreResiduoGenerale,R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_RESIDUO').AsString));
    // incremento i totali di gruppo
    inc(TotOreGruppo,R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE').AsString));
    inc(TotOreFruitoGruppo,R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString));
    inc(TotOreResiduoGruppo,R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_RESIDUO').AsString));
  end;
  ...
  TotOreF50:=0;
  TotSoldiMese:=0;
  if R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_RESIDUO').AsString) <= 0 then
    dlblOreResiduoMese.Font.Style:=[fsBold]
  else
    dlblOreResiduoMese.Font.Style:=[];
  if A065FStampaBudget.chkDettaglioDipendenti.Checked
  and (R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString) <> 0) then
    with A065FStampaBudget.A065MW do
    begin
      selbV430.Close;
...
procedure TA065FStampa.bndTestDipendentiBeforePrint(Sender: TQRCustomBand;
  var PrintBand: Boolean);
begin
  PrintBand:=R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString) <> 0;
end;
...
begin
  if R180OreMinutiExt(A065FStampaBudget.A065MW.cdsStampa.FieldByName('ORE_FRUITO').AsString) <> 0 then
    ...
    begin
      ...
      begin
        Q730.Close;
        Q730.SetVariable('LIVELLO',selbV430.FieldByName('T430' + Parametri.CampiRiferimento.C2_Livello).AsString);
        Q730.SetVariable('DATA',R180FineMese(EncodeDate(Anno,Mese,1)));
        Q730.Open;
      end;
      lblF15.Caption:='';
      lblF30.Caption:='';
      ...
      begin
        ...
        selT071.SetVariable('DATA',EncodeDate(Anno,A065FStampaBudget.A065MW.cdsStampa.FieldByName('Mese').AsInteger,1));
        selT071.Open;
        ...
        begin
          ...
          Fruito:=R180OreMinutiExt(IfThen(A065FStampaBudget.dcmbTipo.KeyValue = '#LIQ#',selT071.FieldByName('LIQUIDNELMESE').AsString,selT071.FieldByName('BANCA_ORE').AsString));
          if A065FStampaBudget.chkCostoInMoneta.Checked and Q730.Locate('MAGGIORAZIONE',StrToFloat(MOld),[]) then
      ...
      begin
        ...
        selT074.SetVariable('DATA',EncodeDate(Anno,A065FStampaBudget.A065MW.cdsStampa.FieldByName('Mese').AsInteger,1));
      ...
      and (A065FStampaBudget.A065MW.selbV430.FieldByName('LIQ_FUORI_BUDGET').AsInteger <> 0) then
        lblOreFuoriBudget.Caption:=Format('(%s)',[R180MinutiOre(A065FStampaBudget.A065MW.selbV430.FieldByName('LIQ_FUORI_BUDGET').AsInteger)])
      else
        lblOreFuoriBudget.Caption:='';
      lblSoldi.Caption:='€' + Format('%10.2n',[Soldi]);
      TotSoldiMese:=TotSoldiMese + Soldi;
      TotSoldiGruppo:=TotSoldiGruppo + Soldi;
      TotSoldiGenerale:=TotSoldiGenerale + Soldi;
    end;
  PrintBand:=Tot > 0;
end;
...
procedure TA065FStampa.FormCreate(Sender: TObject);
begin
  inherited;
  QRep.DataSet:=A065FStampaBudget.A065MW.cdsStampa;
  dlblCodGruppo.DataSet:=QRep.DataSet;
  dlblOreMese.DataSet:=QRep.DataSet;
  dlblOreFruitoMese.DataSet:=QRep.DataSet;
  dlblOreResiduoMese.DataSet:=QRep.DataSet;
  bndDettDipendenti.DataSet:=A065FStampaBudget.A065MW.selbV430;
end;
