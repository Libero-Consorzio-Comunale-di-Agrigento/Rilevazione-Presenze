...
begin
  ...
  QRLGruppo.Caption:='';
  for i:=Low(A047FDialogStampa.MyCampiRaggr) to High(A047FDialogStampa.MyCampiRaggr) do
  begin
    if Trim(QRLGruppo.Caption) <> '' then
      QRLGruppo.Caption:=QRLGruppo.Caption + CRLF;//', ';
    QRLGruppo.Caption:=QRLGruppo.Caption + A047FDialogStampa.MyCampiRaggr[i].NomeLogico + ': ' +  A047FTimbMensaDtM1.TabellaStampa.FieldByName('GRUPPO' + IntToStr(i)).AsString
  end;
end;
