...
uses
  ...
  Buttons, DBCtrls, StdCtrls, Spin ,C001StampaLib, C180FunzioniGenerali,A000UCostanti, A000UMessaggi, A000USessione,A000UInterfaccia,
  Grids, DBGrids, ExtCtrls,DB,ComCtrls, checklst, Menus, 
  A003UDataLavoroBis, C004UParamForm, QueryStorico, Oracle, SelAnagrafe,
  C700USelezioneAnagrafe, C005UDatiAnagrafici, Variants, InputPeriodo;
...
begin
  ...
  A081FStampa.RepR.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
...
procedure TA081FTimbCaus.ControllaPeriodo;
begin
  if DataI <= 0 then
  begin
...
begin
  ...
  A081FStampa.DaData:=DataI;
  A081FStampa.AData:=DataF;
...
{ DataF }
function TA081FTimbCaus._GetDataF: TDateTime;
begin
  Result:=frmInputPeriodo.DataFine;
end;
procedure TA081FTimbCaus._PutDataF(const Value: TDateTime);
begin
  frmInputPeriodo.DataFine:=Value;
end;
{ ----DataF }
{ DataI }
function TA081FTimbCaus._GetDataI: TDateTime;
begin
  Result:=frmInputPeriodo.DataInizio;
end;
procedure TA081FTimbCaus._PutDataI(const Value: TDateTime);
begin
