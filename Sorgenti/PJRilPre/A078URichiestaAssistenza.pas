...
uses
  ...
  IdExplicitTLSClientServerBase, System.Actions, System.ImageList, System.Hash,
      ...
      begin
        ...
        begin
          if ((SelTab.Fields[i].FieldName = 'DATADECORRENZA') or (SelTab.Fields[i].FieldName = 'DATAFINE')) and (Tabella='T430_storico') then
            FreeData.Add(Format('%-30s: %s',[SelTab.Fields[i].FieldName, SelTab.Fields[i].AsString]));
          //CONTROLLO PER NON IMPORTARE DATI INUTILI
          if (Tabella = 'T430_STORICO') and R180In(SelTab.Fields[i].FieldName,['ABCAUSALE2','ABCAUSALE3','ABCAUSALE4','ABCAUSALE5','ABCAUSALE6','ABPRESENZA2','ABPRESENZA3','ABPRESENZA4','ABPRESENZA5','ABPRESENZA6','ABPRESENZA7','ABPRESENZA8']) then
            Esclusione:=True;
          if (Tabella = 'T030_ANAGRAFICO') and R180In(SelTab.Fields[i].FieldName,['COGNOME','NOME','CODFISCALE']) then
            Esclusione:=True;
          if not Esclusione then
          begin
            virg:='';
            if (i > 0) then
              virg:=', ';
