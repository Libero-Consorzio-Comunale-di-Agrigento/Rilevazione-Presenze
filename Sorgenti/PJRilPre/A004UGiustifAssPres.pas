...
uses
  ...
  StrUtils, A087UImpAttestatiMalMW, InputPeriodo;
...
procedure OpenA004GiustifAssPres(Prog:LongInt; Data,Tipo,Ore1,Ore2,Causale:String; DataNas:TDateTime; Cartel:Boolean);
...
begin
  ...
  try
    ...
    if Cartel then
    begin
      if TryStrToDateTime(Data, tmp) then
        DataI:=tmp
      else
        DataI:=0;
      DataF:=DataI;
    ...
    begin
      ...
      DataF:=DataI;  // commessa MAN/08 SVILUPPO#56 - riesame del 06.09.2013
    end;
    ShowModal;
  finally
...
begin
  ...
  BInserisci.Enabled:=not (A004MW.A087ElencoCausali as TA087InfoCertificati).EsisteCausale(VarToStr(ECausale.KeyValue)) or
                     (not Parametri.ModuloInstallato['TORINO_CSI_PRV']);
  BCancella.Enabled:=BCancella.Enabled and (not SolaLettura);
  BInserisci.Enabled:=BInserisci.Enabled and (not SolaLettura);
end;
...
begin
  ...
  A004MW.Var_DaData:=frmInputPeriodo.edtInizio.Text;
...
begin
  ...
  begin
    ...
    Var_DaData:=frmInputPeriodo.edtInizio.Text;
...
var
  ...
  begin
    ...
    A004MW.Var_DaData:=frmInputPeriodo.edtInizio.Text;
    A004MW.Var_AData:=frmInputPeriodo.edtFine.Text;
    A004MW.Var_NumGG:=SpinEdit1.Value;
    A004MW.Var_Causale:=ECausale.Text;
    A004MW.Var_TipoCaus:=RGCausali.ItemIndex;
    if A004MW.GestioneSingolaDM then
      A004MW.Var_Familiari:=StringReplace(VarToStr(dCmbFamiliari.Text),Parametri.TimeSeparatorDef,{$IFNDEF VER210}FormatSettings.{$ENDIF}TimeSeparator,[rfReplaceAll])
    else
...
begin
  A004MW.DataInizio:=DataI;
  if DataF > 0 then
    A004MW.DataFine:=DataF
  else
...
begin
  frmInputPeriodo.edtFine.OnChange:=nil;
  DataF:=0;
  frmInputPeriodo.edtFine.OnChange:=frmInputPeriodoedtFineChange;
end;
...
procedure TA004FGiustifAssPres.frmInputPeriodoedtFineChange(Sender: TObject);
begin
  inherited;
...
procedure TA004FGiustifAssPres.frmInputPeriodoedtInizioChange(Sender: TObject);
begin
  inherited;
  frmInputPeriodo.edtDataChange(Sender);
...
begin
  T047DataInizio:=DataI;
  if T047DataInizio > 0 then
...
{ DataF }
function TA004FGiustifAssPres._GetDataF: TDateTime;
begin
  Result:=frmInputPeriodo.DataFine;
end;

procedure TA004FGiustifAssPres._PutDataF(const Value: TDateTime);
begin
...
{ DataI }
function TA004FGiustifAssPres._GetDataI: TDateTime;
begin
  Result:=frmInputPeriodo.DataInizio;
end;

procedure TA004FGiustifAssPres._PutDataI(const Value: TDateTime);
begin
