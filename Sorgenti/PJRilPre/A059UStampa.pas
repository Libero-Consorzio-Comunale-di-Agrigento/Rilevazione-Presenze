...
procedure TA059FStampa.DetailBand1BeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  ...
  begin
    ...
    selT606.SetVariable('COD_SQUADRA',selT603.FieldByName('Codice').AsString);
    selT606.SetVariable('COD_ORARIO','*'); //Tutti gli orari
    selT606.SetVariable('COD_CONDIZIONE','00001'); //Numero teste
    selT606.SetVariable('DATA',A059FContSquadre.DataInizio);
...
procedure TA059FStampa.QRSubDetail1BeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
var Corr:TDateTime;
...
begin
  ...
  begin
    ...
    QP.SetVariable('Squadra',selT603.FieldByName('Codice').AsString);
    QP.SetVariable('TipoOpe','AND TIPOOPE = ''' + selT606.FieldByName('Cod_TipoOpe').AsString + '''');
    ...
    begin
      ...
      inc(Tot3[i],T3);
      inc(Tot4[i],T4);
      //Leggo i minimi e massimi registrati a livello di squadra / tipo operatore
      Mn1:=StrToIntDef(selT606.FieldByName('MIN1').AsString,0);
      Mn2:=StrToIntDef(selT606.FieldByName('MIN2').AsString,0);
      Mn3:=StrToIntDef(selT606.FieldByName('MIN3').AsString,0);
      Mn4:=StrToIntDef(selT606.FieldByName('MIN4').AsString,0);
      Mx1:=StrToIntDef(selT606.FieldByName('MAX1').AsString,0);
      Mx2:=StrToIntDef(selT606.FieldByName('MAX2').AsString,0);
      Mx3:=StrToIntDef(selT606.FieldByName('MAX3').AsString,0);
      Mx4:=StrToIntDef(selT606.FieldByName('MAX4').AsString,0);
...
begin
  ...
  begin
    ...
    NumTurno1:='';
    NumTurno2:='';
    selT021.Filtered:=False;
    selT021.Filter:='CODICE = ''' + MyDataSet.FieldByName('Orario').asString + '''';
    selT021.Filtered:=True;
    try
      if (StrToInt(T1) > 0) and (Trim(T1) <> '')then
      begin
        n:=1;
        selT021.First;
        if selT021.RecordCount >= StrToInt(T1) then
        begin
          ...
          if Trim(selT021.FieldByName('NUMTURNO').AsString) <> '' then
            if (T1EU = '') or (T1EU = 'E') then
              NumTurno1:=selT021.FieldByName('NUMTURNO').AsString;
        end;
      end;
    except
    end;
    try
      ...
      begin
        n:=1;
        selT021.First;
        if selT021.RecordCount >= StrToInt(T2) then
        begin
          ...
          if Trim(selT021.FieldByName('NUMTURNO').AsString) <> '' then
            if (T2EU = '') or (T2EU = 'E') then
              NumTurno2:=selT021.FieldByName('NUMTURNO').AsString;
        end;
      end;
    except
    end;
...
procedure TA059FStampa.QRBand1BeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
var Corr:TDateTime;
...
begin
  ...
  begin
    ...
    begin
      ...
      //Leggo i minimi e massimi totalizzati per squadra
      Mn1:=StrToIntDef(selT603.FieldByName('TOTMIN1').AsString,0);
      Mn2:=StrToIntDef(selT603.FieldByName('TOTMIN2').AsString,0);
      Mn3:=StrToIntDef(selT603.FieldByName('TOTMIN3').AsString,0);
      Mn4:=StrToIntDef(selT603.FieldByName('TOTMIN4').AsString,0);
      Mx1:=StrToIntDef(selT603.FieldByName('TOTMAX1').AsString,0);
      Mx2:=StrToIntDef(selT603.FieldByName('TOTMAX2').AsString,0);
      Mx3:=StrToIntDef(selT603.FieldByName('TOTMAX3').AsString,0);
      Mx4:=StrToIntDef(selT603.FieldByName('TOTMAX4').AsString,0);
      //Stabilisco lo stile Grassetto
