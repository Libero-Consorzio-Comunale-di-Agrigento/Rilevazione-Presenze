...
type
  TA024FIndPresenzaDtM1 = class(TDataModule)
    ...
    Q162MATURAZ_PROP_IGNORA_SCOSTNEG: TStringField;
    Q162CONGUAGLIO_EQUITURNI: TStringField;
...
begin
  ...
  if not (R180CarattereDef(Q162.FieldByName('TIPO').AsString) in ['A','B','P']) or
    ((not Q162.FieldByName('CODICE2').IsNull) and (Q162.FieldByName('CONGUAGLIO_EQUITURNI').AsString = 'N')) then
...
begin
  if not IndennitaTurno then
    if (not R180In(Sender.AsString,['Z','F','G','H','P'])) then
