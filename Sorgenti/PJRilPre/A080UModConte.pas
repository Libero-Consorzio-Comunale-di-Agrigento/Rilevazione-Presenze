...
uses
  ...
  A000USessione,A000UInterfaccia, StdCtrls, Mask, DBCtrls, ActnList, ImgList, ToolWin,
...
type
  TA080FModConte = class(TR001FGestTab)
    ...
    drgpRecuperoDebitoTipo: TDBRadioGroup;
    ...
    dedtLimiteCausali: TDBEdit;
    btnLimiteCausali: TButton;
    lblLimitiCausali: TLabel;
    dchkRipcomInclusiSaldo: TDBCheckBox;
    Label14: TLabel;
    DBEdit7: TDBEdit;
    tbsIterAutStr: TTabSheet;
    grpIterAutStr: TGroupBox;
    lblIterRichStraord: TLabel;
    lblIterAutStrArrotLiq: TLabel;
    lblIterAutStrMinimoLiq: TLabel;
    lblIterAutStrCausale: TLabel;
    dtxtIterAutStrCausale: TDBText;
    lblIterAutStrArrotEcc: TLabel;
    dcmbIterRichStraord: TDBLookupComboBox;
    dcmbIterAutStrCausale: TDBLookupComboBox;
    dedtIterAutStrMinimoLiq: TDBEdit;
    dedtIterAutStrArrotLiq: TDBEdit;
    dedtIterAutStrArrotEcc: TDBEdit;
    dchkIterAutStrArrotLiqFasce: TDBCheckBox;
    GroupBox10: TGroupBox;
    lblCausaliCompensabiliMensili: TLabel;
    dedtCausaliCompensabiliMensili: TDBEdit;
    btnCausaliCompensabiliMensili: TButton;
    lblRiepassCompensabiliMese: TLabel;
    dedtRiepassCompensabiliMese: TDBEdit;
    btnRiepassCompensabiliMese: TButton;
    drgpCausaliCompMensSaldo: TDBRadioGroup;
    lblCausaliCompensabili: TLabel;
    dedtCausaliCompensabili: TDBEdit;
    btnCausaliCompensabili: TButton;
    lblRiepassCompensabiliAnno: TLabel;
    dedtRiepassCompensabiliAnno: TDBEdit;
    btnRiepassCompensabiliAnno: TButton;
    drgpCausaliCompAnnSaldo: TDBRadioGroup;
    drgpCausaliCompAnnAddebPaghe: TDBRadioGroup;
    drgpRecuperoDebitoPeriodicita: TDBRadioGroup;
    drgpGGVuotoTurnista: TDBRadioGroup;
    drgpCausaliCompMeseRecNeg: TDBRadioGroup;
    ...
    procedure btnLimiteCausaliClick(Sender: TObject);
    procedure DButtonDataChange(Sender: TObject; Field: TField);
  private
    { Private declarations }
    ListaAssenze:TStringList;
    procedure LimiteMensileCalcoloCausaliVisible(Mostra:Boolean);
...
procedure TA080FModConte.LimiteMensileCalcoloCausaliVisible(Mostra:Boolean);
begin
  Mostra:=True;//Sempre visibile!

  lblLimitiCausali.Visible:=Mostra;
  btnLimiteCausali.Visible:=Mostra;
  dedtLimiteCausali.Visible:=Mostra;
end;
...
begin
  ...
  LimiteMensileCalcoloCausaliVisible(drgpTipoApplicazioneLimiteRes.Values[drgpTipoApplicazioneLimiteRes.ItemIndex] = 'EG');
end;

procedure TA080FModConte.GestioneVisibilitaIterAutStr;
var
  LIterAutStr: string;
  LIsStraordAnnuoCausPagComp: Boolean;
begin
  //LIterAutStr:=A080FModConteDtM1.Q025.FieldByName('ITER_AUTORIZZATIVO_STR').AsString;
  LIterAutStr:=DButton.DataSet.FieldByName('ITER_AUTORIZZATIVO_STR').AsString;
  ...
  dcmbIterAutStrCausale.Visible:=LIsStraordAnnuoCausPagComp;
  dtxtIterAutStrCausale.Visible:=LIsStraordAnnuoCausPagComp;
...
procedure TA080FModConte.DButtonDataChange(Sender: TObject; Field: TField);
begin
  inherited;
  LimiteMensileCalcoloCausaliVisible(drgpTipoApplicazioneLimiteRes.Values[drgpTipoApplicazioneLimiteRes.ItemIndex] = 'EG');

  if (Field = nil) or (Field.FieldName = 'ITER_AUTORIZZATIVO_STR') then
...
begin
  ...
  try
    ...
    begin
      ...
      DS:=A080FModConteDtM1.dsrT275EsclNorm;
    end
    else if Sender = btnCausaliCompensabiliMensili then
    begin
      F:=(dedtCausaliCompensabiliMensili.Field as TStringField);
      DS:=A080FModConteDtM1.dsrT275EsclNorm;
    end
    else if Sender = btnRiepAssCompensabiliMese then
    begin
      F:=(dedtRiepassCompensabiliMese.Field as TStringField);
      DS:=A080FModConteDtM1.dsrT265RNF;
      Filtro:='UMISURA = ''O''';
    end
    else if Sender = btnRiepAssCompensabiliAnno then
    begin
      F:=(dedtRiepassCompensabiliAnno.Field as TStringField);
      DS:=A080FModConteDtM1.dsrT265RNF;
      Filtro:='UMISURA = ''O''';
    end;

    A080FCausaliCompensabili:=TA080FCausaliCompensabili.Create(nil);
    A080FCausaliCompensabili.dcmbCausali.ListSource:=DS;
    if Filtro <> '' then
    begin
      DS.DataSet.Filter:=Filtro;
...
procedure TA080FModConte.btnLimiteCausaliClick(Sender: TObject);

  procedure CaricaLista;
  begin
    C013FCheckList.clbListaDati.Clear;
    A080FModConteDtM1.selT275LimiteCaus.Open;
    A080FModConteDtM1.selT275LimiteCaus.First;
    while not A080FModConteDtM1.selT275LimiteCaus.Eof do
    begin
      C013FCheckList.clbListaDati.Items.Add(Format('%5s %s',[A080FModConteDtM1.selT275LimiteCaus.FieldByName('CODICE').asString,
                                                             A080FModConteDtM1.selT275LimiteCaus.FieldByName('DESCRIZIONE').asString]));
      A080FModConteDtM1.selT275LimiteCaus.Next;
    end;
  end;
...
begin
  ...
  try
    CaricaLista;
    R180PutCheckList(dedtLimiteCausali.Field.AsString,5,C013FCheckList.clbListaDati);
    if C013FCheckList.ShowModal = mrOK then
      dedtLimiteCausali.Field.AsString:=R180GetCheckList(5,C013FCheckList.clbListaDati);
  finally
...
begin
  ...
  begin
    ...
    begin
      ...
      insT026.SetVariable('CODICE_OLD',VarToStr(Q026.GetVariable('Codice')));
      insT026.Execute;
