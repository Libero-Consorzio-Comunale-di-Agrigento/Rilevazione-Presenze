...
type
  TA053FSquadreDtM1 = class(TDataModule)
    ...
    Q600MAX_NOTTI_MESE: TIntegerField;
    Q651: TOracleDataSet;
    ...
    procedure Q601BeforeDelete(DataSet: TDataSet);
    procedure Q601BeforePost(DataSet: TDataSet);
  private
...
procedure TA053FSquadreDtM1.Q601BeforeDelete(DataSet: TDataSet);
begin
  Q651CancellaOperatore.SetVariable('Squadra',Q601Squadra.AsString);
  Q651CancellaOperatore.SetVariable('Operatore',Q601Codice.AsString);
  Q651CancellaOperatore.Execute;
  RegistraLog.SettaProprieta('C',R180Query2NomeTabella(DataSet),Copy(Name,1,4),DataSet,True);
end;

procedure TA053FSquadreDtM1.Q601BeforePost(DataSet: TDataSet);
{Gestisco il cambiamento di tipo operatore: lo rifletto sull'area}
begin
  if (QueryPK1.EsisteChiave('T601_TIPIOPERATORE',Q601.RowId,Q601.State,['SQUADRA','CODICE'],[Q601Squadra.AsString, Q601Codice.AsString])) then
    raise Exception.Create('Codice tipo operatore già esistente!');
  if Q601.State = dsEdit then
    if Q601Codice.Value <> Q601Codice.medpOldValue then
    begin
      Q651AggiornaOperatore.SetVariable('Squadra',Q601Squadra.Value);
      Q651AggiornaOperatore.SetVariable('Operatore',Q601Codice.Value);
      Q651AggiornaOperatore.SetVariable('Operatore_Old',Q601Codice.medpOldValue);
      try
...
procedure TA053FSquadreDtM1.Q651NewRecord(DataSet: TDataSet);
begin
  Q651Codice_Squadra.AsString:=Q600Codice.AsString;
end;

procedure TA053FSquadreDtM1.Q600BeforeDelete(DataSet: TDataSet);
begin
  Q601CancellaSquadra.SetVariable('Squadra',Q600Codice.AsString);
  Q601CancellaSquadra.Execute;
  Q651CancellaSquadra.SetVariable('Squadra',Q600Codice.AsString);
...
begin
  ...
  if Q600.State = dsEdit then
    ...
    begin
      Q601AggiornaSquadra.SetVariable('Squadra',Q600Codice.Value);
      Q601AggiornaSquadra.SetVariable('Squadra_Old',Q600Codice.medpOldValue);
      Q651AggiornaSquadra.SetVariable('Squadra',Q600Codice.Value);
      Q651AggiornaSquadra.SetVariable('Squadra_Old',Q600Codice.medpOldValue);
      try
        Q601AggiornaSquadra.Execute;
