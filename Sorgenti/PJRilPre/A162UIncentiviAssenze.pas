...
uses
  ...
  System.Actions, C009UCopiaSu, OracleData, A000UMessaggi;
...
type
  TA162FIncentiviAssenze = class(TR004FGestStorico)
    ...
    procedure dcmbCausaleKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure dedtDataFineExit(Sender: TObject);
    procedure CambiaCodTipoQuota1Click(Sender: TObject);
    procedure pmnCodTipoQuotaPopup(Sender: TObject);
...
begin
  with A162FIncentiviAssenzeDtM do
  begin
    CopiaDa1.Visible:=(selT769.State = dsBrowse) and (selT769.FieldByName('CODTIPOQUOTA').AsString <> '*');
    CopiaDa1.Enabled:=CopiaDa1.Visible;
...
procedure TA162FIncentiviAssenze.dedtDataFineExit(Sender: TObject);
var sTmp: String;
begin
  inherited;
  with A162FIncentiviAssenzeDtM do
  begin
    if selT769.State in [dsEdit,dsInsert] then
    begin
      A162FIncentiviAssenzeMW.ImpostaDecorrenzaFineDatasetLookup;
      //reimposto campi per forzare reload del campo di lookup
      sTmp:=selT769.FieldByName('CODTIPOQUOTA').AsString;
      selT769.FieldByName('CODTIPOQUOTA').AsString:='';
      selT769.FieldByName('CODTIPOQUOTA').AsString:=sTmp;
    end;
  end;
end;
...
procedure TA162FIncentiviAssenze.pmnCodTipoQuotaPopup(Sender: TObject);
begin
  inherited;
  CambiaCodTipoQuota1.Visible:=(DButton.State = dsBrowse) and (A162FIncentiviAssenzeDtM.selT769.FieldByName('CODTIPOQUOTA').AsString = '*');
end;

procedure TA162FIncentiviAssenze.CambiaCodTipoQuota1Click(Sender: TObject);
var InputString:String;
begin
  InputString:=InputBox('Nuovo codice quota', 'Indicare il nuovo codice quota:', '');
  A162FIncentiviAssenzeDtM.A162FIncentiviAssenzeMW.CambiaCodTipoQuota(InputString);
end;

procedure TA162FIncentiviAssenze.Copiada1Click(Sender: TObject);
var InputString:String;
    i:Integer;
begin
  InputString:=InputBox('Nuovo codice quota', 'Indicare il nuovo codice quota:', '');
  if (Trim(InputString) = '')
  or (InputString = '*')
  or (VarToStr(A162FIncentiviAssenzeDtM.A162FIncentiviAssenzeMW.selT765.Lookup('CODICE',InputString,'CODICE')) = '') then
  ...
  try
    ...
    for i:=0 to grdChiaveElemento.RowCount - 1 do
      if grdChiaveElemento.Cells[0,i] = A162FIncentiviAssenzeDtM.selT769.FieldByName('CODTIPOQUOTA').DisplayLabel then
        grdChiaveElemento.Cells[1,i]:=InputString;
    btnEseguiClick(nil);
  finally
    Free;
  end;
