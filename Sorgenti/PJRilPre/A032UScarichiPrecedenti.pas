  ...
  try
    ...
    for i:=0 to lstFiles.Items.Count - 1 do
      ...
      begin
        ...
        {R200FScaricoTimbratureDtM}A032FScarico.R200DM.NFRecupero:=IncludeTrailingPathDelimiter(edtPathTimb.Text) + VetFile[i].Nome;//edtPathTimb.Text + '\' + VetFile[i].Nome;
...
begin
  ...
  begin
    edtPathTimb.Text:=R180GetFilePath(dlgPathTimb.FileName,True);
    CaricaFile(lstFiles,edtPathTimb.Text);
  end;
end;
...
begin
  ...
  edtFOraA.Text:='23.59';
  with A032FScarico.R200DM do
  begin
    QI100.SearchRecord('SCARICO',A032FScarico.EScarico.KeyValue,[srFromBeginning]);
