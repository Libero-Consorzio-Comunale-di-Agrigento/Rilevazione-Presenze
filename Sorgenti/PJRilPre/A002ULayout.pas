...
type
  ...
  private
    ...
    Trascina:Boolean;
    XScroll,YScroll,XDisplacement,YDisplacement,Group:integer;
    NDL:Byte;
    Nome:string;
  ...
  public
    ...
    class function GetScrollBoxTab(Tab:TTabSheet):TWinControl;
    class function GetTab(parControl:TWinControl):TWinControl;
...
begin
  ...
  begin
    ...
    begin
      ...
      TScrollBox(GetScrollBoxTab(PageControl1.Pages[i])).OnMouseDown:=TabSheetMouseDown;
      TScrollBox(GetScrollBoxTab(PageControl1.Pages[i])).OnMouseMove:=TabSheetMouseMove;
      TScrollBox(GetScrollBoxTab(PageControl1.Pages[i])).OnMouseUp:=TabSheetMouseUp;
    end;
  end;
...
begin
  ...
  with A002FAnagrafeGest do
  begin
    //Creazione scrollbox sulle pagine di default
    for i:=0 to 2 do
      if PageControl1.Pages[i] = GetScrollBoxTab(PageControl1.Pages[i]) then
      begin
        SB:=TScrollBox.Create(PageControl1.Pages[i]);
        SB.Parent:=PageControl1.Pages[i];
        SB.Align:=alClient;
        SB.BorderStyle:=bsNone;
      end;

    //1° Pagina
    Layout[0].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[0].LinkComp[0]:=L1;
    Layout[0].LinkComp[1]:=EMatricola;

    Layout[1].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[1].LinkComp[0]:=L2;
    Layout[1].LinkComp[1]:=ECognome;

    Layout[2].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[2].LinkComp[0]:=L3;
    Layout[2].LinkComp[1]:=ENome;

    Layout[3].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[3].LinkComp[0]:=DBRadioGroup1;

    Layout[4].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[4].LinkComp[0]:=L4;
    Layout[4].LinkComp[1]:=ENascita;

    Layout[5].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[5].LinkComp[0]:=L5;
    Layout[5].LinkComp[1]:=LookupComune;
    Layout[5].LinkComp[2]:=EComune;

    Layout[6].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[6].LinkComp[0]:=L6;
    Layout[6].LinkComp[1]:=ECapNas;

    Layout[7].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[7].LinkComp[0]:=L50;
    Layout[7].LinkComp[1]:=EProvNas;

    Layout[8].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[8].LinkComp[0]:=LI060EMail;
    Layout[8].LinkComp[1]:=EI060EMail;

    Layout[9].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[9].LinkComp[0]:=L52;
    Layout[9].LinkComp[1]:=ECodFiscale;

    Layout[10].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[10].LinkComp[0]:=L7;
    Layout[10].LinkComp[1]:=EditBadge;

    Layout[11].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[11].LinkComp[0]:=L53;
    Layout[11].LinkComp[1]:=EEdBadge;

    Layout[12].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[12].LinkComp[0]:=DBRadioGroup2;

    Layout[13].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[13].LinkComp[0]:=DChkDocente;

    Layout[14].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[14].LinkComp[0]:=lblIndirizzo;
    Layout[14].LinkComp[1]:=dedtIndirizzo;

    Layout[15].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[15].LinkComp[0]:=lblComune;
    Layout[15].LinkComp[1]:=dcmbComune; //dedtComune;
    Layout[15].LinkComp[2]:=dedtComune;

    Layout[16].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[16].LinkComp[0]:=lblCap;
    Layout[16].LinkComp[1]:=dedtCap;

    Layout[17].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[17].LinkComp[0]:=lblProvincia;
    Layout[17].LinkComp[1]:=dedtProvincia;

    Layout[18].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[18].LinkComp[0]:=lblTelefono;
    Layout[18].LinkComp[1]:=dedtTelefono;

    Layout[19].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[19].LinkComp[0]:=L15;
    Layout[19].LinkComp[1]:=EInizio;

    Layout[20].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[20].LinkComp[0]:=L16;
    Layout[20].LinkComp[1]:=EFine;

    Layout[21].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[21].LinkComp[0]:=L26;
    Layout[21].LinkComp[1]:=ESquadra;
    Layout[21].LinkComp[2]:=LSquadra;

    Layout[22].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[22].LinkComp[0]:=L27;
    Layout[22].LinkComp[1]:=ETipoOpe;

    Layout[23].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[23].LinkComp[0]:=L17;
    Layout[23].LinkComp[1]:=EInizioServizio;

    Layout[24].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[24].LinkComp[0]:=L54;
    Layout[24].LinkComp[1]:=ETipoRapp;
    Layout[24].LinkComp[2]:=LTipoRapp;

    Layout[25].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[25].LinkComp[0]:=L28;
    Layout[25].LinkComp[1]:=ETerminali;

    Layout[26].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[26].LinkComp[0]:=dchkRapportiUniti;
    //2° Pagina
    Layout[27].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[27].LinkComp[0]:=ECausStraord;

    Layout[28].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[28].LinkComp[0]:=EStraordE;

    Layout[29].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[29].LinkComp[0]:=EStraordU;

    Layout[30].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[30].LinkComp[0]:=EStraordEU;

    Layout[31].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[31].LinkComp[0]:=EStraordEU2;

    Layout[32].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[32].LinkComp[0]:=L29;
    Layout[32].LinkComp[1]:=EContratto;
    Layout[32].LinkComp[2]:=LContratto;

    Layout[33].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[33].LinkComp[0]:=L30;
    Layout[33].LinkComp[1]:=EOrario;

    Layout[34].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[34].LinkComp[0]:=EHTeoriche;

    Layout[35].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[35].LinkComp[0]:=L55;
    Layout[35].LinkComp[1]:=EPartTime;
    Layout[35].LinkComp[2]:=LPartTime;

    Layout[36].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[36].LinkComp[0]:=L56;
    Layout[36].LinkComp[1]:=ETipoCart;
    Layout[36].LinkComp[2]:=LTipoCart;

    Layout[37].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[37].LinkComp[0]:=ETGestione;

    Layout[38].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[38].LinkComp[0]:=L31;
    Layout[38].LinkComp[1]:=EPlusOra;
    Layout[38].LinkComp[2]:=LPlusOra;

    Layout[39].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[39].LinkComp[0]:=L32;
    Layout[39].LinkComp[1]:=ECalendario;
    Layout[39].LinkComp[2]:=LCalendario;

    Layout[40].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[40].LinkComp[0]:=L33;
    Layout[40].LinkComp[1]:=EIPresenza;
    Layout[40].LinkComp[2]:=LIPresenza;

    Layout[41].Parent:=GetScrollBoxTab(PageControl1.Pages[1]);
    Layout[41].LinkComp[0]:=L34;
    Layout[41].LinkComp[1]:=EPOrario;
    Layout[41].LinkComp[2]:=LPOrario;

    //3° Pagina
    Layout[42].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    Layout[42].LinkComp[0]:=L35;
    Layout[42].LinkComp[1]:=EPAssenze;
    Layout[42].LinkComp[2]:=LPAssenze;

    Layout[43].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    ...
    Layout[44].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    ...
    Layout[45].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    Layout[45].LinkComp[0]:=drgpTipoLocalitaDistLavoro;

    Layout[46].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    ...
    Layout[47].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    ...
    Layout[48].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    Layout[48].LinkComp[0]:=lblAziendaBase;
    Layout[48].LinkComp[1]:=dcmbAziendaBase;
    Layout[48].LinkComp[2]:=dlblAziendaBase;

    // altri dati
    Layout[49].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[49].LinkComp[0]:=L57;
    Layout[49].LinkComp[1]:=EInizioIndMat;

    Layout[50].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[50].LinkComp[0]:=L58;
    Layout[50].LinkComp[1]:=EFineIndMat;

    Layout[51].Parent:=GetScrollBoxTab(PageControl1.Pages[2]);
    Layout[51].LinkComp[0]:=lblMedicinaLegale;
    Layout[51].LinkComp[1]:=dcmbMedicinaLegale;
    Layout[51].LinkComp[2]:=dlblMedicinaLegale;

    // domicilio
    Layout[52].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[52].LinkComp[0]:=lblIndirizzoDomBase;
    Layout[52].LinkComp[1]:=dedtIndirizzoDomBase;

    Layout[53].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[53].LinkComp[0]:=lblComuneDomBase;
    Layout[53].LinkComp[1]:=dcmbComuneDomBase;
    Layout[53].LinkComp[2]:=dedtComuneDomBase;

    Layout[54].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[54].LinkComp[0]:=lblCapDomBase;
    Layout[54].LinkComp[1]:=dedtCapDomBase;

    Layout[55].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[55].LinkComp[0]:=lblProvinciaDomBase;
    Layout[55].LinkComp[1]:=dedtProvinciaDomBase;

    Layout[56].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[56].LinkComp[0]:=LI060EMailPEC;
    Layout[56].LinkComp[1]:=EI060EMailPEC;

    Layout[57].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[57].LinkComp[0]:=drgpTipoSoggettoBase;

    Layout[58].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[58].LinkComp[0]:=LI060Cellulare;
    Layout[58].LinkComp[1]:=EI060Cellulare;

    Layout[59].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[59].LinkComp[0]:=LI060EMailPersonale;
    Layout[59].LinkComp[1]:=EI060EMailPersonale;

    Layout[60].Parent:=GetScrollBoxTab(PageControl1.Pages[0]);
    Layout[60].LinkComp[0]:=LI060CellularePersonale;
    Layout[60].LinkComp[1]:=EI060CellularePersonale;

    // IMPORTANTE: incrementare NumComp se si aggiungono componenti!
...
begin
  for i:=0 to (NumComp+NDL) do
    with Layout[i] do
    begin
      if Layout[i].Parent is TScrollBox then
      begin
        (Layout[i].Parent as TScrollBox).HorzScrollBar.Position:=0;
        (Layout[i].Parent as TScrollBox).VertScrollBar.Position:=0;
      end;
      STop:=LinkComp[0].Top;
...
begin
  ...
  begin
    ...
    if (X + XScroll > Layout[i].Rett.Left) and (X + XScroll < Layout[i].Rett.Right) and
       (Y + YScroll > Layout[i].Rett.Top) and (Y + YScroll < Layout[i].Rett.Bottom) then
    begin
      Result:=i;
      xD:=X - (Layout[i].Rett.Left - XScroll);
      yD:=Y - (Layout[i].Rett.Top - YScroll);
      Break;
    end;
  end;
end;
  ...
  begin
    for i:=0 to A002FAnagrafeGest.PageControl1.PageCount - 1 do
      ...
      begin
        ...
        Result:=GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i]);
        Break;
      end;
  end;
end;

class function TA002FLayout.GetScrollBoxTab(Tab:TTabSheet):TWinControl;
{Restituisco Scrollbox all'interno del TabSheet}
var i:integer;
begin
  Result:=Tab;
  for i:=0 to Tab.ControlCount - 1 do
  begin
    if Tab.Controls[i] is TScrollBox then
    begin
      Result:=(Tab.Controls[i] as TScrollBox);
      Break;
    end;
  end;
end;

class function TA002FLayout.GetTab(parControl:TWinControl):TWinControl;
begin
  Result:=nil;
  if parControl is TTabSheet then
    Result:=parControl
  else if parControl is TScrollBox then
    Result:=parControl.Parent;
end;
...
begin
  ...
  begin
    Shape1.Top:=Top - YScroll;
    Shape1.Left:=Left - XScroll;
...
begin
  ...
  if (Shift = [ssAlt]) and ((Key = vk_Up)or(Key = vk_Down)or
    ...
    with Layout[Group].Rett do
    begin
      Top:=Shape1.Top + YScroll;
      Left:=Shape1.Left + XScroll;
      Bottom:=Top + Shape1.Height;
      Right:=Left + Shape1.Width;
    end;
end;
...
begin
  ...
  begin
    XScroll:=(Sender as TScrollBox).HorzScrollBar.ScrollPos;
    YScroll:=(Sender as TScrollBox).VertScrollBar.ScrollPos;
  end;
...
begin
  ...
  begin
    ...
    stbBottom1.Panels[1].Text:='Coordinata X del campo: ' + IntToStr(Shape1.Left + XScroll);
    stbBottom1.Panels[2].Text:='Coordinata Y del campo: ' + IntToStr(Shape1.Top + YScroll);
  end;
end;
...
begin
  ...
  begin
    ...
    for i:=1 to 2 do
      if Layout[group].LinkComp[i] <> nil then
        with Layout[group].LinkComp[i] do
        begin
          varx:=left - OldLeft;
          vary:=top - OldTop;
          top:=Layout[group].LinkComp[0].top + vary;
          left:=Layout[group].LinkComp[0].left + varx;
        end;
    with Layout[Group].Rett do
    begin
      Top:=Shape1.Top + YScroll;
...
begin
  ...
  begin
    ...
    for j:=0 to TScrollBox(GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i])).ControlCount - 1 do
    begin
      if TScrollBox(GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i])).Controls[j].Visible then
      begin
    ...
    try
      ...
      begin
        ...
        begin
          ...
          else if Parent is TScrollBox then
            InsQ033.SetVariable('NomePagina',(Parent.Parent as TTabSheet).Caption);
...
begin
  with A002FAnagrafeDtM1 do
  begin
    if not selT033.Active then
      R180SetReadBuffer(selT033);
    selT033.Open;
    ...
    begin
      ...
      for i:=0 to A002FAnagrafeGest.PageControl1.PageCount - 1 do
        ...
        begin
          NewPage:=False;
          TS:=A002FAnagrafeGest.PageControl1.Pages[i];
          Break;
        end;
      if NewPage then
        //with A002FAnagrafeGest.PageControl1 do
        begin
          ...
          with TS do //TTabSheet.Create(nil) do
          begin
            PageControl:=A002FAnagrafeGest.PageControl1;
            Caption:=selT033.FieldByName('NomePagina').AsString;
          end;
        end;
      if TS = GetScrollBoxTab(TS) then
      begin
...
begin
  for i:=0 to A002FAnagrafeGest.PageControl1.PageCount - 1 do
    if GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i]) is TScrollBox then
    begin
      TScrollBox(GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i])).VertScrollBar.Position:=0;
      TScrollBox(GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i])).HorzScrollBar.Position:=0;
    end;
      ...
      begin
        ...
        for i:=0 to A002FAnagrafeGest.PageControl1.PageCount - 1 do
          ...
          begin
            ...
            Layout[NC].Parent:=GetScrollBoxTab(A002FAnagrafeGest.PageControl1.Pages[i]);
            Break;
          end;
...
{Creo la configurazione di default nel caso non esista smistando i dati liberi su
 Dati liberi 1 e Dati liberi 2}
var
  NC:integer;
  CampoDB:String;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Parente:=GetScrollBoxTab(TTabSheet(Parente));
...
begin
  ...
  begin
    ...
    for i:=0 to PageControl1.PageCount - 1 do
    begin
      TScrollBox(GetScrollBoxTab(PageControl1.Pages[i])).OnMouseDown:=nil;
      TScrollBox(GetScrollBoxTab(PageControl1.Pages[i])).OnMouseMove:=nil;
      TScrollBox(GetScrollBoxTab(PageControl1.Pages[i])).OnMouseUp:=nil;
    end;
  end;
  CanClose:=True;
  Attivazione;
      ...
      try
        ...
        if FSelezionaDati.ShowModal = mrOk then
          ...
          for i:=0 to FSelezionaDati.ListBox2.Items.Count-1 do
            begin
            k:=0;
            while ObjCaption(Layout[k].LinkComp[0],StrCaption,0) <>
                  FSelezionaDati.ListBox2.Items[i] do k:=k+1;
            for j:=0 to 2 do
              if Layout[k].LinkComp[j] <> nil then
                with Layout[k] do
                begin
                  Parent:=CercaParent;
                  LinkComp[j].Parent:=Parent;
                  LinkComp[j].Visible:=True;
                  if R180In(LinkComp[j],[A002FAnagrafeGest.EI060EMail, A002FAnagrafeGest.EI060EMailPersonale, A002FAnagrafeGest.EI060EMailPEC, A002FAnagrafeGest.EI060Cellulare, A002FAnagrafeGest.EI060CellularePersonale]) then
                    Accesso:='R'
                  else
                    Accesso:='S';
                end;
...
begin
  ...
  try
    ...
    edtCoordX.text:=IntToStr(Shape1.Left + XScroll);
    edtCoordY.text:=IntToStr(Shape1.Top + YScroll);
    ...
    begin
      ...
      begin
        xdisplacement:=0;
        ydisplacement:=0;
        Shape1.Left:=StrToInt(Trim(edtCoordX.text)) - XScroll;
        Trascina:=True;
      ...
      begin
        xdisplacement:=0;
        ydisplacement:=0;
        Shape1.Top:=StrToInt(Trim(edtCoordY.text)) - YScroll;
        Trascina:=True;
        TabSheetMouseUp(sender,mbLeft,[],Shape1.Left,Shape1.Top);
      end;
    end;
    ...
    0:
      begin
        for j:=0 to High(Layout) do
          if Layout[j].Parent = GetScrollBoxTab(A002FAnagrafeGest.PageControl1.ActivePage) then
          begin
            for i:=0 to 2 do
