...
uses
  ...
  A002UInterfacciaSt, L001Call, R001UGESTTAB, Variants, System.Actions, A003UDataLavoroBis,
  System.ImageList;
...
type
  TA011FComuniProvinceRegioni = class(TR001FGestTab)
    ...
    tshNazioni: TTabSheet;
    Panel4: TPanel;
    lblSelNazione: TLabel;
    lblCodNazione: TLabel;
    lblDescNazione: TLabel;
    dcmbNazione: TDBLookupComboBox;
    dedtCodNazione: TDBEdit;
    dedtDescNazione: TDBEdit;
    dgrdListaNazioni: TDBGrid;
    dedtCodIRPEFComune: TDBEdit;
    lblCodIRPEFComune: TLabel;
    dtxtD_CodIRPEF: TDBText;
    drgpStato: TDBRadioGroup;
    dedtDataSoppressione: TDBEdit;
    lblDataSoppressione: TLabel;
    btnDataSoppressione: TBitBtn;
    ...
    procedure dgrdListaComuniDrawColumnCell(Sender: TObject; const Rect: TRect;
      DataCol: Integer; Column: TColumn; State: TGridDrawState);
    procedure btnDataSoppressioneClick(Sender: TObject);
    procedure drgpStatoChange(Sender: TObject);
  private
...
begin
  ...
  begin
    ...
    begin
      ...
      CambiaDataSet;
      T483.SearchRecord('COD_NAZIONE',CodTabella,[srFromBeginning]);
    end
    else
    begin
...
procedure TA011FComuniProvinceRegioni.btnDataSoppressioneClick(Sender: TObject);
begin
  inherited;
  A011FComuniProvinceRegioniDtM.T480.FieldByName('DATA_SOPPRESSIONE').AsDateTime:=
    DataOut(A011FComuniProvinceRegioniDtM.T480.FieldByName('DATA_SOPPRESSIONE').AsDateTime,'Data soppressione','G');
end;
...
begin
  ...
  begin
    ...
    else if pgcmain.ActivePageIndex = 3 then
    begin
      DButton.DataSet:=T483;
      dgrdListaNazioni.SetFocus;
      NumRecords;
    end;
    InizializzaDButton;
  end;
...
begin
  ...
  begin
    ...
    btnDataSoppressione.Enabled:=(DButton.State in [dsEdit,dsInsert]);
  end
  else if pgcmain.ActivePageIndex = 1 then
  begin
  ...
  begin
    if (DButton.State in [dsEdit,dsInsert]) and (dgrdListaNazioni.Focused) then
    try
      dedtCodNazione.SetFocus;
    except
    end;
    dgrdListaNazioni.Enabled:=DButton.State = dsBrowse;
    dcmbNazione.Enabled:=DButton.State = dsBrowse;
  end;
end;
...
begin
  ...
  begin
    case pgcmain.ActivePageIndex of
      ...
      3:QueryStampa.Add(T483.SQL.Text);
    end;
  end;
  inherited;
end;
...
procedure TA011FComuniProvinceRegioni.dgrdListaComuniDrawColumnCell(
  Sender: TObject; const Rect: TRect; DataCol: Integer; Column: TColumn;
  State: TGridDrawState);
begin
  inherited;
  if A011FComuniProvinceRegioniDtM.T480.FieldByName('SOPPRESSO').AsString = 'S' then
...
procedure TA011FComuniProvinceRegioni.drgpStatoChange(Sender: TObject);
begin
  inherited;
  lblDataSoppressione.Visible:=drgpStato.ItemIndex = 2;
  dedtDataSoppressione.Visible:=drgpStato.ItemIndex = 2;
  btnDataSoppressione.Visible:=drgpStato.ItemIndex = 2;
end;
