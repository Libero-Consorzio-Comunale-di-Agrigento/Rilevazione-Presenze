...
uses
  ...
  A083UMsgElaborazioni,A000UMessaggi, System.StrUtils, InputPeriodo;
...
type
  TA074FRiepilogoBuoni = class(TForm)
    ...
    procedure frmInputPeriodoedtInizioChange(Sender: TObject);
    procedure frmInputPeriodoedtFineChange(Sender: TObject);
  private
    FileSeq:TextFile;
...
begin
  A074FRiepilogoBuoniDtM1.A074FRiepilogoBuoniMW.CodForm:=IfThen(TipoModulo = TTipoModuloRec.ClientServer,'A074','WA074');
  ...
  begin
    if TipoModulo = TTipoModuloRec.ClientServer then
      ShowMessage(A000MSG_A074_ERR_CAMPO_RIFERIMENTO);
    Close;
  end;
  A074FStampaBuoni.RepR.PrinterSettings.UseStandardPrinter:=(TipoModulo = TTipoModuloRec.COM) and Parametri.UseStandardPrinter;
  A074FStampaAcquisti.QRep.PrinterSettings.UseStandardPrinter:=(TipoModulo = TTipoModuloRec.COM) and Parametri.UseStandardPrinter;
...
begin
  ...
  RegistraMsg.IniziaMessaggio(A074FRiepilogoBuoniDtM1.A074FRiepilogoBuoniMW.CodForm);
  ...
  if PageControl1.ActivePage = tabMaturazione then
    ...
    CartellinoAcquisto;
  btnAnomalie.Enabled:=RegistraMsg.ContieneAnomalie or RegistraMsg.ContieneTipoI;
  if (not RegistraMsg.ContieneAnomalie) and RegistraMsg.ContieneTipoI then
...
begin
  ...
  if ChkInizioAnno.Checked then
    ...
    A074FRiepilogoBuoniDtM1.A074FRiepilogoBuoniMW.R350FCalcoloBuoniDtM.R502ProDtM1.PeriodoConteggi(DataI - 2,DataF);
  ...
  if (Trim(DocumentoPDF) <> '') and (Trim(DocumentoPDF) <> '<VUOTO>') and (Trim(TipoModulo) = TTipoModuloRec.COM)then
  begin
...
begin
  ...
  begin
    if (TipoModulo = TTipoModuloRec.ClientServer) and
       (MessageDlg(A000MSG_A074_DLG_FILE_SEQUENZIALE, mtInformation,[mbYes,mbNo],0,mbNo) = mrNo) then
      Abort;
    if ((TipoModulo = TTipoModuloRec.ClientServer) and (InputBox('Generazione file sequenziale','Password','') <> 'A074FS')) or
       ((TipoModulo = TTipoModuloRec.COM) and (FsPassword <> 'A074FS'))then
...
begin
  ...
  begin
    frmSelAnagrafe.VisualizzaDipendente;
    ProgressBar1.StepBy(1);
    //Considero il personale in servizio negli ultimi 12 mesi, poi il calcolo verificherà puntualmente l'esistenza in servizio nel periodo definito dalle regole di acquisto
    if DipInSer.DipendenteInServizio(C700Progressivo,R180AddMesi(R180InizioMese(DataAcquisto),-12),R180FineMese(DataAcquisto)) then
    begin
...
procedure TA074FRiepilogoBuoni.frmInputPeriodoedtFineChange(Sender: TObject);
begin
  frmInputPeriodo.edtDataChange(Sender);
  AbilitaSalvataggio(DataI,DataF);
end;

procedure TA074FRiepilogoBuoni.frmInputPeriodoedtInizioChange(Sender: TObject);
begin
  frmInputPeriodo.edtDataChange(Sender);
  AbilitaSalvataggio(DataI,DataF);
end;
