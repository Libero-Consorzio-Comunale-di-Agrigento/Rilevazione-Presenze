...
type
  TA100FMISSIONIDTM = class(TR004FGestStoricoDtM)
    ...
    M040DescDestinazione: TStringField;
    M040DescPartenza2: TStringField;
...
begin
  inherited;
  M040.FieldByName('DescPartenza').AsString:=A100FMissioniMW.CodISTATtoCitta(M040.FieldByName('PARTENZA').AsString);
  M040.FieldByName('DescDestinazione').AsString:=A100FMissioniMW.CodISTATtoCitta(M040.FieldByName('DESTINAZIONE').AsString);
...
begin
  inherited;
  // AOSTA_REGIONE - chiamata 91104.ini
  if M040.FieldByName('ID').AsInteger > 0 then
  begin
    if R180MessageBox('La trasferta è collegata ad una richiesta web.'#13#10'Eliminare comunque?'#13#10'Rispondendo Sì, la richiesta web sarà annullata.',DOMANDA) = mrNo then
      Abort;
  end;
  // AOSTA_REGIONE - chiamata 91104.fine
...
// AOSTA_REGIONE - chiamata 91104.ini
procedure TA100FMISSIONIDTM.AfterDelete(DataSet: TDataSet);
begin
  inherited;
