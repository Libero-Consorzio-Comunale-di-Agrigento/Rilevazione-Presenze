...
uses
  ...
  C015UElencoValori, C020UVisualizzaDataSet;
...
type
  TA100FMISSIONI = class(TR001FGestTab)
    ...
    dLblPartenza: TDBText;
    dLblDestinazione: TDBText;
    btnPartenza: TButton;
    btnDestinazione: TButton;
    ...
    procedure pmnAllegatiPopup(Sender: TObject);
    procedure btnPartenzaClick(Sender: TObject);
    procedure btnDestinazioneClick(Sender: TObject);
    procedure btnDettaglioPercorsoClick(Sender: TObject);
  private
    FC021DM: TC021FDocumentiManagerDM;
    function GetCodISTAT:string;
...
uses
  A100UMISSIONIDTM, A100URicalcoloIndTrasp, A100UDettaglioRimborsi,
  A100UDistanzeChilometriche, A131UGestioneAnticipi, A133UTariffeMIssioni, A100UImpRimborsiIter,
  A000UMessaggi;
...
begin
  ...
  btnPartenza.Enabled:=DButton.State in [dsInsert, dsEdit];
  btnDestinazione.Enabled:=DButton.State in [dsInsert, dsEdit];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if M040DESCPARTENZA2.AsString <> '' then
          CmbPartenza.Text:=CmbPartenza.Text + ' - ' + M040DESCPARTENZA2.AsString;
      end
      else
        CmbPartenza.Text:='';
...
function TA100FMISSIONI.GetCodISTAT:string;
var
  Selezione:String;
  vCodice: Variant;
begin
  Result:='';
  Selezione:='select T480.CODICE as CODICE_ISTAT, T480.CITTA, T480.PROVINCIA, T480.CAP' +
             '  from T480_COMUNI T480' +
             ' order by T480.CITTA';
  vCodice:=VarArrayOf(['','','','']);
  OpenC015FElencoValori('T480_COMUNI', A000MSG_A100_MSG_ELENCO_COMUNI,Selezione,'CODICE_ISTAT;CITTA;PROVINCIA;CAP',vCodice,nil,500,400);
  if not VarIsClear(vCodice) then
  begin
    Result:=VarToStr(vCodice[0]);
  end;
end;

procedure TA100FMISSIONI.btnDestinazioneClick(Sender: TObject);
var
  Aus:string;
begin
  inherited;
  Aus:=GetCodISTAT;
  if not Aus.IsEmpty then
  begin
    CmbDestinazione.Text:=Aus;
    A100FMISSIONIDTM.M040.FieldByName('DescDestinazione').AsString:=A100FMISSIONIDTM.A100FMissioniMW.CodISTATtoCitta(Aus);
  end;
end;
...
procedure TA100FMISSIONI.btnPartenzaClick(Sender: TObject);
var
  Aus:string;
begin
  inherited;
  ...
  if not Aus.IsEmpty then
  begin
    CmbPartenza.Text:=Aus;
    A100FMISSIONIDTM.M040.FieldByName('DescPartenza').AsString:=A100FMISSIONIDTM.A100FMissioniMW.CodISTATtoCitta(Aus);
  end;
end;
...
begin
  ...
  with A100FMISSIONIDTM.A100FMissioniMW do
    mnuAggiornaSerAtt.Visible:=mnuAggiornaSerAtt.Visible and
                               (selM043.RecordCount > 0) and
                               (EsisteUSR_M043P_CARICA_GIUST);
end;
...
procedure TA100FMISSIONI.pmnAllegatiPopup(Sender: TObject);
var
  LAbilita: Boolean;
begin
  inherited;
  LAbilita:=(dgrdAllegati.DataSource.DataSet.Active) and
            (dgrdAllegati.DataSource.DataSet.RecordCount > 0);
  actFileVisualizza.Enabled:=LAbilita;
  actFileDownload.Enabled:=LAbilita;
end;
...
begin
  //inherited; // bugfix: spostato in fondo
