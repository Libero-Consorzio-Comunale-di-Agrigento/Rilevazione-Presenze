...
begin
  ...
  begin
    ...
    begin
      ...
      if (selTemp.FieldByName('NT1').AsString = '1') or
         (selTemp.FieldByName('NT2').AsString = '1') then
        Mattino.Lines.Add(S);
      if (selTemp.FieldByName('NT1').AsString = '2') or
         (selTemp.FieldByName('NT2').AsString = '2') then
        Pomeriggio.Lines.Add(S);
      if selTemp.FieldByName('NT1').AsString = '3' then
      begin
        if (selTemp.FieldByName('TURNO2').AsString = '0') or
           (selTemp.FieldByName('TURNO1EU').AsString = 'U') then
          Smonto.Lines.Add(S)
        else
          Notte.Lines.Add(S);
      end;
      if selTemp.FieldByName('NT2').AsString = '3' then
      begin
        if selTemp.FieldByName('TURNO2EU').AsString = 'U' then
          Smonto.Lines.Add(S)
        else
          Notte.Lines.Add(S);
      end;
