...
uses
  ...
  A000USessione, A000UInterfaccia, A000UCostanti, A004UGiustifAssPresMW,
  C180FunzioniGenerali, System.IOUtils, A000UMessaggi, QueryStorico,
  Generics.Collections, C021UDocumentiManagerDM, System.Types;
...
type
  ...
  TA087FImpAttestatiMalMW = class(TR005FDataModuleMW)
    ...
    selT269CAUS_INVALIDITA: TStringField;
    ...
    selT269FLAG_IGNORA_NUOVO_PERIODO: TStringField;
    ...
    procedure selT048InfoCalcFields(DataSet: TDataSet);
  private
    vetCertifAnomalie, vetCertifAnomalieNB: TStringList;
    vetCertifAcquisiti: TStringList;
    GestContinuazione:TA087InfoCertificati;
    C021DM:TC021FDocumentiManagerDM;
    ID_FileXML:integer;
    pBloccoAnomalia:Boolean; //Anomalia bloccante
    procedure PutBloccoAnomalia(Val:Boolean);
    procedure SortCertificati;
    procedure EstraiXML(NodoIN:IXMLNodeList;NPrev:String);
    procedure CreaAnomalia(TestoAnomalia:String;Prog:integer = 0;ID_Certificato:string = ''; pNonBloccante:Boolean = False);
    procedure InizializzaCDataSet(NodoIn:IXMLNodeList; NPrev:String);
    procedure PulisciR600;
    //Gestione salvataggio file XML - 06/2016
    procedure SalvaCertificatoXML(Path:string);
    procedure PulisciFileXMLSenzaCertficato;
    //Gestione salvataggio file XML
  ...
  public
    ...
    //Contatori Certificati
    TotaleCertificati, //Totale certificati presenti su file
    TotCodFiscEsclusi, //Totale cerficati dove il codice fiscale non è presente in anagrafica
    TotEsclusiSelAnag, //Totale certificati esclusi dalla selezione anagrafica
    TotElaborazioniOK, //Totale certificati elaborati andati a buon fine
    TotElaborazioniErr //Totale certificati elaborati con errore
    :integer;
    //=====================
    procedure InsDipT040;
    procedure InsDipT048;
    function ReportContatori:string;
    ...
    property BloccoAnomalia:Boolean read pBloccoAnomalia write PutBloccoAnomalia;
...
procedure TA087FImpAttestatiMalMW.PulisciFileXMLSenzaCertficato;
//Cancello file XML, memorizzati su tabella T960 - T961, senza alcun riferimento su tabella T048
//Nel caso in cui l'inserimento di tutti gli attestati presenti su file xml sia falliti
var
  CancellaXML: TOracleQuery;
  LResCtrl: TResCtrl;
begin
  CancellaXML:=TOracleQuery.Create(nil);
  try
    CancellaXML.Session:=SessioneOracle;
    CancellaXML.SQL.Add('select count(*) as NUMREC');
    CancellaXML.SQL.Add('  from T048_ATTESTATIINPS T048');
    CancellaXML.SQL.Add(' where T048.ID_FILEXML = :ID_FILEXML');
    CancellaXML.DeclareAndSet('ID_FILEXML',otInteger,ID_FileXML{TA087FImpAttestatiMalMW.Public});
    CancellaXML.Execute;
    if CancellaXML.FieldAsInteger('NUMREC') <= 0 then
    begin
      LResCtrl:=C021DM.Delete(ID_FileXML);
      if not LResCtrl.Ok then
      begin
        raise Exception.Create(Format('Errore nella cancellazione del BLOB(ID - %s).',[ID_FileXML]));
      end;
    end;
  finally
    FreeAndNil(CancellaXML);
  end;
end;

procedure TA087FImpAttestatiMalMW.SalvaCertificatoXML(Path:string);
// operazioni in fase di aggiunta del documento
var
  XMLDoc:TDocumento;
  LResCtrl: TResCtrl;
begin
  XMLDoc:=TDocumento.Create;
  try
    XMLDoc.SetNomeFileCompleto(TPath.GetFileNameWithoutExtension(path));
    XMLDoc.Tipologia:=DOC_TIPOL_INPS;
    XMLDoc.Ufficio:=DOC_UFFICIO_PREDEF;
    XMLDoc.PathStorage:='DB'; //Parametri.CampiRiferimento.C90_PathAllegati;

    // se PATH_STORAGE = 'DB' upload documento su database
    if XMLDoc.PathStorage = 'DB' then
    begin
      XMLDoc.Blob:=TLOBLocator.CreateTemporary(SessioneOracle,otBLOB,True);
      XMLDoc.Blob.Tag:=Integer(btFree); // indica che il blob è da distruggere dal distruttore di TDocumento
      XMLDoc.Blob.LoadFromFile(Path);
    end;

    XMLDoc.Note:={$IFDEF WEBPJ}'W' + {$ENDIF}'A087';
    XMLDoc.ExtFile:=TPath.GetExtension(Path);
    XMLDoc.Dimensione:=R180GetFileSize(Path);
    XMLDoc.TempFile:=Path;  //il path serve per gestire l'upload su filesystem

    LResCtrl:=C021DM.Save(XMLDoc);

    ID_FileXML:=XMLDoc.ID;  //!!!SALVO ID_FILEXML in variabile public!!!
  finally
    FreeAndNil(XMLDoc);
  end;
end;
...
procedure TA087FImpAttestatiMalMW.selT048InfoCalcFields(DataSet: TDataSet);
var
  LTipoCert: String;
  LDescTipoCert: String;
  LTipoRicovero: String;
  LDescTipoRicovero: String;
  LAgevolazioni: String;
  LDescAgevolazioni: String;
begin
  // tipo certificato
  LTipoCert:=selT048Info.FieldByName('TIPO_CERTIFICATO').AsString;
  if LTipoCert = 'I' then
    LDescTipoCert:='Inserimento'
  else if LTipoCert = 'C' then
    LDescTipoCert:='Continuazione'
  else if LTipoCert = 'R' then
    LDescTipoCert:='Ricaduta';
  selT048Info.FieldByName('D_TIPO_CERTIFICATO').AsString:=LDescTipoCert;

  // tipo ricovero
  LTipoRicovero:=selT048Info.FieldByName('TIPO_RICOVERO').AsString;
  if LTipoRicovero = 'R' then
    LDescTipoRicovero:='Ricovero'
  else if LTipoRicovero = 'H' then
    LDescTipoRicovero:='Day hospital'
  else if selT048Info.FieldByName('TIPO_ELEMENTO').AsString = 'D' then
    LDescTipoRicovero:='Post - Ricovero';
  LDescTipoRicovero:=selT048Info.FieldByName('D_TIPO_RICOVERO').AsString;

  // agevolazioni
  LAgevolazioni:=selT048Info.FieldByName('AGEVOLAZIONI').AsString;
  if LAgevolazioni = 'T' then
    LDescAgevolazioni:='Terapia salva vita'
  else if LAgevolazioni = 'C' then
    LDescAgevolazioni:='Causa di servizio'
  else if LAgevolazioni = 'I' then
    LDescAgevolazioni:='Invalidità riconosciuta';
  selT048Info.FieldByName('D_AGEVOLAZIONI').AsString:=LDescAgevolazioni;
end;
...
begin
  ...
  vetCertifAnomalieNB:=TStringList.Create;
  ...
  A004MW.Chiamante:='A087';

  C021DM:=TC021FDocumentiManagerDM.Create(nil);
  C021DM.CancellaFileTemp:=False;

  //Se il DB non è torino CSI noscondo le cauali
  selT269.FieldByName('CAUS_GRAVIDANZA').Visible:=Parametri.ModuloInstallato['TORINO_CSI_PRV'];
  selT269.FieldByName('CAUS_PATGRAVI').Visible:=Parametri.ModuloInstallato['TORINO_CSI_PRV'];
  selT269.FieldByName('STATO_CAUSA_MALATTIA').Visible:=Parametri.ModuloInstallato['TORINO_CSI_PRV'];
  selT269.Open;
  selT480.Open;
  //selT265.Open;
  //selT265_All.Open;
...
var
  ...
  begin
    try
      if Not(CDtsTXTFile.State in [dsInsert, dsEdit]) then
        CDtsTXTFile.Append;
      CDtsTXTFile.FieldByName('MATRICOLA').AsString:=Copy(InString,1,10);
      CDtsTXTFile.FieldByName('ANNO_INIZIO').AsString:=Copy(InString,11,4);
      CDtsTXTFile.FieldByName('MESE_INIZIO').AsString:=Copy(InString,15,2);
      CDtsTXTFile.FieldByName('GIORNO_INIZIO').AsString:=Copy(InString,17,2);
      CDtsTXTFile.FieldByName('ANNO_FINE').AsString:=Copy(InString,19,4);
      CDtsTXTFile.FieldByName('MESE_FINE').AsString:=Copy(InString,23,2);
      CDtsTXTFile.FieldByName('GIORNO_FINE').AsString:=Copy(InString,25,2);
      CDtsTXTFile.FieldByName('CAUSALE_MAL').AsString:=Copy(InString,27,4);
      CDtsTXTFile.FieldByName('NPROTOCOLLO').AsString:=Copy(InString,31,100);
      CDtsTXTFile.FieldByName('TIPO_FRUIZIONE').AsString:=Copy(InString,131,1);
      CDtsTXTFile.FieldByName('TIPO_REGISTRAZIONE').AsString:=Copy(InString,132,1);
      CDtsTXTFile.FieldByName('NOTE').AsString:=Copy(InString,133,512);
      CDtsTXTFile.FieldByName('ID_FILETXT').AsString:=Copy(InString,645,14);
      CDtsTXTFile.Post;
    except
      on E:exception do
...
begin
  ...
  FreeAndNil(vetCertifAnomalieNB);
...
begin
  ...
  if vetCertifAnomalie.IndexOf(ID_Certificato) <> -1 then
    Result:=clRed
  else if vetCertifAnomalieNB.IndexOf(ID_Certificato) <> -1 then
...
function TA087FImpAttestatiMalMW.ReportContatori:string;
begin
  Result:=Format('Riepilogo registrazione: Totale certificati[%d] - Dipendenti esclusi dalla selezione anagrafica o codici fiscali non trovati[%d] - Totale elaborati[%d] - Inserimenti falliti[%d]',
                 [TotaleCertificati,(TotCodFiscEsclusi + TotEsclusiSelAnag),TotElaborazioniOK,TotElaborazioniErr]);
end;

procedure TA087FImpAttestatiMalMW.PutBloccoAnomalia(Val:Boolean);
begin
  pBloccoAnomalia:=Val;
  if pBloccoAnomalia then
  begin
    inc(TotElaborazioniErr);
  end
  else
  begin
    TotElaborazioniErr:=0;
  end;
end;

procedure TA087FImpAttestatiMalMW.CreaAnomalia(TestoAnomalia:String;Prog:integer = 0;ID_Certificato:string = ''; pNonBloccante:boolean = False);
...
begin
  ...
  begin
    if pNonBloccante then
    begin
      RegistraMsg.InserisciMessaggio('A',Format(A000MSG_A087_MSG_ANOMALIA_NON_BLOCCANTE,[TestoAnomalia]),Parametri.Azienda,Prog);
      vetCertifAnomalieNB.Add(ID_Certificato);
    end
    else
    begin
      BloccoAnomalia:=True;
  ...
  begin
    //Incremento il contatore degli inserimenti falliti
    inc(TotElaborazioniErr);
...
begin
  Result:='N';
  if Not VarIsNull(BoolIn) and (VarToStr(BoolIn).Trim = '1') or (VarToStr(BoolIn).Trim.ToLower = 'true') then
  begin
    Result:='S';
  end;
end;
...
begin
  ...
  //Se il codice fiscale non è in anagrafica o il dipendente non è inservizio incremento il contatore "TotCodFiscEsclusi"
  if Result then
  begin
    inc(TotCodFiscEsclusi);
  end;
end;
...
begin
  try
    ...
    try
      ...
      CreaCampoAss('datafine_postricovero');
      ...
      VisCampo('datafine_postricovero','Fine post-ricovero',6);
      VisCampo('datarilascio_attestato','Data rilascio',7);
      VisCampo('PREVIEW_TIPOPERIODO','Tipo periodo',8);
      VisCampo('PREVIEW_DIFFDATE','Diff. rilascio e inizio mal.',9);
      VisCampo('idcertificato_attestato','ID certificato',10);
      VisCampo('cognome_reperibilita','Cognome rep.',11);
      VisCampo('via_indirizzo','Via rep.',12);
      VisCampo('civico_indirizzo','N.civico rep.',13);
      VisCampo('cap_indirizzo','CAP rep.',14);
      VisCampo('comune_indirizzo','Comune rep.',15);
      VisCampo('provincia_indirizzo','Provincia rep.',16);
      VisCampo('matricolainps','Matricola INPS',17);
      VisCampo('codsede','Codice sede',18);
      VisCampo('giornatalavorata','Giornata lavorata',19);
      VisCampo('trauma','Trauma',20);
      VisCampo('tiporicovero','Tipo ricovero',21);
      VisCampo('agevolazioni','Agevolazioni',22);
      ...
      TotaleCertificati:=0; //Azzero contatore certificati
      ...
      begin
        ...
        begin
          ...
          CDtsTemp.FieldByName('datafine_postricovero').AsDateTime:=ConvData(GetCampo('datafine_dimissioni'));
...
begin
  ...
  begin
    ElaboraDipendente(i);
    //Anomalia di Riepiloghi bloccati su T040
    if RegistraMsg.ContieneTipoB then
    begin
      RegistraMsg.ContieneTipoB:=False;
      CreaAnomalia(Format(A000MSG_DATIBLOCCATI_ERR_FMT_DATI_NONMODIF,['certificato ' + VetAttestato[i].ID_Certificato + ' del ' + selT048.FieldByName('DATA_INIZIO_MAL').AsString,'T040']),selT048.FieldByName('PROGRESSIVO').AsInteger,VetAttestato[i].ID_Certificato);
    end;
  ...
  begin
    {Se a file XML non corrisponde nessun certificato INPS
    in post elaborazione cancello recordo T960 - T961}
    PulisciFileXMLSenzaCertficato;
  end;
...
begin
  ...
  A004MW.R600DtM1.VisualizzaAnomalie:=False;
  A004MW.R600DtM1.AnomalieNonBloccanti:='1,26';
end;
...
var
  ...
  begin
    Result:=False;
    A004MW.Var_DaData:=DateToStr(DataIn);
    A004MW.Var_AData:=A004MW.Var_DaData;
    A004MW.Var_Causale:=Causale;
    A004MW.Var_Progressivo:=Prog;
    A004MW.Var_TipoGiust:=0;
    A004MW.Var_TipoGiust_Count:=4;
    A004MW.Q040.Close;
    A004MW.Q040.SetVariable('PROGRESSIVO',Prog);
    A004MW.Q040.Open;
    A004MW.Q265.SearchRecord('CODICE',A004MW.Var_Causale,[srFromBeginning]);
    ...
    try
      ...
      on E:exception do
      begin
        CreaAnomalia(LogMsg + ' ' + E.Message,A004MW.Var_Progressivo,VetAttestato[CurrDip].ID_Certificato);
        Result:=True;
        Exit;
      end;
    end;
    for k:=0 to A004MW.R600DtM1.ListAnomalie.Count - 1 do
    begin
      CreaAnomalia(A004MW.R600DtM1.ListAnomalie[k],A004MW.Var_Progressivo,VetAttestato[CurrDip].ID_Certificato);
      Result:=True;
      Exit;
    end;
    for k:=0 to A004MW.R600DtM1.ListAnomalieNonBloccanti.Count - 1 do
      RegistraMsg.InserisciMessaggio('A',Format(A000MSG_A087_MSG_ANOMALIA_NON_BLOCCANTE,[A004MW.R600DtM1.ListAnomalieNonBloccanti[k]]),Parametri.Azienda,A004MW.Var_Progressivo);
  end;
  ...
  begin
    ...
    try
      ...
      begin
        //Se giornata lavorata è uguale a "S" incremento data_inizio_mal di un giorno
        if selT048.FieldByName('GIORNATALAVORATA').AsString = 'S' then
        begin
          //Anomalia non bloccante
          vetCertifAcquisiti.Add(selT048.FieldByName('ID_CERTIFICATO').AsString);
          RegistraMsg.InserisciMessaggio('A', Format(A000MSG_A087_MSG_ANOMALIA_NON_BLOCCANTE,
                                                     [' Inizio malattia posticipato al giorno successivo in quanto dal certificato risulta attività lavorativa completata']),
                                                     Parametri.Azienda,
                                                     selT048.FieldByName('PROGRESSIVO').AsInteger);
          //CreaAnomalia(LogMsg + ' Inizio malattia posticipato al giorno successivo in quanto dal certificato risulta attività lavorativa completata',A004MW.Var_Progressivo,VetAttestato[CurrDip].ID_Certificato);
          DataInizioMalattia:=DataInizioMalattia + 1;
        end;
        if selT269.FieldByName('FLAG_IGNORA_NUOVO_PERIODO').AsString = 'N' then
          A004MW.TipoCertificatoINPS:='I'
        else
          A004MW.TipoCertificatoINPS:='C';
        {R180SetVariable(selT265DettCAssenza,'CODICE',selT269.FieldByName('CAUS_INSERIMENTO').AsString);
        selT265DettCAssenza.Open;}
        if (selT265DettCAssenza.FieldByName('TIPOCUMULO').AsString = 'O') and (selT269.FieldByName('FLAG_IGNORA_NUOVO_PERIODO').AsString = 'N') then
          A004MW.chkNuovoPeriodo:=True;
      end;
      ...
      A004MW.Var_DaData:=DateToStr(DataInizioMalattia);
      A004MW.Var_AData:=DateToStr(DataFineMalattia);
      A004MW.Var_Causale:=Causale;
      A004MW.Var_Progressivo:=selT048.FieldByName('PROGRESSIVO').AsInteger;

      A004MW.Giustif.Causale:=Causale;
      A004MW.Giustif.Inserimento:=True;
      A004MW.Giustif.Modo:='I';
      A004MW.Giustif.DaOre:='  .  ';
      A004MW.Giustif.AOre:='  .  ';

      A004MW.Q265.SearchRecord('CODICE',A004MW.Var_Causale,[srFromBeginning]);
      A004MW.DataInizio:=DataInizioMalattia;
      A004MW.DataFine:=DataFineMalattia;
      A004MW.DataInizioOrig:=selT048.FieldByName('DATA_INIZIO_MAL').AsDateTime;

      if A004MW.DataInizio > A004MW.DataFine then
      ...
      begin
        A004MW.R600DtM1.VisualizzaAnomalie:=True;
        A004MW.InserisciGiustif(False,IDInserimento);
      ...
      begin
        ...
        try
          ...
          //non propago l'eccezione se riguarda laviolazione di chiave della T047 visite fiscali
          on E:Exception do
            if (Pos('ORA-00001',E.Message.ToUpper) = 0) or (Pos('T047_PK',E.Message.ToUpper) = 0) then
              raise;
        end;
      end;
...
begin
  ...
  GetProfilo(selT048.FieldByName('PROGRESSIVO').AsInteger);
  ...
  A004MW.RecapitoAlternativo.Nominativo:=selT048.FieldByName('COGNOME_REP').AsString;
  if not selT048.FieldByName('CODCATASTALE_REP').IsNull then
  begin
    selT480.SearchRecord('CODCATASTALE',selT048.FieldByName('CODCATASTALE_REP').AsString,[srFromBeginning]);
    A004MW.RecapitoAlternativo.CodComune:=Self.selT480.FieldByName('CODICE').AsString;
    A004MW.RecapitoAlternativo.Indirizzo:=TrimRight(selT048.FieldByName('VIA_REP').AsString) + ' ' + selT048.FieldByName('CIVICO_REP').AsString;
    A004MW.RecapitoAlternativo.Cap:=selT048.FieldByName('CAP_REP').AsString;
    A004MW.RecapitoAlternativo.Telefono:='';
    A004MW.RecapitoAlternativo.MedLegale:='';
  end
  else
  begin
    selT480.SearchRecord('CODCATASTALE',selT048.FieldByName('CODCATASTALE_DOM').AsString,[srFromBeginning]);
    A004MW.RecapitoAlternativo.CodComune:=Self.selT480.FieldByName('CODICE').AsString;
    A004MW.RecapitoAlternativo.Indirizzo:=TrimRight(selT048.FieldByName('VIA_DOM').AsString) + ' ' + selT048.FieldByName('CIVICO_DOM').AsString;
    A004MW.RecapitoAlternativo.Cap:=selT048.FieldByName('CAP_DOM').AsString;
    A004MW.RecapitoAlternativo.Telefono:='';
    A004MW.RecapitoAlternativo.MedLegale:='';
  end;
  A004MW.Q040.Close;
  A004MW.Q040.SetVariable('PROGRESSIVO',selT048.FieldByName('PROGRESSIVO').AsInteger);
  A004MW.Q040.Open;
  A004MW.ListAnomalieCompleta.Clear;
  A004MW.R600DtM1.ListAnomalieNonBloccanti.Clear;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        GetPostRicovero.SetVariable('CAUSALI_RICOVERO',CatenaDimissioni + ',' + CatenaPostRic);
      ...
      if (A004MW.ListAnomalieCompleta.Count = 0) and (A004MW.R600DtM1.ListAnomalieNonBloccanti.Count = 0) then
        RegistraMsg.InserisciMessaggio('I',LogMsg + ' ' +  A000MSG_A087_MSG_PERIODO_INSERITO,Parametri.Azienda,A004MW.Var_Progressivo);
    end
    ...
    begin
      ...
      if not VetAttestato[CurrDip].Anomalia then
        RegistraMsg.InserisciMessaggio('I',LogMsg + A000MSG_A087_MSG_PERIODO_ANNUNLLATO,Parametri.Azienda,A004MW.Var_Progressivo);
      selT040.Close;
    end
    ...
    begin
      ...
      if selT040.RecordCount <= 0 then
        CreaAnomalia(LogMsg + ' ' +  Format(A000MSG_A087_MSG_CAUS_NON_TROVATE,[selT048.FieldByName('ID_CERTIFICATO').AsString]),
                     A004MW.Var_Progressivo, VetAttestato[CurrDip].ID_Certificato);
      ...
      for j:=0 to A004MW.R600DtM1.ListAnomalieNonBloccanti.Count - 1 do
        RegistraMsg.InserisciMessaggio('A',Format(A000MSG_A087_MSG_ANOMALIA_NON_BLOCCANTE,[A004MW.R600DtM1.ListAnomalieNonBloccanti[j]]),Parametri.Azienda,A004MW.Var_Progressivo);
      if (A004MW.ListAnomalieCompleta.Count = 0) and (A004MW.R600DtM1.ListAnomalieNonBloccanti.Count = 0) then
        RegistraMsg.InserisciMessaggio('I',LogMsg + A000MSG_A087_MSG_GIUSTIF_RETT,Parametri.Azienda,A004MW.Var_Progressivo);
      selT040.Close;
    end
    ...
    begin
      ...
      for j:=0 to A004MW.R600DtM1.ListAnomalieNonBloccanti.Count - 1 do
        RegistraMsg.InserisciMessaggio('A', Format(A000MSG_A087_MSG_ANOMALIA_NON_BLOCCANTE,[A004MW.R600DtM1.ListAnomalieNonBloccanti[j]]),Parametri.Azienda,A004MW.Var_Progressivo);
      if (A004MW.ListAnomalieCompleta.Count = 0) and (A004MW.R600DtM1.ListAnomalieNonBloccanti.Count = 0) then
        RegistraMsg.InserisciMessaggio('I',Format(A000MSG_A087_MSG_GIUSTIF_INSERITO,[LogMsg]),Parametri.Azienda,A004MW.Var_Progressivo);
    end
  ...
  if not VetAttestato[CurrDip].Anomalia then
  begin
    //Incremento il contatore dell'elaborazioni andate a buon fine
    inc(TotElaborazioniOK);
    //Se non si sono verificate anomalie durante l'elaborazione:
    //Imposto flag elaborato = S
  ...
  begin
    ...
    with A004MW.B021FWebSvcClientDtM do
    begin
      if EsisteRecordT040 then
      begin
        WebSvcRecordT040;
      end;
    end;
...
var
  ...
  begin
    ...
    try
      ...
      if VarToStr(GetCampo('AGEVOLAZIONI_DIMISSIONI')) = 'T' then
        CreaAnomalia(A000MSG_A087_ERR_SALVAVITA, selT030.FieldByName('PROGRESSIVO').AsInteger,'',True);
...
begin
  ...
  vetCertifAnomalieNB.Clear;
  vetCertifAcquisiti.Clear;
  SetLength(VetAttestato,0);
  {inserisco Blob file xml SOLO se sono in inserimento certificati}
  if evtInserimentoChecked then
    SalvaCertificatoXML(evtGetPathFile);
  while Not CDtsTemp.Eof do
  begin
    ...
    InsT048.SetVariable('ID_FILEXML',ID_FileXML);
    if (UpperCase(CDtsTemp.FieldByName('TIPO_OPERAZIONE').AsString) = 'ATTESTATO') and
    ...
    begin
      ...
      try
        ...
        end //Invalidità riconosciuta
        else if (VarToStr(GetCampo('AGEVOLAZIONI_ATTESTATO')) = 'I') and (not selT269.FieldByName('CAUS_INVALIDITA').IsNull) then
        begin
          InsT048.SetVariable('CAUSALE_MAL',selT269.FieldByName('CAUS_INVALIDITA').AsString)
        end
        else
        begin
