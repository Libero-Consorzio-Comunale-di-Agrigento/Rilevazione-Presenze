...
begin
  ...
  if R502ProDtM1.blocca <> 0 then
    Result:='Anomalia bloccante: ' + R502ProDtM1.DescAnomaliaBloccante;
end;
...
begin
  ...
  begin
    ...
    if cdsT020.Active then AddDato('XParam comp.vers.prec.',cdsT020.FieldByName('XPARAM_COMP_OLDVERS').AsString)
    else                   AddDato('XParam comp.vers.prec.','');
    ...
    AddDato('*** TURNI REPERIBILITA''/GUARDIA/PREST.AGG. ***','');
    ...
    AddDato('Prestazioni aggiuntive',TurniExtraPianificatiDalleAlle['G']);
    ...
    AddDato('Straord. richiedibile',R180MinutiOre(EccedIterDisponibile));
    AddDato('Abbattimento straord.liq.',R180MinutiOre(minabbstr));
    AddDato('Detrazioni per arrotondamento straord.',IntToStr(prpStrarrdet));
