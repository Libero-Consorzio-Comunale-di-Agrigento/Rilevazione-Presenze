inherited A099FUtilityDBMW: TA099FUtilityDBMW
  ...
  object selLogMsg: TOracleDataSet
    SQL.Strings = (
        ...
        'select substr('#39'I000_LOGINFO'#39',1,30) tabella,count(*) from i000_lo' +
        'ginfo'
      'union '
      
        'select substr('#39'I005_MSGINFO'#39',1,30),count(*) from mondoedp.i005_m' +
        'sginfo'
      'union '
      
        'select substr('#39'I005_MSGINFO_'#39'||:azienda,1,30),count(*) from mond' +
        'oedp.i005_msginfo where azienda = :azienda')
    ReadBuffer = 5
    Optimize = False
    Variables.Data = {
  ...
  object selDataFiles: TOracleDataSet
    SQL.Strings = (
      'select * from DBA_DATA_FILES '
      'order by TABLESPACE_NAME,FILE_ID,FILE_NAME')
    ReadBuffer = 5
    Optimize = False
    AfterOpen = selDataFilesAfterOpen
    Left = 472
    Top = 8
  end
  object selExpAllTab: TOracleDataSet
    SQL.Strings = (
      'select T.TABLE_NAME'
      '  from MEDPV_EXPTAB_DDL T'
      '  where 1 = 1'
      '       :FILTRO'
  ...
  object selMEDPV_EXPTAB_DATI: TOracleDataSet
    SQL.Strings = (
      'select T.TABLE_NAME '
      '  from MEDPV_EXPTAB_DATI T'
      ' order by T.TABLE_NAME')
    Optimize = False
    Left = 432
    Top = 64
