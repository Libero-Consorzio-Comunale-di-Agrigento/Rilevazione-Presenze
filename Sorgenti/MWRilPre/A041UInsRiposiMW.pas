...
uses
  ...
  RP502Pro, R600, DB, C180FunzioniGenerali, Math, Oracle;
...
type
  ...
  private
    ...
    function PreparaInserimento(Tipo: String; DataMese: TDateTime; pRielab: Boolean): Boolean;
    ...
    procedure SetRiposiNeg(Prog: Integer; DataI: TDateTime);
...
begin
  ...
  (R400 as TR400FCartellinoDtM).A027SelAnagrafe:=SelAnagrafe;
  (R400 as TR400FCartellinoDtM).SoloAggiornamento:=True;
...
begin
  ...
  begin
    ...
    begin
      ...
      try
        (R400 as TR400FCartellinoDtM).CartolinaDipendente(SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger,R180Anno(DataCorr),R180Mese(DataCorr),1,R180GiorniMese(DataCorr));
      except
        on E: Exception do
        begin
      ...
      if FALSE and ((R400 as TR400FCartellinoDtM).R450DtM1.salannoatt < 0) and (selT267.FieldByName('RIPOSO_COMPENSATIVO_SALDONEG').AsString <> '') then
        NRipDaEliminare:=SalvaRiposi.Count
      else if ((R400 as TR400FCartellinoDtM).R502ProDtM1.giornlav = 0) or ((R400 as TR400FCartellinoDtM).R502ProDtM1.minmonteore = 0) then
        NRipDaEliminare:=0
      else
      begin
        if selT267.FieldByName('RIPOSO_COMPENSATIVO_SALDONEG').AsString = '' then
          NRipDaEliminare:=-Trunc(Min(0,(R400 as TR400FCartellinoDtM).R450DtM1.salannoatt)/((R400 as TR400FCartellinoDtM).R502ProDtM1.minmonteore/(R400 as TR400FCartellinoDtM).R502ProDtM1.giornlav))
        else
          NRipDaEliminare:=Trunc(R180Arrotonda(-1*(Min(0,(R400 as TR400FCartellinoDtM).R450DtM1.salannoatt)/((R400 as TR400FCartellinoDtM).R502ProDtM1.minmonteore/(R400 as TR400FCartellinoDtM).R502ProDtM1.giornlav)),1,'E'));
      end;

      for i:=SalvaRiposi.Count - 1 downto 0 do
        if (NRipDaEliminare > 0) and (R180Mese(StrToDateTime(SalvaRiposi[i])) = R180Mese(DataCorr)) then
        begin
          SetRiposiNeg(SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger,StrToDateTime(SalvaRiposi[i]));
...
function TA041FInsRiposiMW.PreparaInserimento(Tipo:String;(*InizioMese*)DataMese:TDateTime; pRielab: Boolean): Boolean;
var Riga,CodInt:String;
    PrioritaFestivi: Boolean;
    Prosegui:Boolean;
begin
  //Recupero:=0;
  Result:=True;
  ...
  PrioritaFestivi:=selT267.FieldByName('PRIORITA_FESTIVI').AsString = 'S';

  if Tipo <> 'R' then
    Prosegui:=True
  else
  begin
    if PrioritaFestivi and (not pRielab) then   //PRIORITA_FESTIVI e prima elaborazione -> elabora solo i giorni festivi
      Prosegui:=GiornoFestivo(DataMese)
    else if PrioritaFestivi and pRielab then    //PRIORITA_FESTIVI e seconda elaborazione -> elabora solo i giorni NON festivi
      Prosegui:=not GiornoFestivo(DataMese)
    else if (not PrioritaFestivi) and (not pRielab) then //NO PRIORITA_FESTIVI e prima elaborazione -> elabora indistintamente il giorno
      Prosegui:=True
    else
      Prosegui:=False;
  end;
  ...
  begin
    Esegui:=False;
    if (selT267.FieldByName('LIMITE_SALDO').AsString = 'S') and (not pRielab) then
      SalvaRiposi.Clear;
    ...
    if (selT267.FieldByName('CANCELLAZIONE_CAUSALE').AsString = 'S') and (not pRielab) then
    ...
    if selT267.FindField('GIUST_NONELABORATI') <> nil then
      QGiustificativi.SetVariable('RICHIESTEITERAUTORIZZATIVO', selT267.FieldByName('GIUST_NONELABORATI').AsString)
    else
      QGiustificativi.SetVariable('RICHIESTEITERAUTORIZZATIVO','S');
...
procedure TA041FInsRiposiMW.SetRiposiNeg(Prog: Integer; DataI: TDateTime);
{Cambia le causali dei giorni di riposo esistenti sulla T040Giustificativi con le causali per i casi di saldo negativo.}
begin
  if selT267.FieldByName('RIPOSO_COMPENSATIVO_SALDONEG').AsString <> '' then
  begin
    updT040.SetVariable('Progressivo',Prog);
    updT040.SetVariable('DataI',DataI);
    updT040.SetVariable('CAUSALE',selT267.FieldByName('RIPOSO_COMPENSATIVO').AsString);
    updT040.SetVariable('CAUSALENEG',selT267.FieldByName('RIPOSO_COMPENSATIVO_SALDONEG').AsString);
    updT040.Execute;
    SessioneOracle.Commit;
  end
  else
    PulRiposi(Prog,DataI,DataI);
end;
...
begin
  ...
  delT040.SetVariable('CausaleNeg',selT267.FieldByName('RIPOSO_COMPENSATIVO_SALDONEG').AsString);
  try
    delT040.Execute;
    SessioneOracle.Commit;
  except
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Causale:=selT267.FieldByName('RIPOSO_ORDINARIO').AsString;
        Riga:='ATTENZIONE: Nel giorno ' + DateToStr(DataCorr) + ' sono state esaurite le competenze della causale ' + Causale;
        RegistraMsg.InserisciMessaggio('A',Riga,'',SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger);
      end
      ...
      else if (selT267.FieldByName('RIPOSO_COMPENSATIVO').AsString = '') and (CompTot < 0) then
      begin
        Riga:='ATTENZIONE: Nel giorno ' + DateToStr(DataCorr) + ' sono state esaurite le competenze della causale ' + Causale;
        RegistraMsg.InserisciMessaggio('A',Riga,'',SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger);
      end;
  ...
  begin
    ...
    begin
      ...
      begin
        Riga:='ATTENZIONE Inserimento ' + Giustif.Causale + ' fallito sul giorno ' + DateToStr(DataCorr);
        RegistraMsg.InserisciMessaggio('A',Riga,'',SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger);
...
begin
  ...
  try
    ...
    if Q265.SearchRecord('CODICE',QGiustificativi.FieldByName('Causale').AsString,[srFromBeginning]) then //Giust.assenza
    begin
      //Se Causale che diminuisce le ore, la ignoro
      if (VarToStr(Q265.Lookup('Codice',QGiustificativi.FieldByName('Causale').AsString,'INFLUCONT')) = 'D') and
         R180In(QGiustificativi.FieldByName('TipoGiust').AsString,['N','D']) then
      begin
        Continue;
      end;
...
begin
  ...
  begin
    if PreparaInserimento('I',DataCorr,False) then
  ...
  begin
    ...
    //Se è impostata la priorità sui festivi elabora solo per i festivi, altrimenti tutti i giorni
    if PreparaInserimento('R',DataCorr,False) then
      Inserimento('R',DataCorr);
    DataCorr:=DataCorr+1;

    //Se è impostata la priorità sui festivi ripete il ciclo per i giorni lavorativi al cambio mese
    if (R180FineMese(DataCorr) <> OldFine) and
       (selT267.FieldByName('PRIORITA_FESTIVI').AsString = 'S') then
    begin
      DataCorr:=R180InizioMese(OldFine);
      while DataCorr <= OldFine do
      begin
        if PreparaInserimento('R',DataCorr,True) then
          Inserimento('R',DataCorr);
        DataCorr:=DataCorr+1;
      end;
    end;
  ...
  while DataCorr <= DataFine do
  begin
    if PreparaInserimento('F',DataCorr,False) then
