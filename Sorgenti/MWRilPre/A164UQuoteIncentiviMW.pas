...
uses
  ...
  Data.DB, C180FunzioniGenerali, Oracle, Variants, A000UMessaggi, A000UInterfaccia, A000UCostanti;
...
type
  ...
  TA164FQuoteIncentiviMW = class(TR005FDataModuleMW)
    ...
    selT765RIF_SALDO_ANNO_CORR: TStringField;
    selT450: TOracleDataSet;
    selEsclusione: TOracleDataSet;
  ...
  public
    ...
    function getRifSaldoAnnoCorrByCod(Cod: String): Boolean;
    ...
    function ListTipiRapporto: TElencoValoriChecklist;
    function ListEsclusioneValori: TElencoValoriChecklist;
  end;
...
begin
  if (VarToStr(selT765.Lookup('CODICE',FselT770_Funzioni.FieldByName('CODTIPOQUOTA').AsString,'TIPOQUOTA')) = 'Q')
  and (R180OreMinutiExt(FselT770_Funzioni.FieldByName('NUM_ORE').AsString) <> 0) then
    ...
    FselT770_Funzioni.FieldByName('TOTNETTO').AsFloat:=R180Arrotonda(
      FselT770_Funzioni.FieldByName('IMPORTO').AsFloat *
      FselT770_Funzioni.FieldByName('VALUT_STRUTTURALE').AsFloat / 100 ,0.00001,'P');
end;
...
function TA164FQuoteIncentiviMW.getRifSaldoAnnoCorrByCod(Cod: String): Boolean;
begin
  Result:=False;
  if selT765.SearchRecord('CODICE',Cod,[srFromBeginning]) then
    Result:=(selT765.FieldByName('TIPOQUOTA').AsString = 'I') and (selT765.FieldByName('RIF_SALDO_ANNO_CORR').AsString = 'S');
end;
...
function TA164FQuoteIncentiviMW.ListTipiRapporto: TElencoValoriChecklist;
var
  codice: String;
begin
...
function TA164FQuoteIncentiviMW.ListEsclusioneValori: TElencoValoriChecklist;
var
  codice: String;
...
begin
  ...
  with selEsclusione do
  begin
    Close;
    SQL.Clear;
    DeleteVariables;
    A000GetTabella(Parametri.CampiRiferimento.C7_EscludeIncentivo,T,C,Storico);
    if T = 'T480_COMUNI' then
      ...
      D:='DESCRIZIONE';
    Sql.Add(Format('SELECT DISTINCT %s CODICE, %s DESCRIZIONE FROM %s',[C,D,T]));
    if Storico = 'S' then
    begin
      SQL.Add(Format('WHERE DECORRENZA = (SELECT MAX(DECORRENZA) FROM %s WHERE %s = T1.%s AND DECORRENZA <= :DECORRENZA)',[T,C,C]));
      DeclareVariable('DECORRENZA',otDate);
      SetVariable('DECORRENZA',Parametri.DataLavoro);
    end;
    Sql.Add(Format('ORDER BY %s',[C]));
    Open;
    First;
    LungCodEscl:=R180Lunghezzacampo(Fields[0]);
    while not Eof do
    begin
      codice:=FieldByName('Codice').AsString;
      Result.lstCodice.Add(codice);
      Result.lstDescrizione.Add(Format('%-' + IntToStr(LungCodEscl) + 's %s',[codice, FieldByName('Descrizione').AsString]));
      Next;
    end;
  end;
end;
