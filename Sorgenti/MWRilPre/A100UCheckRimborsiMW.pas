...
uses
  ...
  A000UInterfaccia, C180FunzioniGenerali, System.StrUtils, System.Variants, Oracle,
  Datasnap.DBClient;
...
begin
  ...
  // importo
  ImportoVariatoM051:=(selM051.State = dsEdit) and (selM051.FieldByName('IMPORTO').Value <> selM051.FieldByName('IMPORTO').OldValue);
  if (ImportoVariatoM051) and
     (selM051.FieldByName('IMPORTO_ORIGINALE').IsNull) then
  begin
    selM051.FieldByName('IMPORTO_ORIGINALE').AsVariant:=selM051.FieldByName('IMPORTO').OldValue;
  end;
end;
