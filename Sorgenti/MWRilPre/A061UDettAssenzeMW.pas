...
uses
  ...
  R600, Rp502Pro, USelI010, A000UCostanti, A000USessione, A000UInterfaccia, A000UMessaggi,
...
type
  ...
  private
    ...
    function  CheckFiltroCausali(ElencoCausali:String):Boolean;
  public
    selI010:TselI010;
    ...
    StampaNominativo, StampaMatricola, StampaBadge, StampaDatiAnagrafici: Boolean;
    AssenzeConsiderateItemIndex, ValidateItemIndex, OrdinamentoItemIndex: Integer;
    ...
    StructDatiAnagrafici:TList<TItemsValues>;
    ValuesDatiAnagrafici:TList<TItemsValues>;
    procedure ControlliBloccanti(sElencoCausali:String;bDatiIndividuali,bTotGenerali:Boolean);
...
begin
  ...
  StructDatiAnagrafici:=TList<TItemsValues>.Create;
  ValuesDatiAnagrafici:=TList<TItemsValues>.Create;
end;
...
begin
  ...
  FreeAndNil(StructDatiAnagrafici);
  FreeAndNil(ValuesDatiAnagrafici);
  FreeAndNil(selI010);
  FreeAndNil(R600DtM1);
...
procedure TA061FDettAssenzaMW.ControlliBloccanti(sElencoCausali:String;bDatiIndividuali,bTotGenerali:Boolean);
begin
  if sElencoCausali.IsEmpty then
    raise Exception.Create(A000MSG_A061_ERR_CAUSALE);
  if (not bDatiIndividuali) and (not bTotGenerali) then
...
begin
  ...
  if StampaDatiAnagrafici then
  begin
    for i:=0 to StructDatiAnagrafici.Count - 1 do
      TabellaStampa.FieldDefs.Add('DA_' + StructDatiAnagrafici[i].Item,ftString,60,False);
    TabellaStampa.FieldDefs.Add('DatiAnagrafici',ftString,1000,False);
  end;
  ...
  begin
    for i:=0 to StructDatiAnagrafici.Count - 1 do
    begin
      TabellaStampa.FieldByName('DA_' + StructDatiAnagrafici[i].Item).DisplayWidth:=20;
      TabellaStampa.FieldByName('DA_' + StructDatiAnagrafici[i].Item).DisplayLabel:=R180Capitalize(StructDatiAnagrafici[i].Value);
    end;
    TabellaStampa.FieldByName('DATIANAGRAFICI').Visible:=False;
  end;
...
begin
  ...
  begin
    ...
    R600DtM1.Progressivo:=ProgressivoC700;
    R600DtM1.GetIDFamiliare;
  ...
  if StampaDatiAnagrafici then
  begin
    TabellaStampa.FieldByName('DatiAnagrafici').Value:=Copy(DatiAnagrafici,1,1000);
    for i:=0 to StructDatiAnagrafici.Count - 1 do
      TabellaStampa.FieldByName('DA_' + StructDatiAnagrafici[i].Item).Value:=ValuesDatiAnagrafici[i].Value;
  end;
  if NumFamiliari <> 0 then
  ...
  begin
    ...
    begin
      ...
      if QGiustificativiAssenza.FieldByName('TipoGiust').AsString = 'M' then
        ...
        CumuloGiorni:=CumuloGiorni+QGiustificativiAssenza.FieldByName('TOT_QUANT').AsFloat;
...
function TA061FDettAssenzaMW.CheckFiltroCausali(ElencoCausali:String):Boolean;
{restituisce True se l'elenco causali non corrisponde all'insieme totale delle causali disponibili sulla T265}
var S:String;
begin
  Result:=True;
  with selT265Codice do
  try
    Open;
    First;
    while not Eof  do
    begin
      S:=S + IfThen(S <> '',',') + '''' + FieldByName('CODICE').AsString + '''';
      Next;
    end;
  finally
    Close;
  end;
  if Length(R180GetCsvIntersect(S,ElencoCausali)) = Length(S) then
    Result:=False;
end;

procedure TA061FDettAssenzaMW.CreaSqlGiustificativi(ElencoCausali:String);
var StrSQL: String;
    FiltroCausali:Boolean;
begin
  //Verifica se è necessario applicare il fitro sulle causali o sono state selezionate tutte
  FiltroCausali:=CheckFiltroCausali(ElencoCausali);
  ...
  begin
    ...
    begin
      ...
      if CausaliCumulateChecked then //LORENA 30/03/2005
        SQL.Add('''Ins.'' OPERAZIONE,DATA,''*''||TIPOGIUST CAUSALE,-1 PROGRCAUSALE,TO_DATE(''01011900'',''DDMMYYYY'') DATANAS,TIPOGIUST,TO_CHAR(DAORE,''SSSSS'')/60 DAORE,TO_CHAR(AORE,''SSSSS'')/60 AORE,'''' CONIUGE') //LORENA 30/03/2005
      else
      ...
      begin
        ...
        SQL.Add('SELECT /*+ INDEX(T040_GIUSTIFICATIVI T040_DATANAS)*/');
        if CausaliCumulateChecked then //LORENA 30/03/2005
          SQL.Add('''Ins.'' OPERAZIONE,T040.DATA,''*''||TIPOGIUST CAUSALE,-1 PROGRCAUSALE,TO_DATE(''01011900'',''DDMMYYYY'') DATANAS,T040.TIPOGIUST,to_char(T040.DAORE,''SSSSS'')/60 DAORE,to_char(T040.AORE,''SSSSS'')/60 AORE, ''I'' CONIUGE')//LORENA 30/03/2005
        else
        ...
        SQL.Add('SG101.GRADOPAR in (''CG'',''AL'',''UC'',''CF'') AND');
        ...
        SQL.Add('SELECT /*+ INDEX(T040_GIUSTIFICATIVI T040_DATANAS)*/');
        if CausaliCumulateChecked then //LORENA 30/03/2005
          SQL.Add('''Ins.'' OPERAZIONE,T046.DATA,''*''||TIPOGIUST CAUSALE,-1 PROGRCAUSALE,TO_DATE(''01011900'',''DDMMYYYY'') DATANAS,T046.TIPOGIUST,OREMINUTI(T046.DAORE) DAORE,OREMINUTI(T046.AORE) AORE,''E'' CONIUGE')//LORENA 30/03/2005
        else
    ...
    begin
      ...
      SQL.Add('              ''N'',OREMINUTI(TO_CHAR(DAORE,''HH24.MI'')),''M'',0.5,1) QUANTITA FROM ');
      SQL.Add('  (');
      ...
      if FiltroCausali then
        ...
        SQL.Add('  ,T265_CAUASSENZE WHERE 1=1 ');
