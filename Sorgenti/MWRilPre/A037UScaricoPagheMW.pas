...
uses
  ...
  C180FunzioniGenerali, QueryStorico, Rp502Pro, R500Lin, R450, R600, USelI010;
...
type
  ...
  TA037FScaricoPagheMW = class(TR005FDataModuleMW)
    ...
    selM050CapArt: TOracleDataSet;
    selM052CapArt: TOracleDataSet;
...
//Caratto Obsoleto    procedure EliminaSingolaVoce(P, I: Integer; DataCassa: TDateTime);
    ...
    function EsisteElemento(ACorr, MCorr, AnnoDal, MeseDal:Integer; CodPaghe,Misura,Capitolo,Articolo:String; Range:Integer): Integer;
    procedure SalvaValoreGenerico(CI,CodPaghe:String; Valore:Currency; Misura:String; AnnoDal,MeseDal:Word; DataCassa:TDateTime); overload;
    procedure SalvaValoreGenerico(CI,CodPaghe:String; Valore:Currency; Misura:String; AnnoDal,MeseDal:Word; DataCassa:TDateTime; Capitolo:String; Articolo:String); overload;
    ...
    procedure GestisciDatiAtipici(DataCassa: TDateTime);
  public
    ...
    function ApriFileSequenziale(NomeFile:String;Aggiungi:Boolean; var F: TextFile): String;
    function VerificaTabella(Nome:String; Ricrea:Boolean): String;
    function ImpostaTabella(Nome: String;Ricrea: Boolean): String;
...
function TA037FScaricoPagheMW.ApriFileSequenziale(NomeFile:String;Aggiungi:Boolean; var F: TextFile): String;
begin
  Result:='';//True;
...
begin
  ...
  with OperSQL do
  try
    SQL.Text:='select MEDPF_LONG2CHAR(''T195_VOCIVARIABILI'',''CAPITOLO'') DEFAULT_VALUE from dual';
    try
      Open;
      Def_Capitolo:=FieldByName('DEFAULT_VALUE').AsString;
      Def_Articolo:=Def_Capitolo;
    except
    end;
  finally
    Close;
...
function TA037FScaricoPagheMW.ImpostaTabella(Nome:String; Ricrea: Boolean):String;
...
begin
  ...
  //Dati atipici
  if selT190.SearchRecord('CODINTERNO;FLAG',VarArrayOf(['450','S']),[srFromBeginning]) then
  begin
    try
      selT077.Close;
      selT077.SetVariable('DATA1',DataIndietro);
      selT077.SetVAriable('DATA2',R180FineMese(Data));
      selT077.SetVariable('PROGRESSIVO',Prog);
      selT077.Open;
    except
      Result:=Result + 'selT077';
    end;
  end;
end;
...
begin
  ...
  if selT195.SearchRecord('DataRif;Dal;VocePaghe;Capitolo;Articolo',VarArrayOf([VettCI[I].Riferimento,VettCI[I].Dal,VettCI[I].CodicePaghe,VettCI[I].Capitolo,VettCI[I].Articolo]),[srFromBeginning]) then
    repeat
      ValorePrec:=ValorePrec + selT195.FieldByName('Valore').AsFloat;
      ImportoPrec:=ImportoPrec + selT195.FieldByName('Importo').AsFloat;
    until not selT195.SearchRecord('DataRif;Dal;VocePaghe;Capitolo;Articolo',VarArrayOf([VettCI[I].Riferimento,VettCI[I].Dal,VettCI[I].CodicePaghe,VettCI[I].Capitolo,VettCI[I].Articolo]),[]);
  ...
  begin
    ...
    Ins195.SetVariable('Cod_Interno',VettCI[I].CodiceInterno);
    Ins195.SetVariable('Data_Cassa',DataCassa);
    Ins195.SetVariable('Capitolo',VettCI[I].Capitolo);
    Ins195.SetVariable('Articolo',VettCI[I].Articolo);
    try
      Ins195.Execute;
    except
      on E:Exception do
...
begin
  ...
  begin
    ...
    VettCI[I].Capitolo:=Def_Capitolo;
    VettCI[I].Articolo:=Def_Articolo;
  end;
end;

function TA037FScaricoPagheMW.EsisteElemento(ACorr,MCorr,AnnoDal,MeseDal:Integer; CodPaghe,Misura,Capitolo,Articolo:String; Range:Integer):Integer;
var I:Integer;
begin
  ...
  for I:=1 to Range do
    ...
    if (VettCI[I].Anno = ACorr) and (VettCI[I].Mese = MCorr) and
       ...
       (VettCI[I].CodicePaghe = CodPaghe) and (VettCI[I].MisuraInterna = Misura) and
       (VettCI[I].Capitolo = Capitolo) and (VettCI[I].Articolo = Articolo)
    then
    begin
      Result:=I;
      Break;
...
begin
  SalvaValoreGenerico(CI,CodPaghe,Valore,Misura,AnnoDal,MeseDal,DataCassa,Def_Capitolo,Def_Articolo);
end;

procedure TA037FScaricoPagheMW.SalvaValoreGenerico(CI,CodPaghe:String; Valore:Currency; Misura:String; AnnoDal,MeseDal:Word; DataCassa:TDateTime; Capitolo:String; Articolo:String);
var
  App:Integer;
  Arr:Real;
  ValPers:Currency;
...
begin
  ...
  if Trim(CodPaghe) = '' then
    exit;
  //Se è previsto di invertire importo con quantità cambio l'unità di misura
  if selT193.FieldByName('SPOSTA_VALIMP').AsString = 'S' then
  begin
    if (Misura = 'H') or (Misura = 'N') then
      Misura:='V'
    else
      if Misura = 'V' then
        Misura:='N'
  ...
  App:=EsisteElemento(ACorr, MCorr, AnnoDal,MeseDal,CodPaghe,Misura,Capitolo,Articolo,NumRecord);
  ...
  begin
    ...
    VettCI[NumRecord].Capitolo:=Capitolo;
    VettCI[NumRecord].Articolo:=Articolo;
    App:=NumRecord;
  end;
      ...
      begin
        for i:=1 to NumRecord do
          if (VettCI[i].CodicePaghe = FieldByName('VocePaghe').AsString) and
             ...
             (R180InizioMese(VettCI[i].Dal) = R180Iniziomese(FieldByName('Dal').AsDateTime)) and
             (VettCI[i].Capitolo = FieldByName('CAPITOLO').AsString) and
             (VettCI[i].Articolo = FieldByName('ARTICOLO').AsString)
          then
          begin
            Esiste:=True;
            Break;
        ...
        begin
          ...
          begin
            ...
            //Alberto 02/05/2017
            VettCI[NumRecord].Capitolo:=FieldByName('CAPITOLO').AsString;
            VettCI[NumRecord].Articolo:=FieldByName('ARTICOLO').AsString;
          end;
        end;
      end;
      Next;
...
begin
  ...
  begin
    for xx:=Low(m_tab_giustificativi) to High(m_tab_giustificativi) do
      for yy:=1 to MaxGiustif do
        m_tab_giustificativi[xx,yy]:=tab_giustificativi_vuoto;
    ...
    DataCon:=EncodeDate(ACorr,MCorr,1);
    FlagAsse1:=0;
    FlagAsse2:=0;
    n_rpassemes:=0;
    //Controllo se ci sono dei giustificativi
    z916_startGiust;
    if R180InizioMese(Q040.FieldByName('DATA').AsDateTime) <> EncodeDate(ACorr,MCorr,1) then
      s_trovato:='no';
    D:=0;
    while s_trovato = 'si' do
    begin
      if D <> Q040.FieldByName('DATA').AsDateTime then
      begin
        D:=Q040.FieldByName('DATA').AsDateTime;
        BM:=Q040.GetBookmark;
	    	try
          //if A037FScaricoPaghedtM1.Q265.SearchRecord('CODICE',Q040Causale.AsString,[srFromBeginning]) then
          begin
            ConteggiAssenze;
            if FlagAsse1 = 2 then
              Break;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  begin
                    Q040.GotoBookmark(BM);
                    MalattiaCSI[1].VocePaghe:=VarToStr(selT265.Lookup('CODICE',Q040.FieldByName('CAUSALE').AsString,'VOCEPAGHE2'));
                    MalattiaCSI[2].VocePaghe:=VarToStr(selT265.Lookup('CODICE',Q040.FieldByName('CAUSALE').AsString,'VOCEPAGHE3'));
                    MalattiaCSI[3].VocePaghe:=VarToStr(selT265.Lookup('CODICE',Q040.FieldByName('CAUSALE').AsString,'VOCEPAGHE4'));
                    MalattiaCSI[4].VocePaghe:=VarToStr(selT265.Lookup('CODICE',Q040.FieldByName('CAUSALE').AsString,'VOCEPAGHE5'));
                    MalattiaCSI[5].VocePaghe:=VarToStr(selT265.Lookup('CODICE',Q040.FieldByName('CAUSALE').AsString,'VOCEPAGHE6'));
                  end;
  ...
  begin
    ...
    begin
      ...
      begin
        if MalattiaCSI[1].VocePaghe = VarToStr(selT265.Lookup('CODICE',AssenzeMese[i].Causale,'VOCEPAGHE2')) then
        begin
          bRegEvRid:=False;
          for xx:=Low(MalattiaCSI) to High(MalattiaCSI) do
          begin
...
procedure TA037FScaricoPagheMW.ConteggiAssenze;
var i:Byte;
begin
  with R502ProDtM1 do
  begin
    Conteggi('Cartolina',selAnagrafe.FieldByName('PROGRESSIVO').AsInteger,Q040Data.AsDateTime);
    if Blocca <> 0 then
    begin
      flagasse2:=2;
      flagasse1:=2;
...
begin
  ...
  begin
    ...
    if not Parametri.ModuloInstallato['TORINO_CSI_PRV'] then //Per TORINO_CSI l'arrotondamento viene già gestito nella Rp502 a livello di singola fruizione
    begin
...
begin
  ...
  begin
    ...
    begin
      ...
      if SalvaValore then
        //SalvaOreLavorateFasce(VettConst[CurrCodInt].CodInt,QOreLavFascePOre_Lav.Value,OreLav,VettConst[CurrCodInt].Misura,Primo,DataCassa);
...
begin
  ...
  //R450DtM1.ConteggiMese('Generico',ACorr,MCorr,C700Progressivo);
...
begin
  ...
  begin
    ...
    try
      ...
      else if (VettConst[I].CodInt = '450') and (VoceAbilitata(VettConst[I].CodInt)) then
        //Dati atipici
        GestisciDatiAtipici(DataCassa);
...
var CodPaghe:String;
    Misura,C:String;
    ValIntero,ValParziale:Currency;
  procedure SalvaIndKM(Valore:Currency; Capitolo:String; Articolo:String);
  begin
    if selM052.FieldByName('DATARIF_VOCEPAGHE').AsString = 'M' then
      SalvaValoreGenerico(VettConst[CurrCodInt].CodInt,CodPaghe,Valore,Misura,R180Anno(selM052.FieldByName('DATADA').AsDateTime),R180Mese(selM052.FieldByName('DATADA').AsDateTime),DataCassa)
    else
...
begin
  ...
  begin
    ...
    repeat
      ...
      if (FieldByName('IMPORTOINDENNITA').AsFloat = 0) and (FieldByName('KMPERCORSI').AsFloat <> 0) then
      begin
        Misura:='N';
        C:='KMPERCORSI';
      end;
      //Verifica esistenza di gestione con capitolo/articolo
      selM052CapArt.Close;
      if not FieldByName('CATEG_RIMBORSO').IsNull then
      begin
        selM052CapArt.SetVariable('ID_MISSIONE',FieldByName('ID_MISSIONE').AsInteger);
        selM052CapArt.SetVariable('CODICE',FieldByName('CODICEINDENNITAKM').AsString);
        try
          selM052CapArt.Open;
        except
        end;
      end;
      if selM052CapArt.Active and (selM052CapArt.RecordCount > 0) then
      //Gestione con capitolo/articolo e percentualizzazione
      begin
        ValIntero:=selM052.FieldByName(C).AsCurrency;
        while not selM052CapArt.Eof do
        begin
          ValParziale:=selM052CapArt.FieldByName(C).AsCurrency;
          ValParziale:=Trunc(ValParziale*100)/100;
          ValIntero:=ValIntero - ValParziale;
          if selM052CapArt.RecNo = selM052CapArt.RecordCount then
            ValParziale:=ValParziale + ValIntero;
          SalvaIndKM(ValParziale,selM052CapArt.FieldByName('CAPITOLO').AsString,selM052CapArt.FieldByName('ARTICOLO').AsString);
          selM052CapArt.Next;
        end
      end
      else
        SalvaIndKM(FieldByName(C).AsCurrency,Def_Capitolo,Def_Articolo);
    until not SearchRecord('Data',EncodeDate(ACorr,MCorr,1),[]);
  end;
end;
...
var CodPaghe:String;
    OK:Boolean;
    ValIntero,ValParziale:Currency;
  procedure SalvaRimborsoSpese(Valore:Currency; Capitolo:String; Articolo:String);
  begin
    if selM050.FieldByName('DATARIF_VOCEPAGHE').AsString = 'M' then
      SalvaValoreGenerico(VettConst[CurrCodInt].CodInt,CodPaghe,Valore,VettConst[CurrCodInt].Misura,R180Anno(selM050.FieldByName('DATADA').AsDateTime),R180Mese(selM050.FieldByName('DATADA').AsDateTime),DataCassa,Capitolo,Articolo)
    else
      SalvaValoreGenerico(VettConst[CurrCodInt].CodInt,CodPaghe,Valore,VettConst[CurrCodInt].Misura,0,0,DataCassa,Capitolo,Articolo);
  end;
begin
  with selM050 do
  begin
    ...
    repeat
      ...
      begin
        ...
        begin
          //Verifica esistenza di gestione con capitolo/articolo
          selM050CapArt.SetVariable('ID_MISSIONE',FieldByName('ID_MISSIONE').AsInteger);
          selM050CapArt.SetVariable('CODICE',FieldByName('CODICERIMBORSOSPESE').AsString);
          try
            selM050CapArt.Open;
          except
          end;
        end;
        if selM050CapArt.Active and (selM050CapArt.RecordCount > 0) then
        //Gestione con capitolo/articolo e percentualizzazione
        begin
          ValIntero:=selM050.FieldByName(C).AsCurrency;
          while not selM050CapArt.Eof do
          begin
            ValParziale:=selM050CapArt.FieldByName(C).AsCurrency;
            ValParziale:=Trunc(ValParziale*100)/100;
            ValIntero:=ValIntero - ValParziale;
            if selM050CapArt.RecNo = selM050CapArt.RecordCount then
              ValParziale:=ValParziale + ValIntero;
            SalvaRimborsoSpese(ValParziale,selM050CapArt.FieldByName('CAPITOLO').AsString,selM050CapArt.FieldByName('ARTICOLO').AsString);
            selM050CapArt.Next;
          end
        end
        else
        //Gestione standard
        begin
          SalvaRimborsoSpese(selM050.FieldByName(C).AsCurrency,Def_Capitolo,Def_Articolo);
        end;
      end;
    until not SearchRecord('Data',EncodeDate(ACorr,MCorr,1),[]);
  end;
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if (Valore = 'N') and (Parametri.RagioneSociale.ToUpper = 'AZIENDA DI SERVIZI ALLA PERSONA "GOLGI - REDAELLI"') then
              Valore:='G';
          end
          ...
          else if VettTipo[j] = 'I' then  //capitolo
            Valore:=IfThen(VettCI[I].Capitolo <> Def_Capitolo,VettCI[I].Capitolo)
          else if VettTipo[j] = 'L' then  //articolo
            Valore:=IfThen(VettCI[I].Articolo <> Def_Articolo,VettCI[I].Articolo);
        if FileSeq then
...
procedure TA037FScaricoPagheMW.GestisciDatiAtipici(DataCassa: TDateTime);
var Valore: Currency;
    DS,DSErr:String;
begin
  if selT077.SearchRecord('Data',EncodeDate(ACorr,MCorr,1),[srFromBeginning]) then
    repeat
      try
        Valore:=StrToFloat(selT077.FieldByName('VALORE').AsString);
      except
        DS:={$IFNDEF VER210}FormatSettings.{$ENDIF}DecimalSeparator;
        DSErr:=IfThen(DS = ',','.',',');
        Valore:=StrToFloat(StringReplace(selT077.FieldByName('VALORE').AsString,DSErr,DS,[]));
      end;
      SalvaValoreGenerico(VettConst[CurrCodInt].CodInt,
                                       selT077.FieldByName('VOCEPAGHE').AsString,
                                       Valore,
                                       selT077.FieldByName('UMISURA').AsString ,
                                       0,0,DataCassa);
    until not selT077.SearchRecord('Data',EncodeDate(ACorr,MCorr,1),[]);
end;
