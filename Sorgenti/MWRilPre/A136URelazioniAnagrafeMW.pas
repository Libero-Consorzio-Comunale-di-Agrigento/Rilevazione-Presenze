...
type
  ...
  public
    ...
    function ControlloOrdine(bSolaLettura:Boolean): String;
    //Stampa Relazioni
    ...
    begin
      ...
      if not Result then
        ...
        begin
          ...
          if Result and not FselI030_Funzioni.FieldByName('COLONNA').IsNull then
            if Assigned(A136ImpostaMsgWarning) then
      ...
      begin
        ...
        if not Result then
          ...
          begin
            ...
            Execute;
            Result:=FieldAsInteger(0) = 0;
            if Result and not FselI030_Funzioni.FieldByName('COL_ORIGINE').IsNull then
              if Assigned(A136ImpostaMsgWarning) then
...
function TA136FRelazioniAnagrafeMW.ControlloOrdine(bSolaLettura:Boolean):String;
var
  BM:TBookMark;
  arrRel:array of TCtrlOrdine;
  i,j:Integer;
begin
  Result:='';
  if bSolaLettura then exit; //segnalo le incongruenze solo a chi può intervenire
  SetLength(arrRel,0);
  ...
  begin
    ...
    BM:=GetBookMark;
      ...
      begin
        i:=Length(arrRel);
        SetLength(arrRel,i + 1);
        //trasferisco le relazioni su un array per velocizzare i controlli
        arrRel[i].Tabella:=FieldByName('TABELLA').AsString;
        arrRel[i].Colonna:=FieldByName('COLONNA').AsString;
        arrRel[i].Ordine:=FieldByName('ORDINE').AsInteger;
        arrRel[i].Decorrenza:=FieldByName('DECORRENZA').AsDateTime;
        arrRel[i].DecorrenzaFine:=FieldByName('DECORRENZA_FINE').AsDateTime;
        arrRel[i].TabellaOrigine:=FieldByName('TAB_ORIGINE').AsString;
        arrRel[i].ColonnaOrigine:=FieldByName('COL_ORIGINE').AsString;
        Next;
      end;
      GoToBookMark(BM);
	  finally
    ...
    EnableControls;
    //Per ogni dato pilotato...
    for i:=0 to Length(arrRel) - 1 do
      //...che non ho già segnalato...
      if Pos(arrRel[i].Tabella + '.' + arrRel[i].Colonna,Result) <= 0 then
        //...verifico se, nelle relazioni...
        for j:=0 to Length(arrRel) - 1 do
          //...in cui è pilota, ha un ordine minore o uguale in un periodo che interseca quello in cui è pilotato...
          if (arrRel[j].TabellaOrigine = arrRel[i].Tabella)
          and (arrRel[j].ColonnaOrigine = arrRel[i].Colonna)
          and (arrRel[j].Ordine <= arrRel[i].Ordine)
          and (arrRel[j].Decorrenza <= arrRel[i].DecorrenzaFine)
          and (arrRel[j].DecorrenzaFine >= arrRel[i].Decorrenza) then
          begin
            Result:=Result + IfThen(Result <> '',',') + arrRel[i].Tabella + '.' + arrRel[i].Colonna;
            Break;
          end;
    SetLength(arrRel,0);
  end;
  if Result <> '' then
    Result:='Attenzione! Non è stato impostato correttamente l''ordine di esecuzione delle relazioni che usano le seguenti colonne: ' + CRLF +
            StringReplace(Result,',',CRLF,[rfReplaceAll]) + CRLF +
            'Di conseguenza tali campi non verranno aggiornati correttamente. Sistemare la situazione!';
end;
