...
type
  ...
  TA094FSkLimitiStraordMW = class(TR005FDataModuleMW)
    ...
    Upd811: TOracleQuery;
    Del811: TOracleQuery;
    selT830: TOracleDataSet;
    dsrT830: TDataSource;
    selT203: TOracleDataSet;
    selT830DATA: TDateTimeField;
    selT830VARIAZ_ORE_SUPPL: TStringField;
    selT830VARIAZ_ORE_STRAORD: TStringField;
    selT830NOTE: TStringField;
    selT830PROGRESSIVO: TIntegerField;
    selT830ORE_SUPPL_ANNO: TStringField;
    selT830ORE_STRAORD_ANNO: TStringField;
    GET_LIMITI_TEORICI_PT: TOracleQuery;
    ...
    procedure selT825RegistraLog(DataSet: TDataSet);
    ...
    procedure selT830NewRecord(DataSet: TDataSet);
    procedure selT830BeforePost(DataSet: TDataSet);
    procedure selT830BeforeEdit(DataSet: TDataSet);
    procedure selT830BeforeDelete(DataSet: TDataSet);
    procedure selT830RegistraLog(DataSet: TDataSet);
    procedure selT830CalcFields(DataSet: TDataSet);
  private
    //usati da A094
    ...
    function GetEsisteT203:Boolean;
  public
    //usati da A094 - WA094
    ...
    property EsisteT203: Boolean read GetEsisteT203;
  end;
...
begin
  ...
  selT830.SetVariable('ORDERBY','ORDER BY DATA DESC');
...
procedure TA094FSkLimitiStraordMW.selT825RegistraLog(DataSet: TDataSet);
begin
...
procedure TA094FSkLimitiStraordMW.selT830RegistraLog(DataSet: TDataSet);
begin
  inherited;
  RegistraLog.RegistraOperazione;
  SessioneOracle.Commit;
...
procedure TA094FSkLimitiStraordMW.selT830BeforeDelete(DataSet: TDataSet);
begin
  inherited;
  //il riepilogo da verificare è il T825 anche per la T830
  if selDatiBloccati.DatoBloccato(FProgressivoCorrente,selT830.FieldByName('DATA').AsDateTime,'T825') then
    raise Exception.Create(selDatiBloccati.MessaggioLog);
  RegistraLog.SettaProprieta('C',R180Query2NomeTabella(DataSet),NomeOwner,DataSet,True);
end;

procedure TA094FSkLimitiStraordMW.selT830BeforeEdit(DataSet: TDataSet);
begin
  inherited;
  //il riepilogo da verificare è il T825 anche per la T830
  if selDatiBloccati.DatoBloccato(FProgressivoCorrente,selT830.FieldByName('DATA').AsDateTime,'T825') then
...
procedure TA094FSkLimitiStraordMW.selT830BeforePost(DataSet: TDataSet);
var TotContrattuale,TotIndividuale:Integer;
begin
  inherited;
  if selT830.FieldByName('DATA').IsNull then
    raise Exception.Create(Format(A000MSG_ERR_FMT_CAMPO_OBBLIGATORIO,['Data']));
  selT830.FieldByName('DATA').AsDateTime:=R180InizioMese(selT830.FieldByName('DATA').AsDateTime);

  if (GET_LIMITI_TEORICI_PT.GetVariable('PERCPT') = null) or (GET_LIMITI_TEORICI_PT.GetVariable('PERCPT') = 100) then
    raise Exception.Create('Il dipendente non è in regime di part-time nel mese indicato');

  TotIndividuale:=R180OreMinuti(selT830.FieldByName('VARIAZ_ORE_SUPPL').AsString) + R180OreMinuti(selT830.FieldByName('VARIAZ_ORE_STRAORD').AsString);
  TotContrattuale:=GET_LIMITI_TEORICI_PT.GetVariable('MAXSUPPLANNO_PT') + GET_LIMITI_TEORICI_PT.GetVariable('MAXSTRANNO_PT');
  if TotIndividuale > TotContrattuale then
    raise Exception.Create('Le ore indicate superano la disponibilità contrattuale!');

  //il riepilogo da verificare è il T825 anche per la T830
  if selDatiBloccati.DatoBloccato(FProgressivoCorrente,selT830.FieldByName('DATA').AsDateTime,'T825') then
    raise Exception.Create(selDatiBloccati.MessaggioLog);

   if QueryPK1.EsisteChiave('T830_VARIAZ_STRAORD_PT_IND ',selT830.RowId,selT830.State,['PROGRESSIVO','DATA'],[IntToStr(FProgressivoCorrente),selT830.FieldByName('DATA').AsString]) then
    raise Exception.Create(A000MSG_ERR_CHIAVE_DUPLICATA);

  if not selT830.FieldByName('VARIAZ_ORE_SUPPL').IsNull then
    OreMinutiValidate(selT830.FieldByName('VARIAZ_ORE_SUPPL').AsString);
  if not selT830.FieldByName('VARIAZ_ORE_STRAORD').IsNull then
    OreMinutiValidate(selT830.FieldByName('VARIAZ_ORE_STRAORD').AsString);
...
procedure TA094FSkLimitiStraordMW.selT830CalcFields(DataSet: TDataSet);
begin
  inherited;
  selT830.FieldByName('ORE_SUPPL_ANNO').AsString:='';
  selT830.FieldByName('ORE_STRAORD_ANNO').AsString:='';
  if (not selT830.FieldByName('DATA').IsNull) (*and (selT830.State in [dsEdit,dsInsert])*) then
  begin
    with GET_LIMITI_TEORICI_PT do
    begin
      if (GetVariable('PROGRESSIVO') <> selT830.FieldByName('PROGRESSIVO').Value) or
         (GetVariable('DATA') <> selT830.FieldByName('DATA').Value) then
      begin
        SetVariable('PROGRESSIVO',selT830.FieldByName('PROGRESSIVO').AsInteger);
        SetVariable('DATA',selT830.FieldByName('DATA').AsDateTime);
        Execute;
      end;
      selT830.FieldByName('ORE_SUPPL_ANNO').AsString:=R180MinutiOre(GetVariable('MAXSUPPLANNO_PT'));
      selT830.FieldByName('ORE_STRAORD_ANNO').AsString:=R180MinutiOre(GetVariable('MAXSTRANNO_PT'));
    end;
  end;
end;

procedure TA094FSkLimitiStraordMW.selT830NewRecord(DataSet: TDataSet);
begin
  inherited;
  selT830.FieldByName('PROGRESSIVO').AsInteger:=FProgressivoCorrente;
  selT830.FieldByName('DATA').AsDateTime:=R180InizioMese(Parametri.DataLavoro);
end;
...
function TA094FSkLimitiStraordMW.GetEsisteT203: Boolean;
begin
  Result:=False;
  with selT203 do
  begin
    Open;
    Result:=FieldByName('CONTA').AsInteger > 0;
    Close;
  end;
end;
