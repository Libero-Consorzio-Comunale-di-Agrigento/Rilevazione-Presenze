...
uses
  ...
  Rp502Pro, DatiBloccati, C018UIterAutDM, Datasnap.DBClient;
...
type
  ...
  TA100FMissioniMW = class(TR005FDataModuleMW)
    ...
    USR_M043P_CARICA_GIUST: TOracleQuery;
    updM140Riapri: TOracleQuery;
    updM140Annullamento: TOracleQuery;
    selT850CodIter: TOracleQuery;
  ...
  private
    ...
    // AOSTA_REGIONE - chiamata 91104.ini
    procedure AnnullaRichiestaMissione(const PId: Integer);
    // AOSTA_REGIONE - chiamata 91104.fine
  public
    ...
    TabTariffe: Boolean;
    EsisteUSR_M043P_CARICA_GIUST: Boolean;
    ...
    function CodISTATtoCitta(CodiceISTAT:string):string;
...
begin
  ...
  // AOSTA_REGIONE - chiamata 91104.ini
  // datamodule per iter autorizzativi web
  C018:=TC018FIterAutDM.Create(Self);
  C018.Iter:=ITER_MISSIONI;
  C018.AccessoReadOnly:=False;
  C018.PreparaDataSetIter(selM140,tiRichiesta);
  // AOSTA_REGIONE - chiamata 91104.fine
  ...
  *)
  ...
  with USR_M043P_CARICA_GIUST do
  begin
    SetVariable('AGGIORNA', 'N');
    SetVariable('ID', 0);
    Execute;
    EsisteUSR_M043P_CARICA_GIUST:=GetVariable('CONTA_PROC') > 0;
  end;
...
function TA100FMissioniMW.CodISTATtoCitta(CodiceISTAT:string):string;
var
  IntCod:integer;
begin
  Result:='';
  CodiceISTAT:=CodiceISTAT.Trim;
  if (CodiceISTAT.Length = 6) and
     TryStrToInt(CodiceISTAT,IntCod)  then
  begin
    R180SetVariable(selT480,'CODICE',CodiceISTAT);
    selT480.Open;
    if selT480.RecordCount > 0 then
    begin
      Result:=selT480.FieldByName('CITTA').AsString;
    end;
  end;
end;
...
begin
  ...
  // E' previsto il rimborso pasto
  begin
    if True (*Parametri.RagioneSociale <> 'ISTITUTO ZOOPROFILATTICO SPERIMENTALE del Piemonte-Liguria-Valle D''aosta'*) then
    begin  //TORINO_ZOOPROFILATTICO-169935: rimosso il calcolo personalizzato che era stato introdotto con chiamate 68443 e 69953 del 2012 V. 8.4(5).
      // ******************************** INIZIO CALCOLO DEL RIMBORSO SPETTANTE ****************************
      M013F_CALC_RIMB_PASTO.SetVariable('CODICE', Q010.FieldByName('CODICE').AsString);
      M013F_CALC_RIMB_PASTO.SetVariable('TIPOREGISTRAZIONE', Q010.FieldByName('TIPO_MISSIONE').AsString);
      TotRimborsiPasto:=0;
...
begin
  ...
  // E' previsto il rimborso pasto
  begin
    if False (*Parametri.RagioneSociale = 'ISTITUTO ZOOPROFILATTICO SPERIMENTALE del Piemonte-Liguria-Valle D''aosta'*) then
      //TORINO_ZOOPROFILATTICO-169935: rimosso il calcolo personalizzato che era stato introdotto con chiamate 68443 e 69953 del 2012 V. 8.4(5).
...
begin
  ...
  begin
    ...
    Mostra:=False;
    SelM050.Close;
    SelM050.SetVariable('PROG', Q050.FieldByName('PROGRESSIVO').AsInteger);
    SelM050.SetVariable('MSCARICO',
    Q050.FieldByName('MESESCARICO').AsDateTime);
    SelM050.SetVariable('MCOMPETENZA', Q050.FieldByName('MESECOMPETENZA').AsDateTime);
    SelM050.SetVariable('DDA', Q050.FieldByName('DATADA').AsDateTime);
    SelM050.SetVariable('ODA', Q050.FieldByName('ORADA').AsString);
    SelM050.Execute;

    if (SelM050.RowCount > 0) or Not
       (Q050.FieldByName('COD_VALUTA_EST').IsNull) then
      Mostra:=True;
...
begin
  inherited;
  if (VarToStr(Q050.FieldByName('IMPRIMB_VALEST').OldValue) <> VarToStr(Q050.FieldByName('IMPRIMB_VALEST').Value)) and
      not ((VarToStr(Q050.FieldByName('IMPRIMB_VALEST').OldValue) = '') and (Q050.FieldByName('IMPRIMB_VALEST').Value = 0)) and
      not ((Q050.FieldByName('IMPRIMB_VALEST').OldValue = 0) and (VarToStr(Q050.FieldByName('IMPRIMB_VALEST').Value) = '')) and
  ...
  if (VarToStr(Q050.FieldByName('COSTORIMB_VALEST').OldValue) <> VarToStr(Q050.FieldByName('COSTORIMB_VALEST').Value)) and
      not ((VarToStr(Q050.FieldByName('COSTORIMB_VALEST').OldValue) = '') and (Q050.FieldByName('COSTORIMB_VALEST').Value = 0)) and
      not ((Q050.FieldByName('COSTORIMB_VALEST').OldValue = 0) and (VarToStr(Q050.FieldByName('COSTORIMB_VALEST').Value) = null)) and
      not (Q050.FieldByName('COD_VALUTA_EST').IsNull) then
  begin
...
begin
  ...
  try
    ...
    if RegoleTrovate and (not EsisteUSR_M043P_CARICA_GIUST) then
  ...
  if Not RegoleTrovate then
    Abort;
  if not EsisteUSR_M043P_CARICA_GIUST then
...
begin
  ...
  // AOSTA_REGIONE - chiamata 91104.ini
  IdMissioneOld:=FSelM040_Funzioni.FieldByName('ID').AsInteger;
  // AOSTA_REGIONE - chiamata 91104.fine
...
// AOSTA_REGIONE - chiamata 91104.ini
procedure TA100FMissioniMW.AnnullaRichiestaMissione(const PId: Integer);
// annulla la richiesta di missione con l'id indicato
var
  CodIter: String;
  Annullamento: String;
  ErrMsg: String;
begin
  // controlla l'id richiesta
  if PId <= 0 then
  begin
    ErrMsg:=Format('L''ID della richiesta indicato non è valido: %d',[PId]);
    raise Exception.Create(ErrMsg);
  end;

  // estrae il codice iter della richiesta
  try
    selT850CodIter.SetVariable('ID',PId);
    selT850CodIter.Execute;
    CodIter:=selT850CodIter.FieldAsString('COD_ITER');
  except
    on E: Exception do
    begin
      ErrMsg:=Format('La richiesta da annullare non è presente: %s',[E.Message]);
      raise Exception.Create(ErrMsg);
    end;
  end;

  // motivazione di annullamento (max 40 chr)
  Annullamento:=Format('Missione eliminata da operatore %s',[Parametri.Operatore]).Substring(0,40);

  // salva la motivazione di annullamento
  try
    updM140Annullamento.SetVariable('ID',PId);
    updM140Annullamento.SetVariable('ANNULLAMENTO',Annullamento);
    updM140Annullamento.Execute;
  except
    on E: Exception do
    begin
      ErrMsg:=Format('Errore durante l''impostazione della motivazione di annullamento sulla richiesta: %s',[E.Message]);
      //raise Exception.Create(ErrMsg);
    end;
  end;

  // imposta proprietà C018
  C018.Id:=PId;
  C018.CodIter:=CodIter;

  // chiude l'iter della richiesta
  try
    C018.InsUltimeAutorizzazioni(C018.LivMaxAut + 1,C018SI,C018AUTOMATICO,'S',Annullamento,True);
  except
    on E: Exception do
    begin
      ErrMsg:=ErrMsg + CRLF + Format('Errore durante la chisura dell''iter della richiesta: %s',[E.Message]);
      //raise Exception.Create(ErrMsg);
    end;
  end;

  // imposta tipo richiesta A = annullata
  try
    C018.SetTipoRichiesta('A');
  except
    on E: Exception do
    begin
      ErrMsg:=ErrMsg + CRLF + Format('Errore durante l''impostazione del tipo richiesta "A" sulla richiesta: %s',[E.Message]);
      //raise Exception.Create(ErrMsg);
    end;
  end;

  (*Non dovrebbe più servire:
    - il trasporto degli anticipi sui rimborsi non deve essere gestita perchè la missione ormai è stata cancellata
    - i giustificativi vengono eliminati dal trigger M040_AFTERDELETE
  // esegue procedura m050p_carica_rimborsi_daiter
  try
    M050P_CARICA_RIMBORSI_DAITER.SetVariable('ID',PId);
    M050P_CARICA_RIMBORSI_DAITER.Execute;
  except
    on E: Exception do
    begin
      ErrMsg:=Format('Errore durante l''esecuzione della procedure M050P_CARICA_RIMBORSI_DAITER: %s',[E.Message]);
      raise Exception.Create(ErrMsg);
    end;
  end;
  *)
...
procedure TA100FMissioniMW.M040AfterDelete;
begin
  // annullamento missione web se presente
  if IdMissioneOld > 0 then
    AnnullaRichiestaMissione(IdMissioneOld);
end;
// AOSTA_REGIONE - chiamata 91104.fine
...
begin
  USR_M043P_CARICA_GIUST.SetVariable('AGGIORNA', 'S');
  USR_M043P_CARICA_GIUST.SetVariable('ID', FSelM040_Funzioni.FieldByName('ID_MISSIONE').AsInteger);
  USR_M043P_CARICA_GIUST.Execute;
