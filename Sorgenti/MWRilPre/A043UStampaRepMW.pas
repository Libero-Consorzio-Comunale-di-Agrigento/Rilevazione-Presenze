...
type
  ...
  private
    ...
    procedure SalvataggioDatiTurni(Prog:Integer); {$IFNDEF DEBUG} inline; {$ENDIF !DEBUG}
  ...
  public
    ...
    NessunaAnomalia: Boolean;
    SalvaDatiTurni: Boolean;
    ...
    procedure ElaboraElemento(const PProg: Integer);
  ...
  with QSostitutivo do
    ...
    begin
      ...
      begin
        ...
        R502ProDtM1.FunzioneChiamante:='A043';
        if OraI >= OraF then
        begin
...
begin
  ...
  // se il campo non è indicato esce subito restituendo una stringa vuota
  if Campo = '' then
    Exit;
...
var DataInizio,DataFine,OraInizio,OraFine,OraI,OraF:String;
    ...
    Tolleranza,OreCompresenza:Integer;
    OreNonCaus:Boolean;
    BloccaOld:Word;
begin
  ...
  begin
    ...
    begin
      ...
      //Reimposto il flag di presenza in servizio che può essere stato alterato dai conteggi della compresenza
      R502ProDtM1.dipinser:='si';
    end;
  end
  else
...
begin
  ...
  if (not CalcolaIndennita) and (not A043chkCumula) then
  begin
    TabellaStampa.Post;
    exit;
  end;
...
begin
  ...
  begin
    GettRep:=0;
    A043ProgressBar.StepIt;
    A043ProgressBar.Repaint;
    ...
    begin
      ...
      // prosegue solo se sono indicate le ore del turno da contratto
      if R180OreMinutiExt(Reperib) > 0 then
      begin
...
procedure TA043FStampaRepMW.ElaboraElemento(const PProg: Integer);
var
  A: Word;
  M: Word;
  G: Word;
begin
  // eliminazione situazione precedente in caso di aggiornamento riepilogo
  if SalvaDatiTurni then
  begin
    // PRE:
    //   in caso di aggiornamento riepilogo
    //   il periodo [A043DataI..A043DataF] deve essere di un singolo mese intero
    //Assert((A043DataI = R180InizioMese(A043DataI)) and (A043DataF = R180FineMese(A043DataI)), 'Il periodo di elaborazione non è di un mese singolo!');

    // elimina i turni se il riepilogo del mese non è bloccato
    if not selDatiBloccati.DatoBloccato(PProg,A043DataI,'T340') then
    begin
      try
        // elimina i turni del mese
        DecodeDate(A043DataI,A,M,G);
        QDelete.SetVariable('Progressivo',PProg);
        QDelete.SetVariable('Anno',A);
        QDelete.SetVariable('Mese',M);
        QDelete.Execute;

        // registra operazione sui log
        RegistraLog.SettaProprieta('C','T340_TURNIREPERIB',CodForm,nil,True);
        RegistraLog.InserisciDato('PROGRESSIVO','',PProg.ToString);
        RegistraLog.InserisciDato('DATA','',DateToStr(A043DataI));
  ...
  // lettura dei turni pianificati nel mese
  Q380Pianif.Close;
  Q380Pianif.SetVariable('Progressivo',PProg);
  Q380Pianif.SetVariable('Data1',A043DataI);
  Q380Pianif.SetVariable('Data2',A043DataF);
  Q380Pianif.Open;

  // elabora i dati della reperibilità
  ElaboraReperib;
end;
...
begin
  ...
  // ciclo su tabella stampa
  TabellaStampa.First;
  while not TabellaStampa.Eof do
  begin
    if TabellaStampa.FieldByName('PROGRESSIVO').AsInteger <> LProg then
    begin
      ChiusuraRiepilogo(LProg);
      AzzeraContatori;
      LProg:=TabellaStampa.FieldByName('PROGRESSIVO').AsInteger;
    end;
    CumuloGiornaliero;
    TabellaStampa.Next;
  end;

  // chiude riepilogo dell'ultimo progressivo considerato
  ChiusuraRiepilogo(LProg);
end;
...
begin
  ...
  // gestione cumulo mese precedente
  ...
  // salvataggio riepilogo su database
...
begin
  ...
  // imposta le variabili fisse per l'inserimento
  QInsert.SetVariable('Progressivo',Prog);
  QInsert.SetVariable('Anno',A);
  QInsert.SetVariable('Mese',M);

  // inserisce un record per ogni riepilogo del mese
  for i:=0 to High(Riepilogo) do
  begin
    ...
    try
      // inserisce i dati su T340
      QInsert.Execute;

      // registra l'operazione sui log
