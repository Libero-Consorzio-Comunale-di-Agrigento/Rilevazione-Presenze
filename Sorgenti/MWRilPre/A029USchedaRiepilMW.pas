...
type
  ...
  TA029FSchedaRiepilMW = class(TR005FDataModuleMW)
    ...
    selT077TIPO: TStringField;
    selEsisteT070: TOracleQuery;
    selT074LiqPROGRESSIVO: TFloatField;
    selT074LiqDATA: TDateTimeField;
    selT073CompPROGRESSIVO: TFloatField;
    selT073CompDATA: TDateTimeField;
    ...
    procedure selT077VALORE_AUTGetText(Sender: TField; var Text: string; DisplayText: Boolean);
    procedure selT077VALORE_MANSetText(Sender: TField; const Text: string);
    procedure selT071BeforePost(DataSet: TDataSet);
    procedure selT071BeforeEdit(DataSet: TDataSet);
    procedure selT072BeforeInsert(DataSet: TDataSet);
    procedure selT077BeforeEdit(DataSet: TDataSet);
    procedure selT077BeforePost(DataSet: TDataSet);
    procedure selT075BeforeDelete(DataSet: TDataSet);
    procedure selT075BeforeEdit(DataSet: TDataSet);
    procedure selT075BeforeInsert(DataSet: TDataSet);
    procedure selT075BeforePost(DataSet: TDataSet);
    procedure selT074BeforeEdit(DataSet: TDataSet);
    procedure selT074BeforeInsert(DataSet: TDataSet);
    procedure selT074BeforePost(DataSet: TDataSet);
    procedure selT076BeforeEdit(DataSet: TDataSet);
    procedure selT076BeforeDelete(DataSet: TDataSet);
    procedure selT076BeforeInsert(DataSet: TDataSet);
    procedure selT076BeforePost(DataSet: TDataSet);
    procedure selT074LiqBeforeDelete(DataSet: TDataSet);
    procedure selT074LiqBeforeEdit(DataSet: TDataSet);
    procedure selT072ApplyRecord(Sender: TOracleDataSet; Action: Char;
      var Applied: Boolean; var NewRowId: string);
    procedure selT073CompApplyRecord(Sender: TOracleDataSet; Action: Char;
      var Applied: Boolean; var NewRowId: string);
    procedure selT074LiqApplyRecord(Sender: TOracleDataSet; Action: Char;
      var Applied: Boolean; var NewRowId: string);
    procedure selT077ApplyRecord(Sender: TOracleDataSet; Action: Char;
      var Applied: Boolean; var NewRowId: string);
    procedure selT074NewRecord(DataSet: TDataSet);
    procedure selT074AfterOpen(DataSet: TDataSet);
  private
    ...
    procedure selT074TIPO_RECORDValidate(Sender: TField);
    procedure T074Validate(Sender: TField);
    procedure SetQLook75(D: String);
    procedure RegistraLogDataSet(RLog:TRegistraLog; DataSetLog:TOracleDataSet; Azione:Char);
  ...
  public
    ...
    function IsRecordSchedaChangedOrDeleted(var RErrMsg: String): Boolean;
...
begin
  ...
  if Field = selT071OreLavorate then
    R450DtM1.L07.tminlavmese[i]:=R180OreMinuti(Field.AsString)
  else if Field = selT071Ore1Assest then
  //Ore Assestamento 1
    R450DtM1.L07.tdatiassestamento[1].tminassest[i]:=R180OreMinuti(Field.AsString)
  else if Field = selT071Ore2Assest then
  //Ore Assestamento 2
    R450DtM1.L07.tdatiassestamento[2].tminassest[i]:=R180OreMinuti(Field.AsString)
  else if Field = selT071OreEccedGiorn then
  //Ore Eccedenze giornaliere
    R450DtM1.L07.tminstrmens[i]:=R180OreMinuti(Field.AsString)
  else if (R450DtM1.LiquidazioneDistribuita = 'S') and (Field = selT071LiquidNelMese) then
  //Ore Sraordinario liquidabile
    R450DtM1.L07.tLiqNelMese[i]:=R180OreMinuti(Field.AsString);
  CalcolaDati;
end;
...
begin
  ...
  begin
    R450DtM1.L07.debormes:=R180OreMinuti(FieldByName('DebitoOrario').AsString);
    R450DtM1.L07.debpomes:=R180OreMinuti(FieldByName('DebitoPo').AsString);
    ...
    R450DtM1.L07.eccsolocompmes:=R180OreMinuti(FieldByName('OreEccedComp').AsString);
    R450DtM1.L07.EccSoloCompMesOltreSoglia:=R180OreMinuti(FieldByName('OreEccedCompOltreSoglia').AsString);
    R450DtM1.L07.vareccliqmes:=R180OreMinuti(FieldByName('OreVariazEcc').AsString);
    R450DtM1.L07.scostnegmes:=R180OreMinuti(FieldByName('ScostNeg').AsString);
    R450DtM1.L07.compnegmes:=StrToIntDef(VarToStr(selT077.Lookup('PROGRESSIVO;DATA;DATO',VarArrayOf([FieldByName('PROGRESSIVO').AsInteger,FieldByName('DATA').AsDateTime,I011_NEGATIVI_COMPENSABILI]),'VALORE_AUT')),0);
    R450DtM1.L07.ripcommes:=R180OreMinuti(FieldByName('RipCom').AsString);
    R450DtM1.L07.abbripcommes:=R180OreMinuti(FieldByName('AbbRipCom').AsString);
    (*RecAnnoCorr*)R450DtM1.L07.abbannoattmes:=R180OreMinuti(FieldByName('RecAnnoCorr').AsString);
    (*RecAnnoPrec*)R450DtM1.L07.abbannoprecmes:=R180OreMinuti(FieldByName('RecAnnoPrec').AsString);
    R450DtM1.L07.abbliqannoattmes:=R180OreMinuti(FieldByName('RecLiqCorr').AsString);
    R450DtM1.L07.abbliqannoprecmes:=R180OreMinuti(FieldByName('RecLiqPrec').AsString);
    R450DtM1.L07.OreCompLiquidate:=R180OreMinuti(FieldByName('OreComp_Liquidate').AsString) + R180OreMinuti(FieldByName('BancaOre_Liq_Var').AsString);
    R450DtM1.L07.BancaOreLiqVar:=R180OreMinuti(FieldByName('BancaOre_Liq_Var').AsString);
    R450DtM1.L07.OreCompRecuperate:=R180OreMinuti(FieldByName('OreComp_Recuperate').AsString);
    R450DtM1.L07.RiposiNonFruitiOre:=FieldByName('RiposiNonFruitiOre').AsString;
    R450DtM1.L07.RiepAssCompensateAnno:=R180OreMinuti(FieldByName('RIEPASS_COMPENSATE_ANNO').AsString);
    R450DtM1.L07.RiepAssCompensateMese:=R180OreMinuti(FieldByName('RIEPASS_COMPENSATE_MESE').AsString);
  ...
  begin
    ...
    begin
      ...
      R450DtM1.L07.tminlavmese[i]:=R180OreMinuti(FieldByName('OreLavorate').AsString);
      R450DtM1.L07.tdatiassestamento[1].tminassest[i]:=R180OreMinuti(FieldByName('Ore1Assest').AsString);
      R450DtM1.L07.tdatiassestamento[2].tminassest[i]:=R180OreMinuti(FieldByName('Ore2Assest').AsString);
      R450DtM1.L07.tminstrmens[i]:=R180OreMinuti(FieldByName('OreEccedGiorn').AsString);
      R450DtM1.L07.tstrliquidatomm[i]:=R180OreMinuti(FieldByName('OreStraordLiq').AsString);
      R450DtM1.L07.tLiqNelMese[i]:=R180OreMinuti(FieldByName('LiquidNelMese').AsString);
      inc(i);
      Next;
    end;
    First;
    ...
    B2:=selT074Liq.GetBookmark;
      ...
      begin
        ...
        begin
          ...
          begin
            inc(j);
            R450DtM1.RiepPresCartellino[iRPC].OreRese[j]:=R180OreMinuti(FieldByName(FieldDefs[i].Name).AsString);
          end;
        end;
        //Ore compensabili registrate
        R450DtM1.RiepPresCartellino[iRPC].Compensabile:=R180OreMinuti(VarToStr(selT073Comp.Lookup('CAUSALE',FieldByName('CAUSALE').AsString,'COMPENSABILE')));
        ...
        if selT074Liq.SearchRecord('CAUSALE',FieldByName('CAUSALE').AsString,[srFromBeginning]) then
          repeat
            inc(j);
            R450DtM1.RiepPresCartellino[iRPC].Liquidato[j]:=R180OreMinuti(selT074Liq.FieldByName('LIQUIDATO').AsString);
            //A029FSchedaRiepil.VariazioniRiepPres.Liquidato[j]:=R180OreMinuti(sel074Liq.FieldByName('LIQUIDATO').AsString);
...
procedure TA029FSchedaRiepilMW.RegistraLogDataSet(RLog:TRegistraLog; DataSetLog:TOracleDataSet; Azione:Char);
begin
  inherited;
  case Azione of
    'I':RLog.SettaProprieta('I',R180Query2NomeTabella(DataSetLog),NomeOwner,DataSetLog,True);
    'U':RLog.SettaProprieta('M',R180Query2NomeTabella(DataSetLog),NomeOwner,DataSetLog,True);
    'D':RLog.SettaProprieta('C',R180Query2NomeTabella(DataSetLog),NomeOwner,DataSetLog,True);
  end;
  if Azione in ['I','U','D'] then
  begin
    RLog.RegistraOperazione;
  end;
end;

procedure TA029FSchedaRiepilMW.selT073CompApplyRecord(Sender: TOracleDataSet;
  Action: Char; var Applied: Boolean; var NewRowId: string);
begin
  inherited;
  RegistraLogDataSet(RegistraLogT073,Sender,Action);
end;
...
procedure TA029FSchedaRiepilMW.selT074LiqApplyRecord(Sender: TOracleDataSet;
  Action: Char; var Applied: Boolean; var NewRowId: string);
begin
  inherited;
  RegistraLogDataSet(RegistraLogT074,Sender,Action);
end;

procedure TA029FSchedaRiepilMW.selT074LiqBeforeDelete(DataSet: TDataSet);
begin
  Abort;
end;

procedure TA029FSchedaRiepilMW.selT074LiqBeforeEdit(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
  ...
  if IsRecordSchedaChangedOrDeleted(LErrore) then
  begin
    selT070_Funzioni.Cancel;
    R180MessageBox(LErrore,ESCLAMA);
    Abort;
  end;
end;
...
procedure TA029FSchedaRiepilMW.selT075BeforeDelete(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT075BeforeEdit(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT075BeforeInsert(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT075BeforePost(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT076BeforeDelete(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT076BeforeEdit(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT076BeforeInsert(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT076BeforePost(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT077ApplyRecord(Sender: TOracleDataSet;
  Action: Char; var Applied: Boolean; var NewRowId: string);
begin
  inherited;
  RegistraLogDataSet(RegistraLogT077,Sender,Action);
end;
...
procedure TA029FSchedaRiepilMW.selT077BeforeEdit(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT077BeforePost(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT077VALORE_AUTGetText(Sender: TField; var Text: string; DisplayText: Boolean);
begin
  inherited;
  Text:=Sender.AsString;
  if not Sender.IsNull then
  begin
    if selT077.FieldByName('TIPO').AsString = 'O' then
      Text:=R180MinutiOre(StrToIntDef(Sender.AsString,0));
  end;
end;

procedure TA029FSchedaRiepilMW.selT077VALORE_MANSetText(Sender: TField; const Text: string);
begin
  inherited;
  ...
  begin
    if selT077.FieldByName('TIPO').AsString = 'O' then
    try
      OreMinutiValidate(Text);
      Sender.AsInteger:=R180OreMinuti(Text);
    except
      Sender.AsString:='';
    end;
  end;
...
begin
  ...
  begin
    Liquidato:=R180OreMinuti(VarToStr(selT071LiquidNelMese.Value)) -
               R180OreMinuti(VarToStr(selT071LiquidNelMese.medpOldValue));
    if Liquidato <> 0 then
    begin
      inc(NDaLiquidare);
      DaLiquidare[NDaLiquidare].Liquidato:=Liquidato;
      DaLiquidare[NDaLiquidare].Maggiorazione:=selT071Maggiorazione.medpOldValue;
      DaLiquidare[NDaLiquidare].CodFascia:=selT071CodFascia.medpOldValue;
    end;
  end;
  RegistraLogDataSet(RegistraLogT071,Sender,Action);
end;
...
procedure TA029FSchedaRiepilMW.selT071BeforeEdit(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT071BeforePost(DataSet: TDataSet);
var
  LErrore: String;
begin
  // verifica che la scheda T070 non sia stata modificata o eliminata nel frattempo
...
begin
  ...
  selT071.FieldByName('Totale').AsString:=R180MinutiOre(R180OreMinuti(selT071.FieldByName('OreLavorate').AsString) +
                                       R180OreMinuti(selT071.FieldByName('Ore1Assest').AsString) +
                                       R180OreMinuti(selT071.FieldByName('Ore2Assest').AsString));
...
procedure TA029FSchedaRiepilMW.selT072ApplyRecord(Sender: TOracleDataSet;
  Action: Char; var Applied: Boolean; var NewRowId: string);
begin
  inherited;
  RegistraLogDataSet(RegistraLogT072,Sender,Action);
end;
...
procedure TA029FSchedaRiepilMW.selT072BeforeInsert(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT074AfterOpen(DataSet: TDataSet);
begin
  inherited;
  DataSet.FieldByName('TIPO_RECORD').ReadOnly:=True;
end;
      ...
      begin
        ...
        begin
          for i:=0 to selT074.FieldCount - 1 do
            if not R180In(selT074.Fields[i].FieldName.ToUpper,['CAUSALE','TIPO_RECORD','TOTALE','D_CAUSALE']) then
            begin
        ...
        begin
          ...
          SetVariable('Data',FselT070_Funzioni.FieldByName('Data').AsDateTime);
          SetVariable('Causale',selT074.FieldByName('Causale').AsString);
          SetVariable('TIPO_RECORD',selT074.FieldByName('TIPO_RECORD').AsString);
          Execute;
        end;
      end;
...
begin
  ...
  if (PresenzeAnnue.TotaliPresenza.LiquidatoMese <> 0) or
     (R180OreMinuti(selT073Comp.FieldByName('Compensabile').AsString) <> 0) then
    raise Exception.Create(A000MSG_A029_ERR_DELETE_LIQUID);

  //Cancellazione impedita per record Automatico
  if DataSet.FieldByName('TIPO_RECORD').AsString = 'A' then
    Abort;
end;

procedure TA029FSchedaRiepilMW.selT074BeforeEdit(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
  ...
  //Modifica impedita per record Automatico
  if DataSet.FieldByName('TIPO_RECORD').AsString = 'A' then
    Abort;
end;

procedure TA029FSchedaRiepilMW.selT074BeforeInsert(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
...
procedure TA029FSchedaRiepilMW.selT074BeforePost(DataSet: TDataSet);
var
  LErrore: String;
begin
  inherited;
  Dataset.FieldByName('TIPO_RECORD').AsString:=Dataset.FieldByName('TIPO_RECORD').AsString.ToUpper;
  // verifica che la scheda T070 non sia stata modificata o eliminata nel frattempo
  if IsRecordSchedaChangedOrDeleted(LErrore) then
  begin
    selT070_Funzioni.Cancel;
...
begin
  ...
  begin
    if not R180In(selT074.Fields[i].FieldName.ToUpper,['CAUSALE','TIPO_RECORD','TOTALE','D_CAUSALE']) then
    begin
      Totale:=Totale + R180OreMinuti(selT074.Fields[i].AsString);
    end;
  end;
...
begin
  ...
  begin
    ...
    FieldByName('TIPO_RECORD').AsString:='A';
    Post;
...
begin
  ...
  begin
    ...
    if (FselT070_Funzioni.FieldByName('CAUSALE1MINASS').AsString = '') and
       (R180OreMinuti(selT071Ore1Assest.AsString) <> 0) then
  ...
  begin
    ...
    if (FselT070_Funzioni.FieldByName('CAUSALE2MINASS').AsString = '') and
       (R180OreMinuti(selT071Ore2Assest.AsString) <> 0) then
      raise Exception.Create(A000MSG_A029_ERR_CAUS_ASSESTAMENTO);
  end;
  ...
  begin
    try
      ...
      try
        ...
        begin
          TotaliCalcolati.TotLiqNelMese:=TotaliCalcolati.TotLiqNelMese + R180OreMinuti(selT071LiquidNelMese.AsString); //Liquidato nel mese
          selT071.Next;
        end;
...
begin
  ...
  selT074.Close;
  Carat:='A';
  SQLSelect:='SELECT T073.ROWID,T073.CAUSALE,T073.TIPO_RECORD';
  ...
  begin
    ...
    else if UpperCase(selT074.FieldDefs[i].Name) = 'TIPO_RECORD' then
    begin
      selT074.Fields[i].DisplayLabel:='Tipo';
      selT074.Fields[i].OnValidate:=selT074TIPO_RECORDValidate;
      //selT074.Fields[i].ReadOnly:=True;
    end
    else
    begin
      selT074.Fields[i].DisplayLabel:=DispLab[i - 1];
      (selT074.Fields[i] as TStringField).EditMask:='!9000:00;1;_';
      selT074.Fields[i].Tag:=i - 1;
...
procedure TA029FSchedaRiepilMW.selT074TIPO_RECORDValidate(Sender: TField);
begin
  if not R180In(Sender.AsString.ToUpper,['A','M']) then
    raise Exception.Create(A000MSG_A029_ERR_TIPO_RECORD);
end;
...
begin
  ...
  try
    ...
    begin
      SE:=SE + R180OreMinuti(selT075.FieldByName('ORESTRAORD').AsString);
      selT075.Next;
    end;
  finally
...
begin
  ...
  begin
    ...
    while not Eof do
    begin
      if R180OreMinuti(FieldByName('Liquidato').AsString) <> 0 then
      begin
  ...
  begin
    ...
    while not Eof do
    begin
      if R180OreMinuti(FieldByName('Compensabile').AsString) <> 0 then
      begin
        PresenzeLiquidate:=True;
        Break;
      end;
  ...
  if selT072.SearchRecord('TIPO_RECORD','M',[srFromBeginning]) then
    raise Exception.Create(A000MSG_A029_ERR_DELETE_INDPRESMAN);
  if selT074.SearchRecord('TIPO_RECORD','M',[srFromBeginning]) then
    raise Exception.Create(A000MSG_A029_ERR_DELETE_RIEPPRESMAN);
  ...
  begin
    ...
    Execute;
    Sql.Clear;
    Sql.Add(Format('DELETE FROM T077_DATISCHEDA WHERE Progressivo = %d AND Data = ''%s''',[SelAnagrafe.FieldByName('Progressivo').AsInteger,FormatDateTime('dd/mm/yyyy',Data)]));
    Execute;
  end;
...
begin
  if LiquidCompBloccata and
     (R180OreMinuti(FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').AsString) + R180OreMinuti(FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').AsString) >
      R180OreMinuti(VarToStr(FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').OldValue)) + R180OreMinuti(VarToStr(FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').OldValue))) then
    raise Exception.Create(A000MSG_A029_ERR_LIQ_BLOC);
  if (FselT070_Funzioni.FieldByName('CAUSALE1MINASS').Value <> FselT070_Funzioni.FieldByName('CAUSALE1MINASS').OldValue) then
  begin
    if (VarToStr(FselT070_Funzioni.FieldByName('CAUSALE1MINASS').OldValue) <> '') and (not A000FiltroDizionario('CAUSALI GIUSTIFICAZIONE',FselT070_Funzioni.FieldByName('CAUSALE1MINASS').OldValue)) then
      raise Exception.Create(A000MSG_ERR_CAUSALE_DIZIONARIO);
    if (VarToStr(FselT070_Funzioni.FieldByName('CAUSALE1MINASS').Value) <> '') and (not A000FiltroDizionario('CAUSALI GIUSTIFICAZIONE',FselT070_Funzioni.FieldByName('CAUSALE1MINASS').Value)) then
      raise Exception.Create(A000MSG_ERR_CAUSALE_DIZIONARIO);
  end;
  if (FselT070_Funzioni.FieldByName('CAUSALE2MINASS').Value <> FselT070_Funzioni.FieldByName('CAUSALE2MINASS').OldValue) then
  begin
    if (VarToStr(FselT070_Funzioni.FieldByName('CAUSALE2MINASS').OldValue) <> '') and (not A000FiltroDizionario('CAUSALI GIUSTIFICAZIONE',FselT070_Funzioni.FieldByName('CAUSALE2MINASS').OldValue)) then
      raise Exception.Create(A000MSG_ERR_CAUSALE_DIZIONARIO);
    if (VarToStr(FselT070_Funzioni.FieldByName('CAUSALE2MINASS').Value) <> '') and (not A000FiltroDizionario('CAUSALI GIUSTIFICAZIONE',FselT070_Funzioni.FieldByName('CAUSALE2MINASS').Value)) then
      raise Exception.Create(A000MSG_ERR_CAUSALE_DIZIONARIO);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        DeltaLiq:=R180OreMinuti(VarToStr(selT071.FieldByName('LiquidNelMese').Value)) - R180OreMinuti(VarToStr(selT071.FieldByName('LiquidNelMese').OldValue));
      ...
      MaxLiquidato:=MaxLiquidato + R180OreMinuti(VarToStr(selT071LiquidNelMese.Value));
      Next;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        if UpdateStatus in [usModified,usInserted] then
        begin
          DeltaLiq:=R180OreMinuti(VarToStr(selT074Liq.FieldByName('Liquidato').Value)) - R180OreMinuti(VarToStr(selT074Liq.FieldByName('Liquidato').OldValue));
          OreLiq:=OreLiq + DeltaLiq;
        ...
        MaxLiquidato:=MaxLiquidato + R180OreMinuti(VarToStr(selT074Liq.FieldByName('Liquidato').Value));
      end;
      Next;
    end;
    Filtered:=True;
...
begin
  ...
  if Aggiorna then
  begin
    VariazioniRiepPres.Compensabile:=R180OreMinuti(OreEsclCompMese);
    R450DtM1.AppRiepPres.CompensabileMese:=R180OreMinuti(OreEsclCompMese);
  end;
...
begin
  ...
  if FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').IsNull and FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').isNull then
    exit;
  if (R180OreMinuti(FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').AsString) + R180OreMinuti(FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').AsString)) = 0 then
    exit;
  if (R180OreMinuti(FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').AsString) + R180OreMinuti(FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').AsString) - R450DtM1.L07.OreCompLiquidate) > (R450DtM1.BancaOreResidua + IfThen(R450DtM1.BancaOreEsclusaSaldi = 'S', R450DtM1.BancaOreResiduaPrec)) then
    Result:=A000MSG_A029_ERR_DISP_LIQ;
end;

function TA029FSchedaRiepilMW.CalcolaCompLiquidato:Integer;
begin
  Result:=(R180OreMinuti(VarToStr(FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').Value)) + R180OreMinuti(VarToStr(FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').Value))) - (R180OreMinuti(VarToStr(FselT070_Funzioni.FieldByName('ORECOMP_LIQUIDATE').OldValue)) + R180OreMinuti(VarToStr(FselT070_Funzioni.FieldByName('BANCAORE_LIQ_VAR').OldValue)));
end;
...
begin
  ...
  if VarToStr(selT305.Lookup('CODICE',FselT070_Funzioni.FieldByName('CAUSALE1MINASS').AsString,'LIMITE_LIQ')) = 'S' then
    Result:=Result + Abs(Min(R180OreMinuti(VarToStr(selT071.FieldByName('ORE1ASSEST').Value)),0)) - Abs(Min(R180OreMinuti(VarToStr(selT071.FieldByName('ORE1ASSEST').OldValue)),0));
  if VarToStr(selT305.Lookup('CODICE',FselT070_Funzioni.FieldByName('CAUSALE2MINASS').AsString,'LIMITE_LIQ')) = 'S' then
    Result:=Result + Abs(Min(R180OreMinuti(selT071.FieldByName('ORE2ASSEST').AsString),0)) - Abs(Min(R180OreMinuti(VarToStr(selT071.FieldByName('ORE2ASSEST').OldValue)),0));
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      Liquidato:=R180OreMinuti(VarToStr(selT071.FieldByName('LiquidNelMese').Value)) - R180OreMinuti(VarToStr(selT071.FieldByName('LiquidNelMese').OldValue));
      inc(TotLiquidato,Liquidato);
      inc(TotDaLiquidare,R180OreMinuti(selT071.FieldByName('StrDaLiq').AsString));
      if Liquidato > R180OreMinuti(selT071.FieldByName('StrDaLiq').AsString) then
      begin
...
begin
  Result:='';
  if R180OreMinuti(selT074Liq.FieldByName('LIQUIDATO').AsString) = 0 then
    exit;

  PresenzeAnnue:=TotaliPresenza(CodiceCausPresenza,False,'');
  //Residuo:=R180OreMinuti(A029FSchedaRiepil.grdPresAnnueTot.Cells[6,0]);
  //Residuo:=R180OreMinuti(A029FSchedaRiepil.grdPresAnnueTot.Cells[4,0]) - R180OreMinuti(A029FSchedaRiepil.grdPresAnnueTot.Cells[5,0]);

  //Caratto 28/05/2013 utilizzo struttura dati al posto della griglia direttamente
  //  Residuo:=R180OreMinuti(A029FSchedaRiepil.grdPresAnnueTot.Cells[4,0]);
  ...
  begin
    ...
    begin
      Liquidato:=R180OreMinuti(selT074Liq.FieldByName('LIQUIDATO').AsString) - VariazioniRiepPres.Liquidato[i+1]; //VariazioniRiepPres.Liquidato parte da indice 1 e non 0
      Break;
    end;
  end;
  ...
  //if not(TipoOreCausalizzate in [tocEscluse]) and (Liquidato > R180OreMinuti(A029FSchedaRiepil.SGStraord.Cells[8,0])) then
  //TotaliCalcolati impostati in CalcolaDati
...
procedure TA029FSchedaRiepilMW.selT074NewRecord(DataSet: TDataSet);
begin
  inherited;
  Dataset.FieldByName('TIPO_RECORD').AsString:='M';
end;
...
var
  PresenzeAnnue: TPresenzeAnnue;
begin
  if R180OreMinuti(selT073Comp.FieldByName('COMPENSABILE').AsString) = 0 then
    exit;

  PresenzeAnnue:=TotaliPresenza(CodiceCausPresenza,False,'');

  (*if (R180OreMinuti(sel073CompCOMPENSABILE.AsString) - A029FSchedaRiepil.VariazioniRiepPres.Compensabile) >
     R180OreMinuti(A029FSchedaRiepil.grdPresAnnueTot.Cells[5,0]) then*)
   //Caratto sostituito valori di griglia con valori calcolati
  (*if R180OreMinuti(selT073Comp.FieldByName('COMPENSABILE').AsString) >
       (R180OreMinuti(A029FSchedaRiepil.edtOreEsclCompMeseEff.Text) +
       R180OreMinuti(A029FSchedaRiepil.grdPresAnnueTot.Cells[6,0])) then*)
  if R180OreMinuti(selT073Comp.FieldByName('COMPENSABILE').AsString) >
...
function TA029FSchedaRiepilMW.IsRecordSchedaChangedOrDeleted(var RErrMsg: String): Boolean;
begin
  Result:=False;
  RErrMsg:='';

  if selT070_Funzioni.State = dsEdit then
  begin
    // se il dataset principale è in modifica, verifica che il rowid esista
    selEsisteT070.SetVariable('PAR_ROW_ID',selT070_Funzioni.RowId);
    selEsisteT070.Execute;
    Result:=selEsisteT070.FieldAsInteger(0) = 0;
    if Result then
      RErrMsg:=A000TraduzioneStringhe(A000MSG_A029_ERR_SCHEDA_MODIF_ESTERNO);
  end;
end;
