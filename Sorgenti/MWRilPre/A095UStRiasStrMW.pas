...
begin
  ...
  try
    TabellaStampa.FieldDefs.Add('StrPag_Disp',ftString,8,False);
  except
  end;
  try
  ...
  TabellaStampa.FieldByName('TotStrF').DisplayLabel:='Straord. esterno';
  TabellaStampa.FieldByName('StrIn').DisplayLabel:='Straord. interno';
  TabellaStampa.FieldByName('StrLiq').DisplayLabel:='Eccedenza disponibile';
  TabellaStampa.FieldByName('MaxLiq').DisplayLabel:='Liquid. massimo';
  TabellaStampa.FieldByName('StrPag_Disp').DisplayLabel:='Straord. disp.';
  TabellaStampa.FieldByName('StrPag').DisplayLabel:='Straord. liquidato';
  TabellaStampa.FieldByName('StrPag').EditMask:='!900:00;1;_';
  TabellaStampa.FieldByName('EccRes').DisplayLabel:='Eccedenza residuabile';
  TabellaStampa.FieldByName('MaxRes').Visible:=False;
end;

procedure TA095FStRiasStrMW.ImpostaCampiTabellaStampaReadonly(const PReadonly: Boolean);
var
  i: Integer;
begin
  ...
  // il campo "StrPag" (Straord. liquidato) è sempre modificabile
  TabellaStampa.FieldByName('StrPag').ReadOnly:=False;
end;
...
begin
  ...
  begin
    ...
    StrLiq:=R450DtM1.StrFattoMeseTotale;
  ...
  TabellaStampa.FieldByName('StrPag_Disp').Value:=TabellaStampa.FieldByName('StrPag').Value;
  TabellaStampa.FieldByName('EccRes').Value:=R180MinutiOre(EccRes);
  bResiduoBloccato:=selDatiBloccati.DatoBloccato(SelAnagrafe.FieldByName('PROGRESSIVO').AsInteger,Data,'T071S');
  if bResiduoBloccato then
    TabellaStampa.FieldByName('Anomalia').AsString:='X';
  if (StrPag = 0) and (TabellaStampa.FieldByName('Anomalia').AsString = '') then
  begin
    TabellaStampa.Cancel;
    ImpostaCampiTabellaStampaReadonly(True);
    exit;
  end;
  if (bLiquidazione) and (StrPag > 0) and (not bResiduoBloccato) then
  begin
    TempDataLiq:=Data;
...
procedure TA095FStRiasStrMW.LiquidazioneDatiModificati(Anno, Mese, TipoDisponibilita, IdRiga: Integer);
var
  Prog, StrPag: Integer;
  Anomalia: String;
  Esegui: Boolean;
  ImpLiq: Real;
  Data, TempDataLiq: TDateTime;
begin
  ...
  StrPag:=R180OreMinuti(TabellaStampa.FieldByName('StrPag').AsString);

  // se i dati non sono bloccati e si richiede una liquidazione significativa
  // verifica la disponibilità
  if (Anomalia <> 'X') and (StrPag > 0) then
  begin
    // eseguo i conteggi del mese
    if (R450DtM1.Progress400 = Prog) and (Anno = R450DtM1.Anno400) and (Mese = R450DtM1.Mese400) then
    ...
    begin
      ...
      begin
        ...
        // la funzione carica la struttura dati in A029FBudgetDtM1 per effettuare il calcolo dell'importo
        // richiesto in base alla distribuzione delle ore in fasce
        Liquidazione(True,Prog,StrPag,TempDataLiq,TipoDisponibilita);
        ImpLiq:=A029FLiquidazione.A029FBudgetDtM1.CtrlLiqGetImporto(Prog,TempDataLiq);

        // controllo sforamento budget
        Esegui:=A029FLiquidazione.A029FBudgetDtM1.ControllaBudget(False,Prog,TempDataLiq,StrPag,ImpLiq);
      end;

      // se è tutto ok effettua la liquidazione
      if Esegui then
      begin
        Liquidazione(False,Prog,StrPag,TempDataLiq,TipoDisponibilita);
        RegistraLog.SettaProprieta('M','T071_SCHEDAFASCE',nomeOwner,nil,True);
...
procedure TA095FStRiasStrMW.TabellaStampaBeforePost(DataSet: TDataSet);
var
  LNominativo,LStrPag,LStrPagDisp: String;
  LStrPagM, LStrPagDispM: Integer;
begin
  inherited;

  // salva informazioni in variabili di appoggio
  LNominativo:=TabellaStampa.FieldByName('Cognome').AsString + ' ' + TabellaStampa.FieldByName('Nome').AsString;
  LStrPag:=TabellaStampa.FieldByName('StrPag').AsString;
  LStrPagDisp:=TabellaStampa.FieldByName('StrPag_Disp').AsString;

  // controllo che il liquidato non superi la quantità disponibile
  LStrPagM:=R180OreMinutiExt(LStrPag);
  LStrPagDispM:=R180OreMinutiExt(LStrPagDisp);
  if LStrPagM > LStrPagDispM then
    raise Exception.Create(Format('La quantità da liquidare per il dipendente'#13#10'%s'#13#10'supera la disponibilità:'#13#10'[%s] > [%s]',[LNominativo,LStrPag,LStrPagDisp]));
end;
...
begin
  ...
  FreeAndNil(A029FLiquidazione);//.Free;
  inherited;
end;
