...
var
  ...
  function IterAbilitatoGiustif(giust: t_tgius_dallealle):Boolean;
  begin
    //TORINO_CITTADELLASALUTE: considerare anche i giustificativi di assenza dalle..alle
    //Il giustificativo, per essere abilitato all'iter deve avere attivato il parametro ITER_ECCGG
    //e avere il punto di entrata antecedente l'entrata nominale oppure
    //il punto di uscita successivo all'uscita nominale dell'orario
    Result:=False;
    if (R502ProDtM.ValStrT265[Parametri.CampiRiferimento.C90_W026CausAbbattGiustAss,'INFLUCONT'] = 'D') and
       (R502ProDtM.ValStrT265[giust.tcausdaa,'ITER_ECCGG'] = 'S') then
    begin
      //Nessun punto nominale di riferimento: considero sempre il giustificativo
      if R502ProDtM.n_timbrnom = 0 then
        Result:=True
      //Un unico punto nominale di riferimento o comunque orario flessibile (continuato o spezzato)
      else if (R502ProDtM.n_timbrnom = 1) or (R502ProDtM.TipoOrario = 'A') then
      begin
        with R502ProDtM.ttimbraturenom[R502ProDtM.n_timbrnom] do
          Result:=(giust.tminutia > tminutin_u)
                  or
                  (giust.tminutida < tminutin_e + max(0,Flex - FlexPM));
      end
      //Diversi punti nominali (orario a turni)
      else if R502ProDtM.n_timbrnom > 1 then
      begin
        //Punto nominale riconosciuto dalle timbrature
        if R502ProDtM.UltimaUscitaNominaleUsata > 0 then
        begin
          Result:=(giust.tminutia > R502ProDtM.UltimaUscitaNominaleUsata)
                  or
                  (giust.tminutida < R502ProDtM.PrimaEntrataNominaleUsata)
        end
        else
        begin
          //Nessun punto nominale riconosciuto dalle timbrature: considero il primo punto nominale
          with R502ProDtM.ttimbraturenom[1] do
            Result:=(giust.tminutia > tminutin_u)
                    or
                    (giust.tminutida < tminutin_e + max(0,Flex - FlexPM));
        end;
      ...
      begin
        ...
        begin
          ...
          Break;
                ...
                (not IterAbilitatoGiustif(tgius_dallealle[i])) and
                (timbArr[idx].tminutid_e = tgius_dallealle[i].tminutida) and
        ...
        begin
          ...
          if IterAbilitatoGiustif(tgius_dallealle[i]) then
            Tipo:='GP';
          Result:=i;
          Break;
        end;
    ...
    begin
      ...
      begin
        ...
        if ttimbraturedip[i].tminutid_e >= ttimbraturedip[i].tminutid_u then
          Continue;
        if (ttimbraturedip[i].Tag = 'NON CONTEGGIATO') and (Parametri.CampiRiferimento.C90_W026TipoStraord = 'A') then
          Continue;
        if (ttimbraturedip[i].Tag <> 'NON CONTEGGIATO') and (Parametri.CampiRiferimento.C90_W026TipoStraord = 'N') then
          Continue;
        ...
        // verifica se le timbrature sono riferite al giorno precedente
      ...
      if (Parametri.CampiRiferimento.C90_W026TipoStraord = 'N') and
         (Parametri.CampiRiferimento.C90_W026Spezzoni = 'T') then
      begin
        for i:=0 to High(SpezzoniNonAbilitati) do
          if (SpezzoniNonAbilitati[i].TipoSpez <> 'FO') and
             (SpezzoniNonAbilitati[i].TipoAbil = 'NC') then
          begin
            // determina la posizione di ttimbraturedip[i] nel nuovo array
            x:=PosizioneIns(SpezzoniNonAbilitati[i].Inizio);
            SetLength(timbArr,Length(timbArr) + 1);
            // spostamento elementi
            for j:=Length(timbArr) - 1 downto x + 1 do
            begin
            ...
            // copia elemento
            timbArr[x].inserita:=False;//ttimbraturedip[i].inserita;
            timbArr[x].oralegsol:=False;//ttimbraturedip[i].oralegsol;
            timbArr[x].spezzata:=False;//ttimbraturedip[i].spezzata;
            timbArr[x].tag:='';//ttimbraturedip[i].tag;
            timbArr[x].tcausale_e:=CausaleVuota;//ttimbraturedip[i].tcausale_e;
            timbArr[x].tcausale_u:=CausaleVuota;//ttimbraturedip[i].tcausale_u;
            timbArr[x].tflagarr_e:='';//ttimbraturedip[i].tflagarr_e;
            timbArr[x].tflagarr_u:='';//ttimbraturedip[i].tflagarr_u;
            timbArr[x].tminutid_e:=SpezzoniNonAbilitati[i].Inizio;
            timbArr[x].tminutid_u:=SpezzoniNonAbilitati[i].Fine;
            timbArr[x].tpuntnomin:=0;//ttimbraturedip[i].tpuntnomin;
            timbArr[x].tpuntnominold:=0;//ttimbraturedip[i].tpuntnominold;
            timbArr[x].trilev_e:='';//ttimbraturedip[i].trilev_e;
            timbArr[x].trilev_u:='';//ttimbraturedip[i].trilev_u;
            // verifica se le timbrature sono riferite al giorno precedente
            if timbArr[x].tminutid_e >= 6000 then
