...
type
  ...
  public
    ...
    const
      ...
      D_ModRecupero:array[0..7] of TItemsValues = (
        ...
        (Item:'Anno precedente e anno corrente (solo Liq)'; Value:'LP'),
        (Item:'Anno corrente (Comp-Liq)';                   Value:'AC'),
        (Item:'Anno corrente (Liq-Comp)';                   Value:'AL'),
        (Item:'Anno corrente (solo Liq)';                   Value:'LA'),
      ...
      D_PropPtVGG:array[0..4] of TItemsValues = (
        (Item:'Perc.PartTime'; Value:'A'),
        (Item:'Fino a 2gg lav';Value:'D'),
        (Item:'Fino a 3gg lav';Value:'E'),
        (Item:'Su settimi';    Value:'B'),
        (Item:'Su quinti';     Value:'C')
        );
  end;
...
procedure TA016FCausAssenzeMW.DataModuleCreate(Sender: TObject);
begin
  inherited;
...
procedure TA016FCausAssenzeMW.T265BeforeDelete;
begin
  if T265.FieldByName('CODICE').AsString = Parametri.CampiRiferimento.C90_W026CausAbbattGiustAss then
    raise Exception.Create(Format(A000MSG_A016_ERR_FMT_CAUS_PARAZIENDALI,[VarToStr(T265.FieldByName('CODICE').medpOldValue)]));
end;
...
begin
  ...
  //Codice in uso nei parametri aziendali
  if (T265.State = dsEdit) and
     (T265.FieldByName('CODICE').medpOldValue <> T265.FieldByName('CODICE').Value) and
     (VarToStr(T265.FieldByName('CODICE').medpOldValue) = Parametri.CampiRiferimento.C90_W026CausAbbattGiustAss) then
    raise Exception.Create(Format(A000MSG_A016_ERR_FMT_CAUS_PARAZIENDALI,[VarToStr(T265.FieldByName('CODICE').medpOldValue)]));

  if (T265.State = dsEdit) and
     (VarToStr(T265.FieldByName('CODICE').medpOldValue) = Parametri.CampiRiferimento.C90_W026CausAbbattGiustAss) and
     (T265.FieldByName('INFLUCONT').medpOldValue <> T265.FieldByName('INFLUCONT').Value) and
     (VarToStr(T265.FieldByName('INFLUCONT').Value) <> 'D') then
    raise Exception.Create(Format(A000MSG_A016_ERR_FMT_INFLUCONT_DIMINUISCEORE,[VarToStr(T265.FieldByName('CODICE').medpOldValue)]));
  ...
  if R180In(T265.FieldByName('PROPPTVGG').AsString,['B','C']) then
  try
    if T265.FieldByName('UMisura').AsString <> 'G' then
      raise Exception.Create(A000MSG_A016_ERR_PROPPTVGG_UM);
    if Q260.FieldByName('CONTASOLARE').AsString = 'S' then
      raise Exception.Create(A000MSG_A016_ERR_PROPPTVGG_RAGGR);
    if Q260.FieldByName('RESIDUABILE').AsString = 'S' then
      raise Exception.Create(A000MSG_A016_ERR_PROPPTVGG_RAGGR);
   except
     if Assigned(VisualizzaTab) then
       VisualizzaTab(3);
...
procedure TA016FCausAssenzeMW.GetSourceSQL(ObjPlSql:String);
begin
