  ...
  object selT765: TOracleDataSet
    SQL.Strings = (
      'select T.CODICE,T.DESCRIZIONE, T.TIPOQUOTA,'
        ...
        '    '#39'D'#39','#39'Saldo collettivo valutativo'#39','#39'N'#39','#39'Saldo Coll. val. non ' +
        'obbl.'#39','#39'Q'#39','#39'Quota quantitativa'#39','#39'P'#39','#39'Penalizzazione'#39') DESC_TIPOQ' +
        'UOTA,'
      
        '  ACCONTI, CAUSALE_ASSESTAMENTO, IMPOSTA_MESE_COMP_MAX, RIF_SALD' +
        'O_ANNO_CORR, GIORNI_MESE'
      'from T765_TIPOQUOTE T '
      'where DECORRENZA = (SELECT MAX(DECORRENZA) FROM T765_TIPOQUOTE'
      '                     WHERE CODICE = T.CODICE'
      '                       AND DECORRENZA <= :DATA)'
