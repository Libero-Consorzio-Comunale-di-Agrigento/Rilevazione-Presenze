...
type
  ...
  TA145FComVisiteFiscaliMW = class(TR005FDataModuleMW)
    ...
    selT047NOMINATIVO: TStringField;
    selT047DETTAGLI_INDIRIZZO: TStringField;
...
begin
  ...
  begin
    ...
    Add('MedLegComune',ftString,60,False);
    Add('MedLegCap',ftString,5,False);
    Add('MedLegTel',ftString,15,False);
    Add('MedLegEmail',ftString,200,False);
    ...
    Add('DettagliIndirizzo',ftString,50,False);
...
begin
  ...
  if (not datiElab.bProl) and
     (selT047.FieldByName('OPERAZIONE').AsString = 'I') and
     (not selT047.FieldByName('NUOVA_DATA_FINE').IsNull) then
    Exit;
  ...
  TabellaStampa.FieldByName('DettagliIndirizzo').Value:=selT047.FieldByName('DETTAGLI_INDIRIZZO').Value;
  ...
  // dati di domicilio
  TabellaStampa.FieldByName('Nominativo').AsString:=selT047.FieldByName('NOMINATIVO').AsString;
...
// Indirizzo, Comune, Cap, Provincia e Telefono + Nominativo
var
  Domicilio,Indirizzo,DettIndirizzo,Comune,Cap,Provincia,Telefono,Nominativo: String;
begin
  Indirizzo:=Trim(TabellaStampa.FieldByName('Indirizzo').AsString);
  DettIndirizzo:=Trim(TabellaStampa.FieldByName('DettagliIndirizzo').AsString);
  Comune:=TabellaStampa.FieldByName('Comune').AsString;
  Cap:=TabellaStampa.FieldByName('Cap').AsString;
  Provincia:=TabellaStampa.FieldByName('Provincia').AsString;
  Telefono:=TabellaStampa.FieldByName('Telefono').AsString;
  Nominativo:=TabellaStampa.FieldByName('Nominativo').AsString;
  ...
  // se è specificato il nominativo lo include nei dati di domicilio
  if Nominativo <> '' then
    Domicilio:=Format('c/o %s',[Nominativo]) + IfThen(Domicilio = '','',' ') + Domicilio;

  // concatena dettagli indirizzo (solo per stampa elenco)
  if datiElab.iTipoStampa = 0 then
    if DettIndirizzo <> '' then
      Domicilio:=Domicilio + CRLF + '(' + DettIndirizzo + ')';
