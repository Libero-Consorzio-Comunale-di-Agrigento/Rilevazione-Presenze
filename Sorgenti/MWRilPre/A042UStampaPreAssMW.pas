...
type
  ...
  TA042FStampaPreAssMW = class(TR005FDataModuleMW)
    ...
    procedure TabellaStampaAfterOpen(DataSet: TDataSet);
...
procedure TA042FStampaPreAssMW.TabellaStampaAfterOpen(DataSet: TDataSet);
begin
  inherited;
  if TabellaStampa.FindField('GRUPPO') <> nil then
    TabellaStampa.FieldByName('GRUPPO').DisplayWidth:=30;
end;

procedure TA042FStampaPreAssMW.TabellaStampaBeforePost(DataSet: TDataSet);
begin
  inherited;
  //Valorizzazioni orarie in minuti e ore.centesimi per estrazioni xls totalizzabili.
  with DataSet do
  begin
    if FindField('NumOre') <> nil then
    begin
      if FindField('NumOreCentesimi') <> nil then
        FieldByName('NumOreCentesimi').AsFloat:=R180OreMinuti(FieldByName('NumOre').AsString) / 60;
      if FindField('NumMinuti') <> nil then
        FieldByName('NumMinuti').AsInteger:=R180OreMinuti(FieldByName('NumOre').AsString);
    end;
    if FindField('TotaleConsecutive') <> nil then
    begin
      if FindField('TotaleOreCentesimi') <> nil then
        FieldByName('TotaleOreCentesimi').AsFloat:=R180OreMinuti(FieldByName('TotaleConsecutive').AsString) / 60;
      if FindField('TotaleMinuti') <> nil then
        FieldByName('TotaleMinuti').AsInteger:=R180OreMinuti(FieldByName('TotaleConsecutive').AsString);
    end;
  end;
end;
...
begin
  ...
  begin
    // Se oggi non è presente alcuna timbratura potrei comunque dover segnare come presente il dipendente,
    // per es. se ieri ha iniziato un turno notturno ma oggi non lo ha ancora concluso (assenza timbratura
    // di uscita).
    if (R502ProDtM1.estimbprec = 'si') and (R502ProDtM1.verso_pre = 'E') and (R502ProDtM1.data_pre = R502ProDtM1.datacon - 1) then //...ieri è presente una timbratura di entrata e...
    begin
      // Ieri è presente una timbratura di ingresso.
      if R502ProDtM1.datacon < Date then
      begin
        // Se sto analizzando timbrature passate rispetto alla data di sistema e domani ho una timbratura
        // di uscita, vuol dire che il dipendente ha iniziato il turno ieri e lo ha terminato domani, quindi
        // oggi è presente
        if ((R502ProDtM1.estimbsucc = 'si') and (R502ProDtM1.verso_suc = 'U') and (R502ProDtM1.data_suc >= R502ProDtM1.datacon + 1)) then
          Result:=True;
      end
      else if R502ProDtM1.datacon >= Date then  // Se stiamo guardando una timbratura di oggi o del futuro...
      begin
        if (
             ((R502ProDtM1.TipoOrario = 'E') and (R502ProDtM1.PeriodoLavorativo = 'T1')) //... l'orario è a turni con scavalco della mezzanotte oppure...
             or
             (R502ProDtM1.caus_pre <> '') // ... la timbratura di entrata di ieri ha una causale (usata per indicare un turno notturo)...
           )
        then
        begin
          // In questo caso il dipendente ha iniziato un turno notturo ieri, ma non ha timbrato l'uscita perchè è ancora in servizio.
          // Lo considero presente.
          Result:=True;
        end;
      end;
    end;
...
begin
  ...
  TabellaStampa.FieldDefs.Add('TotaleOreCentesimi',ftFloat,0,False);
  TabellaStampa.FieldDefs.Add('TotaleMinuti',ftInteger,0,False);
...
begin
  ...
  TabellaStampa.FieldDefs.Add('NumOreCentesimi',ftFloat,0,False);
  TabellaStampa.FieldDefs.Add('NumMinuti',ftInteger,0,False);
  TabellaStampa.IndexDefs.Clear;
