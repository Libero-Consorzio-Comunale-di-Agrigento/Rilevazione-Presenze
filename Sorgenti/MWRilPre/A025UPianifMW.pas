...
begin
  ...
  //Annullare la modifica e uscire crea problemi cul cloud. gestito come eccezione
  (*
  ...
  *)
  if SelT080.FieldByName('DATA').IsNull then
    raise Exception.Create(A000MSG_A025_ERR_DATA);

  if SelT080.FieldByName('ORARIO').IsNull and SelT080.FieldByName('INDPRESENZA').IsNull and SelT080.FieldByName('DATOLIBERO').IsNull and SelT080.FieldByName('NOTE').IsNull then
    raise Exception.Create(Format(A000MSG_A025_ERR_FMT_DATI,[SelT080.FieldByName('DATOLIBERO').DisplayLabel]));
  ...
  if (SelT080.FieldByName('DATA').AsString <> '') and
     ...
     (SelT080.FieldByName('INDPRESENZA').AsString = '') and
     (SelT080.FieldByName('NOTE').AsString = '') then
    ...
    {else if SelT080.FieldByName('NOTE').AsString = '' then
      raise Exception.Create('Sono richiesti o il codice Orario o il codice Indennità o il codice ' +
        SelT080.FieldByName('DATOLIBERO').DisplayLabel + '!')};
...
begin
  ...
  if (not PulOrarioChecked) and (not PulIndennitaChecked) and (not PulDatoChecked) and
    ...
    if not DatoLiberoEnabled then
      ...
      raise Exception.Create(Format(A000MSG_A025_ERR_FMT_DATI,[DatoLiberoCaption]));
  if (Orario = '') and
    ((Turno1 <> '') or (Turno2 <> '') or
