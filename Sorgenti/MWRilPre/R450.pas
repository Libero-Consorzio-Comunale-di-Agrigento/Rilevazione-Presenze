...
const
  XP_R450_BANCAORE_ABBATTIBILE_v1049 = '<BANCAORE_ABBATTIBILE_v1049>';
  XP_R450_SALDONEGATIVOMINIMO_TIPO2  = '<SALDONEGATIVOMINIMO_TIPO2>';
  XP_R450_SALDIMOBILI_RES_T130       = '<SALDIMOBILI_RES_T130>';
  XP_R450_SALDIMOBILI_LIQNODIST      = '<SALDIMOBILI_LIQNODIST>';
  XP_R450_NEGATIVI_COMPENSABILI      = '<NEGATIVI_COMPENSABILI>';

  GESTIONE_FAD_NO = 'NO';
  GESTIONE_FAD_OE = 'OE'; //Ore escluse
  GESTIONE_FAD_OL = 'OL'; //Ore liquidabili
  GESTIONE_FAD = GESTIONE_FAD_NO;
...
type
  ...
  TRiepilogoMese = record
    ...
    RiepAssCompensateAnno:Integer;
    RiepAssCompensateMese:Integer;
  ...
  private
    ...
    FRiepAssCompensateAnno:Integer;
    FRiepAssCompensateMese:Integer;
    ...
    OrePermessi           :TOrePermessi;
    DescT025              :String;
    FXParam               :String;
    SessioneOracleR450    :TOracleSession;
    FConservaConteggiUltimoMese: Boolean;
    ...
    function GetNoteAcquistoBuoniTicket:String;
    ...
    function GetOreCausLimiteEccRes:LongInt;
    ...
    procedure incFasceMaggiori(value:Integer; var dest:array of Integer; source:array of Integer);
    //Iter autorizzativo straordinario
    function GetIterAutStrOreCausalizzate: Integer;
    function GetXParam(Param: String): Boolean;
    function GetRiepassCompensazioneDisponibile(Tipo: String = 'ANNO'): Integer;
    function GetOrePermesso: TOrePermessi;
  public
    { Public declarations }
    ...
    SaldoMobile_EscludeRecPrec:Boolean;  //LASPEZIA
    CausaliCompensabili,CausaliCompAnnSaldo,CausaliCompAnnAddebPaghe:String;
    CausaliCompensabiliMensili,CausaliCompMensSaldo,CausaliCompMeseRecNeg:String;
    RiepassCompensabiliAnno,RiepassCompensabiliMese:String;
    progress400:Integer;
    anno400:Integer;
    mese400:Integer;
    ...
    RecuperoDebitoPeriodicita: String;
    ...
    tfasce:array [1..MaxFasce] of String;
    tmaggioraz:array[1..MaxFasce] of Integer;
    ...
    IterAutStrCausale     :String;
    procedure DeallocaQueryStampa;
    procedure ResetConteggi;
    procedure ParametrizzazioneQueryStampa(Q:Integer);
    ...
    property OrePermesso:TOrePermessi read GetOrePermesso;

    property RiepassCompensazioneDisponibile[Tipo:String]:Integer read GetRiepassCompensazioneDisponibile;
    ...
    property NoteAcquistoBuoniTicket:String read GetNoteAcquistoBuoniTicket;
    ...
    property TipoConteggio:Integer read GetTipoConteggio;
    ...
    property OreCausLimiteEccRes:LongInt read GetOreCausLimiteEccRes;
    ...
    property PercPT:Real read GetPercPT;
    ...
    property RiepAssCompensateMese:LongInt read FRiepAssCompensateMese;
    property RiepAssCompensateAnno:LongInt read FRiepAssCompensateAnno;
    //Iter autorizzativo straordinario
    property IterAutStrOreCausalizzate:Integer read GetIterAutStrOreCausalizzate;
    property XParam[Param:String]:Boolean read GetXParam;
    property ConservaConteggiUltimoMese: Boolean read FConservaConteggiUltimoMese write FConservaConteggiUltimoMese;
  end;
...
begin
  ...
  selT025.BeforeOpen:=TSessioneIrisWIN.BeforeOpenEvent;
  selT026.BeforeOpen:=TSessioneIrisWIN.BeforeOpenEvent;
  selT275.BeforeOpen:=TSessioneIrisWIN.BeforeOpenEvent;
  selT305.BeforeOpen:=TSessioneIrisWIN.BeforeOpenEvent;
  selT025.Open;
  selT026.Open;
  selT275.Open;
  selT305.Open;
  ...
  FConservaConteggiUltimoMese:=False;

  OrePermessi.Progressivo:=-1;
  OrePermessi.Data:=0;
end;
...
procedure TR450DtM1.ResetConteggi;
begin
  selT070.SetVariable('PROGRESSIVO',0);
end;
...
function TR450DtM1.GetTipoConteggio:Integer;
var xx:Integer;
begin
  Result:=4;
  Serbatoi[1]:='CP';
...
begin
  ...
  SaldoMobile_EscludeRecPrec:=False;
  CausaliCompensabili:='';
  CausaliCompAnnAddebPaghe:='D';
  CausaliCompMeseRecNeg:='D';
  CausaliCompensabiliMensili:='';
  RiepassCompensabiliAnno:='';
  RiepassCompensabiliMese:='';
  CausaliCompMensSaldo:='M';
  CausaliCompAnnSaldo:='A';
  TroncaEccedenze:='CL';
  TroncaLiquidabile:='CL';
  LimiteEccRes:='N';
  LimiteEccLiq:='N';
  ...
  if selStorico.LocDatoStorico(R180FineMese(EncodeDate(AnnoCorr400,MeseCorr400,1))) and
    ...
    begin
      ...
      Serbatoi[FieldByName('LiqAtt').AsInteger]:='LA';
      try
        FXParam:=FieldByName('XPARAM').AsString;
      except
        //Settaggio di default se non esiste colonna XParam
        FXParam:=XP_R450_BANCAORE_ABBATTIBILE_v1049;
        if Pos('<>',DescT025) > 0 then
          FXParam:=FXParam + XP_R450_SALDONEGATIVOMINIMO_TIPO2;
      end;
      ...
      try SaldoMobile_AbbatteMax:=R180OreMinuti(FieldByName('Abbattimento_Mobile_Max').AsString); except end;
      ...
      try SaldoMobile_EscludeRecPrec:=FieldByName('Abbatt_Mobile_Esclude_Recprec').AsString = 'S'; except end;
      ...
      try RecuperoDebitoMax:=R180OreMinuti(FieldByName('RecuperoDebito_Max').AsString); except end;
      try RecDebitoMaxTollerato:=R180OreMinuti(FieldByName('RecDebito_MaxTollerato').AsString); except end;
      try RecuperoDebitoTipo:=FieldByName('RecuperoDebito_Tipo').AsString; except end;
      try RecuperoDebitoPeriodicita:=FieldByName('RecuperoDebito_Periodicita').AsString; except end;
      ...
      try
        if not FieldByName('LimiteCompA').IsNull then
          LimiteCompA:=R180OreMinuti(FieldByName('LimiteCompA').AsString);
      except end;
      try
        if not FieldByName('Mesi_SaldoAnnoPrec').IsNull then
          MesiSaldoAnnoPrec:=FieldByName('Mesi_SaldoAnnoPrec').AsInteger;
      except end;
      ...
      try RiepAssCompensabiliAnno:=FieldByName('RIEPASS_COMPENSABILI_ANNO').AsString; except end;
      try RiepAssCompensabiliMese:=FieldByName('RIEPASS_COMPENSABILI_MESE').AsString; except end;
      try CausaliCompAnnAddebPaghe:=FieldByName('CAUSALI_COMP_ANN_ADDEB_PAGHE').AsString; except end;
      try CausaliCompMeseRecNeg:=FieldByName('CAUSALI_COMP_MESE_RECNEG').AsString; except end;
      try CausaliCompMensSaldo:=FieldByName('CAUSALI_COMP_MENS_SALDO').AsString; except end;
      try CausaliCompAnnSaldo:=FieldByName('CAUSALI_COMP_ANN_SALDO').AsString; except end;
      ...
      try LimiteEccResCaus:=FieldByName('LIMITE_MM_ECCRES_CAUSALI').AsString; except end;
      if (LimiteEccResCaus = '') and
         (UpperCase(Parametri.RagioneSociale) = 'A.O.U. CITTA'' DELLA SALUTE E DELLA SCIENZA DI TORINO')
      then
        LimiteEccResCaus:='WREC';

      try LimiteLiquidabileAnnuo:=FieldByName('TRASF_SUPERO_LIQANN').AsString; except end;
      try SogliaCompLiq:=R180OreMinuti(FieldByName('SOGLIA_COMP_LIQ').AsString); except end;
      try ArrSogliaCompLiq:=R180OreMinuti(FieldByName('ARR_SOGLIA_COMP_LIQ').AsString); except end;
      try SaldoNegativoMinimo:=R180OreMinuti(FieldByName('SALDO_NEGATIVO_MINIMO').AsString); except end;
      ...
      try ArrotondaCP:=R180OreMinuti(FieldByName('ARRREC_COMPPREC').AsString); except end;
      try ArrotondaCA:=R180OreMinuti(FieldByName('ARRREC_COMPATT').AsString); except end;
      try ArrotondaLP:=R180OreMinuti(FieldByName('ARRREC_LIQPREC').AsString); except end;
      try ArrotondaLA:=R180OreMinuti(FieldByName('ARRREC_LIQATT').AsString); except end;
      try ArrRecScostNeg:=R180OreMinuti(FieldByName('ARRREC_SCOSTNEG').AsString); except end;
      ...
      try BancaOreMensArr:=R180OreMinuti(FieldByName('BANCA_ORE_MENS_ARR').AsString); except end;
      ...
      try IterAutStrCausale:=FieldByName('ITER_AUTSTR_CAUSALE').AsString; except end;
      ...
      try RipComInclusiSaldo:=FieldByName('RIPCOM_INCLUSI_SALDO').AsString = 'S'; except end;

      //Parametri per il passaggio anno
      try
        CampoNullo:=StringReplace(StringReplace(Trim(FieldByName('PA_LIMITE').AsString),'.','',[]),':','',[]) = '';
        PALimite:=IfThen(CampoNullo,9999999,R180OreMinuti(FieldByName('PA_LIMITE').AsString));
      except end;
      try
        CampoNullo:=StringReplace(StringReplace(Trim(FieldByName('PA_LIMITESALDOATT').AsString),'.','',[]),':','',[]) = '';
        PALimiteSaldoAtt:=IfThen(CampoNullo,9999999,R180OreMinuti(FieldByName('PA_LIMITESALDOATT').AsString));
      except end;
      try
        CampoNullo:=StringReplace(StringReplace(Trim(FieldByName('PA_LIMITESALDOPREC').AsString),'.','',[]),':','',[]) = '';
        PALimiteSaldoPrec:=IfThen(CampoNullo,9999999,R180OreMinuti(FieldByName('PA_LIMITESALDOPREC').AsString));
      except end;
...
function TR450DtM1.GetXParam(Param: String): Boolean;
begin
  Result:=Pos(Param,FXParam) > 0
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      cdsStrAnnuo.FieldDefs.Add('Perc_PT',ftFloat);
      cdsStrAnnuo.FieldDefs.Add('StrAnno_PT',ftInteger);
      cdsStrAnnuo.FieldDefs.Add('SupplAnno_PT',ftInteger);
    end;
  ...
  if not cdsStrAnnuo.Locate('Data',EncodeDate(AnnoCorr400,MeseCorr400,1),[]) then
    ...
    begin
      ...
      cdsStrAnnuo.FieldByName('Perc_PT').AsFloat:=GetVariable('Perc_PT');
      cdsStrAnnuo.FieldByName('StrAnno_PT').AsInteger:=GetVariable('MaxStrAnno_PT');
      cdsStrAnnuo.FieldByName('SupplAnno_PT').AsInteger:=GetVariable('MaxSupplAnno_PT');
      cdsStrAnnuo.Post;
    end;
  ...
  if cdsStrAnnuo.Locate('Data',EncodeDate(Annocorr400,MeseCorr400,1),[]) then
    ...
    (*Perc_PT viene restituito dalla proprietà specifica PercPT
    else if UpperCase(Dato) = 'PERC_PT' then
      Result:=cdsStrAnnuo.FieldByName('Perc_PT').AsInteger
    *)
    else if UpperCase(Dato) = 'STRAORD_PT' then
      Result:=cdsStrAnnuo.FieldByName('StrAnno_PT').AsInteger
    else if UpperCase(Dato) = 'SUPPL_PT' then
      Result:=cdsStrAnnuo.FieldByName('SupplAnno_PT').AsInteger;
end;

function TR450DtM1.GetPercPT:Real;
begin
  Result:=0;
  GetEccAutAnno('');
  if cdsStrAnnuo.Locate('Data',EncodeDate(Annocorr400,MeseCorr400,1),[]) then
    Result:=cdsStrAnnuo.FieldByName('Perc_PT').AsFloat;
end;
...
begin
  ...
  if LimiteEccLiq = 'Z' then
    ...
    NewStrAutMen:=R180OreMinuti('9999.59');
  if LimiteEccRes = 'Z' then
    NewEccResAutMen:=0
  else
    NewEccResAutMen:=R180OreMinuti('9999.59');
  GetLiq:=False;
  GetRes:=False;
  (*TroncaEccedenze:='S';
  TroncaLiquidabile:='S';*)
  ...
  begin
    ...
    repeat
      ...
      begin
        //Residuabile
        inc(TeorResAutMen,R180OreMinuti(FieldByName('ORE_TEORICHE').AsString));
        if not FieldByName('ORE').IsNull then
        begin
          if NewEccResAutMen = R180OreMinuti('9999.59') then
            NewEccResAutMen:=0;
          inc(NewEccResAutMen,R180OreMinuti(FieldByName('ORE').AsString));
          GetRes:=True;
        end;
      end
      else
      begin
        //Liquidabile
        inc(TeorStrAutMen,R180OreMinuti(FieldByName('ORE_TEORICHE').AsString));
        if not FieldByName('ORE').IsNull then
        begin
          ...
          begin
            inc(StrAutMenNoCau,R180OreMinuti(FieldByName('ORE').AsString));
            if NewStrAutMen = R180OreMinuti('9999.59') then
              NewStrAutMen:=0;
            inc(NewStrAutMen,R180OreMinuti(FieldByName('ORE').AsString));
            GetLiq:=True;
          end;
        end;
      end;
  ...
  //TORINO_CITTADELLASALUTE: gestione del limite mensile da riepilogo causale WREC
  if (not GetRes) and (*(TroncaEccedenze = 'EG') and*) (LimiteEccResCaus <> '') then
  begin
    //Lettura totale ore rese con LimiteEccResCaus (da RiepPres)
    if OreCausLimiteEccRes > 0 then
    begin
      NewEccResAutMen:=OreCausLimiteEccRes;
      GetRes:=True;
    end;
  end;
  ...
  begin
    ...
    begin
      if selT810.SearchRecord('CAMPO1;ANNO;MESE',VarArrayOf([selStorico.FieldByName(CL1).AsString,AnnoCorr400,MeseCorr400]),[srFromBeginning]) then
        NewStrAutMen:=R180OreMinuti(selT810.FieldByName('Valore').AsString);
    end
    else
    begin
  ...
  begin
    ...
    begin
      if selT811.SearchRecord('CAMPO1;ANNO;MESE',VarArrayOf([selStorico.FieldByName(CR1).AsString,AnnoCorr400,MeseCorr400]),[srFromBeginning]) then
        NewEccResAutMen:=R180OreMinuti(selT811.FieldByName('Valore').AsString);
    end
    else
    begin
...
function TR450DtM1.GetOreCausLimiteEccRes:LongInt;
{Ore rese nel mese dalle causali indicate dal parametro LIMITE_MM_ECCLIQ_CAUSALI: casuali che determinano il limite mensile al posto della T820}
var S:String;
begin
  Result:=0;
  for S in LimiteEccResCaus.Split([',']) do
  begin
    if selT275.SearchRecord('CODICE',S,[srFromBeginning]) and
       (selT275.FieldByName('ORENORMALI').AsString <> 'A') and
       (IndiceRiepPres(S) >= 0) then
      Result:=R180SommaArray(RiepPres[IndiceRiepPres(S)].OreReseMese);
  end;
end;
...
var AnnoOld,AnnoResidui,i,yy:Integer;
  ...
  begin
    ...
    L07.notggmesVar:=0;
    L07.notminmesVar:='';
    L07.RiepAssCompensateAnno:=0;
    L07.RiepAssCompensateMese:=0;
    salmeseatt:=0;
    OreEsclComp:=0;
    NFasceMese:=0;
    RiposiNonFruitiGG:=0;
    ...
    FRiepassCompensateAnno:=0;
    FRiepassCompensateMese:=0;
    for i:=0 to High(RiepPres) do
    begin
...
begin
  ...
  DataInizioConteggi:=ConteggiMobili;
  ...
  begin
    SetVariable('PROGRESSIVO',Progress400);
    SetVariable('DATA',DataInizioConteggi);
    Close;
    Open;
  end;
  if (Progress400 <> selT070.GetVariable('PROGRESSIVO')) or
     (DataInizioConteggi <> selT070.GetVariable('DATA1')) or
  ...
  begin
    ...
    begin
      ...
      SetVariable('PROGRESSIVO',Progress400);
      SetVariable('DATA1',DataInizioConteggi);
    ...
    begin
      Close;
      SetVariable('PROGRESSIVO',Progress400);
      SetVariable('DATA1',R180AddMesi(DataInizioConteggi,-1));
      SetVariable('DATA2',EncodeDate(Anno400,Mese400,1));
      Open;
    end;
    R180SetVariable(selT011,'PROGRESSIVO',Progress400);
    R180SetVariable(selT011,'DATA1',DataInizioConteggi);
  ...
  if ConservaConteggiUltimoMese then
    //Non si vuole azzerare i dati mensili in modo che rimangano disponibili al chiamante (per es. R600),
    //indipendentemente dal fatto che siano quelli della scheda richiesta (Anno400,Mese400) o di una scheda precedente
    exit;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        MaxResiduo:=R180OreMinuti(VarToStr(selT275.Lookup('CODICE',FieldByName('Causale').AsString,'RESIDUABILE')));
        if (MaxResiduo > 0) and
        ...
        begin
          ...
          for xx:=1 to 6 do RiepPresResiduo[k].Residuo[xx]:=R180OreMinuti(FieldByName('ORE_FASCIA' + IntToStr(xx)).AsString);
          for xx:=1 to 6 do RiepPresResiduo[k].AnnoPrec[xx]:=R180OreMinuti(FieldByName('ORE_FASCIA' + IntToStr(xx)).AsString);
    ...
    for i:=0 to High(RiepPres) do
    begin
      MaxResiduo:=R180OreMinuti(VarToStr(selT275.Lookup('CODICE',RiepPres[i].Causale,'RESIDUABILE')));
      ...
      if Pos('<URCP>',DescT025) > 0 then
        ...
        begin
          ...
          begin
            MaxResiduo:=0;
            for xx:=1 to 6 do
              MaxResiduo:=MaxResiduo + R180OreMinuti(FieldByName('ORE_FASCIA' + IntToStr(xx)).AsString);
            MaxResiduo:=min(MaxResiduo,R180OreMinuti(VarToStr(selT275.Lookup('CODICE',RiepPres[i].Causale,'RESIDUABILE'))));
          end
          else
            MaxResiduo:=0;
        end;
...
begin
  ...
  begin
    ...
    if selT820.SearchRecord('ANNO;MESE;CAUSALE',VarArrayOf([AnnoCorr400,MeseCorr400,RiepPres[i].Causale]),[srFromBeginning]) then
      RiepPres[i].LimiteMensile:=R180OreMinuti(selT820.FieldByName('ORE').AsString);
  end;
...
function TR450DtM1.GetRiepassCompensazioneDisponibile(Tipo: String = 'ANNO'): Integer;
var Causali,Causale:String;
    Giustif:TGiustificativo;
    ResiduoMese,ResiduoAnno:Integer;
    Data:TDateTime;
begin
  Result:=0;
  ResiduoMese:=0;
  ...
  if Tipo = 'ANNO' then
    Causali:=RiepassCompensabiliAnno
  else if Tipo = 'MESE' then
    Causali:=RiepassCompensabiliMese;

  CreazioneR600;
  for Causale in Causali.Split([',']) do
  begin
    Giustif.Causale:=Causale;
    Giustif.Modo:='N';
    Data:=R180FineMese(EncodeDate(Anno400,Mese400,1));
    R600DtM.GetAssenze(Progress400,Data,Data,Data,Giustif);
    inc(ResiduoMese,trunc(R600DtM.ValResiduo));
    if Mese400 < 12 then
    begin
      Data:=R180FineMese(EncodeDate(Anno400,12,1));
      R600DtM.GetAssenze(Progress400,Data,Data,Data,Giustif);
    end;
    inc(ResiduoAnno,trunc(R600DtM.ValResiduo));

    Result:=min(ResiduoMese,ResiduoAnno);
  end;
end;
...
begin
  ...
  if selT820.SearchRecord('ANNO;MESE;CAUSALE',VarArrayOf([AnnoCorr400,MeseCorr400,RiepPres[k].Causale]),[srFromBeginning]) then
    RiepPres[k].LimiteMensile:=R180OreMinuti(selT820.FieldByName('ORE').AsString);
...
var i,h,Residuo:Integer;
begin
  Residuo:=R180SommaArray(RiepPres[k].Residuo);
  if (Residuo <= 0) or (Abbatti <= 0) then
    exit;

  //Abbattimento della parte solo compensabile
  for i:=1 to MaxFasce do
  begin
  ...
  //Abbattimento anche della parte liquidabile
  if (Abbatti > 0) and (Residuo > 0) then
  begin
    for i:=1 to MaxFasce do
    begin
...
procedure TR450DtM1.CreazioneR600;
begin
  if R600DtM = nil then
  begin
...
begin
  ...
  //Aosta_ASL: devono poter liquidare mesi pregressi anche se esiste già liquidazione
  //parametro T275.LIQUIDAZIONE_MESIPREC + ore escluse dalle normali
  if Result then
  begin
    if (VarToStr(selT275.Lookup('CODICE',Causale,'ORENORMALI')) = 'A') and
       (VarToStr(selT275.Lookup('CODICE',Causale,'LIQUIDAZIONE_MESIPREC')) = 'S')
     then
       Result:=False;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      //inc(Result,RiepPres[i].RecuperoMese);
      Break;
    end;
  end;
end;
...
begin
  ...
  locLeggiTabella:=LeggiTabella;
  if (not LeggiTabella) and
     (XParam[XP_R450_SALDIMOBILI_RES_T130]) then
  begin
    selT130.SetVariable('ANNO1',AnnoCorr400);
    selT130.SetVariable('ANNO2',AnnoCorr400);
    selT130.Close;
    selT130.Open;
    locLeggiTabella:=selT130.SearchRecord('ANNO',AnnoCorr400,[srFromBeginning]);
  end;

  if locLeggiTabella then
  begin
    with selT130 do
      ...
      begin
        s_trovato:='si';
        l13_sallav_min:=R180OreMinuti(FieldByname('SaldoOreLav').AsString);
        l13_respo_min:=R180OreMinuti(FieldByname('SaldoOrePo').AsString);
        l13_rescnl_min:=R180OreMinuti(FieldByname('OreCompensabili').AsString);
        l13_resrco_min:=R180OreMinuti(FieldByname('RiposiComp').AsString);
        l13_banca_ore:=R180OreMinuti(FieldByname('Banca_Ore').AsString);
...
begin
  ...
  if (l13_sallav_min - l13_respo_min) > l13_rescnl_min then
    stranno_prec:=l13_sallav_min - l13_respo_min - l13_rescnl_min
  else
    stranno_prec:=0;
  ...
  if (AnnoCorr400 <> Anno400) or (MeseCorr400 <> mese400) or (caller = 'Generico') then
    ...
    begin
      ...
      begin
        ...
        L07.debormes:=R180OreMinuti(FieldByName('DebitoOrario').AsString);
        L07.debggtot:=0;//R180OreMinuti(FieldByName('DebitoGG_Tot').AsString);
        L07.debpomes:=R180OreMinuti(FieldByName('DebitoPo').AsString);
        ...
        L07.eccsolocompmes:=R180OreMinuti(FieldByName('OreEccedComp').AsString);
        L07.EccSoloCompMesOltreSoglia:=R180OreMinuti(FieldByName('OreEccedCompOltreSoglia').AsString);
        L07.vareccliqmes:=R180OreMinuti(FieldByName('OreVariazEcc').AsString);
        L07.scostnegmes:=R180OreMinuti(FieldByName('ScostNeg').AsString);
        //ASTI_ASL: negativi comopensabili
        L07.compnegmes:=0;
        if XParam[XP_R450_NEGATIVI_COMPENSABILI] then
          L07.compnegmes:=StrToIntDef(LeggiValoreT077(I011_NEGATIVI_COMPENSABILI, EncodeDate(AnnoCorr400,MeseCorr400,1)),0);
        L07.ripcommes:=R180OreMinuti(FieldByName('RipCom').AsString);
        L07.abbripcommes:=R180OreMinuti(FieldByName('AbbRipCom').AsString);
        L07.NFasce:=0;
        L07.AddebitoPaghe:=R180OreMinuti(FieldByName('AddebitoPaghe').AsString);
        (*RecAnnoCorr*)L07.abbannoattmes:=R180OreMinuti(FieldByName('RecAnnoCorr').AsString);
        (*RecAnnoPrec*)L07.abbannoprecmes:=R180OreMinuti(FieldByName('RecAnnoPrec').AsString);
        L07.abbliqannoattmes:=R180OreMinuti(FieldByName('RecLiqCorr').AsString);
        L07.abbliqannoprecmes:=R180OreMinuti(FieldByName('RecLiqPrec').AsString);
        L07.OreCompLiquidate:=R180OreMinuti(FieldByName('OreComp_Liquidate').AsString) + R180OreMinuti(FieldByName('BancaOre_Liq_Var').AsString);
        L07.BancaOreLiqVar:=R180OreMinuti(FieldByName('BancaOre_Liq_Var').AsString);
        L07.OreCompRecuperate:=R180OreMinuti(FieldByName('OreComp_Recuperate').AsString);
        L07.CarenzaObbligatoria:=R180OreMinuti(FieldByName('Carenza_Obbligatoria').AsString);
        (*RIETI*)L07.RiposiNonFruitiOre:=FieldByName('RiposiNonFruitiOre').AsString;
        L07.RiepAssCompensateAnno:=R180OreMinuti(FieldByName('RIEPASS_COMPENSATE_ANNO').AsString);
        L07.RiepAssCompensateMese:=R180OreMinuti(FieldByName('RIEPASS_COMPENSATE_MESE').AsString);
        ...
        begin
          ...
          repeat
            ...
            L07.tminlavmese[i2]:=R180OreMinuti(FieldByName('OreLavorate').AsString);
            L07.tmininturno[i2]:=R180OreMinuti(FieldByName('OreIndTurno').AsString);
            L07.tdatiassestamento[1].tminassest[i2]:=R180OreMinuti(FieldByName('Ore1Assest').AsString);
            L07.tdatiassestamento[2].tminassest[i2]:=R180OreMinuti(FieldByName('Ore2Assest').AsString);
            L07.tminstrmens[i2]:=R180OreMinuti(FieldByName('OreEccedGiorn').AsString);
            L07.tstrliquidatomm[i2]:=R180OreMinuti(FieldByName('OreStraordLiq').AsString);
            L07.tLiqNelMese[i2]:=R180OreMinuti(FieldByName('LiquidNelMese').AsString);
            inc(i2);
...
begin
  ...
  (*RIETI*)RiposiNonFruitiOre:=R180OreMinuti(L07.RiposiNonFruitiOre);
  FRiepAssCompensateAnno:=L07.RiepAssCompensateAnno;
  FRiepAssCompensateMese:=L07.RiepAssCompensateMese;
...
begin
  ...
  for i:=0 to High(RiepPres) do
    ...
    else if R180In(VarToStr(selT275.Lookup('CODICE',RiepPres[i].Causale,'ORENORMALI')),['B','D']) then
    begin
    ...
    begin
      //Alberto - 02/04/2015: Ore incluse nelle normali liquidate dal riepilogo delle presenze compensabili (MASSA_ASL1)
      inc(totcauscompliqmm,R180SommaArray(RiepPres[i].LiquidatoMese));
    end
end;
...
var i,j,xx,iRM,StrEsternoApp,app,app1,app2,DiffRecup,DiffRecupTot:Integer;
    ...
    appBancaOreAbbattibile:Integer;
    ...
    {Abbattimento saldo non usato entro MeseAbb}
    var i,AbbTemp:Integer;
    begin
      if selT026.FieldByName('TIPO_ABBATTIMENTO').AsString = 'P' then
      begin
        //Abbattimento anno prec. (ROMA_HSANDREA)
        if MeseCorr400 = MeseAbb then
        begin
          ScostNegDaAbbattere:=0;
          SaldoDaAbbattere:=0;
          if AbbattRifCompensabile = 'S' then
            inc(SaldoDaAbbattere,salcompannoprec_prec);
          if AbbattRifLiquidabile = 'S' then
            inc(SaldoDaAbbattere,salliqannoprec_prec);
          SaldoDaAbbattere:=max(0,min(SaldoDaAbbattere,salcompannoprec_prec + salliqannoprec_prec));
        end;
      end
      else
      begin
        //Gestione originale (Melegnano): Abbattimento del saldo non usato per coprire gli scostamenti negativi
      ...
      dec(salfmprecnetto,SaldoDaAbbattere);
      //Anno precedente
      if AbbattRifCompensabile = 'S' then
      begin
        //Abbattimento saldo compensabile anno precedente
        if BancaOrePreservata = 'S' then
          AbbTemp:=Max(0,salcompannoprec_prec - GetBOInclusaSaldi(l13_banca_ore_recuperata))
        else
    ...
    var i,j,k,x,Q,Q1,iRM,Abbattimento,BO,BOCorr,BOPrec:Integer;
    ...
    begin
      ...
      //Roma_HSAndrea: proposta di gestione degli abbattimenti considerando il liquidato come recupero entro il periodo mobile
      if XParam[XP_R450_SALDIMOBILI_LIQNODIST] and
        (Pos('L',SaldoMobile_SaldiUsati) > 0) and
        (LiquidazioneDistribuita = 'N') then
      begin
        for i:=x to High(RiepilogoMese) do
        begin
          Q1:=RiepilogoMese[i].OreLiqMese;
          if Q1 > 0 then
          begin
            for j:=max(x,i - MesiSaldoprec) to i - 1 do
            begin
              Q:=max(0,min(Q1,min(RiepilogoMese[j].SaldoMese,RiepilogoMese[j].StrLiqGGMese) - (RiepilogoMese[j].OreLiqMese + RiepilogoMese[j].OreLiqDalMese)));
              if Q > 0 then
              begin
                dec(Q1,Q);
                dec(RiepilogoMese[i].OreLiqMese,Q);
                inc(RiepilogoMese[i].RecuperoMensile,Q);
                inc(RiepilogoMese[j].OreLiqDalMese,Q);
              end;
            end;
          end;
        end;
      ...
      if SaldoMobile_EscludeRecPrec then //LASPEZIA
      begin
        //Se i conteggi mobili partono nell'anno corrente, il saldo anno prec e relativo recupero non devono abbattere il saldo abbattibile
        if DataInizioConteggi >= EncodeDate(AnnoCorr400,1,1) then
          SaldoPrec:=SaldoPrec + RiepilogoMese[x].RecuperoAnnoPrec + RiepilogoMese[x].RecuperoAnnoAtt;
        //Limito il saldo utilizzabile al saldo mese
        SaldoPrec:=min(SaldoPrec,RiepilogoMese[x].SaldoMese);
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (AnnoCorr400 = Anno400) and (MeseCorr400 = Mese400) and (iRiep >= 0) then
      ...
      //Allineamento dei dati effettivi del riepilogo della causale
      for i:=1 to MaxFasce do
      begin
    ...
    var i,j,k,SaldiMese,OreLiquidate,SaldoPrec,
        SaldoFin,RecupMesePrec,OffsetRecupero:Integer;
        D:TDateTime;
    begin
      FAddebitoPagheRec:=0;
      if RecuperoDebito < 0 then
        exit;
      //Se periodicità fissa, proseguo solo se MeseCorr400 corrisponde al mese del fine periodo
      if (RecuperoDebitoPeriodicita = 'F') and (MeseCorr400 mod RecuperoDebito > 0) then
        exit;
      SaldiMese:=0;
      OreLiquidate:=0;
      SaldoPrec:=0;
      ...
      //Se è anche in uso la periodicità fissa, devo considerare il RiepilogoMese in modo diverso
      if (PeriodicitaAbbattimento = 'F') and (MesiSaldoPrec > 0) then
        D:=R180AddMesi(D,1);
      ...
      for i:=0 to High(RiepilogoMese) do
        ...
        begin
          ...
          SaldoFin:=0;
          OffsetRecupero:=IfThen((PeriodicitaAbbattimento = 'F') and (MesiSaldoPrec > 0),0,1);
          for j:=i + OffsetRecupero to High(RiepilogoMese) do
          begin
          ...
          if (PeriodicitaAbbattimento = 'F') and (MesiSaldoPrec > 0) then
            SaldoPrec:=RiepilogoMese[i].SaldoFineMesePrec
          else
  ...
  procedure CompensazioneDaRiepAssenze(TipoComp:String = 'A');
  {Compensazione del negativo con le ore residue da riepilogo assenze}
  var i,k,m,m2,Saldo,Saldo2,Saldo3:Integer;
  begin
    if TipoComp = 'A' then
    begin
      Saldo:=salannoatt;
      m:=L07.RiepAssCompensateAnno;
    end
    else
    begin
      Saldo:=salmeseatt;
      m:=L07.RiepAssCompensateMese;
    end;
//    with TStringList.Create do
//    try
//      if TipoComp = 'A' then
...
//          Continue;
        ...
        //if (TipoComp = 'A') or (CausaliCompMensSaldo = 'A') then
//        if ((TipoComp = 'A') and (CausaliCompAnnSaldo = 'A'))
//        or ((TipoComp = 'M') and (CausaliCompMensSaldo = 'A'))
//        then
//        begin
//          //Considero tutta la disponibilità annuale
//          Saldo2:=Saldo;
//          AbbattiRiepPres(k,Saldo);
//          m:=Saldo2 - Saldo;
//        end
//        else// if TipoComp = 'M' then
//        if ((TipoComp = 'A') and (CausaliCompAnnSaldo = 'M'))
//        or ((TipoComp = 'M') and (CausaliCompMensSaldo = 'M'))
//        then
//        begin
//          //Considero la sola disponibilità mensile
//          Saldo2:=Saldo;
...
//        inc(RiepPres[k].CompensabileAnnoEff,m);
        ...
        inc(salannoatt,m);
        inc(salannonetto,m);
        //Alberto 19/07/2017: MESSINA_UNIVERSITA
        //Nel caso di Compensazione mensile e Reupero dello straordinario precedente,
        //distribuisco la compensazione anche sul liquidabile, se possibile.
        m2:=0;
        if (TipoComp = 'M') and (RecupStraordPrec = 'S') then
        begin
          m2:=Max(0,R180SommaArray(tstrannom_prec) - R180SommaArray(tstrannom));
          m2:=min(m,m2);
          m2:=min(max(0,salcompannoatt + m),m2);
          dec(m,m2);
        end;
        inc(salcompannoatt,m);
        inc(salliqannoatt,m2);
        incFasceMaggiori(m2,tstrannom,tstrannom_prec);
      end;
//    finally
//      Free;
//    end;
  end;
  procedure CompensazioneDaCausaliEscluse(TipoComp:String; AggiornaSaldoAnnuo:Boolean);
  {Compensazione del negativo con le ore escluse dalle normali}
  var i,k,m,m2,Saldo,Saldo2,Saldo3:Integer;
  begin
    if TipoComp = 'A' then
      AggiornaSaldoAnnuo:=True;

    if TipoComp = 'A' then
      Saldo:=Abs(Min(0,salannoatt))
    else if TipoComp = 'M' then
      Saldo:=Abs(Min(0,salmeseatt))
    else if TipoComp = 'RM' then  //Recuperi mensili
      Saldo:=Max(0,sottrserbcomodo);
    with TStringList.Create do
    try
      if TipoComp = 'A' then
        CommaText:=CausaliCompensabili
        ...
        CommaText:=CausaliCompensabiliMensili
      else if TipoComp = 'RM' then
        CommaText:='';//'FADEX';//CausaliCompensabiliRecuperiMensili;
      for i:=0 to Count - 1 do
      begin
        if Saldo = 0 then
          Break;
        ...
        if ((TipoComp = 'A') and (CausaliCompAnnSaldo = 'A'))
        or ((TipoComp = 'M') and (CausaliCompMensSaldo = 'A'))
        or ((TipoComp = 'RM') (*and (CausaliCompMensSaldo = 'A')*))
        then
        begin
          //Considero tutta la disponibilità annuale
          Saldo2:=Saldo;
      ...
      Free;
      if TipoComp = 'RM' then
        dec(sottrserbcomodo,Max(0,sottrserbcomodo) - Saldo);

      if not ((RiposoNonFruito <> '') and (RiposoRecupLiquid = 'R')) then
      begin
        //TORINO_ASLCITTA <159110> - 163630 - aggiorna di saldi riposi compensativi
  ...
  procedure AssestamentoAnnuale(Assestamento:Integer);
  {Abbattimento saldi con le variazioni/liquidazioni fatte sulle ore degli anni precedenti}
    ...
    begin
      ...
      repeat
        ...
        if VarToStr(selT275.Lookup('CODICE',FieldByName('CAUSALE').AsString,'ORENORMALI')) <> 'A' then
          ...
          k:=IndiceRiepPres(FieldByName('CAUSALE').AsString);
        if (k >= 0) and (R180OreMinuti(FieldByName('ORE').AsString) > 0) then
        begin
  ...
  var i,k,app,app1:Integer;
      TmpRiepPres:array [1..MaxFasce] of LongInt;
      S:String;
  begin
    ...
    //devo considerare le fasce delle causali di LimiteEccResCaus per calcolare il liquidabile residuo
    for i:=1 to High(TmpRiepPres) do
      TmpRiepPres[i]:=0;
    if TroncaOre and BancaOre and (*(TroncaEccedenze = 'EG') and*) (LimiteEccResCaus <> '') and (OreCausLimiteEccRes > 0) then
    begin
      //TmpRiepPres contiene le ore in fasce riferite alle causali di LimiteEccResCaus
      for S in LimiteEccResCaus.Split([',']) do
      begin
        k:=IndiceRiepPres(S);
        if selT275.SearchRecord('CODICE',S,[srFromBeginning]) and
           ...
           (k >= 0) then
          for i:=1 to NFasceMese do
            inc(TmpRiepPres[i],RiepPres[k].OreReseMese[i]);
      end;
      //Applico prima l'abbattimento a TmpRiepPres
      app1:=max(0,R180SommaArray(TmpRiepPres) - strliqggmese);
      for i:=1 to NFasceMese do
      begin
        if app1 = 0 then
          Break;
        app:=min(TmpRiepPres[i],app1);
        dec(TmpRiepPres[i],app);
        dec(app1,app);
      end;
    end;

    //Applico abbattimento di TotAbb a tminstrmen, potendo considerare però solo la differenza tminstrmen - TmpRiepPres
    for i:=1 to NFasceMese do
    begin
      if TotAbb = 0 then
        Break;
      app:=min(tminstrmen[i] - TmpRiepPres[i],TotAbb);
...
begin
  if (MesiSaldoAnnoPrec = 0) and (MeseCorr400 = 1) and (MesiSaldoPrec < 0) then
  begin
    salannoatt_prec:=0;
    salannonetto_prec:=0;
    salliqannoprec_prec:=0;
    salcompannoprec_prec:=0;
    salpoanno_prec:=0;
    stranno_prec:=0;
    eccsolocompanno_prec:=0;
    salripcom_prec:=0;
    BancaOreResiduaPrec_prec:=0;
    for xx:=Low(tstranno_prec) to High(tstranno_prec) do tstranno_prec[xx]:=0;
    for xx:=Low(tstrannom_prec) to High(tstrannom_prec) do tstrannom_prec[xx]:=0;
    for xx:=Low(tstrannoprecm) to High(tstrannoprecm) do tstrannoprecm[xx]:=0;
    for xx:=Low(tsupann_prec) to High(tsupann_prec) do tsupann_prec[xx]:=0;
    for xx:=Low(tstrliq_prec) to High(tstrliq_prec) do tstrliq_prec[xx]:=0;
  end;
  ...
  //Saldo riposi compensativi dell'anno
  salripcom:=salripcomfmprec + salripcommes;
  //Lavorato mensile al netto dei riposi compensativi da turnazione maturati
  //Alberto 19/07/2017: per TORINO_CDS RipComInclusiSaldo = True: si lasciano i riposi compensativi nel saldo
  totoreresnorip:=totoreres - IfThen(not RipComInclusiSaldo,ripcommes);
  ...
  for i:=1 to NFasceMese + 1 do
  begin
    if (tdatiassestamen[1].tcauassest <> '') and (selT305.FindField('AUMENTA_ECC_GIORN') <> nil) and (Trim(VarToStr(selT305.Lookup('CODICE',tdatiassestamen[1].tcauassest,'AUMENTA_ECC_GIORN'))) = 'S') then
      inc(tminstrmen[i],max(0,tdatiassestamen[1].tminassest[i]));
    if (tdatiassestamen[2].tcauassest <> '') and (selT305.FindField('AUMENTA_ECC_GIORN') <> nil) and (Trim(VarToStr(selT305.Lookup('CODICE',tdatiassestamen[2].tcauassest,'AUMENTA_ECC_GIORN'))) = 'S') then
      inc(tminstrmen[i],max(0,tdatiassestamen[2].tminassest[i]));
  ...
  begin
    //if Pos('<>',DescT025) > 0 then
    if XParam[XP_R450_SALDONEGATIVOMINIMO_TIPO2] then
  ...
  //Compensazione saldo negativo mensile con ore escluse dalle normali
  if (CausaliCompMeseRecNeg = 'D') and (CausaliCompensabiliMensili <> '') and (salmeseatt < 0) then
    CompensazioneDaCausaliEscluse('M',False);
  ...
  begin
    ...
    //ASTI_ASL: abbattimento dei negativi non concessi dal recupero col compensabile
    //          quelli concessi sono salvati in compnegmes, gli altri devono essere giustificati.
    if XParam[XP_R450_NEGATIVI_COMPENSABILI] then
    begin
      if (compnegmes < 0) and (scostnegmes < compnegmes) then
        inc(app2,min(0,compnegmes) - min(0,scostnegmes));
    end;
  ...
  salannonetto:=salfmprecnetto + salmeseattnocau;
  for i:=1 to NFasceMese + 1 do
  ...
  //Recupero da Ore escluse
  if GESTIONE_FAD = GESTIONE_FAD_OE then
  begin
    app:=99999;
    y421_AbbattiSerbatoio('OE',0,abbannoprecmescom,app,abbannoatteff,DiffRecup);
    y421_AbbattiSerbatoio('OE',0,abbliqannoprecmescom,app,abbannoatteff,DiffRecup);
    y421_AbbattiSerbatoio('OE',0,abbannoattmescom,app,abbannoatteff,DiffRecup);
    y421_AbbattiSerbatoio('OE',0,abbliqannoattmescom,app,abbannoatteff,DiffRecup);
  end;
  ...
  begin
    //Se i conteggi mobili partono nell'anno corrente, il saldo anno prec e relativo recupero non devono abbattere il saldo abbattibile
    if DataInizioConteggi >= EncodeDate(AnnoCorr400,1,1) then
      RecuperoMensile:=RecuperoMensile - abbannopreceffComodo;
      //RecuperoMensile:=RecuperoMensile - (abbannopreceff + abbannoatteff);
  end;
  ...
  begin
    ...
    if SaldoMobile_EscludeRecPrec then  //LASPEZIA
    begin
      //Se i conteggi mobili partono nell'anno corrente, il recupero è il solo abbattimento dell'anno corrente
      if DataInizioConteggi >= EncodeDate(AnnoCorr400,1,1) then
        OreRecuperate:=OreRecuperate - abbannopreceffComodo;
        //OreRecuperate:=-Min(salmeseattnocau,0);
    end;
  end;
  ...
  begin
    ...
    //Se salcompannoatt < 0 cerco di recuperarlo su diffsalserb
    if salcompannoatt < 0 then
    begin
      app:=min(-salcompannoatt,diffsalserb);
  ...
  dec(salannoatt,totcauscompliqmm);
  //dec(salannonetto,???);
  ...
  //MASSA_ASL1: abbattimento da liquidazione di causali incluse ma solo compensabili!
  app:=totcauscompliqmm;
  if app > 0 then //se = 0 da non fare! perchè può alterare i saldi annuali negativi
  begin
    Abbatti(salcompannoatt,app);
    Abbatti(salcompannoprec,app);
    Abbatti(salliqannoprec,app);
    for i:=1 to MaxFasce do
    begin
      app1:=Max(0,Min(app,tstrannom[i] - tstrliq[i]));
  ...
  if (RiepAssCompensabiliAnno <> '') and (L07.RiepAssCompensateAnno > 0) then
    CompensazioneDaRiepAssenze('A');

  //Compensazione saldo negativo con ore escluse dalle normali
  if (CausaliCompMeseRecNeg = 'D') and (CausaliCompensabiliMensili <> '') and (salmeseatt < 0) then
    CompensazioneDaCausaliEscluse('M',True);

  if (CausaliCompAnnAddebPaghe = 'P') and (salannoatt < 0) then
    CompensazioneDaCausaliEscluse('A',True);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          inc(salannonetto,tbancaore[xx] - app);
  ...
  if eccsolocompres < 0 then
    eccsolocompres:=0;
  //Compensazione saldo negativo con ore escluse dalle normali
  if (CausaliCompAnnAddebPaghe = 'D') and (salannoatt < 0) then
    CompensazioneDaCausaliEscluse('A',True);
  ...
  //Abattimento ore dell'anno precedente
  if (MesiSaldoAnnoPrec > 0) and (MeseCorr400 = MesiSaldoAnnoPrec) and (MesiSaldoPrec < 0) then
  begin
    app:=max(0,salcompannoprec + salliqannoprec);
    dec(salannoatt,app);
    dec(salannonetto,app);
    inc(FOreTroncate,app);
    salcompannoprec:=0;
  ...
  RiepilogoMese[iRM].OreLiqDalMese:=0;
  ...
  RiepilogoMese[iRM].RecuperoAnnoAtt:=0;//abbannoattmes;
  RiepilogoMese[iRM].RecuperoAnnoPrec:=abbannopreceffComodo;//abbannoprecmes;
  ...
  RiepilogoMese[iRM].SaldoFineMesePrec:=salfmprec;
  ...
  begin
    ...
    {Alberto 26/06/2017: spostato sotto in modo da consentire l'abbattimento periodico delle causali
     se è attiva la periodicità mobile e non l'abbattimento dei saldi
    for xx:=0 to High(RiepPres) do
  ...
  begin
    ...
    if (BancaOreAbbattibile = 'S') then
    begin
      //Alberto 31/10/2006: Biella_ASL12: recupero automatico della banca ore se diminuisce il compensabile annuo
      if XParam[XP_R450_BANCAORE_ABBATTIBILE_v1049] then
        //Versione originale: non considera eventuale residuo anno precedente
        appBancaOreAbbattibile:=FBancaOreResidua + FBancaOreRecuperata + FBancaOreRecInterna + FBancaOreLiquidata
      else
        //Alberto 11/03/2019 - GENOVA_COMUNE: considera anche eventuale residuo anno precedente (FBancaOreRecAnnoPrec)
        appBancaOreAbbattibile:=FBancaOreResidua + (FBancaOreRecuperata + FBancaOreRecInterna - FBancaOreRecAnnoPrec) + FBancaOreLiquidata;
      if appBancaOreAbbattibile < FBancaOreAnno then
      begin
        FBancaOreRecInternaMensile:=FBancaOreAnno - appBancaOreAbbattibile;
  ...
  begin
    ...
    inc(salannonetto,SaldoNegativoMinimo + IfThen(SaldoNegativoMinimoTipo = '1',SaldoNegativoMinimoEcced,0));
    inc(totoreres,SaldoNegativoMinimo);
    inc(totoreresnorip,SaldoNegativoMinimo);
...
procedure TR450DtM1.y420_sottrserb(OreEscLuse:Boolean = False);
{Sottrazione eventuali negativi dai quattro serbatoi}
var i,differenza:Integer;
begin
  if sottrserbcomodo = 0 then exit;
  if (GESTIONE_FAD = GESTIONE_FAD_OE) and OreEscluse  then
    y422_CompensazioneDaCausaliEscluse('RM');
  if sottrserbcomodo = 0 then exit;
  for i:=1 to 4 do
  begin
...
begin
  ...
  if TipoSerbatoio = 'OE' then
  begin
    sottrserbcomodo:=recupero;
    y422_CompensazioneDaCausaliEscluse('RM');
    recupero:=sottrserbcomodo;
    Abb:=recupero - serbatoio;
    dec(serbatoio,Abb);
    inc(abbeff,Abb);
    exit;
  end;
...
procedure TR450DtM1.y422_CompensazioneDaCausaliEscluse(TipoComp:String = 'A');
{Compensazione del negativo con le ore escluse dalle normali}
var i,k,m,m2,Saldo,Saldo2,Saldo3:Integer;
begin
  if TipoComp = 'A' then
    ...
    Saldo:=Abs(Min(0,salmeseatt))
  else if TipoComp = 'RM' then  //Scostamenti negativi
    Saldo:=Max(0,sottrserbcomodo);
  with TStringList.Create do
  try
    if TipoComp = 'A' then
      ...
      CommaText:='';//'FADEX';//CausaliCompensabiliMensili + ',' + CausaliCompensabili;
    for i:=0 to Count - 1 do
    begin
      if Saldo = 0 then
        Break;
...
{Ricerca dati di abbattimento saldi (Melegnano / Roma_HSAndrea)}
begin
  Result:=False;
  with selT026 do
  begin
...
begin
  ...
  begin
    ...
    RiepilogoRecuperi[k].Recupero:=RM[i].SaldoMeseNoCau + RM[i].OreRecuperate;
    if SaldoMobile_EscludeRecPrec then //LASPEZIA
    begin
      //Se i conteggi mobili partono nell'anno corrente, il saldo anno prec e relativo recupero non devono abbattere il saldo abbattibile per cui lo riaggiungo
      if DataInizioConteggi >= EncodeDate(Anno400,1,1) then
        RiepilogoRecuperi[k].Recupero:=RiepilogoRecuperi[k].Recupero + RM[i].RecuperoAnnoPrec + RM[i].RecuperoAnnoAtt;
      RiepilogoRecuperi[k].Recupero:=min(RiepilogoRecuperi[k].Recupero,RM[i].SaldoMese);
    end;
...
begin
  Result:='';
  R180SetVariable(selT077,'PROGRESSIVO',Progress400);
  R180SetVariable(selT077,'DATA1',Data);
  R180SetVariable(selT077,'DATA2',Data);
  with selT077 do
  begin
    Open;
...
procedure TR450DtM1.ParametrizzazioneQueryStampa(Q:Integer);
begin
  case Q of
...
function TR450DtM1.GetOrePermesso: TOrePermessi;
var
  PBRecSuPaghe,PBDaRec : Real;
  Giustif:TGiustificativo;
begin
  if (OrePermessi.Progressivo <> Progress400) or (OrePermessi.Data <> EncodeDate(AnnoCorr400,MeseCorr400,1)) then
  begin
    OrePermessi.Addebitate:=0;
    OrePermessi.DaRecuperare:=0;
    OrePermessi.Progressivo:=progress400;
    OrePermessi.Data:=EncodeDate(AnnoCorr400,MeseCorr400,1);

    PBRecSuPaghe:=0;
    PBDaRec:=0;

    selT040CumuloT.Close;
    selT040CumuloT.SetVariable('Progressivo',Progress400);
    selT040CumuloT.SetVariable('Data1',EncodeDate(AnnoCorr400,MeseCorr400,1));
    selT040CumuloT.SetVariable('Data2',R180FineMese(EncodeDate(AnnoCorr400,MeseCorr400,1)));
    selT040CumuloT.Open;
    while not selT040CumuloT.Eof do
    begin
      CreazioneR600;
      R600DtM.ScaricoPaghe:=True;
      Giustif.Causale:=selT040CumuloT.FieldByName('CODICE').AsString;
      Giustif.Modo:='I';
      R600DtM.GetAssenze(Progress400,EncodeDate(AnnoCorr400,MeseCorr400,1),EncodeDate(AnnoCorr400,MeseCorr400,1),0,Giustif);
      if R600DtM.ValResiduo > 0 then
        PBRecSuPaghe:=PBRecSuPaghe + R600DtM.ValResiduo;
      R600DtM.ScaricoPaghe:=False;
      R600DtM.GetAssenze(Progress400,EncodeDate(AnnoCorr400,MeseCorr400,1),EncodeDate(AnnoCorr400,MeseCorr400,1),0,Giustif);
      //if R600DtM1.ValResiduo > 0 then //Alberto 08/11/2013: si stampa sempre il dato anche se negativo (PIACENZA_COMUNE - RE20)
      PBDaRec:=PBDaRec + R600DtM.ValResiduo;
      selT040CumuloT.Next;
    end;
    //Permessi non recuperati addebitati sul cedolino
    OrePermessi.Addebitate:=Round(PBRecSuPaghe);
    //Permessi da recuperare prima che vengano addebitati su paghe
    OrePermessi.DaRecuperare:=Round(PBDaRec-PBRecSuPaghe);
  end;
  Result:=OrePermessi;
end;
...
begin  //ORE RESE DA ASSENZA
  Result:=0;
  if selT070.RecordCount > 0 then
    Result:=R180OreMinuti(selT070.FieldByName('OREASSENZE').AsString);
end;
...
begin  //INDENNITA' NOTTURNA IN ORE
  Result:=0;
  if selT070.RecordCount > 0 then
    Result:=R180OreMinuti(selT070.FieldByName('INDTURNOORE').AsString) + R180OreMinuti(selT070.FieldByName('INDTURNOORE_VAR').AsString);
end;
...
begin  //ORE RESE INAIL
  Result:=0;
  if selT070.RecordCount > 0 then
    Result:=R180OreMinuti(selT070.FieldByName('ORE_INAIL').AsString);
end;
...
begin  //SPEZZONI TURNI REPERIBILITA' PIANIFICATI
  ...
  try
    if QTurniReperib.Active and QTurniReperib.SearchRecord('ANNO;MESE',VarArrayOf([anno400,mese400]),[srFromBeginning]) then
    repeat
      if R180OreMinuti(QTurniReperib.FieldByName('SPEZZONI').AsString) <> 0 then
      begin
        if Result <> '' then
          Result:=Result + ' ';
...
begin  //ORE MAGGIORATE DA TURNI REPERIBILITA' PIANIFICATI
  ...
  try
    if QTurniReperib.Active and QTurniReperib.SearchRecord('ANNO;MESE',VarArrayOf([anno400,mese400]),[srFromBeginning]) then
    repeat
      if R180OreMinuti(QTurniReperib.FieldByName('MAGGIORATE').AsString) <> 0 then
      begin
        if Result <> '' then
          Result:=Result + ' ';
...
begin  //ORE NON MAGGIORATE DA TURNI REPERIBILITA' PIANIFICATI
  ...
  try
    if QTurniReperib.Active and QTurniReperib.SearchRecord('ANNO;MESE',VarArrayOf([anno400,mese400]),[srFromBeginning]) then
    repeat
      if R180OreMinuti(QTurniReperib.FieldByName('NONMAGGIORATE').AsString) <> 0 then
      begin
        if Result <> '' then
          Result:=Result + ' ';
...
function TR450DtM1.GetNoteAcquistoBuoniTicket:String;
begin  //NOTE ACQUISTO TICKET/BUONI
  Result:='';
  try
    if QAcquistoMensa.Active and QAcquistoMensa.SearchRecord('DATA',R180FineMese(EncodeDate(anno400,mese400,1)),[srFromBeginning]) then
      Result:=QAcquistoMensa.FieldByName('NOTE').AsString;
  except
  end;
end;
...
begin  //ORE ANNO PRECEDENTE
  ...
  try
    if selT130.SearchRecord('ANNO',Anno400,[srFromBeginning]) then
      Result:=R180OreMinuti(selT130.FieldByName('SALDOORELAV').AsString);
  except
  end;
end;
...
begin  //ECCEDENZA SOLO COMPENSABILE ANNO PRECEDENTE
  ...
  try
    if selT130.SearchRecord('ANNO',Anno400,[srFromBeginning]) then
      Result:=R180OreMinuti(selT130.FieldByName('ORECOMPENSABILI').AsString);
  except
  end;
end;

function TR450DtM1.GetBancaOreEccedente(StraordAutorizzato:Integer):Integer;
var idx,strpag:Integer;
    S:String;
begin
  Result:=0;
...
function TR450DtM1.GetIterAutStrOreCausalizzate: Integer;
{Iter autorizzativo Straordinario + Ore Causalizzate (SAVONA_ASL2)
 Restituisce le ore cauisalizzate riferite alla causale indicata in IterAutStrCausale}
var idx:Integer;
begin
  Result:=0;
  if IterAutStrCausale.Trim = '' then
    exit;

  idx:=IndiceRiepPres(IterAutStrCausale);
  if idx < 0 then
    exit;

  //Reso nel mese
  Result:=R180SommaArray(RiepPres[idx].OreReseMese);
  //Limito il reso nel mese al residuo annuale, considerando quanto già liquidato nel mese stesso
  Result:=min(Result,R180SommaArray(RiepPres[idx].Residuo) + R180SommaArray(RiepPres[idx].LiquidatoMese));
  Result:=max(0,Result);
end;

procedure TR450DtM1.incFasceMaggiori(value:Integer; var dest:array of Integer; source:array of Integer);
var i,q:Integer;
begin
  for i:=min(High(dest),High(source)) downto 0 do
  begin
    q:=max(0,source[i] - dest[i]);
    q:=min(value,q);
    inc(dest[i],q);
    dec(value,q);
  end;
