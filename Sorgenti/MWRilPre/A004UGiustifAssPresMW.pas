...
type
  ...
  public
    ...
    property Nominativo: String read FNominativo write FNominativo;
    ...
    property DettagliIndirizzo: String read FDettagliIndirizzo write FDettagliIndirizzo;
  ...
  TA004FGiustifAssPresMW = class(TR005FDataModuleMW)
    ...
    selT048DATA_INIZIO_MAL: TDateTimeField;
    selT048DATA_FINE_MAL: TDateTimeField;
    selT048COGNOME_REP: TStringField;
    selT048INDIRIZZO: TStringField;
    selT048CAP: TStringField;
    selT048CITTA: TStringField;
    selT048PROV: TStringField;
    selT048CODCATASTALE: TStringField;
    dsrT048: TDataSource;
    selT050NOTE_GIUSTIFICATIVO: TStringField;
    selT050NOTE_RESP: TStringField;
  ...
  private
    ...
    procedure GeneraPeriodiAssenza(const POperazione,PCausale:String; const PDataInizio,PDataFine:TDateTime);
    function  FormattaDatiRichiesta(const CausIns: Boolean; var Note, CausInserita, NomeResp: String): String;
    ...
    procedure InviaEMail(const PDestResponsabile:Boolean; const PProg:Integer; const POggetto,PTesto:String);
  ...
  public
    ...
    B021FWebSvcClientDtM: TB021FWebSvcClientDtM; // datamodule per chiamate webservice firlab
    // EMPOLI_ASL11 - commessa 2014/054 SVILUPPO#1.ini
    // variabili spostate da private per utilizzo in A119MW
    ErroreCancellazione: String;
    DatoBloccatoCancellazione: Boolean;
    A087ElencoCausali:TObject;
    //Filtri si DataSet selT050
    procedure selT050FiltroCausale(Val:string);
...
begin
  ...
  // creazione datamodulo per webservice firlab
  B021FWebSvcClientDtM:=TB021FWebSvcClientDtM.Create(Owner);
...
begin
  ...
  FreeAndNil(R600DtM1);
  try
    //if B021FWebSvcClientDtM.Owner <> nil then  //Era stato introdotto in Aprile 2013, ma così genera MemoryLeak se Owner = nil
        ...
        begin
          ...
          begin
            ...
            begin
              Trovato:=True;
              DataCorr:=DataInizio;
              while (DataCorr <= DataFine) and (Trovato) do
              begin
                Trovato:=False;
                if SearchRecord('DATA',StrToDateTime(Var_Familiari),[srFromBeginning]) then
                repeat
                  if R180Between(DataCorr,FieldByName('DECORRENZA').AsDateTime,FieldByName('DECORRENZA_FINE').AsDateTime) then
                  begin
...
begin
  ...
  // se necessario resetta struttura dati per chiamata firlab
  ...
  try
    ...
    if Var_Familiari <> '' then
      ...
      R600DtM1.RiferimentoDataNascita.Data:=0;
    case Var_TipoGiust of
    ...
    begin
            ...
            begin
              ...
              begin
                if (R600DtM1.GGSignific) and
                   ...
                   (TG = 'I') and
                   (not SelDatiBloccati.DatoBloccato(Var_Progressivo,R180InizioMese(DataCorr),'T040')) and
                ...
                begin
                  ...
                  begin
                    ...
                    RegistraLog.SettaProprieta('I','T040_GIUSTIFICATIVI',IfThen(Chiamante <> '',Chiamante,NomeOwner),Q040,True);
                    Parametri.Operatore:=OperatoreBck;
                    //Per successiva chiamata al web sevice (Firlab)
                    if B021FWebSvcClientDtM.RegistraRecordT040('I',Q040,Var_tipoCaus,Chiamante) then
                    begin
                      Q040.Post;
                      PostEseguito:=True;
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  while CausaleSuccessiva and not R600DtM1.ControlliCausSucc(Var_Causale,Giustif,DataCorr) do
                  begin
                    if (VarToStr(R600DtM1.Q265.Lookup('CODICE',Var_Causale,'CAUSALE_SUCCESSIVA')) <> '') then
                    begin
                      Var_Causale:=VarToStr(R600DtM1.Q265.Lookup('CODICE',Var_Causale,'CAUSALE_SUCCESSIVA'));
                      if not R180InConcat(Var_Causale,ElencoCausali) then
                        ElencoCausali:=ElencoCausali + ',' + Var_Causale
                      else
                        CausaleSuccessiva:=False;
                    end
                ...
                begin
                  // commessa 2011/178
                  //B021FWebSvcClientDtM.RegistraAnomaliaT040('I',Var_TipoCaus,Var_Progressivo,DataCorr,Var_Causale,R600DtM1.FormattaAnomaliaWeb(R600DtM1.UltimaAnomalia,False),Chiamante);
                  // annulla operazione: abort
                  Abort;
                end;
              end
      ...
      if not SelDatiBloccati.DatoBloccato(Var_Progressivo,R180InizioMese(DataCorr),'T040') then
      begin
        Q040.Append;
        if (not InsNormale) and (IdRichiestaWeb <> 0) then
          Q040.FieldByName('Id_Richiesta').AsLargeInt:=IdRichiestaWeb
        ...
        then
        begin
          R600DtM1.scrDieciGiorniPrima.SetVariable('PROGRESSIVO',Var_Progressivo);
          R600DtM1.scrDieciGiorniPrima.SetVariable('DATA',DataCorr);
          R600DtM1.scrDieciGiorniPrima.SetVariable('GSIGNIFIC',R600DtM1.Q265.FieldByName('GSIGNIFIC').AsString);
          CausPeriodi:=Var_Causale + ',' + R600DtM1.Q265.FieldByName('CAUSALI_CUMULO_L133').AsString;
          CausPeriodi:='''' + StringReplace(CausPeriodi,',',''',''',[rfReplaceAll]) + '''';
          R600DtM1.scrDieciGiorniPrima.SetVariable('CAUSPERIODI',CausPeriodi);
          R600DtM1.scrDieciGiorniPrima.Execute;
          if R600DtM1.scrDieciGiorniPrima.GetVariable('NUMGG') <= 9 then
          begin
        ...
        // registrazione solo a fine inserimento
        B021FWebSvcClientDtM.RegistraRecordT040('I',Q040,Var_TipoCaus,Chiamante);//Per successiva chiamata al web sevice (Firlab)
        ...
        begin
          ...
          // 27/08/2018: no gestione visita fiscale se CHECK_SOLOCOMPETENZE = S
          VisitaFiscale:=R600DtM1.Q265.FieldByName('VISITA_FISCALE').AsString;
          if (VisitaFiscale = 'S') and
             (TG = 'I') and
             (R600DtM1.GetValStrT230(R600DtM1.Q265.FieldByName('CODICE').AsString,'CHECK_SOLOCOMPETENZE',DataCorr) <> 'S') then
  ...
  begin
    ...
    R600DtM1.VisFiscali.Nominativo:=RecapitoAlternativo.Nominativo;
    R600DtM1.VisFiscali.CodComune:=RecapitoAlternativo.CodComune;
    R600DtM1.VisFiscali.Indirizzo:=RecapitoAlternativo.Indirizzo;
    R600DtM1.VisFiscali.DettagliIndirizzo:=RecapitoAlternativo.DettagliIndirizzo;
  ...
  begin
    ...
    else if (AcquisizioneWebInCorso) then
    begin
      PeriodoNonInserito('');
    end;
  end;
  ...
  if B021FWebSvcClientDtM.EsisteRecordT040 and (Chiamante <> 'A087') then
  begin
    B021FWebSvcClientDtM.Autorizzatore:=B021Autorizzatore;
  ...
  begin
    ...
    begin
      ...
      if Esegui then
        R600DtM1.GestioneGiustifAssFuturi(Var_Progressivo,DataFine - 1,Giustif,Var_Familiari{$IFNDEF IRISWEB},ProgressBar{$ENDIF IRISWEB});
    end;
...
var Abbattimento,App,App1,App2,DaOre:Integer;
    D,Dal,Al:TDateTime;
...
begin
  ...
  begin
    ...
    //Spezzoni di ore normali
    App2:=R502DtM.RiepPresNonCau - R502DtM.RiepAssenza[TO_CSI_ABB_ECCSETT,'HHVAL'];
...
var DataCorr:TDateTime;
  ...
  procedure CheckGiustifAssFuturi;
  // controllo competenze giustificativi assenza futuri
  var n:Integer;
      D:TDateTime;
  begin
    ...
    begin
      Giustif.Causale:=Var_Causale;
      D:=DataFine;
      n:=R600DtM1.ContaGiustifAssFuturi(Var_Progressivo,D,Giustif,Var_Familiari);
      //Alberto 08/10/2018 - CCNL 2018: se non c'è nulla che riguardi Var_Causale, verifico se c'è stata una cancellazione delle giornate automatiche riferite ai permessi orari per visite mediche
      if (n <= 0) and (Cancellazione_CausaleHMAssenza.Causale <> '') then
      begin
        Giustif.Causale:=Cancellazione_CausaleHMAssenza.Causale;
        D:=Cancellazione_CausaleHMAssenza.Data;
        n:=R600DtM1.ContaGiustifAssFuturi(Var_Progressivo,D,Giustif,Var_Familiari);
      end;
      if n > 0 then
      begin
        Msg:=Format(A000TraduzioneStringhe(A000MSG_A004_DLG_FMT_RIALLINEA),[n]);
        if R180MessageBox(Msg,DOMANDA) = mrYes then
          R600DtM1.GestioneGiustifAssFuturi(Var_Progressivo,D,Giustif,Var_Familiari{$IFNDEF IRISWEB},ProgressBar{$ENDIF IRISWEB});
      end;
      R600DtM1.ChiudiGiustifAssFuturi;
    end;
  end;
begin
  ...
  // se necessario chiama manualmente il metodo ResetRecordT040 a inizio elaborazione
  if EseguiCommit then
    B021FWebSvcClientDtM.ResetRecordT040;
  ...
  try
    ...
    begin
      ...
      case TG of
        'I','M': Trovato:=Q040.Locate('Data;Causale;TipoGiust',VarArrayOf([DataCorr,Var_Causale,TG]),[]);
      ...
      begin
        ...
        begin
          case TG of
            'I','M': Trovato:=Q040.Locate('Data;Causale;TipoGiust',VarArrayOf([DataCorr,Caus10GGDopo,TG]),[]);
      ...
      begin
        ...
        try
          ...
          begin
            ...
            begin
              case TG of
                'I','M': Trovato:=Q040.Locate('Data;Causale;TipoGiust',VarArrayOf([DataCorr,LstCatena[i],TG]),[]);
      ...
      begin
        if Var_Causale = FIRLAB_RIPOSO then
          Trovato:=Q040.Locate('Data;Causale;TipoGiust',VarArrayOf([DataCorr,FIRLAB_RIPOSO,'I']),[])
        else if Var_Causale = FIRLAB_GIUST then
          Trovato:=Q040.Locate('Data;Causale;TipoGiust',VarArrayOf([DataCorr,FIRLAB_GIUST,'N']),[])
      end;
      ...
      DatoBloccato:=selDatiBloccati.DatoBloccato(Var_Progressivo,R180InizioMese(DataCorr),'T040');
      if Trovato and (not DatoBloccato) then
      begin
        try
          //Registrazione per chiamata al webservice (firlab)
          if (Var_Causale = FIRLAB_GIUST) and (TG = 'D') then
            B021FWebSvcClientDtM.RegistraRecordT040('C',Q040,Var_TipoCaus,Chiamante,TG,Var_DaOre,Var_AOre)
          else
            B021FWebSvcClientDtM.RegistraRecordT040('C',Q040,Var_TipoCaus,Chiamante);
    ...
    begin
      ...
      if (Chiamante = 'B021') and (B021Autorizzatore <> '') then
        Parametri.Operatore:=B021Autorizzatore;
      RegistraLog.SettaProprieta('C','T040_GIUSTIFICATIVI',IfThen(Chiamante <> '',Chiamante,NomeOwner),nil,True);
      Parametri.Operatore:=OperatoreBck;
    ...
    begin
      ...
      if (Errore <> '') and (AcquisizioneWebInCorso) then
      begin
        if CancNormale then
          raise Exception.Create(Errore)
        else
  ...
  // controllo competenze giustificativi assenza futuri
  CheckGiustifAssFuturi;

  // webservice firlab
  if B021FWebSvcClientDtM.EsisteRecordT040 and EseguiCommit then
  begin
    B021FWebSvcClientDtM.Autorizzatore:=B021Autorizzatore;
    B021FWebSvcClientDtM.WebSvcRecordT040;
  end;
...
begin
  ...
  try
    ...
    RegistraLog.SettaProprieta('I','T046_GIUSTIFICATIVIFAMILIARI',IfThen(Chiamante <> '',Chiamante,NomeOwner),nil,True);
    RegistraLog.InserisciDato('PROGRESSIVO',IntToStr(Var_Progressivo),'');
    RegistraLog.InserisciDato('CAUSALE',Var_Causale,'');
    RegistraLog.InserisciDato('DAL - AL',Format('%s - %s',[DateToStr(DataInizio),DateToStr(DataFine)]),'');
    RegistraLog.RegistraOperazione;
...
begin
  ...
  try
    ...
    delT046.Execute;
    RegistraLog.SettaProprieta('C','T046_GIUSTIFICATIVIFAMILIARI',IfThen(Chiamante <> '',Chiamante,NomeOwner),nil,True);
    RegistraLog.InserisciDato('PROGRESSIVO',IntToStr(Var_Progressivo),'');
    RegistraLog.InserisciDato('CAUSALE',Var_Causale,'');
    RegistraLog.InserisciDato('DAL - AL',Format('%s - %s',[DateToStr(DataInizio),DateToStr(DataFine)]),'');
    RegistraLog.RegistraOperazione;
...
begin
  ...
  C018DM:=TC018FIterAutDM.Create(SessioneOracleA004);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          begin
            // procede con l'inserimento del/i giustificativo/i
      ...
      begin
        if selT050.FieldByName('AUTORIZZAZIONE').AsString = 'N' then
          Msg:='Annullamento richiesta di '
        else
          Msg:='Elaborazione richiesta di ';
        if TipoRich = 'D' then
          Msg:=Msg + 'fruizione giustificativo'
        else if TipoRich = 'R' then
          Msg:=Msg + 'revoca giustificativo'
        else if TipoRich = 'C' then
          Msg:=Msg + 'cancellazione giustificativo';
        Msg:=Format('%s: %s (%s)',[Msg,selT050.FieldByName('NOMINATIVO').AsString,selT050.FieldByName('MATRICOLA').AsString]);
        try
          if selT050.FieldByName('AUTORIZZAZIONE').AsString = 'N' then
            Testo:='Si avvisa che la seguente richiesta non sarà riportata sul cartellino in quanto l''autorizzazione è stata negata da parte di '
          else
            Testo:='Si avvisa che la seguente richiesta è stata riportata sul cartellino in seguito ad autorizzazione da parte di ';
          Testo:=
            Testo + // + 'Si avvisa che la seguente richiesta è stata riportata sul cartellino in seguito ad autorizzazione da parte di ' +
            selT050.FieldByName('NOMINATIVO_RESP').AsString + CRLF +
            'Data richiesta: ' + selT050.FieldByName('DATA').AsString + CRLF +
            'Tipo richiesta: ' + IfThen(selT050.FieldByName('TIPO_RICHIESTA').AsString = 'D','Fruizione',
                                 IfThen(selT050.FieldByName('TIPO_RICHIESTA').AsString = 'R','Revoca',
                                 IfThen(selT050.FieldByName('TIPO_RICHIESTA').AsString = 'C','Cancellazione'
                                 ))) + CRLF +
            'Periodo:        ' + selT050.FieldByName('DAL').AsString + ' - ' + selT050.FieldByName('AL').AsString + CRLF +
            'Causale:        ' + selT050.FieldByName('CAUSALE').AsString + ' - ' + selT050.FieldByName('DESCRIZIONE').AsString + CRLF +
            'Tipo giustif.:  ' + IfThen(selT050.FieldByName('TIPOGIUST').AsString = 'I','Giornata intera',
                                 IfThen(selT050.FieldByName('TIPOGIUST').AsString = 'M','Mezza giornata',
                                 IfThen(selT050.FieldByName('TIPOGIUST').AsString = 'N','Ore: ' + selT050.FieldByName('NUMEROORE').AsString,
                                 IfThen(selT050.FieldByName('TIPOGIUST').AsString = 'D','Dalle ' + selT050.FieldByName('NUMEROORE').AsString + ' alle ' + selT050.FieldByName('AORE').AsString
                                 )))) +
            IfThen(not selT050.FieldByName('DATANAS').IsNull,CRLF + 'Familiare rif.: ' + selT050.FieldByName('DATANAS').AsString) +
            IfThen(not selT050.FieldByName('NOTE_RESP').IsNull,CRLF + 'Note del resp.: ' + selT050.FieldByName('NOTE_RESP').AsString);
        except
          Testo:='';
        end;
        InviaEMail(False,selT050.FieldByName('PROGRESSIVO').AsInteger,Msg,Testo);
      end;
...
begin
  ...
  try
    ...
    on E:Exception do
      if Pos('ORA-01480',E.Message) > 0 then
      while s <> '' do
      try
...
begin
  ...
  if not selT050.FieldByName('NOTE_GIUSTIFICATIVO').IsNull then
    Result:=Result + ' [' + selT050.FieldByName('NOTE_GIUSTIFICATIVO').AsString + ']';
...
begin
  ...
  Var_Note:=selT050.FieldByName('NOTE_GIUSTIFICATIVO').AsString;
  ...
  //Se gestione non interattiva non si considera l'inserimento di gg di assenza in presenza di timbrature
  if  (not PInterattiva) then
  begin
    R600DtM1.AnomalieNonBloccanti:=Parametri.CampiRiferimento.C90_W010AnomNonBloccanti;
    if UpperCase(Parametri.RagioneSociale) = 'A.O.U. CITTA'' DELLA SALUTE E DELLA SCIENZA DI TORINO' then
  ...
  begin
    ...
    Giustif.Note:=selT050.FieldByName('NOTE_GIUSTIFICATIVO').AsString;
    chkNuovoPeriodo:=False;
    ...
    if PIns then
    begin
      RegistraMsg.InserisciMessaggio('I',Format('InserisciGiustif.Inizio - ID=%d - RichiesteIterAutorizzativo=%s',[selT050.FieldByName('ID').AsInteger,IfThen(R600DtM1.RichiesteIterAutorizzativo,'True','False')]),'',selT050.FieldByName('PROGRESSIVO').AsInteger);
      InserisciGiustif(False,selT050.FieldByName('ID').AsInteger);
      RegistraMsg.InserisciMessaggio('I',Format('InserisciGiustif.Fine - ID=%d - RichiesteIterAutorizzativo=%s',[selT050.FieldByName('ID').AsInteger,IfThen(R600DtM1.RichiesteIterAutorizzativo,'True','False')]),'',selT050.FieldByName('PROGRESSIVO').AsInteger);
    end
    else
    begin
      RegistraMsg.InserisciMessaggio('I',Format('EseguiCancellazione.Inizio - ID=%d - RichiesteIterAutorizzativo=%s',[selT050.FieldByName('ID').AsInteger,IfThen(R600DtM1.RichiesteIterAutorizzativo,'True','False')]),'',selT050.FieldByName('PROGRESSIVO').AsInteger);
      EseguiCancellazione;
      RegistraMsg.InserisciMessaggio('I',Format('EseguiCancellazione.Fine - ID=%d - RichiesteIterAutorizzativo=%s',[selT050.FieldByName('ID').AsInteger,IfThen(R600DtM1.RichiesteIterAutorizzativo,'True','False')]),'',selT050.FieldByName('PROGRESSIVO').AsInteger);
    end;
...
procedure TA004FGiustifAssPresMW.InviaEMail(const PDestResponsabile:Boolean; const PProg:Integer; const POggetto,PTesto:String);
var
  C017FEMailDtM: TC017FEMailDtM;
begin
  C017FEMailDtM:=TC017FEMailDtM.Create(nil);
  try
    C017FEMailDtM.InviaEMail(SessioneOracleA004,PDestResponsabile,PProg,POggetto,PTesto,406);
    if not C017FEMailDtM.MailInviata then
      RegistraMsg.InserisciMessaggio('A','Invio email fallito.','',PProg);
  finally
...
begin
  ...
  begin
    ...
    // cancella (per ogni giorno) la richiesta che sarà elaborata, indipendentemente dall'esito
    B021FWebSvcClientDtM.ResetRecordT040;
    D:=selT050.FieldByName('DAL').AsDateTime;
    while D <= selT050.FieldByName('AL').AsDateTime do
    begin
      ...
      // 0 = presenza
      // 1 = assenza
      TipoCaus:=IfThen(selT050.FieldByName('TIPOCAUS').AsString = 'A',1,0);
      if not B021FWebSvcClientDtM.RegistraRecordT050('C',TipoCaus,
        selT050.FieldByName('PROGRESSIVO').AsInteger,D,selT050.FieldByName('CAUSALE').AsString,
        selT050.FieldByName('TIPOGIUST').AsString,selT050.FieldByName('NUMEROORE').AsString,
        selT050.FieldByName('AORE').AsString,selT050.FieldByName('DATANAS').AsDateTime,CancRichiesta,'') then
        Break;
      D:=D + 1;
    end;
...
begin
  if (Copy(E.Message,1,9) = 'ORA-00001') or ((Pos('ORA-00001',E.Message) > 0) and (Pos('T040_PK',E.Message) > 0)) then
  begin
...
procedure TA004FGiustifAssPresMW.selT050FiltroCausale(Val:string);
begin
  if not Val.IsEmpty then
  begin
    R180SetVariable(selT050,'FILTRO_CAUSALE',Format('and T050.CAUSALE = ''%s''', [Val]));
  end
  else
  begin
    R180SetVariable(selT050,'FILTRO_CAUSALE',null);
  end;
  selT050.Open;
end;
