...
type
  ...
  TA075FFineAnnoMW = class(TR005FDataModuleMW)
    ...
    selT430RapportiUnificati: TOracleQuery;
...
begin
  ...
  begin
    SelAnagrafeRapportiUniti:='N';
    if not FDipInSer.DipendenteInServizio(ProgrAnagafica,EncodeDate(R180Anno(DataRif),1,1),EncodeDate(R180Anno(DataRif),1,1)) then
    begin
      with selT430RapportiUnificati do
      begin
        SetVariable('PROGRESSIVO',ProgrAnagafica);
        SetVariable('DATA',DataRif);
        Execute;
        SelAnagrafeRapportiUniti:=FieldAsString(0);
      end;
    end;
    if (FDipInSer.DipendenteInServizio(ProgrAnagafica,EncodeDate(R180Anno(DataRif),1,1),EncodeDate(R180Anno(DataRif),1,1))) or
       (SelAnagrafeRapportiUniti = 'S')
    then
    begin
