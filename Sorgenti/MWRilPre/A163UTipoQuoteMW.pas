...
begin
  ...
  if not R180In(selT765.FieldByName('TIPOQUOTA').AsString,['A','S','T','I','V','C','Q']) then
    selT765.FieldByName('GIORNI_MESE').Value:=Null;

  if (selT765.FieldByName('TIPOQUOTA').AsString = 'I') and not (selT765.FieldByName('RIF_SALDO_ANNO_CORR').AsString = 'S') then
    selT765.FieldByName('ACCONTI').AsString:='';
  if (selT765.FieldByName('TIPOQUOTA').AsString = 'I') and not selT765.FieldByName('ACCONTI').IsNull then
    try
      lstAcconti:=TStringList.Create;
      lstAcconti.Clear;
      lstAcconti.CommaText:=selT765.FieldByName('ACCONTI').AsString;
      for i:=0 to lstAcconti.Count - 1 do
      begin
        selT765b.Close;
        selT765b.SetVariable('CODICE',selT765.FieldByName('CODICE').AsString);
        selT765b.SetVariable('ACCONTO',lstAcconti.Strings[i]);
        selT765b.Open;
        if selT765b.RecordCount > 0 then
          raise exception.Create(Format(A000MSG_A163_ERR_FMT_ACCONTO_USATO,[lstAcconti.Strings[i],selT765b.FieldByName('CODICE').AsString]));
      end;
    finally
...
begin
  ...
  else if selT765.FieldByName('TIPOQUOTA').AsString = 'N' then
    selT765.FieldByName('D_TIPOQUOTA').AsString:='Saldo Coll. val. non obbl.'
