...
uses
  ...
  DBClient, Provider, C180FunzioniGenerali;
...
type
  ...
  public
    ...
    procedure AggiornaElencoCdc(Query: TOracleDataSet);
   end;
...
begin
  ...
  FselT433_Funzioni.DisableControls;
  try
    ...
    FselT433_Funzioni.EnableControls;
  end;
end;
...
begin
  R180SetVariable(selT430,'Campo',Campo);
  R180SetVariable(selT430,'Progressivo',Progressivo);
  R180SetVariable(selT430,'Datalavoro',DataLavoro);
  selT430.Open;
end;
...
procedure TA082FCdcPercentMW.AggiornaElencoCdc(Query: TOracleDataSet);
var DecoDesc:TDateTime;
begin
  if Query.VariableIndex('DECORRENZA') >= 0 then
  begin
    if selT433_Funzioni.Active and not selT433_Funzioni.FieldByName('DECORRENZA_FINE').IsNull then
      DecoDesc:=selT433_Funzioni.FieldByName('DECORRENZA_FINE').AsDateTime
    else
      DecoDesc:=EncodeDate(3999,12,31);
    R180SetVariable(Query,'DECORRENZA',DecoDesc);
  end;
  if not Query.Active then
  begin
    R180SetReadBuffer(Query);
    Query.Open;
  end;
end;
