...
uses
  ...
  Dialogs, R005UDataModuleMW, DB, OracleData, DBClient, C180FunzioniGenerali,
  A000UInterfaccia, A000UCostanti, A000USessione, A000UMessaggi, StrUtils;
...
type
  ...
  TXParamClientDataSet = class(TClientDataSet)
    public
      procedure SolaLettura(ReadOnly: Boolean);
      procedure ImpostaParametri(List: Array of String; Parametri:String);
      function OttieniParametri: String;
      procedure ValidateValore(Sender: TField);
      procedure CapitalizzaValore(Sender: TField; const Text: string);
      procedure BeforeInsertDelete(DataSet: TDataSet);
  end;
  ...
  TA006FModelliOrarioMW = class(TR005FDataModuleMW)
    ...
    selT021PNID_TURNO: TIntegerField;
    selT021PNVALENZA_GGASS: TFloatField;
    selaT020: TOracleDataSet;
    selT021PNINDTURNO: TStringField;
    selT021PNINDFESTIVA: TStringField;
  ...
  public
    ...
    cdsXParam: TXParamClientDataSet;
    dsrXParam: TDataSource;
    cdsXParamComp: TXParamClientDataSet;
    dsrXParamComp: TDataSource;
    ...
    const
      ...
      D_DetrazRiepPrNorm: array [0..3] of TItemsValues = (
        (Item:'Causale singola';   Value:'CS'),
        (Item:'Causali ordinate';  Value:'CO'),
        (Item:'Fasce basse';       Value:'FB'),
        (Item:'Ordine cronologico';Value:'HD')
        );
      D_TipoFasciaStraordinario: array [0..1] of TItemsValues = (
        (Item:'FO';           Value:'FO'),
        (Item:'STR';          Value:'STR')
        );
...
procedure TXParamClientDataSet.SolaLettura(ReadOnly: Boolean);
var
  Position:Integer;
begin
  DisableControls;
  ...
  while not Eof do
  begin
    FieldByName('VALORE').ReadOnly:=ReadOnly;
    Next;
  end;
  if Position > 0 then
    RecNo:=Position;
...
procedure TXParamClientDataSet.ValidateValore(Sender: TField);
begin
  if not R180In(UpperCase(Sender.AsString), ['S', 'N']) then
    raise Exception.Create('Sono utilizzabili solo valori "S" o "N". Correggere.');
end;

procedure TXParamClientDataSet.BeforeInsertDelete(DataSet: TDataSet);
begin
  Abort;
end;

procedure TXParamClientDataSet.CapitalizzaValore(Sender: TField; const Text: string);
begin
  Sender.AsString:=UpperCase(Text);
end;

procedure TXParamClientDataSet.ImpostaParametri(List: Array of String; Parametri: String);
var
  i, Position:Integer;
  ReadOnlyNome, ReadOnlyValore: Boolean;
begin
  DisableControls;
  BeforeInsert:=nil;
  BeforeDelete:=nil;
  try
    ...
    begin
      Append;
      ReadOnlyNome:=FieldByName('NOME').ReadOnly;
      ReadOnlyValore:=FieldByName('VALORE').ReadOnly;
      FieldByName('NOME').ReadOnly:=False;
      FieldByName('VALORE').ReadOnly:=False;
      FieldByName('NOME').AsString:=List[i];
      if Pos(UpperCase(List[i]),UpperCase(Parametri)) > 0 then
        FieldByName('VALORE').AsString:='S'
      else
        FieldByName('VALORE').AsString:='N';
      FieldByName('NOME').ReadOnly:=ReadOnlyNome;
      FieldByName('VALORE').ReadOnly:=ReadOnlyValore;
      Post;
    end;
    if Position > 0 then
      RecNo:=Position
    ...
    EnableControls;
    BeforeInsert:=BeforeInsertDelete;
    BeforeDelete:=BeforeInsertDelete;
  end;
end;

function TXParamClientDataSet.OttieniParametri: String;
var
  Position:Integer;
begin
  DisableControls;
  ...
  while not Eof do
  begin
    if UpperCase(FieldByName('VALORE').AsString) = 'S' then
...
begin
  ...
  // XPARAM
  cdsXParam:=TXParamClientDataSet.Create(self);
  cdsXParam.FieldDefs.Add('NOME',ftString,40);
  cdsXParam.FieldDefs.Add('VALORE',ftString,1);
  cdsXParam.CreateDataSet;
  cdsXParam.FieldByName('NOME').DisplayLabel:='Nome parametro';
  cdsXParam.FieldByName('NOME').ReadOnly:=True;
  cdsXParam.FieldByName('VALORE').DisplayLabel:='Attivo S/N';
  cdsXParam.FieldByName('VALORE').ReadOnly:=True;
  cdsXParam.FieldByName('VALORE').OnValidate:=cdsXParam.ValidateValore;
  cdsXParam.FieldByName('VALORE').OnSetText:=cdsXParam.CapitalizzaValore;
  dsrXParam:=TDataSource.Create(self);
  dsrXParam.DataSet:=cdsXParam;

  // XPARAM-RETROCOMPATIBILITÀ
  cdsXParamComp:=TXParamClientDataSet.Create(self);
  cdsXParamComp.FieldDefs.Add('Nome',ftString,40, True);
  cdsXParamComp.FieldDefs.Add('Valore',ftString,1, True);
  cdsXParamComp.CreateDataSet;
  cdsXParamComp.FieldByName('NOME').DisplayLabel:='Nome parametro';
  cdsXParamComp.FieldByName('NOME').ReadOnly:=True;
  cdsXParamComp.FieldByName('VALORE').DisplayLabel:='Attivo S/N';
  cdsXParamComp.FieldByName('VALORE').ReadOnly:=True;
  cdsXParamComp.FieldByName('VALORE').OnValidate:=cdsXParamComp.ValidateValore;
  cdsXParamComp.FieldByName('VALORE').OnSetText:=cdsXParamComp.CapitalizzaValore;
  dsrXParamComp:=TDataSource.Create(self);
  dsrXParamComp.DataSet:=cdsXParamComp;
...
begin
  inherited;
  FreeAndNil(cdsXParam);
  FreeAndNil(cdsXParamComp);
  FreeAndNil(dsrXParam);
  FreeAndNil(dsrXParamComp);
  FreeAndNil(lstT275);
  FreeAndNil(lstT020);
end;

procedure TA006FModelliOrarioMW.CaricaLstT020;
begin
  inherited;
  lstT020.Clear;
  with selaT020 do
  begin
    SetVariable('DATA',selT020.FieldByName('DECORRENZA').AsDateTime);
    Open;
    while not Eof do
    begin
      lstT020.Add(Format('%-5s %s',[FieldByName('CODICE').AsString,FieldByName('DESCRIZIONE').AsString]));
      Next;
    end;
    Close;
  end;
...
begin
  ...
  if DataSet.FieldByName('TIPO_FASCIA').AsString = 'PN' then
  begin
    if not R180In(DataSet.FieldByName('INDTURNO').AsString,['O','N']) then
      raise Exception.Create(Format('Per %S sono ammessi solo i valori ammessi:' + CRLF +
                                    'O: indennità maturata come da orario' + CRLF +
                                    'N: Indennità non maturata',[DataSet.FieldByName('INDTURNO').DisplayLabel]));
    if not R180In(DataSet.FieldByName('INDFESTIVA').AsString,['O','N']) then
      raise Exception.Create(Format('Per %S sono ammessi solo i valori ammessi:' + CRLF +
                                    'O: indennità maturata come da orario' + CRLF +
                                    'N: indennità non maturata',[DataSet.FieldByName('INDFESTIVA').DisplayLabel]));
  end;
end;
...
{Controlli complessivi prima della conferma di registrazione}
  ...
  procedure VerificaIdTurno;
  var EsisteIdNum,EsisteIdNull:Boolean;
      MinIdTurno,MaxIdTurno:Integer;
      lstIdTurno:TStringList;
  begin
    selT021PN.DisableControls;
    selT021PN.First;
    EsisteIdNum:=False;
    EsisteIdNull:=False;
    MinIdTurno:=999;
    MaxIdTurno:=0;
    lstIdTurno:=TStringList.Create;
    try
      while not selT021PN.Eof do
      begin
        if selT021PN.FieldByName('ID_TURNO').IsNull then
          EsisteIdNull:=True
        else
        begin
          EsisteIdNum:=True;
          if lstIdTurno.IndexOf(selT021PN.FieldByName('ID_TURNO').AsString) >= 0 then
            raise Exception.Create('Esistono 2 turni con lo stesso ''Num''=' + selT021PN.FieldByName('ID_TURNO').AsString)
          else
          lstIdTurno.Add(selT021PN.FieldByName('ID_TURNO').AsString);
          if selT021PN.FieldByName('ID_TURNO').AsInteger > MaxIdTurno then
            MaxIdTurno:=selT021PN.FieldByName('ID_TURNO').AsInteger;
          if selT021PN.FieldByName('ID_TURNO').AsInteger < MinIdTurno then
            MinIdTurno:=selT021PN.FieldByName('ID_TURNO').AsInteger;
        end;
        selT021PN.Next;
      end;
      if EsisteIdNull and EsisteIdNum then
        raise Exception.Create('La colonna ''Num'' deve essere valorizzata per tutti i turni o per nessuno.');
      if EsisteIdNum then
      begin
        if MinIdTurno <> 1 then
          raise Exception.Create('Il primo turno deve avere ''Num''=1.');
        if MaxIdTurno <> selT021PN.RecordCOunt then
          raise Exception.Create('I turni devono avere i valori di ''Num'' consecutivi.');

      end;
    finally
      FreeAndNil(lstIdTurno);
      selT021PN.First;
      selT021PN.EnableControls;
    end;
...
begin
  ...
  if R180In(Parametri.Operatore,['MONDOEDP','SYSMAN','TESTR']) then
  begin
    selT020.FieldByName('XPARAM').AsString:=cdsXParam.OttieniParametri;
    selT020.FieldByName('XPARAM_COMP_OLDVERS').AsString:=cdsXParamComp.OttieniParametri;
  end;  
end;
...
begin
  if (CodiceOld <> CodiceNew) or (DecorrenzaOld <> DecorrenzaNew) then
  begin
    RegistraLog.SettaProprieta('M','T020_ORARI',NomeOwner,nil,True);
    if CodiceOld <> CodiceNew  then
...
begin
  ...
  selT021PNID_TURNO.Visible:=FTipoOrario in [toE];
