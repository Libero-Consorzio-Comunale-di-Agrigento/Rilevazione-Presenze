...
uses
  ...
  StrUtils,RegistrazioneLog, A004UGiustifAssPresMW;
...
type
  ...
  TR250FScaricoGiustificativiDtM = class(TR004FGestStoricoDtM)
    ...
    T040P_TRIGGER_BEFORE: TOracleQuery;
    T040P_TRIGGER_AFTER: TOracleQuery;
...
begin
  ...
  try
    ...
    RegistraMsg.InserisciMessaggio('I','FINE ACQUISIZIONE GIUSTIFICATIVI');
  end;
end;

procedure TR250FScaricoGiustificativiDtM.GestioneParametrizzazione;
var NB,NumScartate:Integer;
    NomeTabella,Condizione,Errore:String;
begin
  ...
  try
    ...
    begin
      //Richieste web autorizzate
      A004MW.Chiamante:='R250';
      ...
      A004MW.selT050.SetVariable('AZIENDA',selI090.FieldByName('AZIENDA').AsString);
      A004MW.selT050.SetVariable('HINTT030V430',Parametri.CampiRiferimento.C26_HintT030V430);
      A004MW.selT050.SetVariable('FILTRO_MODALITA','and t050.elaborato = ''N''');
      A004MW.selT050.SetVariable('FILTRO_PERIODO',null);
      A004MW.selT050.Open;
      nNumRiga:=A004MW.selT050.RecordCount;
      NumScartate:=0;
      if A004MW.selT050.RecordCount > 0 then
        A004MW.AcquisizioneRichiesteWeb(False,False,Errore,NumScartate);
    end;
  finally
    if SelI150.FieldByName('TIPOFILE').AsString = 'F' then
    begin
...
var
  sIn, sMatricola, sChiave, sExprChiave, sBadge, sAnnoDa, sAnnoA, sMeseDa, sMeseA, sGiornoDa, sGiornoA, sHHDa, sHHA, sMMDa, sMMA, sCausale, sTipo,
  sDataDa, sDataA, sOraDa, sOraA, sIdentificazione, sNumGiorni, sFamiliare:String;
  CausaleOriginale, Var_Valida:String;
...
begin
  ...
  begin
    ...
    //CHIAVE ALTERNATIVA
    if (Trim(selI150.FieldByName('CHIAVE').AsString) <> '0,0') and (Trim(selI150.FieldByName('CHIAVE').AsString) <> '') then
    begin
      RecuperaDatoDaFile(sIn,SelI150.FieldByName('CHIAVE').AsString,sChiave);
      sExprChiave:=Trim(SelI150.FieldByName('EXPR_CHIAVE').AsString);
    end;
    //BADGE
    ...
    if (sBadge = '') and (sMatricola = '') and (sChiave = '') then
    begin
      //Si tratta di un'anomalia e la aggiungo alle liste delle anomalie
      RegistraMsg.InserisciMessaggio('A','Riga n. ' + inttostr(nNumRiga) + ' - Matricola/Badge/Chiave non indicati.',selI090.FieldByName('AZIENDA').AsString);
      sMessaggio:='Riga n. ' + inttostr(nNumRiga) + ' - Matricola/Badge/Chiave non indicati.';
      Exit;
    end
    ...
    begin
      ...
      // indicazione chiave alternativa alla matricola
      LMatricola.Caption:=IfThen(sChiave <> '',sChiave,sMatricola);
    ...
    // estrazione progressivo
    if sChiave <> '' then
    begin
      sIdentificazione:=' (CHIAVE ' + sChiave + ')';
      // estrazione del progressivo data la chiave alternativa al badge
      Lst:=TStringList.Create;
      try
        selChiave.Close;
        Lst:=FindVariables(sExprChiave,False); // non considera i duplicati
        Lst.CaseSensitive:=False;
        // se l'espressione contiene la variabile :DATA la dichiara
        // altrimenti la elimina, se presente ma non necessaria
        if (Lst.IndexOf('Data') > -1) and
           (selChiave.VariableIndex('Data') = -1) then
        begin
          selChiave.DeclareVariable('Data',otDate);
        end
        else if (Lst.IndexOf('Data') = -1) and
                (selChiave.VariableIndex('Data') > -1) then
        begin
          selChiave.DeleteVariable('Data');
        end;
        selChiave.SQL.Text:=sExprChiave;
        selChiave.SetVariable('Chiave',sChiave);
        if selChiave.VariableIndex('Data') > -1 then
          selChiave.SetVariable('Data',StrToDateDef(sDataA,Date));
        try
          selChiave.Open;
        except
          on E:Exception do
        ...
        if selChiave.RecordCount = 0 then
        begin
          RegistraMsg.InserisciMessaggio('A','Riga n. ' + inttostr(nNumRiga) + ' - Il dipendente indicato non risulta presente in anagrafica' + sIdentificazione,selI090.FieldByName('AZIENDA').AsString);
          sMessaggio:='Riga n. ' + inttostr(nNumRiga) + ' - Il dipendente indicato non risulta presente in anagrafica' + sIdentificazione;
          exit;
        end;
        nProgressivo:=selChiave.FieldByName('Progressivo').AsInteger;
      finally
        FreeAndNil(Lst);
      end;
    end
    ...
    begin
      ...
      begin
        try
          ...
          FormattaData.SetVariable('FORMATO',SelI150.FieldByName('FORMATODATA').AsString);
          FormattaData.Execute;
          A004MW.Var_Familiari:=FormattaData.FieldAsString(0);
        except
    ...
    if (sTipo = 'N') and (sOraDa='00.00') then
    begin
      //Si tratta di un anomalia e la aggiungo alle liste delle anomalie
    ...
    *)
    ...
    begin
      ...
      R600DtM1.AnomalieNonBloccanti:='1,7,10';
      //***ProgressBar:=nil;
      ...
      Chiamante:='R250';
      Var_Valida:='S';
      //chiamata alla procedure pl/sql
      if selI150.FieldByName('TRIGGER_BEFORE').AsString = 'S' then
      begin
        T040P_TRIGGER_BEFORE.SetVariable('P_SCARICO',selI150.FieldByName('CODICE').AsString);
        T040P_TRIGGER_BEFORE.SetVariable('P_PROGRESSIVO', Var_Progressivo);
        T040P_TRIGGER_BEFORE.SetVariable('P_CAUSALE', Var_Causale);
        T040P_TRIGGER_BEFORE.SetVariable('P_TIPOGIUST', sTipo[1]);
        T040P_TRIGGER_BEFORE.SetVariable('P_DADATA', DataInizio);
        T040P_TRIGGER_BEFORE.SetVariable('P_ADATA', DataFine);
        T040P_TRIGGER_BEFORE.SetVariable('P_DAORE', Var_DaOre);
        T040P_TRIGGER_BEFORE.SetVariable('P_AORE', Var_AOre);
        T040P_TRIGGER_BEFORE.SetVariable('P_FAMILIARE', null);
        if Var_Familiari <> '' then
          T040P_TRIGGER_BEFORE.SetVariable('P_FAMILIARE', StrToDateTime(Var_Familiari));
        try
          T040P_TRIGGER_BEFORE.Execute;
          Var_Valida:=T040P_TRIGGER_BEFORE.GetVariable('P_VALIDA');
        except
          on E:Exception do
            RegistraMsg.InserisciMessaggio('A','Esecuzione T040P_TRIGGER_BEFORE: ' + E.Message);
        end;
      end;
      if Var_Valida = 'S' then
      begin
      ...
      if selI150.FieldByName('TRIGGER_AFTER').AsString = 'S' then
      begin
        T040P_TRIGGER_AFTER.SetVariable('P_SCARICO',selI150.FieldByName('CODICE').AsString);
        T040P_TRIGGER_AFTER.SetVariable('P_PROGRESSIVO', Var_Progressivo);
        T040P_TRIGGER_AFTER.SetVariable('P_CAUSALE', Var_Causale);
        T040P_TRIGGER_AFTER.SetVariable('P_TIPOGIUST', sTipo[1]);
        T040P_TRIGGER_AFTER.SetVariable('P_DADATA', DataInizio);
        T040P_TRIGGER_AFTER.SetVariable('P_ADATA', DataFine);
        T040P_TRIGGER_AFTER.SetVariable('P_DAORE', Var_DaOre);
        T040P_TRIGGER_AFTER.SetVariable('P_AORE', Var_AOre);
        T040P_TRIGGER_AFTER.SetVariable('P_FAMILIARE', null);
        if Var_Familiari <> '' then
          T040P_TRIGGER_AFTER.SetVariable('P_FAMILIARE', StrToDateTime(Var_Familiari));
        try
          T040P_TRIGGER_AFTER.Execute;
        except
          on E:Exception do
            RegistraMsg.InserisciMessaggio('A','Esecuzione T040P_TRIGGER_AFTER: ' + E.Message);
        end;
      end;
    end;
