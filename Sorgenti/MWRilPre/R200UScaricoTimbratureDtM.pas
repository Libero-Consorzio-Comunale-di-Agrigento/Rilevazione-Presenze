...
type
  ...
  public
    ...
    NFCorrente,NFAppoggio,NFScartate,NFCopia,NFAnomalie,NFRecupero:String;
...
var
  NB:Integer;
  NomeFileOrig,NomeFileRinom: String;
begin
  RegistraMsg.InserisciMessaggio('I','Parametrizzazione ' + QI100.FieldByName('SCARICO').AsString);

  // recuperare file uploadato da Wa032
  if UploadFileWA032 <> '' then
  begin
    NomeFileOrig:=UploadFileWA032;
    UploadFileWA032:='';
  end
  else if NFRecupero = '' then
    NomeFileOrig:=QI100.FieldByName('NomeFile').AsString
  else
    NomeFileOrig:=NFRecupero;
  if ScrollBox <> nil then
  begin
    LScarico.Caption:=QI100.FieldByName('SCARICO').AsString;
    ScrollBox.Repaint;
  end;
  //NFCopia:=ExtractFilePath(NomeFileOrig) + 'TB' + FormatDateTime('yymmdd',Date) + '.';
  //La copia TB viene fatta sempre sul path di riferimento dello scarico
  NFCopia:=ExtractFilePath(QI100.FieldByName('NomeFile').AsString) + 'TB' + FormatDateTime('yymmdd',Date) + '.';
  try
  ...
  //Nome file per anomalie durante inserimento
  NFAnomalie:=R180GetFilePath(NomeFileOrig) + '\Anomalie_' + R180GetFileName(NomeFileOrig);
  //Alberto 30/03/2006: se il file non esiste esco subito senza segnalare nulla
  if FileExists(NomeFileOrig) then
  begin
    try
      QI090.Refresh;
      QI090.First;
    ...
    NomeFileRinom:=NomeFileOrig + '.medp';
    ...
    if not FileExists(NomeFileRinom) then
    begin
      if not RenameFile(NomeFileOrig,NomeFileRinom) then
      begin
        RegistraMsg.InserisciMessaggio('A','Impossibile rinominare il file ' + NomeFileOrig + ' in ' + NomeFileRinom);
        Abort;
      end;
    end;
    // sposta il file sotto "Archivi\Temp"
    if not RenameFile(NomeFileRinom,NFCorrente) then
    begin
      RegistraMsg.InserisciMessaggio('A','Impossibile spostare il file ' + NomeFileRinom + ' in ' + NFCorrente);
      Abort;
    end;
  ...
  begin
    RegistraMsg.InserisciMessaggio('A','File inesistente: ' + NomeFileOrig);
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      RegistraMsg.InserisciMessaggio('I','  Parametrizzazione: ' + QI100.FieldByName('SCARICO').AsString,FieldByName('Azienda').AsString);
      Q100.Close;
      Q430.Close;
        ...
        begin
          ...
          try
            ...
            begin
              ...
              begin
                // registra anomalia
                try RegistraMsg.InserisciMessaggio('A',E.Message); except end;
...
begin
  ...
  try
    ...
    if IVerso = QI100.FieldByName('Entrata').AsString then
      IVerso:='E'
    else if IVerso = QI100.FieldByName('Uscita').AsString then
      IVerso:='U';
...
//============================================================================
    ...
    //Alberto 06/10/2018 - CASERTA_ASL: il verso non corrisponde ad E/U
    if (not TimbrRegistrata) and (not R180In(IVerso,['E','U'])) then
    begin
      if TRUE then
      begin
        InsTimb(Q103);
...
begin
  try
    ...
    begin
      //Estraggo Causale con le stesse coordinate del Badge/Chiave
      if Mappa.UtilizzaChiave then
      begin
        Causale:=Copy(S,Mappa.Chiave.Posiz,Mappa.Chiave.Lung);
        ICausale:=Copy(Causale + '     ',1,Mappa.Chiave.Lung);
      end
      else
      begin
