...
uses
  ...
  Rp502Pro, R600, C180FunzioniGenerali, DatiBloccati;
...
begin
  // verifica blocco dei riepiloghi giustificativi
  if selDatiBloccati.DatoBloccato(Prog,R180InizioMese(Data1),'T040') then
    Exit;
  ...
  begin
    ...
    for j:=0 to High(R502ProDtM.triepgiuspres[i].CoppiaEU) do
      ...
      begin
        ...
        DalleAlle[high(DalleAlle)].CausE:=False;
        DalleAlle[high(DalleAlle)].CausU:=False;
        ...
        //Tengo traccia del fatto che lo spezzone da inserire è legato alla prima timbratura di entrata della giornata
        if (Length(R502ProDtM.TimbratureOriginali) > 0) and
           (R502ProDtM.TimbratureOriginali[0].esiste_e) and
           (R502ProDtM.TimbratureOriginali[0].e = R502ProDtM.triepgiuspres[i].CoppiaEU[j].u) and
           (R502ProDtM.TimbratureOriginali[0].caus_e = R502ProDtM.triepgiuspres[i].tcauspres)
        then
          DalleAlle[high(DalleAlle)].CausE:=True;
      end;
    DiffFruitoReso:=0;
    ...
    begin //Inserimento giustificativi dalle..alle
      ...
      try
        ...
        if TIns > T then
        begin
          //Se il giustificativo è legato alla prima timbratura della giornata, lo si accorcia posticipandone l'inizio
          if DalleAlle[j].CausE then
            inc(DalleAlle[j].Dalle,Tins - T)
          else
...
begin
  ...
  begin
    ...
    if (j > 0) and (j <= High(GGBancaOreEcc[idx].MinStrGio)) then
      GGBancaOreEcc[idx].MinStrGio[j]:=R502DtM.tminstrgio[i];
  end;
end;
...
var D:TDateTime;
  ...
  {Registro l'abbattimento che verrà inserito successivamnte come giustificativo}
  var i:Integer;
  begin
    for i:=0 to High(GGBancaOreEcc) do
      if D = GGBancaOreEcc[i].Data then
