inherited A128FPianPrestazioniAggiuntiveMW: TA128FPianPrestazioniAggiuntiveMW
  OldCreateOrder = True
  OnDestroy = DataModuleDestroy
  Height = 132
  Width = 310
  object selT030: TOracleDataSet
    SQL.Strings = (
      'SELECT MATRICOLA,PROGRESSIVO FROM T030_ANAGRAFICO')
    ReadBuffer = 10000
    Optimize = False
    Filtered = True
    Left = 16
    Top = 16
  end
  object Q330: TOracleDataSet
    SQL.Strings = (
      'SELECT * FROM T330_REG_ATT_AGGIUNTIVE ORDER BY CODICE')
    Optimize = False
    QBEDefinition.QBEFieldDefs = {
      
      4F004C004C004F005F0050005400010000000000}
    Filtered = True
    Left = 64
    Top = 16
    object Q330CODICE: TStringField
      DisplayWidth = 10
      FieldName = 'CODICE'
      Origin = 'T350_REGREPERIB.CODICE'
      Size = 5
    end
    object Q330DESCRIZIONE: TStringField
      FieldName = 'DESCRIZIONE'
      Origin = 'T350_REGREPERIB.DESCRIZIONE'
      Size = 40
    end
    object Q330ORAINIZIO: TDateTimeField
      FieldName = 'ORAINIZIO'
      Origin = 'T350_REGREPERIB.ORAINIZIO'
    end
    object Q330ORAFINE: TDateTimeField
      FieldName = 'ORAFINE'
      Origin = 'T350_REGREPERIB.ORAFINE'
    end
    object Q330CONTROLLO_PT: TStringField
      FieldName = 'CONTROLLO_PT'
      Size = 1
    end
  end
  object D330: TDataSource
    DataSet = Q330
    Left = 96
    Top = 16
  end
  object selaT332: TOracleDataSet
    SQL.Strings = (
      'SELECT '
      '    DATA,'
      '    PROGRESSIVO,'
      '    TURNO1,'
      '    ORAINIZIO_TURNO1,'
      '    ORAFINE_TURNO1, '
      '    TURNO2,'
      '    ORAINIZIO_TURNO2,'
      '    ORAFINE_TURNO2  '
      'FROM '
      '    T332_PIAN_ATT_AGGIUNTIVE'
      'WHERE '
      '    DATA = :DATA AND '
      '    PROGRESSIVO = :PROGRESSIVO')
    Optimize = False
    Variables.Data = {
      04000000020000000A0000003A0044004100540041000C000000000000000000
      0000180000003A00500052004F0047005200450053005300490056004F000300
      00000000000000000000}
    Left = 144
    Top = 16
  end
  object Q430Contratto: TOracleDataSet
    SQL.Strings = (
      
        'SELECT T430.CONTRATTO,T200.DESCRIZIONE,ABPRESENZA1,DECODE(T430.P' +
        'ARTTIME,'#39#39',100,NVL(T460.PIANTA,100)) PERCPT'
      
        '  FROM T430_STORICO T430, T200_CONTRATTI T200, T460_PARTTIME T46' +
        '0'
      
        '  WHERE T430.PROGRESSIVO = :PROGRESSIVO AND :DATA BETWEEN T430.D' +
        'ATADECORRENZA AND T430.DATAFINE'
      
        '    AND T430.CONTRATTO = T200.CODICE AND T430.PARTTIME = T460.CO' +
        'DICE(+)')
    Optimize = False
    Variables.Data = {
      0400000002000000180000003A00500052004F00470052004500530053004900
      56004F000300000000000000000000000A0000003A0044004100540041000C00
      00000000000000000000}
    Left = 224
    Top = 16
  end
  object insT332: TOracleQuery
    SQL.Strings = (
      'insert into T332_PIAN_ATT_AGGIUNTIVE'
      
        '  (DATA, PROGRESSIVO, TURNO1, ORAINIZIO_TURNO1, ORAFINE_TURNO1, ' +
        'TURNO2, ORAINIZIO_TURNO2, ORAFINE_TURNO2)'
      'values'
      
        '  (:DATA, :PROGRESSIVO, :TURNO1, :ORAINIZIO_TURNO1, :ORAFINE_TUR' +
        'NO1, :TURNO2, :ORAINIZIO_TURNO2, :ORAFINE_TURNO2)')
    Optimize = False
    Variables.Data = {
      
      000000000000}
    Left = 16
    Top = 72
  end
  object updT332: TOracleQuery
    SQL.Strings = (
      'UPDATE '
      #9'T332_PIAN_ATT_AGGIUNTIVE '
      'SET '
      #9'TURNO1 = :TURNO1,'
      #9'ORAINIZIO_TURNO1 = :ORAINIZIO_TURNO1, '
      #9'ORAFINE_TURNO1 = :ORAFINE_TURNO1,'
      #9'TURNO2 = :TURNO2,'
      #9'ORAINIZIO_TURNO2 = :ORAINIZIO_TURNO2, '
      #9'ORAFINE_TURNO2 = :ORAFINE_TURNO2'#9
      'WHERE '
      #9'PROGRESSIVO = :PROGRESSIVO AND DATA = :DATA')
    Optimize = False
    Variables.Data = {
      
      000000000000}
    Left = 64
    Top = 72
  end
  object delT332: TOracleQuery
    SQL.Strings = (
      'DELETE FROM T332_PIAN_ATT_AGGIUNTIVE '
      'WHERE PROGRESSIVO = :PROGRESSIVO AND '
      '      DATA = :DATA'
      '      :WHERE')
    Optimize = False
    Variables.Data = {
      0400000003000000180000003A00500052004F00470052004500530053004900
      56004F000300000000000000000000000A0000003A0044004100540041000C00
      000000000000000000000C0000003A0057004800450052004500010000000000
      000000000000}
    Left = 112
    Top = 72
  end
  object cdsParametri: TClientDataSet
    Aggregates = <>
    FieldDefs = <
      item
        Name = 'Turno1'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'Turno2'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'ORAINIZIO_TURNO1'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'ORAFINE_TURNO1'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'ORAINIZIO_TURNO2'
        DataType = ftString
        Size = 20
      end
      item
        Name = 'ORAFINE_TURNO2'
        DataType = ftString
        Size = 20
      end>
    IndexDefs = <>
    Params = <>
    StoreDefs = True
    Left = 184
    Top = 72
    object cdsParametriTurno1: TStringField
      FieldName = 'Turno1'
      OnChange = cdsParametriCodiceTurnoChange
      OnSetText = cdsParametriCodiceTurnoSetText
      OnValidate = cdsParametriCodiceTurnoValidate
    end
    object cdsParametriTurno2: TStringField
      FieldName = 'Turno2'
      OnChange = cdsParametriCodiceTurnoChange
      OnSetText = cdsParametriCodiceTurnoSetText
      OnValidate = cdsParametriCodiceTurnoValidate
    end
    object cdsParametriORAINIZIO_TURNO1: TStringField
      FieldName = 'ORAINIZIO_TURNO1'
      OnChange = cdsParametriOrarioChange
      OnSetText = cdsParametriOrarioSetText
      OnValidate = cdsParametriOrarioValidate
      EditMask = '!90:00;1;_'
      Size = 5
    end
    object cdsParametriORAFINE_TURNO1: TStringField
      FieldName = 'ORAFINE_TURNO1'
      OnChange = cdsParametriOrarioChange
      OnSetText = cdsParametriOrarioSetText
      OnValidate = cdsParametriOrarioValidate
      EditMask = '!90:00;1;_'
      Size = 5
    end
    object cdsParametriORAINIZIO_TURNO2: TStringField
      FieldName = 'ORAINIZIO_TURNO2'
      OnChange = cdsParametriOrarioChange
      OnSetText = cdsParametriOrarioSetText
      OnValidate = cdsParametriOrarioValidate
      EditMask = '!90:00;1;_'
      Size = 5
    end
    object cdsParametriORAFINE_TURNO2: TStringField
      FieldName = 'ORAFINE_TURNO2'
      OnChange = cdsParametriOrarioChange
      OnSetText = cdsParametriOrarioSetText
      OnValidate = cdsParametriOrarioValidate
      EditMask = '!90:00;1;_'
      Size = 5
    end
  end
  object dsrParametri: TDataSource
    DataSet = cdsParametri
    Left = 248
    Top = 72
  end
end
