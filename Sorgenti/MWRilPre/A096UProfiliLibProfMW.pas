...
begin
  ...
  if selT311.FieldByName('GIORNO').AsString <> 'FS' then
  begin
    selaT311.SetVariable('COND_FERIALE', 'or (    oreminuti(alle_continue) >= oreminuti(''24.00'')' +
                                         '    and ((:giorno = ''1'' and giorno = ''7'') or (:giorno <> ''1'' and giorno = TO_CHAR(TO_NUMBER(:giorno) - 1)))' +
                                         '    and oreminuti(:dalle) <= oreminuti(alle))' +
                                         'or (    oreminuti(:alle_continue) >= oreminuti(''24.00'')' +
                                         '    and ((:giorno = ''7'' and giorno = ''1'') or (:giorno <> ''7'' and giorno = TO_CHAR(TO_NUMBER(:giorno) + 1)))' +
                                         '    and oreminuti(:alle) >= oreminuti(dalle))');
    if selaT311.VariableIndex('ALLE') < 0 then
      selaT311.DeclareVariable('ALLE',otString);
    selaT311.SetVariable('ALLE',selT311.FieldByName('ALLE').AsString);
  end
  else
  begin
    selaT311.SetVariable('COND_FERIALE', '');

    if selaT311.VariableIndex('ALLE') >= 0 then
      selaT311.DeleteVariable('ALLE');
  end;

  selaT311.SetVariable('GIORNO',selT311.FieldByName('GIORNO').AsString);
...
begin
  if selT311.FieldByName('Giorno').AsString = 'FS' then
    selT311.FieldByName('D_Giorno').AsString:='Festivo'
  else if selT311.FieldByName('Giorno').AsString = '' then
    selT311.FieldByName('D_Giorno').AsString:=''
  else
