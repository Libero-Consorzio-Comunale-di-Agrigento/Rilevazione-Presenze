...
// unit condivisa con i seguenti progetti:
// - B020 (web services)
// - B110 (web services)
...
uses
  SysUtils, Classes, StrUtils, DB, OracleData,
  ...
  A041UInsRiposiMW, R300UAccessiMensaDtM, R350UCalcoloBuoniDtM, R400UCartellinoDtM,
  Datasnap.DBClient;
...
type
  ...
  public
    ...
    procedure CreazioneA041(DataI,DataF:TDateTime);
    ...
    procedure AnnullaTagLstRaveComp;
  end;
...
begin
  ...
  begin
    R400FCartellinoDtM.cdsAnagrafico.Close;
...
procedure TW009FStampaCartellinoDtm.CreazioneA041(DataI,DataF:TDateTime);
begin
    ...
    try
      repeat
        ...
        begin
          ...
          if RegLog then
          begin
            RegistraLog.SettaProprieta('I','T070_SCHEDARIEPIL','W009',nil,True);
...
begin
  Result:='';
  try
    W009CSStampa.Enter;//CSStampa.Enter;
    ...
    try
      ...
      if R400FCartellinoDtM.AggiornamentoScheda and ((ImgTop + ImgAlt) > 0) then
      begin
        S:=Format('''%s''',[A000TraduzioneStringhe(A000MSG_R400_MSG_AGGIORNAMENTO_SCHEDA)]);
        rvComp:=RVProject.ProjMan.FindRaveComponent('lblTitolo2',rvPage);
...
begin
  ...
  with R400FCartellinoDtM do
    try
      ...
      ListaSettaggi.Clear;
      R180SetVariable(selT951,'Codice',R400FCartellinoDtM.Q950Int.FieldByName('CODICE').AsString);
      selT951.Open;
      selT951.First;
...
var i,j,i2:Integer;
    ...
    begin
      ...
      begin
        ...
        if R400FCartellinoDtM.cdsAnagrafico.FindField(DataField) <> nil then
        begin
          R400FCartellinoDtM.cdsAnagrafico.FieldByName(DataField).Visible:=True;
          R400FCartellinoDtM.cdsAnagrafico.FieldByName(DataField).DisplayLabel:=Capt;
        end;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        //Accessibilità non vedenti
        if R400FCartellinoDtM.cdsDettaglio.FindField(DataField) <> nil then
        begin
          R400FCartellinoDtM.cdsDettaglio.FieldByName(DataField).Visible:=True;
          R400FCartellinoDtM.cdsDettaglio.FieldByName(DataField).DisplayLabel:=Capt;
        end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //Accessibilità non vedenti
          if R400FCartellinoDtM.cdsRiepilogo.FindField(DataField) <> nil then
          begin
            R400FCartellinoDtM.cdsRiepilogo.FieldByName(DataField).Visible:=True;
            R400FCartellinoDtM.cdsRiepilogo.FieldByName(DataField).DisplayLabel:=Capt;
          end;
          if (Top + Height) > MaxTop then
            MaxTop:=(Top + Height);
        end;
    ...
    begin
      ...
      begin
        ...
        DataField:='Campo' + Posiz;
        //Accessibilità non vedenti
        if R400FCartellinoDtM.cdsAssenze.FindField(DataField) <> nil then
        begin
          R400FCartellinoDtM.cdsAssenze.FieldByName(DataField).Visible:=True;
          R400FCartellinoDtM.cdsAssenze.FieldByName(DataField).DisplayLabel:=IfThen(Capt <> '',Capt,'Descrizione');
          //Solo per il Riepilogo Assenze, posizionamento della descrizione di fianco al codice, o in prima posizione
          if R400FCartellinoDtM.cdsAssenze.FieldByName(DataField).FieldName = 'Campo908' then
          begin
            if R400FCartellinoDtM.cdsAssenze.FindField('Campo901') <> nil then
              R400FCartellinoDtM.cdsAssenze.FieldByName(DataField).Index:=R400FCartellinoDtM.cdsAssenze.FieldByName('Campo901').Index + 1
            else
              R400FCartellinoDtM.cdsAssenze.FieldByName(DataField).Index:=0;
          end;
        end;
    ...
    begin
      ...
      begin
        ...
        DataField:='Campo' + Posiz;
        //Accessibilità non vedenti
        if R400FCartellinoDtM.cdsPresenze.FindField(DataField) <> nil then
        begin
          R400FCartellinoDtM.cdsPresenze.FieldByName(DataField).Visible:=True;
          R400FCartellinoDtM.cdsPresenze.FieldByName(DataField).DisplayLabel:=IfThen(Capt <> '',Capt,'Descrizione');
        end;
  ...
  begin
    ...
    if Sender = 'Dettaglio' then
      case StrToInt(Posiz) of
        C_TI1..C_TI8,C_GI1..C_GI2,C_TM1,C_ANM,C_LPR,C_NOTE:CreaMemo_Dett
      else
        CreaLab_Dett;
      end;
...
procedure TW009FStampaCartellinoDtm.AnnullaTagLstRaveComp;
var i:Integer;
    rc:TObject;
begin
  ...
  begin
    ...
    if (rc is TRaveComponent) then
      (rc as TRaveComponent).Tag:=0
    else if (rc is TRaveText) then
      (rc as TRaveText).Tag:=0
    else if (rc is TRaveDataText) then
      (rc as TRaveDataText).Tag:=0;
  end;
  rc:=RVProject.ProjMan.FindRaveComponent('bndDettaglio',rvPage);
  if rc is TRaveComponent then
    (rc as TRaveComponent).Tag:=0;
end;
