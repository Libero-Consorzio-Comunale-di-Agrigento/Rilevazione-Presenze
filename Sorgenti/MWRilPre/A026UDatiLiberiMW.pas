...
uses
  SysUtils, Classes, StrUtils, Oracle, DB, OracleData, A000UInterfaccia, R005UDataModuleMW,
  A099UUtilityDBMW, Variants, C180FunzioniGenerali;
...
type
  TA026FDatiLiberiMW = class(TR005FDataModuleMW)
    ...
    scrGetDataDefault: TOracleQuery;
    selColsNULLABLE: TOracleQuery;
    insI501ValDefault: TOracleQuery;
    insI501StoricoValDefault: TOracleQuery;
    procedure DataModuleCreate(Sender: TObject);
    procedure DataModuleDestroy(Sender: TObject);
  private
    { Private declarations }
    A099FUtilityDBMW:TA099FUtilityDBMW;
    FDatoObbligatorio:boolean;
    FDatoDefault:string;
    procedure SetDatoDefault(Valore:string);
  ...
  public
    ...
    DS:TDataSet;
    function GetDatoDefault(NomeDato:string):string;
    procedure EditDatoDefault;
    procedure EditDatoObbligatorio;
    function GetDatoObbligatorio(NomeDato:string):boolean;
    ...
    property DatoDefault:string read FDatoDefault write SetDatoDefault;
    property DatoObbligatorio:boolean read FDatoObbligatorio write FDatoObbligatorio;
  end;
...
procedure TA026FDatiLiberiMW.SetDatoDefault(Valore:string);
var
  Formato:string;
  Lunghezza:integer;
  ValoreFloat:Extended;
...
begin
  ...
  begin
    ...
    Lunghezza:=DS.FieldByName('LUNGHEZZA').AsInteger;
    if Formato = 'S' then
    begin
      //Controlli su formato stringa
      if Valore.Length > Lunghezza then
        raise Exception.Create('Valore di dafault troppo lungo.');
    end
    else if Formato = 'N' then
    begin
      //Controlli su formato numerico
      if not TryStrToFloat(Valore,ValoreFloat) then
        raise Exception.Create('Valore di default non numerico.');
    end
    else if Formato = 'D' then
    begin
      //Controlli su formato data
      if (Valore.ToUpper <> 'SYSDATE') and (Copy(Valore.ToUpper,1,8) <> 'TO_DATE(') then
      begin
        if not TryStrToDateTime(Valore,ValoreDate) then
          raise Exception.Create('Valore di default non è una data valida.');
      end;
    end;
  end;
  FDatoDefault:=Valore;
...
function TA026FDatiLiberiMW.GetDatoDefault(NomeDato:string):string;
begin
  Result:='';
  scrGetDataDefault.ClearVariables;
  scrGetDataDefault.SetVariable('COLUMN_NAME',NomeDato);
  try
    scrGetDataDefault.Execute;
    Result:=VarToStr(scrGetDataDefault.GetVariable('RESULT')).Trim;
  except
  end;
...
procedure TA026FDatiLiberiMW.EditDatoDefault;
var
  Formato, Valore:string;
begin
  //Formattazione valori in base al formato
  if not DatoDefault.IsEmpty then
  begin
    Valore:=DatoDefault;
    Formato:=DS.FieldByName('FORMATO').AsString;
    if Formato = 'S' then
    ...
    begin
      if (Valore.ToUpper <> 'SYSDATE') and (Copy(Valore.ToUpper,1,8) <> 'TO_DATE(') then
        Valore:='to_date(''' + Valore + ''',''DD/MM/YYYY'')';
    end;
  end
  else
    Valore:='null';
  ...
  OperSQL.ClearVariables;
  OperSQL.DeclareVariable('NOMECAMPO',otSubst);
  OperSQL.DeclareVariable('VALOREDEFAULT',otSubst);
  OperSQL.SQL.Add('alter table T430_STORICO modify :NOMECAMPO default :VALOREDEFAULT');
  OperSQL.SetVariable('NOMECAMPO',DS.FieldByName('NOMECAMPO').AsString);
  OperSQL.SetVariable('VALOREDEFAULT',Valore);
  try
    OperSQL.Execute;
  except
    on E:Exception do
      raise Exception.Create('Applicazione del valore di default fallita!' + CRLF + E.Message);
  end;

  if DatoDefault <> 'null' then
  begin
    //Uupddate della T430 impostando il valore di default sui valori nulli eventuali
    OperSQL.SQL.Clear;
    OperSQL.ClearVariables;
    OperSQL.SQL.Add('update T430_STORICO T430');
    OperSQL.SQL.Add('   set T430.:NOMECAMPO = :VALOREDEFAULT');
    OperSQL.SQL.Add(' where T430.:NOMECAMPO is null');
    OperSQL.SetVariable('NOMECAMPO',DS.FieldByName('NOMECAMPO').AsString);
    OperSQL.SetVariable('VALOREDEFAULT',Valore);
    try
      OperSQL.Execute;
      ...
      on E:Exception do
        raise Exception.Create('Modifica di T430_STORICO col valore di default fallita!' + CRLF + E.Message);
    end;
    //Insereimento valore di default su tabella I501
    if DS.FieldByName('TABELLA').AsString = 'S' then
    begin
      //Gestione storica S/N
      if DS.FieldByName('STORICO').AsString = 'S' then
      begin
        insI501StoricoValDefault.SetVariable('DATO',DS.FieldByName('NOMECAMPO').AsString);
        insI501StoricoValDefault.SetVariable('VALORE',Valore);
        if DS.FieldByName('LUNG_DESC').AsInteger >= Length('Valore di Default')  then
          insI501StoricoValDefault.SetVariable('VALOREDESC','Valore di Default')
        else
          insI501StoricoValDefault.SetVariable('VALOREDESC','');
        try
          insI501StoricoValDefault.Execute;
        except
          on e:exception do
            raise Exception.Create('Inserimento del valore di default nella tabella di decodifica fallito!' + CRLF + E.Message);
        end;
      end
      else
      begin
        insI501ValDefault.SetVariable('DATO',DS.FieldByName('NOMECAMPO').AsString);
        insI501ValDefault.SetVariable('VALORE',Valore);
        if DS.FieldByName('LUNG_DESC').AsInteger >= Length('Valore di Default')  then
          insI501ValDefault.SetVariable('VALOREDESC','Valore di Default')
        else
          insI501ValDefault.SetVariable('VALOREDESC','');
        try
          insI501ValDefault.Execute;
        except
          on e:exception do
            raise Exception.Create('Inserimento del valore di default nella tabella di decodifica fallito!' + CRLF + E.Message);
        end;
...
procedure TA026FDatiLiberiMW.EditDatoObbligatorio;
begin
  OperSQL.SQL.Clear;
  OperSQL.DeleteVariables;
  OperSQL.ClearVariables;
  OperSQL.SQL.Add('alter table T430_STORICO modify :NOMECAMPO ' + IfThen(DatoObbligatorio,'not') + ' null');
  OperSQL.DeclareAndSet('NOMECAMPO',otSubst,DS.FieldByName('NOMECAMPO').AsString);
  try
    OperSQL.Execute;
  except
    on E:Exception do
      raise Exception.Create('Applicazione del vincolo di obbligatorietà fallita!' + CRLF + E.Message);
  end;
end;


function TA026FDatiLiberiMW.GetDatoObbligatorio(NomeDato:string):boolean;
begin
  Result:=False;
  selColsNULLABLE.ClearVariables;
  selColsNULLABLE.SetVariable('COLUMN_NAME',NomeDato);
  try
    selColsNULLABLE.Execute;
    Result:=selColsNULLABLE.FieldAsString('NULLABLE') <> 'Y';
  except
  end;
end;
...
begin
  ...
  try
    ...
    if DS.FieldByName('Formato').AsString = 'S' then
      SQL:=Format('ALTER TABLE T430_Storico ADD %s VARCHAR2(%d)',[DS.FieldByName('NomeCampo').AsString,Lung])
    else if DS.FieldByName('Formato').AsString = 'N' then
      SQL:=Format('ALTER TABLE T430_Storico ADD %s NUMBER',[DS.FieldByName('NomeCampo').AsString])
    else
      SQL:=Format('ALTER TABLE T430_Storico ADD %s DATE',[DS.FieldByName('NomeCampo').AsString]);
    Q430.Sql.Add(SQL);
    Q430.Execute;
    if Parametri.VersioneOracle >= 10 then
      DropQ430_Appoggio.SQL.Text:='DROP TABLE T430_APPOGGIO PURGE'
...
begin
  if Copy(DS.FieldByName('NomeCampo').AsString.ToUpper,1,2) = 'D_' then
    raise Exception.Create('Il nome non può cominciare con "D_"!');
  T030.Close;
