...
uses
  ...
  Db, Menus, Buttons, ExtCtrls, ComCtrls, StdCtrls, Mask, DBCtrls, checklst, Oracle,
  ActnList, ImgList, ToolWin, SelAnagrafe, Qrctrls, qrprntr, Math, Printers, OracleData,
  Variants, QRExport, QRPDFFilt, QuickRpt, C013UCheckList, Generics.Collections, System.Actions,
  {$IFDEF MSWINDOWS} Winapi.ShellAPI, {$ENDIF MSWINDOWS} A000UCostanti, A000USessione,
  A000UInterfaccia, A000UMessaggi, A062UQueryServizio, A083UMsgElaborazioni,
  C001StampaLib, C004UParamForm, C005UDatiAnagrafici, C007ULabelSize, C012UVisualizzaTesto,
  C180FunzioniGenerali, C700USelezioneAnagrafe, L021Call, R001UGESTTAB,
  R003UGeneratoreStampeMW, System.Diagnostics, System.ImageList, InputPeriodo;
...
type
  ...
  TR003FGeneratoreStampe = class(TR001FGestTab)
    ...
    N7: TMenuItem;
    Generazionetabelladaultimaelaborazione1: TMenuItem;
    btnAnomalie: TBitBtn;
    frmInputPeriodo: TfrmInputPeriodo;
    btnSalva: TBitBtn;
    dchkRotturaPeriodiNonContigui: TDBCheckBox;
    ...
    procedure Generazionetabelladaultimaelaborazione1Click(Sender: TObject);
...
uses R003UGeneratoreStampeDtM, R003UFormato, R003UStampa, R003UDatiCalcolati;
...
begin
  ...
  C004_R003:=CreaC004(SessioneOracle,R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.CodForm,Parametri.ProgOper,False);
...
procedure TR003FGeneratoreStampe.Generazionetabelladaultimaelaborazione1Click(Sender: TObject);
var
  msg,LastExec_Time,LastExec_App,LastExec_Codice,LastExec_Dal,LastExec_Al,LastExec_Anagra:String;
begin
  inherited;
  C004_R003.selT001.Refresh;
  LastExec_Time:=C004_R003.GetParametro('LASTEXEC_TIME','');
  LastExec_App:=C004_R003.GetParametro('LASTEXEC_APP','');
  LastExec_Codice:=C004_R003.GetParametro('LASTEXEC_CODICE','');
  LastExec_Dal:=C004_R003.GetParametro('LASTEXEC_DAL','');
  LastExec_Al:=C004_R003.GetParametro('LASTEXEC_AL','');
  LastExec_Anagra:=C004_R003.GetParametro('LASTEXEC_ANAGRA','');
  ...
  if C700SelAnagrafe.RecordCount = 0 then
    raise Exception.Create('Nessuna anagrafica selezionata!');
  ...
  if MessageDlg('La tabella verrà generata a partire da questa elaborazione:' + CRLF + msg + CRLF + 'Proseguire?',mtConfirmation,[mbYes,mbNo],0,mbNo) = mrNo then
    exit;

  if VarToStr(R003FGeneratoreStampeDtm.Q910.Lookup('CODICE',LastExec_Codice,'TABELLA_GENERATA')) = '' then
    raise Exception.Create('La stampa non consente la generazione della tabella!');
  if not R003FGeneratoreStampeDtm.Q910.SearchRecord('CODICE',LastExec_Codice,[srFromBeginning]) then
    raise Exception.Create('Stampa "' + LastExec_Codice + '" non disponibile!');
  DataI:= StrToDateTime(LastExec_Dal);
  DataF:= StrToDateTime(LastExec_Al);

  R003FGeneratoreStampeDtm.GeneraTabellaOnly:=True;
  try
    btnAnteprimaClick(btnTabella);
  finally
    R003FGeneratoreStampeDtm.GeneraTabellaOnly:=False;
  end;
end;
...
begin
  ...
  begin
    ...
    (ctrl as TEdit).Text:=Valore
  else if ctrl is TMaskEdit then
    (ctrl as TMaskEdit).Text:=Valore;
end;
...
begin
  ...
  begin
    Result:=(ctrl as TEdit).Text;
  end
  else if ctrl is TMaskEdit then
  begin
    Result:=(ctrl as TMaskEdit).Text;
  end
end;
...
begin
  ...
  Generazionetabelladaultimaelaborazione1.Enabled:=DButton.State = dsBrowse;
...
procedure TR003FGeneratoreStampe.btnAnomalieClick(Sender: TObject);
begin
  inherited;
  frmSelAnagrafe.SalvaC00SelAnagrafe;
  C700Distruzione;
  OpenA083MsgElaborazioni(Parametri.Azienda,Parametri.Operatore,Copy(Self.Name,1,4),'');
...
var D,D1:TDateTime;
    SQLOri,CdcTab,CdcCod,CdcSt,S,ST433,NomeFile:String;
    i,ProgErrato,Preference:Integer;
    DaData: TDateTime;
    AData: TDateTime;
...
begin
  ...
  *)
  ...
  begin
    ...
    RotturaPeriodiNonContigui:=dchkRotturaPeriodiNonContigui.Checked;
  ...
  try
    ...
    //Controllo se stampa in uso da altro operatore
    S:=R003FGeneratoreStampeDtM.ControllaSemaforo;
    if (S <> '') and (TipoModulo = 'CS') then
    begin
      if MessageDlg(S + #13#10 + 'L''esecuzione richiesta può bloccare la precedente elaborazione.' + #13#10 + 'Continuare?',mtConfirmation,[mbYes,mbNo],0) = mrNo then
        Abort;
    end;
    R003FGeneratoreStampeDtM.BloccaSemaforo;

    // segnala avvio elaborazione
    StatusBar.Panels[3].Text:='Elaborazione in corso...';
    StatusBar.Repaint;

    // avvio timer di precisione
    LStopwatch:=TStopwatch.StartNew;

    RegistraMsg.IniziaMessaggio(Copy(Self.Name,1,4));
    with R003FGeneratoreStampeDtM do
    begin
      ...
      begin
        exeDropTabs.Lines.Add(selDropTabs.FieldByName('DROPTAB').AsString.Replace(';',IfThen(Parametri.VersioneOracle >= 10,' purge') + ';'));
        selDropTabs.Next;
      end;
      exeDropTabs.Execute;
      ...
      begin
        S:='T920' + R003FGeneratoreStampeMW.CreaNomeTabella(selDropUserTabs.FieldByName('TABELLA_GENERATA').AsString,DaData,AData);
        S:=Format('  select max(''drop table %s%s'') into stmt from user_objects where object_type = ''TABLE'' and object_name = ''%s'';',[S,IfThen(Parametri.VersioneOracle >= 10,' purge'),S]);
        exeDropTabs.Lines.Add('declare');
        exeDropTabs.Lines.Add('  stmt varchar2(10000) := null;');
        exeDropTabs.Lines.Add('begin');
        exeDropTabs.Lines.Add(S);
        exeDropTabs.Lines.Add('  if stmt is not null then execute immediate stmt; end if;');
        exeDropTabs.Lines.Add('end;');
        exeDropTabs.Lines.Add('/');
        //exeDropTabs.Lines.Add('DROP TABLE T920' + R003FGeneratoreStampeMW.CreaNomeTabella(selDropUserTabs.FieldByName('TABELLA_GENERATA').AsString,DaData,AData) + IfThen(Parametri.VersioneOracle >= 10,' PURGE') + ';');
    ...
    try
      CreaTabelle;
      C004_R003.Cancella001;
      if TabellaStampa <> '' then
      begin
        //registrazione su C004 dei dati di stampa: nome stampa e dal..al
        C004_R003.PutParametro('LASTEXEC_TIME',FormatDateTime('dd/mm/yyyy hh.mm',Now));
        C004_R003.PutParametro('LASTEXEC_APP',Parametri.Applicazione);
        C004_R003.PutParametro('LASTEXEC_CODICE',R003FGeneratoreStampeDtM.Q910.FieldByName('CODICE').AsString);
        C004_R003.PutParametro('LASTEXEC_DAL',frmInputPeriodo.edtInizio.Text);
        C004_R003.PutParametro('LASTEXEC_AL',frmInputPeriodo.edtFine.Text);
        //salvare numero record
      end;
      SessioneOracle.Commit;
      try
        if not R003FGeneratoreStampeDtM.GeneraTabellaOnly then
      ...
      try
        ...
        begin
          //registrazione su C004 dei dati di stampa: nome stampa e dal..al
          C004_R003.PutParametro('LASTEXEC_ANAGRA',R003FGeneratoreStampeDtM.NumAnagraficheElaborate.ToString);
          SessioneOracle.Commit;
        end;
      except
      end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if LowerCase(Copy(DocumentoPDF,Length(DocumentoPDF) - 4,5)) = '.xlsx' then
            R180DatasetToXlsxFile(SessioneOracle,True,R003FGeneratoreStampeDtM.cds920,DocumentoPDF);
        end;
      end
      ...
      else if (Sender = btnSalva) and (TipoModulo = 'CS') then
      begin
        SaveDialog1.Filter:='File Excel|*.xlsx|File CSV|*.csv';
        ...
        begin
          ...
          begin
            ...
            R180DatasetToXlsxFile(SessioneOracle,True,R003FGeneratoreStampeDtM.cds920, SaveDialog1.FileName);
          end
          ...
          begin
            ...
            R180DatasetToCsv(R003FGeneratoreStampeDtM.cds920,SaveDialog1.FileName);
          end;
          {$IFDEF MSWINDOWS}
  ...
  // visualizza durata elaborazione
  StatusBar.Panels[3].Text:='Tempo di esecuzione: ' + FormatDateTime(FMT_DURATA_ELABORAZIONE,LStopwatch.ElapsedMilliseconds / MSecsPerDay);
  StatusBar.Repaint;
...
begin
  if DButton.DataSet.RecordCount = 0 then
    exit;
  SaveDialog1.Filter:='Scripts SQL (*.SQL)|*.SQL|Files di testo (*.txt)|*.txt|Tutti i files (*.*)|*.*';
...
procedure TR003FGeneratoreStampe.Stampa1Click(Sender: TObject);
begin
  QueryStampa.Clear;
  QueryStampa.Add('select T910.*');
  QueryStampa.Add('  from T910_RIEPILOGO T910');
  QueryStampa.Add(' where T910.APPLICAZIONE = ''' + Parametri.Applicazione + '''');
  QueryStampa.Add(' and T910.TEMP_B028 = ''N''');
  QueryStampa.Add(' order by T910.CODICE');
  inherited;
end;
