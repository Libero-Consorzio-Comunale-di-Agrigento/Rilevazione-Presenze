...
// ########################################################################## //
//                                                                            //
//                 Questa unit è inclusa nei progetti web                     //
//                                                                            //
// ########################################################################## //
...
uses
  ...
  R004UGestStorico,
  {$ENDIF}
  StdCtrls, // richiesta in classe TInterfacciaR004 (field di tipo TCheckBox)
  Dialogs,  // richiesta in procedure BeforePost (richiamo a MessageDlg)
  Controls, // richiesta in procedure BeforePost (simbolo mrYes utilizzato per confronto valore di MessageDlg)
  SysUtils, Classes,
  A000UCostanti, A000UMessaggi, A000USessione, A000UInterfaccia,
  Oracle, OracleData, Db, Variants,
  C180FunzioniGenerali, RegistrazioneLog, StrUtils, System.UITypes;
...
begin
  ...
  if (Copy(Self.Name,1,4) = 'P430') and InterfacciaR004.StoricizzazioneInCorso then
    RegistraMsg.InserisciMessaggio('A','Fine BeforePost: Decorrenza: ' + TOracleDataSet(DataSet).FieldByName('Decorrenza').AsString + IfThen(DD > 0,' - Ini. prec: ' + DateToStr(DD)) + IfThen(DF > 0,' - Fin. prec: ' + DateToStr(DF)) +
                                                             ' - chkStoriciPrec: ' + IfThen(InterfacciaR004.chkStoriciPrec.Checked,'True','False') +
                                                             ' - chkStoriciSucc: ' + IfThen(InterfacciaR004.chkStoriciSucc.Checked,'True','False') +
                                                             IfThen(InterfacciaR004.chkStoriciPrec.Checked or InterfacciaR004.chkStoriciSucc.Checked,' - QUpdateStorici: ' + QUpdateStorici.SQL.Text),
                                   '',TOracleDataSet(DataSet).FieldByName('PROGRESSIVO').AsInteger);
end;
    ...
    try
      ...
      begin
        ...
        if (InterfacciaR004.LChiavePrimaria.IndexOf(NomeCampo) = -1) and
           (UpperCase(NomeCampo) <> 'DECORRENZA') and
           not ((UpperCase(NomeCampo) = 'DECORRENZA_FINE') and InterfacciaR004.AllineaSoloDecorrenzeIntersecanti and InterfacciaR004.GestioneDecorrenzaFine)
        then
        try
...
var ChiaveSenzaDec,ChiaveConDec:Variant;
    ...
    i:Integer;
begin
  if (Copy(Self.Name,1,4) = 'P430') and RegistraMsg.ContieneTipoA then
    RegistraMsg.InserisciMessaggio('A','Inizio AfterPost: Passaggio da AllineaDecorrenzaFine ' + IfThen(not InterfacciaR004.GestioneStoricizzata or not InterfacciaR004.GestioneDecorrenzaFine,'NON ') + 'previsto' +
                                                                   ' - GestioneStoricizzata: ' + IfThen(InterfacciaR004.GestioneStoricizzata,'True','False') +
                                                                 ' - GestioneDecorrenzaFine: ' + IfThen(InterfacciaR004.GestioneDecorrenzaFine,'True','False') +
                                                                        ' (OttimizzaStorico: ' + IfThen(InterfacciaR004.OttimizzaStorico,'True','False') +
                                                                  ' - bExecuteUpdateStorici: ' + IfThen(bExecuteUpdateStorici,'True','False') + ')',
                                   '',TOracleDataSet(DataSet).FieldByName('PROGRESSIVO').AsInteger);
...
var NomeTab,Chiave,FiltroChiave:String;
    i,j:Integer;
begin
  ...
  begin
    ...
    //Nel caso di P430, l'allineamento avviene solo per il Progressivo corrente. Per le altre tabelle questa logica è difficilmente estendibile e rimane l'allineamento dell'intera tabella
    if Copy(Self.Name,1,4) = 'P430' then
      for j:=0 to InterfacciaR004.LValoriOriginali.Count - 1 do
        if Pos('=' + InterfacciaR004.LChiavePrimaria[i] + '=','=' + InterfacciaR004.LValoriOriginali[j]) > 0 then
        begin
          if FiltroChiave <> '' then
            FiltroChiave:=FiltroChiave + ' and ';
          FiltroChiave:=FiltroChiave + InterfacciaR004.LValoriOriginali[j];//Funziona solo per campi numerici
          Break;//esce dal ciclo di j
        end;
  end;
  if (Copy(Self.Name,1,4) = 'P430') and RegistraMsg.ContieneTipoA then
    RegistraMsg.InserisciMessaggio('A','Passaggio AllineaDecorrenzaFine: NomeTab: ' + NomeTab + ' - Chiave: ' + Chiave + ' - AllineaSoloDecorrenzeIntersecanti: ' + IfThen(InterfacciaR004.AllineaSoloDecorrenzeIntersecanti,'True','False'),'',TOracleDataSet(DataSet).FieldByName('PROGRESSIVO').AsInteger);
  ...
  try
    ...
    SQL.Add('  where');
    if Trim(FiltroChiave) <> '' then
      SQL.Add('    ' + FiltroChiave + ' and');
    ...
    try
      Execute;
      if (Copy(Self.Name,1,4) = 'P430') and RegistraMsg.ContieneTipoA then
        RegistraMsg.InserisciMessaggio('A','Eseguito AllineaDecorrenzaFine: SQL.Text: ' + SQL.Text,'',TOracleDataSet(DataSet).FieldByName('PROGRESSIVO').AsInteger);
      TOracleDataSet(DataSet).RefreshRecord;
    except
      on E: Exception do
      begin
        RegistraMsg.InserisciMessaggio('A','Errore AllineaDecorrenzaFine: ' + E.Message + ' - SQL.Text: ' + SQL.Text,'',TOracleDataSet(DataSet).FieldByName('PROGRESSIVO').AsInteger);
        if Copy(Self.Name,1,4) = 'P430' then
          ShowMessage('Si è verificato un errore in fase di allineamento delle date di decorrenza del periodo storico. Contattare il servizio di assistenza e segnalare l''errore!');
      end;
    end;
  finally
    Free;
