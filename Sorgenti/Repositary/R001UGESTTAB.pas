...
uses
  ...
  ComCtrls, Menus, Grids, DbCtrls, DBGrids, StdCtrls, StrUtils,
  ...
  C700USelezioneAnagrafe, ToolbarFiglio, System.Actions
  {$IFNDEF VER250}{<> XE4}
  , System.ImageList
  {$ENDIF}
  ;
...
type
  ...
  private
    ...
    procedure CreaNuovaStampaPiantaOrg(S:String);
...
begin
  ...
  actCopiaSu.Visible:=(Self.Name = 'A006FModOra') or
                      ...
                      (Self.Name = 'P077FGeneratoreStampe') or
                      (Self.Name = 'Q060FCommesse') or
                      (Self.Name = 'S200FEstremiStruttura') or
                      (Self.Name = 'S203FStampaPiantaOrganica');
...
begin
  ...
  if WC = nil then
    WC:=Self.FindNextControl(nil,true,true,false);//Prende il primo componente in base al TabOrder della form
  if (WC <> nil) and (WC is TDBGrid) and ((WC as TDBGrid).ReadOnly) then
    WC:=Self.FindNextControl(WC,true,true,false);
  if (WC <> nil) and (not WC.CanFocus) then
    WC:=Self.FindNextControl(WC,true,true,false);
  if WC = nil then
    WC:=Self.FindNextControl(nil,true,true,false);//Prende il primo componente in base al TabOrder della form

  if (WC <> nil) and WC.CanFocus then
    WC.SetFocus;
  DButton.DataSet.Insert;
end;

procedure TR001FGestTab.DButtonDataChange(Sender: TObject; Field: TField);
begin
  try
    if Field = nil then
      NumRecords;
...
begin
  ...
  for i:=0 to Self.ComponentCount - 1 do
    ...
    begin
      ...
      //Rendo read-only il dataset collegato, se TOracleDataSet
      if (TfrmToolbarFiglio(Self.Components[i]).DButton <> nil) and
         (TfrmToolbarFiglio(Self.Components[i]).DButton.DataSet is TOracleDataSet) then
      begin
        if not(Browse) and not(SolaLettura) then
        begin
          //Dataset in sola lettura: Registro lo stato ReadOnly originale prima di modificarlo a True
          TfrmToolbarFiglio(Self.Components[i]).TFReadOnly:=(TfrmToolbarFiglio(Self.Components[i]).DButton.DataSet as TOracleDataSet).ReadOnly;
          (TfrmToolbarFiglio(Self.Components[i]).DButton.DataSet as TOracleDataSet).ReadOnly:=True;
        end
        else
        begin
          //Dataset modificabile: Ripristino lo stato originale di ReadOnly
          (TfrmToolbarFiglio(Self.Components[i]).DButton.DataSet as TOracleDataSet).ReadOnly:=TfrmToolbarFiglio(Self.Components[i]).TFReadOnly;
        end;
      end;
    end;
end;
  ...
  try
    ...
    for i:=0 to FieldCount - 1 do
      ...
      begin
        ...
        Grid.Cells[0,j]:=IfThen((Fields[i].Calculated) or (Fields[i].Lookup),'(*)') + Fields[i].DisplayLabel;
        if (Fields[i].Calculated) or (Fields[i].Lookup) then
          EsisteCalcLookup:=True;
      end;
    if EsisteCalcLookup then
      chkFiltro.Caption:='&Applica filtro - i campi con (*) non sono considerati';
    Grid.RowCount:=Campi.Count + 1;
    if ShowModal = mrOk then
    begin
      Filtro:='';
      for i:=1 to Grid.RowCount - 1 do
        if (Trim(Grid.Cells[1,i]) <> '') and
           (not chkFiltro.Checked or not((Fields[i - 1].Calculated) or (Fields[i - 1].Lookup))) then
        begin
...
begin
  ...
  try
    ...
    //Abilito il popup di chiusura e riposizionamento solo nel caso in cui la form erediti da R004
    if DButton.DataSet.FindField('CODICE') <> nil then
      C001FFiltroTabelle.Chiudieposizionati1.Visible:=True;
    C001FFiltroTabelle.ShowModal;
    {-------------------------------------------------------------------------
    Riposiziono dataset su record selezionato della maschera C001FFiltroTabelle
    --------------------------------------------------------------------------}
    if (DButton.State = dsBrowse) and C001FFiltroTabelle.PosizionatiSuRecord then
    begin
      CampiChiave:='CODICE';
      SetLength(ValoriChiave,1);
      ValoriChiave[High(ValoriChiave)]:=C001FFiltroTabelleDTM.Query1.FieldByName('CODICE').AsVariant;
      DButton.DataSet.Locate(CampiChiave,ValoriChiave,[]);
    end;
    {------------------------------------------------------------------------}
...
begin
  ...
  if UpperCase(Self.Name) = 'A006FMODORA' then
    ...
    T:='Nome cartellino:'
  else if UpperCase(Self.Name) = 'Q060FCOMMESSE' then
  begin
    T:='Commessa:';
    Nuovo:=CodiceCopiaSu;
  end;
  ...
  begin
    ...
    try
      ...
      else if UpperCase(Self.Name) = 'S203FSTAMPAPIANTAORGANICA' then
        CreaNuovaStampaPiantaOrg(Nuovo)
      else
...
begin
  ...
  try
    ...
    else if UpperCase(Self.Name) = 'A205FSQUADRE' then
      A000AggiornaFiltroDizionario('SQUADRE TURNI','',S);
    if Posiziona then
    begin
      DButton.DataSet.Refresh;
      NumRecords;
...
procedure TR001FGestTab.CreaNuovaStampaPiantaOrg(S:String);
var Q:TOracleQuery;
    ODS:TOracleDataSet;
    Tab,Campi,Valori,CampoCod:String;
    i:Integer;
begin
  Q:=TOracleQuery.Create(nil);
  try
    Q.Session:=TOracleDataSet(DButton.DataSet).Session;
    Q.SQL.Add(Format('SELECT COUNT(*) CODICE FROM SG508_STAMPAPIANTA WHERE CODICE = ''%s''',[S]));
    Q.Execute;
    if Q.FieldAsInteger('CODICE') > 0 then
      raise exception.Create('Codice già esistente!')
    else
    begin
      Q.SQL.Clear;
      Q.SQL.Add('DELETE FROM SG509_DETTAGLIOSTAMPA WHERE CODICE_STAMPA = ''' + S + '''');
      Q.Execute;
      Q.SQL.Clear;
      Q.SQL.Add('DELETE FROM SG510_DATISTAMPAPIANTA WHERE CODICE_STAMPA = ''' + S + '''');
      Q.Execute;
      Q.SQL.Clear;
      Q.SQL.Add('DELETE FROM SG511_STAMPAVALORI WHERE CODICE_STAMPA = ''' + S + '''');
      Q.Execute;
      ODS:=TOracleDataSet.Create(nil);
      try
        ODS.Session:=TOracleDataSet(DButton.DataSet).Session;
        for i:=1 to 4 do
        begin
          case i of
            1: Tab:='SG508_STAMPAPIANTA';
            2: Tab:='SG509_DETTAGLIOSTAMPA';
            3: Tab:='SG510_DATISTAMPAPIANTA';
            4: Tab:='SG511_STAMPAVALORI';
          end;
          Campi:='';
          ODS.Close;
          ODS.SQL.Clear;
          ODS.SQL.Add(Format('SELECT COLUMN_NAME FROM COLS WHERE TABLE_NAME = ''%s'' order by COLUMN_ID',[Tab]));
          ODS.Open;
          while not ODS.Eof do
          begin
            Campi:=Campi + IfThen(Campi <> '',',') + ODS.FieldByName('COLUMN_NAME').AsString;
            ODS.Next;
          end;
          CampoCod:=IfThen(i = 1,'CODICE','CODICE_STAMPA');
          Valori:=StringReplace(',' + Campi + ',',',' + CampoCod + ',',',''' + S + ''',',[rfReplaceAll]);
          Valori:=Copy(Valori,2,Length(Valori) - 2);
          Q.SQL.Clear;
          Q.SQL.Add('INSERT INTO ' + Tab);
          Q.SQL.Add('(' + Campi + ')');
          Q.SQL.Add('SELECT ' + Valori);
          Q.SQL.Add('FROM ' + Tab + ' WHERE ' + CampoCod + ' = ''' + DButton.DataSet.FieldByName('CODICE').AsString + '''');
          Q.Execute;
        end;
        Q.Session.Commit;//Al termine degli interventi
        DButton.DataSet.Refresh;
      finally
        ODS.Free;
      end;
    ...
    Q.Free;
    Q.Session.Rollback;//Nel caso in cui qualcosa sia andato storto
  end;
end;
