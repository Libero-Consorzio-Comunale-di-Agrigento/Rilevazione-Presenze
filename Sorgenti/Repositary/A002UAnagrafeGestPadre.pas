...
uses
  ...
  Mask, DB, Buttons, OracleData, Oracle, ToolWin, ActnList, ImgList,
  A000UCostanti, A000UInterfaccia, A000UMessaggi, A000USessione,
  A002ULayout, A002UInterfacciaSt,
  A005UDatiLiberi, A011UComuniProvinceRegioni,
  A082UCdcPercent, 
  ...
  C006UStoriaDati, C013UCheckList, C180FunzioniGenerali, C700USelezioneAnagrafe,
  L001Call, L021Call, QueryStorico,
  Variants, Printers, Math, Shellapi
...
type
  ...
  TA002FAnagrafeGestPadre = class(TForm)
    ...
    lblAziendaBase: TLabel;
    dcmbAziendaBase: TDBLookupComboBox;
    dlblAziendaBase: TDBText;
    actShortTerm: TAction;
    ToolButton22: TToolButton;
    TShortTerm: TToolButton;
    EI060EMailPEC: TDBEdit;
    LI060EMailPEC: TLabel;
    drgpTipoSoggettoBase: TDBRadioGroup;
    LI060Cellulare: TLabel;
    EI060Cellulare: TDBEdit;
    ToolButton23: TToolButton;
    TLoginDipendente: TToolButton;
    actLoginDipendente: TAction;
    N9: TMenuItem;
    Logindipendente1: TMenuItem;
    mnuRiepilogorapportidilavoro: TMenuItem;
    EI060EMailPersonale: TDBEdit;
    EI060CellularePersonale: TDBEdit;
    LI060CellularePersonale: TLabel;
    LI060EMailPersonale: TLabel;
    ...
    procedure actShortTermExecute(Sender: TObject);
  ...
  private
    ...
    ShortTermStartDateHROld, ShortTermCessDateHROld:TDateTime;
...
begin
  ...
  actShortTerm.Visible:=Parametri.CampiRiferimento.C1_CedoliniConValuta = 'S';
end;
...
begin
  ...
  with A002FAnagrafeDtM1 do
    btnStoricizza.Enabled:=(AbilAnagra = aaReadWrite) and (Parametri.Storicizzazione = 'S') and (Q030.State = dsBrowse) and Q430.Active and (Q430.RecordCount > 0);
...
begin
  ...
  begin
    ...
    Storico[41].CampoInput:=dcmbAziendaBase;
    //MONDOEDP - Commessa: MAN/02
    Storico[42].CampoInput:=dchkRapportiUniti;
  end;
end;
...
procedure TA002FAnagrafeGestPadre.mnuRiepilogorapportidilavoroClick(Sender: TObject);
begin
  A002FAnagrafeVista.frmSelAnagrafe.SalvaC00SelAnagrafe;
  C700Distruzione;
  OpenA202RapportiLavoro(A002FAnagrafeDtM1.Q030.FieldByName('Progressivo').AsInteger);
...
begin
  ...
  Storicizzati.Enabled:=(cmbDateDecorrenza.ItemIndex > 0) and
                        (   (Parametri.StoriaInizioFine <> 'S')
                         or ((A002FAnagrafeDtM1.Q430.FieldByName('DATADECORRENZA').AsDateTime > A002FAnagrafeDtM1.Q430.FieldByName('INIZIO').AsDateTime) and
                             (A002FAnagrafeDtM1.Q430.FieldByName('FINE').IsNull or
                             (A002FAnagrafeDtM1.Q430.FieldByName('DATADECORRENZA').AsDateTime < A002FAnagrafeDtM1.Q430.FieldByName('FINE').AsDateTime))));
  ...
  mnuRiepilogorapportidilavoro.Visible:=((PopupMenu1.PopupComponent = EInizio) or (PopupMenu1.PopupComponent = EFine) or (PopupMenu1.PopupComponent = EPAssenze))
                                        and
                                        (A000GetInibizioni('Funzione','OpenA202RapportiLavoro') <> 'N');
  N2.Visible:=Nuovo.Visible or Relazioni.Visible or False;
end;
...
begin
  ...
  begin
    ...
    A002FAnagrafeMW.selCOLS.Refresh;
  ...
  with A002FAnagrafeDtM1 do
    ...
    begin
      if PopupMenu1.PopupComponent = LookupComune then
        ...
        c:=dedtComune.Text
      else if PopupMenu1.PopupComponent = dcmbComuneDomBase then
        c:=dedtComuneDomBase.Text
    ...
    else if PopupMenu1.PopupComponent.Tag = 16 then
    begin
      //OpenAxxxAziendeBase((PopupMenu1.PopupComponent as TDBLookupComboBox).Text)
    end
...
var
  ...
  ProgSave:Integer;
  AziendaBasePrec,AziendaBaseAtt,Msg:String;
begin
  if (Parametri.CampiRiferimento.C1_CedoliniConValuta = 'S') and (Sender <> actConferma) then
  begin
    ShortTermStartDateHROld:=A002FAnagrafeDtM1.Q430.FieldByName('START_DATE_HR').AsDateTime;
    ShortTermCessDateHROld:=A002FAnagrafeDtM1.Q430.FieldByName('CESS_DATE_HR').AsDateTime;
  end;
  ...
  begin
    ShortTermStartDateHROld:=0;
    ShortTermCessDateHROld:=0;
    ...
    try
      ...
      begin
        if E.Message = A000MSG_ERR_DATABASE_RECORD_LOCKED then
          R180MessageBox(A000TraduzioneStringhe(A000MSG_A002_ERR_SCHEDA_LOCK), ESCLAMA)
        else
  ...
  begin
    ...
    begin
      ...
      begin
        Msg:=A002FAnagrafeMW.VerificaCampiNonAbilitati(Q430.FieldByName('Progressivo').AsInteger,Q430.FieldByName('DataDecorrenza').AsDateTime);
        if Msg <> '' then
        begin
          Msg:=Format(A000MSG_A002_ERR_FMT_DECO_CAMPI_DISABIL,['cancellazione',Msg]);
        ...
        begin
          ...
          if not A002FAnagrafeDtM1.A002FAnagrafeMW.TryLockRecordT030(Q430.FieldByName('Progressivo').AsInteger) then
          begin
            R180MessageBox(A000TraduzioneStringhe(A000MSG_A002_ERR_SCHEDA_LOCK), ESCLAMA);
            Exit;
          end;
          ...
          try
            ...
            begin
              ...
              (*
              Q430.Close;
              Q430.SetVariable('DataLav',DecorrenzaSave - 1);
              Q430.Open;
              AziendaBasePrec:=Q430.FieldByName('AZIENDA_BASE').AsString;
              //Mi riposiziono sul record attuale per l'eventuale cancellazione del periodo o per l'annullamento dell'operazione
              Q430.Close;
              Q430.SetVariable('DataLav',DecorrenzaSave);
              Q430.Open;
              *)
              AziendaBaseAtt:=Q430.FieldByName('AZIENDA_BASE').AsString;
              AziendaBasePrec:='';
              with TQueryStorico.Create(nil) do
              try
                Session:=SessioneOracle;
                GetDatiStorici('T430AZIENDA_BASE',Q430.FieldByName('PROGRESSIVO').AsInteger,DecorrenzaSave - 1,DecorrenzaSave - 1);
                AziendaBasePrec:=FieldByName('T430AZIENDA_BASE').AsString;
              finally
                Free;
              end;
              ...
              if (AziendaBasePrec = AziendaBaseAtt)
              or (R180MessageBox(Format(A000MSG_A002_DLG_FMT_DEL_AZIENDA,[AziendaBasePrec,AziendaBaseAtt]),'DOMANDA') = mrYes) then
              begin
              ...
              begin
                ...
                begin
                  ...
                  if not A002FAnagrafeMW.VerificaCedAntePrimoStoricoStip then
                    R180MessageBox(A000MSG_P430_ERR_CED_ANTE_PRIMO_STORICO,INFORMA);
                  if not A002FAnagrafeMW.AggiornaPeriodiStorici  then
                    R180MessageBox(A000MSG_A002_ERR_DIP_IN_USO,ERRORE);
            ...
            // rimuove il lock su T030
            A002FAnagrafeDtM1.A002FAnagrafeMW.UnlockRecordT030Rollback;
          end;
        end;
      end;
    end;
  ...
  if (Parametri.CampiRiferimento.C1_CedoliniConValuta = 'S') and
     ((Sender = actConferma) or (Sender = actCancella)) then
    with A002FAnagrafeDtM1.Q430 do
    begin
      if (ShortTermStartDateHROld <> FieldByName('START_DATE_HR').AsDateTime) or
         (ShortTermCessDateHROld <> FieldByName('CESS_DATE_HR').AsDateTime) then
      begin
         A002FAnagrafeDtM1.OpenA002ShortTerm(
           FieldByName('Progressivo').AsInteger,
           'Variazione',
           FieldByName('TIPORAPPORTO').AsString,
           StrToDate('31/12/3999'),
           IfThen(FieldByName('START_DATE_HR').AsString = '',StrToDate('31/12/3999'),FieldByName('START_DATE_HR').AsDateTime),
           IfThen(FieldByName('CESS_DATE_HR').AsString = '',StrToDate('31/12/3999'),FieldByName('CESS_DATE_HR').AsDateTime));
      end;
    end;
end;
...
procedure TA002FAnagrafeGestPadre.EMailDblClick(Sender: TObject);
var campo: String;
begin
  campo:='';
  if (Sender = EI060EMailPersonale) then
    campo:='I060EMailPersonale'
  else if Sender = EI060EMailPEC then
    campo:='I060EMailPEC'
  else if Sender = EI060EMail then
    campo:='I060EMail';
  if (campo <> '') and (A002FAnagrafeDtM1.Q030.FieldByName(campo).AsString <> '') then
    ShellExecute(0,'open', pchar('mailto:'+ A002FAnagrafeDtM1.Q030.FieldByName(campo).AsString), NIL, NIL, SW_SHOWNORMAL);
end;
...
begin
  ...
  begin
    ...
    try
      C013FCheckList.Caption:='<A002> Filtro assenze abilitate';
...
begin
  ...
  begin
    C013FCheckList:=TC013FCheckList.Create(nil);
    try
      C013FCheckList.Caption:='<A002> Filtro presenze abilitate';
...
begin
  ...
  begin
    // effettua l'edit su T030, tentando di acquisire il lock sul record
    try
      A002FAnagrafeDtM1.StoricizzazioneInCorso:=True;
      A002FAnagrafeDtM1.Q030.Edit;
    except
      on E: EDatabaseError do
      begin
        A002FAnagrafeDtM1.StoricizzazioneInCorso:=False;
        if E.Message = A000MSG_ERR_DATABASE_RECORD_LOCKED then
          R180MessageBox(A000TraduzioneStringhe(A000MSG_A002_ERR_SCHEDA_LOCK), ESCLAMA)
        else
          R180MessageBox(E.Message, ESCLAMA);
    ...
    (*// spostato sopra per acquisizione lock
    StoricizzazioneInCorso:=True;
    Q030.Edit;
    *)
    A002FAnagrafeDtM1.D430.DataSet.FieldByName('DATADECORRENZA').Clear;
    A002FAnagrafeDtM1.D430.DataSet.FieldByName('DATADECORRENZA').FocusControl;
  end;
end;
...
procedure TA002FAnagrafeGestPadre.actShortTermExecute(Sender: TObject);
begin
  with A002FAnagrafeDtM1.Q430 do
  begin
      A002FAnagrafeDtM1.OpenA002ShortTerm(
      ...
      for i:=0 to High(Layout) do
        if not (Layout[i].LinkComp[0] = nil) then
          ...
          begin
            ...
            A002FCercaCampo.ElencoCampi[j].Pagina:=TTabSheet(TA002FLayout.GetTab(Layout[i].Parent)).Caption;
            A002FCercaCampo.ElencoCampi[j].PageIndex:=TTabSheet(TA002FLayout.GetTab(Layout[i].Parent)).PageIndex;
      ...
      try
        ...
        begin
          ...
          begin
            ...
            PageControl1.ActivePageIndex:=TTabSheet(TA002FLayout.GetTab(A002FLayout.Layout[jLayout].Parent)).PageIndex;
...
procedure TA002FAnagrafeGestPadre.actLoginDipendenteExecute(Sender: TObject);
// visualizza i login dipendente associati al progressivo
var
  LProg: Integer;
begin
  LProg:=A002FAnagrafeDtM1.Q030.FieldByName('PROGRESSIVO').AsInteger;
  OpenA002LoginDipendenti(LProg,False);
end;
