...
uses
  ...
  RegistrazioneLog, QueryPK, QueryStorico, P999UGenerale,
...
type
  ...
  TR003FGeneratoreStampeDtM = class(TDataModule)
    ...
    selVSG101: TOracleDataSet;
    selT919TG: TOracleDataSet;
  ...
  protected
    ...
    procedure CreateIndexX920_xx(NumTab:String);
    function  T920StorageParam:String;
  public
    { Public declarations }
    R003FGeneratoreStampeMW: TR003FGeneratoreStampeMW;  //istanza di A077MW o P077MW
    cds920ProgressivoPrec:Integer;
    NumAnagraficheElaborate:Integer;
    ...
    function ControllaSemaforo:String;
...
begin
  ...
  if R003FGeneratoreStampe.TipoModulo = 'COM' then
    Q910.SetVariable('TEMP_B028',null)
  else
    Q910.SetVariable('TEMP_B028','N');
  Q910.Open;
  Q275U305.Open;
...
begin
  with CD920 do
  begin
    S:='T920_' + ParametriOperatore.ToUpper + IfThen(Num > 0,IntToStr(Num));
    SQL.Clear;
    SQL.Add('declare');
    SQL.Add('  stmt varchar2(10000) := null;');
    SQL.Add('begin');
    SQL.Add(Format('  select max(''drop table %s%s'') into stmt from user_objects where object_type = ''TABLE'' and object_name = ''%s'';',[S,IfThen(Parametri.VersioneOracle >= 10,' purge'),S]));
    SQL.Add('  if stmt is not null then execute immediate stmt; end if;');
    SQL.Add('end;');
    try
      Execute;
    except
...
function TR003FGeneratoreStampeDtM.ControllaSemaforo:String;
begin
  Result:='';
  try
    with selT919 do
    begin
      ...
      if RecordCount > 0 then
        Result:=Format('Stampa impegnata dall''operatore %s il %s',[Parametri.Operatore,FormatDateTime('dd/mm/yyyy hh.nn.ss',FieldByName('DATA').AsDateTime)]);
    end;
  except
  end;
  if (R003FGeneratoreStampe.TabellaStampa <> '') and R003FGeneratoreStampe.CreaTabellaGenerata then
  try
    with selT919TG do
    begin
      Close;
      SetVariable('OPERATORE',ParametriOperatore);
      SetVariable('TABELLA_GENERATA',R003FGeneratoreStampe.TabellaStampa);
      Open;
      if RecordCount > 0 then
        Result:=Format('La tabella %s è in generazione dall''operatore %s il %s',[R003FGeneratoreStampe.TabellaStampa,FieldByName('OPERATORE').AsString,FormatDateTime('dd/mm/yyyy hh.nn.ss',FieldByName('DATA').AsDateTime)]);
    end;
  except
  end;
end;
...
begin
  ...
  if (R003FGeneratoreStampe.TabellaStampa <> '') and R003FGeneratoreStampe.CreaTabellaGenerata then
    selT919.FieldByName('TABELLA_GENERATA').AsString:=R003FGeneratoreStampe.TabellaStampa
  else
    selT919.FieldByName('TABELLA_GENERATA').AsString:='';
...
begin
    ...
    begin
      ...
      begin
        ...
        case DatiStampa[i].F of
          0,4:begin
            S:=S + ' VARCHAR2(' + IntToStr(min(max(2000,DatiStampa[i].W),DatiStampa[i].W * 2)) + ')';
  ...
  begin
    ...
    if Join <> '' then
      SQL.Add('WHERE ' + Join + ' and 1<>1');//1<>1 condizione sempre False per garnatire di restituire sempre 0 records ed evitare join pesanti
    //Dati calcolati
    with R003FGeneratoreStampeMW do
      for i:=0 to High(DatiStampa) do
    ...
    SQL.Text:=StringReplace(SQL.Text,':UTENTE','''' + ParametriOperatore + '''',[rfReplaceAll,rfIgnorecase]);
    Open;
  end;
...
begin
  ...
  begin
    ...
    AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,'T920_KEY3','VARCHAR2(40)',otString);
    AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,'T920_KEYTOT3','VARCHAR2(40)',otString);
    for i:=0 to High(R003FGeneratoreStampeMW.DatiStampa) do
      ...
      begin
        ...
        if D = 'DESCRIZIONEASSENZE' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'VARCHAR2(100)',otString) else
        ...
        if D = 'COMPASSTOTALI1' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'COMPASSTOTALI2' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'COMPASSTOTALI3' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'COMPASSTOTALI4' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'COMPASSTOTALI5' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'COMPASSTOTALI6' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        ...
        if D = 'ASSENZEFRUITE1' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZEFRUITE2' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZEFRUITE3' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZEFRUITE4' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZEFRUITE5' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZEFRUITE6' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        ...
        if D = 'ASSENZERESIDUE1' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZERESIDUE2' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZERESIDUE3' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZERESIDUE4' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZERESIDUE5' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        if D = 'ASSENZERESIDUE6' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'NUMBER',otString) else
        ...
        if D = 'DATAFAMILIARE' then AddDatoTabCollegata(OQIns,CreateTable,InsertList,InsertVal,D,'DATE',otDate);
      end;
    CD920.SQL.Add(CreateTable);
    CD920.SQL.Add(') ' + T920StorageParam);
    CD920.Execute;
    CreateIndexX920_xx('3');
    ...
    for i:=0 to High(DatiStampa) do
      ...
      begin
        ...
        if DatiStampa[i].F in [1,2] then
          ...
          AddDatoTabCollegata(Ins920x,CreateTable,InsertList,InsertVal,D,'VARCHAR2(' + max(1,R003FGeneratoreStampeMW.selCols.FieldByName('DATA_LENGTH').AsInteger).ToString + ')',otString);
      end;
    CD920.SQL.Add(CreateTable);
    CD920.SQL.Add(') ' + T920StorageParam);
    CD920.Execute;
...
procedure TR003FGeneratoreStampeDtM.CreateIndexX920_xx(NumTab:String);
{Creazione indice sulla colonna PROGRESSIVOx}
begin
  with CD920 do
  begin
    SQL.Text:=Format('CREATE INDEX %s ON %s(%s) TABLESPACE %s NOPARALLEL',['X920_' + ParametriOperatore + NumTab,'T920_' + ParametriOperatore + NumTab,'PROGRESSIVO' + NumTab,Parametri.TSAusiliario]);
    Execute;
  end;
end;

function TR003FGeneratoreStampeDtM.T920StorageParam:String;
begin
  Result:=' TABLESPACE ' + Parametri.TSAusiliario + ' PCTFREE 5 PCTUSED 80 NOPARALLEL';
end;
...
begin
  ...
  begin
    DeleteVariables;
    SQL.Clear;
    SQL.Add(Format('SELECT %s' + Campi,[Parametri.CampiRiferimento.C26_HintT030V430]));
...
procedure TR003FGeneratoreStampeDtM.GetFamiliari;
const
  RInterno = 30;
begin
  R180SetVariable(selVSG101,'PROGRESSIVO',C700Progressivo);
  R180SetVariable(selVSG101,'DATA2',R003FGeneratoreStampeMW.AData);
  selVSG101.Open;
  GetSerbatoioStd(RInterno,selVSG101);
end;
...
var i,P:Integer;
    S:String;
begin
  //da non eseguire se si sta generando la tabella finale senza elaborazione
...
begin
  ...
  for i:=0 to High(R003FGeneratoreStampeMW.Serbatoi) do
    with R003FGeneratoreStampeMW.Serbatoi[i] do
      ...
      begin
        ...
        ODS.ReadBuffer:=1000;
        ODS.Open;
        NumAnagraficheElaborate:=ODS.RecordCount;
      end
      else
      begin
  ...
  begin
    ...
    SessioneOracle.Commit;
    {Soluzione precedente a 64bit
     selT920Stampa.ApplyUpdates([aaInsert]);
     SessioneOracle.Commit;
...
begin
  {Filtro delle tabelle collegate: se è attiva la gestione dei peridi storici, sul campo Data specificato}
  with R003FGeneratoreStampe do
  begin
    Accept:=True;
    //Inutile, la query contiene già il filtro per progressivo//Accept:=DataSet.FieldByName(R003FGeneratoreStampeMW.TabelleCollegate[DataSet.Tag].Progressivo).AsInteger = DipendenteCorrente.Progressivo;
...
begin
  ...
  try
    ...
    begin
      ...
      Intervallo:=Trunc(Min(D2,C700SelAnagrafe.FieldByName('T430DATAFINE').AsDateTime) -
                        Max(D1,C700SelAnagrafe.FieldByName('T430DATADECORRENZA').AsDateTime)) + 1;
...
var i:Integer;
    S,NC:String;
begin
  //Gestisco la chiave per modificare i records già presenti
  if Q910.FieldByName('TABELLA_GENERATA_KEY').AsString <> '' then
  ...
  selT920Stampa.ClearFields;
  for i:=0 to cds920.FieldCount - 1 do
  try
    NC:=cds920.Fields[i].FieldName;
    if cds920.Fields[i] is TStringField then
    begin
      S:=cds920.Fields[i].AsString;
      selT920Stampa.FieldByName(NC).AsString:=S;
    end
    else
      selT920Stampa.FieldByName(NC).Value:=cds920.Fields[i].Value;
  except
   //Alberto 03/09/2014: in rari casi se non si inizializzano i campi dà access violation
   //Alberto 16/06/2017: dà eccezione AV su alcune stampe degli stipendi; visto sulle colonne T430D_CDC_COSTO e AV_DESC_ACCORPAMENTO, su TORINO_ASLCITTA e ASTI
   //                    Sembra che la presenza di caratteri particolari come '°' possa dare origine al problema....
    on E:Exception do
    begin
      //Alberto 16/06/2017: Provo a riassegnare il valore come stringa, diminuito di un carattere (esperimento???)
      try
        selT920Stampa.FieldByName(NC).Clear;
        S:=cds920.Fields[i].AsString;
        selT920Stampa.FieldByName(NC).AsString:=S.Substring(1,S.Length-2);
      except
        on E:Exception do
        begin
          selT920Stampa.FieldByName(NC).Clear;
          RegistraMsg.InserisciMessaggio('A',Format('%s=%s; Exception: %s',[cds920.Fields[i].FieldName,cds920.Fields[i].AsString,E.Message]),'',cds920.FieldByName('PROGRESSIVO').AsInteger);
        end;
      end;
    end;
  end;
...
begin
  Accept:=True;
  if (R003FGeneratoreStampe <> nil) and (R003FGeneratoreStampe.TipoModulo <> 'COM') then
