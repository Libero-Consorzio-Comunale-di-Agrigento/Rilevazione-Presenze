...
uses
  ...
  QRExport, C001StampaLib, Variants, QRWebFilt, QRPDFFilt,
...
var i,P,C:Integer;
    L:TQRLabel;
    QRCB:TQRChildBand;
    EsisteRotturaPeriodiStorici:Boolean;
    CampoDiRotturaAggiuntivo:String;
...
begin
  ...
  begin
    ...
    // BENEVENTO_HRUMMO - chiamata 169529.ini
    EsisteRotturaPeriodiStorici:=False;
    CampoDiRotturaAggiuntivo:='';
    // BENEVENTO_HRUMMO - chiamata 169529.fine
    ...
    begin
      ...
      if R180In(Identificatore(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Ordinamento[i].Nome),['T430DATADECORRENZA','T430DATAFINE']) then
      begin
        if R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Ordinamento[i].Rottura then
          CampoDiRotturaAggiuntivo:=''
        else
          CampoDiRotturaAggiuntivo:=Identificatore(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Ordinamento[i].Nome);
      end;
      // BENEVENTO_HRUMMO - chiamata 169529.ini
      C:=R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.GetDato(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Ordinamento[i].Nome,False);
      if (C >= 0) and
        (R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Dati[C].X = 0) and
        ((Copy(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Dati[C].D,1,4) = 'T430') or
        (Copy(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Dati[C].D,1,4) = 'P430') or
        (UpperCase(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Dati[C].D) = 'CDCPERC_CODICE') or
        (UpperCase(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Dati[C].D) = 'CDCPERC_PERCENTUALE') or     // daniloc. 21.04.2010
        (UpperCase(R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.Dati[C].D) = 'CDCPERC_DESCRIZIONE')) then
      begin
        EsisteRotturaPeriodiStorici:=True;
      end;
      // BENEVENTO_HRUMMO - chiamata 169529.fine
    end;
    if R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.PeriodiStorici and
       R003FGeneratoreStampeDtM.R003FGeneratoreStampeMW.RotturaPeriodiNonContigui and
       EsisteRotturaPeriodiStorici and (CampoDiRotturaAggiuntivo <> '') then
    begin
      BIntestazione.Expression:=BIntestazione.Expression + ' + cds920.' + CampoDiRotturaAggiuntivo;
    end;
    ...
    //Gestione Interna del dato VP_MISURA (non specificato dall'utente)
    //Creo il componente TQRLabel che verrà usato nel DettaglioBeforePrint
  ...
  begin
    begin
      {$IF CompilerVersion < 31} //QR5 (Delphi XE4)
