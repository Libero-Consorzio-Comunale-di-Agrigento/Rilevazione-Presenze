...
begin
  ...
  if Parametri.DefTipoPersonale = 'E' then
    frmSelAnagrafe.SoloPersonaleInternoVal:=False;
...
begin
  ...
  begin
    ...
    begin
      C700DataLavoro:=Parametri.DataLavoro; //Necessario forzare aggiornamento di C700DataLavoro per evitare disallineamenti
      StatusBar.Panels[2].Text:='Data di lavoro:' + FormatDateTime('dd/mm/yyyy',DataLavoro);
...
begin
  ...
  frmSelAnagrafe.SalvaC00SelAnagrafe;
  try
    C700FSelezioneAnagrafe.DataLavoro:=DataLavoro;
...
begin
  ...
  //Resetto le date della C700, che possono essere state impostate diversamente da altre maschere
  C700DataLavoro:=Parametri.DataLavoro;
...
begin
  if R180MessageBox('Uscire dalla sessione di lavoro?',DOMANDA) = mrNo then
  begin
    Action:=caNone;
  end;
...
begin
  with A002FAnagrafeDtM1 do
    ...
    begin
      ...
      if (*Filtrocessati1.Checked and*) ((DataInizioRap > DataLavoro) or (DataFineRap < DataLavoro)) then //I cessati sempre visualizzati in rosso [AOSTA_REGIONE - Chiamata: 165039]
