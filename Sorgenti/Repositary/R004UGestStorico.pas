...
type
  ...
  TR004FGestStorico = class(TForm)
    ...
    procedure DButtonUpdateData(Sender: TObject);
  private
    ...
    function CheckRelazioniEsistenti(pUsaOldValues: Boolean = False):boolean;
  protected
    ...
    procedure CercaStoricoCorrente(ElencoCampi:String = '';ValoriCampi:TStringList = nil);
...
function TR004FGestStorico.CheckRelazioniEsistenti(pUsaOldValues: Boolean = False): boolean;
begin
  Result:=True;
  ...
  CheckRelazioni.SetVariable('CHIAVE',R180GetCampiConcatenati(DButton.DataSet,InterfacciaR004.LChiavePrimaria,pUsaOldValues));
  ...
  if VarToStr(CheckRelazioni.GetVariable('RESULT')) = 'E1' then
    ...
    raise Exception.Create('Cancellazione e modifica impedite in quanto il dato è in uso:' + #13#10 + CheckRelazioni.GetVariable('RESULT'))
  else
    Result:=False;
end;
...
procedure TR004FGestStorico.DButtonUpdateData(Sender: TObject);
var
  i:integer;
  Modificato:Boolean;
begin
  if DButton.DataSet.State = dsEdit then
  begin
    Modificato:=False;
    for i:=0 to InterfacciaR004.LChiavePrimaria.Count - 1 do
    begin
      if DButton.DataSet.FieldByName(InterfacciaR004.LChiavePrimaria[i]).OldValue <> DButton.DataSet.FieldByName(InterfacciaR004.LChiavePrimaria[i]).Value then
      begin
        Modificato:=True;
        Break;
      end;
    end;
    if Modificato and CheckRelazioniEsistenti(True) then
      Abort;
  end;
end;
      ...
      begin
        ...
        begin
          ...
          begin
            GetDateDecorrenza;
            if not CercaXDecorrenza then
              CercaStoricoCorrente(Copy(ElencoCampi,2,1000),Valori);
          end;
        end
        else
        begin
  ...
  C001FScegliCampi:=TC001FScegliCampi.Create(nil);
    try
      ...
      begin
        ...
        for i:=0 to InterfacciaR004.LChiavePrimaria.Count - 1 do
        begin
          CampiChiave:=CampiChiave + InterfacciaR004.LChiavePrimaria[i];
          if i < InterfacciaR004.LChiavePrimaria.Count - 1 then
            CampiChiave:=CampiChiave + ';';
        end;
        SetLength(ValoriChiave,0);
        for i:=0 to InterfacciaR004.LChiavePrimaria.Count - 1 do
        begin
          SetLength(ValoriChiave,Length(ValoriChiave) + 1);
          ValoriChiave[High(ValoriChiave)]:=C001FFiltroTabelleDTM.Query1.FieldByName(InterfacciaR004.LChiavePrimaria[i]).AsVariant;
        end;
...
begin
  ...
  try
    if DButton.DataSet.FindField(DButton.DataSet.FieldDefs[i].Name) <> nil then
...
procedure TR004FGestStorico.CercaStoricoCorrente(ElencoCampi:String = '';ValoriCampi:TStringList = nil);
var S,RI,PK:String;
    V:Variant;
    idx:Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      if InterfacciaR004.LChiavePrimaria.IndexOf(Campo) = -1 then
      begin
    ...
    begin
      ...
      begin
        ...
        if ValAgg = R180GetCampiConcatenati(DButton.DataSet,SLCampiAgg) then
          RI:=RowID;
        Next;
      end;
    end;
