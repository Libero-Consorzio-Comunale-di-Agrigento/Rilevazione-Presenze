...
type
  ...
  TSerbatoi = record
    ...
    DatoDalAl:String;              //Campo scelto per effettuare il filtro dal..al
  ...
  protected
    DatiRiep:array of TRiep;
    procedure CaricaDatiRiep; virtual; abstract;
  ...
  public
    ...
    PeriodiStorici,CDCPercentualizzati, EsistonoDatiDaProporzionare, RotturaPeriodiNonContigui: Boolean;
    CodForm:String;
...
begin
  ...
  RotturaPeriodiNonContigui:=False;
  ...
  lstFunzioniSQL.CommaText:=':DAL,:AL,:UTENTE,INSTR,SUBSTR,TO_CHAR,TO_DATE,TO_NUMBER,ROUND,TRUNC,LPAD,RPAD,NVL,' +
                  'DECODE,SYSDATE,CHR,ASCII,TRIM,LTRIM,RTRIM,REPLACE,LENGTH,OREMINUTI,MINUTIORE,' +
                  'ADD_MONTHS,LAST_DAY,GREATEST,LEAST,MONTHS_BETWEEN,SIGN,MOD';
  ...
  begin
    ...
    begin
      if (not FieldByName('CODICE_STAMPA').IsNull) and
         (FieldByName('CODICE_STAMPA').AsString <>  FSelT910_Funzioni.FieldByName('CODICE').AsString) and
         (TipoModulo <> 'COM')
      then
      begin
        Next;
...
// cfr. A002MW
function TR003FGeneratoreStampeMW.FormattaEMail(pMatricola, pI060Mail:string):string;
var
...
// cfr. A002MW
function TR003FGeneratoreStampeMW.GetDatoI060(const PMatricola, PNomeDato: string; const PSeparatore: string = '; '): string;
var
  LDato: string;
begin
  Result:='';
...
begin
  ...
  begin
    ...
    else if UpperCase(DatiStampa[i].D) = 'I060EMAIL' then
    begin
      Ins920.SetVariable(DatiStampa[i].D,FormattaEMail(SelAnagrafe.FieldByName('MATRICOLA').AsString,'EMAIL'));
    end
    else if UpperCase(DatiStampa[i].D) = 'I060EMAIL_PEC' then
    begin
      Ins920.SetVariable(DatiStampa[i].D,FormattaEMail(SelAnagrafe.FieldByName('MATRICOLA').AsString,'EMAIL_PEC'));
    end
    else if UpperCase(DatiStampa[i].D) = 'I060CELLULARE' then
    begin
      Ins920.SetVariable(DatiStampa[i].D,GetDatoI060(SelAnagrafe.FieldByName('MATRICOLA').AsString,'CELLULARE'));
    end
    //Dati anagrafici
...
begin
  ...
  begin
    ...
    begin
      ...
      Ins911.SetVariable('FORMATO',Copy(Fmt,1,2000));
  ...
  begin
    ...
    begin
      if FSelT910_Funzioni.Fields[i].FieldName = 'APPLICAZIONE' then
        ...
        S:=AggiungiApice(EliminaRitornoACapo(FSelT910_Funzioni.Fields[i].AsString));
  ...
  begin
    ...
    begin
      ...
      S2:=Format('''%s'',''%s'',''%s'',''%s'',''%s'',%d,%d,%d,%d,''%s'',''%s'',''%s'',''%s'',''%s''',
               [Nome,
                AggiungiApice(lblDati.Nome),
                AggiungiApice(lblDati.Capt),
                lblDati.banda,
                Tot,
                lblDati.Y,
                lblDati.X,
