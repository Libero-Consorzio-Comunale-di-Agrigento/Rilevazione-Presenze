...
var Path:String;
begin
  Path:=R180GetFilePath(Application.ExeName,True) + 'Archivi\Temp';
  if not DirectoryExists(Path) then
    Path:=R180GetFilePath(Application.ExeName,True);
...
//    ProgressBar.Max:=high(TabelleCancella) * C700SelAnagrafe.RecordCount;
  ...
  begin
    ...
    tempoCancellazione:=B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.InizializzaTempo(DataI, DataF);
    Anom:=False;
    btnAnomalie.Enabled:=False;
    for i:=0 to chkLstTabelle.Items.Count - 1 do
    begin
...
begin
  ...
  begin
    ...
    // Conferma
    if R180MessageBox(MessaggioConfermaStoricizzazione(rgpPeriodi.ItemIndex,DataI, DataF),'DOMANDA') = mrNo then
      Exit;
    // Preparazione
  ...
  ParametriStoricizzazione.DataDa:=DataI;
  ParametriStoricizzazione.DataA:=DataF;
...
begin
  ...
  Msg:=B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.MessaggioConfermaRiallineamentoGiust(DataI,C700SelAnagrafe.RecordCount);
  ...
  try
    ...
    begin
      try
        ...
        if not B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW.ElaborazioneRiallinementoGiustDipendente(CausList,DataI) then
        begin
...
begin
  ...
  begin
    ...
    if TabelleCancella[indice].Peri='E' then
      ...
      comodo:=FormatDateTime('dd/mm/yyyy',DataI)+' - '+FormatDateTime('dd/mm/yyyy',DataF);
    mess:=mess+comodo;
    ...
    begin
      ...
      B007FManipolazioneDatiMW.CancellaDate(indice, DataI, DataF, tempoCancellazione);
    end
    else
    begin
      C700SelAnagrafe.First;
      ...
      begin
        ...
        if (RisMsg=mrAll) or (RisMsg=mrYes) then
        begin
          B007FManipolazioneDatiMW.CancellaDateDipendente(indice, DataI, DataF, tempoCancellazione);
        end;
        C700SelAnagrafe.Next;
      end;
    end;
  ...
  begin
    ...
    begin
      ...
      begin
        messaggio:='Attenzione!!! Verranno cancellati i dati della tabella '+
          TabelleCancella[indice].Desc+#13#10+'del periodo: '+
          FormatDateTime('dd/mm/yyyy',DataI) + ' - ' + FormatDateTime('dd/mm/yyyy',DataF) + #13#10 +
          'Vuoi continuare?';
        RisMsg:=MessageDlg(messaggio,mtConfirmation,[mbYes, mbNo, mbAll, mbCancel],0);
        if (RisMsg = mrNo) or (RisMsg = mrCancel) then
        begin
      ...
      B007FManipolazioneDatiMW.CancellaDaA(indice, DataI, DataF);
    end
    else
    begin
      C700SelAnagrafe.First;
      ...
      begin
        ...
        if (RisMsg=mrAll) or (RisMsg=mrYes) then
        begin
          B007FManipolazioneDatiMW.CancellaDaADipendente(indice, DataI, DataF);
        end;
        C700SelAnagrafe.Next;
      end;
    end;
...
begin
  ...
  begin
    ...
    begin
      messaggio:='Attenzione!!! Verranno cancellati i dati della tabella '+
         'GIUSTIFICATIVI dalla data: '+FormatDateTime('dd/mm/yyyy',DataI)+
         ' alla data '+FormatDateTime('dd/mm/yyyy',DataF)+#13#10+
         'Vuoi continuare?';
      RisMsg:=MessageDlg(messaggio,mtConfirmation,[mbYes, mbNo, mbAll, mbCancel],0);
      if (RisMsg = mrNo) or (RisMsg = mrCancel) then
      begin
    ...
    begin
      if PageControl1.ActivePage = tabCancellazioneDati then
      begin
        B007FManipolazioneDatiMW.CancellaDatiGiustif(DataI,DataF);
      end
      else
      begin
        for i:=0 to chklstCausGiust.Items.Count - 1 do
        begin
          if chklstCausGiust.Checked[i] then
          begin
            B007FManipolazioneDatiMW.CancellaCodiceGiustifProg(Trim(Copy(chklstCausGiust.Items.Strings[i],1,5)), DataI, DataF, 0);
          end;
        end;
      end;
    end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          ' del periodo: '+FormatDateTime('dd/mm/yyyy',DataI)+' - '+FormatDateTime('dd/mm/yyyy',DataF)+#13#10+
          'relativi al dipendente: '+C700SelAnagrafe.FieldByName('Cognome').AsString+' '+
          C700SelAnagrafe.FieldByName('Nome').AsString+' Badge; '+C700SelAnagrafe.FieldByName('T430Badge').AsString+
          #13#10+'Vuoi continuare?';
          RisMsg:=MessageDlg(messaggio,mtConfirmation,[mbYes, mbNo, mbAll, mbCancel],0);
        ...
        begin
          if PageControl1.ActivePage = tabCancellazioneDati then
          begin
            B007FManipolazioneDatiMW.CancellaDatiGiustifDipendente(DataI,DataF);
          end
          else
          begin
            ProgressBar.StepBy(1);
            ...
            begin
              if chklstCausGiust.Checked[i] then
              begin
                B007FManipolazioneDatiMW.CancellaCodiceGiustifProg(Trim(Copy(chklstCausGiust.Items.Strings[i],1,5)), DataI, DataF, C700Progressivo);
              end;
            end;
          end;
        end;
...
begin
  ...
  begin
    ...
    messaggio:=messaggio + ' dalla causale ' + edtOldCausale.Text + ' alla causale ' + dCmbCausali.Text + #13#10 +
      ' dalla data ' + FormatDateTime('dd/mm/yyyy',DataI) + ' alla data ' + FormatDateTime('dd/mm/yyyy',DataF) + #13#10 +
    ...
    if (RisMsg=mrNo) or (RisMsg=mrCancel) then
      Exit;
    with B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW do
      Anom:=not ElaborazioneRicodificaGiust(DataI,
                                            DataF,
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          with B007FManipolazioneDatiDtM1.B007FManipolazioneDatiMW do
            if (not ElaborazioneRicodificaGiustDipendente(DataI,
                                                          DataF,
                                                          Trim(edtOldCausale.Text),
                                                          dCmbCausali.Text,
...
procedure TB007FManipolazioneDati.frmInputPeriodobtnDataInizioClick(Sender: TObject);
begin
...
procedure TB007FManipolazioneDati.frmInputPeriodoedtFineExit(Sender: TObject);
begin
  if DataF <= 0 then
  begin
...
procedure TB007FManipolazioneDati.frmInputPeriodoedtInizioExit(Sender: TObject);
begin
...
begin
  ...
  //INIZIO
  frmInputPeriodo.edtInizio.Enabled:=(PageControl1.ActivePage <> tabUnioneProgressivi) and
                                      (PageControl1.ActivePage <> tabMigrazioneDomicilio) and
                                      (PageControl1.ActivePage <> tabCestino);
  frmInputPeriodo.lblInizio.Enabled:=frmInputPeriodo.edtInizio.Enabled;
  frmInputPeriodo.btnDataInizio.Enabled:=frmInputPeriodo.edtInizio.Enabled;
  //FINE
  frmInputPeriodo.edtFine.Enabled:=(frmInputPeriodo.edtInizio.Enabled) and
                                    (PageControl1.ActivePage <> tabRiallineaGiust);
  frmInputPeriodo.lblFine.Enabled:=frmInputPeriodo.edtFine.Enabled;
  frmInputPeriodo.btnDataFine.Enabled:=frmInputPeriodo.edtFine.Enabled;
  //BOTTONI NAVIGAZIONE
  frmInputPeriodo.btnIndietro.Enabled:=frmInputPeriodo.btnDataInizio.Enabled and frmInputPeriodo.btnDataFine.Enabled;
  frmInputPeriodo.btnAvanti.Enabled:=frmInputPeriodo.btnDataInizio.Enabled and frmInputPeriodo.btnDataFine.Enabled;
  ...
  begin
    R180AbilitaOggetti(frmInputPeriodo, False);
  end
  else if PageControl1.ActivePage = tabMigrazioneDomicilio then
  begin
...
begin
  ...
  begin
    R180AbilitaOggetti(frmInputPeriodo, True);
    gpbTabelle.Enabled:=True;
...
begin
  //INIZIO
  frmInputPeriodo.edtInizio.Enabled:=(rgpPeriodi.ItemIndex = 0) or (rgpPeriodi.ItemIndex = 1);
  frmInputPeriodo.lblInizio.Enabled:=frmInputPeriodo.edtInizio.Enabled;
  frmInputPeriodo.btnDataInizio.Enabled:=frmInputPeriodo.edtInizio.Enabled;
  //FINE
  frmInputPeriodo.edtFine.Enabled:=rgpPeriodi.ItemIndex = 0;
  frmInputPeriodo.lblFine.Enabled:=frmInputPeriodo.edtFine.Enabled;
  frmInputPeriodo.btnDataFine.Enabled:=frmInputPeriodo.edtFine.Enabled;
  //BOTTONI NAVIGAZIONE
  frmInputPeriodo.btnAvanti.Enabled:=frmInputPeriodo.edtInizio.Enabled and frmInputPeriodo.edtFine.Enabled;
  frmInputPeriodo.btnIndietro.Enabled:=frmInputPeriodo.btnAvanti.Enabled;
...
begin
  R180AbilitaOggetti(frmInputPeriodo, RadioGroup5.ItemIndex = 1);
end;
...
begin
  ...
  begin
    ...
    if MyCestino.Get_selI025.RecordCount > 0 then
      dGrdCestino.SelectedRows.CurrentRowSelected:=True;
      //rdCestino.DataSource.DataSet.GotoBookmark(dGrdCestino.SelectedRows.Items[i]);
  end;
end;
...
begin
  ...
  begin
    ...
    try
      ...
      A023FAllTimbMW.LeggiTimbratureUguali(DatiAnag,DataI,DataF,False);
    except
      on E: Exception do
      begin
        if LMessaggi.Count = 0 then
        begin
          LMessaggi.Add(Format('Elenco errori riscontrati durante la lettura delle timbrature da allineare dal %s al %s',[DateToStr(DataI),DateToStr(DataF)]));
          LMessaggi.Add('');
        end;
...
begin
  ...
  begin
    ...
    begin
      try
        A023FAllTimbMW.Allinea(DS.FieldByName('PROGRESSIVO').AsInteger,DataI,DataF);
        ...
        begin
          if LMessaggi.Count = 0 then
          begin
            LMessaggi.Add(Format('Elenco errori riscontrati durante l''allineamento delle timbrature dal %s al %s',[DateToStr(DataI),DateToStr(DataF)]));
            LMessaggi.Add('');
          end;
...
{ DataF }
function TB007FManipolazioneDati._GetDataF: TDateTime;
begin
  Result:=frmInputPeriodo.DataFine;
end;
procedure TB007FManipolazioneDati._PutDataF(const Value: TDateTime);
begin
  frmInputPeriodo.DataFine:=Value;
end;
{ ----DataF }
{ DataI }
function TB007FManipolazioneDati._GetDataI: TDateTime;
begin
  Result:=frmInputPeriodo.DataInizio;
end;
procedure TB007FManipolazioneDati._PutDataI(const Value: TDateTime);
begin
