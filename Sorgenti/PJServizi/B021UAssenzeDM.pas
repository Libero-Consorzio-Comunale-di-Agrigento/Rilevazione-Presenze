...
begin
  ...
  Result.AddPair('data',TJsonString.Create(TB021FUtils.ConvertiDateStr(Ass.FData)));
...
begin
  ...
  if not TB021FUtils.ConvertiStrDate(GetParam('data'),DataAss) then
  begin
  ...
  if DataFamStr = '' then
    DataNasFam:=0
  else if not TB021FUtils.ConvertiStrDateTime(DataFamStr,DataNasFam) then
  begin
