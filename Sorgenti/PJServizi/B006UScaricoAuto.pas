...
type
  ...
  private
    ...
    function  ChekParametriRegistro:Boolean;
    function  ValPriority:Integer;
...
function TB006FScaricoAuto.ValPriority:Integer;
begin
  Result:=3;
...
begin
  try
    ...
    C004FParamForm.PutParametro('PRIORITY',IntToStr(ValPriority));
    ...
    R180PutRegistro(HKEY_LOCAL_MACHINE,'B006','Priority',IntToStr(ValPriority))
  except
  end;
end;

function TB006FScaricoAuto.ChekParametriRegistro:Boolean;
{Per verificare se i contenuto del registro è <> da quello visibile sulla maschera, quando per es. la scrittura sul registro fallisce: però sembra non avere effetto}
begin
  Result:=True;
  if R180GetRegistro(HKEY_LOCAL_MACHINE,'B006','Pianificazione','') <> IfThen(chkAcquisisciDaPianificazione.Checked,'S','N') then
    Result:=False
  else if R180GetRegistro(HKEY_LOCAL_MACHINE,'B006','Intervallo','') <> SpinEdit1.Text then
    Result:=False
  else if R180GetRegistro(HKEY_LOCAL_MACHINE,'B006','RigheLog','') <> SEdtSpinRowLog.Text then
    Result:=False
  else if R180GetRegistro(HKEY_LOCAL_MACHINE,'B006','DatabaseList','') <> edtDatabaseList.Text then
    Result:=False
  else if R180GetRegistro(HKEY_LOCAL_MACHINE,'B006','Priority','') <> IntToStr(ValPriority) then
    Result:=False;
end;
