...
uses
  ...
  C700USelezioneAnagrafe, QueryPK, RegistrazioneLog, MConnect, ActiveX, ShellAPI, C017UEMailDtM,
  IOUtils;
...
type
  TB019FGeneratoreStampeDtM = class(TDataModule)
    ...
    selT926EMAIL_INDIRIZZI_A: TStringField;
    selT926EMAIL_INDIRIZZI_CC: TStringField;
    selT926EMAIL_INDIRIZZI_CCN: TStringField;
    selT926EMAIL_OGGETTO: TStringField;
    selT926EMAIL_TESTO: TStringField;
    selT926EMAIL_INDIRIZZI_NOTIFICA_ELAB: TStringField;
...
var Applicazione,NomeFile,NomeFileCorr,
    ...
    lstCmd{,lstFile},lstMsg:TStringList;
    i:Integer;
    LogUnixFormat:Boolean;
    wR180SyncProcessExecResults:T180SyncProcessExecResults;
    sOggetto,sAllegato,sTesto,sDestinatariA,sDestinatariCC,sDestinatariCCN: String;
  ...
  begin
    try
      //TRegistraMsg(SessioneIWB019.RegistraMsg).InserisciMessaggio('I','C90_EMailSMTPHost=' + SessioneIWB019.Parametri.CampiRiferimento.C90_EMailSMTPHost);
      C017DtM:=TC017FEMailDtM.Create(SessioneIWB019);
      try
        C017DtM.SollevaEccezioni:=True;
        C017DtM.Sessione:=SessioneOracle;
        C017DtM.Destinatari:=sDestinatariA;
        C017DtM.CercaDestinatari:=False;
        C017DtM.DestinatariCC:=sDestinatariCC;
        C017DtM.DestinatariCCN:=sDestinatariCCN;
        C017DtM.Oggetto:=sOggetto;
        C017DtM.Allegato:=sAllegato;
      ...
      on E:Exception do
      begin
        TRegistraMsg(SessioneIWB019.RegistraMsg).InserisciMessaggio('A','  Errore durante l''invio dell''email: ' + '[C017] ' + E.Message + ' (' + E.ClassName + ')',SessioneIWB019.Parametri.Azienda);
        raise;
      end;
    end;
  end;
begin
  ...
  try
    ...
    //Invio notifica inizio elaborazione stampa
    if Trim(selT926.FieldByName('EMAIL_INDIRIZZI_NOTIFICA_ELAB').AsString) <> '' then
    begin
      sOggetto:='IrisWIN: ' + selT926.FieldByName('CODICE_STAMPA').AsString + ' Inizio elaborazione';
      sAllegato:='';
      sTesto:='';
      for i:=0 to selT926.FieldCount - 1 do
        sTesto:=sTesto + IfThen(sTesto <> '',CRLF) + selT926.Fields[i].FieldName + ': ' + selT926.Fields[i].AsString;
      sDestinatariA:=selT926.FieldByName('EMAIL_INDIRIZZI_NOTIFICA_ELAB').AsString;
      sDestinatariCC:='';
      sDestinatariCCN:='';
      InviaEMail;
    end;
    ...
    try
      ...
      TRegistraMsg(SessioneIWB019.RegistraMsg).InserisciMessaggio('I',Format('Azienda: %s - Stampa: %s - dal %s al %s - Selezione: %s',[SessioneIWB019.Parametri.Azienda,selT926.FieldByName('CODICE_STAMPA').AsString,selDalAlFile.FieldByName('DAL').AsString,selDalAlFile.FieldByName('AL').AsString,selT926.FieldByName('SELEZIONE').AsString]),SessioneIWB019.Parametri.Azienda);
    except
      on E:Exception do
      begin
    ...
    begin
      ...
      begin
        ...
        //Alberto 05/03/2018: eliminazione file preesistente
        DeleteFile(NomeFileCorr);
      end
      else
      begin
        NomeFileCorr:='';
    ...
    //Invio file generato dalla stampa
    if (NomeFileCorr <> '')
    and TFile.Exists(NomeFileCorr)
    and (   (Trim(selT926.FieldByName('EMAIL_INDIRIZZI_A').AsString) <> '')
         or (Trim(selT926.FieldByName('EMAIL_INDIRIZZI_CC').AsString) <> '')
         or (Trim(selT926.FieldByName('EMAIL_INDIRIZZI_CCN').AsString) <> '')) then
    begin
      sOggetto:=IfThen(Trim(selT926.FieldByName('EMAIL_OGGETTO').AsString) <> '',selT926.FieldByName('EMAIL_OGGETTO').AsString,'IrisWIN: ' + selT926.FieldByName('CODICE_STAMPA').AsString + ' ' + ExtractFileName(NomeFileCorr));
      sAllegato:=NomeFileCorr;
      sTesto:=selT926.FieldByName('EMAIL_TESTO').AsString;
      sDestinatariA:=selT926.FieldByName('EMAIL_INDIRIZZI_A').AsString;
      sDestinatariCC:=selT926.FieldByName('EMAIL_INDIRIZZI_CC').AsString;
      sDestinatariCCN:=selT926.FieldByName('EMAIL_INDIRIZZI_CCN').AsString;
      InviaEMail;
    end;

    //Invio notifica fine elaborazione stampa
    if Trim(selT926.FieldByName('EMAIL_INDIRIZZI_NOTIFICA_ELAB').AsString) <> '' then
    begin
      sOggetto:='IrisWIN: ' + selT926.FieldByName('CODICE_STAMPA').AsString + ' Fine elaborazione';
      sAllegato:='';
      sTesto:='';
      lstMsg:=TStringList.Create;
      try
        TRegistraMsg(SessioneIWB019.RegistraMsg).LeggiMessaggi(TRegistraMsg(SessioneIWB019.RegistraMsg).ID);
        TRegistraMsg(SessioneIWB019.RegistraMsg).GetListaMessaggi(lstMsg);//LeggiMessaggi('B019,P077COM');
        sTesto:=lstMsg.Text;
        if (NomeFileCorr <> '')  and (not TFile.Exists(NomeFileCorr)) then
          sTesto:=sTesto + CRLF + Format('Attenzione: file %s non esistente',[NomeFileCorr]);
      finally
        lstMsg.Free;
      end;
      sDestinatariA:=selT926.FieldByName('EMAIL_INDIRIZZI_NOTIFICA_ELAB').AsString;
