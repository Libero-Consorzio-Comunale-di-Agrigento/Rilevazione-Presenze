...
begin
  ...
  Result.AddPair('dataInizio',TJSONString.Create(TB021FUtils.ConvertiDateStr(Ass.FDataInizio)));
  Result.AddPair('dataFine',TJSONString.Create(TB021FUtils.ConvertiDateStr(Ass.FDataFine)));
  ...
  begin
    ...
    hObj.AddPair('data',TJSONString.Create(TB021FUtils.ConvertiDateStr(Giust.FDataInizio)));
  ...
  try
    ...
    begin
      ...
      if jPairGiust.JsonString.Value = 'richiesta' then
        ...
        TB021FUtils.ConvertiStrDate(jPairGiust.JsonValue.Value,TGiustificativo(Result).FDataInizio)
      else if jPairGiust.JsonString.Value = 'dataFine' then
        TB021FUtils.ConvertiStrDate(jPairGiust.JsonValue.Value,TGiustificativo(Result).FDataFine)
        ...
        TB021FUtils.ConvertiStrDateTime(jPairGiust.JsonValue.Value,TGiustificativo(Result).FDataFamiliare)
...
begin
  ...
  begin
    ...
    if not TB021FUtils.ConvertiStrDate(TempStr,Inizio) then
    begin
    ...
    if not TB021FUtils.ConvertiStrDate(TempStr,Fine) then
    begin
    ...
    if Fine < Inizio then
    begin
      RErrMsg:='Parametro data fine (' + TB021FUtils.ConvertiDateStr(Fine) + ') precedente a parametro data inizio (' + TB021FUtils.ConvertiDateStr(Inizio) + ')';
      Exit;
    end;
    ...
    begin
      ...
      OraInizio:=GetParam('orainizio');
      OraFine:=GetParam('orafine');
    end;
  end
  ...
  try
    try
      ...
      begin
        ...
        Var_NumGG:=0;
        Var_DaData:=TB021FUtils.ConvertiDateStr(G.FDataInizio); //sDataDa;
        Var_AData:=TB021FUtils.ConvertiDateStr(G.FDataFine); //sDataA;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          Var_DaOre:=StringReplace(OraInizio,':','.',[]); //sOraDa;
          Var_AOre:=StringReplace(OraFine,':','.',[]); //sOraA;
          Var_NumGG:=0;
          ...
          if (OraInizio <> '') and (OraFine <> '') then //Causale = '99001' then
          begin
            //Var_TipoGiust:=2;
            //Modo:='N';
            //Var_DaOre:='00.00';
