...
begin
  Result:=Format('day=%s|shiftName=%s',[TB021FUtils.ConvertiDateStr(FDay),FShiftName]);
end;
...
begin
  ...
  Result.AddPair('start',TJSONString.Create(TB021FUtils.ConvertiDateStr(T.FStart)));
  Result.AddPair('end',TJSONString.Create(TB021FUtils.ConvertiDateStr(T.FEnd)));
  ...
  begin
    ...
    begin
      ...
      esObj.AddPair('day',TJSONString.Create(TB021FUtils.ConvertiDateStr(ES.FDay)));
      if ES.FOnCall then
      ...
      esObj.AddPair('shiftStart',TJSONString.Create(TB021FUtils.ConvertiDateTimeStr(ES.FShiftStart)));
      esObj.AddPair('shiftEnd',TJSONString.Create(TB021FUtils.ConvertiDateTimeStr(ES.FShiftEnd)));
      esObj.AddPair('expectedShiftStart',TJSONString.Create(TB021FUtils.ConvertiDateTimeStr(ES.FExpectedShiftStart)));
      esObj.AddPair('expectedShiftEnd',TJSONString.Create(TB021FUtils.ConvertiDateTimeStr(ES.FExpectedShiftEnd)));
      esObj.AddPair('shiftBreakStart',TJSONString.Create(TB021FUtils.ConvertiDateTimeStr(ES.FShiftBreakStart)));
      esObj.AddPair('shiftBreakEnd',TJSONString.Create(TB021FUtils.ConvertiDateTimeStr(ES.FShiftBreakEnd)));
  ...
  try
    with TTurni(Result) do
      ...
      begin
        ...
        if TKey = 'start' then
          TB021FUtils.ConvertiStrDate(TVal,FStart)
        else if TKey = 'end' then
          TB021FUtils.ConvertiStrDate(TVal,FEnd)
              ...
              begin
                ...
                begin
                  ...
                  begin
                    ...
                    if ESKey = 'day' then
                      TB021FUtils.ConvertiStrDate(ESVal,FShifts[i].FEmployeeShifts[j].FDay)
                      ...
                      TB021FUtils.ConvertiStrDateTime(ESVal,FShifts[i].FEmployeeShifts[j].FShiftStart)
                    else if ESKey = 'shiftEnd' then
                      TB021FUtils.ConvertiStrDateTime(ESVal,FShifts[i].FEmployeeShifts[j].FShiftEnd)
                    else if ESKey = 'expectedShiftStart' then
                      TB021FUtils.ConvertiStrDateTime(ESVal,FShifts[i].FEmployeeShifts[j].FExpectedShiftStart)
                    else if ESKey = 'expectedShiftEnd' then
                      TB021FUtils.ConvertiStrDateTime(ESVal,FShifts[i].FEmployeeShifts[j].FExpectedShiftEnd)
                    else if ESKey = 'shiftBreakStart' then
                      TB021FUtils.ConvertiStrDateTime(ESVal,FShifts[i].FEmployeeShifts[j].FShiftBreakStart)
                    else if ESKey = 'shiftBreakEnd' then
                      TB021FUtils.ConvertiStrDateTime(ESVal,FShifts[i].FEmployeeShifts[j].FShiftBreakEnd)
...
begin
  ...
  begin
    ...
    if not TB021FUtils.ConvertiStrDate(GetParam('inizio'),Inizio) then
    begin
      RErrMsg:='Parametro [inizio] non valido';
      Exit;
    end;

    if not TB021FUtils.ConvertiStrDate(GetParam('fine'),Fine) then
    begin
...
var
  ...
  begin
    ...
    try
      ...
      begin
        RisultatoStd.AddWarning('Nessuna pianificazione da eliminare nel periodo ' + TB021FUtils.ConvertiDateStr(Inizio) +  ' - ' + TB021FUtils.ConvertiDateStr(Fine));
