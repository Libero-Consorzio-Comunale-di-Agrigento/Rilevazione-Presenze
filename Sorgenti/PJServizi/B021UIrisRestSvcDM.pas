...
uses
  ...
  Web.HTTPApp, Datasnap.DSHTTPWebBroker, IdHTTPWebBrokerBridge;
...
{$METHODINFO ON}
  ...
  public
    LogAttivo: Boolean;
    URLRichiesta: String;
    PageRichiesta: String;
    PerPageRichiesta: String;
    Token: String;
    Timestamp: String;
    ...
    function cancelGiustificativi(PParDatabase,PParAzienda,PParUtenteI070,PParMatricola,PParCausale,PParInizio,PParFine,PParOraInizio,PParOraFine:String): TJSONObject;
    ...
    // #################            CUNEO_ASLCN1            ################# //
    // ###################################################################### //
    function I060Utenti(PParDatabase,PParAzienda,PParUtenteI070,PParCodiceFiscale,PParMatricola,PParNomeUtente,PParPassword:String): TJSONObject;
    function updateI060Utenti(PParDatabase,PParAzienda,PParUtenteI070:String; PParUtenti: TJSONObject): TJSONObject;
...
begin
  if LogAttivo then
    Log(TB021FUtils.GetLogFmt(Self.ClassName,PNomeProc,PLogInfo));
end;
...
begin
  ...
  begin
    PParDatabase:=TB021FUtils.GetDatabaseEffettivo(PParDatabase);
  ...
  begin
    PParAzienda:=TB021FUtils.GetAziendaEffettiva(PParAzienda);
  ...
  begin
    PParUtenteI070:=TB021FUtils.GetUtenteI070Effettivo(PParUtenteI070);
  ...
  Result.SessioneOracle.ThreadSafe:=True;
...
begin
  ...
  try
    DM:=TB021FAnagraficoDM.Create(LSIW,
                                  TB021FUtils.GetDatiConnessioneEffettivi(PParDatabase,PParAzienda,PParUtenteI070),
                                  True,
                                  OPER_READ,
                                  LogAttivo,
                                  Token,
...
var
  ...
  i: Integer;
  LParam, LParPage, LParPerPage: String;
...
begin
  ...
  // gestione eventuali parametri di paginazione nella querystring e url per link
  LParPage:='';
  LParPerPage:='';
  LUrl:='';
  {$IFDEF WEBSVC}  //B021
  LModule:=GetDataSnapWebModule;
  LUrl:=LModule.Request.PathInfo;
  ...
  begin
    ...
    if LowerCase(LParKeyValue[0]) = 'page' then
      LParPage:=LParKeyValue[1]
    else if LowerCase(LParKeyValue[0]) = 'per_page' then
      LParPerPage:=LParKeyValue[1]
  end;
  {$ELSE}            //IrisWEB
  LParPage:=PageRichiesta;
  LParPerPage:=PerPageRichiesta;
  LUrl:=URLRichiesta;
  {$ENDIF}
  ...
  try
    ...
    LInfoLog.Parametri.Add('url',LUrl);
    LInfoLog.Parametri.Add('page',LParPage);
    LInfoLog.Parametri.Add('per_page',LParPerPage);
    LInfoLog.LogAccesso;
  finally
    FreeAndNil(LInfoLog);
  end;
  ...
  try
    ...
    try
      ...
      DM.SetParam('url',LUrl);
      DM.SetParam('page',LParPage);
      DM.SetParam('per_page',LParPerPage);
      Result:=DM.EseguiOperazione;
      DM.Disconnetti;
    finally
      FreeAndNil(DM);
...
function TB021FIrisRestSvcDM.cancelGiustificativi(PParDatabase,PParAzienda,PParUtenteI070,PParMatricola,PParCausale,PParInizio,PParFine,PParOraInizio,PParOraFine:String): TJSONObject;
...
begin
  ...
  try
    ...
    LInfoLog.Parametri.Add('inizio',PParInizio);
    LInfoLog.Parametri.Add('fine',PParFine);
    LInfoLog.Parametri.Add('orainizio',PParOraInizio);
    LInfoLog.Parametri.Add('orafine',PParOraFine);
  ...
  try
    ...
    try
      ...
      DM.SetParam('inizio',PParInizio);
      DM.SetParam('fine',PParFine);
      DM.SetParam('orainizio',PParOraInizio);
      DM.SetParam('orafine',PParOraFine);
...
begin
  ...
  begin
    try
      ...
      on E: Exception do
        R180ScriviMsgLog(FILE_LOG,TB021FUtils.GetLogFmt(Self.ClassName,'LogAccesso',Format('errore log I000: %s',[E.Message])));
    end;
  end;
end;

// Utenti
function TB021FIrisRestSvcDM.I060Utenti(PParDatabase, PParAzienda, PParUtenteI070,
 PParCodiceFiscale, PParMatricola, PParNomeUtente, PParPassword: String): TJSONObject;
var
  LSIW:TSessioneIrisWIN;
  DM: TB021FUtentiDM;
  LInfoLog: TInfoLog;
begin
  ...
  try
    ...
    LInfoLog.ClassNameServizio:=TB021FTurniDM.ClassName;
    LInfoLog.Operazione:=OPER_READ;
    LInfoLog.Parametri.Add('codice_fiscale',PParCodiceFiscale);
    LInfoLog.Parametri.Add('matricola',PParMatricola);
    LInfoLog.Parametri.Add('nome_utente',PParNomeUtente);
    LInfoLog.Parametri.Add('password',PParPassword);
    LInfoLog.LogAccesso;
  finally
    FreeAndNil(LInfoLog);
  end;
  ...
  try
    DM:=TB021FUtentiDM.Create(LSIW,
                             ...
                             Timestamp);
    try
      DM.SetParam('CodFiscale',PParCodiceFiscale);
      DM.SetParam('Matricola',PParMatricola);
      DM.SetParam('NomeUtente',PParNomeUtente);
      DM.SetParam('Password',PParPassword);
      Result:=DM.EseguiOperazione;
      DM.Disconnetti;
    finally
      FreeAndNil(DM);
...
function TB021FIrisRestSvcDM.updateI060Utenti(PParDatabase, PParAzienda, PParUtenteI070: String; PParUtenti: TJSONObject): TJSONObject;
// turni - update
var
  LSIW:TSessioneIrisWIN;
  DM: TB021FUtentiDM;
...
begin
  ...
  try
    ...
    LInfoLog.ClassNameServizio:=TB021FTurniDM.ClassName;
    LInfoLog.Operazione:=OPER_UPDATE;
    LInfoLog.Parametri.Add('JObject',PParUtenti.ToString);
    LInfoLog.LogAccesso;
  finally
    FreeAndNil(LInfoLog);
  end;
