...
type
  ...
  TB021FTest = class(TForm)
    ...
    procedure chkEseguiScriptClick(Sender: TObject);
  private
    Inizio: TDateTime;
    ...
    function GetURLCompleto(pUrl:String = ''): String;
  public
  end;
...
begin
  ...
  LPostData:=('{                                    '#13#10 +
//              '   "Matricola":"25",                 '#13#10 +
              '   "CodFiscale":"CRRDVD82A25D205A",   '#13#10 +
              '   "NomeUtente":"UsrB021",            '#13#10 +
              '   "Password":"UsrB021",              '#13#10 +
              '   "EMail":"UsrB021@mondoedp.com",    '#13#10 +
              '   "NomeProfilo":"TESTB021",          '#13#10 +
              '   "Permessi":"WEBRESP",             '#13#10 +
              '   "InizioValidita":"23/12/2021"     '#13#10 +
              '}');
  LDataRif:=FormatDateTime('dd-mm-yyyy',R180InizioMese(Date));
  LUtente:='MANCOP';
  LPassword:='MEDP';
...
function TB021FTest.GetURLCompleto(pUrl:String = ''):String;
var
  LQueryString: string;
begin
  if pUrl = '' then
    ...
    Result:=Format('%s/%s',[cmbServerURL.Text,pUrl]);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        FJsonData:=TB021FUtils.PulisciStringaJson(memPostData.Text);
      end;
      Res:=EseguiPost('json',FJsonData);
    end;
    memOutputSvc.Lines.Append(Res);
  except
    on E: Exception do
    begin
      memOutputSvc.Lines.Append(Format('%s: %s',[E.ClassName,E.Message]));
...
begin
  ...
  try
    ...
    LUnixTimeCorr:=TB021FUtils.GetUnixTimeCorrente;

    LMetodo:=MetodoList.Items[cmbMetodo.ItemIndex];
    if LMetodo.Name = 'AnaOpeFSE' then
    begin
      // imposta autenticazione specifica per Aosta_Asl
      edtQueryString.Text:=Format('?time=%d&auth=%s',[LUnixTimeCorr,TB021FUtils.GetHashAutenticazioneAostaAsl(LUnixTimeCorr)]);
    end
    ...
    begin
      ...
      begin
        ...
        LToken:=TB021FUtils.CreaToken(LUnixTimeCorr,LPassphrase);
    ...
    begin
      ...
      Inizio:=Now;
      if chkEseguiScript.Checked then
      begin
        for i:=0 to memoScript.Lines.Count - 1 do
        begin
          FUrl:=GetURLCompleto(memoScript.Lines[i]);
          lblConta.Caption:=Format('%d / %d',[i + 1,memoScript.Lines.Count]);
          lblConta.Repaint;
...
procedure TB021FTest.chkEseguiScriptClick(Sender: TObject);
begin
  lblMetodoURL.Enabled:=not chkEseguiScript.Checked;
  lblParJson.Enabled:=not chkEseguiScript.Checked;
  edtMethodURL.Enabled:=not chkEseguiScript.Checked;
  edtParametri.Enabled:=not chkEseguiScript.Checked;
  memoScript.Enabled:=chkEseguiScript.Checked;
end;
