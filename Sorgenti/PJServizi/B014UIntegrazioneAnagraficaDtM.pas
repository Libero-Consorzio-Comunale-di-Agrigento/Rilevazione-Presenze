...
uses
  ...
  Clipbrd, IdExplicitTLSClientServerBase, IniFiles;
...
type
  ...
  private
    ...
    function  StrToDateFmt(Data,Formato: String): TDateTime;
  public
    { Public declarations }
    RigheLog:Integer;
...
begin
  ...
  try
    ...
    if (selIA100.FieldByName('TIPO_STRUTTURA').AsString = 'F') and
       (selIA100.FieldByName('DIREZIONE_DATI').AsString = 'I') then
      ImportazioneFile;
    ...
    try
      ...
      if (not selIA100.FieldByName('AZIENDA').IsNull) and (selIA100.FieldByName('AZIENDA').AsString <> '*') then
      begin
        if selI090.FieldByName('AZIENDA').AsString <> selIA100.FieldByName('AZIENDA').AsString then
        begin
          selI090.Next;
          Continue;
        end;
      ...
      TRegistraMsg(SessioneIWB014.RegistraMsg).InserisciMessaggio('I','    Struttura: ' + selIA100.FieldByName('NOME_STRUTTURA').AsString,selI090.FieldByName('AZIENDA').AsString);
...
begin
  ...
  NomeFile:=R180GetFileName(selIA100.FieldByName('NOME_FILE').AsString);
  ...
  try
    ...
    IdFTP.Connect;
    if R180GetFilePath(selIA100.FieldByName('NOME_FILE').AsString,True) <> '' then
      IdFTP.ChangeDir(R180GetFilePath(selIA100.FieldByName('NOME_FILE').AsString,True));
...
begin
    ...
    try
      ...
      begin
        if Trim(Strings[i]) = '' then
          Continue;
        ChiaveUpd:=ChiaveUpd + VarToStr(selIA110.Lookup('NOME_DATO',Strings[i],'CAMPO')) + '=''' + AggiungiApice(Trim(selDatiInput.FieldByName(Strings[i]).AsString)) + '''';
        ChiaveInsCol:=ChiaveInsCol + VarToStr(selIA110.Lookup('NOME_DATO',Strings[i],'CAMPO'));
        ChiaveInsVal:=ChiaveInsVal + '''' + AggiungiApice(Trim(selDatiInput.FieldByName(Strings[i]).AsString)) + '''';
        if i < Count - 1 then
        begin
  ...
  try
    ...
    try
      Valore:=DateToStr(StrToDateFmt(Valore,selIA110.FieldByName('FMT_DATA').AsString));
    except
      Valore:='';
    end;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        try
          Dal:=StrToDateFmt(selDatiInput.FieldByName(StrutturaFissa.Decorrenza).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','DECORRENZA','FMT_DATA')));
        except
        end;
        if (StrutturaFissa.Scadenza <> '') and (not selDatiInput.FieldByName(StrutturaFissa.Scadenza).IsNull) then
        try
          Al:=StrToDateFmt(selDatiInput.FieldByName(StrutturaFissa.Scadenza).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','SCADENZA','FMT_DATA')));
        except
        end;
...
begin
  ...
  try
    ...
    try
      if selIA100.FieldByName('DIREZIONE_DATI').AsString = 'I' then
        SetVariable('DATA_REGISTRAZIONE',StrToDateFmt(selDatiInput.FieldByName(StrutturaFissa.Data).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','DATA','FMT_DATA'))))
      else
    ...
    try
      if selIA100.FieldByName('DIREZIONE_DATI').AsString = 'I' then
        SetVariable('DECORRENZA',StrToDateFmt(selDatiInput.FieldByName(StrutturaFissa.Decorrenza).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','DECORRENZA','FMT_DATA'))))
      else
      begin
        if selDatiOutput.FieldByName(VarToStr(selIA110.Lookup('INTESTAZIONE','DECORRENZA','CAMPO'))).DataType in [ftDate,ftDateTime] then
          ...
          SetVariable('DECORRENZA',StrToDateFmt(selDatiOutput.FieldByName(VarToStr(selIA110.Lookup('INTESTAZIONE','DECORRENZA','CAMPO'))).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','DECORRENZA','FMT_DATA'))));
      end;
    except
    end;
    if StrutturaFissa.Scadenza <> '' then
    try
      if selIA100.FieldByName('DIREZIONE_DATI').AsString = 'I' then
        SetVariable('SCADENZA',StrToDateFmt(selDatiInput.FieldByName(StrutturaFissa.Scadenza).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','SCADENZA','FMT_DATA'))))
      else
      begin
        ...
        SetVariable('SCADENZA',StrToDateFmt(selDatiOutput.FieldByName(VarToStr(selIA110.Lookup('INTESTAZIONE','SCADENZA','CAMPO'))).AsString,VarToStr(selIA110.Lookup('INTESTAZIONE','SCADENZA','FMT_DATA'))));
      end;
    except
    end;
    if selIA100.FieldByName('DIREZIONE_DATI').AsString = 'I' then
  ...
  begin
    ...
    try
      ...
      try
        ...
        while True do
          try
            IdFTP.List(lst,R180GetFileName(selIA100.FieldByName('NOME_FILE').AsString) + '.lck',False);
            if lst.Count = 0 then
              Abort;
            if (lst.Count > 0) and (Pos('NO SUCH FILE OR DIRECTORY',UpperCase(lst.Text)) > 0) then
              Abort;
          except
            R180AppendFile(R180GetFileName(selIA100.FieldByName('NOME_FILE').AsString) + '.lck','');
            IdFTP.Put(R180GetFileName(selIA100.FieldByName('NOME_FILE').AsString + '.lck'),selIA100.FieldByName('NOME_FILE').AsString + '.lck',False);
            Break;
          end;
        lst.Clear;
        try
          IdFTP.List(lst,R180GetFileName(selIA100.FieldByName('NOME_FILE').AsString),False);
        except
          lst.Add('*');
        end;
        if (lst.Count > 0) and (lst[0] = R180GetFileName(selIA100.FieldByName('NOME_FILE').AsString)) then
...
function TB014FIntegrazioneAnagraficaDtM.StrToDateFmt(Data, Formato: String): TDateTime;
var
  DataOut,TimeOut: String;
  i: Integer;
begin
  ...
  if Formato <> '' then
  begin
    DataOut:={$IFNDEF VER210}FormatSettings.{$ENDIF}ShortDateFormat.ToUpper;
