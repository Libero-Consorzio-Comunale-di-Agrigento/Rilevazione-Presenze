...
type
  ...
  TB014FIntegrazioneAnagrafica = class(TForm)
    ...
    procedure AllineaIA110Click(Sender: TObject);
  private
...
begin
  ...
  if SessioneOracle.Connected then
    ...
    begin
      dgrdIA100.Columns[0].PickList.Clear;
      dgrdIA110.Columns[0].PickList.Clear;
      dgrdIA100.Columns[0].PickList.Add('*');
      dgrdIA110.Columns[0].PickList.Add('*');
      Open;
      while not Eof do
      begin
        dgrdIA100.Columns[0].PickList.Add(FieldByName('AZIENDA').AsString);
...
begin
  memoLog.Clear;
  //if FileExists(B014FIntegrazioneAnagraficaDtM.FileLog) then
  //  memoLog.Lines.LoadFromFile(B014FIntegrazioneAnagraficaDtM.FileLog);
  try
...
procedure TB014FIntegrazioneAnagrafica.btnPulisciLogClick(Sender: TObject);
{Eliminazione del file di log}
begin
{
  if FileExists(B014FIntegrazioneAnagraficaDtM.FileLog) then
    if R180MessageBox('Eliminare il file ' + B014FIntegrazioneAnagraficaDtM.FileLog + '?',DOMANDA) = mrYes then
    begin
      DeleteFile(B014FIntegrazioneAnagraficaDtM.FileLog);
      memoLog.Clear;
    end;
}
end;
...
begin
  ...
  AllineaIA110.Enabled:=B014FMonitorIntegrazioneDtM.selIA100.FieldByName('AZIENDA').AsString <> '*';
end;
...
procedure TB014FIntegrazioneAnagrafica.AllineaIA110Click(Sender: TObject);
begin
  B014FMonitorIntegrazioneDtM.AllineaAziendaIA110;
end;
