...
//('C90_EMAIL_SENDER_INDIRIZZO','C90_EMAIL_SMTPHOST','C90_EMAIL_USERNAME','C90_EMAIL_PASSWORD')
  ...
  if B005FAggIrisDtM1.selI091.Lookup('TIPO','C90_EMAIL_PASSWORD','DATO') <> null then
    edtPassword.Text:=R180Decripta(B005FAggIrisDtM1.selI091.Lookup('TIPO','C90_EMAIL_PASSWORD','DATO'),I091CryptKey);
end;
...
begin
  ...
  try
    ...
    FileSearch:=ExtractFilePath(Application.ExeName) + 'Archivi\Log\Script_*.log';
    if FindFirst(FileSearch,$3F,F) = 0 then
    repeat
      TIdAttachmentFile.Create(IdMessage.MessageParts, ExtractFilePath(Application.ExeName) + 'Archivi\Log\' + F.Name);
    until FindNext(F) <> 0;
    FindClose(F);
    FileSearch:=ExtractFilePath(Application.ExeName) + 'Archivi\Log\Proc_*.log';
    if FindFirst(FileSearch,$3F,F) = 0 then
    repeat
      TIdAttachmentFile.Create(IdMessage.MessageParts, ExtractFilePath(Application.ExeName) + 'Archivi\Log\' + F.Name);
    until FindNext(F) <> 0;
