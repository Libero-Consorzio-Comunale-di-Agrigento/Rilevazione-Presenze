...
//{$IFDEF IRISWEB}
{$IF Defined(IRISWEB) or Defined(BPL)}

uses
  SysUtils, StrUtils, Math, Classes, Controls, Windows, PsAPI, Variants,
  {$IFNDEF WEBSVC}
  IWDBGrids,
  ...
  IWCompMemo, meIWDBLookupComboBox, meTIWCheckListBox,
  ...
  {$IF CompilerVersion >= 31}System.JSON,{$ENDIF}
  meIWRadioGroup, meIWGrid, OracleData,
  {$ENDIF WEBSVC}
  ActnList, Db, C180FunzioniGenerali;
...
const
  ...
  fileImgSalva              = 'img\btnDownload.png';
  ...
  DBG_COMANDI     = 'DBG_COMANDI';
  DBG_COMANDI2    = 'DBG_COMANDI2';
  DBG_ITER        = 'DBG_ITER';
...
{$IFNDEF WEBSVC}
procedure C190SelezionaCheckGroup(CheckGroup:TmeTIWAdvCheckGroup; Val:Boolean);
...
function  C190DBGridCreaMemo(POwner:TComponent; PParent: TWinControl; const FN:String; const StileCss:String; const MaxLunghezza:Integer = -1):TmeIWMemo;
...
function  C190CssToggleClass(const PCurrentCss, PCssClass: String; const PAbilita: Boolean): String;
procedure C190PutCheckList(S:String; L:Word; CL:TmeTIWAdvCheckGroup; Separator:char=',');
function  C190GetCheckList(L:Word; CL:TmeTIWAdvCheckGroup; Separator:char=','; ValoriDaSelezionare:Boolean=True):String; Overload;
function  C190GetCheckList(L:Word; CL:TmeTIWCheckListBox; Separator:char=','; ValoriDaSelezionare:Boolean=True):String; Overload;
...
function  C190ConvertiInversoSimboliHtml(const S: String): String;
...
procedure C190VisualizzaElementoAsync(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean);
procedure C190VisualizzaElementoConVisibility(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean);
procedure C190VisualizzaElementoConVisibilityAsync(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean);
procedure C190Comp2JsonString(Sender: TComponent; var Json: TJSONObject; NomeComp:String = '');
procedure C190CaricaMepdMulticolumnComboBox(Combo:TMedpIWMultiColumnComboBox;ODS:TDataset;const CampoCod: String = 'CODICE'; const CampoDesc: String = 'DESCRIZIONE'); overload;
procedure C190CaricaMepdMulticolumnComboBox(Combo:TMedpIWMultiColumnComboBox;ODS:TDataset;ArrCampi:array of string); overload;
function  C190ImpostaCssInvisibile(css:String; bVisibile:Boolean): String;
function  C190ImpostaCssNascosto(css:String; bVisibile:Boolean): String;
...
{$IFNDEF WEBSVC}
uses A000UInterfaccia, A000UMessaggi, System.IOUtils, IWGlobal;
...
function C190ImpostaCssInvisibile(css:String; bVisibile:Boolean): String;
begin
  Result:=StringReplace(css,' invisibile', '', [rfReplaceAll]);
  if not bVisibile then
    Result:=Result + ' invisibile';
end;

function C190ImpostaCssNascosto(css:String; bVisibile:Boolean): String;
begin
  Result:=StringReplace(css,' nascosto', '', [rfReplaceAll]);
  if not bVisibile then
    Result:=Result + ' nascosto';
end;
...
begin
  ...
  begin
    ...
    FriendlyName:=FN;
    RenderSize:=False;
    if (MaxLunghezza > 0) and (Pos('maxlength',Result.ExtraTagParams.Text.ToLower) = 0) then
      Result.ExtraTagParams.Add(Format('maxlength="%d"',[MaxLunghezza]));
  end;
end;
...
begin
  ...
  begin
    ...
    LCssTemp:=Trim(ACell.Css);
    if (LCssTemp <> '') and
       (Intestazione or RigheAlterne or EvidenziaRigaSel) then
    begin
      LCssTemp:=StringReplace(LCssTemp,'riga_selezionata','',[rfReplaceAll]);
      LCssTemp:=StringReplace(LCssTemp,'riga_intestazione','',[rfReplaceAll]);
      LCssTemp:=StringReplace(LCssTemp,'riga_bianca','',[rfReplaceAll]);
      LCssTemp:=StringReplace(LCssTemp,'riga_colorata','',[rfReplaceAll]);
    end;
...
begin
  ...
  else if Comp is TmeIWImageFile then
  begin
    TmeIWImageFile(Comp).Enabled:=PAbilita;
    LImgFile:=TmeIWImageFile(Comp).ImageFile.Filename;
    if PAbilita and (LImgFile.Contains(IMG_DISABLED)) then
      TmeIWImageFile(Comp).ImageFile.Filename:=TmeIWImageFile(Comp).ImageFile.Filename.Replace(IMG_DISABLED,'')
    else if (not PAbilita) and (not LImgFile.Contains(IMG_DISABLED)) then
    begin
      // imposta il path dell'immagine disabilitata (solo se il file corrispondente esiste)
      //   il pattern delle immagini disabilitate è:
      //   [nome_file_originale]_Disabled[.estensione]
      LImgFile:=LImgFile.Replace('.',IMG_DISABLED + '.');
      if TFile.Exists(TPath.Combine(gSC.ContentPath,LImgFile)) then
        TmeIWImageFile(Comp).ImageFile.Filename:=LImgFile;
    end;
    TmeIWImageFile(Comp).Css:=C190CssToggleClass(TmeIWImageFile(Comp).Css,'cursor_pointer',PAbilita);
  end
  else
  begin
...
function C190CssToggleClass(const PCurrentCss, PCssClass: String; const PAbilita: Boolean): String;
// toggle di una particolare classe css
var
  LPosClass: Integer;
  LHasClass: Boolean;
begin
  LPosClass:=R180CercaParolaIntera(PCssClass,PCurrentCss,' ');
  LHasClass:=LPosClass > 0;
  if PAbilita then
  begin
    // se si vuole aggiungere la classe e la classe non è presente, la aggiunge
    Result:=PCurrentCss + IfThen(not LHasClass,' ' + PCssClass);
  end
  else if LHasClass then
  begin
    // se si vuole rimuovere la classe e la classe è presente, la rimuove
    Result:=PCurrentCss.Remove(LPosClass - 1,PCssClass.Length);
  end;

  // trim finale del risultato
  Result:=Result.Trim;
end;
...
function C190GetCheckList(L:Word; CL:TmeTIWAdvCheckGroup; Separator:char=','; ValoriDaSelezionare:Boolean=True):String;
var i:Integer;
begin
  Result:='';
  for i:=0 to CL.Items.Count - 1 do
  begin
    if CL.IsChecked[i] = ValoriDaSelezionare then
    begin
      if Result <> '' then
...
// Overload per gestire oggetti di tipo meTIWCheckListBox
function C190GetCheckList(L:Word; CL:TmeTIWCheckListBox; Separator:char=','; ValoriDaSelezionare:Boolean=True):String;
var i:Integer;
begin
  Result:='';
  for i:=0 to CL.Items.Count - 1 do
  begin
    if CL.Selected[i] = ValoriDaSelezionare then
    begin
      if Result <> '' then
        Result:=Result + Separator;
      Result:=Result + Trim(Copy(CL.Items[i],1,L));
...
begin
  ...
  Result:=StringReplace(Result,CR,'<br>',[rfReplaceAll]);
  Result:=StringReplace(Result,LF,'<br>',[rfReplaceAll]);
  Result:=StringReplace(Result,'"','&quot;',[rfReplaceAll]);
  Result:=StringReplace(Result,'€','&#8364;',[rfReplaceAll]);
...
function C190ConvertiInversoSimboliHtml(const S: String): String;
// converte gli escape HTML nei relativi caratteri
begin
  Result:=S;
  ...
  Result:=StringReplace(Result,'<br>',CRLF,[rfReplaceAll]);
  {Result:=StringReplace(Result,CR,'<br>',[rfReplaceAll]);
  Result:=StringReplace(Result,LF,'<br>',[rfReplaceAll]);}
  Result:=StringReplace(Result,'&lt;','<',[rfReplaceAll]);
  Result:=StringReplace(Result,'&gt;','>',[rfReplaceAll]);
...
function JQActionVisualizzaIDConVisibility(Nome:String;Visualizza:Boolean):String; inline;
begin
  if Visualizza then
    Result:='$(''#' + Nome + ''').css("visibility","visible");'
  else
    Result:='$(''#' + Nome + ''').css("visibility","hidden");';
end;
...
procedure C190VisualizzaElementoAsync(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean); inline;
var s: String;
begin
  s:=JQActionVisualizzaID(Nome,Visualizza);
  GGetWebApplicationThreadVar.CallBackResponse.AddJavaScriptToExecuteAsCDATA(s);
  C190VisualizzaElemento(JQuery,Nome,Visualizza);
end;
...
procedure C190VisualizzaElementoConVisibilityAsync(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean); inline;
var
  s:String;
begin
  // Nasconde e visualizza un elemento modificando la proprietà CSS "visibility".
  // A differenza di "display: none", "visibility: hidden" mantiene comunque lo spazio occupato
  // dall'elemento.
  s:=JQActionVisualizzaIDConVisibility(Nome,Visualizza);
  GGetWebApplicationThreadVar.CallBackResponse.AddJavaScriptToExecuteAsCDATA(s);
  C190VisualizzaElementoConVisibility(JQuery,Nome,Visualizza);
end;

procedure C190VisualizzaElementoConVisibility(JQuery:TIWJQuerywidget;Nome:String;Visualizza:Boolean); inline;
var
  s:String;
begin
  s:=JQActionVisualizzaIDConVisibility(Nome,True);
  RemoveJQAction(JQuery,s);
  s:=JQActionVisualizzaIDConVisibility(Nome,False);
  RemoveJQAction(JQuery,s);

  JQuery.OnReady.Add(JQActionVisualizzaIDConVisibility(Nome,Visualizza));
end;
...
begin
  try
    ...
    else if Sender is TmeIWDBLookupComboBox then
      Value:=VarToStr((Sender as TmeIWDBLookupComboBox).KeyValue)
...
procedure C190CaricaMepdMulticolumnComboBox(Combo:TMedpIWMultiColumnComboBox;ODS: TDataset;const CampoCod: String = 'CODICE'; const CampoDesc: String = 'DESCRIZIONE');
begin
  ODS.First;
  Combo.Items.Clear;
...
procedure C190CaricaMepdMulticolumnComboBox(Combo:TMedpIWMultiColumnComboBox;ODS:TDataset;ArrCampi:array of string);
var
  ListaCampi:string;
  i:integer;
begin
  ODS.First;
  Combo.Items.Clear;
  Combo.ColCount:=Length(ArrCampi);
  while not ODS.Eof do
  begin
    ListaCampi:='';
    for i:=Low(ArrCampi) to High(ArrCampi) do
    begin
      ListaCampi:=ListaCampi + ODS.FieldByName(ArrCampi[i]).AsString;
      if i < High(ArrCampi) then
      begin
        ListaCampi:=ListaCampi + ';';
      end;
    end;
    Combo.AddRow(ListaCampi);
    ODS.Next;
  end;
end;
