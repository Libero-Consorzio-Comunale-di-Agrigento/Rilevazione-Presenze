...
uses
  ...
  IWCompLabel, meIWLabel, System.Types, IWCompCheckbox, meIWCheckBox,
  System.StrUtils;
...
type
  ...
  private
    iIWFile: Integer;
    FIterMaxAllegati: Integer;
    FIterMaxDimAllegatoMB: Integer;
...
begin
  ...
  if not (WR000DM.ModuliAccessori.IsAbilitato(Parametri.Azienda,'ITER_AUTORIZZATIVI')) then
    raise Exception.Create('Funzione non disponibile: modulo Iter Autorizzativi non abilitato!');
  ...
  FIterMaxAllegati:=StrToIntDef(Parametri.CampiRiferimento.C90_IterMaxAllegati,999);
  if FIterMaxAllegati < 0 then
    FIterMaxAllegati:=999;
  FIterMaxDimAllegatoMB:=StrToIntDef(Parametri.CampiRiferimento.C90_IterMaxDimAllegatoMB,5);
end;
...
begin
  ...
  begin
    if FIterMaxAllegati = 1 then
      lblInfo.Caption:=Format(A000TraduzioneStringhe(A000MSG_WC026_MSG_FMT_INFO_ALLEGATI_1),[FIterMaxDimAllegatoMB])
    else
      lblInfo.Caption:=Format(A000TraduzioneStringhe(A000MSG_WC026_MSG_FMT_INFO_ALLEGATI_N),[FIterMaxAllegati,FIterMaxDimAllegatoMB]);
  end
  else
  begin
...
begin
  ...
  grdAllegati.medpTestoNoRecord:=A000TraduzioneStringhe(A000MSG_WC026_MSG_NESSUN_ALLEGATO);
...
begin
  ...
  begin
    ...
    { DONE : TEST IW 14 OK }
    (*TEST_IW_XIV//IWImg.DontSubmitFiles:=False;*)
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        LExtFile:=grdAllegati.medpValoreColonna(i,'EXT_FILE');
        IWLnk:=TmeIWLink(IWC);
        IWLnk.Css:='link file_ext';
        { DONE : TEST IW 14 OK }
        (*TEST_IW_XIV//IWLnk.DontSubmitFiles:=True;*)
        IWLnk.ExtraTagParams.Add(Format('data-file-ext=%s',[LExtFile]));
...
begin
  ...
  Result:=True;
  if grdAllegati.medpDataSet.RecordCount >= FIterMaxAllegati then
    raise Exception.Create(Format(A000TraduzioneStringhe(A000MSG_WC026_ERR_FMT_MAX_ALLEGATI),[FIterMaxAllegati]));
end;
...
begin
  ...
  try
    ...
    PopolaDati;
    //btnChiudiClick(nil); // aggiunto per evitare la scomparsa del pulsante "chiudi" in seguito ad un eccezione con popup aperto
  end;
end;
  ...
  try
    ...
    begin
     ...
     { DONE : TEST IW 14 OK }
      ...
      if LDimMB > FIterMaxDimAllegatoMB then
      begin
        ...
        // solleva eccezione
        raise Exception.Create(Format(A000TraduzioneStringhe(A000MSG_ERR_FMT_DIM_ALLEGATO),[FIterMaxDimAllegatoMB,LDimMB]));
      end;
