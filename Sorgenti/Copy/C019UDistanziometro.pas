...
uses A000UCostanti, IdHttp, IdURI,
     StrUtils, System.SysUtils,
     DBXJSON,{$IF CompilerVersion >= 31}System.JSON,{$ENDIF}
     IdAuthentication,IdHeaderList,IdAuthenticationDigest, IdAuthenticationSSPI;
...
type
  ...
  private
    class function ParseResultGoogleJson(const PStringaJson: string; var RDistanza: Integer; var RErrMsg: String): Boolean; static;
    class procedure ProxyAuthorization(Sender: TObject; Authentication: TIdAuthentication; var Handled: Boolean);
    class procedure SelectProxyAuthorization(Sender: TObject; var AuthenticationClass: TIdAuthenticationClass; AuthInfo: TIdHeaderList);
  public
    class function GetDistanza(const PPartenza, PDestinazione: String; var RErrore: String; PProxy:String; const PTipoServizio: TTipoServizio = tsGoogleJson): Integer; static;
    class function FormattaLocalita(const PNomeLocalita, PCap, PProvincia: String; const PTipoServizio: TTipoServizio = tsGoogleJson): String; static;
  end;
...
class function TC019FDistanziometro.GetDistanza(const PPartenza, PDestinazione: String; var RErrore: String; PProxy:String; const PTipoServizio: TTipoServizio = tsGoogleJson): Integer;
var
...
begin
  ...
  case PTipoServizio of
    ...
    tsGoogleJson:
      begin
        ...
        Url:=GOOGLE_MAPS_DIST_WS_URL_SSL + GOOGLE_MAPS_DIST_WS_OUTPUT_JSON;
        ParUrl:=Format('?key=%s&origins=%s&destinations=%s&sensor=%s&language=%s',
                       [GOOGLE_MAPS_API_KEY,LPartenza,LDestinazione,GOOGLE_MAPS_DIST_WS_PAR_SENSOR,GOOGLE_MAPS_DIST_WS_PAR_LANGUAGE]);
      end;
    tsViaMichelinRest:
      begin
        raise Exception.Create('Non implementato');
  ...
  try
    try
      ...
      if (PProxy <> '') and (Pos(':',PProxy) > 0) then
      begin
        with IdHttp do
        begin
          //PProxy = host:port
          ProxyParams.ProxyServer:=Copy(PProxy,1,Pos(':',PProxy) - 1);                          //'proxy2.asl5sp.int';
          ProxyParams.ProxyPort:=StrToIntDef(Copy(PProxy,Pos(':',PProxy) + 1,Length(PProxy)),0);//8080;
          HTTPOptions:=[hoInProcessAuth,hoForceEncodeParams];
          OnProxyAuthorization:=ProxyAuthorization;
          OnSelectProxyAuthorization:=SelectProxyAuthorization;
        end;
      end;
...
class procedure TC019FDistanziometro.SelectProxyAuthorization(Sender: TObject; var AuthenticationClass: TIdAuthenticationClass; AuthInfo: TIdHeaderList);
begin
  inherited;
  ...
  (*E' inutile!!
  (Sender as TIdHTTP).ProxyParams.ProxyUsername:='asl5sp\mondoedp.presenze.01';
  (Sender as TIdHTTP).ProxyParams.ProxyPassword:='presenzesp2008';
  *)
end;

class procedure TC019FDistanziometro.ProxyAuthorization(Sender: TObject; Authentication: TIdAuthentication; var Handled: Boolean);
begin
  Handled:=True;
end;
  ...
  try
    try
      ...
      begin
        ...
        jRowObj:=(jRowsArr.Items[0] as TJSONObject);
        jElementsArr:=(jRowObj.Get('elements').JSONValue as TJSONArray);

        // considera solo l'elemento 0
        jElementObj:=(jElementsArr.Items[0] as TJSONObject);
