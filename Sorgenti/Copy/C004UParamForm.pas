...
type
  ...
  public
    { Public declarations }
    function GetParametro(Nome,ValDef:String):String; overload;
    function GetParametro(MyComp:TComponent; ValDef:String):String; overload;
    procedure PutParametro(Nome,Valore:String); overload;
    procedure PutParametro(MyComp:TComponent; Valore:String); overload;
...
function TC004FParamForm.GetParametro(MyComp:TComponent; ValDef:String):String;
begin
  Result:=ValDef;
  if selT001.Locate('NOME',UpperCase(MyComp.Name),[]) then
...
begin
  try
    DelT001.Execute;
    {$IFDEF IRISWEB}{$IFNDEF WEBPJ}DelT001.Session.Commit;{$ENDIF}{$ENDIF}
  except
  end;
end;

procedure TC004FParamForm.PutParametro(MyComp:TComponent; Valore:String);
begin
  try
    with InsT001 do
    begin
      SetVariable('NOME',UpperCase(MyComp.Name));
      SetVariable('VALORE',Valore);
      Execute;
      {$IFDEF IRISWEB}{$IFNDEF WEBPJ}Session.Commit;{$ENDIF}{$ENDIF}
    end;
  except
  end;
end;
