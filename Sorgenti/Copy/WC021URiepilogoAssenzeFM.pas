...
uses
  ...
  A000UInterfaccia, medpIWMessageDlg, StrUtils, DateUtils, IWAdvRadioGroup,
  meTIWAdvRadioGroup, C004UParamForm, Math;
...
type
  ...
  TWC021FRiepilogoAssenzeFM = class(TWR200FBaseFM)
    ...
    grdSintetica: TmeIWGrid;
    rgpTipoRiepilogo: TmeTIWAdvRadioGroup;
    procedure btnChiudiClick(Sender: TObject);
    procedure IWFrameRegionCreate(Sender: TObject);
    procedure grdRiepilogoRenderCell(ACell: TIWGridCell; const ARow,
      AColumn: Integer);
    procedure grdSinteticaRenderCell(ACell: TIWGridCell; const ARow,
      AColumn: Integer);
  ...
  private
    ...
    procedure IntestazioneGrdSintetica(UMisura: String);
...
begin
  ...
  if (ARow > 0) and ((AColumn = 0) or (AColumn = 7) or (AColumn = 8)) then
  begin
    ACell.css:=ACell.css + ' font_grassetto';
    if (AColumn = 0) then
      ACell.css:=ACell.css + ' intestazione';
  end;
...
procedure TWC021FRiepilogoAssenzeFM.grdSinteticaRenderCell(ACell: TIWGridCell;
  const ARow, AColumn: Integer);
begin
  if not C190RenderCell (ACell,ARow,AColumn,True,True,False) then
    Exit;
  ...
  if (ARow = 1) and (AColumn = RowFruitoCorrente) then  //Riga Fruito A.C.
  begin
    if R600Dtm1.UMisura = 'G' then  //Giorni
    begin
      if StrToFloatDef(grdSintetica.Cell[1,RowFruitoCorrente].Text,0) < StrToFloatDef(R600Dtm1.RiepilogoAssenze[0].FruizMinimaAC,0) then
        ACell.css:=ACell.css + ' font_blu';
    end
    else  //Ore
    begin
      if R180OreMinutiExt(grdSintetica.Cell[1,RowFruitoCorrente].Text) < R180OreMinutiExt(R600Dtm1.RiepilogoAssenze[0].FruizMinimaAC) then
        ACell.css:=ACell.css + ' font_blu';
    end;
  end;
...
begin
  ...
  C004DMRiep:=CreaC004(SessioneOracle,'WC021',Parametri.ProgOper,False);
  GetParametriFunzione;
end;

procedure TWC021FRiepilogoAssenzeFM.GetParametriFunzione;
begin
  rgpTipoRiepilogo.ItemIndex:=IfThen(C004DMRiep.GetParametro('rgpTipoRiepilogo','D') = 'S',0,1);
end;

procedure TWC021FRiepilogoAssenzeFM.PutParametriFunzione;
begin
  C004DMRiep.Cancella001;
  C004DMRiep.PutParametro('rgpTipoRiepilogo',IfThen(rgpTipoRiepilogo.ItemIndex = 0,'S','D'));
  try SessioneOracle.Commit; except end;
end;

procedure TWC021FRiepilogoAssenzeFM.rgpTipoRiepilogoClick(Sender: TObject);
begin
  inherited;
...
begin
  ...
  begin
    ...
    FDataNas:=DataNas;
    RiepilogaAssenze(FProgressivo,FDataRiepilogo,Giustif,False,FDataNas);
    FC:=FineCumulo;
...
begin
  ...
  begin
    ...
    IntestazioneGrdSintetica(UMisura);
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          grdSintetica.Cell[0,Row].Text:='Comp. prec.';
          grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
      ...
      begin
        ...
        grdSintetica.Cell[0,Row].Text:='Comp. corr.';
        grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
      ...
      begin
        ...
        begin
          ...
          grdSintetica.Cell[0,Row].Text:='Fruito prec.';
          grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
          if TotaleGiorni and RiepilogoAssenze[0].EsisteResiduo then
      ...
      begin
        ...
        grdSintetica.Cell[0,Row].Text:='Fruito corr.';
        grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
        if TotaleGiorni then
      ...
      begin
        ...
        begin
          ...
          grdSintetica.Cell[0,Row].Text:='Residuo prec.';
          grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
          if TotaleGiorni and RiepilogoAssenze[0].EsisteResiduo then
      ...
      begin
        ...
        grdSintetica.Cell[0,Row].Text:='Residuo corr.';
        grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
        if TotaleGiorni then
      ...
      begin
        ...
        begin
          ...
          grdSintetica.Cell[0,Row].Text:='Residuo tot.';
          grdSintetica.Cell[1,Row].Text:=grdRiepilogo.Cell[Row,7].Text;
          if TotaleGiorni then
    ...
    grdSintetica.ColumnCount:=Row - IfThen(RiepilogoAssenze[0].CompAnnoSolare,2);
    //Alberto 15/02/2006
    ...
    begin
      ...
      begin
        ...
        if j = 7 then
          for i:=1 to grdSintetica.ColumnCount - 1 do
            grdSintetica.Cell[1,i].Text:='';
      end;
    end;
    VisualizzaComponenti;
...
procedure TWC021FRiepilogoAssenzeFM.IntestazioneGrdSintetica(UMisura: String);
begin
...
procedure TWC021FRiepilogoAssenzeFM.VisualizzaComponenti;
begin
  C190VisualizzaElemento(JQuery,'grpRaggruppamento',rgpTipoRiepilogo.ItemIndex <> 0);
  grdRiepilogo.Visible:=rgpTipoRiepilogo.ItemIndex <> 0;
...
begin
  ...
  if C004DMRiep <> nil then
    FreeAndNil(C004DMRiep);
  FreeAndNil(R600DtM1);
  ReleaseOggetti;
  Free;
end;
