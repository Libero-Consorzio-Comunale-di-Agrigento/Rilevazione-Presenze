...
begin
  ...
  try
    ...
    if ODS <> nil then
    begin
      if sNomeTabella = 'M042_LOCALITA' then
      begin
        C015FElencoValori.selTab:=ODS;
        C015FElencoValori.sPv_SqlText:=ODS.SQL.Text;
        C015FElencoValori.DSelTab.DataSet:=C015FElencoValori.selTab;
      end
      else if (sNomeTabella = 'T962_TIPO_DOCUMENTI') or
              (sNomeTabella = 'T963_UFFICIO_DOCUMENTI') then
      begin
        C015FElencoValori.dgrdElencoValori.Options:=C015FElencoValori.dgrdElencoValori.Options - [dgMultiSelect];
        C015FElencoValori.selTab:=ODS;
        C015FElencoValori.sPv_SqlText:=ODS.SQL.Text;
        C015FElencoValori.DSelTab.DataSet:=C015FElencoValori.selTab;
      end
...
procedure TC015FElencoValori.FormDestroy(Sender: TObject);
begin
  selTab:=nil;
end;
...
begin
  ...
  begin
    if (dgrdElencoValori.Columns[i].Visible) and
       (dgrdElencoValori.Columns[i].Field.FieldKind = fkData) and
       (dgrdElencoValori.Columns[i] <> Column)
    then
      sDep:=sDep + dgrdElencoValori.Columns[i].FieldName + ',';
  end;
  if sDep <> '' then
    sDep:='ORDER BY ' + Copy(sDep,1,length(sDep)-1);
  if SelTab.VariableIndex('ORDERBY') = -1 then
    SelTab.SQL.Add(sDep)
  else
    SelTab.SetVariable('ORDERBY',sDep);
...
procedure TC015FElencoValori.actRicercaTestoContenutoExecute(Sender: TObject);
var
  Trovato: Integer;
begin
...
procedure TC015FElencoValori.DSelTabDataChange(Sender: TObject; Field: TField);
var
  A:variant;
  i:integer;
  sMyNomeCampi:TStringList;
