...
uses
  ...
  Datasnap.DBClient, FunzioniGenerali, System.DateUtils,
  System.StrUtils;
...
type
  TC023FInfoDati = class(TForm)
    ...
    procedure dgrdInfoEditButtonClick(Sender: TObject);
  private
...
uses
  ...
  A000UInterfaccia, A000UMessaggi, Oracle, C012UVisualizzaTesto, C021UDocumentiManagerDM;
...
begin
  ...
  if Assigned(C018) and (C018.Owner = Self) and (C018.Name = 'C023') then
...
begin
  ...
  dgrdInfo.DataSource.DataSet:=C018.selT851;
  C018.Id:=IDRichiesta;
  C018.Iter:=FC018IterInfo.Iter;
...
procedure TC023FInfoDati.dgrdInfoEditButtonClick(Sender: TObject);
var
  Str:TStringList;
begin
  inherited;
  Str:=TStringList.Create;
  Str.Text:=dgrdInfo.DataSource.DataSet.FieldByName('NOTE').AsString;
  try
