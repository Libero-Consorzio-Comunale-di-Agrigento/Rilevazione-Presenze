...
uses
  ...
  Windows, System.StrUtils, IdStack, System.Hash, IdGlobal, IdCoderMIME, EncdDecd, System.Zip;
...
type
  ...
  private
    ...
    function  PreparaZipFile(var pZipFile: string): TResCtrl;
  public
    Maschera: String;
    CancellaFileTemp: Boolean;
    function  FindById(const PId: Integer): Boolean;  //al momento non utilizzato
    function  GetById(const PId: Integer; const PReadBinaryData: Boolean; out RDoc: TDocumento; pSalvaFile: Boolean=True): TResCtrl;
    function  AddAllegatoByIdToZip(PId: Integer; pRadiceNomeDoc: string; var pZipFile: string): TResCtrl;
    function  AddFileToZip(pData: TStream; pNomeFile: string; var pZipFile: string): TResCtrl;
    procedure CloseZip;
    ...
    function  GetBase64HashSHA512(Stream: TStream): String;
    procedure AssegnaSessione(pSession: TOracleSession = nil);
  end;
...
function TC021FDocumentiManagerDM.GetById(const PId: Integer; const PReadBinaryData: Boolean; out RDoc: TDocumento; pSalvaFile: Boolean=True): TResCtrl;
...
var
  LUrlServizio: String;
begin
  LUrlServizio:=FUrlB021 + Format('cancelC021DocumentoBlob/*/%s/%s/%s/%d',[Parametri.Azienda,Parametri.Operatore,StringtoBase64Url(PPath),PId]);
...
var
  LUrlServizio: String;
begin
  LUrlServizio:=FUrlB021 + Format('cancelC021DocumentoBlob/*/%s/%s/%s/%s',[Parametri.Azienda,Parametri.Operatore,StringtoBase64Url(PPath),PFileName]);
  Result:=_DeleteJson(LUrlServizio);
end;
...
var
  LUrlServizio: String;
begin
  LUrlServizio:=FUrlB021 + Format('C021DocumentoBlob/*/%s/%s/%s/%d',[Parametri.Azienda,Parametri.Operatore,StringtoBase64Url(PPath),PId]);
...
var
  LUrlServizio: String;
begin
  LUrlServizio:=FUrlB021 + Format('C021DocumentoBlob/*/%s/%s/%s/%s',[Parametri.Azienda,Parametri.Operatore,StringtoBase64Url(PPath),PFileName]);
  Result:=_DownloadJson(LUrlServizio,RFileBase64);
end;
...
begin
  ...
  try
    ...
    on E: Exception do
    begin
      FSessioneOracle.Rollback;  // rollback in caso di errore
      Result.Ok:=False;
...
begin
  ...
  LUrlServizio:=FUrlB021 + Format('acceptC021DocumentoBlob/*/%s/%s',[Parametri.Azienda,Parametri.Operatore]);
  LB021DM:=TB021FWebSvcClientDtM.Create(Owner);
  try
    // chiamata al servizio web
    Result:=LB021DM.EseguiMetodoRest(LUrlServizio, 'json', 'POST', InputString);
  finally
    FreeAndNil(LB021DM);
  end;
end;
...
begin
  ...
  if PDoc.Blob = nil then
    PDoc.Blob:=TLOBLocator.CreateTemporary(FSessioneOracle,otBLOB,True);
  try
...
procedure TC021FDocumentiManagerDM.AssegnaSessione(pSession: TOracleSession = nil);
var  //Per utilizzo da B005 forza una sessione diversa
  i:integer;
begin
  if Assigned(pSession) then
    FSessioneOracle:=pSession
  else
  ...
  begin
    if Components[i] is TOracleQuery then
      (Components[i] as TOracleQuery).Session:=FSessioneOracle
    else if Components[i] is TOracleDataSet then
      (Components[i] as TOracleDataSet).Session:=FSessioneOracle;
  end;
end;
...
function TC021FDocumentiManagerDM.PreparaZipFile(var pZipFile: string): TResCtrl;
//Definisce il nome del file zip e apre FZip nella modalità corretta
begin
  Result.Ok:=False;
  try
    // determina il nome completo del file da generare
    if pZipFile = '' then
    begin
      pZipFile:=TPath.Combine(R180GetOSTempDir, 'Allegati.zip');
      {$IFDEF IRISWEB}{$IFNDEF WEBSVC}
      pZipFile:=TPath.Combine(GGetWebApplicationThreadVar.UserCacheDir, 'Allegati.zip');
      {$ENDIF}{$ENDIF}

      // cancella eventuale file già esistente
      if TFile.Exists(pZipFile) then
        TFile.Delete(pZipFile);
    end;
    ...
    if TFile.Exists(pZipFile) then
      FZip.Open(pZipFile, zmReadWrite)
    else
      FZip.Open(pZipFile, zmWrite);

  except
    on E: Exception do
    begin
      Result.Messaggio:=Format('Errore in creazione del file compresso %s:'#13#10'%s',[pZipFile, E.Message]);
      Exit;
    end;
  end;
...
function TC021FDocumentiManagerDM.AddAllegatoByIdToZip(pId: Integer; pRadiceNomeDoc: string; var pZipFile: string): TResCtrl;
//Carica il documento nel file .zip
//NB deve essere chiamata la procedura di chiusura quando è finito il caricamento
var
  LFileBase64: String;
  RDoc: TDocumento;
  NomeFile: string;
begin
  ...
  Result:=GetByID(pId, True, RDoc, False);
  if not Result.Ok then
    Exit;

  Result:=PreparaZipFile(pZipFile);
  if not Result.Ok then
    Exit;

  try
    //Compone il nome del file caricato nello zip
    NomeFile:= Format('%s_%s'+(*_%s*)'.%s', [pRadiceNomeDoc, pId.ToString, (*RDoc.NomeFile,*) RDoc.ExtFile]); //Escluso il nome del file originale come da richiesta PIACENZA_COMUNE - 2021/011 SVILUPPO#2

    //salva il documento in base al tipo di storage
    if (RDoc.PathStorage = DOC_STORAGE_DB) and not ((RDoc.Blob = nil) or (RDoc.Blob.IsNull)) then
      FZip.Add(RDoc.Blob, NomeFile)
    else
    begin
      FZip.Add(RDoc.FileBase64, NomeFile)
    end;

  except
    on E: Exception do
    begin
      Result.Messaggio:=Format('Errore in fase di salvataggio del file compresso (ID %d, storage: %s):'#13#10'%s',[RDoc.Id,IfThen(RDoc.PathStorage = DOC_STORAGE_DB,'database','file system'),E.Message]);
      Exit;
    end;
  end;

  // operazione ok
  Result.Messaggio:=NomeFile;
  Result.Ok:=True;
end;

function TC021FDocumentiManagerDM.AddFileToZip(pData: TStream; pNomeFile: string; var pZipFile: string): TResCtrl;
begin
  Result:=PreparaZipFile(pZipFile);
  if not Result.Ok then
    Exit;
  try
    pData.Position:=0;
    FZip.Add(pData, pNomeFile);
  except
    on E: Exception do
    begin
      Result.Messaggio:=Format('Errore in fase di salvataggio del file compresso (file: %s):'#13#10'%s',[pNomeFile,E.Message]);
      Exit;
    end;
  end;
...
procedure TC021FDocumentiManagerDM.CloseZip;
begin
  if Assigned(FZip) then
  begin
    FZip.Close;
...
function TC021FDocumentiManagerDM.StringtoBase64Url(Value: String): String;
begin
  Result:=StringReplace(EncodeString(Value), '=', '', [rfReplaceAll]);
