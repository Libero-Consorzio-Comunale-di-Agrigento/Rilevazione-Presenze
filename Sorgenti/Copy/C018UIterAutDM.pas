...
uses
  ...
  , meIWCheckBox, medpIWDBGrid, meIWImageFile, C190FunzioniGeneraliWeb, IWApplication
  {$ENDIF} {$ENDIF}
...
type
  ...
  TC018TipoChekAutor = class(TPersistent)
  public
    Tipo:String;
  end;
  ...
  // eccezioni
  EC018RichiestaNonValidaException = Exception;

  // tipologia iter
  TC018TipoIter = (tiNone,tiRichiesta,tiAutorizzazione);
  // tipo richiesta
  TTipoRichiesta = (trRichiesta, trAutorizzazione);
  ...
  TTipoRichiesteDatiArr = array [0..23] of TTipoRichiesteDati;
...
const
  ...
  TIPORICHIESTE_DATI: TTipoRichiesteDatiArr = (
    ...
    (TipoRich:trDaDefinire;        Nome:'chkDaDefinire';        Caption:'da definire';             FiltroResp:'';                                             FiltroDip:'T_ITER.ELABORATO = ''N'' and T850.TIPO_RICHIESTA = ''P'' and decode(T851P.STATO,''N'',''N'',''S'') = ''S'' and (decode(T850R.STATO,''N'',null,T850.ID_REVOCA) is null or T850R.STATO = ''N'')'; FiltroClientResp:''; FiltroClientDip:''),
    ...
    (TipoRich:trRevocate;          Nome:'chkRevocate';          Caption:'revocate';                FiltroResp:'(T_ITER.ELABORATO = ''N'') and (T850.ID_REVOCATO is not null or decode(T850R.STATO,''N'',null,T850.ID_REVOCA) is not null)'; FiltroDip:'(T_ITER.ELABORATO = ''N'') and (T850.ID_REVOCATO is not null or decode(T850R.STATO,''N'',null,T850.ID_REVOCA) is not null)'; FiltroClientResp:''; FiltroClientDip:''),
    ...
    (TipoRich:trDaAutScadute;      Nome:'chkDaAutScadute';      Caption:'da aut scadute';          FiltroResp:'T_ITER.DATA < TRUNC(SYSDATE)';                 FiltroDip:'T_ITER.DATA < TRUNC(SYSDATE)';      FiltroClientResp:''; FiltroClientDip:'')
  );
...
type
  ...
  TC018FIterAutDM = class(TR005FDataModuleMW)
    ...
    scrUSR_PCK_ITER_SPECIALISTI: TOracleQuery;
    selSG101: TOracleDataSet;
    selT230: TOracleDataSet;
    updT100Flag: TOracleQuery;
  ...
  private
    ...
    {$ENDIF}
    // R013.fine
    procedure ImpostaVariabiliExprRichiesta(PLivello:integer = 0; PStato:string = ''; PResponsabile:string = ''; PAutorizzAutomatica:string = '');
    ...
    function  ScorriCondizValidita(CondizioniBloccanti:Boolean; PLivello:integer = 0; PStato:string = ''; PResponsabile:string = ''; PAutorizzAutomatica:string = ''):Boolean;
    ...
    function  GetLivelliObblFase(const PFase: Integer): TLivelliFase;
    function  GetLivelloObbligatorio(const PLivello: Integer):Boolean;
    ...
    procedure ImpostaFiltriRichiestaPerIter;
    {$ENDIF}
    {$ENDIF}
    // R013.fine
  public
    ...
    NotificaAttivita:Boolean;
    selTabellaIter:TDataSet;
    Chiamante: String;
    PosponiInvioMail: Boolean;
    ...
    {$IF CompilerVersion >= 31}
    {versione 10.1 Berlin+}
    function  LeggiNote(var RNote: TNoteRichiesta; var RErrMsg: String): Boolean;
    {$ENDIF}
    function  LeggiNoteEsistenti(PFormatoHTML:Boolean = False):String;
    ...
    procedure SetValoriAut(grd: TmedpIWDBGrid; rowGrd,rowComp,idxS,idxN: Integer; chkAutorizzazioneClick: TprocObject);
    {$ENDIF}
    {$IFNDEF _WEBSVC_}
    ...
    function  ValiditaRichiesta(const PTipoRichiesta: String = ''):Boolean;
    ...
    property CondizioniValiditaOk: Boolean read FCondizioniValiditaOk;
    ...
    property LivelliObblFase[const PFase: Integer]: TLivelliFase read GetLivelliObblFase;
    property LivelloObbligatorio[const PLivello: Integer]: Boolean read GetLivelloObbligatorio;
    property Revocabile:Boolean read FRevocabile;
    property MsgNotifica: String read FMsgNotifica;
    ...
    property ColonnaPeriodoInizio: String read GetColonnaPeriodoInizio;
    property ColonnaPeriodoFine: String read GetColonnaPeriodoFine;
    // R013.ini
    // solo per irisweb
    {$IFNDEF _IRISWEB_}
    {$IFNDEF _WEBSVC_}
...
begin
  try
    ...
    begin
      ...
      Result:=A000TraduzioneStringhe(A000MSG_MSG_FINOAL) + ' ' + DateToStr(Fine)
    else if Fine = DATE_MAX then
      Result:=A000TraduzioneStringhe(A000MSG_MSG_DAL) + ' ' + DateToStr(Inizio)
    else
    begin
      AnnoIni:=R180Anno(Inizio);
      AnnoFine:=R180Anno(Fine);
      if (Inizio = R180InizioMese(Inizio)) and (Fine = R180FineMese(Inizio)) then
        Result:=A000TraduzioneStringhe(A000MSG_MSG_MESE) + ' ' + FormatDateTime('mmmm yyyy',Inizio)
      else if (AnnoIni = AnnoFine) and (Inizio = EncodeDate(AnnoIni,1,1)) and (Fine = EncodeDate(AnnoIni,12,31)) then
        Result:=Format('%s %d',[A000TraduzioneStringhe(A000MSG_MSG_ANNO),AnnoIni])
      else
        Result:=Format(A000TraduzioneStringhe(A000MSG_MSG_FMT_DALAL),[DateToStr(Inizio),DateToStr(Fine)]);
    end;
  except
    on E: Exception do
      Result:=E.Message;
...
begin
  ...
  NotificaAttivita:=False;
  PosponiInvioMail:=False;
...
begin
  ...
  try
    ...
    if DelDatiDB and R180In(Self.Iter,[ITER_GIUSTIF,ITER_TIMBR,ITER_MISSIONI,ITER_STRGIORNO]) then
    begin
      ...
      //ITER_TIMBR: imposto il filtro in modo che sia eliminata solo la timbratura modificata (PALERMO_ARPA - 157021)
      delDBDato.SetVariable('FILTRO',IfThen(Self.Iter = ITER_TIMBR,' AND FLAG = ''I'''));
      delDBDato.Execute;
      if Self.Iter = ITER_TIMBR then
      begin
        //Ripristino la timbratura originale (PALERMO_ARPA - 157021)
        updT100Flag.SetVariable('ID',Self.ID);
        updT100Flag.Execute;
      end;
    ...
    if (Self.Iter = ITER_STRGIORNO) and (Livello <= 1) then
    begin
      //Aggiornamento tipo richiesta su T850
      SetTipoRichiesta('R');
    end;
...
begin
  ...
  try
    try
      ...
      if selIterGenerico.VariableIndex('DataLavoro') >= 0 then
      ...
      if DelDatiDB and R180In(FIter,[ITER_GIUSTIF,ITER_TIMBR,ITER_MISSIONI,ITER_STRGIORNO]) then
      begin
        //Cancellazione dato inserito su tabella
        delDBDato.SetVariable('TABELLA_ITER',GetTabellaDB.NomeTabella);
        delDBDato.SetVariable('NOME_DATO',GetTabellaDB.NomeDatoID);
...
begin
  ...
  begin
    ...
    begin
      LEsisteOrderBy:=True;
      PDataset.SQL.Insert(i, Format('and T_ITER.%s = %d',[GetIDIter,Self.Id]));
      Break;
    end;
  end;
  if not LEsisteOrderBy then
    PDataset.SQL.Add(Format('and T_ITER.%s = %d',[GetIDIter,Self.Id]));
end;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          else if (Iter = ITER_STRGIORNO) (*and (LivelloElab >= LivMaxObb) *) then //come da W026 il TipoRichiesta cambia a qualsiasi livello
          begin
            SetTipoRichiesta('E');
          end;
        end;
...
begin
  ...
  else if Iter = ITER_CERTIFICAZIONI then
    Result:='VSG230_ITER_CERTIFICAZIONI'
  else if Iter = ITER_DOCUMENTALE then
    Result:='T960_DOCUMENTI_INFO'
  else if Iter = ITER_BUDGET_STRAORD then
    Result:='T700_ITER_BUDGETANNO'
  else if Iter = ITER_DISPTURNI then
    Result:='T600_RICHIESTEDISPTURNI'
  else
...
begin
  ...
  begin
    Result.NomeTabella:='T960_DOCUMENTI_INFO';
    Result.NomeDatoID:='ID_RICHIESTA';
  end
  else if Iter = ITER_STRGIORNO then
  begin
    Result.NomeTabella:='T320_PIANLIBPROFESSIONE';
    Result.NomeDatoID:='ID_RICHIESTA';
  end;
end;
...
begin
  if Iter = ITER_MISSIONI then
    Result:='T_ITER.ROWID,T_ITER.ID,T_ITER.PROGRESSIVO,T_ITER.FLAG_DESTINAZIONE,T_ITER.FLAG_ISPETTIVA,T_ITER.DATADA,T_ITER.DATAA,T_ITER.ORADA,T_ITER.ORAA,' +
            ...
            'M140F_GETPARTENZA(T_ITER.ID) PARTENZA,M140F_GETDESTINAZIONI(T_ITER.ID) ELENCO_DESTINAZIONI,M140F_GETRIENTRO(T_ITER.ID) RIENTRO,T_ITER.MISSIONE_RIAPERTA, T_ITER.CAPITOLO_TRASFERTA'
  else if Iter = ITER_GIUSTIF then
    Result:='T_ITER.ROWID,T_ITER.ID,T_ITER.PROGRESSIVO,T_ITER.CAUSALE,T_ITER.TIPOGIUST,T_ITER.DAL,T_ITER.AL,T_ITER.NUMEROORE,T_ITER.DATANAS,T_ITER.AORE,T_ITER.ELABORATO,T_ITER.NUMEROORE_PREV,T_ITER.AORE_PREV,T_ITER.MOTIVAZIONE,T_ITER.NOTE_GIUSTIFICATIVO'
  else if Iter = ITER_STRMESE then
    Result:='T_ITER.ROWID,T_ITER.ID,T_ITER.PROGRESSIVO,T_ITER.DATA,T_ITER.ID_CONGUAGLIO,T_ITER.TIPO,T_ITER.ORE_ECCED_CALC,T_ITER.ORE_ECCEDENTI,T_ITER.ORE_DACOMPENSARE,T_ITER.ORE_DALIQUIDARE,T_ITER.CAUSALE,T_ITER.ORE_CAUSALIZZATE,' +
            'T_ITER.ORE_CAUSALIZZATE_DALIQ,T_ITER.ORE_CAUSALIZZATE_DACOMP,T_ITER.MIN_ORE_DALIQUIDARE,T_ITER.MIN_ORE_DACOMPENSARE,T_ITER.MAX_ORE_DALIQUIDARE,T_ITER.MAX_ORE_CAUSALIZZATE_DALIQ'
  else if Iter = ITER_ORARIGG then
    ...
    Result:='T_ITER.ROWID,T_ITER.ID,T_ITER.PROGRESSIVO,T_ITER.DATA,T_ITER.ORA,T_ITER.VERSO,T_ITER.CAUSALE,MEDPF_GETCAUSALEDESC(''T275,T305'',T_ITER.CAUSALE) D_CAUSALE,' +
            'T_ITER.OPERAZIONE,T_ITER.ELABORATO,T_ITER.CAUSALE_ORIG,MEDPF_GETCAUSALEDESC(''T275,T305'',T_ITER.CAUSALE_ORIG) D_CAUSALE_ORIG,T_ITER.VERSO_ORIG,T_ITER.MOTIVAZIONE,T_ITER.RILEVATORE_RICH,T_ITER.RILEVATORE_ORIG'
  else if Iter = ITER_STRGIORNO then
    Result:='T_ITER.*'
  else if Iter = ITER_CARTELLINO then
    ...
    Result:='T_ITER.ROWID_T,T_ITER.*' //T_ITER.ROWID sostituito volutamente con T_ITER.ROWID_T
  else if Iter = ITER_DOCUMENTALE then
    Result:='T_ITER.ROWID,T_ITER.*'
  else if Iter = ITER_BUDGET_STRAORD then
    Result:='T_ITER.ROWID,T_ITER.*'
...
begin
  if Iter = ITER_MISSIONI then
    ...
    Result:='T_ITER.DATA_CREAZIONE'
  else if Iter = ITER_BUDGET_STRAORD then
    Result:='T_ITER.DECORRENZA'
  else if Iter = ITER_DISPTURNI then
    Result:='T_ITER.DATA'
  else
...
begin
  ...
  if Iter = ITER_MISSIONI then
    ...
    Result:='T960PERIODO_AL'
  else if Iter = ITER_BUDGET_STRAORD then
    Result:='T700DECORRENZA'
  else if Iter = ITER_DISPTURNI then
    Result:='T600DATA'
  else
...
begin
  ...
  if Iter = ITER_MISSIONI then
    ...
    Result:='T_ITER.AL'  //'T850.DATA'
  else if Iter = ITER_DOCUMENTALE then
    Result:='T_ITER.DATA_CREAZIONE'
  else if Iter = ITER_BUDGET_STRAORD then
    Result:='T_ITER.DECORRENZA_FINE'
  else if Iter = ITER_DISPTURNI then
    Result:='T_ITER.DATA'
  else
...
begin
  ...
  if Iter = ITER_MISSIONI then
    ...
    Result:='T_ITER.ID DESC'
  else if Iter = ITER_DOCUMENTALE then
    Result:='T_ITER.DATA_CREAZIONE DESC'
  else if Iter = ITER_BUDGET_STRAORD then
    Result:='T_ITER.DECORRENZA DESC, T_ITER.TIPO, T_ITER.CODGRUPPO'
  else if Iter = ITER_DISPTURNI then
    Result:='T_ITER.DATA DESC, T_ITER.DAORE'
  else
...
begin
  ...
  selT002_Richiesta.Open;
  selT002_Richiesta.First;
  while not selT002_Richiesta.Eof do
  begin
    Result:=Result + selT002_Richiesta.FieldByName('RIGA').AsString + #13#10;
    selT002_Richiesta.Next;
  end;
  ...
  begin
    Result:='select /* LEADING(T030A) */' + #13#10 +
            ...
            '  ' + ColonneIter + ',' + #13#10 +
            IfThen(Iter <> ITER_BUDGET_STRAORD,'  T030A.MATRICOLA, T030A.COGNOME || '' '' || T030A.NOME NOMINATIVO, T030A.SESSO,') + #13#10 +
            {if Iter <> ITER_BUDGET_STRAORD then
              Result:=Result + '  T030A.MATRICOLA, T030A.COGNOME || '' '' || T030A.NOME NOMINATIVO, T030A.SESSO,';
            Result:=Result + #13#10 +}
            '  T850.STATO AUTORIZZAZIONE,T850.DATA DATA_RICHIESTA,T850.TIPO_RICHIESTA,' + #13#10 +
            '  to_number(decode(T850R.STATO,''N'',null,T850.ID_REVOCA)) ID_REVOCA,' + #13#10 +
            '  T850.ID_REVOCATO,T850.AUTORIZZ_AUTOMATICA,T850.COD_ITER,' + #13#10 +
            ...
            '  null MSGAUT_SI, null MSGAUT_NO, '#13#10 + // in fase di richiesta non servono questi dati
            ...
            IfThen(Iter <> ITER_BUDGET_STRAORD, ',' + #13#10 +
                                                '  T030_ANAGRAFICO T030A' + #13#10 +
                                                'where exists (select /* UNNEST */ T030.PROGRESSIVO from' + #13#10 +
                                                '              ' + QVistaOracle + #13#10 +
                                                '              :FILTRO_ANAG' + #13#10 +
            ...
            'and '
              ...
              Result:=Result + #13#10 + 'where' + #13#10;

            Result:=Result + }
            'T850.ITER = :ITER and T850.ID = T_ITER.' + GetIDIter + #13#10 +
            IfThen(Chiamante = 'C018','and nvl(T850.COD_ITER,''null'') <> ''-ASSEGNATO-''' + #13#10) +
     ...
     // TORINO_ITC.ini
     // ordinamento spostato in R013Open
            ':ORDINAMENTO '#13#10;
     //     'order by' + #13#10 +
...
begin
  ...
  selT002_Autorizzazione.Open;
  selT002_Autorizzazione.First;
  while not selT002_Autorizzazione.Eof do
  begin
    Result:=Result + selT002_Autorizzazione.FieldByName('RIGA').AsString + #13#10;
    selT002_Autorizzazione.Next;
  end;

  // indica il caso della validazione cartellino lato dipendente
  LCartellinoDip:=(Iter = ITER_CARTELLINO) and Parametri.InibizioneIndividuale;

  // se non esiste una ridefinizione specifica utilizza la query standard
  if Result = '' then
  begin
    if Parametri.CampiRiferimento.C26_C018_Hint = 'ORDERED' then
      Result:='select /*+ ORDERED */' + #13#10
    else if not NotificaAttivita then
      Result:='select /* LEADING(T030A) */' + #13#10
    else
      Result:='select /* LEADING(T_ITER) */' + #13#10;
    Result:=Result +
            '  ' + ColonneIter + ',' + #13#10;
    if (not NotificaAttivita) and (Iter <> ITER_BUDGET_STRAORD) then
      Result:=Result +
            '  T030A.MATRICOLA, T030A.COGNOME || '' '' || T030A.NOME NOMINATIVO, T030A.SESSO,' + #13#10
    else
      Result:=Result +
            '  ''MAT'' MATRICOLA, ''NOMINATIVO'' NOMINATIVO, ''S'' SESSO,' + #13#10;
    Result:=Result +
            '  T850.COD_ITER, T850.DATA DATA_RICHIESTA, T850.STATO AUTORIZZAZIONE, T850.TIPO_RICHIESTA,' + #13#10 +
            '  to_number(decode(T850R.STATO,''N'',null,T850.ID_REVOCA)) ID_REVOCA,' + #13#10 +
            '  T850.ID_REVOCATO,' + #13#10 +
            ...
            '  T851F_MSGAUT_SI(T850.ID, :ITER) MSGAUT_SI, T851F_MSGAUT_NO(T850.ID, :ITER) MSGAUT_NO, '#13#10 +
            '  I060F_NOMINATIVO(:AZIENDA, T851.RESPONSABILE) NOMINATIVO_RESP,' + #13#10 +
            IfThen(LCartellinoDip,
    ...
    begin
      Result:=Result +
              ...
              '              ' + QVistaOracle + #13#10 +
              '              :FILTRO_ANAG' + #13#10;
      if not NotificaAttivita then
      begin
        Result:=Result +
              //'              and T030A.PROGRESSIVO = T030.PROGRESSIVO)' + #13#10 +
              '              and T_ITER.PROGRESSIVO = T030.PROGRESSIVO)' + #13#10 +
              'and T_ITER.PROGRESSIVO = T030A.PROGRESSIVO' + #13#10;
      end
      else
        Result:=Result +
      ...
      Result:=Result + 'where' + #13#10;

    Result:=Result +
            'T850.ITER = :ITER and T850.ID = T_ITER.' + GetIDIter + #13#10 +
            IfThen(Chiamante = 'C018','and nvl(T850.COD_ITER,''null'') <> ''-ASSEGNATO-''' + #13#10) +
            IfThen(R180In(Iter,[ITER_CERTIFICAZIONI,ITER_DOCUMENTALE]),'and T850.TIPO_RICHIESTA = ''D''' + #13#10) +
            'and T850R.ITER(+) = :ITER and T850R.ID(+) = T850.ID_REVOCA' + #13#10 +
    ...
    //if not NotificaAttivita then
    //begin
    //  LOrderBy:=Format('  NOMINATIVO, T030A.MATRICOLA, %s, T850.DATA DESC',[OrderByIter]);
    //  Result:=Result + Format('order by'#13#10'%s',[LOrderBy]);
    //end;
    // TORINO_ITC.fine
  end;
...
var
  wProfiloWebIterAutorizzativi:String;
  LDS: TOracleDataSet;
begin
  ...
  begin
    LDS:=(PDataSet as TOracleDataSet);
    LDS.ClearVariables;
    LDS.DeleteVariables;
    LDS.SQL.Clear;
    if PTipoIter = tiRichiesta then
    begin
      // query per dipendente
      LDS.SQL.Text:=SQLRichiestaIter;
    end
    else if PTipoIter = tiAutorizzazione then
    begin
      // query per autorizzatore
      LDS.SQL.Text:=SQLAutorizzazioneIter;

      // gestione particolare per iter cartellino
      if (Iter = ITER_CARTELLINO) and Parametri.InibizioneIndividuale and (Parametri.ProfiloWEBIterAutorizzativi = '') then
        wProfiloWebIterAutorizzativi:=T860ITER_DEFAULT
      else
        wProfiloWebIterAutorizzativi:=Parametri.ProfiloWebIterAutorizzativi;

      // se non è previsto un profilo iter significa che non sono definite le autorizzazioni
      // ai vari livelli delle strutture, per cui l'applicativo non può operare
      if wProfiloWebIterAutorizzativi = '' then
      ...
      // imposta profilo iter
      LDS.DeclareVariable('PROFILO',otString);
      LDS.SetVariable('PROFILO',wProfiloWebIterAutorizzativi);
    end;
    LDS.DeclareVariable('AZIENDA',otString);
    LDS.DeclareVariable('ITER',otString);
    if Pos(':DATALAVORO',UpperCase(LDS.SQL.Text)) > 0 then
      LDS.DeclareVariable('DATALAVORO',otDate);
    LDS.DeclareVariable('FILTRO_PERIODO',otSubst);
    LDS.DeclareVariable('FILTRO_VISUALIZZAZIONE',otSubst);
    LDS.DeclareVariable('FILTRO_STRUTTURA',otSubst);
    LDS.DeclareVariable('FILTRO_ALLEGATI',otSubst);
    if Pos(':FILTRO_ANAG',UpperCase(LDS.SQL.Text)) > 0 then
      LDS.DeclareVariable('FILTRO_ANAG',otSubst);
    if Pos(':FILTRO_PROGRESSIVI',UpperCase(LDS.SQL.Text)) > 0 then
      LDS.DeclareVariable('FILTRO_PROGRESSIVI',otSubst);
    // TORINO_ITC.ini
    // l'ordinamento è gestito in R013
    LDS.DeclareVariable(':ORDINAMENTO',otSubst);
    LDS.SetVariable('AZIENDA',Parametri.Azienda);
    LDS.SetVariable('ITER',Iter);
    ...
    begin
      ...
      if Parametri.CampiRiferimento.C26_C018_Hint = 'LEADING' then
        LDS.SQL.Text:=StringReplace(LDS.SQL.Text,'/* LEADING','/*+ LEADING',[]);
      //if (Pos(INI_PAR_C018_NO_UNNEST,W000ParConfig.ParametriAvanzati) = 0) then
      if Parametri.CampiRiferimento.C26_C018_Unnest = 'S' then
        LDS.SQL.Text:=StringReplace(LDS.SQL.Text,'/* UNNEST */','/*+ UNNEST */',[]);
    end
    else
    begin
    ...
    begin
      LDS.SQL.Text:=StringReplace(LDS.SQL.Text,'/* UNNEST */',Parametri.CampiRiferimento.C26_HintT030V430,[]);
      // chiamata <74701>.ini
      ...
      // chiamata <74701>.fine
        LDS.SQL.Text:=StringReplace(LDS.SQL.Text,'/*+ UNNEST */',Format('/*+ %s UNNEST */',[StringReplace(StringReplace(Parametri.CampiRiferimento.C26_HintT030V430,'/*+','',[]),'*/','',[])]),[]);
    end;
    {$ENDIF}
    {$ENDIF}
...
var
  ...
  wProfiloWebIterAutorizativi:String;
  // R013.ini
  {$IFNDEF _IRISWEB_}
  {$IFNDEF _WEBSVC_3}
...
begin
  ...
  begin
    ...
    begin
      FDescIter:=A000TraduzioneStringhe(A000IterAutorizzativi[i].Desc);
      Break;
    end;
  end;
  FRevocabile:=False;
  ...
  selI075.Open;
  FTipoRichiesta:='';
  FCondizioniValiditaOk:=False;
  FNote:='';
  FRevocabile:=selI093.FieldByName('REVOCABILE').AsString = 'S';
  FMsgNotifica:=selI093.FieldByName('MSG_NOTIFICA').AsString;
  ...
  begin
    ...
    // EMPOLI_ASL11 - commessa 2014/054 SVILUPPO#2.ini
    // gestione allegati
    if (R180In(Iter,[ITER_GIUSTIF,ITER_MISSIONI,ITER_CERTIFICAZIONI])) and // disponibile al momento solo su giustificativi e missioni
       (Parametri.ModuloInstallato['ITER_AUTORIZZATIVI']) and
  ...
  selMailPerAutorizzatore.SetVariable('COLONNA_ID',GetIDIter);
  ...
  selMailPerRichiedente.SetVariable('COLONNA_ID',GetIDIter);
...
{$IFNDEF _IRISWEB_}
{$IFNDEF _WEBSVC_}
procedure TC018FIterAutDM.ImpostaFiltriRichiestaPerIter;
// imposta i filtri richiesta in modo specifico per ogni iter
begin
  // PRE:
  //   - Iter impostato
  //   - Responsabile impostato
  if Iter = ITER_GIUSTIF then
  begin
    ...
    begin
      ...
      begin
        // le richieste da autorizzare considerano anche quelle autorizzate ma non ancora elaborate
        //FiltroRichiesta[trDaAutorizzare]:=Format('(%s) or (T_ITER.ELABORATO = ''N'' and (T851.STATO = ''N'' or T851.LIVELLO = I096F_ULTIMOLIV_OBB(:ITER, T850.COD_ITER)))',
        FiltroRichiesta[trDaAutorizzare]:=Format('(%s) or (T851F_LIVELLO_VISIBILE(T850.ITER, T850.COD_ITER, T851.LIVELLO, T851.STATO, T851.AUTORIZZ_AUTOMATICA, T851.ID, T_ITER.ELABORATO) = ''S'')',
                                                 [FiltroRichiesta[trDaAutorizzare]]);
        // le richieste autorizzate / negate considerano SOLO quelle già elaborate
        FiltroRichiesta[trAutorizzate]:=Format('(%s) and (T_ITER.ELABORATO <> ''N'' or T851.LIVELLO <> I096F_ULTIMOLIV_OBB(:ITER, T850.COD_ITER))',
                                               [FiltroRichiesta[trAutorizzate]]);
        FiltroRichiesta[trNegate]:=Format('(%s) and (T_ITER.ELABORATO <> ''N'' or T851.LIVELLO <> I096F_ULTIMOLIV_OBB(:ITER, T850.COD_ITER))',
                                          [FiltroRichiesta[trNegate]]);
      end
      else
      begin
        ...
        FiltroRichiesta[trDaAutorizzare]:=Format('(%s) and (T_ITER.ELABORATO = ''N'')',
                                                 [FiltroRichiesta[trDaAutorizzare]]);
    ...
    begin
      // richiesta
      FiltroRichiesta[trDaAutorizzare]:='decode(T850.TIPO_RICHIESTA,''P'',T851P.STATO,T850.STATO) is null';
    end;
  end
  else if Iter = ITER_TIMBR then
  begin
  ...
  begin
    ...
    begin
      FiltroRichiesta[trDaEffettuare]:=Format('T850.TIPO_RICHIESTA = ''%s''',[T325TR_I]);
    end;

    FiltroRichiesta[trAutorizzate]:=Format('T850.TIPO_RICHIESTA = ''%s''',[T325TR_E]);
    if Parametri.CampiRiferimento.C90_W026ConfermaAutorizzazioni = 'S' then
    begin
      // da autorizzare: tipo richiesta R
      FiltroRichiesta[trDaAutorizzare]:=Format('T850.TIPO_RICHIESTA = ''%s''',[T325TR_R]);
    end
    else
    begin
      FiltroRichiesta[trDaAutorizzare]:=Format('T850.TIPO_RICHIESTA = ''%s'' and T850.STATO is null',[T325TR_R]);
      if Responsabile and (Parametri.CampiRiferimento.C90_W026Spezzoni = 'T') then
        FiltroRichiesta[trAutorizzate]:=FiltroRichiesta[trAutorizzate] + ' and T850.STATO = ''' + C018SI + '''';
    end;

    // ridefinizione filtri clientdataset
    FiltroRichiestaClient[trAutorizzate]:=Format('TIPO_RICHIESTA = ''%s''',[T325TR_E]);
    if Parametri.CampiRiferimento.C90_W026ConfermaAutorizzazioni = 'S' then
    begin
      FiltroRichiestaClient[trDaAutorizzare]:=Format('TIPO_RICHIESTA = ''%s''',[T325TR_R]);
    end
    else
    begin
      FiltroRichiestaClient[trDaAutorizzare]:=Format('TIPO_RICHIESTA = ''%s'' and AUTORIZZAZIONE is null',[T325TR_R]);
      if Responsabile and (Parametri.CampiRiferimento.C90_W026Spezzoni = 'T') then
        FiltroRichiestaClient[trAutorizzate]:=FiltroRichiestaClient[trAutorizzate] + ' and AUTORIZZAZIONE = ''' + C018SI + '''';
    end;
  end
  else if Iter = ITER_MISSIONI then
  begin
    ...
    begin
      ...
      FiltroRichiesta[trDaAutorizzare]:=Format('(nvl(T850.STATO,''X'') <> ''%s'') and (T851F_FASE_CORRENTE(''%s'',''%s'',T850.COD_ITER,T850.ID) in (%d,%d))',
                                               [C018NO,Parametri.Azienda,Iter,M140FASE_INIZIALE,M140FASE_CASSA]);
      // AOSTA_REGIONE - chiamata 91877.fine
      FiltroRichiesta[trAutorizzate]:=Format('T851F_FASE_CORRENTE(''%s'',''%s'',T850.COD_ITER,T850.ID) not in (%d,%d)',
                                             [Parametri.Azienda,Iter,M140FASE_INIZIALE,M140FASE_CASSA]);
    end;
  end
  else if Iter = ITER_CARTELLINO then
  begin
  ...
  begin
    // le richieste scadute considerano solo quelle passate ma ancora da autorizzare
    FiltroRichiesta[trDaAutScadute]:=Format('(%s) and (%s)',
                                           [FiltroRichiesta[trDaAutorizzare],FiltroRichiesta[trDaAutScadute]]);

    // le richieste da autorizzare considerano solo quelle future
    FiltroRichiesta[trDaAutorizzare]:=Format('(%s) and NOT (%s)',
                                             [FiltroRichiesta[trDaAutorizzare],FiltroRichiesta[trDaAutScadute]]);
  end
  else
    raise Exception.Create(Format('ImpostaFiltriRichiestaPerIter fallito: il codice iter %s non è gestito!',[Iter]));
end;
{$ENDIF !_WEBSVC_}
{$ENDIF IRISWEB}
...
begin
  ...
  begin
    ...
    begin
      MailOggetto:='''Certificazione per il giorno ''||to_char(T850.DATA,''dd/mm/yyyy'')'': ''||T030.COGNOME||'' ''||T030.NOME||'' (''||T030.MATRICOLA||'')''';
      MailCorpo:='null';
    end
    else if Iter = ITER_DOCUMENTALE then
    begin
      MailOggetto:='''Richiesta di inserimento su documentale''';
      MailCorpo:='null';
    end
    else if Iter = ITER_BUDGET_STRAORD then
    begin
      MailOggetto:='''Richiesta di modifica del budget annuale''';
      MailCorpo:='null';
    end
    else if Iter = ITER_DISPTURNI then
    begin
      MailOggetto:='''Inserimento di disponibilità turni''';
      MailCorpo:='null';
    end
    else
  ...
  begin
    ...
    begin
      ...
      MailCorpo:='''Si avvisa che la richiesta di seguito riportata e'''' stata ''||decode(T851.STATO,''N'',''respinta'',''concessa'')||chr(13)||chr(10)||' +
                 ...
                 '''Loc. partenza:  ''||M041F_GETDESCLOCALITA(M140F_GETPARTENZA(T_ITER.ID))||chr(13)||chr(10)||' +
                 '''Loc. destinaz.: ''||M140F_GETDESTINAZIONI_DESC(T_ITER.ID)||chr(13)||chr(10)||' +
                 '''Loc. rientro:   ''||M041F_GETDESCLOCALITA(M140F_GETRIENTRO(T_ITER.ID))||chr(13)||chr(10)';
    end
    else if Iter = ITER_CARTELLINO then
    begin
    ...
    begin
      MailOggetto:='''Autorizzazione ''||decode(T851.STATO,''N'',''respinta'',''concessa'')||'' per richiesta di certificazione: ''||' +
                   'T030.COGNOME||'' ''||T030.NOME||'' (''||T030.MATRICOLA||'')''';
      MailCorpo:='null';
    end
    else if Iter = ITER_DOCUMENTALE then
    ...
    begin
      MailOggetto:='''Autorizzazione ''||decode(T851.STATO,''N'',''respinta'',''concessa'')||'' per inserimento di disponibilità turni: ''||' +
                   'T030.COGNOME||'' ''||T030.NOME||'' (''||T030.MATRICOLA||'')''';
      MailCorpo:='null';
    end
    else
...
begin
  ...
  begin
    ...
    { In caso di C1_CedoliniConValuta = 'S' (TORINO_ITC) abbiamo un iter autorizzativo
      che rende trDaAutFinale e trAutFinale di fatto uguali a trDaAutorizzare e trDaAutorizzare
      a livello di filtro. }
    if (Parametri.InibizioneIndividuale) and
       (Parametri.CampiRiferimento.C1_CedoliniConValuta <> 'S') then
...
// restituisce i livelli min e max per la fase indicata
// PRE: CodIter impostato
var
  Liv: Integer;
begin
  Result.Min:=99;
  Result.Max:=0;
  if selI096.SearchRecord('COD_ITER;FASE',VarArrayOf([CodIter,PFase]),[srFromBeginning]) then
  repeat
    ...
    if Liv > Result.Max then
      Result.Max:=Liv;
  until not selI096.SearchRecord('COD_ITER;FASE',VarArrayOf([CodIter,PFase]),[]);
  (*
  selI096.First;
...
function TC018FIterAutDM.GetLivelliObblFase(const PFase: Integer): TLivelliFase;
// restituisce i livelli obbligatori  min e max per la fase indicata
// PRE: CodIter impostato
var
  Liv: Integer;
begin
  Result.Min:=99;
  Result.Max:=0;
  if selI096.SearchRecord('COD_ITER;FASE;OBBLIGATORIO',VarArrayOf([CodIter,PFase,'S']),[srFromBeginning]) then
  repeat
    Liv:=selI096.FieldByName('LIVELLO').AsInteger;
    if Liv < Result.Min then
      Result.Min:=Liv;
    if Liv > Result.Max then
      Result.Max:=Liv;
  until not selI096.SearchRecord('COD_ITER;FASE;OBBLIGATORIO',VarArrayOf([CodIter,PFase,'S']),[]);
  (*
  selI096.First;
  while selI096.SearchRecord('COD_ITER;FASE;OBBLIGATORIO',VararrayOf([CodIter,PFase,'S']),[srFromCurrent]) do
  begin
    Liv:=selI096.FieldByName('LIVELLO').AsInteger;
    if Liv < Result.Min then
      Result.Min:=Liv;
...
function TC018FIterAutDM.GetLivelloObbligatorio(const PLivello: Integer):Boolean;
begin
  Result:=False;
  if selI096.SearchRecord('COD_ITER;LIVELLO',VarArrayOf([CodIter,PLivello]),[srFromBeginning]) then
    Result:=selI096.FieldByName('OBBLIGATORIO').AsString = 'S';
end;

procedure TC018FIterAutDM.ImpostaVariabiliExprRichiesta(PLivello:integer = 0; PStato:string = ''; PResponsabile:string = ''; PAutorizzAutomatica:string = '');
var
  i,TipoVariabile:Integer;
  Filtro:String;
  TipoRichiesta:TTipoRichiesta;
  FiltroCampo:array of string;
begin
  if PLivello = 0 then
  begin
    TipoRichiesta:=trRichiesta;
    FiltroCampo:=VarArrayOf(['NOME_UTENTE','PROFILO_UTENTE']);
  end
  else
  begin
    TipoRichiesta:=trAutorizzazione;
    FiltroCampo:=VarArrayOf(['ID','LIVELLO','STATO','RESPONSABILE','AUTORIZZ_AUTOMATICA','NOME_UTENTE','PROFILO_UTENTE']);
  end;
  ...
  begin
    ...
    begin
      if not R180In(UpperCase(selTabellaIter.Fields[i].FieldName),FiltroCampo) then
      begin
  ...
  if TipoRichiesta = trAutorizzazione then
  begin
    if R180CercaParolaIntera(':ID',Filtro,',;()=<>|!/+-*') > 0  then
    begin
      selDualExprRichiesta.DeclareVariable('ID',otInteger);
      selDualExprRichiesta.SetVariable('ID',Id);
    end;
    if R180CercaParolaIntera(':LIVELLO',Filtro,',;()=<>|!/+-*') > 0  then
    begin
      selDualExprRichiesta.DeclareVariable('LIVELLO',otInteger);
      selDualExprRichiesta.SetVariable('LIVELLO',PLivello);
    end;
    if R180CercaParolaIntera(':STATO',Filtro,',;()=<>|!/+-*') > 0  then
    begin
      selDualExprRichiesta.DeclareVariable('STATO',otString);
      selDualExprRichiesta.SetVariable('STATO',PStato);
    end;
    if R180CercaParolaIntera(':RESPONSABILE',Filtro,',;()=<>|!/+-*') > 0  then
    begin
      selDualExprRichiesta.DeclareVariable('RESPONSABILE',otString);
      selDualExprRichiesta.SetVariable('RESPONSABILE',PResponsabile);
    end;
    if R180CercaParolaIntera(':AUTORIZZ_AUTOMATICA',Filtro,',;()=<>|!/+-*') > 0  then
    begin
      selDualExprRichiesta.DeclareVariable('AUTORIZZ_AUTOMATICA',otString);
      selDualExprRichiesta.SetVariable('AUTORIZZ_AUTOMATICA',PAutorizzAutomatica);
    end;
  end;
end;
...
begin
  inherited;
  selT851.FieldByName('C_TITOLO_LIVELLO').AsString:=selT851.FieldByName('TITOLO_LIVELLO').AsString;
  //Traduzione 'Richiesta'
  selT851.FieldByName('C_TITOLO_LIVELLO').AsString:=StringReplace(selT851.FieldByName('C_TITOLO_LIVELLO').AsString,A000MSG_WC018_MSG_RICHIESTA,A000TraduzioneStringhe(A000MSG_WC018_MSG_RICHIESTA),[rfIgnoreCase]);
  //Traduzione 'Autorizzazione'
  selT851.FieldByName('C_TITOLO_LIVELLO').AsString:=StringReplace(selT851.FieldByName('C_TITOLO_LIVELLO').AsString,A000MSG_WC018_MSG_AUTORIZZAZIONE,A000TraduzioneStringhe(A000MSG_WC018_MSG_AUTORIZZAZIONE),[rfIgnoreCase]);
  //Traduzione 'Visto'
  selT851.FieldByName('C_TITOLO_LIVELLO').AsString:=StringReplace(selT851.FieldByName('C_TITOLO_LIVELLO').AsString,A000MSG_WC018_MSG_VISTO,A000TraduzioneStringhe(A000MSG_WC018_MSG_VISTO),[rfIgnoreCase]);
...
begin
  ...
  RicercaCodIter;
  {?}
  if not selI097.SearchRecord('BLOCCANTE;LIVELLO',VarArrayOf(['N',0]),[srFromBeginning]) then
    exit;
  if (CodIter <> '') and (selTabellaIter.State in [dsInsert,dsEdit]) then
  begin
    // valuta validità richiesta
    FTipoRichiesta:=PTipoRichiesta;
    FCondizioniValiditaOk:=ScorriCondizValidita(False);
    Result:=FCondizioniValiditaOk;
    FTipoRichiesta:='';
  end;
end;

function TC018FIterAutDM.ValiditaRichiesta(const PTipoRichiesta: String = ''):Boolean;
// restituisce True se la richiesta è ok
// oppure False altrimenti
begin
  Result:=True;
  ...
  RicercaCodIter;
  {?}
  if not selI097.SearchRecord('BLOCCANTE;LIVELLO',VarArrayOf(['S',0]),[srFromBeginning]) then
    exit;
  if (CodIter <> '') and (selTabellaIter.State in [dsInsert,dsEdit]) then
  begin
    // valuta validità richiesta
    FTipoRichiesta:=PTipoRichiesta;
    FCondizioniValiditaOk:=ScorriCondizValidita(True);
    Result:=FCondizioniValiditaOk;
    FTipoRichiesta:='';
  end;
end;

function TC018FIterAutDM.ScorriCondizValidita(CondizioniBloccanti:Boolean; PLivello:integer = 0; PStato:string = ''; PResponsabile:string = ''; PAutorizzAutomatica:string = ''):Boolean;
var
  Filtro,Messaggio,MsgTipoRichiesta,ColPeriodoInizio:String;
  TipoRichiesta:TTipoRichiesta;
begin
  Result:=True;
  FMessaggioOperazione:='';
  ...
  begin
    TipoRichiesta:=trRichiesta;
    MsgTipoRichiesta:='La richiesta';
  end
  else
  begin
    TipoRichiesta:=trAutorizzazione;
    MsgTipoRichiesta:='L''autorizzazione';
  end;

  //Verifico condizione automatica che la richiesta non sia antecedente a Parametri.WEBCartelliniDataMin (escluso iter SCIOPERI, CERTIFICAZIONI E DOCUMENTALE in cui la colonna DATA non è un dato esplicitamente caricato su selTabellaIter)
  if TipoRichiesta = trRichiesta then
  begin
    ColPeriodoInizio:=StringReplace(GetColonnaPeriodoInizio,'T_ITER.','',[]);
    if CondizioniBloccanti and not R180In(Iter,[ITER_SCIOPERI,ITER_CERTIFICAZIONI,ITER_DOCUMENTALE]) and (ColPeriodoInizio <> '') then
    begin
  ...
  //begin Gestione livelli I097
  if selI097.SearchRecord('COD_ITER',CodIter,[srFromBeginning]) then
  begin
    repeat
      if not R180In(selI097.FieldByName('LIVELLO').AsInteger,[IfThen(PLivello = 0,0,-1),PLivello]) then
      begin
        Continue;
      end;
      ...
      if (selI097.FindField('TIPO_RICHIESTA') <> nil) and
         (selI097.FieldByName('TIPO_RICHIESTA').AsString <> '*') and
         ((selI097.FieldByName('TIPO_RICHIESTA').AsString <> FTipoRichiesta) or
          (selI097.FieldByName('TIPO_RICHIESTA').AsString = Format('-%s',[FTipoRichiesta]))) then
      begin
        Continue;
      end;
      ...
      begin
        SQL.Text:=Format('select count(*) TOT from DUAL where %s',[Filtro]);
        ImpostaVariabiliExprRichiesta(PLivello,PStato,PResponsabile,PAutorizzAutomatica);
        try
          Execute;
          if FieldAsInteger('TOT') = 0 then
          begin
            ...
            begin
              // messaggio di errore non indicato: ne imposta uno di default
              Messaggio:=Format('%s non rispetta la condizione di validità num. %d della struttura "%s".',[MsgTipoRichiesta, selI097.FieldByName('NUM_CONDIZ').AsInteger,CodIter]);
            end
            else
            begin
              ...
              try
                ...
                begin
                  Messaggio:=Format('%s non rispetta la condizione di validità num. %d della struttura "%s".' + CRLF +
                                    '(messaggio di errore non indicato)',
                                    [MsgTipoRichiesta, selI097.FieldByName('NUM_CONDIZ').AsInteger,CodIter]);
                end;
              except
                Messaggio:=Format('%s non rispetta la condizione di validità num. %d della struttura "%s".' + CRLF +
          ...
          begin
            ...
            begin
              Messaggio:=Format('Validazione dell''autorizzazione per la struttura "%s" num. %d fallita: %s',[CodIter,selI097.FieldByName('NUM_CONDIZ').AsInteger,E.Message]);
            end;
            if selI097.FieldByName('BLOCCANTE').AsString = 'S' then
            begin
              Break;
  ...
  if (selI097.FieldByName('BLOCCANTE').AsString = 'S') and R180In(selI097.FieldByName('LIVELLO').AsInteger,[-1,PLivello]) then
...
begin
  ...
  //Verifico condizione automatica che la richiesta non sia antecedente a Parametri.WEBCartelliniDataMin (escluso iter SCIOPERI in cui la colonna DATA non è un dato esplicitamente caricato su selTabellaIter)
  ColPeriodoInizio:=StringReplace(GetColonnaPeriodoInizio,'T_ITER.','',[]);
  if CondizioniBloccanti and not R180In(Iter,[ITER_SCIOPERI,ITER_CERTIFICAZIONI,ITER_DOCUMENTALE]) and (ColPeriodoInizio <> '') then
  begin
    if selTabellaIter.FieldByName(ColPeriodoInizio).AsDateTime < Parametri.WEBCartelliniDataMin then
  ...
  if selI097.SearchRecord('COD_ITER;LIVELLO',VarArrayOf([CodIter,0]),[srFromBeginning]) then
    repeat
      if CondizioniBloccanti <> (selI097.FieldByName('BLOCCANTE').AsString = 'S') then
        Continue;
      Filtro:=selI097.FieldByName('CONDIZ_VALIDITA').AsString.Trim;
      ...
      if Messaggio <> '' then
        FMessaggioOperazione:=FMessaggioOperazione + IfThen(FMessaggioOperazione <> '',CRLF,'') + Messaggio;
    until not selI097.SearchRecord('COD_ITER;LIVELLO',VarArrayOf([CodIter,0]),[]);

  if (selI097.FieldByName('BLOCCANTE').AsString = 'S') and (selI097.FieldByName('LIVELLO').AsInteger = 0) then
    FMessaggioOperazione:=Messaggio;
end;
}
...
begin
  ...
  //Per le revoche impedita la richiesta di allegati
  if R180In(selTabellaIter.FieldByName('TIPO_RICHIESTA').AsString,['R','C']) then
    exit;
...
// restituisce S se la struttura prevede allegati modificabili dopo un'autorizzazione intermedia
// oppure N altrimenti
var
  Condiz: string;
begin
  Result:='N';//Result:=VarToStr(selI095.Lookup('COD_ITER',CodIter,'ALLEGATI_MODIFICABILI'));

  Condiz:=VarToStr(selI095.Lookup('COD_ITER',CodIter,'ALLEGATI_MODIFICABILI'));

  // se nella condizione è già specificato direttamente uno dei flag, lo considera
  if R180In(Condiz,['N','S']) then
  begin
    Result:=Condiz;
    Exit;
  end;
...
begin
  selDualExprRichiesta.ClearVariables;
  selDualExprRichiesta.DeleteVariables;
  selDualExprRichiesta.SQL.Clear; // daniloc. 21.11.2012
  selDualExprRichiesta.SQL.Add('begin');
  selDualExprRichiesta.SQL.Add(PScriptAutorizz);
  selDualExprRichiesta.SQL.Add('end;');

  ImpostaVariabiliExprRichiesta(RLivello,RStato,RResponsabile,RAutorizzAutomatica);
  (*
  ...
  begin
    selDualExprRichiesta.DeclareVariable('LIVELLO',otInteger);
    selDualExprRichiesta.SetVariable('LIVELLO',RLivello);
  end;
  if R180CercaParolaIntera(':STATO',ScriptUpper,',;()=<>|!/+-*') > 0  then
  begin
    selDualExprRichiesta.DeclareVariable('STATO',otString);
    selDualExprRichiesta.SetVariable('STATO',RStato);
  end;
  if R180CercaParolaIntera(':RESPONSABILE',ScriptUpper,',;()=<>|!/+-*') > 0  then
  begin
    selDualExprRichiesta.DeclareVariable('RESPONSABILE',otString);
    selDualExprRichiesta.SetVariable('RESPONSABILE',RResponsabile);
  end;
  if R180CercaParolaIntera(':AUTORIZZ_AUTOMATICA',ScriptUpper,',;()=<>|!/+-*') > 0  then
  begin
    selDualExprRichiesta.DeclareVariable('AUTORIZZ_AUTOMATICA',otString);
    selDualExprRichiesta.SetVariable('AUTORIZZ_AUTOMATICA',RAutorizzAutomatica);
  end;
  if R180CercaParolaIntera(':NOME_UTENTE',ScriptUpper,',;()=<>|!/+-*') > 0  then
  begin
    selDualExprRichiesta.DeclareVariable('NOME_UTENTE',otString);
    selDualExprRichiesta.SetVariable('NOME_UTENTE',Parametri.Operatore);
  end;
  if R180CercaParolaIntera(':PROFILO_UTENTE',ScriptUpper,',;()=<>|!/+-*') > 0  then
  begin
    selDualExprRichiesta.DeclareVariable('PROFILO_UTENTE',otString);
    selDualExprRichiesta.SetVariable('PROFILO_UTENTE',Parametri.ProfiloWEB);
  end;
  *)

  try
    selDualExprRichiesta.Execute;
    if selDualExprRichiesta.VariableIndex('LIVELLO') >= 0 then
      RLivello:=selDualExprRichiesta.GetVariable('LIVELLO');
    if selDualExprRichiesta.VariableIndex('STATO') >= 0 then
      RStato:=VarToStr(selDualExprRichiesta.GetVariable('STATO'));
    if selDualExprRichiesta.VariableIndex('RESPONSABILE') >= 0 then
      RResponsabile:=VarToStr(selDualExprRichiesta.GetVariable('RESPONSABILE'));
    if selDualExprRichiesta.VariableIndex('AUTORIZZ_AUTOMATICA') >= 0 then
      RAutorizzAutomatica:=VarToStr(selDualExprRichiesta.GetVariable('AUTORIZZ_AUTOMATICA'));
  except
    on E:Exception do
      FMessaggioOperazione:='Errore nell''esecuzione dello script:' + CRLF + selDualExprRichiesta.SQL.Text + CRLF + E.Message;
  end;
end;
...
begin
  ...
  FCondizioniValiditaOk:=ScorriCondizValidita(True);
  if not FCondizioniValiditaOk then
  begin
    FTipoRichiesta:='';
    exit;
  end;
  ...
  begin
    ...
    SetVariable('RICHIEDENTE',IfThen(Iter = ITER_CARTELLINO, '', Parametri.Operatore));
  ...
  selTabellaIter.FieldByName(GetIDIter).AsInteger:=Id;
  RegistraLog.SettaProprieta('I',TabellaIter,IterCodForm,selTabellaIter,True);
  ...
  if (not R180In(Iter,[ITER_CERTIFICAZIONI,ITER_DOCUMENTALE])) or (PTipoRichiesta = SG230TR_D) then
    Result:=VerificaRichiestaEsistente(AutorizzAutomatica);
  FTipoRichiesta:='';

  //TORINO_ASLCITTA - CONVTO1: esecuzione rigida del package USR_PCK_ITER_SPECIALISTI.T050P_ASSEGNA_RICHIESTE_NA;
  if R180In(Iter,[ITER_TIMBR,ITER_GIUSTIF]) and (CodIter = '-NONASSEGNATO-') then
  try
    scrUSR_PCK_ITER_SPECIALISTI.Execute;
  except
  end;
...
begin
      ...
      repeat
        ...
        begin
          ...
          begin
            ...
            LivSucc:=InsAutorizzazione(Liv,C018SI,C018AUTOMATICO,PAutorizzAutomatica,'');
  ...
  if (MaxLivAutorizzato = 0) and (not PosponiInvioMail) then
...
// parametri:
//   - PTabella: serve per forzare il nome della tabella dell'iter specifico da cui eliminare il record
var
  LTabella: String;
...
begin
  ...
  // imposta il nome della tabella dell'iter specifico
  if PTabella <> '' then
    LTabella:=PTabella
  else
    LTabella:=TabellaIter;

  (*Alberto 21/07/2017: lascio cancellare anche gli allegati
  ...
  *)
  ...
  RegistraLog.SettaProprieta('C',LTabella,FIterCodForm,selTabellaIter,True);
  delIterRichiesta.ClearVariables;
  delIterRichiesta.SetVariable('TABELLA',LTabella);
  delIterRichiesta.SetVariable('ITER',Iter);
  delIterRichiesta.SetVariable('COLONNA_ID', GetIDIter);
  delIterRichiesta.SetVariable('ID',Id);
  try
    LeggiAllegati;  //Leggo preventivamente gli allegati per eliminazione successiva
    delIterRichiesta.Execute;
    //Eliminazione allegati associati alla richiesta cancellata
    if selT853_T960.RecordCount > 0 then
    try
      C021DM:=TC021FDocumentiManagerDM.Create(Self);
      while not selT853_T960.Eof do
      begin
        LResCtrl:=C021DM.Delete(selT853_T960.FieldByName('ID_T960').AsInteger);
        if not LResCtrl.Ok then
        begin
          //Al momento non segnalo nulla
...
var
  LLivObbSucc:Integer;
  LElencoLivelli:String;
  LMailOggetto,LMailCorpo: String;
  LMailTag: Integer;
{$ENDIF}
{$ENDIF}
begin
  ...
  begin
    if selI096.SearchRecord('COD_ITER',CodIter,[srFromBeginning]) then
      repeat
        ...
        begin
          if (selI096.FieldByName('OBBLIGATORIO').AsString = 'S') and (selI096.FieldByName('LIVELLO').AsInteger > LivelloObb) and (LLivObbSucc = 999) then
            LLivObbSucc:=selI096.FieldByName('LIVELLO').AsInteger
          else if (selI096.FieldByName('OBBLIGATORIO').AsString = 'N') and (selI096.FieldByName('LIVELLO').AsInteger > LivelloNoObb) and (selI096.FieldByName('LIVELLO').AsInteger < LLivObbSucc) then
            LElencoLivelli:=LElencoLivelli + selI096.FieldByName('LIVELLO').AsString + ',';
        end;
      until not selI096.SearchRecord('COD_ITER',CodIter,[]);
  end;
  if LLivObbSucc < 999 then
    LElencoLivelli:=LElencoLivelli + IntToStr(LLivObbSucc)
  else
    LElencoLivelli:=Copy(LElencoLivelli,1,Length(LElencoLivelli) - 1); //tolgo l'ultima virgola
  if LElencoLivelli = '' then
    exit;
  //Preparazione testo
  LMailTag:=TagAutorizzazione;
  if Parametri.CampiRiferimento.C90_EMailRespOttimizzata = 'S' then
  begin
    LMailOggetto:=Parametri.CampiRiferimento.C90_EMailRespOggetto;
    if LMailOggetto = '' then
      LMailOggetto:='Notifica presenza richieste da autorizzare';
    LMailCorpo:=Parametri.CampiRiferimento.C90_EMailRespTesto;
    if LMailCorpo = '' then
      LMailCorpo:='Si avvisa che sono presenti richieste in attesa di autorizzazione.';
  end
  else
  begin
    // estrae dati per la mail da inviare all'autorizzatore
    LMailOggetto:='';
    LMailCorpo:='';
    selMailPerAutorizzatore.SetVariable('PROGRESSIVO',selTabellaIter.FieldByName('PROGRESSIVO').AsInteger);
    selMailPerAutorizzatore.SetVariable('COLONNA_ID',GetIDIter);
    selMailPerAutorizzatore.SetVariable('ID',Id);
    if selMailPerAutorizzatore.VariableIndex('OPERAZIONE') >= 0 then
      selMailPerAutorizzatore.SetVariable('OPERAZIONE',Operazione);
    try
      selMailPerAutorizzatore.Execute;
      if selMailPerAutorizzatore.RowCount > 0 then
      begin
        LMailOggetto:=selMailPerAutorizzatore.FieldAsString('MAIL_OGGETTO');
        LMailCorpo:=selMailPerAutorizzatore.FieldAsString('MAIL_CORPO');
      end;
    except
      on E:Exception do
      begin
  ...
  if (LMailOggetto <> '') or (LMailCorpo <> '') then
    WR000DM.InviaEMail(True,selTabellaIter.FieldByName('PROGRESSIVO').AsInteger,LMailOggetto,LMailCorpo,LMailTag,Iter,CodIter,LElencoLivelli);
  {$ENDIF}
  {$ENDIF}
end;
...
begin
  ...
  begin
    ...
    SetVariable('COLONNA_ID',GetIDIter);
    SetVariable('ID',Id);
...
begin
  RicercaCodIter;
  AssegnaCodIter(selTabellaIter.FieldByName('ID').AsInteger,Iter,CodIter);
end;
...
begin
  ...
  (*
  // prepara log di modifica di TIPO_RICHIESTA
  LOldTipoRichiesta:=FTipoRichiesta;
  if (selTabellaIter <> nil) and (selTabellaIter.FindField('TIPO_RICHIESTA') <> nil) then
  ...
  *)
  // aggiornamento tipo richiesta su T850
  updT850TipoRichiesta.ClearVariables;
  updT850TipoRichiesta.SetVariable('ITER',Iter);
  updT850TipoRichiesta.SetVariable('ID',Id);
  updT850TipoRichiesta.SetVariable('TIPO_RICHIESTA',PTipoRichiesta);
  updT850TipoRichiesta.Execute;
  LOldTipoRichiesta:=VarToStr(updT850TipoRichiesta.GetVariable('OLD_TIPO_RICHIESTA'));
  ...
  begin
    ...
    begin
      for L:=PLivello + 1 to Liv do
        Result:=InsAutorizzazione(L,C018SI,C018AUTOMATICO,C018SI,'');
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        FMessaggioOperazione:=A000TraduzioneStringhe(A000MSG_C018_ERR_ALLEGATI_OBBLIGATORI);
        Exit;
      end;
    end;
  end;
  ...
  //begin Gestione livelli I097
  if not ScorriCondizValidita(True,PLivello,PStato,PResponsabile,PAutorizzAutomatica) (*FMessaggioOperazione <> '' *) then
    Exit;
  //end Gestione livelli I097
  ...
  *)
  ...
  begin
    RegistraLog.SettaProprieta('I',TabellaIter,IterCodForm,nil,True);
    if Iter <> ITER_BUDGET_STRAORD then
  ...
  begin
    ...
    if (PStato = C018SI) and (not PosponiInvioMail) then
      ...
      if selI096.SearchRecord('COD_ITER',CodIter,[srFromBeginning]) then
        repeat
          ...
          begin
            ...
            //Alberto 19/06/2017: Mi riposiziono sull'ultimo livello autorizzato, perchè la chiamata a InsAutorizzazione può spostare avanti il cursore di selI096
...
begin
  ...
  NoteArr:=PNote.Split([CRLF],1,TStringSplitOptions.None);
...
begin
  ...
  R180SetVariable(selT851,'COLONNA_ID',GetIDIter);
  ...
  //Gestione tabella iter senza colonna "PROGRESSIVO"
  R180SetVariable(selT851,'PROGRESSIVO',IfThen(TabellaIter = 'T700_ITER_BUDGETANNO', 'decode(T_ITER.ID,null,0,0)', 'T_ITER.PROGRESSIVO'));
...
begin
  ...
  begin
    ...
    Result:=Format('<html><h3>' + A000TraduzioneStringhe(A000MSG_C018_MSG_STRUTTURA) + ' %s</h3>%s',[DescCodIter,Result]);
    // MONDOEDP - commessa MAN/07 SVILUPPO#58.fine
  end;
end;
...
{$IF CompilerVersion >= 31}
{versione 10.1 Berlin+}
function TC018FIterAutDM.LeggiNote(var RNote: TNoteRichiesta; var RErrMsg: String): Boolean;
var
...
begin
  ...
  SetLength(RNote.NoteArr,selT851.RecordCount);

  // ciclo per caricare l'array delle note
  selT851.First;
  i:=0;
  while not selT851.Eof do
  begin
    LNoteLiv:=TNoteLivello.Create;
    LNoteLiv.Livello:=selT851.FieldByName('LIVELLO').AsInteger;
    LNoteLiv.DescLivello:=selT851.FieldByName('DESC_LIVELLO').AsString;
    LNoteLiv.Note:=selT851.FieldByName('NOTE').AsString.Trim;
    LNoteLiv.Autorizzazione:=selT851.FieldByName('STATO').AsString;
    LNoteLiv.AutorizzAutomatica:=selT851.FieldByName('AUTORIZZ_AUTOMATICA').AsString;
    LNoteLiv.Nominativo:=selT851.FieldByName('NOMINATIVO').AsString;
    RNote.NoteArr[i]:=LNoteLiv;
    {
    //Visualizzo nominativo e note solo se esistono e se non si tratta di Autorizzazione Automatica
    if (((LNote <> '') or (Trim(FieldByName('NOMINATIVO').AsString) <> '')) and (FieldByName('AUTORIZZ_AUTOMATICA').AsString <> 'S'))
       or
...
function TC018FIterAutDM.LeggiNoteEsistenti(PFormatoHTML:Boolean = False):String;
//Lettura delle sole note esistenti.
//I ritorni a capo sono realizzati con <br>
//Il tag <html> serve per essere gestito correttamente da jQuery
begin
  Result:='';
  ...
  begin
    ...
    begin
      //Visualizzo nominativo e note solo se esistono e se non si tratta di Autorizzazione Automatica
      if (Trim(FieldByName('NOTE').AsString) <> '') and (FieldByName('AUTORIZZ_AUTOMATICA').AsString <> 'S') then
      begin
        if Result <> '' then
          Result:=Result + CRLF;
        Result:=Result + FieldByName('NOMINATIVO').AsString + ': ' + Trim(FieldByName('NOTE').AsString);
      end;
      Next;
    end;
  end;
...
begin
  try
    ...
    begin
      ...
      Result:=Format('<html><h3>' + A000TraduzioneStringhe(A000MSG_C018_MSG_ALLEGATI) + '</h3><table style=''border: 0 none;''><tbody>%s</tbody></table>',[Result]);
    end;
  except
    on E: Exception do
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      Legenda:=Format(RIGA_LEGENDA,['bg_giallo',A000TraduzioneStringhe(A000MSG_C018_MSG_ALLEGATI_ESISTENTI)]);
    end
    else
    begin
      ...
      begin
        ...
        Legenda:=Format(RIGA_LEGENDA,['bg_rosso',A000TraduzioneStringhe(A000MSG_C018_MSG_ALLEGATI_OBBLIGATORI)]);
      end
      else
      begin
        PImg.ImageFile.FileName:=fileImgAllegati;
        Legenda:=Format(RIGA_LEGENDA,['bg_alleg_facoltativi',A000TraduzioneStringhe(A000MSG_C018_MSG_ALLEGATI_FACOLTATIVI)]);
      end;
    end;
...
var
  TmpLivAut: Integer;
  TmpAut, LMsgAutSi, LMsgAutNo: String;
  IWChk: TmeIWCheckBox;
begin
  if grd = nil then
    Exit;
  ...
  if idxS >= 0 then
  begin
    IWChk:=(grd.medpCompCella(rowGrd,rowComp,idxS) as TmeIWCheckBox);
    IWChk.Checked:=StatoAutorizzato(TmpAut);
    IWChk.Css:=IWChk.Css + ' autSi';
    IWChk.OnClick:=chkAutorizzazioneClick;
    IWChk.medpTag:=TC018TipoChekAutor.Create;//C018SI;
    (IWChk.medpTag as TC018TipoChekAutor).Tipo:=C018SI;
    // EMPOLI_ASL11 - chiamata <95332>.ini
    // conferma autorizzazione lato client
    LMsgAutSi:=grd.medpValoreColonna(rowGrd,'MSGAUT_SI').Trim;
    IWChk.Confirmation:=LMsgAutSi;
    // EMPOLI_ASL11 - chiamata <95332>.fine
  end;

  // check autorizzazione NO
  if idxN >= 0 then
  begin
    IWChk:=(grd.medpCompCella(rowGrd,rowComp,idxN) as TmeIWCheckBox);
    if NumValoriPossibili[TmpLivAut] = 1 then
      IWChk.Css:='invisibile'
    else
      IWChk.Css:=IWChk.Css + ' autNo';
    IWChk.Checked:=StatoNegato(TmpAut);
    IWChk.OnClick:=chkAutorizzazioneClick;
    IWChk.medpTag:=TC018TipoChekAutor.Create;
    (IWChk.medpTag as TC018TipoChekAutor).Tipo:=C018NO;
    // EMPOLI_ASL11 - chiamata <95332>.ini
    // conferma negazione lato client
    LMsgAutNo:=grd.medpValoreColonna(rowGrd,'MSGAUT_NO').Trim;
    IWChk.Confirmation:=LMsgAutNo;
    // EMPOLI_ASL11 - chiamata <95332>.fine
  end;
end;
{$ENDIF}
...
begin
  ...
  begin
    W000RegistraLog('Errore',GetTestoLog('C018') + Format('Attenzione: la richiesta %d risulta in stato X = Esclusa dai conteggi; provo a ripristinarla',[Id]));
    try
      SetTipoRichiesta('P');
      SessioneOracle.Commit;
      W000RegistraLog('Errore',GetTestoLog('C018') + Format('Attenzione: la richiesta %d è stata ripristinata allo stato P = Preventiva',[Id]));
    except
      on E: Exception do
        W000RegistraLog('Errore',GetTestoLog('C018') + Format('Errore: la richiesta %d non è stata ripristinata allo stato P = Preventiva per il seguente errore: %s',[Id,E.Message]));
    end;
  end;
  {$ENDIF}
  {$ENDIF}
