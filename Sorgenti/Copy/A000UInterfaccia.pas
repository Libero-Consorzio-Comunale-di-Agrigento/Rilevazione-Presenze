...
begin
  {Alberto-Dario 09/06/2015: Attenzione!
     ...
     Per es: CloseSesson: tag:=tag - 1 --> tag = 0
             ...
             SessioneWeb.Destroy: tag = 1 (e non più 0)
     La cosa però non sembra creare problemi alle transazioni o alla integrità di lstSessioniOracle
  }
  ...
  try
    try
      ...
      begin
        {$IFDEF WEBPJ}
        //IrisCloud: si forza sempre il logoff (il parametro LogoffDbPool non è significativo, da togliere)
        SessioneIrisWIN.SessioneOracle.Logoff;
        {$ELSE}
        //IrisWEB: si forza il logoff se richiesto dal parametro o se c'è una sessione db per ogni sessione web
...
const
  SOGLIA = 15;
begin
  // se il timeout corrente è già inferiore alla soglia, termina immediatamente
...
// restituisce true se il timeout di sessione è ridotto rispetto al valore originale
...
begin
  ...
  try
    ...
    if not TFile.Exists(LCurrJsFile) then
      raise Exception.CreateFmt('Il seguente file javascript non è presente'#13#10'%s',[LCurrJsFile]);
    ...
    on E: Exception do
      raise Exception.CreateFmt('Si è verificato un errore durante la fase di avvio:'#13#10'%s',[E.Message]);
  end;
end;
...
begin
  ...
  //in caso di ws rest forzo il browser chrome per evitare incompatibilità con intraweb
  {$IFDEF INTRAWEBSVC}
...
begin
  ...
  *)
  ...
  Sembra che sia dovuto ai datamoduli ma in produzione dà ancora più problemi
  Al momento l'unica soluzione è aumentare il timeout al massimo (per es. 999)
...
begin
  ...
  try
    ...
    begin
      // determina la form sulla quale si è verificato l'errore
...
begin
  ...
  begin
    // servizi datasnap
    try
      // alla propria sessione datasnap è associato un oggetto di tipo TSessioneIrisWin
