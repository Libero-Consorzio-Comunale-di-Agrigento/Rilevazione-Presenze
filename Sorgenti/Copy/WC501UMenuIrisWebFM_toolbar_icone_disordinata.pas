...
uses
  WR206UMenuFM, A000UInterfaccia, A000UCostanti, A000USessione, C180FunzioniGenerali,
  ...
  IWControl, IWCompMenu, ImgList, IWImageList, ActnList, IWCompJQueryWidget,
  IWVCLComponent, IWBaseLayoutComponent, IWBaseContainerLayout, IWContainerLayout,
  IWApplication, IWTemplateProcessorHTML, meIWMenu;
...
begin
  ...
  // 2. "autorizzazione ecced. giornaliere" in base a parametro aziendale C90_W026TipoRichiesta
  ...
  begin
    actRichiestaStraordGG.Caption:='Autorizzazione ecced. giornaliere';
    actRichiestaStraordGG.Hint:=actRichiestaStraordGG.Caption;
...
begin
  ...
  begin
    with TW002FRicercaAnagrafe(F) do
    begin
  ...
  begin
    TR010FPaginaWeb(Self.Parent).MsgBox.MessageBox('Nessuna anagrafica selezionata',INFORMA);
    Exit;
  end;
  ...
  else if (Sender = actMessaggi) or (FTag = 408) then
  begin
    // 408 - messaggi
    if NewForm then
      F:=TW011FMessaggi.Create(A000App);
  end
  ...
  begin
    ...
    begin
      ...
      F.SetParam('AL',DATE_NULL);
      TW029FPesatureIndividuali(F).GetPesiDip;
    end;
  end
  ...
  begin
    ...
    begin
      ...
      F.SetParam('DAL',R180InizioMese(Parametri.DataLavoro));
      F.SetParam('DAL',R180FineMese(Parametri.DataLavoro));
      F.SetParam('SINGOLO',False);
    end;
  end
  ...
  begin
    ...
    begin
      ...
      F.SetParam('AL',DATE_NULL);
      TW031FSchedeQuantIndividuali(F).GetSchedeDip;
    end;
  end
  ...
  else if (Sender = actPubblicazioneDocumenti) or (FTag = 44) then
  begin
    WR000DM.Responsabile:=False;
    if NewForm then
  ...
  begin
    ...
    begin
      ...
      if F.GetParam('PROGRESSIVO') = '' then
        F.SetParam('PROGRESSIVO',TR010FPaginaWeb(Self.Parent).medpProgressivo);
