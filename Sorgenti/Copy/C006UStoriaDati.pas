...
uses
  ...
  Db, OracleData, Oracle, Variants, StrUtils,
  A000UInterfaccia, A000UCostanti, A000USessione, C180FunzioniGenerali, USelI010;
...
type
  ...
  private
    ...
    Anno:String;
    pTipoDato,pTab,pCod,pStorico:String;
    I010:TselI010;
  ...
  public
    ...
    LeggiDescrizioni:Boolean;
    procedure GetStoriaDato(P:Integer; Dato:String; Data:TDateTime = 0; StoriaInizioFine:String = '');
    function DescrizioneDatoStorico(TipoDato,Codice:String):string;
    procedure GetStoriaDatoP430(P:Integer; Dato:String; Data:TDateTime = 0; StoriaInizioFine:String = '');
  end;
...
begin
  ...
  pCod:='';
  pStorico:='';
  LeggiDescrizioni:=True;
end;

procedure TC006FStoriaDati.GetStoriaDato(P:Integer; Dato:String; Data:TDateTime = 0; StoriaInizioFine:String = '');
var i:Integer;
    ...
    sStoriaInizioFine:String;
begin
  pTipoDato:='';
  pTab:='';
  pCod:='';
  pStorico:='';
  sStoriaInizioFine:=IfThen(StoriaInizioFine = '',Parametri.StoriaInizioFine,StoriaInizioFine);
  ...
  for i:=0 to Q430.FieldCount - 1 do
    ...
    while not Q430.Eof do
      begin
      if (sStoriaInizioFine = 'N') or (Q430.FieldByName('Fine').IsNull) then
        Cessazione:=EncodeDate(3999,12,31)
      else
        Cessazione:=Q430.FieldByName('Fine').AsDateTime;
...
begin
  ...
  begin
    if Codice = '0' then Result:='Giorn./mens. da gg.lav.turnisti'
    ...
    else if Codice = '4' then Result:='Giorn. da orario, mens. da gg.lavorativi'
    else if Codice = '5' then Result:='Giorn. da orario, mens. da gg.lav.turnisti';
  end
  ...
  else if (TipoDato = 'AZIENDA_BASE') and (Codice = T440AZIENDA_BASE) then
    Result:=Parametri.RagioneSociale
  else
  begin
    Codice:=AggiungiApice(Codice);
    if (Codice <> '') and ((TipoDato <> pTipoDato) or (pTab = '')) then
    begin
      pTipoDato:=TipoDato;
      A000GetTabella(pTipoDato,pTab,pCod,pStorico);
    end;
    if (Codice <> '') and (pTab <> '') and (pTab <> 'T430_STORICO') then
    begin
      S:=QLookup.SQL.Text;
      QLookup.SQL.Clear;
      if pTab = 'T480_COMUNI' then
        QLookup.SQL.Add(Format('SELECT CITTA FROM %s WHERE %s = ''%s''',[pTab,pCod,Codice]))
      else if pStorico = 'S' then
      begin
        QLookup.SQL.Add('SELECT DESCRIZIONE FROM ' + pTab + ' T1 WHERE ' + pCod + ' = ''' + Codice + '''');
        QLookup.SQL.Add(' AND DECORRENZA = (SELECT MAX(DECORRENZA) FROM ' + pTab + ' WHERE ' + pCod + ' = ''' + Codice + '''');
        ...
        QLookup.SQL.Add(Format('SELECT DESCRIZIONE FROM %s WHERE %s = ''%s''',[pTab,pCod,Codice]));
      if S <> QLookup.SQL.Text then
        QLookup.Close;
      QLookup.Open;
...
procedure TC006FStoriaDati.GetStoriaDatoP430(P:Integer; Dato:String; Data:TDateTime = 0; StoriaInizioFine:String = '');
var i:Integer;
    DescrizioneStorico:String;
    Valore:String;
    ObjStoria:RecStoria;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (sStoriaInizioFine = 'N') or (selP430Storico.FieldByName('Fine').IsNull) then
        Cessazione:=EncodeDate(3999,12,31)
      else
        Cessazione:=selP430Storico.FieldByName('Fine').AsDateTime;
...
begin
  ...
  begin
    ...
    else if Codice = 'T' then Result:='Si, tutti i mesi'
    else if Codice = 'X' then Result:='Si, senza ricalcolo imposta'
    else Result:='No';
  end
  ...
  begin
    ...
    else if Codice = 'D' then Result:='Detrazioni per redditi di lavoro dipendente e assimilati senza ulteriori detrazioni'
    else if Codice = 'C' then Result:='Detrazioni per assegni corrisposti dal coniuge'
  ...
  begin
    ...
    else if Codice = 'CP' then Result:='Creditore pignoratizio'
    else Result:='?';
  end
  ...
  else if TipoDato = 'TIPO_ADESIONE_FPC' then
  begin
    if Codice = 'N' then Result:='Nessuna'
    else if Codice = 'V' then Result:='Volontaria'
    else if Codice = 'I' then Result:='Informativa silenzio-assenso'
    else if Codice = 'R' then Result:='Recesso silenzio-assenzo'
    else if Codice = 'S' then Result:='Silenzio-assenso';
  end
  else
  begin
    Codice:=AggiungiApice(Codice);
    ...
    begin
      pTipoDato:=TipoDato;
      A000GetTabellaP430(TipoDato,pTab,pCod,pStorico);
    end;
    if (Codice <> '') and (pTab <> '') and (pTab <> 'P430_ANAGRAFICO') then
    begin
      S:=QLookup.SQL.Text;
      QLookup.SQL.Clear;
      if pTab = 'T030_ANAGRAFICO' then
        QLookup.SQL.Add(Format('SELECT COGNOME || '' '' || NOME FROM %s WHERE %s = ''%s''',[pTab,pCod,Codice]))
      else if pTab = 'T480_COMUNI' then
        QLookup.SQL.Add(Format('SELECT CITTA FROM %s WHERE %s = ''%s''',[pTab,pCod,Codice]))
      else if pTab = 'P004_CODICITABANNUALI' then
