inherited WA123FPartecipazioniSindacatiDM: TWA123FPartecipazioniSindacatiDM
  Height = 122
  Width = 311
  inherited selTabella: TOracleDataSet
    SQL.Strings = (
      'select T247.*, T247.ROWID'
      'from T247_PARTECIPAZIONISINDACATI T247'
      'WHERE PROGRESSIVO = :PROGRESSIVO '
      ':ORDERBY')
    Variables.Data = {
      0400000002000000100000003A004F0052004400450052004200590001000000
      0000000000000000180000003A00500052004F00470052004500530053004900
      56004F00030000000000000000000000}
    BeforeInsert = BeforeInsert
    BeforePost = BeforePostNoStorico
    OnCalcFields = selTabellaCalcFields
    OnNewRecord = OnNewRecord
    object selTabellaPROGRESSIVO: TIntegerField
      FieldName = 'PROGRESSIVO'
      Required = True
      Visible = False
    end
    object selTabellaDADATA: TDateTimeField
      DisplayLabel = 'Da data'
      DisplayWidth = 10
      FieldName = 'DADATA'
      Required = True
      OnValidate = selTabellaDADATAValidate
      DisplayFormat = 'dd/mm/yyyy'
      EditMask = '!99/99/0000;1;_'
    end
    object selTabellaADATA: TDateTimeField
      DisplayLabel = 'A data'
      DisplayWidth = 10
      FieldName = 'ADATA'
      OnValidate = selTabellaADATAValidate
      DisplayFormat = 'dd/mm/yyyy'
      EditMask = '!99/99/0000;1;_'
    end
    object selTabellaCOD_SINDACATO: TStringField
      DisplayLabel = 'Sindacato'
      FieldName = 'COD_SINDACATO'
      Required = True
      OnChange = selTabellaCOD_SINDACATOChange
      Size = 10
    end
    object selTabellaDESC_SINDACATO: TStringField
      DisplayLabel = 'Descr. Sindacato'
      DisplayWidth = 30
      FieldKind = fkCalculated
      FieldName = 'DESC_SINDACATO'
      Size = 50
      Calculated = True
    end
    object selTabellaCOD_ORGANISMO: TStringField
      DisplayLabel = 'Organismo'
      FieldName = 'COD_ORGANISMO'
      Required = True
      OnChange = selTabellaCOD_ORGANISMOChange
      Size = 5
    end
    object selTabellaDESC_ORGANISMO: TStringField
      DisplayLabel = ' Descr. Organismo'
      DisplayWidth = 20
      FieldKind = fkCalculated
      FieldName = 'DESC_ORGANISMO'
      Size = 50
      Calculated = True
    end
  end
  inherited selEstrazioneDati: TOracleDataSet
    Left = 112
  end
  inherited dsrEstrazioniDati: TDataSource
    Left = 112
  end
  inherited selT900: TOracleDataSet
    Left = 200
  end
  inherited selT901: TOracleDataSet
    Left = 256
  end
end
