...
type
  TWA004FRecapitoVisFiscaliFM = class(TWR200FBaseFM)
    ...
    edtDettagliIndirizzo: TmeIWEdit;
  ...
  private
    ...
    procedure btnOKClick1(Sender: TObject; R: TmeIWModalResult; KI:String);
...
begin
  ...
  with A004MW do
  begin
    FNominativo:=RecapitoAlternativo.Nominativo;
    ...
    FDettagliIndirizzo:=RecapitoAlternativo.DettagliIndirizzo;
    ...
    begin
      edtNominativo.Enabled:=selT047.FieldByName('DATA_PRIMA_COMUNICAZIONE').IsNull;
      ...
      edtDettagliIndirizzo.Enabled:=selT047.FieldByName('DATA_PRIMA_COMUNICAZIONE').IsNull;
      ...
      FNominativo:=selT047.FieldByName('NOMINATIVO').AsString;
      ...
      FDettagliIndirizzo:=selT047.FieldByName('DETTAGLI_INDIRIZZO').AsString;
  ...
  // imposta i valori nei campi a video
  edtNominativo.Text:=FNominativo;
  edtComune.Text:=FDesComune;
  edtCap.Text:=FCap;
  edtIndirizzo.Text:=FIndirizzo;
  edtDettagliIndirizzo.Text:=FDettagliIndirizzo;
  ...
  C190VisualizzaElemento(JQuery,'grpInsManualeINPS',Parametri.ModuloInstallato['TORINO_CSI_PRV']);
end;
...
begin
  ...
  FNominativo:=Trim(edtNominativo.Text);
  FDesComune:=edtComune.Text;
  FCap:=Trim(edtCap.Text);
  FIndirizzo:=Trim(edtIndirizzo.Text);
  FDettagliIndirizzo:=Trim(edtDettagliIndirizzo.Text);
  FTelefono:=Trim(edtTelefono.Text);
  FNote:=Trim(memNote.Text);

  if not MsgBox.KeyExists('btnOKClick') then
  begin
    // controllo se sono indicate le note ma i dettagli indirizzo è vuoto
    if (FNote <> '') and (FDettagliIndirizzo = '') then
    begin
      MsgBox.ClearKeys;
      MsgBox.WebMessageDlg(A000MSG_A004_DLG_NOTE_NO_DETT_INDIRIZZO,mtConfirmation,[mbYes,mbNo],btnOKClick1,'btnOKClick');
      exit;
    end;
  end;
  MsgBox.ClearKeys;

  // se è indicato il nome del comune effettua la ricerca per identificarlo univocamente
  if FDesComune <> '' then
  begin
    // salva valori precedenti che verrebbero sovrascritti dalla lookuprecord
    LCapPrec:=edtCap.Text;
    LMedLegPrec:=cmbMedLegale.Text;

    if IntfWebT480.LookupRecord(False) then
    begin
      MsgBox.MessageBox(A000TraduzioneStringhe(A000MSG_A004_MSG_RICERCA_COMUNE),INFORMA);
      Exit;
    end;

    // è stato trovato un solo comune: valorizza i dati per successivo salvataggio
    FCodComune:=IntfWebT480.ValoreChiave;
    FDesComune:=edtComune.Text;

    // imposta il CAP trovato se non era valorizzato dall'utente
    if LCapPrec = '' then
      FCap:=edtCap.Text;

    // ripristina la medicina legale se era già valorizzata
    if LMedLegPrec <> '' then
      cmbMedLegale.Text:=LMedLegPrec;
  end;
  ...
  A004MW.RecapitoAlternativo.Nominativo:=FNominativo;
  ...
  A004MW.RecapitoAlternativo.DettagliIndirizzo:=FDettagliIndirizzo;
...
procedure TWA004FRecapitoVisFiscaliFM.btnOKClick1(Sender: TObject; R: TmeIWModalResult; KI: String);
begin
  if R = mrNo then
  begin
    MsgBox.ClearKeys;
