unit WA119UPartecipazioneScioperiDettFM;
...
type
  TWA119FPartecipazioneScioperiDettFM = class(TWR205FDettTabellaFM)
    lblData: TmeIWLabel;
...
uses WA119UPartecipazioneScioperi, WA119UPartecipazioneScioperiDM;

{$R *.dfm}

procedure TWA119FPartecipazioneScioperiDettFM.CaricaMultiColumnComboBox;
begin
  inherited;
  with (WR302DM as TWA119FPartecipazioneScioperiDM).A119MW do
  begin
    CaricaComboBox(cmbCausale,selT265);
  end;
end;

procedure TWA119FPartecipazioneScioperiDettFM.CaricaComboBox(CmbBox: TMedpIWMultiColumnComboBox; DataSet: TOracleDataSet);
begin
  with DataSet do
  begin
    Refresh;
...
procedure TWA119FPartecipazioneScioperiDettFM.Componenti2DataSet;
begin
  inherited;
  with (WR302DM as TWA119FPartecipazioneScioperiDM) do
  begin
    selTabella.FieldByName('CAUSALE').AsString:=cmbCausale.Text;
  end;
end;

procedure TWA119FPartecipazioneScioperiDettFM.DataSet2Componenti;
begin
  inherited;
  with (WR302DM as TWA119FPartecipazioneScioperiDM) do
  begin
...
procedure TWA119FPartecipazioneScioperiDettFM.drgpTipoGiustClick(Sender: TObject);
begin
  inherited;
...
procedure TWA119FPartecipazioneScioperiDettFM.cmbCausale2AsyncChange(
  Sender: TObject; EventParams: TStringList; Index: Integer; Value: string);
begin
  SetDescrizioneCausale;
end;

procedure TWA119FPartecipazioneScioperiDettFM.SetDescrizioneCausale;
// imposta la label di descrizione della causale in base al valore della combobox
begin
  lblDescCausale.Caption:=VarToStr((WR302DM as TWA119FPartecipazioneScioperiDM).A119MW.selT265.Lookup('CODICE',cmbCausale.Text,'DESCRIZIONE'));
end;

procedure TWA119FPartecipazioneScioperiDettFM.imgC700SelezioneAnagrafeClick(Sender: TObject);
begin
  with (WR302DM as TWA119FPartecipazioneScioperiDM).selTabella do
  begin
  if State in [dsEdit,dsInsert] then
    with (Self.Owner as TWA119FPartecipazioneScioperi).grdC700 do
    begin
...
procedure TWA119FPartecipazioneScioperiDettFM.ResultSelAnagrafe(Sender: TObject; Result: Boolean);
...
begin
  if Result then
  begin
    S:=Trim((Self.Owner as TWA119FPartecipazioneScioperi).grdC700.WC700FM.SQLCreato.Text);
    if Pos('ORDER BY',UpperCase(S)) > 0 then
      S:=Copy(S,1,Pos('ORDER BY',UpperCase(S)) - 1);
    //(WR302DM as TWA119FPartecipazioneScioperiDM).selTabella.FieldByName('SELEZIONE_ANAGRAFICA').AsString:=TrasformaV430(S);
    (WR302DM as TWA119FPartecipazioneScioperiDM).selTabella.FieldByName('SELEZIONE_ANAGRAFICA').AsString:=S.Replace(#13#10,' ',[rfReplaceAll]);
  end;
end;

procedure TWA119FPartecipazioneScioperiDettFM.OnDsrStateChange;
begin
...
procedure TWA119FPartecipazioneScioperiDettFM.OnInserimento;
