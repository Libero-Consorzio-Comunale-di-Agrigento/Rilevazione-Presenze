...
begin
  ...
  cdsDipendenti.FieldDefs.Add('COGNOME',ftString,50,False);
  cdsDipendenti.FieldDefs.Add('NOME',ftString,50,False);
...
begin
  ...
  begin
    ...
    FieldDefs.Add('COGNOME',ftString,50,False);
    FieldDefs.Add('NOME',ftString,50,False);
...
begin
  ...
  begin
    ...
    begin
      ...
      if (selCols.FieldByName('TABELLA').AsString = 'P264_MOD730IMPORTI') then //dettaglio 730
      begin  //Copio le testate del 730 sul nuovo progressivo. Dopo lo spostamento del dettaglio cancello le vecchie testate (vedi PulisciTab)
        QSQL.SQL.Clear;
        QSQL.SQL.Add('INSERT INTO P262_MOD730TESTATA');
        QSQL.SQL.Add('SELECT ' + selAnagrafe.FieldByName('PROGRESSIVO').AsString + ' PROGRESSIVO, ANNO, COD_CAAF, DATA_ARRIVO_MOD730, DATA_ARRIVO_RETTIF, DATA_ARRIVO_INTEGR, DATA_SOSP_IRPEF, DATA_SOSP_ADDIZ_REG, DATA_SOSP_ADDIZ_PROV, DATA_SOSP_ADDIZ_COM, DATA_SOSP_TASS_SEP,');
        QSQL.SQL.Add('       COD_TIPOCONG, COD_RETTIF, COD_ESITO_2RATA, COD_ESITO_CONGNONEFF, INTEGRATIVO, NON_LIQUID, COD_ESITO_2RATA_CONIUGE, COD_ESITO_2RATA_CEDOL, COD_ESITO_2RATA_CEDOL_CONIUGE, COD_MOTIVO_RETTIF, COD_MOTIVO_INTEGR');
        QSQL.SQL.Add('FROM P262_MOD730TESTATA');
        QSQL.SQL.Add('WHERE PROGRESSIVO = ' + IntToStr(OldProgressivo));
        try
          QSQL.Execute;
        except
          ...
          begin
            Anom:=True;
            s:=Format(A000MSG_B007_MSG_FMT_ERR_262,[E.Message]);
            RegistraMsg.InserisciMessaggio('A',s,'',OldProgressivo);
          end;
        end;
      end;
      if (selCols.FieldByName('TABELLA').AsString <> 'P272_RETRIBUZIONE_CONTRATTUALE') and
         ...
         (selCols.FieldByName('TABELLA').AsString <> 'P593_CONTABDATIINDIVIDUALI') and
         (selCols.FieldByName('TABELLA').AsString <> 'P262_MOD730TESTATA') then //già gestito prima e dopo
      begin
    ...
    begin
      ...
      PulisciTab('P262_MOD730TESTATA',OldProgressivo);
