...
type
  ...
  public
    ...
    DettDip,TotGen,RigheVuote,PresGGLav,AssGGLav,GGInt,AssQM,DettGG,DettFam,FruizGG,FruizMG,FruizHH,FruizDH,PeriodiStorici:Boolean;
    OldProg,OldStraord,iEsportaXml,
    iAss,iNumDipPeriodo,iNumDipArrot,iDebitoGGInt,iAssArrot,iRiepArrot:Integer;
    TotGiorniMMCorr,TotGiorniMMPrec,NumEventi,MaxGG:Integer;
    selI010:TselI010;
    selI010B:TselI010;
...
begin
  ...
  PeriodiStorici:=False{Comportamento nuovo};//True{Comportamento attuale};
  CaricaPickList;
  UserNameText:=Parametri.CampiRiferimento.C40_WebSrv_User;
  PasswordText:=Parametri.CampiRiferimento.C40_WebSrv_Pwd;
  CodEnteText:=Parametri.CampiRiferimento.C40_EnteL104;
  URLWSText:=Parametri.CampiRiferimento.C40_WebSrv_URLL104;
end;
...
begin
  ...
  begin
    ...
    begin
      s:=StringReplace(s,':DataLavoro BETWEEN V430.T430DataDecorrenza AND V430.T430DataFine','1=1',[rfReplaceAll,rfIgnoreCase]);
      s:=StringReplace(s,':DATALAVORO BETWEEN NVL(P430DECORRENZA,:DATALAVORO) AND NVL(P430DECORRENZA_FINE,:DATALAVORO)','1=1',[rfReplaceAll,rfIgnoreCase]);
    end;
...
begin
  ...
  begin
    ...
    Dataset.FieldDefs.Add('COGNOME',ftString,50,False);
    Dataset.FieldDefs.Add('NOME',ftString,50,False);
  ...
  begin
    ...
    Dataset.FieldDefs.Add('COGNOMEFAM',ftString,50,False);
    Dataset.FieldDefs.Add('NOMEFAM',ftString,50,False);
...
begin
  ...
  selT920.SetVariable('DADATA', IfThen(PeriodiStorici,selV430.FieldByName('DEC').AsDateTime,DaData));
  selT920.SetVariable('ADATA',  IfThen(PeriodiStorici,selV430.FieldByName('FINE').AsDateTime,AData));
  if PresGGLav then
...
begin
  ...
  selT920Ass.SetVariable('DADATA', IfThen(PeriodiStorici,selV430.FieldByName('DEC').AsDateTime,DaData));
  selT920Ass.SetVariable('ADATA',  IfThen(PeriodiStorici,selV430.FieldByName('FINE').AsDateTime,AData));
...
begin
  ...
  else if selSG101.FieldByName('GRADOPAR').AsString = 'UC' then
    cdsRisultato.FieldByName('GRADOPAR').AsString:=selSG101.FieldByName('GRADOPAR').AsString + '-Unito civilmente'
  else if selSG101.FieldByName('GRADOPAR').AsString = 'CF' then
    cdsRisultato.FieldByName('GRADOPAR').AsString:=selSG101.FieldByName('GRADOPAR').AsString + '-Convivente di fatto';
...
var DescDatoAz:String;
begin
  //Username PerlaPA
  if Trim(UserNameText) = '' then
    DescDatoAz:=A000DescDatiEnte('C40_WEBSRV_USER')
  //Password PerlaPA
  else if Trim(PasswordText) = '' then
    DescDatoAz:=A000DescDatiEnte('C40_WEBSRV_PWD')
  //Codice ente L104
  else if Trim(CodEnteText) = '' then
    DescDatoAz:=A000DescDatiEnte('C40_ENTEL104')
  //URL L104
  else if Trim(URLWSText) = '' then
    DescDatoAz:=A000DescDatiEnte('C40_WEBSRV_URLL104');
  if DescDatoAz <> '' then
    raise Exception.Create(Format(A000MSG_A151_ERR_FMT_DATO_AZ_VUOTO,[DescDatoAz]));
