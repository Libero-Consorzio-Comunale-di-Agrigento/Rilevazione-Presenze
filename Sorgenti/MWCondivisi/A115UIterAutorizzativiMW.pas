...
type
  TA115FIterAutorizzativiMW = class(TR005FDataModuleMW)
    ...
    selI097TIPO_RICHIESTA: TStringField;
    selI097LIVELLO: TIntegerField;
...
begin
  ...
  GestAllegati:=R180In(selI093.FieldByName('ITER').AsString,[ITER_MISSIONI,ITER_GIUSTIF,ITER_CERTIFICAZIONI]);
...
begin
  ...
  if (selI096.State = dsEdit) and (selI096.FieldByName('LIVELLO').AsString = '') (*(selI096.FieldByName('LIVELLO').medpOldValue <> selI096.FieldByName('LIVELLO').Value)*) then
  begin
  ...
  for Elemento in selI096.FieldByName('VALORI_POSSIBILI').AsString.Split([',']) do
    if Elemento.Length > 1 then
      raise Exception.Create('Il dato "Valori possibili" deve contenere una lista di singoli caratteri separati da virgola.'#13#10 +
                             'Il valore ' + Elemento + ' è più lungo di un carattere!');
...
begin
  ...
  selI097.FieldByName('TIPO_RICHIESTA').AsString:=UpperCase(selI097.FieldByName('TIPO_RICHIESTA').AsString);
  arresta:=False;
  case selI097.FieldByName('TIPO_RICHIESTA').AsString.Length of
  0: arresta:=True;
  1: arresta:=((selI097.FieldByName('TIPO_RICHIESTA').AsString[1] <> '*') and
              (not (selI097.FieldByName('TIPO_RICHIESTA').AsString[1] in cTipoRichiesta )));
  2: arresta:=((selI097.FieldByName('TIPO_RICHIESTA').AsString[1] <> '-') or
              (not (selI097.FieldByName('TIPO_RICHIESTA').AsString[2] in cTipoRichiesta )));
  end;
  if arresta then
    raise Exception.Create(Format('Valore %s non valido per il campo"Tipo richiesta"',[selI097.FieldByName('TIPO_RICHIESTA').AsString]));

  //Se Livello è <> 0 forza il campo bloccante a "S"
  if selI097.FieldByName('LIVELLO').AsInteger <> 0 then
  begin
    selI097.FieldByName('BLOCCANTE').AsString:='S';
  end;
end;
...
var
  ...
  begin
    ...
    OQ.SetVariable('COLONNA_ID',C018DM.GetIdIter);
    with FindVariables(UpperCase(selI093.FieldByName(Campo).AsString), False) do
    try
      if (Pos('RESP',Campo) > 0) and (IndexOf('OPERAZIONE') = -1) then
        raise Exception.CreateFmt('Errore sul%s della mail per %s: è obbligatorio utilizzare la variabile :OPERAZIONE!',[Sez,Dest]);
      for i:=0 to Count - 1 do
      begin
        if OQ.VariableIndex(Strings[i]) = -1 then
          OQ.DeclareVariable(Strings[i],otString);
      end;
    finally
      Free;
    end;
