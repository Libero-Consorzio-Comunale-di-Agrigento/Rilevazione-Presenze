create or replace function CONTROLLO_RIPOSI(prog in integer,data_conteggio in date,causali_scart in string, causali_abb in string default '', causali_rep in varchar2 default '') return integer is
  cursor c1 is
    select to_char(T.DATAORA,'dd-mm-yyyy hh24.mi.ss') DATAORA_TIMB,T.*,T.ROWID from VT100_TIMB_DATAORA T
  ...
  int_f date;
  uscita date;
  int_i_text varchar2(20);
  int_f_text varchar2(20);
begin
  intervallo_max:=0;
  intervallo:=1;
  ...
  for t1 in c1 loop
    ...
    for t2 in c2(t1.DATAORA) loop
      ...
      if t2.VERSO = 'U' then
        ...
        int_i_text:=to_char(int_i,'dd-mm-yyyy hh24.mi.ss');
        int_f_text:=to_char(int_f,'dd-mm-yyyy hh24.mi.ss');
