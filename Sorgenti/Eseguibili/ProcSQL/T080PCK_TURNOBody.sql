...
begin
  if TIPO = 'O' then
    ...
    begin
      select distinct T081.TURNO1, T081.ORARIO into OUT_TURNO1, OUT_ORARIO
        from T081_PROVVISORIO T081
       where T081.PROGRESSIVO = INPROG
         and T081.DATA = INDATA
		 and T081.FLAGAGG = decode(I091F_GETDATO('C11_PIANIFORARIPROG'),'N','N','I');
    exception
      when NO_DATA_FOUND then
        null;
    end;
...
function GETTIPOGIORNO(P_PROGRESSIVO in integer, P_DATA in DATE, P_DOMENICA_FESTIVA in varchar2 :='S') return varchar2 is
  wFest varchar2(1);
  wFestSucc varchar2(1);
  wGG varchar2(1);
  result varchar2(5);
begin
  select /*+ ordered*/
    to_char(V010A.DATA - 1,'d') GG,
    V010A.FESTIVO,
    V010B.FESTIVO
  into wGG, wFest, wFestSucc  
  from V010_CALENDARI V010A, V010_CALENDARI V010B
  where V010A.PROGRESSIVO = P_PROGRESSIVO
  and   V010A.DATA = P_DATA
  and  V010B.PROGRESSIVO(+) = V010A.PROGRESSIVO
  and V010B.DATA(+) = V010A.DATA + 1;
  
  if (wFest = 'S') or (wGG = '7' and P_DOMENICA_FESTIVA = 'S') then
    result:='F';
  elsif wFestSucc = 'S' then
    result:='P';
  else
    result:=wGG;
  end if;
