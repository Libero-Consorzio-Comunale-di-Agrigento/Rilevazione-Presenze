create or replace procedure ALLINEA_PERIODI_STIPENDI (
...
-- a quelli del precedente unificando in questo caso i due periodi
  ...
  CURSOR CT430 is
    ...
    from   T430_STORICO
    where  progressivo = PROGIRIS
    and    datadecorrenza between nvl(P_DAL,PCK_CONST.DATA_NUL) and nvl(P_AL,PCK_CONST.DATA_INF);
  CURSOR CP430 (p_decorrenza I030_RELAZIONI_ANAGRAFE.decorrenza%TYPE,
...
begin
  ...
  for RP430 in CP430 (TO_DATE('01011900','DDMMYYYY'),TO_DATE('31123999','DDMMYYYY')) loop
    if NROWIDUPDATE is not null then
      if dFINE_PREC <> RP430.DECORRENZA - 1 then
        --dbms_output.put_line('update P430_ANAGRAFICO set DECORRENZA_FINE = RP430.DECORRENZA - 1 where ROWID = NROWIDUPDATE;');
  ...
  if NROWIDUPDATE is not null then
    --dbms_output.put_line('update P430_ANAGRAFICO set DECORRENZA_FINE = 31/12/3999 where ROWID = NROWIDUPDATE;');
  ...
  if PRE_ALLINEA_ESEGUITA = 'N' then
    PRE_ALLINEA_PERIODI_STIPENDI(wCAMPI_NOT_NULL, wESP_CUR_REL, wDECORRENZE, wCAMPI_SEL, wTAB_FROM, wCOND_WHERE, wCONSIDERA_REL_ALTRA_TAB, -1, 'S', 'N');
  end if;
    ...
    if wDECORRENZE is not null then
      ...
      while INSTR(S_APP_DEC,',') > 0 loop
        ...
        if DEC_STO between greatest(DECO_MIN,nvl(P_DAL,PCK_CONST.DATA_NUL)) and nvl(P_AL,PCK_CONST.DATA_INF) then
          --dbms_output.put_line('CREAZIONE_STORICO_STIPENDI(PROGIRIS,DEC_STO,null);');  
    ...
    loop
      if DBMS_SQL.FETCH_ROWS(CURSORE_DINAMICO_I030) > 0 then
        ...
        if SRELAZIONE is not null then
          ...
          for RP430 in CP430(I030_DECORRENZA,I030_SCADENZA) loop
            ...
            begin
              ...
              loop -- INIZIO CONTINUA_ESTR_RELAZIONE --
                ...
                if DBMS_SQL.FETCH_ROWS(CURSORE_DINAMICO_SEL_REL) > 0 then
				  ...
				  --se il valore letto è nullo, provo a leggere quello successivo (la query delle decode in union può restituire il valore nullo ed il valore effettivo)
  ...
  if PRE_ALLINEA_ESEGUITA = 'N' then
    wDECORRENZE:=null;
    PRE_ALLINEA_PERIODI_STIPENDI(wCAMPI_NOT_NULL, wESP_CUR_REL, wDECORRENZE, wCAMPI_SEL, wTAB_FROM, wCOND_WHERE, wCONSIDERA_REL_ALTRA_TAB, PROGIRIS, 'N', 'S');
  end if;
    --Creazione periodi storici per ogni decorrenza individuata
  if (wDECORRENZE is not null) and ((PRE_ALLINEA_ESEGUITA = 'N') or (nvl(wESP_CUR_REL,'nvl') = ' and 1 = 2 ')) then
    -- CREO LE DECORRENZE RICAVATE
    S_APP_DEC:=wDECORRENZE;
    while INSTR(S_APP_DEC,',') > 0 loop
      DEC_STO:=TO_DATE(SUBSTR(S_APP_DEC,1,8),'DDMMYYYY');
  ...
  ESPRESSIONE:='select DISTINCT P430.ROWID, P430.DECORRENZA, P430.DECORRENZA_FINE, ' || wCAMPI_SEL || ' from P430_ANAGRAFICO P430' || wTAB_FROM ||' where P430.PROGRESSIVO = :PROGIRIS';
  if P_DAL is not null or P_AL is not null then
    ESPRESSIONE:=ESPRESSIONE||' and P430.DECORRENZA <= to_date('''||to_char(nvl(P_AL,PCK_CONST.DATA_INF),'ddmmyyyy')||''',''ddmmyyyy'')';
    ESPRESSIONE:=ESPRESSIONE||' and P430.DECORRENZA_FINE >= to_date('''||to_char(nvl(P_DAL,PCK_CONST.DATA_NUL),'ddmmyyyy')||''',''ddmmyyyy'')';
  end if;  
  ESPRESSIONE:=ESPRESSIONE||wCOND_WHERE || ' order by P430.DECORRENZA';
  -- cursore che legge i periodi storici
  CURSORE_DINAMICO_P430:=DBMS_SQL.OPEN_CURSOR;
  begin
  --dbms_output.put_line('p430: '||ESPRESSIONE);    
    ...
    if DBMS_SQL.FETCH_ROWS(CURSORE_DINAMICO_P430) > 0 then
      ...
      if VALOREDATIOLD is null then
        ...
        if VALOREDATI1 = VALOREDATIOLD then
          ...
          --dbms_output.put_line('delete P430_ANAGRAFICO where ROWID = NROWID;');
          ...
          if ULTIMAdFINE is not null then
            ...
            --dbms_output.put_line('update P430_ANAGRAFICO set DECORRENZA_FINE = ULTIMAdFINE where ROWID = NROWIDUPDATE');
  ...
  update P430_ANAGRAFICO set DECORRENZA_FINE = TO_DATE('31123999','DDMMYYYY') where
    PROGRESSIVO = PROGIRIS and DECORRENZA = (select MAX(DECORRENZA) from P430_ANAGRAFICO where PROGRESSIVO = PROGIRIS);
  exception
    when others then null;
  ...
  --dbms_output.put_line('update P430_ANAGRAFICO P430 set DECORRENZA = dDECORRENZA where PROGRESSIVO = PROGIRIS and...');  
