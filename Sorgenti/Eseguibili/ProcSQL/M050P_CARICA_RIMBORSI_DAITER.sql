create or replace procedure M050P_CARICA_RIMBORSI_DAITER(pID in integer) as
  ...
  cursor c3 is
    select M040.ID_MISSIONE, M040.PROGRESSIVO, M040.MESESCARICO, M040.MESECOMPETENZA, M040.DATADA, M040.ORADA, M020.TIPO_QUANTITA,
           M150.CODICE, nvl(M049.SOMMA,'S') DA_RIMBORSARE, min(M150.COD_VALUTA) COD_VALUTA, sum(nvl(M150.RIMBORSO_VARIATO,M150.RIMBORSO)) RIMBORSO
    from   M040_MISSIONI M040, M150_RICHIESTE_RIMBORSI M150, M020_TIPIRIMBORSI M020, M049_TIPOPAGAMENTO M049
    where  M040.ID_MISSIONE = pID
    and    M040.ID_MISSIONE = M150.ID
    and    M150.CODICE = M020.CODICE
    and    M150.STATO = 'S'
    and    M150.TIPORIMBORSO = M049.CODICE(+)
    group by M040.ID_MISSIONE, M040.PROGRESSIVO, M040.MESESCARICO, M040.MESECOMPETENZA, M040.DATADA, M040.ORADA, M020.TIPO_QUANTITA, M150.CODICE, nvl(M049.SOMMA,'S');
...
begin
    ...
    if wCOUNT_A = 0 then
      ...
      values
        (t3.ID_MISSIONE, t3.PROGRESSIVO, t3.MESESCARICO, t3.MESECOMPETENZA, t3.DATADA, t3.ORADA,
         t3.CODICE, t3.COD_VALUTA, decode(t3.TIPO_QUANTITA,'F',0,decode(t3.DA_RIMBORSARE,'S',t3.RIMBORSO,0)), t3.RIMBORSO);
    else
      ...
      set IMPORTORIMBORSOSPESE = IMPORTORIMBORSOSPESE + decode(t3.TIPO_QUANTITA,'F',0,decode(t3.DA_RIMBORSARE,'S',t3.RIMBORSO,0)),
  ...
  if wRIMB_DETT = 'S' then
    ...
    insert into M051_DETTAGLIORIMBORSO (PROGRESSIVO, MESESCARICO, MESECOMPETENZA, DATADA, ORADA,
      ...
      select M040.PROGRESSIVO, M040.MESESCARICO, M040.MESECOMPETENZA, M040.DATADA, M040.ORADA,
             ...
             nvl(M150.RIMBORSO_VARIATO,M150.RIMBORSO), nvl(M150.TIPORIMBORSO,wTIPO_RIMBORSO), null
  ...
  select nvl(max(DATO),'D') into wSTATO 
  from MONDOEDP.I091_DATIENTE 
  where AZIENDA = I090F_GETAZIENDACORRENTE
  and TIPO = 'C8_W032_STATOMISSIONE_AUORIZZATA';
  if wStato not in ('D','S','L') then
    wStato:='D';
  end if;  
