create or replace function T430F_GIORNISERVIZIO(
  ...
  P_FILTRA_DECORRENZE in varchar2 :='N' --S = vengono considerati solo i periodi storici con decorrenza compresa tra P_DAL e P_AL
) return integer as
  wConto integer;
  result integer;
begin
  ...
  if P_TIPO_PERIODO = 'S' then
    ...
    and    P_AL  >= INIZIO
    and    (P_FILTRA_DECORRENZE = 'N' or (DATADECORRENZA <= P_AL and DATAFINE >= P_DAL)) 
    and exists (
      ...
      and (P_FILTRA_DECORRENZE = 'N' or (T430B.DATADECORRENZA <= P_AL and T430B.DATAFINE >= P_DAL)) 
      );
