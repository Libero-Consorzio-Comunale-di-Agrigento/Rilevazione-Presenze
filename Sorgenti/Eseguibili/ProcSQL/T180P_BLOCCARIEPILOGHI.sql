create or replace procedure T180P_BLOCCARIEPILOGHI(P_PROGRESSIVO IN INTEGER, P_RIEPILOGO IN VARCHAR2, P_DAL IN DATE, P_AL IN DATE) as
  cursor c1 is 
...
-- Traccia dell'apertura dell'iter di validazione cartellino  
  if P_RIEPILOGO in ('T860') then
    d:=P_DAL;
    while d <= P_AL loop
      select count(*) into wConto from T070_SCHEDARIEPIL where PROGRESSIVO = P_PROGRESSIVO and DATA = d;
      if (wConto > 0) and (T077F_LEGGIVALORE(P_PROGRESSIVO, d, PCK_CONST.I011BLOCCO_T860, 'M') is null) then
        T077P_SCRIVIVALORE(P_PROGRESSIVO, d, 'M', PCK_CONST.I011BLOCCO_T860, to_char(sysdate,'dd/mm/yyyy hh24.mi'));
      end if;
      d:=add_months(d,1);
    end loop;
  end if;
