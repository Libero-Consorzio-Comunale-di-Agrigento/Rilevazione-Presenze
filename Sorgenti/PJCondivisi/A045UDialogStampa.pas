...
uses
  ...
  A000UMessaggi, A045UStatAssenzeMW, QRPDFFilt, QRExport, InputPeriodo;
...
type
  ...
  public
    ...
    Progressivo:LongInt;
    sDescTipiRapporto,DocumentoOutput:string;
...
begin
  ...
  DataI:= A045MW.DataInizio;
  DataF:= A045MW.DataFine;
...
var i: integer;
begin
  A045MW.DataInizio:= DataI;
  A045MW.DataFine:= DataF;
...
begin
  ...
  begin
    ...
    //begin
      ...
      if (Sender = Self) and (R180EstraiExtFile(DocumentoOutput).ToLower = EXT_PDF) then
      begin
        A045FStampa.RepR.ShowProgress:=False;
        A045FStampa.RepR.ExportToFilter(TQRPDFDocumentFilter.Create(DocumentoOutput));
      end
      else if (Sender = Self) and (R180EstraiExtFile(DocumentoOutput).ToLower = EXT_XLS) then
      begin
        A045FStampa.RepR.ShowProgress:=False;
        A045FStampa.RepR.ExportToFilter(TQRXLSFilter.Create(DocumentoOutput));
      end
...
begin
  try
    OpenC012VisualizzaTesto('<A045> Visualizzazione file di esportazione della statistica ministeriale assenze',edtFile.Text,nil,
                            'Visualizzazione file di esportazione della statistica ministeriale assenze del periodo '+ frmInputPeriodo.edtInizio.Text + ' - ' + frmInputPeriodo.edtFine.Text);
  except
...
procedure TA045FDialogStampa.AggiornaDateLista;
begin
  if (A045MW.DataInizio <> DataI) or (A045MW.DataFine <> DataF) then
    PopolaListaQualifiche;
  A045MW.DataInizio:= DataI;
  A045MW.DataFine:= DataF;
end;

procedure TA045FDialogStampa.frmInputPeriodobtnAvantiClick(Sender: TObject);
begin
  frmInputPeriodo.btnAvantiClick(Sender);
  AggiornaDateLista;
end;

procedure TA045FDialogStampa.frmInputPeriodobtnDataFineClick(Sender: TObject);
begin
  frmInputPeriodo.btnDataFineClick(Sender);
  AggiornaDateLista;
end;

procedure TA045FDialogStampa.frmInputPeriodobtnDataInizioClick(Sender: TObject);
begin
  frmInputPeriodo.btnDataInizioClick(Sender);
  AggiornaDateLista;
end;

procedure TA045FDialogStampa.frmInputPeriodobtnIndietroClick(Sender: TObject);
begin
  frmInputPeriodo.btnIndietroClick(Sender);
  AggiornaDateLista;
end;

procedure TA045FDialogStampa.frmInputPeriodoedtFineExit(Sender: TObject);
begin
  AggiornaDateLista;
end;

procedure TA045FDialogStampa.frmInputPeriodoedtInizioExit(Sender: TObject);
begin
...
{ DataF }
function TA045FDialogStampa._GetDataF: TDateTime;
begin
  Result := frmInputPeriodo.DataFine;
end;
procedure TA045FDialogStampa._PutDataF(const Value: TDateTime);
begin
  frmInputPeriodo.DataFine := Value;
end;
{ ----DataF }
{ DataI }
function TA045FDialogStampa._GetDataI: TDateTime;
begin
  Result := frmInputPeriodo.DataInizio;
end;
procedure TA045FDialogStampa._PutDataI(const Value: TDateTime);
begin
