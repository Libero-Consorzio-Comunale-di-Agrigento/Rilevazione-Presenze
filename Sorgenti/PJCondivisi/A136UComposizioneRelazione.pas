...
uses
  ...
  Menus, ActnList, StrUtils,A000UMessaggi, System.Actions;
    ...
    *)
    ...
    begin
      ...
      try
        ...
        sAbbinaValori:='';
        bElementiNonStandard:=R180CercaParolaIntera('SUBSTR',SQLRelazione,',(') > 0;
        ...
        begin
          ...
          if (Copy(SQLRelazione,1,15) = 'SELECT DISTINCT')
...
begin
  inherited;
  SqlColonne:=A136FRelazioniAnagrafeDtm.A136FRelazioniAnagrafeMW.selCols.SQL.Text;
  SqlColonne:=StringReplace(SqlColonne,':TABELLA',''''+edtTabellaPilota.Text +'''',[rfReplaceAll]);
  SqlColonne:=StringReplace(SqlColonne,':PILOTA','''S''',[rfReplaceAll]);
  Abilitato:=False;
  while not Abilitato do
  begin
    OpenC015FElencoValori('COLS','<A136> Selezione della colonna pilota',SqlColonne,'COLUMN_NAME',vCodice,nil,350);
