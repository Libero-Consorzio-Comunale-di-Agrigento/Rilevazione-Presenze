...
uses
  Windows, Messages, SysUtils, Variants, Classes, Graphics, Controls, Forms, QRPDFFilt,
  Dialogs, Menus, StdCtrls, CheckLst, Buttons, Mask, ExtCtrls, Printers, DB, DBClient,
...
procedure TP690FStampaFondi.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  if TipoModulo = 'CS' then
  begin
...
begin
  ...
  if P690FStampaFondiDtM.P690MW.FondiDisponibili(StrToDate(edtDecorrenzaDa.Text),StrToDate(edtDecorrenzaA.Text)) then
  begin
    clbFondi.Items.Clear;
    for i:=0 to P690FStampaFondiDtM.P690MW.ListaFondi.Count - 1 do
      clbFondi.Items.Add(P690FStampaFondiDtM.P690MW.ListaFondi[i]);
    //Seleziono i fondi in clbFondi in base alla ListaFondiSel che arriva dal B028
    if TipoModulo = 'COM' then
      for i:=0 to clbFondi.Items.Count - 1 do
        clbFondi.Checked[i]:=R180InConcat(Trim(Copy(clbFondi.Items[i],1,Pos(' - ',clbFondi.Items[i]) - 1)),ListaFondiSel);
  end;
end;
...
begin
  for i:=0 to clbFondi.Count - 1 do
    ...
    begin
      if s <> '' then
        s:=s + ',';
      s:=s + '''' + Trim(Copy(clbFondi.Items[i],1,Pos(' - ',clbFondi.Items[i]) - 1)) + '''';
    end;
  if TipoModulo = 'CS' then
    if s = '' then
  ...
  begin
    ...
    selP688.Close;
    selP688.SetVariable('INIZIO',StrToDate(edtDecorrenzaDa.Text));
    selP688.SetVariable('FINE',StrToDate(edtDecorrenzaA.Text));
  ...
  begin
    ...
    if (TipoModulo = 'COM') then
    begin
      if (Trim(DocumentoPDF) <> '') and (Trim(DocumentoPDF) <> '<VUOTO>') then
      begin
