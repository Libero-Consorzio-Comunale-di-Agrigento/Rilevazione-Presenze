...
type
  TA026FDatiLIberiDtM1 = class(TDataModule)
    ...
    I500STORICIZZABILE: TStringField;
    ...
    procedure I500AfterScroll(DataSet: TDataSet);
  private
...
begin
  ...
  I500.FieldByName('STORICIZZABILE').AsString:='S';
...
begin
  ...
  if A026FDatiLiberi.chkObbligatorio.Checked and (A026FDatiLiberi.edtValDefault.Text = '') then
    raise Exception.Create('Se il dato è obbligatorio è richiesto un valore di default valido!');

  A026FDatiLiberiMW.DatoDefault:=A026FDatiLiberi.edtValDefault.Text;
  A026FDatiLiberiMW.DatoObbligatorio:=A026FDatiLiberi.chkObbligatorio.Checked;
  ...
  begin
    ...
    if Trim(A026FDatiLiberi.edtValDefault.Text) <> A026FDatiLiberiMW.GetDatoDefault(I500.FieldByName('NOMECAMPO').AsString) then
      A026FDatiLiberiMW.EditDatoDefault;
    //Modifica dato default
    if A026FDatiLiberi.chkObbligatorio.Checked <> A026FDatiLiberiMW.GetDatoObbligatorio(I500.FieldByName('NOMECAMPO').AsString) then
  ...
  begin
    ...
    if Trim(A026FDatiLiberi.edtValDefault.Text) <> '' then
      A026FDatiLiberiMW.EditDatoDefault;
    if A026FDatiLiberi.chkObbligatorio.Checked then
      A026FDatiLiberiMW.EditDatoObbligatorio;
...
procedure TA026FDatiLIberiDtM1.I500AfterScroll(DataSet: TDataSet);
begin
  if (A026FDatiLiberiMW <> nil) and (I500.State in [dsBrowse]) then
  begin
    A026FDatiLiberi.chkObbligatorio.Checked:=A026FDatiLiberiMW.GetDatoObbligatorio(I500.FieldByName('NOMECAMPO').AsString);
    A026FDatiLiberi.edtValDefault.Text:=A026FDatiLiberiMW.GetDatoDefault(I500.FieldByName('NOMECAMPO').AsString);
  end
  else if not(I500.State in [dsBrowse]) then
  begin
    A026FDatiLiberi.chkObbligatorio.Checked:=False;
    A026FDatiLiberi.edtValDefault.Text:='';
  end;
end;
