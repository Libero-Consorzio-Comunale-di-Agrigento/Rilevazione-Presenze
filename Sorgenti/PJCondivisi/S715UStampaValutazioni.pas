...
uses
  ...
  A000UInterfaccia, C180FunzioniGenerali, USelT004, Oracle, OracleData, DB, Math, StrUtils;
...
type
  TS715FStampaValutazioni = class(TR002FQRep)
    ...
    qlblDataAccettazioneValutato: TQRLabel;
    bndScaglioniIncentivi: TQRChildBand;
    dlblScaglioniIncentivi: TQRDBText;
    qlblTitoloScaglioniIncentivi: TQRLabel;
    ...
    procedure bndScaglioniIncentiviBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
  private
...
begin
  ...
  dlblScaglioniIncentivi.Font.Size:=dlblDescItem.Font.Size;
...
var EsisteFaseAssegnazionePreventivaObiettivi:Boolean;
...
begin
  ...
  begin
    ...
    qlblAccettazioneValutatoSiNo.Caption:=IfThen(cdsStampaAnagrafico.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'N','NO',
                                          IfThen(cdsStampaAnagrafico.FieldByName('ACCETTAZIONE_VALUTATO').AsString = 'S','SI',''));
    qlblDataAccettazioneValutato.Caption:=IfThen(cdsStampaAnagrafico.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime > 0,'(' + FormatDateTime('dd/mm/yyyy hh:nn',cdsStampaAnagrafico.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime) + ')');
  end;
end;
...
procedure TS715FStampaValutazioni.bndScaglioniIncentiviBeforePrint(Sender: TQRCustomBand; var PrintBand: Boolean);
begin
  inherited;
  qlblTitoloScaglioniIncentivi.Caption:=S715FStampaValutazioniDtM.RecuperaEtichetta('SCAGLIONI_INCENTIVI_S','U');
  PrintBand:=Trim(S715FStampaValutazioniDtM.cdsStampaAnagrafico.FieldByName('SCAGLIONI_INCENTIVI').AsString) <> '';
end;
