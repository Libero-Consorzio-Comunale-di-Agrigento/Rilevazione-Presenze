...
type
  TA146FFotoDipendente = class(TForm)
    ...
    edtPercorso: TDBEdit;
    btnCaricaSuDB: TToolButton;
    ToolButton3: TToolButton;
    btnScaricaDaDB: TToolButton;
    ...
    procedure btnCaricaSuDBClick(Sender: TObject);
    procedure FormCreate(Sender: TObject);
    procedure btnScaricaDaDBClick(Sender: TObject);
  private
    OS:TOracleSession;
...
procedure TA146FFotoDipendente.FormCreate(Sender: TObject);
begin
  //(*
...
begin
  ...
  btnScaricaDaDB.Enabled:=btnCancella.Enabled;
end;
...
begin
  if Image1.Picture.Width + Image1.Picture.Height > 0 then
    StatusBar.Panels[0].Text:='Dimensione foto (pixel): ' + IntToStr(Image1.Picture.Width) + 'x' + IntToStr(Image1.Picture.Height)
  else
...
procedure TA146FFotoDipendente.btnCaricaSuDBClick(Sender: TObject);
begin
  if R180MessageBox('Vuoi effettuare il caricamento delle foto su database?',DOMANDA) = mrYes then
  begin
    Screen.Cursor:=crHourGlass;
    try
      try
        A146FFotoDipendenteDtM.A146FMW.CopiaTuttoSuDatabase;
        R180MessageBox('Operazione completata',INFORMA);
      except
        on E: Exception do
        begin
          R180MessageBox(Format('Errore durante l''elaborazione: %s (%s)',[E.Message,E.ClassName]),INFORMA);
        end;
      end;
    finally
...
procedure TA146FFotoDipendente.btnScaricaDaDBClick(Sender: TObject);
begin
    SavePictureDialog.FileName:=Matricola + '_' + Cognome + '_' + Nome;
    PreparaFileJPGorBMP;
    if SavePictureDialog.Execute then
    begin
      if FileExists(SavePictureDialog.FileName)  then
      begin
        if MessageDlg(R180GetFileName(SavePictureDialog.FileName) + ' è già esistente.' + CRLF + 'Sostituire il file?', mtWarning, [mbYes, mbNO], 0) = mrYes then
  ...
  begin
    ...
    begin
      try
        ...
        btnScaricaDaDB.Enabled:=btnModifica.Enabled;
        TestoStatus;
      except
        try
...
begin
  btnCaricaSuDB.Visible:=(Parametri.Operatore = 'MONDOEDP');
...
procedure TA146FFotoDipendente.PreparaFileJPGorBMP;
begin
  if Image1.Picture.graphic is TJPEGimage then
  begin
    SavePictureDialog.Filter:='Jpeg (*.jpg)|*.jpg';
