...
type
  TS715FDialogStampa = class(TForm)
    ...
    procedure rgpStatoAvanzamentoClick(Sender: TObject);
  private
    lstLegendaPunteggi,lstScaglioniIncentivi,lstPresaVisione: TStringList;
...
begin
  ...
  S715FStampaValutazioni.QRep.PrinterSettings.UseStandardPrinter:=(TipoModulo = 'COM') and Parametri.UseStandardPrinter;
...
procedure TS715FDialogStampa.AggiornaStatoAvanzamento;
...
begin
  ...
  try
    ...
    listSG746:=S715FStampaValutazioniDtM.S715FStampaValutazioniMW.ListSG746(DataI,DataF,StatiSel);
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Close;
        SetVariable('DECORRENZA',DataRif);
        SetVariable('DECORRENZA_FINE',DataRif);
        Open;
      end;
      ...
      //Verifico se la scala dei punteggi concorre al calcolo del Punteggio scheda
      ...
      and not PresenzaAnomalie
      and not (chkRiapriScheda.Checked or chkRetrocediStato.Checked) then
        AggiornaPunteggi;
      ...
      if chkAggiornaPunteggi.Checked
      and not PresenzaAnomalie
      and (chkRiapriScheda.Checked or chkRetrocediStato.Checked) then
        AggiornaPunteggi;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          TotPerc:=TotPerc + FieldByName('PERC_VALUTAZIONE').AsFloat;
        CodArea:=FieldByName('COD_AREA').AsString;
        (*Chiamata 115985: Remmato perché in fase di chiusura non possono esserci per nessun motivo elementi valutabili senza punteggio, mentre in avanzamento di stato sì (es. punteggio assegnabile in uno stato successivo)
        Punteggi_Abilitati:=   ((FieldByName('TIPO_VALUTAZIONE').AsString = 'A') and (VarToStr(S715FStampaValutazioniMW.selSG746.Lookup('CODREGOLA;CREA_AUTOVALUTAZIONE',VarArrayOf([selSG710.FieldByName('CODREGOLA').AsString,'S']),'CREA_AUTOVALUTAZIONE')) = 'S'))
                            or (ProprietaArea(FieldByName('COD_AREA').AsString,DataRif,'PUNTEGGI_ABILITATI') = 'S');*)
        if (FieldByName('VALUTABILE').AsString = 'S')
        (*and Punteggi_Abilitati*)
...
procedure TS715FDialogStampa.rgpStatoAvanzamentoClick(Sender: TObject);
begin
  AggiornaStatoAvanzamento;
  AbilitaComponenti;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      //Non aggiorno gli incentivi se la scala dei punteggi non concorre al calcolo del Punteggio scheda
      if not CalcoloPFP then
      begin
        RegistraMsg.InserisciMessaggio('A',FormattaAnomalie(chkAggiornaIncentivi,'La scala dei punteggi della scheda non concorre al calcolo del Punteggio scheda!'),'',FieldByName('PROGRESSIVO').AsInteger);
        Exit;
      end;
    end;
    try
      ...
      begin
        ...
        //Solo per il Saldo annuale collettivo valutativo (D) e per il Saldo annuale collettivo con valutazione non obbligatoria (N),
        //verifico se il dipendente rientra nei gruppi della quota selezionata almeno per un periodo storico nel periodo di valutazione
        ...
        begin
          ...
          selT770.SetVariable('DATARIF',selSG710.FieldByName('AL').AsDateTime);//Alla data di fine periodo di valutazione (tanto le storicizzazioni sono annuali)
          selT770.Open;
          //Esco dal ciclo se non è Saldo annuale collettivo valutativo né Saldo annuale collettivo con valutazione non obbligatoria
          //o se il dipendente rientra nei gruppi, altrimenti passo alla decorrenza precedente
          if not R180In(TipoQuota,['D','N'])
          or (QSGruppoValutatore.RecNo = 1)
          or (selT770.FieldByName('NUM').AsInteger > 0) then
            Break
          else
        ...
        if R180In(TipoQuota,['D','N']) then //Saldo annuale collettivo valutativo o Saldo annuale collettivo con valutazione non obbligatoria
        begin
        ...
        //Recupero la percentuale di incentivo, alla data di valutazione, in base al Punteggio scheda
        with selSG735b do
        begin
          ...
          begin
            RegistraMsg.InserisciMessaggio('A',FormattaAnomalie(chkAggiornaIncentivi,'Il Punteggio scheda (' + FloatToStr(PFP) + ') non rientra tra gli scaglioni di assegnazione degli incentivi!'),'',selSG710.FieldByName('PROGRESSIVO').AsInteger);
            GoTo QuotaSaltata;//Attenzione: è una GoTo label!
          end;
        end;
        with selT775 do
...
var EsisteFaseAssegnazionePreventivaObiettivi,FaseAssegnazionePreventivaObiettiviTerminata,
    Fase1StampaDirigenzaConObiettivi,Fase2StampaDirigenzaConObiettivi,PunteggiAssegnati,
    StampaScaglioniIncentivi,StampaPresaVisioneValutato:Boolean;
    ...
    NValutatori,ProgValutatore,LungDescPFP:Integer;
...
begin
  ...
  begin
    ...
    begin
      ...
      FieldByName('STATO_SCHEDA').AsString:=IfThen(selSG741.FieldByName('CODICE').IsNull or S715FStampaValutazioniMW.selSG746.FieldByName('CODICE').IsNull,'Non valutabile: Regole non trovate',
      ...
      StampaScaglioniIncentivi:=not selSG741.FieldByName('CODTIPOQUOTA_STAMPA').IsNull
                                and (selSG710.FieldByName('CHIUSO').AsString = 'S')
                                and (selSG710.FieldByName('VALUTABILE').AsString = 'S')
                                and (FieldByName('CALCOLO_PFP').AsString = 'S');
      lstScaglioniIncentivi.Clear;
      if StampaScaglioniIncentivi then
      begin
        ...
        R180SetVariable(selSG735,'DATA_VAL',selSG710.FieldByName('DATA').AsDateTime);
        with selSG735 do
        begin
          Open;
          First;
          ...
          while not Eof do
          begin
            if (selSG710.FieldByName('PUNTEGGIO_FINALE_PESATO').AsFloat >= selSG735.FieldByName('PUNTEGGIO_DA').AsFloat)
            and (selSG710.FieldByName('PUNTEGGIO_FINALE_PESATO').AsFloat <= selSG735.FieldByName('PUNTEGGIO_A').AsFloat) then
        ...
        FieldByName('SCAGLIONI_INCENTIVI').AsString:=lstScaglioniIncentivi.Text;
      end;
      StampaPresaVisioneValutato:=R180InConcat('6',selSG741.FieldByName('CAMPI_OPZIONALI_STAMPA').AsString);
      if chkPresaVisione.Checked or StampaPresaVisioneValutato then
      begin
        ...
        begin
          ...
          Filter:='(PROGRESSIVO = ' + IntToStr(selSG710.FieldByName('PROGRESSIVO').AsInteger) + ')' +
                  ...
                  IfThen(StampaPresaVisioneValutato and not chkPresaVisione.Checked,' AND (PROG_UTENTE = PROGRESSIVO)');
          Filtered:=True;
          First;
          while not Eof do
          begin
      ...
      FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime:=selSG710.FieldByName('DATA_ACCETTAZIONE_VALUTATO').AsDateTime;
      if Fase1StampaDirigenzaConObiettivi or Fase2StampaDirigenzaConObiettivi then
  ...
  begin
    ...
    try
      ...
      try
        ...
        S715FStampaValutazioni.QRep.QRPrinter.Free;
        S715FStampaValutazioni.QRep.QRPrinter:=nil;
      end;
...
begin
  ...
  begin
    ...
    try
      dataNow:=FormatDateTime('yyyymmddhhnnss',IncMinute(Now,1));
      ...
      O:=TTimeZone.Local.UTCOffset.Hours; //Per elaborazione notturna gestito UTC+1(ora solare) o UTC+2(ora legale)
      ...
      begin
        ...
        PI.receptionSendingDate.HourOffset:=O;
      end;
      ...
      begin
        ...
        PI.documentDetails.date.HourOffset:=O;
      end;
      ...
      begin
        ...
        rl[0].referenceDate.HourOffset:=O;
      end;
      ...
      il[0]:=dossier.Create;
      s_app:=GetDatoProtocollo('dossierList.number');
      s_app:=StringReplace(s_app,'<#>MATRICOLA<#>',C700SelAnagrafe.FieldByName('MATRICOLA').AsString,[rfReplaceAll]);
      il[0].number:=StrToInt(s_app);
      il[0].officeCode:=GetDatoProtocollo('dossierList.officeCode');
      il[0].registryCode:=GetDatoProtocollo('dossierList.registryCode');
      PI.dossierList:=il;
      try
