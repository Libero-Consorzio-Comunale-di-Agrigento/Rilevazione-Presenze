...
uses
  ...
  DB, CheckLst, Variants,Mask, SelAnagrafe, StrUtils,
  System.Actions, System.ImageList, System.UITypes, L021Call;
...
type
  TA008FProfili = class(TForm)
    ...
    dchkModificaPersonaleEsterno: TDBCheckBox;
    ...
    drgpWebRegistraConsegnaVal: TDBRadioGroup;
    chkPasswordDipendenti: TDBCheckBox;
    chkPasswordOperatori: TDBCheckBox;
    N1: TMenuItem;
    mnuRegoleAccessoIP: TMenuItem;
    dchkModificaDecorrenza: TDBCheckBox;
    chkSelezioneRichiestaPortale: TCheckBox;
    lblAnag: TLabel;
    lblAnagP430: TLabel;
    dchkInserimentoMatricoleP430: TDBCheckBox;
    dchkStoricizzazioneP430: TDBCheckBox;
    dchkModificaDecorrenzaP430: TDBCheckBox;
    dchkEliminaStoriciP430: TDBCheckBox;
    dchkAccessibilitaNonVedenti: TDBCheckBox;
    dchkAutoPianificazione: TDBCheckBox;
    dchkStoriaInizioFine: TDBCheckBox;
    dChkDownloadMassivo: TDBCheckBox;
    imglstBtn: TImageList;
    btnAggiornaFiltroFunzioni: TBitBtn;
    ...
    procedure cmbFiltroFunzioniChange(Sender: TObject);
  private
...
begin
  PageControl1.ActivePage:=tabFiltroDizionario;
...
procedure TA008FProfili.CaricaCmbFiltroFunzioni;
...
begin
  ...
  cmbFiltroFunzioni.Enabled:=((Azione = actConferma) or (Azione = actAnnulla));
  edtContenuto.Enabled:=((Azione = actConferma) or (Azione = actAnnulla));
  btnAggiornaFiltroFunzioni.Enabled:=((Azione = actConferma) or (Azione = actAnnulla));
  ...
  if True then
  begin
    if A008FOperatoriDtM1.BrowseProfili then
      DBGrid2.Options:=DBGrid2.Options + [dgTitleClick]
    else
      DBGrid2.Options:=DBGrid2.Options - [dgTitleClick];
  end;
end;
...
begin
  ...
  mnuRegoleAccessoIP.Enabled:=LTag = 451;
end;
...
begin
  ...
  PageControl1.ActivePage.Visible:=PageControl1.ActivePage.TabVisible;
...
procedure TA008FProfili.btnAggiornaFiltroFunzioniClick(Sender: TObject);
begin
  A008FOperatoriDtM1.AggiornaI073;
  CaricaCmbFiltroFunzioni;
end;
...
begin
  ...
  begin
    ...
    chkSelezioneRichiestaPortale.Enabled:=True;
  end
  else if PageControl1.ActivePage = tabFiltroFunzioni then
  begin
    selCorrente.Refresh;
...
begin
  ...
  begin
    ...
    chkSelezioneRichiestaPortale.Enabled:=False;
  end
...
begin
  if cmbDizionario.Text <> 'ANOMALIE DEI CONTEGGI' then
    with A008FOperatoriDtM1.A181MW.selDizionario do
    begin
      Filtered:=False;
...
procedure TA008FProfili.cmbFiltroFunzioniChange(Sender: TObject);
begin
  with A008FoperatoriDtm1.selI073 do
  try
    DisableControls;
...
begin
  if Espandi then
    ...
    memoFiltroAnagrafe.Height:=Panel1.Height - memoFiltroAnagrafe.Top -1;
end;
...
procedure TA008FProfili.DBGrid2TitleClick(Column: TColumn);
begin
  if actConferma.Enabled then
    exit;

  with A008FOperatoriDtM1.selI073 do
  begin
    if VarToStr(GetVariable('COLONNA_ORDER')) = Column.FieldName then
      SetVariable('COLONNA_ORDER','null')
    else
      SetVariable('COLONNA_ORDER',Column.FieldName);
    Refresh;
  end;
end;
