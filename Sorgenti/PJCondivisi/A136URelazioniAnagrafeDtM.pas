...
begin
  ...
  inherited;
  A136FRelazioniAnagrafe.SegnalaAnomaliaOrdine; //Quando confermo un intervento sulla relazione
end;
...
begin
  inherited;
  selI030.FieldByName('D_TIPO').AsString:=A136FRelazioniAnagrafeMW.DescrizioneTipo(selI030.FieldByName('TIPO').AsString);
...
begin
  ...
  begin
    if not selI030.FieldByName('TABELLA').IsNull and not Sender.IsNull then
    begin
      SelCols.Close;
      ...
      if VarToStr(SelCols.Lookup('COLUMN_NAME',Sender.AsString,'COLUMN_NAME')) <> Sender.AsString then
        raise Exception.Create(Format(A000MSG_ERR_FMT_DATO_ELEM_LISTA,[Sender.DisplayLabel]));
...
begin
  ...
  if not A136FRelazioniAnagrafeMW.VerificaCampiAbilitati(SolaLettura) then
    raise Exception.Create(A000MSG_A136_ERR_UTENTE_NO_DELETE);
  A136FRelazioniAnagrafe.memRelazione.Clear;
end;
...
begin
  inherited;
  selI030COLONNAValidate(selI030COLONNA);
