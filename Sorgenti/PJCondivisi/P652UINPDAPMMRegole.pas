...
uses
  ...
  A000UMessaggi, P651URelazioniTabelle, System.ImageList;
...
type
  TP652FINPDAPMMRegole = class(TR004FGestStorico)
    ...
    pcRegole: TPageControl;
    TabSheet1: TTabSheet;
    TabSheet2: TTabSheet;
    dmemRegolaCalcoloEccezioni: TDBMemo;
    btnRelazioni: TBitBtn;
    ...
    procedure btnRelazioniClick(Sender: TObject);
  private
...
begin
  ...
  P652FINPDAPMMRegole.Width:=800;
  dmemRegolaCalcoloEccezioni.Color:=cl3DLight; //il colore rimane sempre uguale perché è un campo di sola lettura
end;
...
procedure TP652FINPDAPMMRegole.btnRelazioniClick(Sender: TObject);
begin
  inherited;
  with P652FINPDAPMMRegoleDtM do
  begin
    if (P660.FieldByName('NUMERO').AsString = '008') or (P660.FieldByName('NUMERO').AsString = '009') or (P660.FieldByName('NUMERO').AsString = '092') then
      OpenP651RelazioniTabelle('FL','FLUPER_STR')
    else if (P660.FieldByName('NUMERO').AsString = '010') or (P660.FieldByName('NUMERO').AsString = '011') or (P660.FieldByName('NUMERO').AsString = '012') or (P660.FieldByName('NUMERO').AsString = '013') then
      OpenP651RelazioniTabelle('FL','FLUPER_DIP')
    else
      OpenP651RelazioniTabelle('FL','FLUPER_UO');
  end;
end;
...
begin
  inherited;
  AbilitaFiltro; //rimane disabilitato (cfr. Cloud)
  btnStoricizza.Enabled:=False;
  if DButton.State in [dsInsert, dsEdit] then
  begin
    BtnCausaliFLUPER.Enabled:=not ((DButton.Dataset.FieldByName('NOME_FLUSSO').AsString = 'FLUPER') and (DButton.Dataset.FieldByName('PARTE').AsString = 'A') and (DButton.Dataset.FieldByName('NUMERO').AsString = '005'));
    dEdtCausaliFLUPER.Enabled:=(DButton.Dataset.FieldByName('NOME_FLUSSO').AsString = 'FLUPER') and (DButton.Dataset.FieldByName('PARTE').AsString = 'A') and (DButton.Dataset.FieldByName('NUMERO').AsString = '005');
  ...
  begin
    ...
    dEdtCausaliFLUPER.Enabled:=False;
