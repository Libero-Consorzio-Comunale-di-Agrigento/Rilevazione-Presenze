...
uses
  ...
  ExtCtrls, C012UVisualizzaTesto, C013UCheckList, C015UElencoValori, System.Actions, A000UMessaggi;
...
type
  TS700FAreeValutazioni = class(TR004FGestStorico)
    ...
    procedure drgpTipoPunteggioItemsChange(Sender: TObject);
  private
...
begin
  ...
  begin
    ...
    begin
      if Area = '' then
        Area:=FieldByName('COD_AREA').AsString;
  ...
  if S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.Active then
    S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.SearchRecord('COD_VALUTAZIONE',Valutazione,[srFromBeginning]);
  try
...
begin
  ...
  dgrdDettaglio.DataSource:=S700FAreeValutazioniDtM.dsrSG700;
  ...
  SetTabelleRelazionate([S700FAreeValutazioniDtM.selSG701,S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700]);
//  NumRecords;
  AbilitaComponenti;
end;
...
begin
  ...
  (*try
    ...
    begin
      ...
      if S700FAreeValutazioniDtM.selSG701.State <> dsBrowse then
        S700FAreeValutazioniDtM.selSG701.Cancel;
...
procedure TS700FAreeValutazioni.pmnDettaglioPopup(Sender: TObject);
begin
  inherited;
  mniCopiaDettaglio.Enabled:=(DButton.State = dsBrowse)
                         and (dgrdDettaglio.DataSource.State = dsBrowse)
                         and (S700FAreeValutazioniDtM.selSG701.FieldByName('TIPO_PESO_PERCENTUALE').AsString = '1')
                         and (S700FAreeValutazioniDtM.selSG701.FieldByName('TIPO_LINK_ITEM').AsString = '0');
end;

procedure TS700FAreeValutazioni.actCopiaDettaglioExecute(Sender: TObject);
begin
  inherited;
  with S700FAreeValutazioniDtM.S700FAreeValutazioniMW do
  begin
    AreeCopiaDettaglio;
    ...
    try
      ...
      begin
        Caption:='<C013> Aree con decorrenza ' + SG701_Funzioni.FieldByName('DECORRENZA').AsString;
        with selSG701a do
          while not Eof do
          begin
            clbListaDati.Items.Add(Format('%-5s %s',[FieldByName('COD_AREA').AsString,FieldByName('DESCRIZIONE').AsString]));
            Next;
          end;
        ShowModal;
      end;
      ...
      if C013FCheckList.ModalResult = mrOk then
        AreeSel:=Trim(R180GetCheckList(5,C013FCheckList.clbListaDati));
      FreeAndNil(C013FCheckList);
    end;
    if AreeSel = '' then
      abort;
    if R180MessageBox(Format(A000MSG_S700_DLG_FMT_COPIA_DETT,[SG701_Funzioni.FieldByName('COD_AREA').AsString,AreeSel,SG701_Funzioni.FieldByName('DECORRENZA').AsString]),DOMANDA) <> mrYes then
      abort;
    CopiaDettaglio;
  end;
...
begin
  QueryStampa.Clear;
  QueryStampa.Add('SELECT t1.cod_area, t1.descrizione, t1.decorrenza, t1.decorrenza_fine, t1.peso_percentuale, t1.peso_variabile_items, t1.tipo_punteggio_items,');
  QueryStampa.Add('       t2.cod_valutazione, t2.descrizione, t2.peso_percentuale, t2.cod_area_link, t2.cod_valutazione_link, t2.descrizioni_punteggi');
  ...
  NomiCampiR001.Add('t2.peso_percentuale');
  NomiCampiR001.Add('t2.cod_area_link');
  NomiCampiR001.Add('t2.cod_valutazione_link');
  NomiCampiR001.Add('t2.descrizioni_punteggi');
  inherited;
end;
...
begin
  ...
  try
    with C013FCheckList do
    begin
      with S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG746 do
      begin
        SetVariable('DECORRENZA',S700FAreeValutazioniDtM.selSG701.FieldByName('DECORRENZA').AsDateTime);
        SetVariable('DECORRENZA_FINE',S700FAreeValutazioniDtM.selSG701.FieldByName('DECORRENZA_FINE').AsDateTime);
        Open;
        while not Eof do
        begin
    ...
    if State in [dsEdit,dsInsert] then
      ...
      begin
        ...
        and (   (StrToIntDef(dedtItemPersonalizzatiMax.Text,0) > 0)
             or (    S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.Active
                 and (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount > 0))) then
...
begin
  ...
  if S700FAreeValutazioniDtM.dsrSG700.State in [dsBrowse] then
    exit;
  with S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700 do
  begin
    case dgrdDettaglio.SelectedIndex of
      3,4: //Codice area link o Codice elemento link
      begin
        vCodice:=VarArrayOf([FieldByName('COD_AREA_LINK').AsString,FieldByName('COD_VALUTAZIONE_LINK').AsString]);
        OpenC015FElencoValori('SG700_VALUTAZIONI','<S700> Selezione dell''elemento collegato',S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selLinkItem.SQL.Text,'COD_AREA;COD_VALUTAZIONE',vCodice,S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selLinkItem,700);
      ...
      6: //Descrizioni punteggi
      begin
        Str:=TStringList.Create;
        try
          Str.Text:=FieldByName('DESCRIZIONI_PUNTEGGI').AsString;
          OpenC012VisualizzaTesto(FieldByName('DESCRIZIONI_PUNTEGGI').DisplayLabel,'',Str,'',[mbOK,mbCancel]);
          FieldByName('DESCRIZIONI_PUNTEGGI').AsString:=Trim(Str.Text);
        finally
          FreeAndNil(Str);
        end;
      end;
...
procedure TS700FAreeValutazioni.drgpTipoPunteggioItemsChange(Sender: TObject);
begin
  inherited;
  AbilitaComponenti;
end;
...
begin
  if R180MessageBox(A000MSG_S700_MSG_CANC_VALUT,'DOMANDA') = mrYes then
    inherited;
end;

procedure TS700FAreeValutazioni.AbilitaComponenti;
begin
  if not S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.Active then
    exit;
  lblPesoPercentuale.Enabled:=not ((drgpTipoPesoPercentuale.ItemIndex = 0) and (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount > 0)) or dchkPesoEquoItems.Checked;
  ...
  dchkTipoLinkItem.Enabled:=S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount = 0;
  ...
  //dchkPesoVariabileItems.Enabled:=((StrToFloatDef(dedtPesoPercentuale.Text,0) > 0) or (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount = 0)) and (dchkTipoLinkItem.ItemIndex <> 1);
  //dchkPesoVariabileItems.Enabled:=((S700FAreeValutazioniDtM.selSG701.FieldByName('PESO_PERCENTUALE').AsFloat > 0) or (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount = 0)) and (dchkTipoLinkItem.ItemIndex <> 1) and (not dchkPesoEquoItems.Checked or (StrToIntDef(dedtItemPersonalizzatiMax.Text,0) > 0));
  ...
  //dchkPesoEquoItems.Enabled:=((StrToFloatDef(dedtPesoPercentuale.Text,0) > 0) or (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount = 0)) and (dchkTipoLinkItem.ItemIndex <> 1) and (not dchkPesoVariabileItems.Checked or (StrToIntDef(dedtItemPersonalizzatiMax.Text,0) > 0));
  //dchkPesoEquoItems.Enabled:=((S700FAreeValutazioniDtM.selSG701.FieldByName('PESO_PERCENTUALE').AsFloat > 0) or (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount = 0)) and (dchkTipoLinkItem.ItemIndex <> 1) and (not dchkPesoVariabileItems.Checked or (StrToIntDef(dedtItemPersonalizzatiMax.Text,0) > 0));
  ...
  dchkItemTuttiValutabili.Enabled:=(dchkTipoLinkItem.ItemIndex <> 1) and not ((S700FAreeValutazioniDtM.selSG701.FieldByName('PESO_PERCENTUALE').AsFloat = 0) and not dchkPesoVariabileItems.Checked and ((StrToIntDef(dedtItemPersonalizzatiMax.Text,0) > 0) or (S700FAreeValutazioniDtM.S700FAreeValutazioniMW.selSG700.RecordCount > 0)));
  ...
  dgrdDettaglio.Columns[1].Width:=IfThen(R180In(dchkTipoLinkItem.ItemIndex,[1,2]),210,543) + IfThen(dchkPesoEquoItems.Checked,46) + IfThen(drgpTipoPunteggioItems.ItemIndex <> 0,101);
  dgrdDettaglio.Columns[2].Width:=45;
  dgrdDettaglio.Columns[3].Width:=50;
  dgrdDettaglio.Columns[4].Width:=50;
  dgrdDettaglio.Columns[5].Width:=230;
  dgrdDettaglio.Columns[6].Width:=100;
  ...
  dgrdDettaglio.Columns[6].Visible:=drgpTipoPunteggioItems.ItemIndex = 0;
end;
