...
type
  TP652FINPDAPMMRegoleDtM = class(TR004FGestStoricoDtM)
    ...
    P660REGOLA_CALCOLO_ECCEZIONI: TStringField;
...
begin
  ...
  P652FINPDAPMMRegole.btnRelazioni.Enabled:=P652FINPDAPMMRegoleMW.CanRelazioniTabelle;
  if (P660.FieldByName('NOME_FLUSSO').AsString = 'FLUPER') and (P660.FieldByName('PARTE').AsString = 'A') and (P660.FieldByName('NUMERO').AsString = '005') then
    P652FINPDAPMMRegole.lblCausali.Caption:='T.Indet.'
  else
    P652FINPDAPMMRegole.lblCausali.Caption:='Causali';
end;
