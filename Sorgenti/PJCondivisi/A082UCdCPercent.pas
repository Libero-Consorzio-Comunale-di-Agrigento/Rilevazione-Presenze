...
uses
  ...
  ImgList, ToolWin, Mask, A000UMessaggi, System.Actions, System.ImageList;
...
begin
  ...
  begin
    R180SetVariable(selT433,'Progressivo',C700Progressivo);
    if not selT433.Active then
    begin
      selT433.Open;
      A082FCdcPercentMW.cdsT433.EmptyDataSet;
      A082FCdcPercentMW.SituazioneModificata:=False;
      Self.Messaggio:=False;
      Self.Controlli;
      Self.Messaggio:=True;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      if R180Between(Parametri.DataLavoro,selT433.FieldByName('DECORRENZA').AsDateTime,selT433.FieldByName('DECORRENZA_FINE').AsDateTime) then
      begin
...
procedure TA082FCdcPercent.DButtonDataChange(Sender: TObject; Field: TField);
begin
  if not DButton.DataSet.ControlsDisabled then
    inherited;
end;
