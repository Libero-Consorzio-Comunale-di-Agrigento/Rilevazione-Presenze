...
{$ENDIF}
  ...
  begin
    if Components[i] is TOracleQuery then
      (Components[i] as TOracleQuery).Session:={$IFNDEF ISO}SessioneOracle{$ELSE}DBInterno{$ENDIF};
    if Components[i] is TOracleDataSet then
      (Components[i] as TOracleDataSet).Session:={$IFNDEF ISO}SessioneOracle{$ELSE}DBInterno{$ENDIF};
  end;
