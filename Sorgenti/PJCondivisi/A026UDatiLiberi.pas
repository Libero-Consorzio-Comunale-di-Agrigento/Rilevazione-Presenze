...
uses
  ...
  ComCtrls, Grids, CheckLst, ActnList, ImgList, ToolWin, A000UCostanti, A000USessione,A000UInterfaccia,A003UDataLavoroBis, Variants, System.Actions,
...
type
  TA026FDatiLIberi = class(TR001FGestTab)
    ...
    edtValDefault: TEdit;
    chkObbligatorio: TCheckBox;
    Label3: TLabel;
    dchkStoricizzabile: TDBCheckBox;
    ...
    procedure chkObbligatorioClick(Sender: TObject);
...
procedure TA026FDatiLIberi.chkObbligatorioClick(Sender: TObject);
begin
  if DButton.State = dsBrowse then
    chkObbligatorio.Checked:=A026FDatiLiberiDtM1.A026FDatiLiberiMW.GetDatoObbligatorio(DButton.DataSet.FieldByName('NOMECAMPO').AsString);
end;
...
begin
  ...
  begin
    ...
    edtValDefault.ReadOnly:=DButton.State = dsBrowse;
    dchkStoricizzabile.Enabled:=not (DButton.State = dsEdit);
  end;
end;
...
procedure TA026FDatiLIberi.FormShow(Sender: TObject);
begin
  inherited;
  chkObbligatorio.Checked:=A026FDatiLIberiDtM1.A026FDatiLiberiMW.GetDatoObbligatorio(A026FDatiLIberiDtM1.I500.FieldByName('NOMECAMPO').AsString);
  edtValDefault.Text:=A026FDatiLIberiDtM1.A026FDatiLiberiMW.GetDatoDefault(A026FDatiLIberiDtM1.I500.FieldByName('NOMECAMPO').AsString);
end;
...
procedure TA026FDatiLIberi.TAnnullaClick(Sender: TObject);
begin
  inherited;
  edtValDefault.Text:=A026FDatiLIberiDtM1.A026FDatiLiberiMW.GetDatoDefault(A026FDatiLIberiDtM1.I500.FieldByName('NOMECAMPO').AsString);
  chkObbligatorio.Checked:=A026FDatiLIberiDtM1.A026FDatiLiberiMW.GetDatoObbligatorio(A026FDatiLIberiDtM1.I500.FieldByName('NOMECAMPO').AsString);
...
begin
  if (DButton.State = dsEdit) and (dchkStorico.Checked) and
