...
uses
  ...
  C015UElencoValori, A000UCostanti, A000USessione, A000UInterfaccia, A000UMessaggi,
  System.Actions;
...
procedure TA136FRelazioniAnagrafe.FormActivate(Sender: TObject);
begin
  inherited;
  if A136FRelazioniAnagrafe.DButton.State = dsBrowse then //(Quando accedo da menu) oppure (Quando accedo da A002 e chiudo la maschera degli abbinamenti ma non posso intervenire sulla relazione)
    SegnalaAnomaliaOrdine;
end;

procedure TA136FRelazioniAnagrafe.SegnalaAnomaliaOrdine;
var s:String;
begin
  RichiamoEsterno:=False; //Per non eseguire nuovamente SegnalaAnomaliaOrdine quando si è giunti da A002 e si effettua un annullamento successivo all'eventuale primo annullamento (quello sulla relazione su cui si è posizionato in fase di accesso)
  Screen.Cursor:=crHourGlass;
  memRelazione.Visible:=False;
  s:=A136FRelazioniAnagrafeDtm.A136FRelazioniAnagrafeMW.ControlloOrdine(SolaLettura);
  memRelazione.Visible:=True;
  Screen.Cursor:=crDefault;
  if s <> '' then
    ShowMessage(s);
end;

procedure TA136FRelazioniAnagrafe.TAnnullaClick(Sender: TObject);
begin
  inherited;
  if RichiamoEsterno then //Quando accedo da A002, chiudo la maschera degli abbinamenti e posso intervenire sulla relazione ma annullo lo stato di modifica
    SegnalaAnomaliaOrdine;
end;
...
begin
  ...
  //
  bBrowse:=DButton.State = dsBrowse;
  bModifica:=(not SolaLettura) and CamAbi and bBrowse;
  bStoricizza:=bModifica and DButton.DataSet.Active and (DButton.DataSet.RecordCount > 0);
  actModifica.Enabled:=bModifica;
  actCancella.Enabled:=bModifica;
  actStoricizza.Enabled:=bStoricizza;
  ...
  btnStampaRelazioni.Enabled:=DButton.State = dsBrowse;
end;
...
begin
  ...
  begin
    ...
    SqlColonne:=StringReplace(SqlColonne,':PILOTA','''N''',[rfReplaceAll]);
