...
uses
  ...
  SelAnagrafe, R001UGESTTAB, ToolbarFiglio, C600USelAnagrafe, A083UMsgElaborazioni,
  RegolePassword, System.Actions, System.Math, System.ImageList, System.StrUtils;
...
type
  TA008FLoginDipendenti = class(TR001FGestTab)
    ...
    actImpostaDatiDaDominio: TAction;
    N7: TMenuItem;
    actImpostaDatiDaDominio1: TMenuItem;
    ...
    procedure actImpostaDatiDaDominioExecute(Sender: TObject);
    ...
    procedure frmToolbarFiglioactTFGenerica2Execute(Sender: TObject);
   private
  ...
  public
    ...
    procedure OnI090AziendaCambiata;
...
begin
  inherited;
  //Aggiornamento elenco degli account esistenti
  A008FOperatoriDtM1.selI060.Close;
  A008FOperatoriDtM1.selI060.SetVariable('AZIENDA',dcmbAzienda.Text);
  A008FOperatoriDtM1.selI060.Open;
  ...
  begin
    if dgrdLoginDipendente.Columns[i].FieldName='NOME_UTENTE' then
    begin
    ...
    if UpperCase(dgrdLoginDipendente.Columns[i].FieldName)='NOME_UTENTE2' then
    begin
      dgrdLoginDipendente.Columns[i].Visible:=(not A008FOperatoriDtM1.QI090.FieldByName('DOMINIO_DIP').IsNull) and
                                              (A008FOperatoriDtM1.QI090.FieldByName('DOMINIO_DIP_TIPO').AsString = 'LDAP');
      if A008FOperatoriDtM1.QI090.FieldByName('DOMINIO_DIP').IsNull then
        dgrdLoginDipendente.Columns[i].ButtonStyle:=cbsAuto
      else
        dgrdLoginDipendente.Columns[i].ButtonStyle:=cbsEllipsis;
  ...
  // BugFix: MONZA_HSGERARDO <114364>
  // la modalità di gestione massiva viene visualizzata solo se l'accesso alla maschera è in scrittura
  PageControl1.Visible:=not SolaLettura;
  PnlDettaglio.Height:=120 + IfThen(SolaLettura,170);

  // azione per impostazione dati da active directory
  actImpostaDatiDaDominio.Visible:=(not SolaLettura) and
                                   (A008FOperatoriDtm1.QI090.FieldByName('DOMINIO_DIP').AsString <> '') and
                                   (Parametri.CampiRiferimento.C90_EmailSincrDominio = 'S');

  actDefault.Visible:=(Parametri.Azienda = 'AZIN') and (Parametri.Operatore = 'MONDOEDP');
  LastFieldDipClick:='';
  TipoOrdQI060:='ASC';
end;
...
//===============================
  ...
  frmToolbarFiglio.lstLock[3]:=dgrdLoginDipendente;
  frmToolbarFiglio.lstLock[4]:=C600frmSelAnagrafe;
  ...
  begin
    dgrdLoginDipendente.Columns.Items[R180GetColonnaDBGrid(dgrdLoginDipendente,'D_PASSWORD')].Color:=clSilver;
    dgrdLoginDipendente.Columns.Items[R180GetColonnaDBGrid(dgrdLoginDipendente,'D_PASSWORD')].ButtonStyle:=cbsNone;
  end;

  btnModificaPassword.Enabled:=Parametri.I060_Password;
end;

procedure TA008FLoginDipendenti.C600frmSelAnagrafebtnEreditaSelezioneClick(Sender: TObject);
begin
  inherited;
...
procedure TA008FLoginDipendenti.OnI090AziendaCambiata;
begin
  // visualizza azione per impostazione dati da active directory
  actImpostaDatiDaDominio.Visible:=(not SolaLettura) and
                                   (A008FOperatoriDtm1.QI090.FieldByName('DOMINIO_DIP').AsString <> '') and
                                   (Parametri.CampiRiferimento.C90_EmailSincrDominio = 'S');
end;
...
begin
  if Trim(cmbNomeProfilo.Text) = '' then
    raise Exception.Create('Specificare il nome del profilo d''aggiornare.');
  if R180MessageBox('Vuoi aggiornare "' + LowerCase(CampoUpdate) + '" con il nuovo valore?' + CRLF +
                    'La modifica riguarderà il solo profilo ' + cmbNomeProfilo.Text,DOMANDA) = mrNo then
    Exit;
  with A008FOperatoriDtM1 do
  begin
    if StrToIntDef(InputBox('Modifica profilo ' + cmbNomeProfilo.Text, 'Inserire il numero di login che verranno modificati (' + QI060.RecordCount.ToString + '):', ''),-1) <> QI060.RecordCount then
      raise Exception.Create('Il numero indicato non corrisponde alla selezione attuale. Operazione annullata.');
...
begin
  ...
  begin
    ...
    if not (R180MessageBox(mex,'DOMANDA') = mrYes) then
      exit;
    if StrToIntDef(InputBox('Eliminazione login', 'Inserire il numero di login da eliminare (' + QI060.RecordCount.ToString + '):', ''),-1) <> QI060.RecordCount then
      raise Exception.Create('Il numero indicato non corrisponde alla selezione attuale. Operazione annullata.');
    Screen.Cursor:=crHourGlass;
    QI060.First;
    ProgressBar1.Position:=0;
...
begin
  R001LinkC700:=False;
  QueryStampa.Clear;
  QueryStampa.Add('SELECT I060.AZIENDA,I060.MATRICOLA,I060.NOME_UTENTE,I060.DATA_PW,decode(I060.PASSWORD,null,''null'',''****'') PASSWORD, I060.EMAIL,I060.EMAIL_PEC,I060.CELLULARE,');
  QueryStampa.Add('       I061.NOME_PROFILO,I061.PERMESSI,I061.FILTRO_FUNZIONI,I061.FILTRO_ANAGRAFE,I061.FILTRO_DIZIONARIO,');
  ...
  NomiCampiR001.Add('decode(I060.PASSWORD,null,''null'',''****'')');
  NomiCampiR001.Add('I060.EMAIL');
  NomiCampiR001.Add('I060.EMAIL_PEC');
  NomiCampiR001.Add('I060.CELLULARE');
...
procedure TA008FLoginDipendenti.frmToolbarFiglioactTFGenerica2Execute(Sender: TObject);
begin
  inherited;
  //Copia filtro profili su Impostazione profili
  with A008FOperatoriDtm1 do
  begin
    if Trim(selI061.FieldByName('NOME_PROFILO').AsString) <> '' then
    begin
      cmbNomeProfilo.Text:=selI061.FieldByName('NOME_PROFILO').AsString;
      cmbPermessi.Text:=selI061.FieldByName('PERMESSI').AsString;
      cmbFiltroAnagrafe.Text:=selI061.FieldByName('FILTRO_ANAGRAFE').AsString;
      cmbFiltroFunzioni.Text:=selI061.FieldByName('FILTRO_FUNZIONI').AsString;
      cmbIterAutor.Text:=selI061.FieldByName('ITER_AUTORIZZATIVI').AsString;
      cmbFiltroDizionario.Text:=selI061.FieldByName('FILTRO_DIZIONARIO').AsString;
    end;
  end;
end;
...
begin
  if R180In(dgrdLoginDipendente.SelectedField.FieldName, ['NOME_UTENTE','NOME_UTENTE2']) then
  begin
    ...
    try
      ...
      begin
        ...
        try
          for locDom in A008FOperatoriDtM1.QI090.FieldByName('DOMINIO_DIP').AsString.Split([';',',']) do
          begin
            lstUsers.Clear;
            try
              A001GetUsers(lstUsers, A001GetDomainControllerName(locDom));
              if lstUsers.Count = 0 then
                A001GetUsers(lstUsers, locDom);
              if lstUsers.Count > 0 then
              begin
                lstDominio.BeginUpdate;
                try
                  lstDominio.AddStrings(lstUsers);
                finally
                  lstDominio.EndUpdate;
                end;
              end;
            except
            end;
...
procedure TA008FLoginDipendenti.dgrdLoginDipendenteTitleClick(Column: TColumn);
var
...
begin
  ...
  try
    (*Alberto 10/01/2019: esportazione senza pwd in chiaro
    ...
    *)
    A008FOperatoriDtM1.QI060.AfterScroll:=nil;
    ...
    *)
    ...
    A008FOperatoriDtM1.QI060.AfterScroll:=A008FOperatoriDtm1.QI060AfterScroll;
    A008FOperatoriDtm1.QI060AfterScroll(A008FOperatoriDtm1.QI060);
  end;
end;

procedure TA008FLoginDipendenti.actImpostaDatiDaDominioExecute(Sender: TObject);
// imposta alcuni dati dell'account leggendo le informazioni da active directory:
// - email
begin
  // se il dominio non è impostato esce subito
  if A008FOperatoriDtm1.QI090.FieldByName('DOMINIO_DIP').AsString = '' then
  begin
    R180MessageBox('Il dominio di autenticazione per i dipendenti web non è indicato!',INFORMA);
    Exit;
  end;
  ...
  begin
    R180MessageBox('La sincronizzazione dei dati con Active Directory non è attiva!',INFORMA);
    Exit;
  end;
  ...
  if A008FOperatoriDtm1.QI060.RecordCount = 0 then
  begin
    R180MessageBox('Nessun account selezionato: verificare la selezione anagrafica!',INFORMA);
    Exit;
  end;

  // richiesta conferma
  if R180MessageBox('Si desidera impostare la mail degli account selezionati'#13#10 +
                    'utilizzando le informazioni estratte da Active Directory?',DOMANDA) = mrNo then
    Exit;
  ...
  try
    ...
    A008FOperatoriDtm1.ImpostaDatiAccountDaDominio;

    // verifica risultato elaborazione
    if RegistraMsg.ContieneAnomalie then
    begin
      if R180MessageBox('Elaborazione terminata con anomalie.'#13#10 +
                        'Si desidera visualizzarle?',DOMANDA) = mrYes then
      begin
        OpenA083MsgElaborazioni(Parametri.Azienda,Parametri.Operatore,'A008','I,A');
      end
    end
    else
    begin
...
begin
  ...
  if not chkApplicaFiltro.Checked then
    raise Exception.Create('Attenzione! E'' necessario attivare ''Filtra i login selezionati''.');
  with A008FOperatoriDTM1 do
  begin
    mex:=A000MSG_A186_DLG_ELIM_PROFILI;
    if not (R180MessageBox(mex,'DOMANDA') = mrYes) then
      exit;
    if StrToIntDef(InputBox('Eliminazione profili', 'Inserire il numero di login di cui eliminare i profili (' + QI060.RecordCount.ToString + '):', ''),-1) <> QI060.RecordCount then
      raise Exception.Create('Il numero indicato non corrisponde alla selezione attuale. Operazione annullata.');
    Screen.Cursor:=crHourGlass;
    QI060.First;
    ProgressBar1.Position:=0;
...
begin
  inherited;
  actTrigger.Enabled:=not (DButton.State in [dsInsert, dsEdit]);
  actDefault.Enabled:=not (DButton.State in [dsInsert, dsEdit]);
