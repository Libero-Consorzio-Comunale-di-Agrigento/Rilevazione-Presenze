...
uses
  //Windows, Messages, Forms, Graphics, Dialogs,
  ...
  P999UGenerale, A000UCostanti, A000UInterfaccia, Crtl, DBClient, Variants;
...
begin
  ...
  if Pos(':Parametro2',OperSql.SQL.Text) > 0 then
  begin
    OperSql.DeclareVariable(':Parametro2',otSubst);
    OperSql.SetVariable(':Parametro2',selP552.FieldByName('PARAMETRO_2').AsString);
  end;
...
begin
  ...
  begin
    ...
    // PRE: le variabili
    // - Anno
    // - ChkElaboraDatiCONTANN_Checked
    // devono essere correttamente valorizzate con i valori presenti nel form di elaborazione
    //P554MW.Anno:=StrToIntDef(edtAnno.Text,0);
    //P554MW.ChkElaboraDatiCONTANN_Checked:=chkElaboraDatiCONTANN.Checked;
    iIdContoAnnuale:=P554MW.TestataCONTANN(sCodTabella);
    //***.fine
  end;
  //Nel caso di tabelle con tipologia righe qualifica ministeriale se la riga di riferimento è 0 do anomalia
  if (DatiInpCONTANN.TipoTabellaRighe = '0') and (iRiga = 0) then
  begin
    DatiOut.Blocca:=BloccaP948[1] + ' Campo = ' + DatiInpCONTANN.ValoreCostante;
    exit;
  end;
  if DatiOut.Blocca <> '' then
  begin
