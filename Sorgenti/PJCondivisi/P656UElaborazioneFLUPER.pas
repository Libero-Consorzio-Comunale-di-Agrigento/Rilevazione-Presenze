...
begin
  ...
  begin
    if P656FElaborazioneFluperMW.VerificaAlternativaParteAK(StrToDate('01/'+edtMeseDa.Text)) = 'K' then
    begin
      chkElaboraDatiFLUSSOA.Caption:='Calcola dati Flusso Kpmg';
      chkElaboraDatiFLUSSOB1_36.Visible:=False;
...
begin
  ...
  begin
    edtMeseDa.SetFocus;
    raise exception.create(A000MSG_ERR_MESE_STESSO_ANNO);
  end;
  if mm < m then
  begin
    edtMeseA.SetFocus;
    raise exception.create(A000MSG_ERR_MESE_DA_A);
  end;
...
begin
  ...
  begin
    ...
    begin
      if R180MessageBox(A000MSG_DLG_SOSTITUIRE, DOMANDA) = mrNo then
       exit
