...
type
  ...
  private
    ...
    FOnTextChange: TTIWMultiColumnComboBoxEvent;
    FOnAsyncTextChange: TIWAsyncMultiColumnComboBoxEvent;
...
//    procedure HookEvents(AContext: TIWPageContext40; AScriptEvents: TIWScriptEvents); override;
    procedure DoAsyncTextChange(AParams: TStringList);
  public
  ...
  published
    ...
    //Caratto 11/12/2012 -ini- Evento su change della text
    property OnTextChange: TTIWMultiColumnComboBoxEvent read FOnTextChange write FOnTextChange;
    property OnAsyncTextChange: TIWAsyncMultiColumnComboBoxEvent read FOnAsyncTextChange write FOnAsyncTextChange;
    //Caratto 11/12/2012 -fine-
  end;
...
procedure TmeTIWMultiColumnComboBox.DoAsyncTextChange(AParams: TStringList);
begin
  if Assigned(OnAsyncTextChange) then
    OnAsyncTextChange(Self, AParams, ItemIndex, Text);
end;
...
var
  ...
  r, c, i: integer;
  gborder, backcolor, forecolor, rowtitle,  itemText, itemValue: string;
begin
...
//       Add('if (ctrl.value == "")');
       Add('  if (obj.ItemSel == false)');
       Add('    FindElem(''' + HTMLName + '_INPUT'').value = "-1~" + ctrl.value;');
...
//      if (BGColor <> clNone) then
       ...
       //CARATTO 12/12/2012 Cambiato id del componente text
//       Add(' FindElem(''' + HTMLName + '_INPUT'').value = index +"~" + newval;');
       ...
       if Assigned(Self.OnTextChange) then
         submitclick := 'return SubmitClick('#39+HTMLName+#39', -1, '#39'false'#39');'#13;

       if Assigned(Self.OnAsyncTextChange) then
       begin
          ajaxcall := 'processAjaxEvent(''onTextChange'', '
            + HTMLControlImplementation.IWCLName
            + ',''' + HTMLName + '.' + 'DoAsyncTextChange' + ''','
            + 'true' + ', null, '
            + 'true' + ');';

         Acontext.WebApplication.RegisterCallBack(HTMLName+'.DoAsyncTextChange', DoAsyncTextChange);
...
//    else
    ...
    //{$ENDIF}
    //CARATTO aggiunto id per Button
    buttonval := '<BUTTON id="'+ HTMLName + 'button" '+IIF(Enabled and not Readonly,'','disabled ')+'onclick="'+HTMLName+'showdivonclick()"'
     + ' style="cursor:default;width:18px;height:22px;vertical-align:middle;text-align:center;padding:0px;top:-3;position:relative;">'
