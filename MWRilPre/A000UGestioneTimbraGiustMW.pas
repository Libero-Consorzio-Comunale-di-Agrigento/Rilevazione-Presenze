...
//        FTimbrature[Col,Row].IDRichiesta:=0;
          FTimbrature[Col,Row].Clear;
      end;
    1:
      begin
...
//        FGiustificativi[Col,Row].IDRichiesta:=0;
//        FGiustificativi[Col,Row].IDCertificato:='';
          FGiustificativi[Col,Row].Clear;
      end;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      inc(j);
      FNumTimbrature[i]:=j;
      FTimbrature[i,j].Progressivo:=QTimbrature.FieldByName('PROGRESSIVO').AsInteger; //+++
      FTimbrature[i,j].Data:=QTimbrature.FieldByName('Data').AsDateTime; //+++
      ...
      if QTimbrature.FindField('ID_RICHIESTA') <> nil then
        FTimbrature[i,j].IDRichiesta:=QTimbrature.FieldByName('ID_RICHIESTA').AsInteger;
    end;
    QTimbrature.Next;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      inc(j);
      FNumGiustif[i]:=j;
      FGiustificativi[i,j].Progressivo:=QGiustificativi.FieldByName('PROGRESSIVO').AsInteger; //+++
      FGiustificativi[i,j].Data:=QGiustificativi.FieldByName('Data').AsDateTime; //+++
      ...
      FGiustificativi[i,j].IDRichiesta:=QGiustificativi.FieldByName('ID_RICHIESTA').AsInteger;
      FGiustificativi[i,j].IDCertificato:=QGiustificativi.FieldByName('ID_CERTIFICATO').AsString;
  ...
  case Tipo of
    ...
    'M':begin
        ...
        if FGiustificativi[C,R].DaOre <> 0 then
          Result:=Result + ' (' + FormatDateTime('hh:mm',FGiustificativi[C,R].DaOre) + ')';
        end;
...
begin
  Result:=False;
  LTimb:=FTimbrature[Day,Col];
  if (VarIsNull(LTimb.Ora)) or
     (R180VarToDateTime(LTimb.Ora) <> QTimbrature.FieldByName('Ora').AsDateTime) then
    Result:=True;
  if LTimb.Verso <> QTimbrature.FieldByName('Verso').AsString then
    Result:=True;
  if LTimb.Rilevatore <> QTimbrature.FieldByName('Rilevatore').AsString then
    Result:=True;
  if LTimb.Causale <> QTimbrature.FieldByName('Causale').AsString then
    Result:=True;
end;
...
begin
  ...
  FTimbrature[Day,FNumTimbrature[Day]].Progressivo:=QTimbrature.FieldByName('PROGRESSIVO').AsInteger; //+++
  FTimbrature[Day,FNumTimbrature[Day]].Data:=QTimbrature.FieldByName('DATA').AsDateTime; //+++
  FTimbrature[Day,FNumTimbrature[Day]].Ora:=QTimbrature.FieldByName('Ora').AsDateTime;
  FTimbrature[Day,FNumTimbrature[Day]].Verso:=QTimbrature.FieldByName('Verso').AsString;
  FTimbrature[Day,FNumTimbrature[Day]].Flag:='I';
  FTimbrature[Day,FNumTimbrature[Day]].Rilevatore:=QTimbrature.FieldByName('Rilevatore').AsString;
  FTimbrature[Day,FNumTimbrature[Day]].Causale:=QTimbrature.FieldByName('Causale').AsString;
  Sort(Day);
end;
